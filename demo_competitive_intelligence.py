...
class CompetitiveIntelligenceDemo:
    ...
    def _demo_threat_analysis(self):
        ...
        try:
            ...
            print(
                f"• Competitors monitored: {monitoring_result['competitors_monitored']}"
            )
            ...
            print(
                f"• Market opportunity score: {trends_result['market_opportunity_score']:.2f}"
            )

            # Calculate competitive advantage
            advantage_result = (
                self.intelligence_engine.calculate_competitive_advantage()
            )
            print(f"✅ Competitive advantage calculated")
            print(
                f"• Systematic superiority score: {advantage_result['systematic_superiority_score']:.2f}"
            )
            print(
                f"• Time to market advantage: {advantage_result['time_to_market_advantage']:.2f}"
            )
    ...
    def _demo_response_automation(self):
        ...
        try:
            ...
            print(
                f"• Competitive advantage gain: {strategy.competitive_advantage_gain:.2f}"
            )

            # Show implementation plan
            print(f"• Implementation plan ({len(strategy.implementation_plan)} steps):")
            for i, step in enumerate(
                strategy.implementation_plan[:5], 1
            ):  # Show first 5 steps
                print(f"  {i}. {step}")
            ...
            print(
                f"• Execution time: {execution.completed_at - execution.started_at if execution.completed_at and execution.started_at else 'N/A'}"
            )
            ...
            print(
                f"  - Total strategies generated: {status['total_strategies_generated']}"
            )
            print(f"  - Success rate: {status['success_rate']:.2f}")
            print(
                f"  - Average response time: {status['average_response_time_hours']:.2f} hours"
            )
    ...
    def _demo_intelligence_engine(self):
        ...
        try:
            ...
            differentiation = (
                self.intelligence_engine.generate_differentiation_strategy(
                    competitor_move
                )
            )
            ...
            for factor in differentiation["differentiation_factors"][
                :3
            ]:  # Show first 3
                print(f"  - {factor}")
            if len(differentiation["differentiation_factors"]) > 3:
                print(
                    f"  ... and {len(differentiation['differentiation_factors']) - 3} more factors"
                )
            ...
            print(
                f"  - Systematic superiority: {advantage['systematic_superiority_score']:.2f}"
            )
    ...
    def _demo_integrated_workflow(self):
        ...
        try:
            ...
            if self.intelligence_engine:
                monitoring_result = self.intelligence_engine.monitor_competitors()
                print(
                    f"     - Threats identified: {monitoring_result['threats_identified']}"
                )
                print(
                    f"     - Alerts generated: {monitoring_result['alerts_generated']}"
                )
            ...
            if self.response_automation:
                ...
                print(
                    f"     - Strategies generated: {response_status['total_strategies_generated']}"
                )
