...
class PlanningExhaustionAnalyzer:
    ...
    def __init__(self):
        ...
        self.analysis_id = (
            f"planning_exhaustion_{self.analysis_start_time.strftime('%Y%m%d_%H%M%S')}"
        )
    ...
    def detect_diminishing_returns(
        self, effectiveness_history: List[float], window_size: int = 3
    ) -> bool:
        ...
        previous_avg = (
            sum(effectiveness_history[-window_size * 2 : -window_size]) / window_size
        )
    ...
    def calculate_analysis_results(self) -> Dict:
        ...
        effectiveness_values = [
            cycle["effectiveness"] for cycle in self.planning_cycles
        ]
        ...
        effectiveness_trend = (
            "declining"
            if effectiveness_values[-1] < effectiveness_values[0]
            else "improving"
        )
    ...
    def print_analysis_results(self, results: Dict):
        ...
        print(
            f"   Diminishing Returns: {'DETECTED' if results['diminishing_returns_detected'] else 'NOT DETECTED'}"
        )
        print(f"   Optimal Planning Depth: {results['optimal_planning_depth']} cycles")
        print(
            f"   Peak Effectiveness: {results['peak_effectiveness']} (Cycle {results['peak_cycle']})"
        )
        ...
        if results["diminishing_returns_detected"]:
            ...
            print(
                f"   📊 Peak effectiveness was {results['peak_effectiveness']:.3f} at cycle {results['peak_cycle']}"
            )
            print(
                f"   ⚠️ Additional planning beyond cycle {results['optimal_planning_depth']} reduces effectiveness"
            )
        else:
        ...
        if results["total_planning_time"] > 0:
            insights_per_second = (
                results["total_insights_gained"] / results["total_planning_time"]
            )
            print(
                f"   📊 Planning efficiency: {insights_per_second:.2f} insights/second"
            )
