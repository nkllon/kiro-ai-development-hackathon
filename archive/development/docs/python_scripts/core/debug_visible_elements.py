    ...
    try:
        ...
        for i, button in enumerate(buttons[:10], 1):
            ...
            if is_visible:
                ...
                print(
                    f"   {i}. '{text}' - Visible: {is_visible}, Enabled: {is_enabled}"
                )
                if bounding_box:
                    print(f"      Position: {bounding_box}")
        ...
        all_clickable = target_page.query_selector_all(
                ...
                if any(
                    ...
                    if is_visible and is_enabled:
                        navigation_candidates.append(
                            {"element": element, "text": text, "position": bounding_box}
                        )
            except:
                continue
        ...
        filename = (
            f"debug_visible_{hackathon_id}_{submission_id}_{page_title}_{timestamp}.png"
        )
