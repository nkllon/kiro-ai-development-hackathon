...
@click.option(
    "--spec-path", "-s", default=".", help="Path to spec directory containing tasks.md"
)
@click.pass_context
...
def analyze(ctx, output):
    ...
    if not analyzer.tasks:
        click.echo(
            "❌ No tasks found. Make sure tasks.md exists in the spec directory."
        )
        sys.exit(1)
...
@click.option(
    "--status",
    type=click.Choice(["not_started", "in_progress", "completed", "failed", "blocked"]),
