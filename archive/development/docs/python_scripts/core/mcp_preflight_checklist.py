...
class MCPPreflightChecklist:
    ...
    def __init__(self):
        ...
        self.checklist_id = (
            f"preflight_{self.checklist_start_time.strftime('%Y%m%d_%H%M%S')}"
        )
        self.results = {
    ...
    def check_item(
        self, check_name: str, check_function, critical: bool = True
    ) -> Dict:
        ...
        try:
            ...
            print(
                f"   {status_icon} {critical_icon} {check_result['status']} - {check_result['duration_seconds']}s"
            )
    ...
    def check_applescript_capability(self) -> Dict:
        ...
        try:
            ...
            test_script = (
                'tell application "System Events" to return "AppleScript working"'
            )
            result = subprocess.run(
            ...
            if result.returncode == 0:
                ...
                if chrome_result.returncode == 0:
                    ...
                    return {
                        ...
                        "details": {
                            "applescript": "working",
                            "chrome_error": chrome_result.stderr,
                        },
                    }
            else:
                return {
    ...
    def generate_recommendations(self):
        ...
        if "MCP Images Available" in self.results["critical_failures"]:
            recommendations.append(
                "Pull required MCP Docker images: docker pull mcp/memory mcp/desktop-commander pingcap/tidb"
            )
