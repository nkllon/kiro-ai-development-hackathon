...
class DiscoveryEmergencyInterface:
    ...
    def trigger_emergency_protocols(
        self,
        ...
        if not self.is_initialized:
            print(
                "❌ Discovery emergency protocols not initialized. Call initialize_for_discovery_session() first."
            )
            return None
        ...
        try:
            result = self.discovery_emergency.manual_emergency_trigger(
            ...
            if result["success"]:
                ...
                print(
                    f"❌ Failed to activate emergency protocols: {result.get('error')}"
                )
                return result
    ...
    def execute_action(self, action_id: str, additional_params: Dict[str, Any] = None):
        ...
        if not self.is_initialized or not self.discovery_emergency.is_active:
            print(
                "❌ Emergency protocols not active. Call trigger_emergency_protocols() first."
            )
            return None

        try:
            result = self.discovery_emergency.execute_discovery_action(
                action_id, additional_params
            )
            ...
            if result["success"]:
                ...
                print(
                    f"❌ Failed to execute action '{action_id}': {result.get('error')}"
                )
                return result
...
def quick_emergency_trigger(
    session_data: Dict[str, Any], reason: str = "User detected changes"
):
