...
class DevelopmentChecklist:
    ...
    def get_incomplete_by_category(self, category: str) -> List[ChecklistItem]:
        ...
        return [
            item
            for item in self.items
            if item.category == category and not item.completed
        ]
    ...
    def get_completion_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "critical_completion_rate": (
                (completed_critical / len(critical_items)) * 100
                if critical_items
                else 0
            ),
        }
    ...
    def generate_report(self) -> str:
        ...
        report.append(
            f"   Critical items: {status['completed_critical']}/{status['critical_items']}"
        )
        report.append(
            f"   Critical completion: {status['critical_completion_rate']:.1f}%"
        )
        ...
        for category in sorted(categories):
            ...
            report.append(
                f"   {status_icon} {category}: {completed}/{total} ({rate:.1f}%)"
            )
        ...
        # Overall result
        if (
            status["critical_completion_rate"] == 100
            and status["completion_rate"] >= 90
        ):
    ...
    if len(sys.argv) > 1:
        ...
        if command == "status":
            ...
            if len(sys.argv) > 2:
                ...
                print(
                    "Usage: python development_checklist.py complete <item_id> [notes]"
                )
            ...
            if len(sys.argv) > 2:
                ...
                print(
                    "Usage: python development_checklist.py incomplete <item_id> [notes]"
                )
        else:
