            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.ClassDef):
                    ...
                    if self._is_interface_class(node, content):
                        interface_info = self._extract_interface_info(
                            node, file_path, content
                        )

                        # Validate interface creation
                        is_valid, reason, suggestions = (
                            self.registry.validate_interface_creation(
                                interface_info["name"],
                                interface_info["type"],
                                file_path,
                        ...
                        duplicate_patterns = self._check_duplicate_patterns(
                            node, content
                        )
                            ...
                            issues.append(
                                {
                                    ...
                                    "suggestions": [
                                        "Use existing interface from registry",
