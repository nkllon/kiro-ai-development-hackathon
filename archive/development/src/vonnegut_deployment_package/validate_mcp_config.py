            ...
            for env_name, env_value in env_vars.items():
                ...
                if (
                    ...
                    if not os.getenv(env_var_name):
                        ...
                        print(
                            f"   ⚠️ {server_name}.{env_name}: Environment variable {env_var_name} not set"
                        )
        ...
        for server_name, server_config in servers.items():
                ...
                for arg in server_config["args"]:
                    if isinstance(arg, str) and self.looks_like_file_path(arg):
                        if not self.is_absolute_path(
                            arg
                        ) and not self.file_exists_relative(arg):
                            path_issues.append(
                                {
                                    "server": server_name,
                                    "path": arg,
            ...
            if "command" in server_config:
                ...
                if command not in ["node", "docker", "uvx", "python", "python3"]:
                    ...
                    if self.looks_like_file_path(
                        command
                    ) and not self.file_exists_relative(command):
                        path_issues.append(
                            {
                                "server": server_name,
                                "path": command,
            ...
            if env_issue["issue"] == "placeholder_value":
                ...
                if server_name in config["mcpServers"]:
                    if "env" in config["mcpServers"][server_name]:
                        ...
                        print(
                            f"   ✅ Fixed {server_name}.{variable_name}: {env_issue['value']} → {suggested_value}"
                        )
    ...
    def generate_validation_report(self):
        ...
        if self.fixes_applied:
            ...
            for fix in self.fixes_applied:
                print(
                    f"   🔧 {fix['type']}: {fix.get('old_value', 'N/A')} → {fix.get('new_value', 'N/A')}"
                )
