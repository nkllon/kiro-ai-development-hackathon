...
async def simulate_events(server: WarRoomServer):
    ...
    event_types = [
        ...
        (
            "hubris_prevention",
            "Emergency bypass attempt blocked",
            ...
            await server.update_metrics(
                {
                    ...
                    "deployment_status": random.choice(
                        ["stable", "deploying", "rolling_back"]
                    ),
                }
            )
...
async def main():
    ...
    try:
        ...
        await asyncio.gather(server.start_server(), simulate_events(server))
