...
class BeastModeInterfaceGovernance:
        ...
        try:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.ClassDef):
                    ...
                    if any(
                        base.id == "ReflectiveModule"
                        for base in node.bases
                        if hasattr(base, "id")
                    ):
                        # Validate ReflectiveModule implementation
                        violations.extend(
                            self._validate_reflective_module(file_path, node, lines)
                        )

                    # Check for other interface patterns
                    violations.extend(
                        self._validate_interface_patterns(file_path, node, lines)
                    )
    ...
    def _validate_reflective_module(
        self, file_path: Path, class_node: ast.ClassDef, lines: List[str]
    ...
    def _validate_interface_patterns(
        self, file_path: Path, class_node: ast.ClassDef, lines: List[str]
    ) -> List[InterfaceViolation]:
        ...
        if "Manager" in class_name and not any(
            base.id == "ReflectiveModule"
            for base in class_node.bases
            if hasattr(base, "id")
        ):
