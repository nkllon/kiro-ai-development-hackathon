...
**The Requirements ARE the Solution - and we have the evidence to prove it!**""",
    ...
    print("📝 Filling title...")
    result = fill_form_field(
        'input[name="participants_submission_requirements[submission_field_values_attributes][1][value]"]',
        project_data["title"],
    )
    if result is None:
    ...
    result = fill_form_field(
        'textarea[name="participants_submission_requirements[submission_field_values_attributes][5][value]"]',
    ...
    result = fill_form_field(
        'input[name="participants_submission_requirements[submission_field_values_attributes][7][value]"]',
    ...
    result = fill_form_field(
        'input[name="participants_submission_requirements[submission_field_values_attributes][13][value]"]',
        project_data["demo_video"],
    )
    if result is None:
    ...
    result = run_applescript(
        'tell application "Google Chrome" to tell active tab of front window to execute javascript "document.title = \\"Form Filled - \\" + document.title;"'
    )
