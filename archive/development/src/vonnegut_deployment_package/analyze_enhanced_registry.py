...
def load_enhanced_registry(
    registry_file: str = ".beast_mode/enhanced_interface_registry.json",
) -> Dict[str, Any]:
...
def analyze_method_signatures(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if method_stats["total_methods"] > 0:
        method_stats["average_parameters_per_method"] = round(
            total_params / method_stats["total_methods"], 2
        )
...
def analyze_file_locations(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if location_stats["total_interfaces"] > 0:
        location_stats["average_lines_per_interface"] = round(
            total_lines / location_stats["total_interfaces"], 2
        )
...
def analyze_compliance_scores(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for interface_id, interface_data in data.get("interfaces", {}).items():
        ...
        compliance_stats["compliance_distribution"][
            f"{score_range}-{score_range+9}"
        ] += 1
...
def analyze_interface_types(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for interface_type, scores in type_stats["compliance_by_type"].items():
        if scores:
            type_stats["compliance_by_type"][interface_type] = round(
...
def generate_comprehensive_report(data: Dict[str, Any]) -> None:
    ...
    print(
        f"   Methods with docstrings: {method_stats['methods_with_docstrings']} ({method_stats['methods_with_docstrings']/method_stats['total_methods']*100:.1f}%)"
        if method_stats["total_methods"] > 0
        else "   Methods with docstrings: 0"
    )
    print(
        f"   Methods with type annotations: {method_stats['methods_with_type_annotations']} ({method_stats['methods_with_type_annotations']/method_stats['total_methods']*100:.1f}%)"
        if method_stats["total_methods"] > 0
        else "   Methods with type annotations: 0"
    )
    print(f"   Abstract methods: {method_stats['abstract_methods']}")
    print(f"   Decorated methods: {method_stats['decorated_methods']}")
    print(
        f"   Average parameters per method: {method_stats['average_parameters_per_method']}"
    )
    ...
    print(
        f"   Total ubiquitous language terms: {domain_stats['total_ubiquitous_terms']}"
    )
    ...
    print(
        f"   Interfaces with precise locations: {location_stats['interfaces_with_precise_locations']} ({location_stats['interfaces_with_precise_locations']/location_stats['total_interfaces']*100:.1f}%)"
        if location_stats["total_interfaces"] > 0
        else "   Interfaces with precise locations: 0"
    )
    print(
        f"   Average lines per interface: {location_stats['average_lines_per_interface']}"
    )
    ...
    for interface_type, count in type_stats["interfaces_by_type"].most_common():
        ...
        avg_methods = (
            type_stats["methods_by_type"][interface_type] / count if count > 0 else 0
        )
        domain_coverage = len(type_stats["domain_coverage_by_type"][interface_type])
        print(
            f"     {interface_type}: {count} interfaces, {avg_compliance}% avg compliance, {avg_methods:.1f} avg methods, {domain_coverage} domain terms"
        )
...
def main():
    ...
    if data:
        ...
        print(
            "❌ No enhanced registry data available. Run enhanced_interface_registry.py first."
        )
