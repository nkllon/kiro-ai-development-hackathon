...
class BillingAnalysisAgents:
            ...
            analysis_results.append(
                {
                    ...
                    "recommendation": (
                        "Consider optimization" if cost > 300 else "Cost is reasonable"
                    ),
                }
            )
    ...
    def _reporter_agent(self, state: BillingAgentState) -> BillingAgentState:
        ...
        if state["cost_optimizations"]:
            total_savings = sum(
                opt["potential_savings"] for opt in state["cost_optimizations"]
            )
            recommendations.append(
                f"Implement optimizations to save ${total_savings:.2f}"
            )
