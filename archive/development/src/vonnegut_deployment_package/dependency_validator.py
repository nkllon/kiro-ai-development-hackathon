...
class DependencyChecker:
    ...
    CORE_DEPENDENCIES = ["jinja2", "pydantic", "yaml", "click", "aiohttp", "watchdog"]

    # Optional dependencies that may not be available
    OPTIONAL_DEPENDENCIES = ["openai", "anthropic", "langgraph", "keyring"]
    ...
    def validate_core_dependencies(self) -> DependencyStatus:
        ...
        for dep in self.CORE_DEPENDENCIES:
            try:
                ...
                validation_details[dep] = {"status": "missing", "error": str(e)}
    ...
    def generate_installation_commands(self, missing_deps: List[str]) -> List[str]:
        ...
        optional_missing = [
            dep for dep in missing_deps if dep in self.OPTIONAL_DEPENDENCIES
        ]
    ...
    def print_validation_report(
        self, status: DependencyStatus, dependency_type: str = "core"
    ):
...
def main():
    ...
    if core_status.all_available:
        ...
        print(
            "⚠️  Some core dependencies are missing. Please install them before running tests."
        )
        return 1
