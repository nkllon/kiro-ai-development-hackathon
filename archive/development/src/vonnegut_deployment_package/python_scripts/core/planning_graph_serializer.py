...
class PlanningEdge:
    ...
    target_id: str
    relationship_type: (
        str  # depends_on, mitigates, constrains, influences, conflicts_with
    )
    strength: float = 1.0
...
class PlanningGraphSerializer:
    ...
    def export_graph(self, output_file: str):
        ...
        graph_data = {
            ...
            "graph_metadata": {
                ...
                "relationship_types": list(
                    set(edge.relationship_type for edge in self.edges)
                ),
        ...
        print(
            f"   Relationship types: {graph_data['graph_metadata']['relationship_types']}"
        )
        ...
        for edge in self.edges:
            if self.graph.has_edge(edge.source_id, edge.target_id):
                ...
                for graph_edge in self.graph.edges:
                    if (
                        graph_edge["source"] == edge.source_id
                        and graph_edge["target"] == edge.target_id
                    ):
...
class PlanningGraphLoader:
    ...
    def _load_json_graph(self):
        ...
        for edge_data in data["edges"]:
            ...
            self.graph.add_edge(
                edge_data["source_id"], edge_data["target_id"], **edge_data
            )
    ...
    def find_nodes_by_type(self, node_type: str) -> List[Dict[str, Any]]:
        ...
        return [
            node for node in self.nodes.values() if node.get("node_type") == node_type
        ]

    def find_nodes_by_risk_level(self, risk_level: str) -> List[Dict[str, Any]]:
        """Find all nodes with a specific risk level"""
        return [
            node for node in self.nodes.values() if node.get("risk_level") == risk_level
        ]
...
def main():
    ...
    planning_memory_file = (
        ".planning_memory/planning_memory_planning_20250914_200132.json"
    )
