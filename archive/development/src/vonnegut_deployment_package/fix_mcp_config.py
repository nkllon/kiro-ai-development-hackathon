...
class MCPConfigFixer:
    ...
    def fix_environment_variables(self, config):
        ...
        if "voice-mode" in config.get("mcpServers", {}):
            ...
            if "env" in voice_config and "OPENAI_API_KEY" in voice_config["env"]:
                ...
                print(
                    f"      ✅ Fixed voice-mode.OPENAI_API_KEY: {old_value} → {new_value}"
                )
        ...
        if "github" in config.get("mcpServers", {}):
            ...
            if (
                "env" in github_config
                and "GITHUB_PERSONAL_ACCESS_TOKEN" in github_config["env"]
            ):
                ...
                print(
                    f"      ✅ Fixed github.GITHUB_PERSONAL_ACCESS_TOKEN: {old_value} → {new_value}"
                )
    ...
    def is_uvx_available(self):
        ...
        try:
            result = subprocess.run(
                ["uvx", "--version"], capture_output=True, text=True, timeout=5
            )
            return result.returncode == 0
        except:
            return False
