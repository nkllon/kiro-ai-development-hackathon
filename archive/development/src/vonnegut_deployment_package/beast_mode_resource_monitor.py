...
try:
    from beast_mode.billing import (
        GCPBillingMonitor,
...
class BeastModeResourceMonitor:
    ...
    def __init__(self):
        ...
        if BEAST_MODE_BILLING_AVAILABLE and self.config.get("gcp", {}).get(
            "enabled", False
        ):
            try:
    ...
    async def _collect_network_metrics(self):
        ...
        bandwidth_mbps = ((bytes_sent_delta + bytes_recv_delta) * 8) / (
    ...
    async def _collect_financial_metrics(self):
                    ...
                    try:
                        ...
                        if usage_time > cutoff_time:
                            ...
                            cost_by_provider[provider] = (
                                cost_by_provider.get(provider, 0) + cost
                            )
        ...
        if session_hours < 0.1:  # Less than 6 minutes
            ...
            if self.current_metrics["gcp_billing"]:
                hourly_burn_rate = (
                    self.current_metrics["gcp_billing"].daily_cost_usd / 24
                )
            else:
                hourly_burn_rate = (
                    total_cost * 10
                )  # Conservative 10x projection for short sessions
        else:
        ...
        if BEAST_MODE_BILLING_AVAILABLE:
            ...
            unified_metrics = UnifiedFinancialMetrics(
                ...
                cost_by_category={"llm": llm_total_cost, "gcp": gcp_cost, "other": 0.0},
    ...
    async def _display_dashboard(self):
        ...
        if unified:
            ...
            status_name = (
                unified.budget_status.name
                if hasattr(unified.budget_status, "name")
                else str(unified.budget_status)
            )
        ...
        if gcp_billing:
            ...
            if gcp_billing.usage_metrics:
                ...
                if "avg_request_duration_ms" in usage:
                    print(
                        f"   ⏱️  Avg Duration: {usage['avg_request_duration_ms']:.1f}ms"
                    )
                ...
                if "cost_per_request" in usage and "cost_per_cpu_second" in usage:
                    ...
                    if requests > 0 and cpu_seconds > 0:
                        ...
                        print(
                            f"   📊 CPU Efficiency: {cpu_efficiency:.3f} CPU-sec/request"
                        )
                        ...
                        if usage.get("cold_starts", 0) / requests > 0.1:
                            print(
                                f"   💡 Optimization: High cold start ratio ({usage.get('cold_starts', 0)/requests:.1%})"
                            )
                    print()
        ...
        # Token Usage (recent)
        recent_tokens = (
            self.current_metrics["tokens"][-5:]
            if self.current_metrics["tokens"]
            else []
        )
        if recent_tokens:
            ...
            for usage in recent_tokens:
                print(
                    f"   {usage.model}: {usage.total_tokens:,} tokens (${usage.cost_usd:.4f})"
                )
            print()
        ...
        if resources:
            ...
            print(
                f"   💾 Memory: {resources.memory_percent:.1f}% ({resources.memory_used_gb:.1f}GB/{resources.memory_total_gb:.1f}GB)"
            )
            print(
                f"   💿 Disk: {resources.disk_usage_percent:.1f}% ({resources.disk_free_gb:.1f}GB free)"
            )

            if resources.gpu_usage_percent is not None:
                print(
                    f"   🎮 GPU: {resources.gpu_usage_percent:.1f}% ({resources.gpu_memory_used_gb:.1f}GB)"
                )
            print()
    ...
    async def _check_alerts(self):
        ...
        if unified:
            if unified.hourly_burn_rate > thresholds["hourly_burn_rate"]:
                alerts.append(
                    f"🚨 High burn rate: ${unified.hourly_burn_rate:.2f}/hour"
                )

            if unified.budget_remaining_usd < 0:
                alerts.append(
                    f"🚨 Over budget by ${abs(unified.budget_remaining_usd):.2f}"
                )
            ...
            if gcp_cost > gcp_limit:
                alerts.append(
                    f"🚨 GCP costs exceed daily limit: ${gcp_cost:.2f} > ${gcp_limit:.2f}"
                )

        elif financial:
            if financial.hourly_burn_rate > thresholds["hourly_burn_rate"]:
                alerts.append(
                    f"🚨 High burn rate: ${financial.hourly_burn_rate:.2f}/hour"
                )

            if financial.budget_remaining < 0:
                alerts.append(
                    f"🚨 Over budget by ${abs(financial.budget_remaining):.2f}"
                )
    ...
    def log_token_usage(
        ...
        cost_usd = (input_tokens * pricing["input"] / 1000) + (
    ...
    async def _save_session_summary(self):
        ...
        summary = {
            ...
            "final_metrics": {
                "financial": (
                    asdict(self.current_metrics["financial"])
                    if self.current_metrics["financial"]
                    else None
                ),
                "network": (
                    asdict(self.current_metrics["network"])
                    if self.current_metrics["network"]
                    else None
                ),
                "resources": (
                    asdict(self.current_metrics["resources"])
                    if self.current_metrics["resources"]
                    else None
                ),
            },
        }
