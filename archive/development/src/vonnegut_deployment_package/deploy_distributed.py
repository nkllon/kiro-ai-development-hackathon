...
def create_node_deployment_script(
    node_config: Dict, deployment_manifest: Dict, output_dir: str
):
...
def create_kubernetes_manifests(deployment_manifest: Dict, output_dir: str):
    ...
    redis_manifest = {
        "apiVersion": "apps/v1",
        "kind": "Deployment",
        "metadata": {"name": "beast-mode-redis", "labels": {"app": "beast-mode-redis"}},
        "spec": {
            "replicas": 1,
    ...
    agent_manifest = {
        "apiVersion": "apps/v1",
        "kind": "Deployment",
        "metadata": {"name": "beast-mode-agent", "labels": {"app": "beast-mode-agent"}},
        "spec": {
...
def deploy_distributed(
    ...
    try:
        ...
        deployment_id = deployment_manager.create_distributed_deployment(
            environment, nodes
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Create Beast Mode distributed deployment"
    )
    parser.add_argument(
        "--environment",
        default="distributed",
    ...
    parser.add_argument(
        "--nodes", nargs="+", required=True, help="List of target nodes"
    )
    parser.add_argument("--node-config", help="Node configuration file (JSON/YAML)")
    parser.add_argument(
        "--output-dir",
    ...
    parser.add_argument(
        "--kubernetes", action="store_true", help="Also create Kubernetes manifests"
    )
    parser.add_argument(
        "--log-level",
        default="INFO",
