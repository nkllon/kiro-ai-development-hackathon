...
class AnalysisController:
    ...
    def find_analysis_processes(self) -> List:
        ...
        for proc in psutil.process_iter(["pid", "name", "cmdline"]):
            try:
                ...
                if (
                    "rm_rdi" in cmdline.lower()
                    or "analysis" in proc.info["name"].lower()
                ):
    ...
    def kill_analysis(self) -> Dict[str, Any]:
        ...
        if result["success"]:
            ...
            print(
                f"⚠️  {len(remaining)} processes still running - may need manual intervention"
            )
    ...
    def throttle_analysis(self) -> Dict[str, Any]:
        ...
        if result["success"]:
            print(
                "✅ THROTTLING COMPLETE - Analysis processes running at reduced priority"
            )
        else:
    ...
    def stop_analysis(self) -> Dict[str, Any]:
        ...
        if result["success"]:
            ...
            print(
                f"⚠️  {len(final_remaining)} processes still running - manual intervention needed"
            )
    ...
    def status_analysis(self) -> Dict[str, Any]:
        ...
        for proc in processes:
            try:
                ...
                process_info.append({"pid": proc.pid, "error": str(e)})
