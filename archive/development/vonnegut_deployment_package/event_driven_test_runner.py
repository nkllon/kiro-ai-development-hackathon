...
class TestEventHandler(FileSystemEventHandler):
    ...
    def _handle_file_change(self, file_path: str):
        ...
        if file_path in self.last_event_time:
            if (
                now - self.last_event_time[file_path]
            ).total_seconds() < self.debounce_delay:
                return
...
class SystematicTestRunner:
    ...
    async def _run_single_test_file(self, test_file: Path):
        ...
        try:
            ...
            if result.returncode == 0:
                logger.info(
                    f"✅ Tests passed: {test_file.name} ({duration.total_seconds():.2f}s)"
                )
                self._log_test_success(test_file, duration, result.stdout)
            else:
                logger.error(
                    f"❌ Tests failed: {test_file.name} ({duration.total_seconds():.2f}s)"
                )
                self._log_test_failure(
                    test_file, duration, result.stdout, result.stderr
                )
    ...
    def _log_test_success(self, test_file: Path, duration, stdout: str):
        ...
        lines = stdout.split("\n")
        summary_line = next(
            (line for line in lines if "passed" in line and "=" in line), ""
        )
    ...
    async def run_all_tests(self):
        ...
        test_dirs = [Path("tests/unit"), Path("tests/integration"), Path("tests")]
    ...
    def print_test_summary(self):
        ...
        if recent_tests:
            avg_duration = sum(t.get("duration", 0) for t in recent_tests) / len(
                recent_tests
            )
