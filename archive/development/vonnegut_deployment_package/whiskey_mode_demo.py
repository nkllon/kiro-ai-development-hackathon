...
async def simulate_test_data(display: WhiskeyModeDisplay):
    ...
    display.update_test_results(
        {"total": 88, "passed": 84, "failed": 2, "skipped": 2, "duration": 12.3}
    )
    ...
    for i in range(20):
        ...
        display.update_system_health(
            {"cpu": cpu, "memory": memory, "test_velocity": test_velocity}
        )
...
async def main():
    ...
    try:
        await asyncio.gather(display.start_display(), simulate_test_data(display))
