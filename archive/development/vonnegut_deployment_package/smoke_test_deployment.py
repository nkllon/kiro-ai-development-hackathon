...
class SmokeTestRunner:
    ...
    def test_deployment_status(self) -> bool:
        ...
        if not self.deployment_id:
            self.logger.info(
                "No deployment ID provided, skipping deployment status test"
            )
            return True
        ...
        try:
            ...
            if status.status not in ["running", "starting"]:
                self.logger.error(
                    f"Deployment status is {status.status}, expected running or starting"
                )
                return False
    ...
    def test_deployment_health(self) -> bool:
        ...
        if not self.deployment_id:
            self.logger.info(
                "No deployment ID provided, skipping deployment health test"
            )
            return True
    ...
    def test_file_permissions(self) -> bool:
        ...
        try:
            ...
            # Test directories
            directories = [config.agent.spore_directory, "logs", "config"]
    ...
    def test_log_file_creation(self) -> bool:
        ...
        try:
            ...
            test_message = (
                f"Smoke test log entry - {time.strftime('%Y-%m-%d %H:%M:%S')}"
            )
    ...
    def test_network_connectivity(self) -> bool:
        ...
        try:
            ...
            if result != 0:
                self.logger.error(
                    f"Cannot connect to Redis at {config.redis.host}:{config.redis.port}"
                )
                return False
            ...
            if config.redis.host not in ["localhost", "127.0.0.1"]:
                try:
                    ...
                    self.logger.error(
                        f"DNS resolution failed for {config.redis.host}: {e}"
                    )
                    return False
    ...
    def test_deployment_validation(self) -> bool:
        ...
        try:
            ...
            if not report.overall_passed:
                self.logger.warning(
                    f"Deployment validation failed: {report.failed_checks}/{report.total_checks} checks failed"
                )
            ...
            self.logger.info(
                f"Deployment validation passed: {report.passed_checks}/{report.total_checks} checks"
            )
            return True
    ...
    def run_all_tests(self) -> Dict[str, Any]:
        ...
        self.results["duration_ms"] = (
            time.time()
            - time.mktime(
                time.strptime(self.results["started_at"], "%Y-%m-%d %H:%M:%S")
            )
        ) * 1000
        ...
        self.logger.info(
            f"Smoke tests completed: {summary['passed']}/{summary['total']} passed, {summary['failed']} failed"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run Beast Mode deployment smoke tests"
    )
    parser.add_argument(
        "--environment",
        required=True,
    ...
    parser.add_argument("--deployment-id", help="Deployment ID to test (optional)")
    parser.add_argument(
        "--output",
        default="smoke_test_results.json",
        help="Output file for test results (default: smoke_test_results.json)",
    )
    parser.add_argument("--html-report", help="Generate HTML report (provide filename)")
    parser.add_argument(
        "--log-level",
        default="INFO",
