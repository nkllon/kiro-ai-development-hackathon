...
def test_low_confidence_scenario():
    ...
    multi_dimensional_analyzer = MultiDimensionalContextAnalyzer(telemetry_graph, state_model)
    ...
    print(f"   Multi-dimensional confidence: {multi_dimensional_analysis.overall_confidence:.2f}")
    ...
    if (multi_dimensional_analysis.overall_confidence < AUTONOMOUS_NAVIGATION_THRESHOLD and 
        multi_dimensional_analysis.overall_confidence >= EXPLORATORY_THRESHOLD):
        print("   ⚠️ CAUTIOUS MODE TRIGGERED!")
        print("   ✅ Confidence below autonomous threshold - system proceeds with caution")
    else:
...
def test_interactive_recovery_options():
    ...
    test_choices = ["1", "2", "3", "4", "5", "tell me where we are", "guide me", "start fresh"]
...
def test_tiered_memory_management():
    ...
    print(f"   Retrieved current page: {current_page['title'] if current_page else 'None'}")
    print(f"   Retrieved user actions: {len(user_actions) if user_actions else 0} actions")
    ...
    print(f"   Memory qualification queue: {len(memory_manager.memory_qualification_queue)} items")
    
    # Test memory qualification
    memory_manager.qualify_memory("session_telemetry", "persist", "This session data is valuable")
    memory_manager.qualify_memory("navigation_patterns", "discard", "Patterns are too generic")
    
    print(f"   Long-term memory items: {len(memory_manager.long_term_memory)}")
    print(f"   Qualified items: {len([item for item in memory_manager.memory_qualification_queue if 'qualification_decision' in item])}")
...
def test_session_stop_and_recovery():
    ...
    print(f"   Ghostbusters mode detected: {mock_ghostbusters_state.get('ghostbusters_mode', False)}")
    print(f"   User input required: {mock_ghostbusters_state.get('user_input_required', False)}")
    print(f"   Awaiting recovery choice: {mock_ghostbusters_state.get('awaiting_recovery_choice', False)}")
    ...
    session_save_data = {
        ...
        "multi_dimensional_analysis": {"overall_confidence": 0.15, "primary_strategy": "exploratory"}
    }
...
def test_memory_qualification():
    ...
    qualification_items = [
        {
            ...
            "data": {"click_sequence": ["login", "form", "submit"], "time_spent": [2, 5, 1]},
            "reason": "User behavior data that could improve future navigation"
        },
        {
            "key": "error_patterns", 
            "data": {"common_errors": ["timeout", "validation"], "recovery_methods": ["retry", "manual_fix"]},
            "reason": "Error patterns that could help with automatic recovery"
        },
        {
            "key": "page_similarity_data",
            "data": {"visual_hashes": ["abc123", "def456"], "url_patterns": ["devpost.com/*"]},
    ...
    print(f"   Qualification queue items: {len(memory_manager.memory_qualification_queue)}")
    ...
    qualification_decisions = [
        ...
        ("error_patterns", "transform", "Keep error types but remove sensitive recovery methods"),
    ...
    qualified_count = len([item for item in memory_manager.memory_qualification_queue if "qualification_decision" in item])
...
def demonstrate_ghostbusters_workflow():
    ...
    print("🎯 This system prevents autonomous navigation when confidence is critically low")
