...
def mvp_route(spec_directory: str, timeline: int, output: str):
    ...
    try:
        ...
        critical_specs = []
        for spec_dir in specs:
            if any(keyword in spec_dir.name.lower() for keyword in ['framework', 'core', 'engine', 'orchestration']):
                critical_specs.append(spec_dir.name)
        
        # ESTIMATE PHASES
        phases = [
            {"phase": 1, "name": "Foundation", "duration_weeks": timeline // 3, "specs": critical_specs[:2]},
            {"phase": 2, "name": "Integration", "duration_weeks": timeline // 3, "specs": critical_specs[2:4]},
            {"phase": 3, "name": "Validation", "duration_weeks": timeline // 3, "specs": ["testing", "cli"]}
        ]
...
@click.option('--dry-run', is_flag=True, help='Simulate orchestration without execution')
