...
def test_real_world_registry_scenarios():
    ...
    try:
        ...
        print(f"   Can Perform Field Modifications: {health_report.can_perform_field_modifications}")
        ...
        print(f"Git Registry: {'✅' if git_status.is_available else '❌'} ({git_status.health_score:.1%})")
        ...
        print(f"Memory Registry: {'✅' if memory_status.is_available else '❌'} ({memory_status.health_score:.1%})")
        
        # File system registry checker
        fs_checker = FileSystemRegistryChecker(current_dir)
        fs_status = fs_checker.check_availability()
        print(f"File System Registry: {'✅' if fs_status.is_available else '❌'} ({fs_status.health_score:.1%})")
...
def test_graceful_failure_scenarios():
    ...
    try:
        ...
        if results['graceful_shutdown_required']:
            ...
            failed_registries = [
                name for name, status_info in results['critical_registries'].items()
...
def main():
    ...
    try:
        ...
        if real_world_results:
            ...
            print(f"   Boot Time Safe: {'✅' if real_world_results['boot_time_safe'] else '❌'}")
            print(f"   Pre-Use Safe: {'✅' if real_world_results['pre_use_safe'] else '❌'}")
            print(f"   Overall Health: {real_world_results['overall_health']:.1%}")
            print(f"   Git Available: {'✅' if real_world_results['git_available'] else '❌'}")
            print(f"   Memory Available: {'✅' if real_world_results['memory_available'] else '❌'}")
            print(f"   File System Available: {'✅' if real_world_results['filesystem_available'] else '❌'}")
        
        print(f"\nGraceful Failure Scenarios:")
        print(f"   Graceful Shutdown Required: {'🚨' if graceful_failure else '✅'}")
        
        print(f"\nCritical Derived Requirement:")
        print(f"   Requirement Demonstrated: {'✅' if requirement_demonstrated else '❌'}")
        
        # Overall assessment
        if real_world_results and real_world_results['boot_time_safe'] and real_world_results['pre_use_safe']:
