...
class BeastModeTestRunner:
    ...
    def run_test_file(self, test_file, description):
        ...
        if not Path(test_file).exists():
            ...
            self.test_results[description] = {"status": "MISSING", "details": "Test file not found"}
            return False
        ...
        try:
            ...
            self.test_results[description] = {"status": "TIMEOUT", "details": "Test execution timed out"}
            return False
    ...
    def run_direct_test(self, test_script, description):
        ...
        if not Path(test_script).exists():
            ...
            self.test_results[description] = {"status": "MISSING", "details": "Test script not found"}
            return False
        ...
        try:
            ...
            self.log_error(f"{description} - TIMEOUT")
            self.test_results[description] = {"status": "TIMEOUT", "details": "Validation timed out"}
            return False
        except Exception as e:
            self.log_error(f"{description} - ERROR: {e}")
            self.test_results[description] = {"status": "ERROR", "details": str(e)}
    ...
    def run_all_tests(self):
        ...
        for i, test_config in enumerate(test_suite, 1):
            ...
            if test_config["type"] == "pytest":
                success = self.run_test_file(test_config["file"], test_config["description"])
            else:
                success = self.run_direct_test(test_config["file"], test_config["description"])
    ...
    def generate_summary_report(self, passed_suites, total_suites):
        ...
        for component, tests in [("GCP Billing", gcp_tests), ("PDCA Orchestrator", pdca_tests), ("GKE Infrastructure", gke_tests)]:
            if tests:
                passed = sum(1 for t in tests if self.test_results[t]["status"] == "PASSED")
                total = len(tests)
        ...
        print(f"\n🚀 Next Steps:")
        if overall_success:
            print("   1. Deploy GKE infrastructure: cd deployment/gke && PROJECT_ID=your-project ./deploy-gke.sh")
