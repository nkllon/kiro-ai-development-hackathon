...
def main():
    ...
    try:
        ...
        print(
            f"   • Entropy score: {entropy_analysis['entropy_metrics']['entropy_score']:.2f}"
        )
        print(
            f"   • Organization score: {entropy_analysis['entropy_metrics']['organization_score']:.2f}"
        )
        print(
            f"   • Systematic compliance: {entropy_analysis['entropy_metrics']['systematic_compliance']:.2f}"
        )
        ...
        for violation in entropy_analysis["systematic_violations"][:5]:  # Show top 5
            print(
                f"   • {violation['file']}: {violation['violation_type']} ({violation['priority']})"
            )
        ...
        dry_run_results = cleanup_engine.execute_systematic_cleanup(
        ...
        print(
            f"   • Success rate: {(dry_run_results['actions_successful']/dry_run_results['actions_executed']*100):.1f}%"
        )
        ...
        if response in ["y", "yes"]:
            ...
            execution_results = cleanup_engine.execute_systematic_cleanup(
            ...
            print(
                f"   • Success rate: {(execution_results['actions_successful']/execution_results['actions_executed']*100):.1f}%"
            )
            ...
            results_file = (
                ...
                / f"cleanup_execution_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            )
        ...
        if beast_org_score >= 8.0:
            assessment = (
                "🏆 ORGANIZATIONAL EXCELLENCE: Systematic structure maintained!"
            )
            ...
            assessment = (
                "🚨 ORGANIZATIONAL CRISIS: Immediate systematic intervention needed"
            )
        ...
        print(
            f"   • Systematic cleanup plan created with {len(cleanup_plan.cleanup_actions)} actions"
        )
        print(
            f"   • Expected entropy reduction: {cleanup_plan.entropy_reduction_score:.0%}"
        )
        ...
        print(
            f"   'Everyone wins when organizational structure supports systematic work'"
        )
