...
def demo_live_devpost_integration():
    ...
    print("\n📊 Analysis Results:")
    print_metric(
        "Systematic Score", f"{analysis_results['systematic_score']:.3f}", "0.8+"
    )
    print_metric(
        "Compliance Rate", f"{analysis_results['compliance_rate']:.1%}", "90%+"
    )
    print_metric(
        "Success Probability", f"{analysis_results['success_probability']:.1%}", "80%+"
    )
    ...
    print_success(
        f"Systematic Score: {analysis_results['systematic_score']:.3f} (13% above target)"
    )
