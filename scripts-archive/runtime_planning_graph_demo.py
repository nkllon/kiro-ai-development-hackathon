...
class RuntimePlanningAnalyzer:
        ...
        for dimension in dimensions:
            ...
            if mitigations:
                ...
                risk_analysis["unmitigated_risks"].append(
                    {"dimension": dimension["title"], "risk_level": risk_level}
                )
    ...
    def assess_planning_health(self) -> Dict[str, Any]:
        ...
        mitigated_dimensions = 0
        for dimension in dimensions:
            mitigations = self.loader.get_related_nodes(
                dimension["id"], "has_mitigation"
            )
            if mitigations:
        ...
        if total_dimensions > 0:
            health_score += (
                mitigated_dimensions / total_dimensions
            ) * 40  # 40% for mitigation coverage
            health_score += (
                min(len(unknown_factors) / (total_dimensions * 2), 1.0) * 30
            )  # 30% for unknown exploration
            health_score += (
                min(len(constraints) / (total_dimensions * 2), 1.0) * 30
            )  # 30% for constraint identification

        return {
            "overall_score": health_score,
            "mitigation_coverage": (
                mitigated_dimensions / total_dimensions if total_dimensions > 0 else 0
            ),
...
def main():
    ...
    for risk_level, coverage in risk_analysis["mitigation_coverage"].items():
        coverage_pct = (
            (coverage["mitigated_dimensions"] / coverage["total_dimensions"] * 100)
            if coverage["total_dimensions"] > 0
            else 0
        )
        print(
            f"   {risk_level.upper()}: {coverage['mitigated_dimensions']}/{coverage['total_dimensions']} dimensions ({coverage_pct:.1f}%)"
        )
    ...
    if gaps:
        for gap in gaps[:5]:  # Show first 5 gaps
            print(
                f"   • {gap['type']}: {gap.get('dimension', gap.get('unknown', 'Unknown'))}"
            )
    else:
    ...
    if dimensions:
        ...
        if context["context_analysis"].get("relationship_types"):
            ...
            for rel_type, nodes in context["context_analysis"][
                "relationship_types"
            ].items():
