    ...
    try:
        ...
        try:
            ...
            if new_html != initial_html:
                ...
                print(f"   Length change: {len(initial_html)} -> {len(new_html)} ({len(new_html) - len(initial_html):+d})")
                
                # Look for specific changes in step navigation
                if "#steps-navigation" in new_html and "#steps-navigation" in initial_html:
                    print(f"   Step navigation section exists in both")
                    
                    # Extract step navigation from both HTMLs
                    import re
                    
                    initial_steps = re.findall(r'<a[^>]*class="[^"]*step[^"]*"[^>]*>.*?</a>', initial_html, re.DOTALL)
                    new_steps = re.findall(r'<a[^>]*class="[^"]*step[^"]*"[^>]*>.*?</a>', new_html, re.DOTALL)
                    
                    print(f"   Initial steps: {len(initial_steps)}")
                    print(f"   New steps: {len(new_steps)}")
                    
                    # Check for current step changes
                    initial_current = re.findall(r'class="[^"]*current[^"]*"', initial_html)
