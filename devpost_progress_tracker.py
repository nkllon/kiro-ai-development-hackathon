...
class DevPostProgressTracker:
    ...
    def update_test_status(self, unit_tests: int = None, integration_tests: int = None) -> None:
        ...
        self._save_progress()
        print(f"🧪 Test status updated: {unit_tests} unit, {integration_tests} integration")
    ...
    def _update_metrics(self) -> None:
        ...
        self.daily_metrics["completion_percentage"] = (completed / len(self.tasks)) * 100
        self.daily_metrics["milestones_completed"] = sum(1 for m in self.milestones if m.status == "completed")
    ...
    def load_progress(self) -> None:
        ...
        # Load milestones
        self.milestones = [Milestone(**milestone_data) for milestone_data in data.get("milestones", [])]
    ...
    def print_status(self) -> None:
        ...
        print(f"📊 Progress: {metrics['completed_tasks']}/{metrics['total_tasks']} tasks ({metrics['completion_percentage']:.1f}%)")
        print(f"⏱️ Time: {metrics['total_actual_hours']:.1f}/{metrics['total_estimated_hours']:.1f} hours")
        print(f"🧪 Tests: {metrics['tests_passing']} unit, {metrics['integration_tests_passing']} integration")
        print(f"🎯 Milestones: {metrics['milestones_completed']}/{len(self.milestones)} completed")
