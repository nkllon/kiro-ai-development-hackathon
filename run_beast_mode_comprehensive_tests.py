    ...
    try:
        ...
        for i, test_path in enumerate(test_paths):
            ...
            try:
                ...
                print(f"✅ Test execution completed: Exit code {test_result['exit_code']}")
                ...
                print(f"📈 Success Criteria: {'✅ Met' if success_criteria_met else '❌ Not Met'}")
                ...
                if pattern_analysis.get("pattern_frequency"):
                    ...
                    for pattern, count in pattern_analysis["pattern_frequency"].items():
                        ...
                        overall_results["failure_patterns"][pattern] = overall_results["failure_patterns"].get(pattern, 0) + count
                ...
                if perf_analysis:
                    ...
                    if exec_analysis:
                        print(f"  • Duration: {exec_analysis.get('duration_seconds', 0):.2f}s")
                        print(f"  • Rating: {exec_analysis.get('performance_rating', 'unknown')}")
                    
                    resource_analysis = perf_analysis.get("resource_usage_analysis", {})
                    if resource_analysis:
                        print(f"  • Memory: {resource_analysis.get('memory_mb', 0):.1f} MB")
                        print(f"  • CPU: {resource_analysis.get('cpu_percent', 0):.1f}%")
        ...
        # PDCA cycle analysis
        successful_cycles = len([c for c in overall_results["pdca_cycles"] if c.get("success", False)])
        print(f"\n🔄 PDCA Cycle Analysis:")
        print(f"  • Total Cycles: {len(overall_results['pdca_cycles'])}")
        print(f"  • Successful Cycles: {successful_cycles}")
        print(f"  • Cycle Success Rate: {(successful_cycles / len(overall_results['pdca_cycles']) * 100) if overall_results['pdca_cycles'] else 0:.1f}%")
        
        # Failure pattern summary
        if overall_results["failure_patterns"]:
            print(f"\n🔍 Failure Pattern Summary:")
            sorted_patterns = sorted(overall_results["failure_patterns"].items(), key=lambda x: x[1], reverse=True)
        ...
        print(f"  • Patterns Detected: {health_indicators['failure_patterns_detected']}")
        print(f"  • RCA Engine: {health_indicators['rca_engine_status']}")
        
        # Save comprehensive results
        overall_results["end_time"] = datetime.now().isoformat()
        overall_results["duration_seconds"] = (datetime.now() - datetime.fromisoformat(overall_results["start_time"])).total_seconds()
        overall_results["health_indicators"] = health_indicators
        
        # Write results to file
        results_file = log_dir / f"comprehensive_test_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
