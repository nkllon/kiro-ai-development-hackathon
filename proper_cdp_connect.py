...
Connect to existing browser session using the correct CDP approach.
Based on established patterns from the community.

Author: Beast Mode Framework
Date: 2025-09-14
Purpose: Proper CDP connection to existing browser
"""
...
def main():
    """Connect to existing browser using proper CDP approach."""
    print("🔌 Proper CDP Connect to Existing Browser")
    print("=" * 50)
    ...
    try:
        ...
        # Access the default context (this is the key!)
        print("🔍 Accessing browser context...")
        ...
        print(f"✅ Found context with {len(context.pages)} pages")

        # Access the pages in the context
        pages = context.pages
        if not pages:
        ...
        for i, page in enumerate(pages):
            ...
            if "devpost.com" in page.url and "submission" in page.url:
                target_page = page
                print(f"   ✅ Found DevPost page at index {i+1}")
                break

        if not target_page:
            target_page = pages[0]  # Use first page if DevPost not found
            print(f"⚠️ DevPost page not found, using first page: {target_page.url}")

        # Now we can properly interact with the page!
        print(f"\n🎯 Successfully connected to page!")
            ...
            for i, form in enumerate(forms, 1):
                ...
                for j, field in enumerate(fields[:10], 1):  # Show first 10 fields
                    try:
                        ...
                        label = "Unlabeled"
                        if element_id:
                            label_elem = target_page.query_selector(
        ...
        try:
            ...
            filename = f"proper_cdp_{hackathon_id}_{submission_id}_{page_title}_{timestamp}.png"
        ...
        # Interactive mode
        print(f"\n🎮 Interactive Mode")
        print("Commands: analyze, navigate, screenshot, quit")
        ...
        while True:
            try:
                ...
                if command == "quit":
                    ...
                    print(f"📄 Current title: {target_page.title()}")
