...
def example_2_governance_workflow():
    ...
    runner.run_command(
        ["governance", "--status"], "Check current governance system status"
    )
...
""",
    ...
    runner.run_command(
        ["governance", "--check-overlaps"], "Check for overlaps across all specs"
    )
...
def example_4_analysis_workflow():
    ...
    runner.run_command(
        ["analyze", "--all-specs"], "Analyze all existing specs in the system"
    )
...
def example_5_error_handling():
    ...
    runner.run_command(["invalid-command"], "Test invalid command handling")
    ...
    runner.run_command(
        ["governance", "--validate-spec"], "Test missing required argument"
    )
