...
class TestTestRCAIntegrationEngine:
    ...
    def test_initialization_without_rca_engine(self):
        ...
        with patch(
            "src.beast_mode.testing.rca_integration.RCAEngine"
    ...
    def test_prioritize_failures(self, integrator, sample_test_failures):
        ...
        import_failure = next(
            f for f in prioritized if "ImportError" in f.error_message
        )
        ...
        )  # ImportError comes first (higher priority)
    ...
    def test_analyze_test_failures_success(
        self, integrator, mock_rca_engine, sample_test_failures, sample_rca_result
    ):
    ...
    def test_analyze_test_failures_with_patterns(
        self, integrator, mock_rca_engine, sample_test_failures, sample_rca_result
    ):
    ...
    def test_analyze_test_failures_timeout_handling(
        self, integrator, mock_rca_engine, sample_test_failures
    ):
    ...
    def test_analyze_test_failures_rca_engine_failure(
        self, integrator, mock_rca_engine, sample_test_failures
    ):
        """Test handling of RCA engine failures"""
        mock_rca_engine.match_existing_patterns.side_effect = Exception(
            "RCA engine failed"
        )
        ...
        assert any(
            "review test failures manually" in rec.lower()
    ...
    def test_error_handling_in_conversion(self, integrator):
        ...
        assert isinstance(rca_failure, Failure)
        assert (
            rca_failure.component == "test:unknown"
            or "conversion_failed" in rca_failure.failure_id
        )
