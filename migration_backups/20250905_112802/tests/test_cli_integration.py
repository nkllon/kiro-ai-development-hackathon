...
from src.spec_reconciliation.cli import (
    main,
...
"""
    ...
    def test_governance_status_command(self):
        ...
        with patch("sys.argv", ["cli.py", "governance", "--status"]):
            with patch(
                "src.spec_reconciliation.cli.GovernanceController"
            ) as mock_controller:
...
"""
        ...
        with patch(
            "sys.argv", ["cli.py", "governance", "--validate-spec", str(spec_file)]
        ):
            with patch(
                "src.spec_reconciliation.cli.GovernanceController"
            ) as mock_controller:
                ...
                with patch("builtins.print") as mock_print:
                    ...
                    printed_output = str(mock_print.call_args_list)
                    assert (
                        "approved" in printed_output.lower()
                        or "validation" in printed_output.lower()
                    )
    ...
    def test_governance_validate_spec_command_missing_file(self):
        ...
        with patch(
            "sys.argv", ["cli.py", "governance", "--validate-spec", str(missing_file)]
        ):
            with patch("builtins.print") as mock_print:
                main()
    ...
    def test_validate_terminology_command_valid_file(self):
        ...
        with patch(
            "sys.argv", ["cli.py", "validate", "--terminology", str(terminology_spec)]
        ):
            with patch(
                "src.spec_reconciliation.cli.ConsistencyValidator"
            ) as mock_validator:
                mock_instance = MagicMock()
                ...
                with patch("builtins.print") as mock_print:
                    ...
                    printed_output = str(mock_print.call_args_list)
                    assert (
                        "0.85" in printed_output
                        or "Consistency Score" in printed_output
                    )
    ...
    def test_validate_terminology_command_missing_file(self):
        ...
        with patch(
            "sys.argv", ["cli.py", "validate", "--terminology", str(missing_file)]
        ):
            with patch("builtins.print") as mock_print:
                main()
    ...
    def test_validate_interfaces_command_valid_file(self):
        ...
        with patch(
            "sys.argv", ["cli.py", "validate", "--interfaces", str(interface_spec)]
        ):
            with patch(
                "src.spec_reconciliation.cli.ConsistencyValidator"
            ) as mock_validator:
                ...
                with patch("builtins.print") as mock_print:
                    ...
                    printed_output = str(mock_print.call_args_list)
                    assert (
                        "0.75" in printed_output or "Compliance Score" in printed_output
                    )

    def test_validate_consistency_score_command(self):
        """Test validate consistency score command"""
        spec_files = [
            str(self.specs_dir / "terminology_test_spec.md"),
            str(self.specs_dir / "interface_test_spec.md"),
        ]

        with patch(
            "sys.argv", ["cli.py", "validate", "--consistency-score"] + spec_files
        ):
            with patch(
                "src.spec_reconciliation.cli.ConsistencyValidator"
            ) as mock_validator:
            ...
            with patch(
                ...
                with patch(
                    ...
                    with patch("builtins.print") as mock_print:
                        ...
                        printed_output = str(mock_print.call_args_list)
                        assert (
                            "specs monitored" in printed_output or "3" in printed_output
                        )
        ...
        with patch("sys.argv", ["cli.py", "analyze", "--overlap-matrix"]):
            with patch(
                ...
                with patch("builtins.print") as mock_print:
                    ...
                    printed_output = str(mock_print.call_args_list)
                    assert (
                        "overlap matrix" in printed_output
                        or "not yet implemented" in printed_output
                    )
...
class TestCLIErrorHandling:
    ...
    def test_file_permission_errors(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                with patch(
                    "sys.argv", ["cli.py", "validate", "--terminology", str(test_file)]
                ):
                    with pytest.raises(SystemExit):
                        main()
        ...
        with patch(
            ...
            with patch(
                ...
                with patch("builtins.print") as mock_print:
                    ...
                    printed_output = str(mock_print.call_args_list)
                    assert (
                        "Consistency Score" in printed_output
                        or "score" in printed_output
                    )
        ...
        with patch("sys.argv", ["cli.py", "analyze", "--all-specs"]):
            with patch(
                ...
                with patch(
                    ...
                    mock_val = MagicMock()
                    mock_val.get_module_status.return_value = {
                        "terminology_registry_size": 10
                    }
        ...
        for cmd in commands:
            with patch("sys.argv", cmd):
                with patch(
                    ...
                    mock_instance.validate_new_spec.return_value = (
        ...
        for cmd in commands:
            with patch("sys.argv", cmd):
                with patch(
                    ...
                    mock_instance.check_interface_compliance.return_value = (
                        ComplianceReport(
                    ...
                    mock_instance.generate_consistency_score.return_value = (
                        ConsistencyMetrics(
                            overall_score=0.85,
                            consistency_level=ConsistencyLevel.GOOD,
