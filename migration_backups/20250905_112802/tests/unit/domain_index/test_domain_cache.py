...
from src.beast_mode.domain_index.domain_cache import (
    DomainCache,
    DomainSpecificCache,
    CacheEntry,
)
...
class TestDomainSpecificCache:
    ...
    def test_domain_cache_warming(self, domain_cache, sample_domain):
        ...
        # Warm cache
        warmed_count = domain_cache.warm_domains(
            domain_loader, ["test_domain", "nonexistent"]
        )
...
class TestCacheIntegration:
    ...
    def test_cache_with_registry_manager(self):
        ...
        metadata = DomainMetadata(
            "core", "yes", PackagePotential(0.8, [], [], "medium", []), tags=["test"]
        )
        domain = Domain(
            "integration_test",
