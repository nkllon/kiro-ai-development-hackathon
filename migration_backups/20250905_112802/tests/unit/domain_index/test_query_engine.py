...
class TestDomainQueryEngine:
    ...
    def test_natural_language_query_content_intent(self, query_engine):
        ...
        result = query_engine.natural_language_query(
            "domains that contain specific content indicators"
        )
    ...
    def test_determine_intent_capability(self, query_engine):
        ...
        intent = query_engine._determine_intent(
            "what domains support pytest capability"
        )
        assert intent == "capability_search"
    ...
    def test_relationship_query_transitive_dependencies(self, query_engine):
        ...
        results = query_engine.relationship_query(
            "test_domain", "transitive_dependencies"
        )
    ...
    def test_relationship_query_transitive_dependents(self, query_engine):
        ...
        results = query_engine.relationship_query(
            "core_domain", "transitive_dependents"
        )
    ...
    def test_relationship_query_circular_dependencies(
        self, query_engine, sample_domains
    ):
        """Test circular dependency detection"""
        # Create a circular dependency for testing
        sample_domains["core_domain"].dependencies = [
            "api_domain"
        ]  # Creates cycle: test_domain -> core_domain -> api_domain -> core_domain
    ...
    def test_get_transitive_dependencies(self, query_engine, sample_domains):
        ...
        results = query_engine._get_transitive_dependencies(
            "deep_domain", sample_domains
        )
    ...
    def test_find_similar_domains(self, query_engine, sample_domains):
        ...
        results = query_engine._find_similar_domains(
            sample_domains["test_domain"], sample_domains
        )
        ...
        if len(results) > 1:
            ...
            similarities = [
                query_engine._calculate_domain_similarity(
                    sample_domains["test_domain"], domain
                )
    ...
    def test_calculate_domain_similarity(self, query_engine, sample_domains):
        ...
        similarity = query_engine._calculate_domain_similarity(
            sample_domains["test_domain"], sample_domains["api_domain"]
        )
        ...
        self_similarity = query_engine._calculate_domain_similarity(
            sample_domains["test_domain"], sample_domains["test_domain"]
        )
    ...
    def test_detect_circular_dependencies(self, query_engine, sample_domains):
        ...
        circular_chains = query_engine._detect_circular_dependencies(
            "test_domain", sample_domains
        )
    ...
    def test_calculate_coupling_score(self, query_engine, sample_domains):
        ...
        coupling_score = query_engine._calculate_coupling_score(
            sample_domains["test_domain"], sample_domains["core_domain"]
        )
    ...
    def test_calculate_capability_relevance(self, query_engine, sample_domains):
        ...
        relevance = query_engine._calculate_capability_relevance(
            sample_domains["test_domain"], "test"
        )
        ...
        api_relevance = query_engine._calculate_capability_relevance(
            sample_domains["api_domain"], "test"
        )
    ...
    def test_find_extraction_related_domains(self, query_engine, sample_domains):
        ...
        related_domains = query_engine._find_extraction_related_domains(
            sample_domains["core_domain"], sample_domains
        )
    ...
    def test_complex_relationship_scenarios(self, query_engine, sample_domains):
        ...
        trans_deps = query_engine.relationship_query(
            "integration_domain", "transitive_dependencies"
        )
    ...
    def test_determine_enhanced_intent(self, query_engine):
        ...
        intent = query_engine._determine_enhanced_intent(
            "find domains with *.py files", entities
        )
        assert intent == "pattern_search"
        ...
        intent = query_engine._determine_enhanced_intent(
            "domains that can run pytest", entities
        )
        assert intent == "capability_search"
        ...
        intent = query_engine._determine_enhanced_intent(
            "domains that depend on core", entities
        )
    ...
    def test_determine_query_type(self, query_engine):
        ...
        # Relationship query
        query_type = query_engine._determine_query_type(
        ...
        # Comparison query
        query_type = query_engine._determine_query_type(
            "compare test_domain vs api_domain"
        )
    ...
    def test_generate_intelligent_suggestions_many_results(
        self, query_engine, sample_domains
    ):
    ...
    def test_calculate_enhanced_relevance_scores(self, query_engine, sample_domains):
        ...
        scores = query_engine._calculate_enhanced_relevance_scores(
    ...
    def test_rank_and_filter_results(self, query_engine, sample_domains):
        ...
        domains = list(sample_domains.values())
        parsed_query = {"modifiers": ["sort_by:name", "order:asc", "limit:2"]}
    ...
    def test_contextual_suggestions(self, query_engine):
        ...
        suggestions = query_engine._generate_contextual_suggestions(
            partial_query, partial_info
        )
    ...
    def test_template_suggestions(self, query_engine):
        ...
        suggestions = query_engine._generate_template_suggestions(
            partial_query, partial_info
        )
    ...
    def test_query_corrections(self, query_engine):
        ...
        corrections = query_engine.suggest_query_corrections(
            "find domians with dependancies"
        )
    ...
    def test_explain_query_results(self, query_engine):
        ...
        assert (
            explanation["query_interpretation"]["detected_intent"]
            == "capability_search"
        )

    def test_explain_relevance_factors(self, query_engine, sample_domains):
        """Test relevance factor explanation"""
        parsed_query = {"keywords": ["test"], "entities": {"capabilities": ["pytest"]}}

        factors = query_engine._explain_relevance_factors(
            sample_domains["test_domain"], parsed_query
        )
