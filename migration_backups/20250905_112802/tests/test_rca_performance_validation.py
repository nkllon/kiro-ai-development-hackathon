...
class TestPerformanceBenchmarks:
            ...
            failures.append(
                TestFailureData(
                    ...
                    * (i % 5 + 1),  # Varying stack trace sizes
                    ...
                    test_class=(
                        f"TestClass{i % 20}" if i % 3 == 0 else None
                    ),  # Some with classes
    ...
    def test_30_second_analysis_requirement_validation(
        self, performance_monitor, large_failure_dataset
    ):
        """
        ...
        for size in dataset_sizes:
            ...
            print(
                f"Dataset size {size}: {metrics['elapsed_time']:.2f}s, "
    ...
    def test_concurrent_analysis_performance(self, large_failure_dataset):
        ...
        for chunk in chunks[:3]:  # Test with 3 concurrent threads
            thread = threading.Thread(target=analyze_chunk, args=(chunk, results))
        ...
        assert total_time < 45.0, (
            f"Concurrent analysis took {total_time:.2f}s, " f"should be under 45s"
        )
    ...
    def test_scalability_with_increasing_load(self, performance_monitor):
        ...
        for i in range(1, len(performance_data)):
            ...
            time_ratio = curr_data["time"] / max(
                prev_data["time"], 0.001
...
class TestResourceManagement:
    ...
    def test_cpu_usage_limits(self, large_failure_dataset):
        ...
        if cpu_samples:
            ...
            assert (
                avg_cpu < 80.0
            ), f"Average CPU usage was {avg_cpu:.1f}%, should be under 80%"
            assert (
                max_cpu < 95.0
            ), f"Peak CPU usage was {max_cpu:.1f}%, should be under 95%"
