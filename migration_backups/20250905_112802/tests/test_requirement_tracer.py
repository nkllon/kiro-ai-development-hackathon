...
"""
    ...
    def test_determine_reference_type(self, tracer, temp_repo):
        ...
        # Comment reference
        ref_type = tracer._determine_reference_type(
            "# Requirements: 1.1", temp_repo / "code.py"
        )
        ...
        ref_type = tracer._determine_reference_type(
            "_Requirements: 1.1_", temp_repo / "code.py"
        )
    ...
    def test_analyze_traceability(self, tracer):
        ...
        assert (
            len(result.orphaned_implementations) >= 1
        )  # Should find orphaned reference to 3.1
    ...
    def test_generate_traceability_issues(self, tracer):
        ...
        assert (
            len(issues) >= 2
        )  # Should have issues for untraced requirements and low score
...
"""
    ...
    def test_traceability_score_calculation(self, tracer):
        ...
        expected_score = (
            (result.traced_requirements / result.total_requirements * 100)
