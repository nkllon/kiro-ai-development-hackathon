...
def example_4_custom_report_generation():
    ...
    print(
        markdown_report[:500] + "..." if len(markdown_report) > 500 else markdown_report
    )
...
def example_5_performance_optimization():
    ...
    try:
        analysis_result = integrator.analyze_test_failures(
            many_failures, timeout=30, performance_mode=True
        )
        ...
        print(
            f"   Performance: {analysis_result.failures_analyzed / analysis_time:.1f} failures/second"
        )
        ...
        if hasattr(analysis_result, "performance_metrics"):
            ...
            print(f"\nPerformance Metrics:")
            print(
                f"   Pattern matching time: {metrics.get('pattern_matching_time', 0):.2f}s"
            )
...
def example_6_pattern_learning():
    ...
    print(
        f"  Speed improvement: {((first_time - second_time) / first_time * 100):.1f}%"
    )
    ...
    print(
        f"  Database timeout patterns: {len(pattern_library.get_patterns_by_type('database_timeout'))}"
    )
...
def example_7_ci_cd_integration():
    ...
    print(
        f"CI/CD Environment: {ci_environment['BRANCH_NAME']} (Build {ci_environment['BUILD_ID']})"
    )
    ...
    analysis_result = integrator.analyze_test_failures(
        ci_failures, ci_context=ci_environment
    )
