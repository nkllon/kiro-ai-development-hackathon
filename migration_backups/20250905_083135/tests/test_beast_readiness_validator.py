...
class TestCompletenessValidation:
    ...
    def test_requirements_completeness_success(self, validator, complete_backlog_item):
        ...
        req_criterion = next(
            ...
            if c.criterion_name == "requirements_completeness"
        )
        assert req_criterion.passed is True
        assert req_criterion.score >= 0.8
        assert len(req_criterion.details["missing"]) == 0

    def test_requirements_completeness_failure(
        self, validator, incomplete_backlog_item
    ):
    ...
    def test_acceptance_criteria_quality_success(
        self, validator, complete_backlog_item
    ):
        ...
        ac_criterion = next(
            c
            for c in report.criteria_results
            if c.criterion_name == "acceptance_criteria_quality"
        )
        assert ac_criterion.passed is True
        assert ac_criterion.score >= 0.8

    def test_acceptance_criteria_quality_failure(
        self, validator, incomplete_backlog_item
    ):
    ...
    def test_context_adequacy_success(self, validator, complete_backlog_item):
        ...
        context_criterion = next(
            c for c in report.criteria_results if c.criterion_name == "context_adequacy"
        )
    ...
    def test_ambiguity_detection_success(self, validator, complete_backlog_item):
        ...
        ambiguity_criterion = next(
            c
            for c in report.criteria_results
            if c.criterion_name == "ambiguity_absence"
        )
    ...
    def test_ambiguity_detection_failure(self, validator, ambiguous_backlog_item):
        ...
        assert any(
            "perhaps" in amb.lower() for amb in ambiguity_criterion.details["ambiguous"]
        )
    ...
    def test_overall_completeness_scoring(self, validator, complete_backlog_item):
        ...
        assert report.beast_ready == (
            report.overall_score >= validator._beast_readiness_threshold
        )

        # Verify weighted scoring
        total_weight = sum(c.weight for c in report.criteria_results)
        expected_score = (
            sum(c.score * c.weight for c in report.criteria_results) / total_weight
        )
...
class TestCompletenessReport:
    ...
    def test_completeness_report_invalid_score(self):
        ...
        with pytest.raises(
            ValueError, match="Overall score must be between 0.0 and 1.0"
        ):
            CompletenessReport(
...
class TestEdgeCases:
    ...
    def test_very_short_descriptions(self, validator):
        ...
        assert report.beast_ready is False
        assert any(
            "insufficient description" in missing for missing in report.missing_elements
        )
    ...
    def test_non_testable_acceptance_criteria(self, validator):
        ...
        assert ac_criterion.passed is False
        assert any(
            "not testable" in missing for missing in ac_criterion.details["missing"]
        )
        assert any(
            "not measurable" in missing for missing in ac_criterion.details["missing"]
        )
