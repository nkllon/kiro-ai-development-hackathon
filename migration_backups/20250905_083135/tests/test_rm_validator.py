...
class TestRMValidator:
    ...
    def _create_temp_module(
        self, content: str, filename: str = "test_module.py"
    ) -> str:
...
"""
        ...
        assert (
            ...
            == "No ReflectiveModule classes found in module"
        )
...
'''
        ...
        class_violation = any(
            "Too many classes" in issue.description for issue in architectural_issues
        )
        import_violation = any(
            "Too many imports" in issue.description for issue in architectural_issues
        )
        nesting_violation = any(
            "Deep nesting" in issue.description for issue in architectural_issues
        )
    ...
    def test_god_class_detection(self):
        ...
        god_class_issues = [
            issue for issue in result.issues if "Too many methods" in issue.description
        ]
...
"""
        ...
        docstring_issues = [
            issue for issue in result.issues if "docstring" in issue.description
        ]
...
"""
        ...
        indicator_issues = [
            issue for issue in result.issues if "health indicators" in issue.description
        ]
...
"""
        ...
        import_issues = [
            issue for issue in result.issues if "imports" in issue.description
        ]
