...
class TestToolOrchestrationEngine:
    ...
    def test_decision_confidence_assessment(
        self, orchestration_engine, sample_decision_context
    ):
        """Test decision confidence assessment"""
        # Mock intelligence engine response
        with patch.object(
            orchestration_engine.intelligence_engine, "consult_registry_first"
        ) as mock_consult:
            ...
            confidence_result = orchestration_engine._assess_decision_confidence(
            ...
            assert isinstance(
                confidence_result["confidence_level"], DecisionConfidenceLevel
            )

    def test_high_confidence_decision_routing(
        self, orchestration_engine, sample_decision_context
    ):
        """Test high confidence decision routing"""
        # Mock high confidence scenario
        with patch.object(
            orchestration_engine.intelligence_engine, "get_domain_tools"
        ) as mock_domain_tools:
            mock_domain_tools.return_value = [
                "git_status",
                "make_help",
                "python_version",
            ]
            ...
            decision_result = orchestration_engine._make_high_confidence_decision(
                sample_decision_context, preferred_tools=["git_status"]
            )
            ...
            assert (
                decision_result["decision_method"]
                == "model_registry_domain_intelligence"
            )

    def test_medium_confidence_decision_routing(
        self, orchestration_engine, sample_decision_context
    ):
        """Test medium confidence decision routing"""
        # Mock medium confidence scenario
        with patch.object(
            orchestration_engine.multi_perspective_engine,
            ...
            with patch.object(
                orchestration_engine, "_make_high_confidence_decision"
            ) as mock_high_confidence:
                ...
                assert (
                    decision_result["decision_method"]
                    == "registry_plus_basic_perspectives"
                )

    def test_low_confidence_decision_routing(
        self, orchestration_engine, sample_decision_context
    ):
        ...
        with patch.object(
            ...
            mock_analysis.return_value = {
                ...
                "synthesized_recommendation": {"tools": ["git_status"]},
            }
            ...
            assert "selected_tools" in decision_result
            assert (
                decision_result["decision_method"] == "full_multi_stakeholder_analysis"
            )
    ...
    def test_single_tool_execution(
        ...
        result = orchestration_engine._execute_single_tool(
            "test_tool", sample_decision_context, "test_operation"
        )
    ...
    def test_systematic_tool_repair(self, orchestration_engine, sample_tool_definition):
        ...
        # Mock RCA result
        mock_rca_result = {
            "root_causes": [{"suggested_repairs": ["echo 'rca repair'"]}]
        }
        ...
        with patch.object(orchestration_engine, "_perform_tool_rca") as mock_rca:
            ...
            with patch.object(
                orchestration_engine, "_execute_repair_procedure"
            ) as mock_repair:
                mock_repair.return_value = {"success": True}

                with patch.object(
                    orchestration_engine, "_check_tool_health"
                ) as mock_health:
                    mock_health.return_value = {"status": ToolStatus.HEALTHY}

                    repair_result = orchestration_engine._attempt_systematic_repair(
                        "test_tool", mock_rca_result
                    )
    ...
    def test_full_orchestration_workflow(
        self, orchestration_engine, sample_decision_context, sample_tool_definition
    ):
        ...
        with patch.object(
            ...
            with patch.object(
                ...
                with patch("subprocess.run") as mock_subprocess:
                    mock_subprocess.return_value = Mock(
                        stdout="successful execution", stderr="", returncode=0
                    )

                    result = orchestration_engine.orchestrate_tool_execution(
                        sample_decision_context, preferred_tools=["test_tool"]
                    )
    ...
    def test_orchestration_metrics_tracking(
        self, orchestration_engine, sample_decision_context, sample_tool_definition
    ):
        ...
        with patch.object(
            ...
            with patch.object(
                ...
                with patch("subprocess.run") as mock_subprocess:
                    ...
                    orchestration_engine.orchestrate_tool_execution(
                        sample_decision_context
                    )

        # Verify metrics were updated
        assert (
            orchestration_engine.orchestration_metrics["total_orchestrations"]
            > initial_metrics["total_orchestrations"]
        )
        assert (
            orchestration_engine.orchestration_metrics["successful_orchestrations"]
            > initial_metrics["successful_orchestrations"]
        )
