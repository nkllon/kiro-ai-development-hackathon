...
class TestRCAIntegrationWithPatterns:
    ...
    def rca_integrator(self, mock_rca_engine, test_pattern_library):
        ...
        mock_timeout_handler.manage_operation_timeout.return_value = (
            mock_timeout_context
        )
        mock_timeout_handler.get_timeout_recommendations.return_value = {
    ...
    def test_pattern_library_integration_in_status(self, rca_integrator):
        ...
        assert (
            status["test_pattern_library_status"]["module_name"]
            == "test_pattern_library"
        )

    def test_pattern_matching_during_analysis(
        self, test_pattern_library, sample_test_failure
    ):
        ...
        actual_signature = test_pattern_library._generate_test_failure_signature(
            rca_failure
        )
    ...
    def test_pattern_learning_from_successful_analysis(
        self, test_pattern_library, sample_test_failure
    ):
    ...
    def test_pattern_library_optimization(self, rca_integrator):
        ...
        for i in range(3):
            ...
            rca_integrator.test_pattern_library.test_patterns[pattern.pattern_id] = (
                pattern
            )
    ...
    def test_sub_second_performance_requirement(
        self, rca_integrator, sample_test_failure
    ):
        ...
        assert (
            match_time_ms < 1000
        ), f"Pattern matching took {match_time_ms:.2f}ms, exceeds 1 second requirement"
    ...
    def test_error_handling_in_integration(self, rca_integrator, sample_test_failure):
        ...
        with patch.object(
            rca_integrator.test_pattern_library,
            "match_test_patterns",
            ...
            rca_integrator.rca_engine.perform_systematic_rca.return_value = (
                mock_rca_result
            )
