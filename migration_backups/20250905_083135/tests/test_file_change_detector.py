...
class TestFileChangeDetector:
    ...
    def test_categorize_file_source_code(self, file_change_detector):
        ...
        assert (
            file_change_detector._categorize_file("src/module.py")
            == FileCategory.SOURCE_CODE
        )
        assert (
            file_change_detector._categorize_file("lib/utils.js")
            == FileCategory.SOURCE_CODE
        )
        assert (
            file_change_detector._categorize_file("app/main.java")
            == FileCategory.SOURCE_CODE
        )
    ...
    def test_categorize_file_test_code(self, file_change_detector):
        ...
        assert (
            file_change_detector._categorize_file("test_module.py")
            == FileCategory.TEST_CODE
        )
        assert (
            file_change_detector._categorize_file("tests/test_utils.py")
            == FileCategory.TEST_CODE
        )
        assert (
            file_change_detector._categorize_file("spec/utils.spec.js")
            == FileCategory.TEST_CODE
        )
    ...
    def test_categorize_file_documentation(self, file_change_detector):
        ...
        assert (
            file_change_detector._categorize_file("README.md")
            == FileCategory.DOCUMENTATION
        )
        assert (
            file_change_detector._categorize_file("docs/guide.rst")
            == FileCategory.DOCUMENTATION
        )
        assert (
            file_change_detector._categorize_file("CHANGELOG.txt")
            == FileCategory.DOCUMENTATION
        )
    ...
    def test_categorize_file_configuration(self, file_change_detector):
        ...
        assert (
            file_change_detector._categorize_file("config.json")
            == FileCategory.CONFIGURATION
        )
        assert (
            file_change_detector._categorize_file("settings.yaml")
            == FileCategory.CONFIGURATION
        )
        assert (
            file_change_detector._categorize_file("pyproject.toml")
            == FileCategory.CONFIGURATION
        )
    ...
    def test_categorize_file_build_script(self, file_change_detector):
        ...
        assert (
            file_change_detector._categorize_file("Makefile")
            == FileCategory.BUILD_SCRIPT
        )
        assert (
            file_change_detector._categorize_file("build.sh")
            == FileCategory.BUILD_SCRIPT
        )
        assert (
            file_change_detector._categorize_file("setup.py")
            == FileCategory.BUILD_SCRIPT
        )
    ...
    def test_categorize_file_unknown(self, file_change_detector):
        ...
        assert (
            file_change_detector._categorize_file("random.xyz") == FileCategory.UNKNOWN
        )
        assert (
            file_change_detector._categorize_file("binary.bin") == FileCategory.UNKNOWN
        )

    def test_extract_file_changes_from_commit(
        self, file_change_detector, sample_commits
    ):
    ...
    def test_identify_high_impact_changes(
        self, file_change_detector, sample_file_changes
    ):
        """Test identification of high-impact changes."""
        high_impact = file_change_detector._identify_high_impact_changes(
            sample_file_changes
        )
    ...
    def test_calculate_complexity_score(
        ...
        complexity = file_change_detector._calculate_complexity_score(
            sample_file_changes
        )
    ...
    def test_assess_risk_level(self, file_change_detector, sample_file_changes):
        ...
        risk = file_change_detector._assess_risk_level(
            sample_file_changes, complexity_score
        )
    ...
    def test_detect_completion_evidence(
        self, file_change_detector, sample_file_changes
    ):
        ...
        evidence = file_change_detector.detect_completion_evidence(
            sample_file_changes, task_patterns
        )
    ...
    def test_calculate_change_impact(self, file_change_detector, sample_file_changes):
        ...
        impact_scores = file_change_detector.calculate_change_impact(
            sample_file_changes
        )
    ...
    def test_file_matches_task_patterns(self, file_change_detector):
        ...
        assert (
            file_change_detector._file_matches_task_patterns(
    ...
    def test_analyze_task_completion(self, file_change_detector):
        ...
        mapping = file_change_detector._analyze_task_completion(
    ...
    def test_enhanced_task_mapping_with_claimed_tasks(self, file_change_detector):
        ...
        assert (
            file_change_mapping.confidence_score > 0.3
        )  # Adjusted for realistic expectations
        assert any(
            "evidence" in evidence.lower() for evidence in file_change_mapping.evidence
        )

    def test_comprehensive_file_change_analysis(
        self, file_change_detector, sample_commits
    ):
    ...
    def test_calculate_detection_accuracy_metrics(
        self, file_change_detector, sample_file_changes
    ):
    ...
    def test_file_change_breakdown_generation(
        self, file_change_detector, sample_file_changes
    ):
        ...
        analysis = AdvancedFileChangeAnalysis(
            total_files_changed=len(sample_file_changes),
            changes_by_type={
                ChangeType.MODIFIED: sample_file_changes[:2],
                ChangeType.ADDED: [sample_file_changes[2]],
            },
            changes_by_category={
    ...
    def test_pattern_matching_accuracy(self, file_change_detector):
        ...
        for file_path, patterns, should_match in test_cases:
            actual_match = file_change_detector._file_matches_task_patterns(
                file_path, patterns
            )
    ...
    def test_confidence_score_calculation_accuracy(self, file_change_detector):
        ...
        ), f"Expected high confidence, got {mapping.confidence_score}"
        ...
        assert (
            mapping_low.confidence_score < 0.3
        ), f"Expected low confidence, got {mapping_low.confidence_score}"
    ...
    def test_claimed_vs_implemented_validation_accuracy(self, file_change_detector):
        ...
        assert (
            len(validation["missing_evidence_tasks"]) == 1
        )  # poorly_implemented_claimed
        assert (
            len(validation["unclaimed_implementations"]) == 1
        )  # well_implemented_unclaimed
...
class TestFileChangeDetectorIntegration:
    ...
    def test_full_analysis_workflow(self, detector_with_real_data):
        ...
        task_patterns = {
            "git_implementation": ["*git*", "src/beast_mode/compliance/git/*"]
        }
        evidence = detector_with_real_data.detect_completion_evidence(
            all_changes, task_patterns
        )
