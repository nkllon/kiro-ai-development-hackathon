    ...
    def test_pytest_output_parsing_compatibility(self, pytest_output_samples):
        ...
        for version_name, output in pytest_output_samples.items():
            ...
            for failure in failures:
                assert hasattr(
                    failure, "test_name"
                ), f"Missing test_name in {version_name}"
                assert hasattr(
                    failure, "error_message"
                ), f"Missing error_message in {version_name}"
                assert hasattr(
                    failure, "failure_type"
                ), f"Missing failure_type in {version_name}"
                assert (
                    len(failure.error_message) > 0
                ), f"Empty error_message in {version_name}"

                # Verify pytest node ID is extracted
                assert hasattr(
                    failure, "pytest_node_id"
                ), f"Missing pytest_node_id in {version_name}"
...
class TestFailureTypeCategorization:
    ...
    def test_failure_categorization_accuracy(self, failure_categorization_test_cases):
        ...
        for test_case in failure_categorization_test_cases:
            ...
            assert (
                rca_failure.category is not None
            ...
            if (
                "ImportError" in test_case["error_message"]
                or "ModuleNotFoundError" in test_case["error_message"]
            ):
                assert (
                    rca_failure.category == FailureCategory.DEPENDENCY_ISSUE
    ...
    def test_complex_error_message_parsing(self):
        ...
        for error_case in complex_errors:
            ...
            assert (
                rca_failure.category == error_case["expected_category"]
...
class TestInfrastructureFailureCompatibility:
    ...
    def test_docker_failure_analysis(self):
        ...
        for i, failure_case in enumerate(docker_failures):
            ...
            assert (
                rca_failure.category == failure_case["expected_category"]
...
class TestUnknownFailureTypeHandling:
    ...
    def test_unknown_error_graceful_handling(self):
        ...
        unknown_failures = [
            ...
            # Empty error message
            {"error_message": "", "expected_category": FailureCategory.UNKNOWN},
        ]
    ...
    def test_malformed_failure_data_handling(self):
        ...
        for i, malformed_failure in enumerate(malformed_cases):
            try:
                ...
                pytest.fail(
                    f"Should handle malformed failure data gracefully (case {i}): {e}"
                )
