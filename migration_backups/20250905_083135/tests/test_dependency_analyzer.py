...
class TestCircularDependencyDetector:
    ...
    def test_self_dependency(self):
        ...
        domains = {"A": self.create_test_domain("A", ["A"])}
    ...
    def test_performance_large_graph(self):
        ...
        for i in range(100):
            deps = [
                f"domain_{j}" for j in range(max(0, i - 3), i)
            ]  # Each domain depends on previous 3
...
class TestOrphanedFileDetector:
    ...
    def test_orphaned_files_detected(self, mock_glob):
        ...
        with patch.object(
            detector, "_file_matches_pattern", side_effect=mock_file_matches
        ):
            result = detector.detect_orphaned_files()
    ...
    def test_file_pattern_matching(self):
        ...
        assert detector._file_matches_pattern(
            Path("src/core/main.py"), "src/core/**/*.py"
        )
        assert detector._file_matches_pattern(
            Path("src/core/sub/helper.py"), "src/core/**/*.py"
        )
        assert not detector._file_matches_pattern(
            Path("src/other/main.py"), "src/core/**/*.py"
        )
...
class TestDependencyImpactAnalyzer:
    ...
    def test_impact_analysis_modify(self):
        ...
        assert (
            "A" in impact["transitively_affected"]
...
class TestComprehensiveDependencyAnalyzer:
    ...
    def test_comprehensive_analysis_sequential(self):
        ...
        with patch.object(
            analyzer, "_analyze_orphaned_files", return_value={"orphaned_files": []}
        ):
    ...
    def test_performance_with_large_domain_set(self):
        ...
        for i in range(50):  # 50 domains
            deps = [
                f"domain_{j}" for j in range(max(0, i - 2), i)
            ]  # Each depends on previous 2
