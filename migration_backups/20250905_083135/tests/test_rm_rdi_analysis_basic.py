...
class TestAnalysisSystemBasic(unittest.TestCase):
    ...
    def test_analysis_system_structure_exists(self):
        ...
        core_files = ["__init__.py", "data_models.py", "safety.py", "base.py"]
    ...
    def test_operator_control_script_exists(self):
        ...
        self.assertTrue(
            os.access(script_path, os.X_OK), "Control script must be executable"
        )
    ...
    def test_makefile_integration_exists(self):
        ...
        for command in emergency_commands:
            self.assertIn(
                command, content, f"Emergency command {command} must exist in Makefile"
            )
    ...
    def test_safety_guarantees_documented(self):
        ...
        for guarantee in safety_guarantees:
            self.assertIn(
                guarantee, content, f"Safety guarantee {guarantee} must be documented"
            )
    ...
    def test_data_models_are_immutable(self):
        ...
        self.assertIn(
            "@dataclass(frozen=True)", content, "Data models must be immutable"
        )
...
class TestOperatorSafetyDocumentation(unittest.TestCase):
    ...
    def test_operator_safety_guide_exists(self):
        ...
        for command in emergency_commands:
            self.assertIn(
                command, content, f"Emergency command {command} must be documented"
            )
