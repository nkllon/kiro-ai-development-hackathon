...
from multi_dimensional_context_analyzer import (
...
class PageSimilarityResult:
    ...
    def __init__(self):
        ...
        self.similarity_type = (
            "unknown"  # "exact", "visual", "url", "navigation", "dynamic"
        )
...
class SessionRecoveryAnalyzer:
    ...
    def analyze_page_similarity(
        ...
        visual_match = self._check_visual_similarity(
            current_page_data.get("visual_hash")
        )
        if visual_match:
            ...
            url_similarity = self._check_url_similarity(
                current_page_data["url"], visual_match["page_data"]["url"]
            )
            if url_similarity > 0.8:
                ...
                result.recommendations = [
                    "URL is parameterized version of known page",
                    "Use existing navigation model",
                ]
            else:
                result.recommendations = [
                    "Visual match but URL differs significantly",
        ...
        if navigation_match:
            ...
            result.confidence = (
                navigation_match["similarity"] * 0.7
            )  # Lower confidence for navigation-only matches
        ...
        try:
            ...
            for node_id, data in self.telemetry_graph.graph.nodes(data=True):
                if data.get("visual_hash") == current_visual_hash:
                    return {"similarity": 1.0, "page_data": data, "node_id": node_id}
    ...
    def _check_url_similarity(self, current_url: str, stored_url: str) -> float:
        ...
        path_similarity = difflib.SequenceMatcher(
            None, current_parsed.path, stored_parsed.path
        ).ratio()
        ...
        if current_params or stored_params:
            ...
            param_similarity = (
                len(common_params) / len(total_params) if total_params else 0.0
            )
    ...
    def _check_navigation_similarity(
        self, current_page_data: Dict[str, Any]
    ) -> Optional[Dict[str, Any]]:
        ...
        for node_id, data in self.telemetry_graph.graph.nodes(data=True):
            ...
            similarity = self._compare_navigation_elements(
                current_nav_elements, stored_nav_elements
            )

            if (
                similarity > best_similarity and similarity > 0.7
    ...
    def _check_site_specific_quirks(
        self, current_page_data: Dict[str, Any]
    ) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if not hasattr(state, "_session_analyzer"):
            state["_session_analyzer"] = SessionRecoveryAnalyzer(
                state.get("telemetry_graph"), state.get("state_model")
            )
        ...
        multi_dimensional_analysis = (
            multi_dimensional_analyzer.analyze_multi_dimensional_context(
                current_page_data
            )
        )
        ...
        EXPLORATORY_THRESHOLD = (
            0.2  # Below this, we're in "completely confused" territory
        )
        PROMPT_MODE_THRESHOLD = (
            0.4  # Below this, we need conversational decision-making
        )
        ...
        if overall_confidence < EXPLORATORY_THRESHOLD:
            ...
            print(
                f"   Confidence: {overall_confidence:.2f} (below {EXPLORATORY_THRESHOLD})"
            )
        ...
        if similarity_result.similarity_type == "exact":
            ...
            if similarity_result.matched_page_data:
                state["navigation_model"] = similarity_result.matched_page_data.get(
                    "navigation_model"
                )
