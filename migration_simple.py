...
def create_migration_backups(workspace_root: Path) -> Path:
    ...
    backup_dir = (
        workspace_root / "migration_backups" / datetime.now().strftime("%Y%m%d_%H%M%S")
    )
    ...
    try:
        ...
        for layer in compatibility_layers:
            ...
            test_result = {
                ...
                "details": {
                    ...
                    "file_size": (
                        compatibility_file.stat().st_size
        ...
        validation_results["overall_success"] = (
            all(
                ...
                for test in validation_results["compatibility_tests"]
            )
            and all(
                test.get("success", False)
                for test in validation_results["functionality_tests"]
            )
            and all(
                test.get("success", False)
                for test in validation_results["performance_tests"]
            )
        )
