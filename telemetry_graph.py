...
class TelemetryGraph:
    ...
    def add_page_telemetry(self, page_data: Dict[str, Any]) -> str:
        ...
        if self.current_page_hash:
            self.add_navigation_edge(
                self.current_page_hash, page_hash, "page_navigation", True
            )
    ...
    def save_graph(self):
        ...
        print(
            f"💾 Telemetry graph saved: {len(self.graph.nodes)} pages, {len(self.graph.edges)} navigations"
        )
    ...
    def load_graph(self):
        ...
        try:
            ...
            print(
                f"📊 Loaded telemetry graph: {len(self.graph.nodes)} pages, {len(self.graph.edges)} navigations"
            )
    ...
    def get_session_summary(self) -> Dict[str, Any]:
                    ...
                    for target in nodes:
                        if source != target and self.graph.has_edge(source, target):
                            path_data = {
                                "source": source,
                                "target": target,
                                "method": self.graph.edges[source, target].get(
        ...
        return {
            ...
            "success_rate": (successful_navigations / max(len(self.graph.edges), 1))
            * 100,
