...
class EmergencySessionDumper:
    ...
    def _dump_python_environment(self):
        ...
        self.dump_data['python_environment'] = {
            ...
            'threading_info': {
                'active_count': getattr(sys, 'threading', {}).get('active_count', 'unknown')
            },
    ...
    def _dump_git_state(self):
        ...
        try:
            ...
            self.dump_data['git_state'] = {
                'status': git_status.stdout if git_status.returncode == 0 else git_status.stderr,
                'recent_commits': git_log.stdout if git_log.returncode == 0 else git_log.stderr,
                'current_branch': git_branch.stdout.strip() if git_branch.returncode == 0 else 'unknown',
                'remotes': git_remote.stdout if git_remote.returncode == 0 else git_remote.stderr,
    ...
    def _dump_error_logs(self):
        ...
        self.dump_data['error_logs'] = {
            ...
            'sys_last_traceback': traceback.format_exc() if traceback.format_exc() != 'NoneType: None\n' else None
        }
    ...
    def _get_frame_info(self):
        ...
        while frame and depth < 20:  # Limit depth to prevent infinite loops
            frame_info.append({
                ...
                'locals': {k: str(v)[:200] for k, v in frame.f_locals.items()}  # Truncate long values
            })
            frame = frame.f_back
            depth += 1
    ...
    def _get_directory_structure(self, path: Path, max_depth: int = 3, current_depth: int = 0):
