    ...
    messages = [
        ...
        BeastModeMessage(
            ...
            payload={
                ...
                "spore_content": {
                    ...
                    "success_metrics": ["query_time_reduction", "cpu_usage", "memory_usage"]
                }
            },
        ...
        BeastModeMessage(
            ...
            payload={
                ...
                "details": {
                    "before_optimization": {"avg_query_time": "2.3s", "cpu_usage": "85%"},
                    "after_optimization": {"avg_query_time": "0.4s", "cpu_usage": "32%"},
                    "improvement": "82% faster, 62% less CPU usage"
                },
                "next_steps": ["Monitor for 24h", "Apply to other services", "Document learnings"]
            },
        ...
        BeastModeMessage(
            ...
            target="alice-agent",
            payload={"text": "Alice, could you share that database optimization spore with me too?"},
...
async def demonstrate_basic_scanning(history_manager: MessageHistoryManager):
    ...
    for i, msg_entry in enumerate(all_messages[:3]):
        ...
        print(f"  {i+1}. [{msg.type.value}] {msg.source} -> {msg.target or 'BROADCAST'}")
...
async def demonstrate_filtering(history_manager: MessageHistoryManager):
    ...
    for msg_entry in high_priority:
        msg = msg_entry.message
        print(f"  • Priority {msg.priority}: [{msg.type.value}] {msg.source} -> {msg.target}")
...
async def demonstrate_advanced_filtering(history_manager: MessageHistoryManager):
    ...
    for msg_entry in spore_messages:
        msg = msg_entry.message
        print(f"  • [{msg.type.value}] Priority {msg.priority}: {msg.source} -> {msg.target}")
