...
def demo_single_machine_deployment(config_manager):
    ...
    with patch('subprocess.Popen') as mock_popen:
        ...
        with patch('psutil.Process'), patch('psutil.cpu_percent'), \
            ...
            print(f"   System CPU: {health['system_resources'].get('cpu_percent', 0):.1f}%")
            print(f"   System Memory: {health['system_resources'].get('memory_percent', 0):.1f}%")
...
def demo_service_monitoring(config_manager):
    ...
    service = MonitoredService(
        name="demo_service",
        command=["python", "-c", "import time; print('Demo service running'); time.sleep(5)"],
        working_directory=".",
    ...
    print(f"   Health check: {' '.join(service.health_check_command) if service.health_check_command else 'None'}")
    ...
    with patch('psutil.Process') as mock_psutil:
        ...
        print(f"   Memory: {service.metrics.memory_percent}% ({service.metrics.memory_mb:.1f}MB)")
...
def demo_complete_workflow():
    ...
    with patch('subprocess.Popen') as mock_popen:
        ...
        with patch('socket.socket'), patch('redis.Redis'), \
             patch('subprocess.run'), patch('os.path.exists'):
