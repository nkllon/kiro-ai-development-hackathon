...
async def create_agent_network() -> List[BeastModeBusClient]:
    ...
    agents = [
        ...
        BeastModeBusClient(
            ...
            capabilities=["python", "javascript", "react", "nodejs", "testing", "frontend"]
        ),
        BeastModeBusClient(
            agent_id="data_scientist",
            capabilities=["python", "machine_learning", "data_analysis", "pandas", "tensorflow"]
        ),
        BeastModeBusClient(
            agent_id="security_expert",
            capabilities=["security", "penetration_testing", "compliance", "audit", "encryption"]
        )
    ]
    ...
    for agent in agents:
        if await agent.connect():
            ...
            logger.info(f"Connected {agent.agent_id} with capabilities: {agent.capabilities}")
        else:
...
async def demonstrate_discovery(agents: List[BeastModeBusClient]):
    ...
    for agent in agents:
        ...
        for discovered_agent in discovered:
            logger.info(f"  - {discovered_agent.agent_id}: {discovered_agent.capabilities.capabilities}")
    ...
    for capabilities in test_capabilities:
        ...
        if matching_agents:
            ...
            for agent in matching_agents:
                matching_caps = set(agent.capabilities.capabilities).intersection(set(capabilities))
        ...
        if len(capabilities) > 1:
            all_matching_agents = searcher.find_agents_with_all_capabilities(capabilities)
...
async def demonstrate_help_request(agents: List[BeastModeBusClient]):
    ...
    for capabilities, description in help_requests:
        ...
        if potential_helpers:
            ...
            for helper in potential_helpers:
                matching_caps = set(helper.capabilities.capabilities).intersection(set(capabilities))
                logger.info(f"  - {helper.agent_id}: can help with {list(matching_caps)}")
    ...
    logger.info(f"  {requester.agent_id} sent {requester.stats['messages_sent']} messages")
    
    for agent in agents[1:]:
        if agent.stats['messages_sent'] > 0:
            logger.info(f"  {agent.agent_id} sent {agent.stats['messages_sent']} responses")
...
async def show_network_statistics(agents: List[BeastModeBusClient]):
    ...
    for agent in agents:
        ...
        logger.info(f"  - Total discovery messages processed: {stats['discovery_messages_processed']}")
    ...
    logger.info(f"  - Average discoveries per agent: {total_discoveries / len(agents):.1f}")
