...
class CollaborationSchedulerDemo:
    ...
    async def demo_office_hours_management(self) -> None:
        ...
        for agent_id, agent in self.agents.items():
            available = agent.is_agent_available_for_collaboration(
                agent_id, current_time
            )
            status = "Available" if available else "Unavailable"
            self.demo_results.append(
                f"  - {agent_id}: {status} at {current_time.strftime('%A %I:%M %p')}"
            )
        ...
        for agent_id, agent in self.agents.items():
            ...
            if next_slot:
                self.demo_results.append(
                    f"  - {agent_id}: Next 30-min slot at {next_slot.strftime('%A %I:%M %p')}"
                )
    ...
    async def demo_collaboration_scheduling(self) -> None:
        ...
        try:
            ...
            self.demo_results.append(
                f"✓ Scheduled collaboration session (Request ID: {request_id[:8]}...)"
            )
            ...
            if sessions:
                ...
                self.demo_results.append(
                    f"  - Scheduled: {session.scheduled_start.strftime('%A %I:%M %p')}"
                )
                self.demo_results.append(
                    f"  - Duration: {(session.scheduled_end - session.scheduled_start).total_seconds() / 60:.0f} minutes"
                )
    ...
    async def demo_collaboration_workflow(self, session_id: str) -> None:
        ...
        self.demo_results.append(
            "✓ ML specialist responded positively to collaboration request"
        )
        ...
        if success:
            ...
            if session:
                ...
                self.demo_results.append(
                    "✓ Collaboration in progress - knowledge being exchanged"
                )
                ...
                success = await python_expert.end_collaboration_session(
                    session_id, success=True, success_metrics=success_metrics
                )

                if success:
                    self.demo_results.append(
                        "✓ Successfully completed collaboration session"
                    )
                    self.demo_results.append(
                        f"  - Satisfaction score: {success_metrics['satisfaction_score']}/10"
                    )
                    self.demo_results.append(
                        f"  - Action items created: {success_metrics['action_items_created']}"
                    )
                    self.demo_results.append(
                        f"  - Follow-up needed: {success_metrics['follow_up_needed']}"
                    )
    ...
    async def demo_asynchronous_collaboration(self) -> None:
        ...
        self.demo_results.append(
            f"✓ Queued offline collaboration (Queue ID: {queue_id[:8]}...)"
        )

        # Simulate agent coming online and processing queue
        queued_requests = (
            python_expert.collaboration_scheduler.process_offline_collaboration_queue(
                "offline_agent"
            )
        )
        ...
        if queued_requests:
            ...
            self.demo_results.append(
                f"  - Found {len(queued_requests)} queued request(s)"
            )
    ...
    async def demo_pattern_recognition(self) -> None:
        ...
        if recommendations:
            self.demo_results.append(
                f"✓ Generated {len(recommendations)} collaboration recommendation(s)"
            )
            ...
            for i, rec in enumerate(
                ...
                self.demo_results.append(
                    f"    - Participants: {', '.join(rec['participants'])}"
                )
                self.demo_results.append(
                    f"    - Success probability: {rec['success_probability']:.1%}"
                )
                self.demo_results.append(
                    f"    - Suggested duration: {rec['suggested_duration']:.0f} minutes"
                )
                self.demo_results.append(
                    f"    - Common topics: {', '.join(rec['common_topics'][:2])}"
                )
                self.demo_results.append(f"    - Confidence: {rec['confidence']:.1%}")
        else:
            self.demo_results.append(
                "✓ Pattern analysis complete (insufficient data for recommendations)"
            )
    ...
    async def demo_collaboration_statistics(self) -> None:
        ...
        total_successful = sum(
            stats["successful_sessions"] for stats in all_stats.values()
        )

        self.demo_results.append(
            f"  - Total sessions across all agents: {total_sessions}"
        )
        ...
        for agent_id, stats in all_stats.items():
            if stats["total_sessions"] > 0:
                ...
                self.demo_results.append(
                    f"    • Success rate: {(stats['successful_sessions']/stats['total_sessions']*100):.1f}%"
                )
                self.demo_results.append(
                    f"    • Avg duration: {stats['average_duration']:.1f} minutes"
                )
                self.demo_results.append(
    ...
    async def demo_collaboration_callbacks(self) -> None:
        ...
        def on_collaboration_response(agent_id: str, payload: Dict[str, Any]) -> None:
            events_captured.append(
                f"Response from {agent_id}: {'Available' if payload.get('available') else 'Unavailable'}"
            )

        def on_collaboration_start(session_id: str, payload: Dict[str, Any]) -> None:
            events_captured.append(
                f"Session started: {payload.get('topic', 'Unknown topic')}"
            )

        def on_collaboration_end(
            session_id: str, success: bool, metrics: Dict[str, Any]
        ) -> None:
            outcome = "successful" if success else "unsuccessful"
            events_captured.append(
                f"Session ended: {outcome} (score: {metrics.get('satisfaction_score', 'N/A')})"
            )

        # Register callbacks
        python_expert.set_collaboration_callback(
            "on_collaboration_response", on_collaboration_response
        )
        python_expert.set_collaboration_callback(
            "on_collaboration_start", on_collaboration_start
        )
        python_expert.set_collaboration_callback(
            "on_collaboration_end", on_collaboration_end
        )
        ...
        if events_captured:
            self.demo_results.append(
                f"✓ Captured {len(events_captured)} collaboration event(s)"
            )
