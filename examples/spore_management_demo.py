...
async def demonstrate_spore_management():
    ...
    print(
        f"✅ Initialized SporeManager with directory: {spore_manager.spore_directory}"
    )
    ...
    if saved_spores:
        ...
        print(
            f"   Requested capabilities: {request_message.payload['requested_capabilities']}"
        )
    ...
    if cost_spore:
        ...
        context = {
            "resources": [
                ...
                {"id": "storage-1", "type": "storage", "access_pattern": "infrequent"},
                {
                    "id": "instance-2",
                    "type": "compute",
                    "size": "medium",
        ...
        try:
            ...
            for opt in result["optimizations"]:
                print(
                    f"  • {opt['resource']}: {opt['action']} (${opt['savings']}/month)"
                )
