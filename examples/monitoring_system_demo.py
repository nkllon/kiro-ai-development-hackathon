...
class MonitoringDemo:
    ...
    def _setup_demo_callbacks(self) -> None:
        ...
        def alert_handler(alert):
            logger.info(f"🚨 ALERT: {alert.name} - {alert.message} (Severity: {alert.severity})")
            
        def recovery_callback(attempt):
            logger.info(f"🔧 RECOVERY: {attempt.action_name} - {attempt.result} ({attempt.message})")
            
        def status_callback(status):
            logger.info(f"📊 STATUS: Health={status.overall_health}, Alerts={status.active_alerts}, "
    ...
    async def _demo_basic_monitoring(self) -> None:
        ...
        for component_name, component_health in health.items():
            logger.info(f"  {component_name}: {component_health.status} - {component_health.message}")
            
        # Get health summary
        summary = await self.system_monitor.health_monitor.get_health_summary()
        logger.info(f"Health Summary: {summary['healthy']} healthy, {summary['degraded']} degraded, "
    ...
    async def _demo_metrics_collection(self) -> None:
        ...
        if "kpis" in report:
            ...
            if "message_throughput" in kpis:
                ...
                logger.info(f"  Messages: {throughput['messages_sent']} sent, "
                           f"{throughput['messages_received']} received")
                           
            if "message_latency" in kpis:
                latency = kpis["message_latency"]
                logger.info(f"  Latency: avg={latency['avg_ms']:.1f}ms, "
    ...
    async def _demo_alerting_system(self) -> None:
        ...
        critical_alert_id = await self.system_monitor.alert_manager.fire_alert(
            ...
            details={"failure_type": "complete_outage", "affected_services": ["messaging", "storage"]}
        )
        ...
        logger.info(f"Alert Summary: {summary['active_alerts']} active, "
    ...
    async def _demo_recovery_system(self) -> None:
        ...
        result = await self.system_monitor.recovery_manager.trigger_recovery("redis_reconnect")
        logger.info(f"Redis reconnect result: {result}")
        
        # Trigger counter reset
        result = await self.system_monitor.recovery_manager.trigger_recovery("reset_message_counters")
        ...
        logger.info(f"Recovery Summary: {summary['active_recoveries']} active, "
    ...
    async def _demo_integrated_workflow(self) -> None:
        ...
        logger.info(f"System Status: Health={status.overall_health}, "
        ...
        if recovery_history:
            ...
            for attempt in recovery_history[-3:]:  # Show last 3
                logger.info(f"  {attempt.action_name}: {attempt.result} - {attempt.message}")
    ...
    async def _demo_performance_monitoring(self) -> None:
        ...
        for batch in range(10):
            ...
            for i in range(20):  # 20 messages per batch
                task = asyncio.create_task(self._simulate_message_processing(batch * 20 + i))
                tasks.append(task)
    ...
    async def _generate_final_report(self) -> None:
        ...
        report_filename = f"monitoring_demo_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
