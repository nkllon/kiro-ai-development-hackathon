...
class CapabilityVerificationDemo:
        ...
        for agent_id, capabilities in interaction_scenarios.items():
            ...
            for capability, interactions in capabilities.items():
                ...
                if trust_key in self.verifier.trust_scores:
                    ...
                    print(
                        f"      {capability}: {trust_score.trust_level.value} "
    ...
    def demonstrate_collaboration_tracking(self) -> None:
        ...
        collaboration = self.help_system.accept_help_response(
            help_request.request_id, response_id
        )

        print(f"   🚀 Started collaboration session: {collaboration.session_id}")
        print(
            f"   👥 Participants: {collaboration.requester_id} ↔ {collaboration.helper_id}"
        )
        ...
        for scenario in scenarios:
            ...
            if recommendations:
                ...
                for i, rec in enumerate(recommendations, 1):
                    ...
                    print(
                        f"         Confidence: {rec.confidence:.2f} | Trust: {rec.trust_score:.2f}"
                    )
    ...
    def demonstrate_agent_reputation(self) -> None:
        ...
        for agent_data in self.demo_agents:
            ...
            print(
                f"   🎖️  Overall Trust: {reputation['overall_trust_level']} "
                f"(score: {reputation['overall_trust_score']:.2f})"
            )
            print(
                f"   📈 Success Rate: {reputation.get('overall_success_rate', 0):.1%}"
            )
            ...
            if reputation["capabilities"]:
                ...
                for cap_name, cap_data in sorted_caps[:3]:
                    print(
                        f"      • {cap_name}: {cap_data['trust_level']} "
    ...
    def display_system_statistics(self) -> None:
        ...
        print(
            f"   💡 Recommendations Generated: {verification_stats['recommendations_generated']}"
        )
        ...
        print(
            f"   🔍 Discovery Messages: {registry_stats['discovery_messages_processed']}"
        )
        ...
        print(
            f"   ✅ Collaborations Completed: {help_stats['collaborations_completed']}"
        )
