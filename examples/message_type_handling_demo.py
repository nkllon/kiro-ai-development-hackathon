...
from src.beast_mode.messaging.message_router import (
...
class MessageTypeDemo:
    ...
    def _handle_prompt_response(
        self, source: str, response: str, original_prompt: str
    ) -> None:
        """Handle prompt response"""
        logger.info(f"💡 Received response from {source}: {response}")

    def _handle_agent_discovery(
        self, source: str, capabilities: AgentCapabilities
    ) -> None:
        """Handle agent discovery"""
        logger.info(
            f"🔍 Discovered agent {source} with capabilities: {capabilities.capabilities}"
        )
    ...
    def _handle_agent_response(
        ...
        logger.info(
            f"👋 Agent {source} responded with capabilities: {capabilities.capabilities}"
        )

    def _handle_help_wanted(
        self, source: str, required_capabilities: list, description: str
    ) -> bool:
    ...
    def _handle_help_response(self, source: str, response_data: Dict[str, Any]) -> None:
        ...
        logger.info(
            f"🤝 Help response from {source}: can_help={can_help}, confidence={confidence:.2f}"
        )

    def _handle_spore_delivery(
        self, source: str, spore_name: str, spore_data: Dict[str, Any]
    ) -> None:
    ...
    async def _demo_prompt_request_response(self):
        ...
        if responses:
            ...
            logger.info(
                f"Generated response: {response.payload.get('response', 'No response')}"
            )
    ...
    async def _demo_agent_discovery(self):
        ...
        if responses:
            ...
            logger.info(
                f"Our capabilities: {our_response.payload['agent_capabilities']['capabilities']}"
            )
    ...
    async def _demo_spore_system(self):
        ...
        spore_request = self.router.create_test_message(
            MessageType.SPORE_REQUEST, spore_name="demo_spore"
        )
    ...
    async def _demo_validation_and_compatibility(self):
        ...
        for msg_type in MessageType:
            ...
            logger.info(
                f"  {msg_type.value}: {len(type_info.get('required_fields', []))} required fields"
            )
    ...
    def _show_statistics(self):
        ...
        logger.info(f"\nRouter statistics:")
        logger.info(
            f"  Messages routed: {router_stats['router_stats']['messages_routed']}"
        )
        logger.info(
            f"  Messages handled: {router_stats['router_stats']['messages_handled']}"
        )
        logger.info(
            f"  Validation errors: {router_stats['router_stats']['validation_errors']}"
        )
        logger.info(
            f"  Handler errors: {router_stats['router_stats']['handler_errors']}"
        )
...
async def demonstrate_bus_client_integration():
    ...
    client.set_message_callback(
        "on_simple_message",
        lambda source, content: logger.info(
            f"Bus client received: {content} from {source}"
        ),
    )
    ...
    test_messages = [
        client.create_test_message(
            MessageType.SIMPLE_MESSAGE, content="Hello from bus client"
        ),
