...
class HelpSystemDemo:
        ...
        for client in self.clients.values():
            for other_client in self.clients.values():
                if client.agent_id != other_client.agent_id:
                    other_agent = other_client.agent_registry.agents[
                        other_client.agent_id
                    ]
    ...
    async def demo_basic_help_request(self):
        ...
        for match in matches[:3]:  # Show top 3 matches
            logger.info(
                f"  - {match['agent_id']}: {match['match_score']:.2f} match score, "
        ...
        if matches:
            ...
            help_response = helper_client.help_system.process_help_request(
                help_request_message, helper_client.agent_id
            )

            if help_response:
                logger.info(
                    f"{best_helper['agent_id']} responded with confidence {help_response.confidence_score:.2f}"
                )
                logger.info(
                    f"Matching capabilities: {help_response.matching_capabilities}"
                )
                ...
                # Accept the help
                success = requester.accept_help_response(
                    request_id, help_response.response_id
                )
                if success:
                    logger.info(
                        "Help response accepted! Collaboration session started."
                    )
                    ...
                    if sessions:
                        ...
                        logger.info(
                            f"Capabilities being used: {session['capabilities_used']}"
                        )
                        ...
                        if success:
                            ...
                            logger.info(
                                "Metrics: 12 tests written, 45% coverage improvement"
                            )
    ...
    async def demo_multiple_helpers(self):
        ...
        matches = requester.find_agents_for_capabilities(
        ...
        for agent_id, response in responses:
            if accepted_count < 2:  # max_helpers = 2
                ...
                if success:
                    logger.info(
                        f"Accepted help from {agent_id} (confidence: {response.confidence_score:.2f})"
                    )
                    accepted_count += 1
        ...
        for session in active_sessions:
            logger.info(
                f"  - Session with {session['helper_id']}: {session['capabilities_used']}"
            )
        ...
        for test_case in test_cases:
            ...
            if matches:
                ...
                for match in matches:
                    ...
                    logger.info(
                        f"  - {match['agent_id']}: {match['match_score']:.3f} "
    ...
    async def demo_urgency_levels(self):
        ...
        for urgency, description in urgency_tests:
            ...
            logger.info(
                f"  Message priority: {message.priority} (1=highest, 10=lowest)"
            )
    ...
    async def demo_statistics_tracking(self):
        ...
        logger.info(
            f"  Collaborations started: {initial_stats['collaborations_started']}"
        )
        logger.info(
            f"  Collaborations completed: {initial_stats['collaborations_completed']}"
        )
        ...
        logger.info(
            f"  Total agents discovered: {registry_stats['total_agents_discovered']}"
        )
