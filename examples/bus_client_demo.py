...
async def demo_basic_functionality():
    ...
    try:
        ...
        if not (connected1 and connected2):
            print(
                "❌ Failed to connect to Redis. Make sure Redis is running on localhost:6379"
            )
            return
        ...
        print(
            f"   Agent 1: {health1['agent_id']} - Connected: {health1['is_connected']}"
        )
        ...
        await client1.send_simple_message(
            "Hello from Agent 1! This is a test message.", target="demo_agent_2"
        )
        ...
        await client1.send_simple_message("This is a broadcast message to all agents!")
        await asyncio.sleep(0.3)
        ...
        print(
            f"   Agent 1 - Sent: {stats1['messages_sent']}, Received: {stats1['messages_received']}"
        )
        print(
            f"   Agent 2 - Sent: {stats2['messages_sent']}, Received: {stats2['messages_received']}"
        )
...
async def demo_error_handling():
    ...
    bad_client = BeastModeBusClient(
        redis_url="redis://nonexistent:6379", agent_id="error_test"
    )
