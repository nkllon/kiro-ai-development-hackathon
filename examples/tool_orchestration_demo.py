...
def main():
    ...
    for tool_id, tool_info in all_tools.items():
        health_icon = (
            "🟢"
            if tool_info["health_status"] == "healthy"
            else "🔴" if tool_info["health_status"] == "failed" else "🟡"
        )
        print(
            f"      {health_icon} {tool_info['name']} ({tool_info['priority']} priority)"
        )
    ...
    print(
        f"   Available Options: {', '.join(high_confidence_context.available_options)}"
    )
    ...
    hc_result = orchestration_engine.orchestrate_tool_execution(
        high_confidence_context, preferred_tools=["project_structure", "git_status"]
    )
    ...
    if hc_result.primary_result:
        ...
        print(
            f"         Execution Time: {hc_result.primary_result.execution_time_ms}ms"
        )
        if hc_result.primary_result.output:
            output_preview = (
                hc_result.primary_result.output[:100] + "..."
                if len(hc_result.primary_result.output) > 100
                else hc_result.primary_result.output
            )
    ...
    print(
        f"   Stakeholder Requirements: {len(low_confidence_context.stakeholder_requirements)} perspectives"
    )
    ...
    lc_result = orchestration_engine.orchestrate_tool_execution(low_confidence_context)
    ...
    for tool_id, health_status in health_refresh["health_status"].items():
        health_icon = (
            ...
            else "🔴" if health_status == "failed" else "🟡"
        )
    ...
    repair_result = orchestration_engine.orchestrate_tool_execution(
        repair_context, preferred_tools=["failing_tool"]
    )
    ...
    print(
        f"      Overall Success Rate: {decision_analytics.get('overall_success_rate', 0):.1%}"
    )
    print(
        f"      Average Execution Time: {decision_analytics.get('average_execution_time_ms', 0):.0f}ms"
    )
    ...
    for level, count in confidence_dist.items():
        percentage = (
            count / max(1, decision_analytics.get("total_decisions", 1))
        ) * 100
    ...
    print(
        f"      Intelligence Engine: {'🟢 Healthy' if decision_framework.get('intelligence_engine_healthy') else '🔴 Unhealthy'}"
    )
    print(
        f"      RCA Engine: {'🟢 Healthy' if decision_framework.get('rca_engine_healthy') else '🔴 Unhealthy'}"
    )
    print(
        f"      Multi-Perspective Engine: {'🟢 Healthy' if decision_framework.get('multi_perspective_engine_healthy') else '🔴 Unhealthy'}"
    )
