...
def analyze_navigation_element(element, element_type, page):
    ...
    try:
        ...
        href = element.get_attribute("href") or "" if hasattr(element, 'get_attribute') else ""
        src = element.get_attribute("src") or "" if hasattr(element, 'get_attribute') else ""
        onclick = element.get_attribute("onclick") or "" if hasattr(element, 'get_attribute') else ""
        ...
        try:
            parent = element.evaluate("el => el.parentElement") if hasattr(element, 'evaluate') else None
        ...
        if element_type == "devpost_next":
            ...
            if "next" in classes or "next" in text:
                ...
                analysis["confidence"] = 0.9
                analysis["action"] = "backward"
                analysis["reasoning"].append("DevPost step link with previous indicator")
                ...
                analysis["reasoning"].append("DevPost completed step (low navigation priority)")
            else:
        ...
        elif any(word in text for word in ["next", "continue", "save and continue", "proceed", "forward", "→", ">"]):
        ...
        elif any(word in text for word in ["submit", "finish", "complete", "done", "save"]):
            analysis["confidence"] = 0.7
        ...
        if element_type == "image":
            ...
            if any(word in text for word in ["next", "continue", "forward", "back", "previous", "arrow", "nav"]):
                analysis["confidence"] = 0.8
                analysis["action"] = "navigate"
            ...
            if any(word in src.lower() for word in ["arrow", "next", "forward", "back", "nav"]):
        ...
        # Link analysis
            ...
            if any(word in href.lower() for word in ["next", "continue", "forward", "step", "page"]):
                analysis["confidence"] = 0.7
                analysis["action"] = "navigate"
        ...
        # Header element analysis
            ...
            if any(word in classes.lower() for word in ["nav", "navigation", "menu", "breadcrumb"]):
                analysis["confidence"] += 0.3
        ...
        # Clickable div analysis
            ...
            if any(word in onclick.lower() for word in ["next", "continue", "forward", "back", "submit"]):
...
def analyze_button_context(button, page):
    ...
    try:
        ...
        if any(word in text for word in ["next", "continue", "save and continue", "proceed"]):
            analysis["confidence"] = 0.9
            analysis["action"] = "forward"
            analysis["reasoning"].append("Contains forward navigation keywords")
...
def make_navigation_decision(button_analyses, page_url, page_title):
    ...
    medium_confidence_buttons = [b for b in button_analyses if 0.4 <= b["confidence"] < 0.7]
    ...
    for i, analysis in enumerate(button_analyses, 1):
        ...
        print(f"   {i}. {analysis['text']} [{confidence_bar}] {analysis['confidence']:.1f} - {analysis['action']}")
    ...
    if high_confidence_buttons:
        ...
        for i, analysis in enumerate(medium_confidence_buttons, 1):
            print(f"   {i}. {analysis['text']} (confidence: {analysis['confidence']:.1f})")
...
def main():
    ...
    try:
        ...
        while navigation_count < max_navigations:
            ...
            steps_navigation = target_page.query_selector_all("#steps-navigation a.step")
            ...
            clickable_divs = target_page.query_selector_all("div[onclick], div[role='button'], div[tabindex]")
            ...
            all_elements.extend([(link, "devpost_completed") for link in completed_links])
            ...
            for i, (element, element_type) in enumerate(all_elements):
                try:
                    analysis = analyze_navigation_element(element, element_type, target_page)
                    ...
                    try:
                        text = element.text_content().strip() if hasattr(element, 'text_content') else "unknown"
            ...
            decision, confidence = make_navigation_decision(element_analyses, target_page.url, target_page.title())
