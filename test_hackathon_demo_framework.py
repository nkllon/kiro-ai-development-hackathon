...
from hackathon_demo_framework.controllers.hackathon_demo_controller import (
    HackathonDemoController, DemoReadinessLevel
)
...
def test_demo_script_generation():
    ...
    demo_script = demo_controller.generate_demo_script(project_info, time_limit_minutes=5)
...
def test_judge_engagement_optimization():
    ...
    engagement_analysis = demo_controller.optimize_judge_engagement(demo_script, judging_criteria)
    ...
    print(f"   Value Proposition Clarity: {engagement_analysis['value_proposition_clarity']:.3f}")
    print(f"   Technical Balance: {engagement_analysis['technical_balance']:.3f}")
    print(f"   Differentiation Highlight: {engagement_analysis['differentiation_highlight']:.3f}")
    print(f"   Closing Impact: {engagement_analysis['closing_impact']:.3f}")
    print(f"   Improvement Recommendations: {len(engagement_analysis['improvement_recommendations'])}")
...
def test_demo_readiness_assessment():
    ...
    demo_script = demo_controller.generate_demo_script(project_info, time_limit_minutes=5)
    engagement_analysis = demo_controller.optimize_judge_engagement(demo_script, ["Innovation", "Impact"])
    
    # Test demo readiness assessment
    print("\n📊 Testing Demo Readiness Assessment:")
    readiness_level = demo_controller.assess_demo_readiness(validation_result, engagement_analysis)
...
def test_end_to_end_workflow():
    ...
    validation_result = demo_controller.validate_technical_completeness("hackathon_project")
    print(f"      ✅ Technical Score: {validation_result.overall_score:.3f}")
    ...
    demo_script = demo_controller.generate_demo_script(project_info, time_limit_minutes=5)
    print(f"      ✅ Demo Script: {len(demo_script.sections)} sections, {demo_script.duration_minutes} minutes")
    
    # Step 3: Judge engagement optimization
    print(f"\n   3. Judge Engagement Optimization:")
    judging_criteria = ["Technical Innovation", "Business Impact", "Presentation Quality"]
    engagement_analysis = demo_controller.optimize_judge_engagement(demo_script, judging_criteria)
    print(f"      ✅ Engagement Score: {engagement_analysis['engagement_score']:.3f}")
