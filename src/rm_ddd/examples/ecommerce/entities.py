"""
E-commerce entities demonstrating RM-DDD patterns.

This module contains the core e-commerce entities with proper identity,
lifecycle management, and business behavior.
"""
...
class Product(Entity[ProductId]):
    ...
    def update_price(self, new_price: Money):
        ...
        if new_price.currency != self.price.currency:
            raise DomainException(
                f"Currency mismatch: expected {self.price.currency}, got {new_price.currency}",
    ...
    def get_domain_boundaries(self) -> DomainBoundaries:
        ...
        return DomainBoundaries(
            ...
            invariants=[
                ...
                "Customer must have first and last name",
                "Email address must be unique across customers",
                "Addresses must be valid"
            ],
            ubiquitous_language={
                "Customer": "A registered user who can place orders",
                "Email": "Primary contact method for customer",
                "Address": "Physical location for shipping or billing",
                "Account": "Customer's registration and profile information"
            }
        )
...
class OrderItem(Entity[str]):
    ...
    def __init__(self,
                 ...
                 quantity: Quantity,
                 unit_price: Money):
        # Use composite ID for order items
...
class Order(Entity[OrderId]):
    ...
    def add_item(self, product_id: ProductId, quantity: Quantity, unit_price: Money):
        ...
        if self.status != "pending":
            raise DomainException(
                f"Cannot add items to order in {self.status} status",
    ...
    def calculate_subtotal(self) -> Money:
        ...
        # Assume all items have the same currency as the first item
    ...
    def set_shipping_address(self, address: Address):
        ...
        if not address_validation.is_valid:
            raise DomainException(
                f"Invalid shipping address: {address_validation.errors}",
    ...
    def set_billing_address(self, address: Address):
        ...
        if not address_validation.is_valid:
            raise DomainException(
                f"Invalid billing address: {address_validation.errors}",
                error_code="INVALID_BILLING_ADDRESS"
            )
    ...
    def confirm(self):
        ...
        if not self.shipping_address:
            raise DomainException(
                "Shipping address required for order confirmation",
    ...
    def ship(self):
        ...
        if self.status != "confirmed":
            raise DomainException(
                f"Cannot ship order in {self.status} status",
                error_code="INVALID_ORDER_STATUS"
            )
    ...
    def deliver(self):
        ...
        if self.status != "shipped":
            raise DomainException(
                f"Cannot deliver order in {self.status} status",
                error_code="INVALID_ORDER_STATUS"
            )
    ...
    def get_domain_boundaries(self) -> DomainBoundaries:
        ...
        return DomainBoundaries(
            context="order_management",
            invariants=[
                "Order must have at least one item when confirmed",
            ...
            ubiquitous_language={
                ...
                "Confirmation": "Customer's commitment to purchase",
                "Shipping": "Physical delivery of products",
                "Status": "Current state of order processing"
            }
        )
