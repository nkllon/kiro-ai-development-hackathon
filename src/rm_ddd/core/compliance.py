"""
Compliance validation system for RM-DDD components.

This module provides systematic compliance validation for all RM components,
ensuring adherence to domain-driven design principles, RM standards, and
...
class ValidationResult:
    ...
    def add_critical(self, message: str, code: str = "VALIDATION_CRITICAL",
                    component: str = "unknown", context: Optional[Dict[str, Any]] = None):
...
class RMComplianceValidator(ComplianceValidator):
    ...
    def _setup_default_rules(self):
        ...
        self.add_validation_rule("module_id_required", {
            "description": "Module must have a valid module_id",
            "severity": "error"
        })
        
        self.add_validation_rule("health_check_implemented", {
            "description": "Module must implement health check methods",
            "severity": "error"
        })
        
        self.add_validation_rule("capabilities_defined", {
            "description": "Module must define its capabilities",
            "severity": "warning"
        })
        
        self.add_validation_rule("registry_integration", {
...
class DDDComplianceValidator(ComplianceValidator):
    ...
    def _setup_default_rules(self):
        ...
        self.add_validation_rule("domain_boundaries_defined", {
            "description": "Domain components must define clear boundaries",
            "severity": "error"
        })
        
        self.add_validation_rule("invariants_validated", {
            "description": "Domain components must validate invariants",
            "severity": "error"
        })
        
        self.add_validation_rule("ubiquitous_language", {
            "description": "Components should use ubiquitous language",
            "severity": "warning"
        })
        
        self.add_validation_rule("domain_context_specified", {
    ...
    async def validate(self, component: 'ReflectiveModuleBase') -> ValidationResult:
        ...
        # Only validate domain components
        if not hasattr(component, 'domain_context'):
            return result  # Not a domain component, skip DDD validation
        
        domain_component = component  # Type hint for domain-specific methods
...
class ComplianceOrchestrator:
    ...
    async def validate_component(self, component: 'ReflectiveModuleBase') -> ComplianceReport:
        ...
        if total_issues == 0:
            ...
            # Violations are weighted more heavily than warnings
            violation_weight = 10
            warning_weight = 1
            total_weight = len(all_violations) * violation_weight + len(all_warnings) * warning_weight
            max_weight = total_issues * violation_weight  # If all were violations
...
_global_compliance_orchestrator: Optional[ComplianceOrchestrator] = None
