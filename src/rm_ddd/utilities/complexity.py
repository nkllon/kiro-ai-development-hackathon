...
class ComplexityMonitor(DomainReflectiveModule):
    ...
    def _add_basic_metrics(self, target: Any, report: ComplexityReport):
        ...
        try:
            ...
            if inspect.isclass(target):
                method_count = len([m for m in inspect.getmembers(target, predicate=inspect.isfunction)
    ...
    def get_complexity_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'average_score': sum(r.get_overall_score() for r in self._reports.values()) / len(self._reports),
...
def check_complexity_thresholds(target: Any, 
                               cyclomatic_threshold: float = 10.0,
                               cognitive_threshold: float = 15.0) -> bool:
    """
