"""
Separation Core Core Core

This module was extracted from separation_core_core.py
as part of RM-DDD compliance refactoring.
"""

"""
Separation - Consolidated Interface Definition
...
authoritative source for separation.

Consolidated from: /Users/lou/kiro-2/kiro-ai-development-hackathon/src/rm_ddd/infrastructure/separation_core_core_core.py
Consolidation date: 2025-09-13T10:15:07.520892
"""
...
class DependencyValidator(DomainReflectiveModule):
    ...
    def _determine_class_layer(self, cls: Type) -> Optional[LayerType]:
        ...
        elif 'presentation' in module_name or 'api' in module_name or 'web' in module_name:
    ...
    def _determine_class_layer_by_name(self, class_name: str) -> Optional[LayerType]:
        ...
        if any((pattern in class_name_lower for pattern in ['repository', 'adapter', 'client', 'gateway', 'dao', 'orm'])):
            return LayerType.INFRASTRUCTURE
        if any((pattern in class_name_lower for pattern in ['entity', 'aggregate', 'valueobject', 'domainservice'])):
            return LayerType.DOMAIN
        if any((pattern in class_name_lower for pattern in ['applicationservice', 'usecase', 'command', 'query'])):
            return LayerType.APPLICATION
        return None
        ...
        try:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.ClassDef) and node.name == cls.__name__:
                    class_deps = self._extract_dependencies_from_class_node(node, source_file)
    ...
    def _extract_dependencies_from_class_node(self, class_node: ast.ClassDef, file_path: str) -> Dict[str, Dict[str, Any]]:
        ...
        for base in class_node.bases:
            if isinstance(base, ast.Name):
                dependencies[base.id] = {'type': 'inheritance', 'file_path': file_path, 'line_number': base.lineno}
        for node in ast.walk(class_node):
            if isinstance(node, ast.Name):
                if node.id not in ['str', 'int', 'float', 'bool', 'list', 'dict', 'set', 'tuple']:
                    dependencies[node.id] = {'type': 'reference', 'file_path': file_path, 'line_number': getattr(node, 'lineno', 0)}
        return dependencies
            ...
            if isinstance(node, ast.Import):
                ...
                if node.module:
                    for alias in node.names:
                        ...
                        imports[full_name] = {'type': 'from_import', 'line_number': node.lineno}
        if 'domain' in file_path and 'infrastructure' not in file_path:
            for import_name, import_info in imports.items():
                if any((infra_pattern in import_name.lower() for infra_pattern in ['sqlalchemy', 'django', 'flask', 'requests', 'boto3', 'redis'])):
                    violation = LayerViolation(violating_class=Path(file_path).stem, violating_method=None, dependency_class=import_name, violation_type='infrastructure_import_in_domain', layer_from=LayerType.DOMAIN, layer_to=LayerType.INFRASTRUCTURE, message=f'Domain module {file_path} imports infrastructure dependency {import_name}', file_path=file_path, line_number=import_info['line_number'])
    ...
    async def get_module_status(self):
        ...
        from ..core.health import ModuleHealth
        from ..models import ModuleStatus
        return ModuleHealth(status=ModuleStatus.AVAILABLE, message=f'Dependency validator with {len(self._violations)} violations detected', capabilities=await self.get_module_capabilities(), health_indicators={'violations_count': len(self._violations), 'scanned_modules': len(self._scanned_modules), 'last_scan': datetime.now().isoformat()})

    async def get_module_capabilities(self):
        """Get module capabilities."""
        from ..models import ModuleCapability
        return [ModuleCapability(name='dependency_validation', description='Validates architectural layer dependencies', available=True, version='1.0.0')]
    ...
    def domain_layer(self, cls: Type) -> Type:
        ...
        if self._enforcement_enabled:
            ...
            if not validation_result.is_valid:
                raise DomainException(f'Domain layer violation in {cls.__name__}: {validation_result.errors}', error_code='DOMAIN_LAYER_VIOLATION')
        return cls
