...
class ContextIntegration:
    ...
    data_flow: str = "bidirectional"  # bidirectional, upstream_to_downstream, downstream_to_upstream
    ...
    def validate_integration(self) -> ValidationResult:
        ...
        if (self.relationship_type == ContextRelationshipType.SHARED_KERNEL and 
...
class BoundedContext(DomainReflectiveModule):
    ...
    def add_integration(self, integration: ContextIntegration):
        ...
        other_context = (integration.downstream_context 
    ...
    def detect_boundary_violation(self, 
                                 component_name: str, 
                                 external_dependency: str) -> bool:
        """
        ...
        for integration in self._integrations.values():
            if (external_dependency.startswith(integration.upstream_context) or
                external_dependency.startswith(integration.downstream_context)):
                # Dependency is through a defined integration
                return False
        
        # Check if dependency is on infrastructure (allowed)
        if any(infra_pattern in external_dependency.lower() for infra_pattern in [
            'infrastructure', 'repository', 'adapter', 'client'
        ]):
            return False
        
        # Check if dependency is on shared kernel
        shared_kernel_contexts = [
            integration.upstream_context for integration in self._integrations.values()
    ...
    async def get_module_status(self):
        ...
        return ModuleHealth(
            ...
            health_indicators={
                ...
                'registered_components': (len(self._registered_entities) + 
...
class ContextMapper(DomainReflectiveModule):
    ...
    def create_integration(self, 
        ...
        logger.info(f"Created integration: {upstream_context} -> {downstream_context} "
    ...
    def _detect_circular_dependencies(self) -> List[str]:
        ...
        for integration in self._global_integrations:
            ...
            reverse_integration = next(
                (i for i in self._global_integrations
                 if (i.upstream_context == integration.downstream_context and
                     i.downstream_context == integration.upstream_context)),
                None
            )
    ...
    def get_context_relationships(self, context_name: str) -> List[ContextIntegration]:
        ...
        """
        return [
            integration for integration in self._global_integrations
            if (integration.upstream_context == context_name or
                integration.downstream_context == context_name)
        ]
    ...
    def generate_context_map_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'validation_summary': self.validate_context_map().to_dict() if hasattr(ValidationResult, 'to_dict') else {},
            'circular_dependencies': self._detect_circular_dependencies(),
            'total_contexts': len(self._contexts),
            'total_integrations': len(self._global_integrations)
        }
    
    def suggest_integration_patterns(self, 
                                   upstream_context: str,
                                   downstream_context: str) -> List[Dict[str, Any]]:
        """
        ...
        upstream_size = (len(upstream._registered_entities) + 
                        len(upstream._registered_services))
        downstream_size = (len(downstream._registered_entities) + 
                          len(downstream._registered_services))
