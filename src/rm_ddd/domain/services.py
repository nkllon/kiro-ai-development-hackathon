"""
Domain Service base classes for DDD patterns.

This module provides systematic implementations of DDD domain services with
built-in RM compliance, statelessness enforcement, and domain validation.
"""
...
class DomainService(DomainReflectiveModule, ABC):
    ...
    def _validate_statelessness(self) -> ValidationResult:
        ...
        # Filter out allowed variables (those that start with _)
        problematic_variables = [var for var in new_variables if not var.startswith('_')]
        
        if problematic_variables:
            result.add_error(
                f"Domain service has gained state variables: {problematic_variables}",
                code="DS_001",
                component=self.__class__.__name__,
                context={"new_variables": problematic_variables}
            )
        
        # Check for modified initial variables (excluding private ones)
        for var_name in self._instance_variables_at_init:
            if not var_name.startswith('_') and hasattr(self, var_name):
                # This is a simplified check - in practice, you might want more sophisticated state tracking
                pass
    ...
    def validate_service_constraints(self) -> ValidationResult:
        ...
        if not self.service_name or not self.service_name.strip():
            result.add_error(
                "Domain service must have a valid service name",
                code="DS_002",
                component=self.__class__.__name__
            )
        ...
        if not self.domain_context or not self.domain_context.strip():
            result.add_error(
                "Domain service must have a valid domain context",
                code="DS_003",
                component=self.__class__.__name__
            )
    ...
    def get_service_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "stateless_validation_enabled": self._stateless_validation_enabled,
        ...
        for capability_name in self.get_service_capabilities():
            capabilities.append(
                ModuleCapability(
                    ...
                    description=f"Domain service capability: {capability_name}",
    ...
    async def get_health_indicators(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "service_type": self.__class__.__name__,
            "domain_context": self.domain_context,
            "is_stateless": len(service_validation.errors) == 0,
            "service_valid": service_validation.is_valid,
            "domain_valid": domain_validation.is_valid,
            "operation_count": self._operation_count,
            "last_operation": self._last_operation_time.isoformat() if self._last_operation_time else None,
            "validation_errors": len(service_validation.errors) + len(domain_validation.errors),
            "validation_warnings": len(service_validation.warnings) + len(domain_validation.warnings),
            "capabilities": self.get_service_capabilities()
        }
...
class ApplicationService(DomainService):
    ...
    def get_service_capabilities(self) -> List[str]:
        ...
        base_capabilities.extend([
            f"{self.service_name}_transaction_management",
            f"{self.service_name}_workflow_orchestration"
        ])
    ...
    async def get_health_indicators(self) -> Dict[str, Any]:
        ...
        # Add transaction-specific indicators
        base_indicators.update({
            "transaction_count": self._transaction_count,
            "failed_transaction_count": self._failed_transaction_count,
            "transaction_success_rate": (
                (self._transaction_count - self._failed_transaction_count) / max(self._transaction_count, 1)
            ) * 100 if self._transaction_count > 0 else 100.0
        })
...
class InfrastructureService(DomainService):
    ...
    def get_service_capabilities(self) -> List[str]:
        ...
        base_capabilities = super().get_service_capabilities()
        base_capabilities.extend([
            f"{self.service_name}_external_integration",
            f"{self.service_name}_infrastructure_support"
        ])
        return base_capabilities
    ...
    async def get_health_indicators(self) -> Dict[str, Any]:
        ...
        # Add infrastructure-specific indicators
        base_indicators.update({
            "external_call_count": self._external_call_count,
            "external_failure_count": self._external_failure_count,
            "external_success_rate": (
                (self._external_call_count - self._external_failure_count) / max(self._external_call_count, 1)
            ) * 100 if self._external_call_count > 0 else 100.0
        })
...
def create_domain_service(service_class: type, domain_context: str, 
                         service_name: str, **kwargs) -> DomainService:
    """
    ...
    if not issubclass(service_class, DomainService):
        raise DomainException(
            f"Service class {service_class.__name__} must inherit from DomainService",
            error_code="INVALID_SERVICE_CLASS"
        )
    ...
    try:
        ...
        raise DomainException(
            f"Failed to create domain service: {str(e)}",
