...
authoritative source for repositories.
...
class RepositoryRM(Repository[T, ID], DomainReflectiveModule):
    ...
    async def get_health_indicators(self) -> Dict[str, Any]:
        ...
        success_rate = (total_operations - self._error_count) / max(total_operations, 1) * 100 if total_operations > 0 else 100.0
...
def create_repository(repository_class: type, domain_context: str, entity_type: str, **kwargs) -> RepositoryRM:
    ...
    if not issubclass(repository_class, RepositoryRM):
        raise DomainException(f'Repository class {repository_class.__name__} must inherit from RepositoryRM', error_code='INVALID_REPOSITORY_CLASS')
    try:
