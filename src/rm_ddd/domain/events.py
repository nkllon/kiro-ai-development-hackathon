"""
Events Core Core Core

This module was extracted from events_core_core.py
as part of RM-DDD compliance refactoring.
"""

"""
Events - Consolidated Interface Definition
...
Consolidated from: /Users/lou/kiro-2/kiro-ai-development-hackathon/src/rm_ddd/domain/events_core_core_core.py
Consolidation date: 2025-09-13T10:15:07.521631
"""
...
class DomainEvent(ABC):
    ...
    def __init__(self, aggregate_id: Any, event_version: int=1, correlation_id: Optional[UUID]=None, causation_id: Optional[UUID]=None, user_id: Optional[str]=None):
        ...
        """
        self.metadata = EventMetadata(correlation_id=correlation_id, causation_id=causation_id, version=event_version, user_id=user_id)
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if not self._validated:
            ...
            if not validation_result.is_valid:
                raise DomainException(f'Invalid event: {validation_result.errors}', error_code='INVALID_EVENT')
...
class DomainEventPublisher(DomainReflectiveModule):
    ...
    async def publish(self, event: DomainEvent):
        ...
        try:
            ...
            if not validation_result.is_valid:
                raise DomainException(f'Cannot publish invalid event: {validation_result.errors}', error_code='INVALID_EVENT_PUBLICATION')
            ...
            raise DomainException(f'Event publication failed: {str(e)}', error_code='EVENT_PUBLICATION_FAILED')
    ...
    async def _handle_event(self, event: DomainEvent):
        ...
        capable_handlers = [handler for handler in handlers_to_notify if handler.can_handle(event.event_type)]
    ...
    async def get_module_status(self):
        ...
        return ModuleHealth(status=status, message=f'Domain event publisher for {self.domain_context}', capabilities=await self.get_module_capabilities(), health_indicators={'success_rate': success_rate, 'published_events': self._published_events, 'queue_size': self._processing_queue.qsize(), 'handler_count': sum((len(handlers) for handlers in self._handlers.values())) + len(self._global_handlers)})

    async def get_module_capabilities(self):
        """Get module capabilities."""
        return [ModuleCapability(name='domain_event_publishing', description='Publishes and handles domain events', available=True, version='1.0.0')]
