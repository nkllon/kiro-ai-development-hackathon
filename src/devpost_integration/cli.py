...
class DevPostCLI(ReflectiveModule):
    ...
    def create_parser(self) -> argparse.ArgumentParser:
        ...
        interrogate_parser = subparsers.add_parser('interrogate', help='Interrogate DevPost submission form')
        interrogate_parser.add_argument('url', help='DevPost submission URL')
        interrogate_parser.add_argument('--output', help='Output file for form model (JSON)')
        interrogate_parser.add_argument('--headless', action='store_true', help='Run in headless mode')
    ...
    def interrogate_command(self, args) -> Dict[str, Any]:
        ...
        try:
            ...
            input("Press Enter when you're on the submission page and ready to interrogate...")
            ...
            with DevPostFormInterrogation(headless=args.headless) as interrogation:
                ...
                for i, section in enumerate(model.sections, 1):
                    ...
                    for j, field in enumerate(section.fields, 1):
                        ...
                        if field.help_text:
                            print(f"      Help: {field.help_text[:50]}{'...' if len(field.help_text) > 50 else ''}")
                        print()
                ...
                # Save model to file
                output_file = args.output or f"devpost_submission_model_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
