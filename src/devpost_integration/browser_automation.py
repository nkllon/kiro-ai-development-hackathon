...
class DevPostBrowserAutomation(ReflectiveModule):
    ...
    async def initialize_async(self) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Browser automation initialized: {self.browser_type} (headless={self.headless})"
            )
            return True
    ...
    def initialize_sync(self) -> bool:
        ...
        try:
            ...
            if self.browser_type == "chromium":
                ...
                self.sync_browser = self.sync_playwright.firefox.launch(
                    headless=self.headless
                )
            elif self.browser_type == "webkit":
                self.sync_browser = self.sync_playwright.webkit.launch(
                    headless=self.headless
                )
            else:
                raise ValueError(f"Unsupported browser type: {self.browser_type}")
            ...
            logger.info(
                f"Sync browser automation initialized: {self.browser_type} (headless={self.headless})"
            )
            return True
    ...
    async def extract_hackathon_data_async(
        ...
        try:
            ...
            title = (
                await title_element.text_content() if title_element else "Unknown Title"
            )

            # Extract description
            description_element = await self.page.query_selector(
                "[data-testid='hackathon-description'], .hackathon-description, .description"
            )
            description = (
            ...
            # Extract deadline
            deadline_element = await self.page.query_selector(
                "[data-testid='deadline'], .deadline, .submission-deadline"
            )
            deadline = (
                await deadline_element.text_content()
                if deadline_element
                else "No deadline found"
            )

            # Extract requirements
            requirements = []
            req_elements = await self.page.query_selector_all(
                "[data-testid='requirement'], .requirement, .submission-requirement"
            )
            ...
            # Extract prizes
            prizes = []
            prize_elements = await self.page.query_selector_all(
                "[data-testid='prize'], .prize, .award"
            )
            ...
            # Extract sponsors
            sponsors = []
            sponsor_elements = await self.page.query_selector_all(
                "[data-testid='sponsor'], .sponsor, .partner"
            )
            ...
            guidelines_element = await self.page.query_selector(
                "[data-testid='guidelines'], .guidelines, .submission-guidelines"
            )
            submission_guidelines = (
                await guidelines_element.text_content()
                if guidelines_element
    ...
    def extract_hackathon_data_sync(self, hackathon_url: str) -> DevPostHackathonData:
        ...
        try:
            ...
            # Extract description
            description_element = self.sync_page.query_selector(
                "[data-testid='hackathon-description'], .hackathon-description, .description"
            )
            description = (
            ...
            # Extract deadline
            deadline_element = self.sync_page.query_selector(
                "[data-testid='deadline'], .deadline, .submission-deadline"
            )
            deadline = (
                deadline_element.text_content()
                if deadline_element
                else "No deadline found"
            )
            ...
            # Extract requirements
            requirements = []
            req_elements = self.sync_page.query_selector_all(
                "[data-testid='requirement'], .requirement, .submission-requirement"
            )
            for element in req_elements:
            ...
            # Extract prizes
            prizes = []
            prize_elements = self.sync_page.query_selector_all(
                "[data-testid='prize'], .prize, .award"
            )
            for element in prize_elements:
            ...
            # Extract sponsors
            sponsors = []
            sponsor_elements = self.sync_page.query_selector_all(
                "[data-testid='sponsor'], .sponsor, .partner"
            )
            for element in sponsor_elements:
            ...
            # Extract submission guidelines
            guidelines_element = self.sync_page.query_selector(
                "[data-testid='guidelines'], .guidelines, .submission-guidelines"
            )
            submission_guidelines = (
                guidelines_element.text_content()
                if guidelines_element
                else "No guidelines found"
            )
    ...
    async def extract_project_data_async(self, project_url: str) -> DevPostProjectData:
        ...
        try:
            ...
            tag_elements = await self.page.query_selector_all(
                "[data-testid='tag'], .tag, .tech-stack"
            )
            ...
            team_members = []
            member_elements = await self.page.query_selector_all(
                "[data-testid='team-member'], .team-member, .member"
            )
            ...
            github_url = (
                await github_element.get_attribute("href") if github_element else ""
            )

            # Extract demo URL
            demo_element = await self.page.query_selector(
                "a[href*='demo'], a[href*='video'], a[href*='youtube.com'], a[href*='vimeo.com']"
            )
    ...
    def extract_project_data_sync(self, project_url: str) -> DevPostProjectData:
        ...
        try:
            ...
            # Extract tags
            tags = []
            tag_elements = self.sync_page.query_selector_all(
                "[data-testid='tag'], .tag, .tech-stack"
            )
            for element in tag_elements:
            ...
            # Extract team members
            team_members = []
            member_elements = self.sync_page.query_selector_all(
                "[data-testid='team-member'], .team-member, .member"
            )
            for element in member_elements:
            ...
            # Extract demo URL
            demo_element = self.sync_page.query_selector(
                "a[href*='demo'], a[href*='video'], a[href*='youtube.com'], a[href*='vimeo.com']"
            )
