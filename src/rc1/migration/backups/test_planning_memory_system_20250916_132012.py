...
def test_planning_memory_system():
    ...
    session_id = memory_manager.start_planning_session(
        "DevPost Integration RMDDD Refactoring"
    )
    ...
    dimensions = [
        ...
        create_planning_dimension(
            ...
            risk_level="high",
            unknown_factors=[
                "How to manage planning recursion?",
                "When does planning recursion become infinite?",
            ],
            constraints=[
                "Must manage planning recursion",
                "Must prevent infinite loops",
            ],
            mitigation_strategies=[
                "Create planning recursion management",
                "Implement recursion controls",
                "Use recursion validation",
            ],
        ),
    ]
    ...
    scenario = memory_manager.create_first_contact_scenario(
        "DevPost RMDDD Refactoring First Contact"
    )
    ...
    assert (
        summary["planning_exhaustion_level"]
        == loaded_summary["planning_exhaustion_level"]
    )
    ...
    print(
        f"   Critical Constraints: {len(df['constraint_analysis']['critical_constraints'])}"
    )
    print(f"   Critical Unknowns: {len(df['unknown_factors']['critical_unknowns'])}")
    print(
        f"   Planning Exhaustion: {df['planning_completeness']['exhaustion_level']:.1%}"
    )
