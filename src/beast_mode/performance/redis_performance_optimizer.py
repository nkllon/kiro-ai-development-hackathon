...
class RedisPerformanceOptimizer:
    ...
    async def optimize_with_redis_cache(
        self,
        key: str,
    ...
    async def optimize_with_redis_pipeline(
        ...
        result = RedisOptimizationResult(
            ...
            performance_improvement=len(operations)
            / max(len(operations_to_execute), 1),
            redis_operations_saved=len(operations) - len(operations_to_execute),
            cache_hit_rate=(len(operations) - len(operations_to_execute))
            / len(operations),
            execution_time_ms=execution_time,
            metadata={
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        cache_stats = {
            ...
            "hit_rate": (
                self.domain_cache.hits
                / (self.domain_cache.hits + self.domain_cache.misses)
                if (self.domain_cache.hits + self.domain_cache.misses) > 0
                else 0
            ),
    ...
    def generate_optimization_report(self) -> str:
        ...
        report.append(
            f"  Redis Available: {'Yes' if summary['redis_available'] else 'No'}"
        )
        report.append(
            f"  Domain Cache Active: {'Yes' if summary['domain_cache_active'] else 'No'}"
        )
...
def main():
    ...
    async def demo_redis_optimization():
        ...
        # Test caching
        result1 = await optimizer.optimize_with_redis_cache(
            "test_key", expensive_operation
        )
        result2 = await optimizer.optimize_with_redis_cache(
            "test_key", expensive_operation
        )
