...
class PerformanceMonitoringSystem:
    ...
    def _initialize_alert_thresholds(self) -> Dict[str, Dict[str, float]]:
        ...
        return {
            "cpu_percent": {"warning": 70.0, "error": 85.0, "critical": 95.0},
            "memory_percent": {"warning": 75.0, "error": 85.0, "critical": 95.0},
            "disk_usage_percent": {"warning": 80.0, "error": 90.0, "critical": 95.0},
            "response_time_ms": {
                "warning": 1000.0,
                "error": 5000.0,
                "critical": 10000.0,
            ...
            "cache_hit_rate": {"warning": 80.0, "error": 70.0, "critical": 60.0},
        }
    ...
    def _collect_system_metrics(self):
        ...
        try:
            ...
            with self.data_lock:
                ...
                self._store_metric(
                    "memory_used_mb", system_usage.memory_used_mb, MetricType.GAUGE
                )
                self._store_metric("disk_usage_percent", disk.percent, MetricType.GAUGE)
                self._store_metric(
                    "disk_free_gb", system_usage.disk_free_gb, MetricType.GAUGE
                )
    ...
    def _collect_application_metrics(self):
        ...
        try:
            ...
            with self.data_lock:
                ...
                self._store_metric(
                    "active_operations", app_metrics.active_operations, MetricType.GAUGE
                )
                self._store_metric(
                    "completed_operations",
                    app_metrics.completed_operations,
                    MetricType.COUNTER,
                )
                self._store_metric(
                    "failed_operations",
                    app_metrics.failed_operations,
                    MetricType.COUNTER,
                )
                self._store_metric(
                    "response_time_ms",
                    app_metrics.average_response_time_ms,
                    MetricType.TIMER,
                )
                self._store_metric(
                    "throughput_ops_per_second",
                    app_metrics.throughput_ops_per_second,
                    MetricType.GAUGE,
                )
                self._store_metric(
                    "cache_hit_rate", app_metrics.cache_hit_rate, MetricType.GAUGE
                )
                self._store_metric(
                    "error_rate", app_metrics.error_rate, MetricType.GAUGE
                )
                self._store_metric(
                    "queue_size", app_metrics.queue_size, MetricType.GAUGE
                )
                self._store_metric(
                    "active_threads", app_metrics.active_threads, MetricType.GAUGE
                )
    ...
    def _check_alerts(self):
        ...
        recent_metrics = [
            ...
            if (current_time - m.timestamp).total_seconds()
    ...
    def _cleanup_old_data(self):
        ...
        with self.data_lock:
            ...
            self.system_usage = [
                s for s in self.system_usage if s.timestamp > cutoff_time
            ]

            # Clean up application metrics
            self.application_metrics = [
                a for a in self.application_metrics if a.timestamp > cutoff_time
            ]
    ...
    def increment_counter(
        self, name: str, increment: float = 1.0, tags: Dict[str, str] = None
    ):
        ...
        with self.data_lock:
            ...
            return {
                ...
                "system_usage": {
                    ...
                    "memory_percent": (
                        latest_system.memory_percent if latest_system else 0
                    ),
                    "memory_used_mb": (
                        latest_system.memory_used_mb if latest_system else 0
                    ),
                    "disk_usage_percent": (
                    ...
                    "load_average": (
                        latest_system.load_average if latest_system else (0, 0, 0)
                    ),
                    "process_count": (
                        latest_system.process_count if latest_system else 0
                    ),
                    "thread_count": latest_system.thread_count if latest_system else 0,
                },
                "application_metrics": {
                    "active_operations": (
                        latest_app.active_operations if latest_app else 0
                    ),
                    "completed_operations": (
                        latest_app.completed_operations if latest_app else 0
                    ),
                    "failed_operations": (
                        latest_app.failed_operations if latest_app else 0
                    ),
                    "average_response_time_ms": (
                        latest_app.average_response_time_ms if latest_app else 0
                    ),
                    "throughput_ops_per_second": (
                        latest_app.throughput_ops_per_second if latest_app else 0
                    ),
                ...
                "monitoring_uptime_hours": (
                    (current_time - self.start_time).total_seconds() / 3600
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        with self.data_lock:
            ...
            recent_app = [
                a for a in self.application_metrics if a.timestamp > last_hour
            ]
            recent_metrics = [m for m in self.metrics if m.timestamp > last_hour]

            # Calculate averages
            avg_cpu = (
                sum(s.cpu_percent for s in recent_system) / len(recent_system)
                if recent_system
                else 0
            )
            avg_memory = (
                sum(s.memory_percent for s in recent_system) / len(recent_system)
                if recent_system
                else 0
            )
            avg_response_time = (
                sum(a.average_response_time_ms for a in recent_app) / len(recent_app)
                if recent_app
                else 0
            )
            ...
            response_trend = self._calculate_trend(
                [a.average_response_time_ms for a in recent_app]
            )
    ...
    def _generate_recommendations(
        self, avg_cpu: float, avg_memory: float, avg_response_time: float
    ) -> List[str]:
        ...
        if avg_memory > 80:
            recommendations.append(
                "High memory usage detected. Consider implementing memory optimization strategies or increasing available memory."
            )

        if avg_response_time > 1000:
            recommendations.append(
                "High response times detected. Consider implementing caching, database optimization, or async processing."
            )

        if avg_cpu < 20 and avg_memory < 30:
            recommendations.append(
                "System resources are underutilized. Consider increasing workload or reducing resource allocation."
            )

        if not recommendations:
            recommendations.append(
                "System performance is within acceptable ranges. Continue monitoring for trends."
            )
    ...
    def generate_monitoring_report(self) -> str:
        ...
        report.append(
            f"  Average Response Time: {app_metrics['average_response_time_ms']:.2f}ms"
        )
        report.append(
            f"  Throughput: {app_metrics['throughput_ops_per_second']:.2f} ops/sec"
        )
        ...
        report.append(
            f"  Monitoring Uptime: {current_metrics['monitoring_uptime_hours']:.1f} hours"
        )
        report.append(
            f"  Total Metrics Stored: {current_metrics['total_metrics_stored']}"
        )
