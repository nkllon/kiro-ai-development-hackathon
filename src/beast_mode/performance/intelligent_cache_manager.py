...
class IntelligentCacheManager:
    ...
    def set(self, key: str, value: Any, ttl: Optional[int] = None, 
            priority: int = 1, tags: List[str] = None, dependencies: List[str] = None):
        ...
        with self.cache_lock:
            ...
            self.logger.debug(f"Cached entry: {key} (size: {size_bytes} bytes, TTL: {ttl}s)")
    ...
    def _should_store_in_l2(self, entry: CacheEntry) -> bool:
        ...
        # - Large size
        # - Frequently accessed
        return (entry.priority <= 2 or 
                entry.size_bytes > 1024 * 1024 or  # > 1MB
    ...
    def _should_store_in_l3(self, entry: CacheEntry) -> bool:
        ...
        return (entry.priority == 1 and 
                (entry.size_bytes > 10 * 1024 * 1024 or  # > 10MB
    ...
    def _cleanup_expired_entries(self):
        ...
        for cache_name, cache in [("L1", self.l1_cache), ("L2", self.l2_cache), ("L3", self.l3_cache)]:
            expired_keys = []
            ...
            if expired_keys:
                self.logger.info(f"Cleaned up {len(expired_keys)} expired entries from {cache_name} cache")
    ...
    def _optimize_cache_size(self):
        ...
        self.logger.info(f"Evicted entries to reduce cache size by {current_reduction / (1024*1024):.1f}MB")
    ...
    def _update_prediction_model(self):
        ...
        if len(self.prediction_model['hit_rate_history']) > 100:
            self.prediction_model['hit_rate_history'] = self.prediction_model['hit_rate_history'][-100:]
    ...
    def get_cache_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "prediction_model": {
                ...
                "hit_rate_history_length": len(self.prediction_model['hit_rate_history'])
            }
        }
    ...
    def generate_cache_report(self) -> str:
        ...
        report.append(f"  Disk Cache: {'Enabled' if config['disk_cache_enabled'] else 'Disabled'}")
        report.append(f"  Network Cache: {'Enabled' if config['network_cache_enabled'] else 'Disabled'}")
        report.append("")
        
        prediction = cache_info['prediction_model']
        report.append("PREDICTIVE MODEL:")
        report.append(f"  Tracked Keys: {prediction['tracked_keys']}")
        report.append(f"  Hit Rate History: {prediction['hit_rate_history_length']} samples")
        report.append("")
