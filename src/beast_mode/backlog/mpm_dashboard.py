...
authoritative source for mpm_dashboard.
...
class MPMDashboard:
    ...
    def get_portfolio_status(self) -> PortfolioStatus:
        ...
        try:
            ...
            beast_ready_items = sum((1 for item in self._backlog_items.values() if item.beast_readiness_status == BeastReadinessStatus.BEAST_READY))
            in_progress_items = sum((1 for item in self._backlog_items.values() if item.beast_readiness_status == BeastReadinessStatus.IN_EXECUTION))
            blocked_items = sum((1 for item in self._backlog_items.values() if item.beast_readiness_status == BeastReadinessStatus.BLOCKED))
            completed_items = sum((1 for item in self._backlog_items.values() if item.beast_readiness_status == BeastReadinessStatus.COMPLETED))
            ...
            portfolio_status = PortfolioStatus(total_items=total_items, beast_ready_items=beast_ready_items, in_progress_items=in_progress_items, blocked_items=blocked_items, completion_percentage=completion_percentage, track_breakdown=track_breakdown, critical_path_items=critical_path_items, delivery_confidence=delivery_confidence, risk_assessment=risk_assessment, last_updated=datetime.now())
            self._cached_portfolio_status = portfolio_status
            self._cache_timestamp = datetime.now()
            self._performance_metrics['cache_hit_rate'] = self._performance_metrics['cache_hit_rate'] * 0.9
        ...
        try:
            ...
            for item_id, item in self._backlog_items.items():
                if item.beast_readiness_status in [BeastReadinessStatus.BLOCKED, BeastReadinessStatus.DRAFT]:
                    has_dependents = any((dep.target_item_id == item_id for other_item in self._backlog_items.values() for dep in other_item.dependencies))
                    if has_dependents:
    ...
    def _calculate_delivery_confidence(self) -> float:
        ...
        if not self._backlog_items:
            return 0.0
        total_items = len(self._backlog_items)
        beast_ready_count = sum((1 for item in self._backlog_items.values() if item.beast_readiness_status == BeastReadinessStatus.BEAST_READY))
        completed_count = sum((1 for item in self._backlog_items.values() if item.beast_readiness_status == BeastReadinessStatus.COMPLETED))
        blocked_count = sum((1 for item in self._backlog_items.values() if item.beast_readiness_status == BeastReadinessStatus.BLOCKED))
    ...
    def _calculate_validation_quality_bonus(self) -> float:
        ...
        if not validated_items:
            return 0.0
        avg_completeness = sum((item.mpm_validation.completeness_score for item in validated_items)) / len(validated_items)
        avg_confidence = sum((item.mpm_validation.dependency_confidence for item in validated_items)) / len(validated_items)
    ...
    def _update_performance_metrics(self, operation: str, start_time: float) -> None:
        ...
        total_ops = sum((self._performance_metrics[key] for key in ['portfolio_queries', 'impact_analyses', 'scenario_plans'] if key in self._performance_metrics))
        if total_ops > 0:
    ...
    def _calculate_track_timelines(self, scenario_params: Dict[str, Any], constraints: ResourceConstraints) -> Dict[StrategicTrack, datetime]:
        ...
        for track in StrategicTrack:
            track_items = [item for item in self._backlog_items.values() if item.track == track]
            remaining_items = sum((1 for item in track_items if item.beast_readiness_status not in [BeastReadinessStatus.COMPLETED]))
