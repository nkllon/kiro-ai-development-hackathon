...
class FileChangeEvent(BaseModel):
    ...
    def is_documentation_file(self) -> bool:
        ...
        return any((pattern in filename_lower for pattern in doc_patterns)) or self.file_path.suffix.lower() in {'.md', '.txt', '.rst', '.adoc'} or 'docs/' in str(self.file_path).lower()
    ...
    def is_configuration_file(self) -> bool:
        ...
        config_files = {'package.json', 'pyproject.toml', 'requirements.txt', 'Dockerfile', 'docker-compose.yml', 'docker-compose.yaml', '.env', 'config.json', 'config.yaml', 'config.yml', 'setup.py', 'setup.cfg'}
        config_extensions = {'.json', '.yaml', '.yml', '.toml', '.ini', '.cfg'}
        return self.file_path.name in config_files or self.file_path.suffix.lower() in config_extensions

    def is_source_code_file(self) -> bool:
        """Check if file is source code."""
        code_extensions = {'.py', '.js', '.ts', '.jsx', '.tsx', '.java', '.cpp', '.c', '.h', '.cs', '.php', '.rb', '.go', '.rs', '.swift', '.kt', '.scala', '.html', '.css', '.scss', '.sass', '.less'}
...
class DevpostConfig(BaseModel):
    ...
    watch_patterns: List[str] = Field(default_factory=lambda: ['README*', '*.md', 'package.json', 'pyproject.toml', 'media/*'])
