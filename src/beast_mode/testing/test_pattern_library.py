...
authoritative source for test_pattern_library.
...
class TestPatternLibrary(ReflectiveModule):
        ...
        try:
            if Path(self.pattern_metrics_path).exists():
                ...
                for metrics_data in data.get('metrics', []):
                    metrics = TestPatternMetrics(pattern_id=metrics_data['pattern_id'], match_count=metrics_data.get('match_count', 0), successful_applications=metrics_data.get('successful_applications', 0), average_match_time_ms=metrics_data.get('average_match_time_ms', 0.0), last_matched=datetime.fromisoformat(metrics_data['last_matched']) if metrics_data.get('last_matched') else None, effectiveness_score=metrics_data.get('effectiveness_score', 0.0), false_positive_count=metrics_data.get('false_positive_count', 0))
    ...
    def _generate_pattern_from_rca(self, failure: Failure, root_causes: List[RootCause], systematic_fixes: List[SystematicFix]) -> Optional[PreventionPattern]:
        ...
        detection_criteria = [f'Monitor for {primary_root_cause.cause_type.value} in test execution', f'Automated detection of {failure.category.value} failures', 'Proactive test environment validation']
        automated_checks = [f'Automated check for {primary_root_cause.cause_type.value}', 'Continuous test environment monitoring', 'Preventive test dependency validation']
