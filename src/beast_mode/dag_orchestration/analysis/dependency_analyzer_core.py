...
class DependencyAnalyzer:
    ...
    async def analyze_complete_ecosystem(self, spec_directory: str) -> EcosystemAnalysisResult:
        """
        ...
        specification_nodes = self._create_specification_nodes(parsed_specs, task_detection)
        constraint_graph = self.dependency_mapper.create_dependency_graph(task_detection.tasks, task_detection.dependencies, specification_nodes)
        critical_path_analysis = self.critical_path_analyzer.analyze_critical_paths(constraint_graph)
        layer_processing = self.layer_processor.process_layers(specification_nodes, constraint_graph)
        ecosystem_dag = self._create_ecosystem_dag(specification_nodes, task_detection, constraint_graph, critical_path_analysis)
        return EcosystemAnalysisResult(ecosystem_dag=ecosystem_dag, constraint_graph=constraint_graph, critical_path_analysis=critical_path_analysis, layer_processing=layer_processing, parsed_specs=parsed_specs, task_detection=task_detection)
    ...
    async def calculate_critical_paths(self, ecosystem_dag: EcosystemDAG) -> List[CriticalPath]:
        ...
        """
        constraint_graph = ConstraintGraph(nodes={task.task_id: task for task in ecosystem_dag.tasks}, edges=ecosystem_dag.dependencies, adjacency_list={}, reverse_adjacency={}, topological_order=[], dependency_layers={}, conflicts=[])
        constraint_graph = self.dependency_mapper.create_dependency_graph(ecosystem_dag.tasks, ecosystem_dag.dependencies, ecosystem_dag.specifications)
        analysis = self.critical_path_analyzer.analyze_critical_paths(constraint_graph)
        return analysis.all_critical_paths

    async def identify_parallel_opportunities(self, ecosystem_dag: EcosystemDAG) -> List[List[str]]:
        """
        ...
        """
        constraint_graph = self.dependency_mapper.create_dependency_graph(ecosystem_dag.tasks, ecosystem_dag.dependencies, ecosystem_dag.specifications)
        layer_processing = self.layer_processor.process_layers(ecosystem_dag.specifications, constraint_graph)
        parallel_groups = []
    ...
    def _create_specification_nodes(self, parsed_specs: List[ParsedSpec], task_detection: TaskDetectionResult) -> List[SpecificationNode]:
        ...
        for parsed_spec in parsed_specs:
            ...
            specification_nodes.append(SpecificationNode(spec_name=spec_name, spec_path=parsed_spec.spec_path, completion_percentage=completion_percentage, task_count=task_count, completed_tasks=completed_tasks, dependencies=parsed_spec.dependencies, dependents=[], layer=0))
    ...
    def _create_ecosystem_dag(self, specification_nodes: List[SpecificationNode], task_detection: TaskDetectionResult, constraint_graph: ConstraintGraph, critical_path_analysis: CriticalPathAnalysis) -> EcosystemDAG:
        ...
        completed_effort = sum((task.estimated_effort for task in task_detection.tasks if task.completion_status.value == 'completed'))
        completion_percentage = completed_effort / total_effort * 100.0 if total_effort > 0 else 0.0
        remaining_effort = sum((task.estimated_effort for task in task_detection.tasks if task.completion_status.value != 'completed'))
        parallel_opportunities = []
        for layer, task_ids in constraint_graph.dependency_layers.items():
            if len(task_ids) > 1:
                from ..models.dag_models import ParallelGroup
                parallel_tasks = [constraint_graph.nodes[task_id] for task_id in task_ids if task_id in constraint_graph.nodes]
                if parallel_tasks:
