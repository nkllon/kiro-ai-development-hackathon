...
class SpecParser:
    ...
    def _count_requirements(self, content: str) -> int:
        ...
        requirement_headers = re.findall(r'###\s*Requirement\s+\d+', content, re.IGNORECASE)
        requirement_count += len(requirement_headers)
        
        # Count acceptance criteria
        acceptance_criteria = re.findall(r'^\s*\d+\.\s*WHEN.*THEN.*SHALL', content, re.MULTILINE | re.IGNORECASE)
