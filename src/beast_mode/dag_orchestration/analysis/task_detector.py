"""
Task detector for DAG orchestration system.

Identifies tasks, dependencies, and completion status from specification
files with systematic analysis and validation.
"""
...
class TaskDetectionResult:
    ...
    orphaned_tasks: List[str]  # Tasks with no dependencies or dependents
...
class TaskDetector:
    ...
    def __init__(self):
        self.task_id_patterns = [
            r'(\d+(?:\.\d+)*)\s*[.-]\s*(.+)',  # 1.1 - Task name
            r'(\d+(?:\.\d+)*)\s+(.+)',         # 1.1 Task name
            r'([A-Z]+-\d+)\s*[:-]\s*(.+)',     # TASK-1: Task name
        ]
        ...
        self.dependency_keywords = [
            ...
            'builds on', 'extends', 'based on', 'uses'
        ]
    ...
    def detect_tasks_from_specs(self, parsed_specs: List[ParsedSpec]) -> TaskDetectionResult:
        ...
        # Extract tasks from each specification
    ...
    def _parse_task_line(self, line: str, line_index: int, lines: List[str]) -> Optional[Tuple[str, str, TaskStatus, str, List[str], List[str]]]:
        ...
        checkbox_match = re.match(r'^\s*-\s*\[\s*([x\s\-!F])\s*\]\s*(.+)$', line, re.IGNORECASE)
        if checkbox_match:
            ...
            # Get additional details from following lines
        ...
        if numbered_match:
            ...
            # Extract task name (remove status indicators)
    ...
    def _extract_task_details(self, line_index: int, lines: List[str]) -> Tuple[str, List[str], List[str]]:
        ...
        # Look at the next few lines for task details
        for i in range(line_index + 1, min(line_index + 10, len(lines))):
            ...
            # Check if this is a detail line (indented or bullet)
            if (lines[i].startswith('  ') or 
                lines[i].startswith('\t') or 
                line.startswith('-')):
                
                # Extract requirements
                req_match = re.search(r'_Requirements?:\s*([^_]+)_', line, re.IGNORECASE)
                if req_match:
    ...
    def _estimate_effort(self, task_name: str, description: str) -> int:
        ...
        # Effort multipliers based on keywords
        effort_keywords = {
            'implement': 1.5,
            'create': 1.3,
            'build': 1.8,
    ...
    def _resolve_task_dependencies(self, task: TaskNode, task_lookup: Dict[str, TaskNode], name_lookup: Dict[str, str]) -> List[DependencyEdge]:
        ...
        for dep_name in task.dependencies:
            # Try to find dependency by name
            ...
            for task_name, task_id in name_lookup.items():
                if (dep_name_lower in task_name or 
                    task_name in dep_name_lower or
                    self._similarity_score(dep_name_lower, task_name) > 0.8):
                    dependencies.append(DependencyEdge(
                        source_id=task_id,
                        target_id=task.task_id,
                        dependency_type="requires"
