...
class ParallelOptimizer:
    ...
    def _generate_execution_phases(self, parallel_groups: List[ParallelGroup], 
    ...
    def _is_viable_opportunity(self, opportunity: ParallelOpportunity) -> bool:
        ...
        return (
            opportunity.estimated_savings > opportunity.coordination_overhead and
            opportunity.risk_level != "high" and
            len(opportunity.tasks) >= 2
        )
    ...
    def _sort_groups_by_dependencies(self, groups: List[ParallelGroup], 
                                   constraint_graph: ConstraintGraph) -> List[ParallelGroup]:
        """Sort groups by dependency order."""
        # Simple topological sort based on task dependencies
        return sorted(groups, key=lambda g: min(
