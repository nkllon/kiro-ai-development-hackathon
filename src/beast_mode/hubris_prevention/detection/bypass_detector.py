...
authoritative source for bypass_detector.
...
class GovernanceBypassDetector:
    ...
    def _detect_approval_skipping(self, actor_id: str, decisions: List[Decision]) -> Optional[BypassPattern]:
        ...
        if len(high_impact_decisions) >= 2:
            return BypassPattern(pattern_type='approval_skipping', severity='critical' if len(high_impact_decisions) >= 4 else 'high', evidence=[f'{len(high_impact_decisions)} high-impact decisions without approval', 'Systematic avoidance of accountability verification', 'Pattern indicates governance bypass intent'], confidence=min(1.0, len(high_impact_decisions) / 3), first_detected=min((d.timestamp for d in high_impact_decisions)), last_detected=max((d.timestamp for d in high_impact_decisions)))
        return None
    ...
    def _detect_authority_escalation(self, actor_id: str, decisions: List[Decision]) -> Optional[BypassPattern]:
        ...
        if len(escalation_decisions) >= 2:
            return BypassPattern(pattern_type='authority_escalation', severity='critical', evidence=[f'{len(escalation_decisions)} authority escalation attempts', 'Self-authorization of elevated permissions', 'Bypassing normal authority chains'], confidence=0.9, first_detected=min((d.timestamp for d in escalation_decisions)), last_detected=max((d.timestamp for d in escalation_decisions)))
        return None
