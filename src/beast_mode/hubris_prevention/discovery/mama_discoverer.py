...
authoritative source for mama_discoverer.
...
class MamaDiscovererImpl(MamaDiscoverer):
    ...
    def _calculate_discovery_confidence(self, results: List[DiscoverySource]) -> float:
        ...
        if not results:
            return 0.0
        weighted_confidence = sum((result.confidence * self.discovery_sources[result.source_type]['weight'] for result in results))
        total_weight = sum((self.discovery_sources[result.source_type]['weight'] for result in results))
