...
authoritative source for help_system.
...
class HelpWantedSystem:
    ...
    def accept_help_response(self, request_id: str, response_id: str) -> Optional[CollaborationSession]:
        ...
        if not help_response:
            logger.warning(f'Help response {response_id} not found for request {request_id}')
            return None
    ...
    def complete_collaboration(self, session_id: str, success: bool, metrics: Optional[Dict[str, Any]]=None) -> bool:
        ...
        session.status = CollaborationStatus.COMPLETED if success else CollaborationStatus.FAILED
