"""
Beast Mode Bus Client

Provides Redis pub/sub client functionality for agent communication.
"""
...
class BeastModeBusClient:
    ...
    async def connect(self) -> bool:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                # Start agent registry background cleanup
                if self.discovery_enabled:
                    self.agent_registry.start_background_cleanup()
                
                # Start collaboration scheduler background tasks
    ...
    async def disconnect(self) -> None:
        ...
        try:
            ...
            # Stop collaboration scheduler background tasks
    ...
    async def send_help_request(
        ...
        # Only send if we have a real Redis client
        if self.client and self.client is not True:  # True is our mock value
            await self.send_message(message)
            ...
            async for raw_message in self.pubsub.listen():
                ...
                if raw_message['type'] == 'message':
                    try:
                        ...
                        # Handle message with router if available
                        if self.message_router:
                            try:
                                router_responses = await self.message_router.process_message(message)
                                
                                # Send any responses generated by the router
                        ...
                        # Handle message with legacy handlers
        ...
        if self.discovery_enabled:
            try:
                ...
                if updated_agent:
                    ...
                    # If agent is unknown, treat the response as a discovery message
                    # This happens when an agent responds to our discovery but hasn't announced itself
                    logger.debug(f"Received response from unknown agent {message.source}, registering as discovery")
                    
                    # Create a pseudo-discovery message from the response
    ...
    async def _handle_help_request(self, message: BeastModeMessage) -> None:
        ...
        # Use help system to process the request
    ...
    async def _handle_collaboration_request(self, message: BeastModeMessage) -> None:
        ...
        try:
            ...
            # Check if we're available for the requested collaboration
            requested_time = None
            ...
            if is_available and requested_time:
                # Suggest alternative times if not available at requested time
    ...
    async def _handle_collaboration_response(self, message: BeastModeMessage) -> None:
        ...
        try:
            ...
            if payload.get('available', False):
                ...
                # Check for suggested alternative time
    ...
    # Agent Discovery and Registry Methods
    ...
    def get_discovery_stats(self) -> Dict:
        ...
        return {
            ...
            **self.agent_registry.get_registry_stats()
        }
    ...
    def get_message_history(self, limit: Optional[int] = None) -> Dict[str, List[BeastModeMessage]]:
        ...
        return {
            'sent': [],
            'received': recent_messages
        }
    
    # Collaboration Scheduling Methods
    ...
    async def request_collaboration(
        ...
        # Create collaboration session locally
        ...
        # Send collaboration request to each target agent
