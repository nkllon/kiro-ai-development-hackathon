...
if PYDANTIC_AVAILABLE:
    ...
    class BeastModeMessage(BaseModel):
        ...
        def to_dict(self) -> Dict[str, Any]:
            ...
            data = self.dict()
            # Handle datetime serialization for Redis
        ...
        def create_reply(self, sender_id: str, content: Dict[str, Any], 
                        message_type: MessageType = MessageType.DIRECT_MESSAGE) -> 'BeastModeMessage':
...
def create_help_request(sender_id: str, required_capabilities: List[AgentCapability], 
                       description: str, priority: str = "normal") -> BeastModeMessage:
...
def create_heartbeat(agent_id: str, status_info: Dict[str, Any]) -> BeastModeMessage:
    ...
    return BeastModeMessage(
        ...
        expires_at=datetime.fromtimestamp(time.time() + 300)  # 5 minute expiry
    )
...
def filter_messages_by_capability(messages: List[BeastModeMessage], 
                                 agent_capabilities: List[AgentCapability]) -> List[BeastModeMessage]:
