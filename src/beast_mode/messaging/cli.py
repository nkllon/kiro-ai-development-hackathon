...
Command-line interface for interacting with the Beast Mode agent network.
Provides commands for starting agents, sending messages, checking status, and more.
"""
...
from .message_models import (
    BeastModeMessage, MessageType, AgentCapability,
    create_help_request, create_heartbeat, create_agent_announcement
)
...
@cli.command()
@click.option('--agent-type', type=click.Choice(['cost', 'deployment', 'quality', 'all']), 
              default='all', help='Type of agent to start')
...
def start_agents(ctx, agent_type, background):
    ...
    async def _start_agents():
        ...
        from examples.beast_mode_collaboration_agents import (
            CostOptimizationAgent, DeploymentSpecialistAgent, CodeQualityMentorAgent
        )
...
@click.option('--description', prompt='Help description', 
              help='Description of what help you need')
        ...
        if success:
            ...
            def response_handler(message_data):
                try:
                    ...
                    if (message.message_type == MessageType.HELP_RESPONSE and 
                        message.correlation_id == help_request.message_id):
...
def status(ctx):
    ...
    async def _check_status():
        ...
        # Listen for heartbeats to detect active agents
...
@cli.command()
@click.option('--message-type', type=click.Choice([mt.value for mt in MessageType]),
              default='direct_message', help='Type of message to send')
...
@cli.command()
@click.option('--output-format', type=click.Choice(['json', 'markdown', 'text']), 
              default='markdown', help='Output format for network content')
