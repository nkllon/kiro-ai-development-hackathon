...
This module was extracted from cli_core_core.py
as part of RM-DDD compliance refactoring.
"""

"""
Cli - Consolidated Interface Definition
...
authoritative source for cli.

Consolidated from: /Users/lou/kiro-2/kiro-ai-development-hackathon/src/beast_mode/messaging/cli_core_core_core.py
Consolidation date: 2025-09-13T10:15:07.483778
"""
...
@cli.command()
@click.option('--agent-type', type=click.Choice(['cost', 'deployment', 'quality', 'all']), default='all', help='Type of agent to start')
...
@cli.command()
@click.option('--capability', multiple=True, type=click.Choice([cap.value for cap in AgentCapability]), help='Required capabilities (can specify multiple)')
@click.option('--description', prompt='Help description', help='Description of what help you need')
@click.option('--priority', type=click.Choice(['low', 'normal', 'high', 'urgent']), default='normal', help='Request priority')
...
def request_help(ctx, capability, description, priority, sender_id):
    ...
    async def _send_help_request():
        ...
        if not await redis_foundation.initialize():
            ...
            return
        required_caps = [AgentCapability(cap) for cap in capability] if capability else []
        help_request = create_help_request(sender_id=sender_id, required_capabilities=required_caps, description=description, priority=priority)
        success = await redis_foundation.publish('help_requests', help_request.to_dict())
        if success:
            click.echo(f'📨 Help request sent (ID: {help_request.message_id})')
            click.echo(f'📋 Description: {description}')
            if required_caps:
                click.echo(f'🎯 Required capabilities: {[cap.value for cap in required_caps]}')
            ...
            def response_handler(message_data):
                try:
                    ...
                    if message.message_type == MessageType.HELP_RESPONSE and message.correlation_id == help_request.message_id:
...
def status(ctx):
    ...
    async def _check_status():
        ...
        click.echo(f"{status_icon} Redis: {conn_info['host']}:{conn_info['port']} ({conn_info['status']})")
        if conn_info['active_subscriptions']:
            click.echo(f"📡 Active subscriptions: {len(conn_info['active_subscriptions'])}")
        ...
        def heartbeat_handler(message_data):
            try:
                ...
                if message.message_type == MessageType.HEARTBEAT:
                    ...
                    active_agents[message.sender_id] = {'name': agent_info.get('agent_name', 'Unknown'), 'capabilities': agent_info.get('capabilities', []), 'load': agent_info.get('current_load', 0), 'max_tasks': agent_info.get('max_tasks', 0), 'last_seen': datetime.now()}
            except Exception as e:
                if ctx.obj['verbose']:
        ...
        if active_agents:
            ...
            for agent_id, info in active_agents.items():
                ...
                click.echo(f"    Capabilities: {', '.join(info['capabilities'][:3])}{('...' if len(info['capabilities']) > 3 else '')}")
        else:
...
@cli.command()
@click.option('--message-type', type=click.Choice([mt.value for mt in MessageType]), default='direct_message', help='Type of message to send')
...
def send_message(ctx, message_type, recipient, content, subject, sender_id):
    ...
    async def _send_message():
        ...
        message = BeastModeMessage(message_type=MessageType(message_type), sender_id=sender_id, recipient_id=recipient, subject=subject, content=message_content)
        if recipient:
...
@click.option('--channel', default='beast_mode_network', help='Network channel to share to')
...
def share(ctx, message, file_path, channel, sender_id):
    ...
    async def _share_content():
        ...
        if message:
            ...
            click.echo(f"💬 Sharing message: {message[:50]}{('...' if len(message) > 50 else '')}")
        if not content:
        ...
        share_message = BeastModeMessage(message_type=MessageType.BROADCAST_MESSAGE, sender_id=sender_id, subject=f"Network Share: {(file_path_obj.name if file_path else 'Message')}", content=content)
        ...
        if success:
            ...
            if ctx.obj['verbose']:
                click.echo(f'   Content preview: {json.dumps(content, indent=2)[:200]}...')
        else:
...
@cli.command()
@click.option('--output-format', type=click.Choice(['json', 'markdown', 'text']), default='markdown', help='Output format for network content')
