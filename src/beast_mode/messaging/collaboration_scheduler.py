...
authoritative source for collaboration_scheduler.
...
class CollaborationScheduler:
    ...
    def is_agent_available(self, agent_id: str, at_time: Optional[datetime]=None) -> bool:
        ...
        if office_hours.pattern == OfficeHoursPattern.WEEKDAYS:
            ...
            if check_time.weekday() not in office_hours.days_of_week:
                return False
        active_count = len([s for s in self.active_sessions if agent_id in self.sessions[s].participants])
    ...
    def schedule_collaboration(self, participants: List[str], topic: str, session_type: CollaborationType=CollaborationType.SCHEDULED_SESSION, scheduled_start: Optional[datetime]=None, duration_minutes: int=30, description: str='', required_capabilities: Optional[List[str]]=None) -> Optional[CollaborationSession]:
        ...
        try:
            ...
            logger.info(f'Scheduled collaboration session {session.session_id} for {len(participants)} participants')
            return session
        except Exception as e:
    ...
    def start_collaboration_session(self, session_id: str) -> bool:
        ...
        if session.status != CollaborationStatus.SCHEDULED:
            logger.warning(f'Cannot start session {session_id} with status {session.status}')
            return False
    ...
    def _analyze_collaboration_patterns(self) -> None:
        ...
        if not self.pattern_analysis_enabled:
            return
        try:
            completed_sessions = [s for s in self.sessions.values() if s.status == CollaborationStatus.COMPLETED]
    ...
    def _create_collaboration_pattern(self, participants: Tuple[str, ...], sessions: List[CollaborationSession]) -> None:
        ...
        try:
            ...
            successful_sessions = [s for s in sessions if s.success_metrics.get('success', True)]
    ...
    def get_collaboration_recommendations(self, agent_id: str) -> List[Dict[str, Any]]:
        ...
        recommendations = []
        try:
            relevant_patterns = [pattern for pattern in self.collaboration_patterns.values() if agent_id in pattern.participants and pattern.confidence_score > 0.5]
