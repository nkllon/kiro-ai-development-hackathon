"""
Beast Mode Shared State Manager

Manages shared state in Redis regardless of transport choice.
Provides fast collaborative state for all agents.
"""
...
class SharedStateConfig:
    ...
    max_spore_size: int = 1024 * 1024  # 1MB max spore size
...
class BeastModeSharedState:
    ...
    # Agent State Management (Requirements 3.1, 3.2)
    ...
    # Spore Storage Management (Requirements 3.3, 3.4)
    ...
    async def store_spore(self, spore_id: str, spore_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            # Set TTL for spore cleanup
            await self.redis_client.expire(key, self.config.ttl_seconds * 24)  # 24x longer for spores
    ...
    # Collaboration Session Management (Requirements 3.5)
    ...
    async def create_collaboration_session(self, session_id: str, session_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            await self.redis_client.expire(key, self.config.ttl_seconds * 8)  # 8 hours for sessions
    ...
    async def end_collaboration_session(self, session_id: str) -> bool:
        ...
        try:
            # Mark as ended rather than deleting for audit trail
            await self.update_collaboration_session(session_id, {
                "status": "ended",
            ...
            key = f"{self.config.key_prefix}:sessions:{session_id}"
            await self.redis_client.expire(key, 3600)  # 1 hour cleanup
    ...
    # Performance Metrics (Requirements 3.5)
    ...
    async def cleanup_expired_data(self) -> Dict[str, int]:
        ...
        try:
            # This is handled automatically by Redis TTL, but we can report stats
