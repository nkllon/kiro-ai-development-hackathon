...
authoritative source for adr_system.
    ...
    def search_adrs(self, query: str, filters: Dict[str, Any]=None) -> List[Dict[str, Any]]:
        ...
        for adr in self.adrs.values():
            ...
            if 'tag' in filters and (not any((tag in adr.tags for tag in filters['tag']))):
                continue
            results.append({'adr_id': adr.adr_id, 'title': adr.title, 'status': adr.status.value, 'category': adr.category.value, 'date_created': adr.date_created.isoformat(), 'decision_makers': adr.decision_makers, 'summary': adr.context.problem_statement[:200] + '...' if len(adr.context.problem_statement) > 200 else adr.context.problem_statement})
