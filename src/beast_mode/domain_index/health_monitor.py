...
This module was extracted from health_monitor_core_core.py
as part of RM-DDD compliance refactoring.
"""

"""
Health_Monitor - Consolidated Interface Definition
...
authoritative source for health_monitor.

Consolidated from: /Users/lou/kiro-2/kiro-ai-development-hackathon/src/beast_mode/domain_index/health_monitor_core_core_core.py
Consolidation date: 2025-09-13T10:15:07.411473
"""
...
class DomainHealthMonitor(DomainSystemComponent, HealthMonitorInterface):
    ...
    def _create_failed_health_status(self, error_message: str) -> HealthStatus:
        ...
        return HealthStatus(status=HealthStatusType.FAILED, last_check=datetime.now(), issues=[issue], metrics=metrics)
    ...
    def get_health_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            healthy_count = sum((1 for status in self._health_cache.values() if status.status == HealthStatusType.HEALTHY))
            degraded_count = sum((1 for status in self._health_cache.values() if status.status == HealthStatusType.DEGRADED))
            failed_count = sum((1 for status in self._health_cache.values() if status.status == HealthStatusType.FAILED))
            total_issues = sum((len(status.issues) for status in self._health_cache.values()))
            critical_issues = sum((sum((1 for issue in status.issues if issue.severity == IssueSeverity.CRITICAL)) for status in self._health_cache.values()))
            return {'total_domains': total_domains, 'healthy_domains': healthy_count, 'degraded_domains': degraded_count, 'failed_domains': failed_count, 'overall_health_percentage': healthy_count / max(total_domains, 1) * 100, 'total_issues': total_issues, 'critical_issues': critical_issues, 'last_full_check': self._last_full_check.isoformat() if self._last_full_check else None, 'check_statistics': {'total_checks': self.total_checks, 'failed_checks': self.failed_checks, 'issues_detected': self.issues_detected, 'issues_resolved': self.issues_resolved}}
        except Exception as e:
    ...
    def generate_health_report(self, report_type: str='full', domain_name: str=None) -> Dict[str, Any]:
        ...
        with self._time_operation('generate_health_report'):
            try:
                ...
                return {'report_id': report.report_id, 'report_type': report.report_type, 'generated_at': report.generated_at.isoformat(), 'total_domains': report.total_domains, 'overall_health_score': report.overall_health_score, 'critical_issues_count': len(report.critical_issues), 'warning_issues_count': len(report.warning_issues), 'recommendations_count': len(report.recommendations), 'generation_time_ms': report.generation_time_ms}
            except Exception as e:
    ...
    def export_health_report(self, report_id: str, format: str='json') -> Dict[str, Any]:
        ...
        with self._time_operation('export_health_report'):
            try:
                ...
                return {'report_id': report_id, 'format': format, 'exported_at': datetime.now().isoformat(), 'data': exported_data}
            except Exception as e:
