...
class SystematicFailureDetector:
    ...
    def __init__(self):
        ...
        self.recovery_strategies: Dict[FailureMode, List[str]] = self._initialize_recovery_strategies()
    ...
    def analyze_failure(self, 
                       error: Exception, 
                       context: Dict[str, Any]) -> FailureAnalysis:
        ...
        confidence_score = self._calculate_confidence_score(failure_context, root_causes)
    ...
    def _classify_failure_mode(self, error: Exception, context: Dict[str, Any]) -> FailureMode:
        ...
        elif "resource" in error_str and ("exhausted" in error_str or "limit" in error_str):
        ...
        elif "cli" in context_str and ("not found" in error_str or "unavailable" in error_str):
            return FailureMode.CLI_UNAVAILABLE
        else:
            return FailureMode.UNKNOWN_ERROR
    
    def _assess_severity(self, failure_mode: FailureMode, context: Dict[str, Any]) -> FailureSeverity:
    ...
    def _check_systemic_failure(self, context: FailureContext) -> bool:
        ...
        similar_failures = [
            ...
            and (context.timestamp - f.context.timestamp).total_seconds() < 3600  # 1 hour
        ]
        
        return len(similar_failures) >= 3  # 3+ similar failures in 1 hour = systemic
    
    def _calculate_confidence_score(self, context: FailureContext, root_causes: List[str]) -> float:
    ...
    def get_failure_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_confidence": sum(f.confidence_score for f in self.failure_history) / len(self.failure_history)
        }
    ...
    def generate_recovery_report(self) -> str:
        ...
        for analysis in self.failure_history[-5:]:  # Last 5 failures
            report.append(f"  {analysis.failure_id}: {analysis.context.failure_mode.value}")
...
def main():
    ...
    test_failures = [
        ...
        (PermissionError("Access denied"), {"component": "auth", "operation": "token_validation"})
    ]
