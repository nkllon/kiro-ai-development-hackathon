...
authoritative source for file_change_detector.
...
class FileChangeDetector(ReflectiveModule):
    ...
    def _calculate_complexity_score(self, changes: List[FileChange]) -> float:
        ...
        complexity = source_files * 1.0 + test_files * 0.5 + config_files * 0.8 + (total_files - source_files - test_files - config_files) * 0.3
    ...
    def _analyze_task_completion_enhanced(self, task_id: str, task_config: Dict[str, Any], analysis: AdvancedFileChangeAnalysis, claimed_tasks: Optional[List[str]]=None) -> TaskMapping:
        ...
        mapping = TaskMapping(task_id=task_id, task_description=task_config.get('description', ''), confidence_score=0.0, matching_files=[], evidence=[], completion_indicators=[])
        all_changes = []
        for changes in analysis.changes_by_type.values():
            all_changes.extend(changes)
    ...
    def _generate_file_change_breakdown(self, analysis: AdvancedFileChangeAnalysis) -> Dict[str, Any]:
        ...
        for change_type, changes in analysis.changes_by_type.items():
            breakdown['by_type'][change_type.value] = {'count': len(changes), 'files': [change.file_path for change in changes], 'impact_scores': [change.impact_score for change in changes]}
        for category, changes in analysis.changes_by_category.items():
            breakdown['by_category'][category.value] = {'count': len(changes), 'files': [change.file_path for change in changes], 'impact_scores': [change.impact_score for change in changes]}
        return breakdown
    ...
    def _generate_task_completion_recommendations(self, task_mappings: List[TaskMapping], claimed_tasks: Optional[List[str]]) -> List[str]:
        ...
        if low_confidence:
            recommendations.append(f'Review {len(low_confidence)} low-confidence task mappings for accuracy')
        if claimed_tasks:
            mapping_dict = {m.task_id: m for m in task_mappings}
            missing_evidence = [task for task in claimed_tasks if task not in mapping_dict or mapping_dict[task].confidence_score < 0.3]
            if missing_evidence:
                recommendations.append(f'Verify completion of {len(missing_evidence)} claimed tasks with weak evidence')
            unclaimed = [m.task_id for m in task_mappings if m.confidence_score > 0.6 and m.task_id not in claimed_tasks]
            if unclaimed:
    ...
    def _detect_content_indicators(self, file_path: str, indicators: List[str]) -> List[str]:
        ...
        if file_patterns and matching_files:
            ...
            high_impact_files = [f for f in matching_files if self._is_high_impact_file(f, analysis)]
    ...
    def _generate_file_change_breakdown(self, analysis: AdvancedFileChangeAnalysis) -> Dict[str, Any]:
        ...
        breakdown = {'by_type': {}, 'by_category': {}}
        for change_type, changes in analysis.changes_by_type.items():
            breakdown['by_type'][change_type.value] = {'count': len(changes), 'files': [change.file_path for change in changes], 'total_impact': sum((change.impact_score for change in changes))}
        for category, changes in analysis.changes_by_category.items():
            breakdown['by_category'][category.value] = {'count': len(changes), 'files': [change.file_path for change in changes], 'total_impact': sum((change.impact_score for change in changes))}
        return breakdown
    ...
    def _generate_task_completion_recommendations(self, task_mappings: List[TaskMapping], claimed_tasks: Optional[List[str]]) -> List[str]:
        ...
        if claimed_tasks:
            ...
            low_confidence_claimed = [m for m in task_mappings if m.task_id in claimed_set and m.confidence_score < 0.3]
            if low_confidence_claimed:
                recommendations.append(f'Review {len(low_confidence_claimed)} claimed tasks with low implementation evidence')
        if claimed_tasks:
            claimed_set = set(claimed_tasks)
            unclaimed_high_confidence = [m for m in task_mappings if m.task_id not in claimed_set and m.confidence_score >= 0.7]
            if unclaimed_high_confidence:
                recommendations.append(f'Consider claiming {len(unclaimed_high_confidence)} tasks with strong implementation evidence')
