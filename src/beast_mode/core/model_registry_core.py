...
def get_learning_insights(self, domain: Optional[str]=None) -> Dict[str, Any]:
    ...
    try:
        ...
        if all_patterns:
            ...
            insights['learning_trends'] = [f'Accumulated {len(all_patterns)} patterns across {len(domains_to_analyze)} domains', f"Average confidence improved to {insights['avg_confidence']:.2%}", f"Top performing metric: {(max(insights['top_success_metrics'].items(), key=lambda x: x[1]['avg'])[0] if insights['top_success_metrics'] else 'None')}"]
    except Exception as e:
