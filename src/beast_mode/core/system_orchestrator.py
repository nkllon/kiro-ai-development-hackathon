...
authoritative source for system_orchestrator.
...
class BeastModeSystemOrchestrator(ReflectiveModule):
    ...
    def get_health_indicators(self) -> Dict[str, Any]:
        ...
        health_report = self.health_monitor.get_system_health_report()
        return {'system_health': {'status': 'healthy' if self.is_healthy() else 'degraded', 'initialization_complete': self.initialization_complete, 'uptime_compliance': health_report['uptime_metrics']['availability_percentage'] >= 99.9}, 'component_health': health_report['system_overview'], 'uptime_metrics': health_report['uptime_metrics'], 'alert_summary': {'active_alerts': len(health_report['recent_alerts']), 'critical_alerts': len([a for a in health_report['recent_alerts'] if a['severity'] == 'critical']), 'unresolved_alerts': len([a for a in health_report['recent_alerts'] if not a['resolved']])}}
