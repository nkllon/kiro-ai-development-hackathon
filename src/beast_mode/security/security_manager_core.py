...
def perform_security_audit(self) -> SecurityAuditResult:
    ...
    try:
        ...
        expired_keys = [name for name, key_info in self.api_keys.items() if datetime.now() > key_info['expires']]
        if expired_keys:
        ...
        recent_violations = [v for v in self.security_violations if (datetime.now() - v['timestamp']).days <= 7]
        ...
        if compliance_score < 1.0:
            recommendations.extend(['Implement comprehensive vulnerability scanning', 'Setup automated security monitoring', 'Establish security incident response procedures', 'Regular security training for development team'])
        audit_result = SecurityAuditResult(audit_id=audit_id, timestamp=audit_start, compliance_score=compliance_score, vulnerabilities_found=vulnerabilities, compliance_status=compliance_status, recommendations=recommendations, critical_issues=critical_issues)
