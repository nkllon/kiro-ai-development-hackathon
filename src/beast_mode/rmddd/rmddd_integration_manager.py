...
class RMDDDIntegrationManager:
    ...
    def _initialize_default_services(self):
        ...
        default_services = [
            RMDDDService(name="analysis_service", endpoint="http://localhost:8080/api/analysis"),
            RMDDDService(name="migration_service", endpoint="http://localhost:8081/api/migration"),
            RMDDDService(name="sdk_service", endpoint="http://localhost:8082/api/sdk"),
            RMDDDService(name="registry_service", endpoint="http://localhost:8083/api/registry")
        ]
    ...
    def execute_use_case(self, use_case: UseCase, timeout_seconds: int = 300) -> UseCaseResult:
        ...
        try:
            ...
            if unavailable_services:
                raise Exception(f"Required services unavailable: {unavailable_services}")
            ...
            for i, step in enumerate(use_case.steps):
                ...
                if step_result.get('status') == 'failed':
                    ...
                    use_case_result.error_message = step_result.get('error', 'Step execution failed')
                    break
            ...
            use_case_result.duration = (use_case_result.end_time - start_time).total_seconds()
            
            # Move to history
            self.use_case_history.append(use_case_result)
            self.active_use_cases.pop(use_case.id, None)
            
            self.logger.info(f"Use case execution completed: {use_case.id} - {use_case_result.status.value}")
        
        return use_case_result
    
    def _execute_use_case_step(self, step: Dict[str, Any], step_number: int) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"Executing step {step_number}: {step_type} - {step_action}")
    ...
    def _execute_domain_operation(self, action: str, params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _execute_service_call(self, action: str, params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _execute_validation(self, action: str, params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _execute_data_transformation(self, action: str, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "output_sample": {k: v for k, v in list(output_data.items())[:3]}  # First 3 items
        }
    ...
    def generate_integration_report(self) -> str:
        ...
        report.append(f"  Availability: {service_summary['availability_percentage']:.1f}%")
        report.append("")
