...
"""
Whiskey_Mode - Consolidated Interface Definition
...
authoritative source for whiskey_mode.

Consolidated from: /Users/lou/kiro-2/kiro-ai-development-hackathon/src/beast_mode/monitoring/whiskey_mode_core_core_core.py
Consolidation date: 2025-09-13T10:15:07.478108
"""
...
class WhiskeyModeDisplay(ReflectiveModule):
    ...
    def _setup_layout(self) -> None:
        ...
        self.layout['left'].split_column(Layout(name='test_matrix', ratio=3), Layout(name='system_health', ratio=1))
    ...
    def _create_header(self) -> Panel:
        ...
        status_color = {WhiskeyModeStatus.STARTING: 'yellow', WhiskeyModeStatus.ACTIVE: 'green', WhiskeyModeStatus.PAUSED: 'orange', WhiskeyModeStatus.ERROR: 'red'}[self.status]
        status_text = Text(f'● {self.status.value.upper()}', style=f'bold {status_color}')
        header_content = Columns([Align.left(title), Align.center(subtitle), Align.right(status_text)])
    ...
    def _create_system_health(self) -> Panel:
        ...
        health_table.add_row('Memory', f'{self.system_health.memory_percent:.1f}%', mem_sparkline)
        velocity_sparkline = self._create_sparkline([12, 15, 18, 16, self.system_health.test_velocity], 'cyan')
        health_table.add_row('Test Velocity', f'{self.system_health.test_velocity:.1f}/min', velocity_sparkline)
    ...
    def _create_hubris_panel(self) -> Panel:
        ...
        if not self.hubris_alerts:
            ...
            content = Text('\n'.join(['⚠️  Hubris Prevention Active', '', *self.hubris_alerts[-3:]]), style='yellow')
    ...
    def _create_mama_panel(self) -> Panel:
        ...
        if not self.mama_discoveries:
            ...
            content = Text('\n'.join(['👸 Mama Discovery Active', '', *self.mama_discoveries[-2:]]), style='magenta')
