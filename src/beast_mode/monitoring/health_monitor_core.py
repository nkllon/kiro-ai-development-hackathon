...
class HealthMonitor:
    ...
    async def get_health_summary(self) -> Dict[str, Any]:
        ...
        unknown_count = sum((1 for h in self.component_health.values() if h.status == HealthStatus.UNKNOWN))
    ...
    async def _register_default_checks(self) -> None:
        ...
        await self.register_health_check(name='redis_connectivity', check_function=self._check_redis_connectivity, interval_seconds=15, timeout_seconds=3)
        await self.register_health_check(name='redis_pubsub', check_function=self._check_redis_pubsub, interval_seconds=30, timeout_seconds=5)
        await self.register_health_check(name='system_resources', check_function=self._check_system_resources, interval_seconds=60, timeout_seconds=2)
    ...
    async def _check_system_resources(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {'healthy': True, 'message': 'System resource monitoring unavailable (psutil not installed)', 'details': {'note': 'Install psutil for system resource monitoring'}}
        except Exception as e:
