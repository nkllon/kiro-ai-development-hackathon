...
class PrometheusConfigManager:
    ...
    def _load_config(self) -> PrometheusConfig:
        ...
        return PrometheusConfig(
            ...
            mode=PrometheusConfigMode(os.getenv("BEAST_MODE_PROMETHEUS_MODE", "basic")),
            ...
            collection_interval=float(
                os.getenv("BEAST_MODE_PROMETHEUS_INTERVAL", "5.0")
            ),
            ...
            enable_application_metrics=self._get_bool_env(
                "BEAST_MODE_APP_METRICS", True
            ),
            enable_performance_metrics=self._get_bool_env(
                "BEAST_MODE_PERF_METRICS", True
            ),
            # Advanced settings
            ...
            alert_threshold_memory=float(
                os.getenv("BEAST_MODE_MEMORY_THRESHOLD", "80.0")
            ),
    ...
    def _validate_config(self):
        ...
        if self._config.collection_interval < 0.1:
            raise ValueError(
                f"Collection interval too small: {self._config.collection_interval}"
            )

        if self._config.retention_hours < 1:
            raise ValueError(
                f"Retention hours too small: {self._config.retention_hours}"
            )

        self.logger.info(
            f"Prometheus configuration loaded: {self._config.mode.value} mode"
        )
