...
class BeastModeFinal95PercentPush:
    ...
    def aggressive_deletion_strategy(self):
        ...
        for error_file in self.error_files:
            if (
                ".beast_mode/" in error_file["path"]
                or "backup" in error_file["path"].lower()
                or error_file["size"] < 100  # Very small files
                or error_file["size"] > 100000
            ):  # Very large files
