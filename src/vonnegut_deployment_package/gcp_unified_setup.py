...
class GCPUnifiedSetup:
    ...
    def choose_setup_type(self) -> SetupType:
        ...
        print("=" * 40)
        print(
            "1. Personal/Startup (Hackathon) - Create billing account with your payment method"
        )
    ...
    def get_configuration(self, setup_type: SetupType) -> dict:
        ...
        if setup_type == SetupType.PERSONAL:
            ...
            print("=" * 40)
            config["project_name"] = (
                input("Enter project name: ").strip() or "hackathon-project"
            )
            config["region"] = (
                input("Enter region (default: us-central1): ").strip() or "us-central1"
            )
            config["zone"] = (
                input("Enter zone (default: us-central1-a): ").strip()
                or "us-central1-a"
            )
            ...
            print("=" * 40)
            config["billing_account_id"] = input(
                "Enter enterprise billing account ID: "
            ).strip()
            ...
            config["project_name"] = (
                input("Enter project name: ").strip() or "enterprise-project"
            )
            config["enterprise_domain"] = (
                input("Enter enterprise domain: ").strip() or "enterprise.com"
            )
            ...
            config["admin_email"] = (
                input("Enter enterprise admin email: ").strip()
                or "admin@enterprise.com"
            )
    ...
    def run_setup(self):
        ...
        session_id = self.state_manager.create_session(
            self.setup_type, self.configuration
        )
