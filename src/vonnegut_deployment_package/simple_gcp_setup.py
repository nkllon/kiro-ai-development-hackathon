...
def main():
    ...
    api_key = (
        os.getenv("GOOGLE_API_KEY") or input("Enter your Google API key: ").strip()
    )
    email = (
        os.getenv("GCP_EMAIL")
        or input("Enter your GCP email (optional): ").strip()
        or None
    )
    project_id = (
        os.getenv("GCP_PROJECT_ID")
        or input("Enter your GCP project ID (optional): ").strip()
        or None
    )

    # Create configuration
    config = GCPSetupConfig(api_key=api_key, email=email, project_id=project_id)
