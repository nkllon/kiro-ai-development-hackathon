...
try:
    ...
    class MockGCPBillingMonitor:
        ...
        def get_cost_optimization_recommendations(self):
            return [
                {
                    "recommendation": "Right-size Compute Engine instances",
                    "potential_saving": 3.50,
                },
                {
                    "recommendation": "Enable auto-scaling for Cloud Functions",
                    "potential_saving": 1.20,
                },
                {
                    "recommendation": "Optimize storage classes for old data",
                    "potential_saving": 2.10,
                },
            ]
...
class HackathonShowcase:
            ...
            try:
                ...
                if intelligence:
                    desc = getattr(
                        intelligence,
                        "description",
                        f"Systematic intelligence for {domain}",
                    )
    ...
    async def run_complete_demo(self):
        ...
        try:
            ...
            self.print_success(
                f"Systematic Score: {results['pdca']['systematic_score']:.3f} (target: 0.8+)"
            )
            self.print_success(
                f"Model Registry: {results['domains']} domains available"
            )
            self.print_success(
                f"Cost Optimization: ${results['cost_optimization']['savings']:.2f}/day savings"
            )
