...
class DevPostWorkflow:
    ...
    def _build_graph(self) -> StateGraph:
        ...
        workflow.add_node(
            "ghostbusters_consultation", ghostbusters_consultation_refactored_node
        )
    ...
    def _route_from_browser_connection(self, state: DevPostState) -> str:
        ...
        if state["browser_status"].value == "connected":
            ...
            state["errors"]
            and state["recovery_attempts"] < state["max_recovery_attempts"]
        ):
    ...
    def resume_workflow(self, workflow_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if current_state:
                print(
                    f"📍 Current phase: {current_state.values.get('current_phase', 'Unknown')}"
                )
        ...
        try:
            ...
            if state:
                return {
                    ...
                    "user_input_required": state.values.get(
                        "user_input_required", False
                    ),
                    "ghostbusters_mode": state.values.get("ghostbusters_mode", False),
                    "awaiting_recovery_choice": state.values.get(
                        "awaiting_recovery_choice", False
                    ),
                    "awaiting_memory_qualification": state.values.get(
    ...
    def handle_user_input(self, workflow_id: str, user_input: str) -> Dict[str, Any]:
        ...
        try:
                    ...
                    try:
                        ...
                        for qual in qualifications:
                            ...
                            if ":" in qual:
                                ...
                                item_num = (
                                    int(item_num.strip()) - 1
                                )  # Convert to 0-based index
                                decision = decision.strip()

                                if decision.startswith("transform"):
                                    transform_instruction = decision.replace(
                                        "transform", ""
                                    ).strip()
                                    decision = "transform"
                                else:
                                ...
                                # Apply qualification
                                qualification_queue = (
                                    memory_manager.memory_qualification_queue
                                )
            ...
            return {"success": False, "error": f"Failed to handle user input: {str(e)}"}
