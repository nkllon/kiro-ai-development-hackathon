...
class DevPostFormORM:
    ...
    def analyze_forms(self, form_files: List[str]):
        ...
        for form_file in form_files:
            ...
            print("   ✅ Mapped {len(field_mappings)} fields")
    ...
    def create_field_mapping(
        self, field: Dict[str, Any], page_type: str
        ...
        if field_type == "hidden" and not any(
            x in field_name.lower() for x in ["token", "method", "version"]
        ):
            return None
    ...
    def map_field_to_data_source(
        self, field_name: str, label: str, field_type: str, page_type: str
    ) -> tuple:
    ...
    def print_field_summary(self, field_mappings: List[FormFieldMapping]):
        ...
        for mapping in field_mappings:
            if mapping.data_source != "static":
                print(
                    "     • {mapping.label} → {mapping.data_source}.{mapping.data_path}"
                )
    ...
    def fill_form(self, page_type: str, project_data: Dict[str, Any] = None):
        ...
        if page_type not in self.forms:
            print("❌ Unknown page type: {page_type}")
            return False
        if not self.page:
            print("❌ Not connected to browser")
            return False
        form_page = self.forms[page_type]
        print("📝 Filling form: {form_page.page_name}")
        print("   Form ID: {form_page.form_id}")
        print("   Fields: {len(form_page.fields)}")
        ...
        for mapping in form_page.fields:
            if self.fill_field(mapping, data):
                filled_count += 1
        print("✅ Filled {filled_count}/{len(form_page.fields)} fields")
        return True
    ...
    def fill_field(self, mapping: FormFieldMapping, data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            # Try by ID first
            if mapping.field_id:
                field_element = self.page.query_selector("#{mapping.field_id}")
            # Try by name
            if not field_element and mapping.field_name:
                field_element = self.page.query_selector(
                    "[name='{mapping.field_name}']"
                )
            if not field_element:
                print("   ❌ Field not found: {mapping.label}")
                return False
            ...
            if mapping.field_type in ["text", "email", "url", "tel", "number"]:
                ...
                if isinstance(value, str) and Path(value).exists():
                    ...
                    print("   ⚠️ File not found: {value}")
                    return False
            print(
                "   ✅ {mapping.label}: {str(value)[:50]}{'...' if len(str(value)) > 50 else ''}"
            )
            return True
        except Exception:
            print("   ❌ Failed to fill {mapping.label}: {e}")
            return False
    ...
    def get_field_value(self, mapping: FormFieldMapping, data: Dict[str, Any]) -> Any:
        ...
        try:
            ...
            print("   ❌ Error getting value for {mapping.label}: {e}")
            return None
    ...
    def navigate_to_page(self, page_type: str) -> bool:
        ...
        if page_type not in self.forms:
            ...
            return False
        form_page = self.forms[page_type]
        print("🌐 Navigating to: {form_page.page_name}")
        try:
            self.page.goto(form_page.url_pattern, wait_until="networkidle")
            print("📄 Page loaded: {self.page.title()}")
            return True
        except Exception:
    ...
    def submit_form(self, page_type: str) -> bool:
        ...
        if page_type not in self.forms:
            ...
            return False
        form_page = self.forms[page_type]
        print("📤 Submitting form: {form_page.page_name}")
        try:
            ...
            for selector in form_page.submit_selectors:
                ...
                if submit_button and submit_button.is_visible():
                    print("🔄 Clicking submit: {submit_button.text_content().strip()}")
                    submit_button.click()
    ...
    def print_form_analysis(self):
        ...
        for page_type, form_page in self.forms.items():
            ...
            print("   Form ID: {form_page.form_id}")
            print("   URL: {form_page.url_pattern}")
            print("   Fields: {len(form_page.fields)}")
            # Show field mappings
            for mapping in form_page.fields:
                if mapping.data_source != "static":
...
def main():
    ...
    if orm.connect_to_browser():
        print("\n🎮 ORM Commands:")
        print(
            "navigate <page>  - Navigate to page (project_overview, project_details, additional_info)"
        )
