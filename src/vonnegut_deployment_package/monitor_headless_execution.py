...
class ExecutionMonitor:
    ...
    def get_execution_metrics(self, execution_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "critical_path_progress": self._calculate_critical_path_progress(
                execution_data
            ),
        }
    ...
    def _calculate_critical_path_progress(
        ...
        completed_critical = len(
            [t for t in critical_tasks if t.get("status") == "completed"]
        )
    ...
    def display_dashboard(
        self, executions: List[Dict[str, Any]], cost_metrics: Dict[str, Any]
    ):
        ...
        for i, execution_data in enumerate(executions):
            ...
            print(
                f"   📊 Progress: {metrics['progress_percent']:.1f}% ({metrics['completed_tasks']}/{metrics['total_tasks']} tasks)"
            )
            print(
                f"   🔄 Running: {metrics['running_tasks']} | ⏳ Pending: {metrics['pending_tasks']} | ❌ Failed: {metrics['failed_tasks']}"
            )
        ...
        print(
            f"   🔧 Billing Monitor: {'✅ Available' if BILLING_AVAILABLE else '❌ Unavailable'}"
        )
    ...
    def show_execution_details(self, execution_id: str):
        ...
        try:
            ...
            print(
                f"Estimated Duration: {execution_data.get('estimated_duration_hours', 0):.1f} hours"
            )
            print()
            ...
            for spec_name, counts in specs.items():
                ...
                print(
                    f"   {spec_name}: {progress:.1f}% ({counts['completed']}/{counts['total']})"
                )
                if counts["failed"] > 0:
        ...
        if command == "details" and len(sys.argv) > 2:
            ...
            if executions:
                ...
                for execution in executions:
                    print(
                        f"   • {execution.get('plan_id', 'Unknown')} - {execution.get('status', 'Unknown')}"
                    )
            else:
                print("📋 No active executions found")
        else:
            print(
                "Usage: python scripts/monitor_headless_execution.py [list|details <execution_id>]"
            )
    else:
