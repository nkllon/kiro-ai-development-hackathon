...
class TargetedComplianceImprovementSystem:
            ...
            for i, line in enumerate(lines):
                if line.strip().startswith("class ") and "(" in line:
                    ...
                    for j in range(i + 1, len(lines)):
                        if (
                            lines[j].strip()
                            and not lines[j].startswith(" " * (indent_level + 1))
                            and not lines[j].startswith(" " * indent_level)
                        ):
                            class_end = j
                            break
                    else:
    ...
    def _refactor_for_size_compliance_aggressive(self, filepath: str) -> bool:
        ...
        try:
            ...
            for i, (start_line, def_line) in enumerate(definitions):
                ...
                end_line = (
                    definitions[i + 1][0] if i + 1 < len(definitions) else len(lines)
                )
            ...
            self._update_imports_after_aggressive_refactoring(
                filepath, base_name, file_count
            )
            return True
    ...
    def _update_imports_after_aggressive_refactoring(
        self, filepath: str, base_name: str, file_count: int
    ):
        ...
        try:
            ...
            self.errors += 1
            self.report["errors"].append(
                f"Error updating imports for {filepath}: {str(e)}"
            )
