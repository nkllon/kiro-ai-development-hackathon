...
class ComplianceAnalysisAndRecommendations:
    ...
    def analyze_file_counts(self):
        ...
        self.analysis["file_counts"] = {
            ...
            "directory_breakdown": dict(
                sorted(dir_counts.items(), key=lambda x: x[1], reverse=True)[:10]
            ),
    ...
    def analyze_compliance_metrics(self):
        ...
        # Size compliance
        result = subprocess.run(
            ["find", self.target_dir, "-name", "*.py", "-exec", "wc", "-l", "{}", "+"],
            capture_output=True,
            text=True,
        )
        ...
        self.analysis["compliance_metrics"] = {
            ...
            "health_monitoring": {
                ...
                "total_files": total_files,
                "percentage": (
                    (health_count / total_files * 100) if total_files > 0 else 0
                ),
            },
            "registry_integration": {
                ...
                "total_files": total_files,
                "percentage": (
                    (registry_count / total_files * 100) if total_files > 0 else 0
                ),
            },
            "size_compliance": {
        ...
        print(
            f"✅ RDI Compliance: {rdi_count}/{total_files} ({self.analysis['compliance_metrics']['rdi_compliance']['percentage']:.1f}%)"
        )
        print(
            f"✅ Health Monitoring: {health_count}/{total_files} ({self.analysis['compliance_metrics']['health_monitoring']['percentage']:.1f}%)"
        )
        print(
            f"✅ Registry Integration: {registry_count}/{total_files} ({self.analysis['compliance_metrics']['registry_integration']['percentage']:.1f}%)"
        )
        print(
            f"✅ Size Compliance: {total_files - len(large_files)}/{total_files} ({self.analysis['compliance_metrics']['size_compliance']['percentage']:.1f}%)"
        )
    ...
    def identify_issues(self):
        ...
        health_pct = self.analysis["compliance_metrics"]["health_monitoring"][
            "percentage"
        ]
        if health_pct < 95:
            issues.append(
        ...
        registry_pct = self.analysis["compliance_metrics"]["registry_integration"][
            "percentage"
        ]
        if registry_pct < 95:
            issues.append(
    ...
    def create_next_pass_strategy(self):
        ...
        critical_items = [
            r for r in self.analysis["recommendations"] if r["priority"] == "CRITICAL"
        ]
        high_items = [
            r for r in self.analysis["recommendations"] if r["priority"] == "HIGH"
        ]
        medium_items = [
            r for r in self.analysis["recommendations"] if r["priority"] == "MEDIUM"
        ]
