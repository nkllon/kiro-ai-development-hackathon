...
def _find_similar_pages(
    visual_hash: str, telemetry_graph: TelemetryGraph, threshold: int = 5
) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        for node_id, data in telemetry_graph.graph.nodes(data=True):
            if (
                data.get("visual_hash") == visual_hash
                and data.get("url") != telemetry_graph.current_page_hash
            ):
                similar_pages.append(
                    {
                        "node_id": node_id,
...
def _save_session_data(
    telemetry_graph: TelemetryGraph, state_model: DevPostStateModel, force: bool = False
) -> bool:
...
def explore_from_here(page):
    ...
    for tag, count in sorted(
        all_elements["tagCounts"].items(), key=lambda x: x[1], reverse=True
    )[:10]:
        print(f"   {tag}: {count}")
    ...
    nav_elements = page.query_selector_all(
        "nav, [role='navigation'], .nav, .navigation, #navigation"
    )
    breadcrumbs = page.query_selector_all(
    ...
    clickable_elements = page.query_selector_all(
        "[onclick], [onchange], [onsubmit], [onload], button, a, input[type='button'], input[type='submit']"
    )
    ...
    for i, form in enumerate(all_elements["formElements"], 1):
        ...
        if form["placeholder"]:
            print(f"      Placeholder: '{form['placeholder']}'")
    ...
    for i, interactive in enumerate(all_elements["interactiveElements"], 1):
        ...
        if interactive["href"]:
            print(f"      Href: {interactive['href']}")
        if interactive["onclick"] == "present":
            print(f"      Has onclick handler")
    ...
    print(
        f"🌳 This page contains {all_elements['totalElements']} elements with rich interactive content"
    )
    print(
        f"📊 Collected telemetry on {len(all_elements['navigationElements'])} nav, {len(all_elements['formElements'])} form, {len(all_elements['interactiveElements'])} interactive elements"
    )
...
def navigate_devpost_steps():
    ...
    try:
        ...
        for port in chrome_ports:
            try:
                ...
                if response.status_code == 200:
                    ...
                    if pages_info:
                        print(
                            f"✅ Found existing Chrome on port {port} with {len(pages_info)} tab(s)"
                        )
                        print(
                            "🔐 This Chrome instance should have all your extensions (including 1Password)"
                        )
        ...
        if not browser:
            ...
            print(
                "💡 TIP: Start Chrome with: /Applications/Google\\ Chrome.app/Contents/MacOS/Google\\ Chrome --remote-debugging-port=9222"
            )
            ...
            if os.path.exists(user_data_path):
                print(
                    "💾 Found existing browser session data - preserving cookies and login state"
                )
            else:
            ...
            try:
                ...
                pages_info = response.json()
                print(
                    f"✅ New browser session established with {len(pages_info)} tab(s)!"
                )
            except:
                pages_info = []
        ...
        devpost_pages = [
            page for page in pages_info if "devpost.com" in page.get("url", "")
        ]
        ...
        if not devpost_pages:
            ...
            for i, page in enumerate(pages_info):
                print(
                    f"   {i+1}. {page.get('title', 'Untitled')} - {page.get('url', 'No URL')}"
                )

            choice = input(
                "🎯 Select tab number (or 'n' for new DevPost tab): "
            ).strip()
            ...
            if choice.lower() == "n":
                ...
                try:
                    ...
                    if 0 <= tab_index < len(pages_info):
                        ...
                        print(
                            f"✅ Navigated to selected tab: {pages_info[tab_index]['title']}"
                        )
                    else:
            ...
            for i, page in enumerate(devpost_pages):
                status = (
                    "📍 CURRENT"
                    if "submission" in page.get("url", "").lower()
                    else "🌐 OPEN"
                )
        ...
        condition = state_model.detect_current_condition(
            target_page.url, target_page.title(), page_content
        )
        ...
        while True:
            ...
            try:
                ...
                comprehensive_data = {
                    ...
                    "current_step": (
                        page_state.current_step.text
                        if page_state.current_step
                        else "unknown"
                    ),
                ...
                hackathon_id = next(
                    (part for part in url_parts if "hackathon" in part), "unknown"
                )
                submission_id = next(
                    (part for part in url_parts if part.isdigit()), "unknown"
                )
                ...
                comprehensive_data["visual_hash"] = _calculate_image_hash(
                    screenshot_filename
                )

                # Check if we've seen this page before (visual comparison)
                similar_pages = _find_similar_pages(
                    comprehensive_data["visual_hash"], telemetry_graph
                )
                if similar_pages:
                    print(
                        f"🔍 Similar pages found: {len(similar_pages)} previous visits"
                    )
                ...
                print(
                    f"📍 Current Step: {page_state.current_step.text if page_state.current_step else 'Unknown'}"
                )
                ...
                if is_visible and is_enabled:
                    ...
                    status = (
                        ...
                        if "current" in classes
                            ...
                            "✅ COMPLETED" if "completed" in classes else "⏳ AVAILABLE"
                        )
                    )
            ...
            other_buttons = target_page.query_selector_all(
                "button, input[type='button'], input[type='submit']"
            )
            other_options = []
            ...
            try:
                ...
                print(
                    f"\n🎯 Choose navigation (1-{max_choice}, s, a, e, r, t, q): ",
                    end="",
                    flush=True,
                )
                ...
                if choice == "q":
                    ...
                    if telemetry_graph.has_changes():
                        ...
                        save_choice = (
                            input("Save session data? (y/n/f for force save): ")
                            .strip()
                            .lower()
                        )
                    ...
                    submission_id = url_parts[-2] if len(url_parts) > 2 else "unknown"
                    page_title = (
                        target_page.title().replace(" ", "_").replace("/", "_")[:20]
                    )
                    ...
                    print(
                        f"📊 Elements analyzed: {comprehensive_data['totalElements']}"
                    )
                    print(
                        f"🧭 Navigation elements: {len(comprehensive_data['navigationElements'])}"
                    )
                    print(
                        f"📋 Form elements: {len(comprehensive_data['formElements'])}"
                    )
                    print(
                        f"🎮 Interactive elements: {len(comprehensive_data['interactiveElements'])}"
                    )
                    ...
                    save_button = None
                    save_buttons = target_page.query_selector_all(
                        "button, input[type='button'], input[type='submit']"
                    )
