...
class AdaptiveLearningSystem:
    ...
    def _load_knowledge_base(self):
        ...
        try:
            ...
            print(
                f"🧠 Loaded knowledge base with {len(self.knowledge_base['patterns'])} patterns"
            )
    ...
    def _catalog_navigation_elements(self) -> List[Dict[str, Any]]:
        ...
        for link in links:
            if link.is_visible() and link.is_enabled():
                ...
                if text and any(
                    word in text.lower()
                    for word in ["next", "continue", "back", "previous", "submit"]
                ):
                    elements.append(
                        {
    ...
    def _calculate_element_confidence(self, element, element_type: str) -> float:
        ...
        try:
            ...
            if pattern_key in self.knowledge_base["patterns"]:
                ...
                success_rate = pattern.get("success_count", 0) / max(
                    1, pattern.get("attempt_count", 1)
                )
    ...
    def _detect_page_indicators(self) -> Dict[str, Any]:
        ...
        # Progress indicators
        progress_elements = self.page.query_selector_all(
            "[class*='step'], [class*='progress'], [class*='deadline']"
        )
        for elem in progress_elements:
            text = elem.text_content().strip()
            if text and any(
                word in text.lower() for word in ["step", "done", "deadline", "hours"]
            ):
        ...
        nav_elements = self.page.query_selector_all(
            "#steps-navigation, .navigation, .nav"
        )
    ...
    def adaptive_navigate(self):
        ...
        try:
            ...
            while navigation_count < max_navigations:
                ...
                print(
                    f"📊 Performance: {self.performance['navigations_successful']}/{self.performance['navigations_attempted']} success rate"
                )
                ...
                success = self._execute_interaction_with_learning(
                    best_interaction, environment
                )
    ...
    def _find_best_interaction(
        ...
        high_confidence = [
            ...
            if p["confidence"] >= self.learning_config["confidence_threshold"]
        ]
    ...
    def _execute_interaction_with_learning(
        self, interaction: Dict[str, Any], environment: Dict[str, Any]
    ) -> bool:
        """Execute interaction with learning."""
