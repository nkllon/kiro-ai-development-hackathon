...
class MakefileTargetValidator(ReflectiveModule):
    ...
    def __init__(self, repository_root: str = "."):
        ...
        # Initialize registries early so base class health checks have data
        ...
        self.performance_rules: Dict[str, Any] = {}
    ...
    def get_module_info(self) -> Dict[str, Any]:
        """Module metadata for reflective registry consumers."""
        return {
            "module_id": self.module_id,
            "name": "Makefile Target Validator",
            "version": "1.0.0",
    ...
    def get_capabilities(self) -> List[ModuleCapability]:
        """Capabilities advertised by the validator."""
        return [
    ...
    def get_health_status(self) -> ModuleHealth:
        """Return basic health metrics for the validator."""
        uptime = (datetime.now() - self._start_time).total_seconds()
        issues: List[str] = []

        if not self.targets:
            issues.append("No Makefile targets discovered during initialization")

        if self.repository_root.exists() is False:
            issues.append("Repository root not found")
    ...
    def graceful_degradation(self) -> GracefulDegradationResult:
        """Describe fallback behaviour when validation rules partially fail."""
