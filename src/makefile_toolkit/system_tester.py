...
class MakefileSystemTester(ReflectiveModule):
    ...
    def __init__(self, repository_root: str = "."):
        ...
        self.python_cmd = os.environ.get("PYTHON_CMD") or sys.executable
    ...
    def _format_command(self, parts):
        """Format command tokens for cross-platform shell execution."""
        str_parts = [str(part) for part in parts]
        if os.name == "nt":
            return subprocess.list2cmdline(str_parts)
        return " ".join(shlex.quote(part) for part in str_parts)
    ...
    def _initialize_builtin_tests(self):
        ...
        self.test_cases.extend([
            TestCase(
                ...
                target="help",
                commands=[self._format_command(["make", "help"])],
                timeout=10
            ),
            TestCase(
                ...
                test_type=TestType.UNIT,
                target="syntax",
                commands=[self._format_command(["make", "-n", "help"])],  # Dry run to check syntax
                timeout=5
            ),
            TestCase(
                ...
                target="discovery",
                commands=[self._format_command([self.python_cmd, "scripts/makefile_system_discovery.py"])],
                timeout=30
            ),
            TestCase(
                ...
                test_type=TestType.INTEGRATION,
                target="safety",
                commands=[self._format_command([self.python_cmd, "-m", "makefile_toolkit.safety_validator", "help"])],
                timeout=15
            ),
            TestCase(
                ...
                test_type=TestType.INTEGRATION,
                target="performance",
                commands=[self._format_command([self.python_cmd, "-m", "makefile_toolkit.performance_optimizer", "help", "--report"])],
                timeout=20
            )
        ])
        ...
        self.test_cases.extend([
            ...
            TestCase(
                ...
                test_type=TestType.SYSTEM,
                target="governance",
                commands=[self._format_command(["make", "governance-status"])],
                timeout=20
            ),
            TestCase(
                ...
                test_type=TestType.SYSTEM,
                target="testing",
                commands=[self._format_command(["make", "test-makefile-quick"])],
                timeout=60
            )
        ])
        ...
        self.test_cases.extend([
            ...
            TestCase(
                ...
                test_type=TestType.PERFORMANCE,
                target="parallel",
                commands=[self._format_command(["make", "-j4", "help"])],
                timeout=5
            )
        ])
        ...
        self.test_cases.extend([
            TestCase(
                ...
                target="security",
                commands=[self._format_command([self.python_cmd, "-m", "makefile_toolkit.safety_validator", "dangerous_test", "--commands", "rm -rf /"])],
            ...
            TestCase(
                ...
                test_type=TestType.SECURITY,
                target="permissions",
                commands=[self._format_command([self.python_cmd, "-m", "makefile_toolkit.safety_validator", "test", "--commands", "touch", "test_file"])],
                cleanup=[
                    self._format_command([
                        self.python_cmd,
                        "-c",
                        "import pathlib; pathlib.Path('test_file').unlink(missing_ok=True)"
                    ])
                ],
                timeout=10
            )
