"""
Ghostbusters Framework Core Data Models

Foundational data structures that enable systematic analysis,
recovery, and validation across the Beast Mode ecosystem.
"""
...
class RecoveryComplexity(Enum):
    ...
    TRIVIAL = "trivial"      # <1s automated fix
    SIMPLE = "simple"        # <5s automated fix
    MODERATE = "moderate"    # <30s automated fix
    COMPLEX = "complex"      # Requires human review
    CRITICAL = "critical"    # Requires expert intervention
...
class Recommendation:
    ...
    effort_estimate: str = ""  # e.g., "5 minutes", "1 hour", "1 day"
...
class RecoveryAction:
    ...
    id: str = field(default_factory=lambda: str(uuid.uuid4()))
    action_type: str = ""  # e.g., "replace", "insert", "delete", "command"
    target: str = ""  # file path, line number, etc.
    content: str = ""  # new content, command to run, etc.
...
class ConsensusResult:
    ...
    resolution_method: str = ""  # e.g., "majority_vote", "weighted_confidence", "expert_override"
    consensus_duration: float = 0.0  # seconds
    created_at: datetime = field(default_factory=datetime.utcnow)
...
class ValidationCertificate:
    ...
    certificate_level: str = ""  # e.g., "basic", "comprehensive", "production_ready"
...
class MultiDimensionalResult:
    ...
    def __post_init__(self):
        ...
        scores = [self.functional_score, self.performance_score, 
                 self.security_score, self.integration_score, self.overall_confidence]
        for score in scores:
    ...
    def is_production_ready(self, threshold: float = 0.8) -> bool:
        ...
        return all(score >= threshold for score in [
            self.functional_score, self.performance_score,
            self.security_score, self.integration_score
        ])