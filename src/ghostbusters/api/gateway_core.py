...
This module was extracted from gateway.py
...
class GhostbustersAPI:
    ...
    def __init__(self, auth_manager: Optional[AuthenticationManager]=None, rate_limiter: Optional[RateLimiter]=None, circuit_breaker: Optional[CircuitBreaker]=None):
    ...
    async def analyze_code(self, target_path: str, analysis_type: str, configuration: Optional[Dict[str, Any]]=None, auth_token: Optional[str]=None) -> AnalysisResult:
        """
        ...
        try:
            context = AnalysisContext(target_path=target_path, analysis_type=analysis_type, configuration=configuration or {})
            ...
            if not agents:
                raise AnalysisError(f'No agents available for analysis type: {analysis_type}')
            if len(agents) == 1:
                result = await agents[0].analyze(context)
            elif not self._consensus_engine:
                result = await agents[0].analyze(context)
            else:
                consensus_result = await self._consensus_engine.build_consensus(agents, context)
                result = consensus_result.unified_result or AnalysisResult(agent_name='Consensus', confidence=consensus_result.confidence)
            self.circuit_breaker.record_success('analyze_code')
            logger.info(f'Analysis completed for {target_path} with confidence {result.confidence}')
            return result
        except Exception as e:
    ...
    async def get_expert_capabilities(self, auth_token: Optional[str]=None) -> Dict[str, List[str]]:
        """
    ...
    async def build_consensus(self, target_path: str, analysis_type: str, confidence_threshold: float=0.8, configuration: Optional[Dict[str, Any]]=None, auth_token: Optional[str]=None) -> ConsensusResult:
        """
        ...
        try:
            ...
            result = await self._consensus_engine.build_consensus(agents, context, confidence_threshold)
            self.circuit_breaker.record_success('build_consensus')
            logger.info(f'Consensus built for {target_path}: {result.consensus_reached}')
            return result
        except Exception as e:
    ...
    async def validate_multi_dimensional(self, target: str, configuration: Optional[Dict[str, Any]]=None, auth_token: Optional[str]=None) -> MultiDimensionalResult:
        """
        ...
        try:
            ...
            context = AnalysisContext(target_path=target, analysis_type='multi_dimensional_validation', configuration=configuration or {}) if configuration else None
            result = await self._validation_framework.multi_dimensional_test(target, context)
    ...
    async def issue_validation_certificate(self, target: str, validation_results: List[ValidationResult], auth_token: Optional[str]=None) -> ValidationCertificate:
        """
        ...
        try:
            if not self._validation_framework:
                raise ValidationError('Validation framework not available')
            certificate = await self._validation_framework.issue_certificate(target, validation_results)
    ...
    async def get_service_health(self, auth_token: Optional[str]=None) -> Dict[str, Any]:
        """
        ...
        health_status = {'timestamp': datetime.utcnow().isoformat(), 'overall_status': 'healthy', 'services': dict(self._service_health), 'circuit_breaker_status': self.circuit_breaker.get_status(), 'rate_limiter_status': self.rate_limiter.get_status(), 'registered_agents': len(self._registered_agents)}
        critical_services = ['coordinator', 'consensus', 'validation']
        unhealthy_services = [service for service in critical_services if not self._service_health.get(service, False)]
        if unhealthy_services:
            health_status['overall_status'] = 'degraded'
            health_status['unhealthy_services'] = unhealthy_services
        return health_status

    async def get_service_metrics(self, auth_token: Optional[str]=None) -> Dict[str, Any]:
        """
        ...
        if not await self._authenticate(auth_token):
            return {'error': 'Authentication failed'}
        return {'timestamp': datetime.utcnow().isoformat(), 'circuit_breaker_metrics': self.circuit_breaker.get_metrics(), 'rate_limiter_metrics': self.rate_limiter.get_metrics(), 'api_version': '1.0.0', 'uptime': 'calculated_uptime_here'}
