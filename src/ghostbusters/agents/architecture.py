...
Provides systematic analysis of software architecture, design patterns,
...
from ..core.models import (
    AnalysisResult, AnalysisContext, Finding, Recommendation,
    FindingType, Severity, CodeLocation
)
...
class ArchitectureExpert(GhostbustersExpertAgent):
    ...
    def __init__(self, name: str = "ArchitectureExpert", version: str = "1.0.0"):
        ...
        self._capabilities = [
            ...
            "dependency_analysis",
            "modularity_analysis",
            "separation_of_concerns_analysis"
        ]
    ...
    def _init_architecture_patterns(self):
        ...
        self.design_patterns = {
            'singleton': [r'class\s+\w+.*:\s*\n.*_instance\s*=\s*None', r'__new__.*cls\._instance'],
            'factory': [r'class\s+\w*Factory', r'def\s+create_\w+'],
            'observer': [r'class\s+\w*Observer', r'def\s+notify', r'def\s+update'],
        ...
        self.solid_violations = {
            'srp': [r'class\s+\w*Manager\w*', r'class\s+\w*Handler\w*'],  # Single Responsibility
            'ocp': [r'if\s+isinstance.*:', r'type\(.*\)\s*=='],  # Open/Closed
            'lsp': [r'raise\s+NotImplementedError'],  # Liskov Substitution
            'isp': [],  # Interface Segregation (harder to detect statically)
            'dip': [r'import\s+\w+\.\w+\.\w+'],  # Dependency Inversion (concrete imports)
        }
    ...
    async def analyze(self, context: AnalysisContext) -> AnalysisResult:
        ...
        try:
            ...
            result = AnalysisResult(
                ...
                context=context,
                metadata={
                    "architecture_patterns_detected": self._get_detected_patterns(findings),
                    "anti_patterns_detected": self._get_detected_anti_patterns(findings),
                    "solid_violations": self._get_solid_violations(findings),
            ...
            return AnalysisResult(
                ...
                confidence=0.0,
                findings=[Finding(
                    type=FindingType.ARCHITECTURE_VIOLATION,
                    severity=Severity.CRITICAL,
                    description=f"Architecture analysis failed: {str(e)}",
                    confidence=1.0
                )],
                recommendations=[Recommendation(
                    title="Fix Analysis Error",
                    description=f"Resolve architecture analysis issue: {str(e)}",
    ...
    def validate_confidence(self, result: AnalysisResult) -> bool:
        ...
        # High confidence should correlate with comprehensive analysis
    ...
    async def _analyze_project_architecture(self, directory: Path) -> List[Finding]:
        ...
        # Analyze all Python files for architecture issues
    ...
    async def _analyze_file_architecture(self, file_path: Path) -> List[Finding]:
        ...
        try:
            ...
            if file_path.suffix == '.py':
                ...
                # Generic architecture analysis for other files
            ...
            findings.append(Finding(
                ...
                description=f"Could not analyze file architecture: {str(e)}",
                confidence=0.6
            ))
    ...
    def _analyze_python_architecture(self, tree: ast.AST, content: str, file_path: Path) -> List[Finding]:
            ...
            def visit_ClassDef(self, node):
                ...
                if class_lines > 500:
                    self.findings.append(Finding(
                        ...
                        description=f"God Class detected: '{node.name}' has {class_lines} lines",
                        confidence=0.8,
                        evidence={"anti_pattern": "god_class", "lines": class_lines, "class_name": node.name}
                    ))
                ...
                if method_count > 20:
                    self.findings.append(Finding(
                        ...
                        confidence=0.7,
                        evidence={"anti_pattern": "god_class", "methods": method_count, "class_name": node.name}
                    ))
                
                # Check for naming patterns that suggest SRP violations
                if any(pattern in node.name.lower() for pattern in ['manager', 'handler', 'controller', 'util']):
                    self.findings.append(Finding(
                        type=FindingType.ARCHITECTURE_VIOLATION,
                        severity=Severity.MEDIUM,
                        location=CodeLocation(str(self.file_path), node.lineno),
                        description=f"Class name '{node.name}' suggests potential SRP violation",
                        confidence=0.6,
                        evidence={"solid_violation": "srp", "class_name": node.name}
                    ))
            ...
            def visit_FunctionDef(self, node):
                ...
                if param_count > 5:
                    self.findings.append(Finding(
                        ...
                        confidence=0.8,
                        evidence={"anti_pattern": "long_parameter_list", "parameters": param_count}
                    ))
        ...
        if visitor.max_nesting > 4:
            findings.append(Finding(
                ...
                severity=Severity.MEDIUM,
                location=CodeLocation(str(file_path), 1),
                description=f"Deep nesting detected (max depth: {visitor.max_nesting})",
                confidence=0.7,
                evidence={"anti_pattern": "deep_nesting", "max_depth": visitor.max_nesting}
            ))
    ...
    def _analyze_generic_architecture(self, content: str, file_path: Path) -> List[Finding]:
        ...
        if len(lines) > 1000:
            findings.append(Finding(
                ...
                severity=Severity.MEDIUM,
                location=CodeLocation(str(file_path), 1),
                description=f"Very long file ({len(lines)} lines) - consider splitting",
                confidence=0.6,
                evidence={"issue": "long_file", "lines": len(lines)}
            ))
    ...
    def _analyze_directory_structure(self, directory: Path) -> List[Finding]:
        ...
        if len(detected_layers) >= 3:
            findings.append(Finding(
                ...
                confidence=0.7,
                evidence={"pattern": "layered_architecture", "layers": detected_layers}
            ))
        
        # Check for flat structure (potential issue)
        ...
        if len(python_files) > 10 and len(subdirs) < 2:
            findings.append(Finding(
                ...
                severity=Severity.MEDIUM,
                location=CodeLocation(str(directory), 1),
                description=f"Flat structure with {len(python_files)} Python files - consider organizing into modules",
                confidence=0.6,
                evidence={"issue": "flat_structure", "file_count": len(python_files)}
            ))
            ...
            for dep in deps:
                if dep in dependencies and module in dependencies[dep]:
                    findings.append(Finding(
                        type=FindingType.ARCHITECTURE_VIOLATION,
                        severity=Severity.HIGH,
                        location=CodeLocation(str(python_files[0].parent), 1),
                        description=f"Circular dependency detected between {module} and {dep}",
                        confidence=0.7,
                        evidence={"anti_pattern": "circular_dependency", "modules": [module, dep]}
                    ))
            ...
            for pattern in patterns:
                if re.search(pattern, content, re.MULTILINE | re.IGNORECASE):
                    findings.append(Finding(
                        ...
                        severity=Severity.INFO,
                        location=CodeLocation(str(file_path), 1),
                        description=f"Design pattern detected: {pattern_name}",
                        confidence=0.6,
                        evidence={"pattern": pattern_name, "type": "design_pattern"}
                    ))
                    break  # Only report each pattern once per file
    ...
    def _check_solid_violations(self, content: str, file_path: Path) -> List[Finding]:
        ...
        # Check for type checking (OCP violation)
        if re.search(r'if\s+isinstance\s*\(.*,\s*\w+\)', content) or re.search(r'type\s*\(.*\)\s*==', content):
            findings.append(Finding(
                type=FindingType.ARCHITECTURE_VIOLATION,
                severity=Severity.MEDIUM,
                location=CodeLocation(str(file_path), 1),
                description="Potential Open/Closed Principle violation - type checking detected",
                confidence=0.6,
                evidence={"solid_violation": "ocp", "issue": "type_checking"}
            ))
        
        # Check for NotImplementedError (LSP violation)
        if 'NotImplementedError' in content:
            findings.append(Finding(
                type=FindingType.ARCHITECTURE_VIOLATION,
                severity=Severity.MEDIUM,
                location=CodeLocation(str(file_path), 1),
                description="Potential Liskov Substitution Principle violation - NotImplementedError found",
                confidence=0.7,
                evidence={"solid_violation": "lsp", "issue": "not_implemented"}
            ))
        
        # Check for concrete imports (DIP violation)
        concrete_import_pattern = r'from\s+\w+\.\w+\.\w+\s+import\s+\w+'
        if re.search(concrete_import_pattern, content):
            findings.append(Finding(
                type=FindingType.ARCHITECTURE_VIOLATION,
                severity=Severity.LOW,
                location=CodeLocation(str(file_path), 1),
                description="Potential Dependency Inversion Principle violation - concrete imports detected",
                confidence=0.5,
                evidence={"solid_violation": "dip", "issue": "concrete_imports"}
            ))
    ...
    async def _generate_architecture_recommendations(self, findings: List[Finding]) -> List[Recommendation]:
        ...
        if god_classes:
            recommendations.append(Recommendation(
                ...
                description=f"Break down {len(god_classes)} large class(es) into smaller, focused classes",
                priority=Severity.HIGH,
                effort_estimate="2-4 hours per class",
        ...
        if solid_violations:
            recommendations.append(Recommendation(
                ...
                description=f"Refactor {len(solid_violations)} SOLID violation(s) to improve design",
                priority=Severity.MEDIUM,
                effort_estimate="1-2 hours per violation",
                automated_fix_available=False
            ))
        ...
        if circular_deps:
            recommendations.append(Recommendation(
                ...
                description=f"Refactor {len(circular_deps)} circular dependency(ies)",
                priority=Severity.HIGH,
                effort_estimate="1-3 hours per dependency",
                automated_fix_available=False
            ))
    ...
    def _calculate_architecture_confidence(self, findings: List[Finding], target_path: Path) -> float:
        ...
        # Adjust based on file type and analysis depth
        if target_path.is_dir():
            python_files = list(target_path.rglob("*.py"))
            if python_files:
                base_confidence = 0.8  # Better analysis for Python projects
