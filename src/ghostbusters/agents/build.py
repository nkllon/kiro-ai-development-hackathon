...
Provides systematic analysis of build systems, dependency management,
and build configuration best practices.
"""
...
class BuildExpert(GhostbustersExpertAgent):
    ...
    async def analyze(self, context: AnalysisContext) -> AnalysisResult:
        ...
        try:
            ...
            if target_path.is_dir():
                ...
                findings.extend(await self._analyze_project_directory(target_path))
            elif target_path.is_file():
                # Analyze specific build file
            ...
            result = AnalysisResult(
                ...
                context=context,
                metadata={
                    "build_systems_detected": self._get_detected_build_systems(target_path),
    ...
    def validate_confidence(self, result: AnalysisResult) -> bool:
        ...
        # High confidence should correlate with detected build systems
    ...
    async def _analyze_project_directory(self, directory: Path) -> List[Finding]:
        """Analyze project directory for build issues"""
        findings = []
        
        # Find and analyze all build files
        ...
        findings.extend(self._check_missing_build_files(directory))
        
        # Check for build system conflicts
        findings.extend(self._check_build_conflicts(directory))
        
        return findings
    
    async def _analyze_build_file(self, file_path: Path) -> List[Finding]:
        """Analyze specific build file"""
        findings = []
    ...
    async def _analyze_package_json(self, file_path: Path) -> List[Finding]:
        ...
        try:
            ...
            if 'dependencies' in package_data:
                findings.extend(self._check_dependency_versions(
            ...
            findings.extend(self._check_known_vulnerable_packages(
                package_data.get('dependencies', {}), file_path
            ))
            
            # Check scripts section
            if 'scripts' in package_data:
                findings.extend(self._analyze_npm_scripts(
                    package_data['scripts'], file_path
                ))
    ...
    async def _analyze_requirements_txt(self, file_path: Path) -> List[Finding]:
        ...
        try:
            ...
            for line_num, line in enumerate(lines, 1):
                ...
                if package_name.lower() in ['pillow', 'requests', 'urllib3']:  # Example vulnerable packages
                    findings.append(Finding(
                        ...
                        confidence=0.6,
                        evidence={"package": package_name, "issue": "potential_vulnerability"}
                    ))
    ...
    async def _analyze_pyproject_toml(self, file_path: Path) -> List[Finding]:
        ...
        try:
            ...
            data = {}
            try:
                # Simple TOML parsing for basic sections
    ...
    async def _analyze_pom_xml(self, file_path: Path) -> List[Finding]:
        ...
        try:
            ...
            # Basic XML validation and Maven-specific checks
    ...
    def _check_missing_build_files(self, directory: Path) -> List[Finding]:
        ...
        if python_files and not any((directory / f).exists() for f in ['requirements.txt', 'pyproject.toml', 'setup.py']):
            findings.append(Finding(
                ...
                confidence=0.8,
                evidence={"project_type": "python", "missing_files": ["requirements.txt", "pyproject.toml", "setup.py"]}
            ))
        ...
        if js_files and not (directory / "package.json").exists():
            findings.append(Finding(
                ...
                confidence=0.8,
                evidence={"project_type": "javascript", "missing_files": ["package.json"]}
            ))
    ...
    def _check_build_conflicts(self, directory: Path) -> List[Finding]:
        ...
        # Check for multiple Python dependency managers
        python_files = []
        ...
        if len(python_files) > 2:
            findings.append(Finding(
                ...
                confidence=0.7,
                evidence={"conflict_type": "python_dependencies", "files": python_files}
            ))
        ...
        for dep_name, version in dependencies.items():
            ...
            if version in ['*', 'latest'] or version.startswith('^') or version.startswith('~'):
                findings.append(Finding(
                    ...
                    confidence=0.7,
                    evidence={"dependency": dep_name, "version": version, "section": section}
                ))
    ...
    def _check_known_vulnerable_packages(self, dependencies: Dict[str, str], file_path: Path) -> List[Finding]:
        ...
        known_vulnerable = {
            'lodash': ['4.17.15', '4.17.16'],  # Example
            'moment': ['2.24.0'],  # Example
        }
        ...
        for dep_name, version in dependencies.items():
            if dep_name in known_vulnerable:
                findings.append(Finding(
                    ...
                    evidence={"package": dep_name, "version": version, "vulnerability": "known_vulnerable"}
                ))
    ...
    def _calculate_build_confidence(self, findings: List[Finding], target_path: Path) -> float:
        ...
        # Adjust based on detected build systems
