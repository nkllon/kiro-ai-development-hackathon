...
def filter_by_security_clearance(graph, clearance_level):
    ...
    for subj, pred, obj in graph:
        ...
        if classification:
            resource_level = clearance_hierarchy.get(
                str(classification).split("#")[-1], 0
            )
...
def validate_with_security_and_temporal(
    data_files, shape_files, user_context=None, as_of_date=None
):
    ...
    for f in data_files:
        ...
        if user_context and hasattr(user_context, "clearance_level"):
            filtered_g = filter_by_security_clearance(
                temp_g, user_context.clearance_level
            )
            data_g += filtered_g
        else:
            data_g += temp_g
...
def main():
    parser = argparse.ArgumentParser(
        description="Validate RDF data against SHACL shapes with security and temporal support"
    )
    parser.add_argument(
        "--data", nargs="+", required=True, help="RDF data files to validate"
    )
    ...
    parser.add_argument(
        "--as-of-date",
        help="ISO datetime for temporal filtering (e.g., 2024-01-15T10:30:00Z)",
    )
    parser.add_argument(
        "--verbose", action="store_true", help="Verbose output including statistics"
    )
    ...
    try:
        conforms, results_text, stats = validate_with_security_and_temporal(
            args.data, args.shapes, user_context, args.as_of_date
        )
        ...
        if args.verbose:
            ...
            print(
                f"  Temporal filtering: {stats['temporal_filtering']}", file=sys.stderr
            )
            print(
                f"  User clearance: {args.user_clearance or 'public'}", file=sys.stderr
            )
