...
authoritative source for failure_recovery.
...
class FailureRecoverySystem:
    ...
    def _calculate_success_rate(self) -> float:
        ...
        if not self.recovery_history:
            return 0.0
        successful = len([p for p in self.recovery_history if p.status == RecoveryStatus.COMPLETED])
    ...
    def _calculate_average_recovery_time(self) -> float:
        ...
        if not self.recovery_history:
            return 0.0
        total_time = sum((p.estimated_total_duration.total_seconds() for p in self.recovery_history))
