...
authoritative source for boundary_resolver.
...
class ComponentBoundaryResolver(ReflectiveModule):
    ...
    def _create_component_boundary(self, spec_name: str) -> Optional[ComponentBoundary]:
        ...
        try:
            ...
            return ComponentBoundary(component_name=spec_name, component_type=component_type, primary_responsibilities=responsibilities, boundary_constraints=constraints, interface_contracts=[f"{spec_name.title().replace('-', '')}Interface"], allowed_dependencies=[], forbidden_access=[], shared_services=[])
        except Exception as e:
        ...
        for dep_name in boundary.allowed_dependencies:
            ...
            for contract in contracts:
                if contract.provider_component == dep_name:
                    ...
                    break
            dependency = DependencyRelationship(dependent_component=boundary.component_name, dependency_component=dep_name, dependency_type='interface' if interface_contract else 'service', interface_contract=interface_contract, is_circular=False, validation_status='valid')
    ...
    def _detect_boundary_violations(self, boundaries: List[ComponentBoundary], contracts: List[InterfaceContract], dependency_graph: Dict[str, List[DependencyRelationship]]) -> List[BoundaryViolation]:
        ...
        for boundary in boundaries:
            for responsibility in boundary.primary_responsibilities:
                if responsibility in responsibility_map:
                    violation = BoundaryViolation(violation_type=BoundaryViolationType.FUNCTIONAL_OVERLAP, violating_component=boundary.component_name, target_component=responsibility_map[responsibility], description=f'Overlapping responsibility: {responsibility}', severity='medium', remediation_steps=['Clarify responsibility ownership', 'Move responsibility to single component', 'Create shared service if needed'], detected_at=datetime.now())
