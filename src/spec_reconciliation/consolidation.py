...
authoritative source for consolidation.
...
class SpecConsolidator(ReflectiveModule):
    ...
    def _extract_requirements(self, content: str) -> List[RequirementAnalysis]:
        ...
        for i, (title, user_story, criteria_text) in enumerate(matches):
            ...
            requirements.append(RequirementAnalysis(requirement_id=f'R{i + 1}', content=req_text, functionality_keywords=functionality_keywords, acceptance_criteria=criteria_lines, stakeholder_personas=stakeholder_personas, complexity_score=self._calculate_requirement_complexity(req_text), quality_score=self._calculate_requirement_quality(criteria_lines)))
        return requirements
    ...
    def _calculate_requirement_quality(self, criteria_lines: List[str]) -> float:
        ...
        if not criteria_lines:
            return 0.0
        quality_indicators = {'has_criteria': 0.4 if criteria_lines else 0, 'criteria_completeness': min(len(criteria_lines) / 5, 1.0) * 0.3, 'uses_ears_format': 0.3 if any(('WHEN' in line and 'THEN' in line and ('SHALL' in line) for line in criteria_lines)) else 0}
        ...
        for spec_name, spec_data in parsed_specs.items():
            ...
            for other_spec, other_data in parsed_specs.items():
                if other_spec != spec_name:
                    ...
                    shared_interfaces = set((interface['name'] for interface in spec_data['interfaces'])).intersection(set((interface['name'] for interface in other_data['interfaces'])))
...
class UnifiedRequirement:
    ...
    def _define_validation_criteria(self, target_specs: List[str], overlap_analysis: OverlapAnalysis) -> List[ValidationCriterion]:
        ...
        validation_criteria.append(ValidationCriterion(criterion_id='QUAL_001', description='Overall specification quality is maintained or improved', validation_method='quality_score_comparison', success_threshold=0.9, measurement_approach='Calculate weighted average quality scores'))
        validation_criteria.append(ValidationCriterion(criterion_id='TRACE_001', description='Complete traceability is maintained', validation_method='traceability_completeness_check', success_threshold=1.0, measurement_approach='Verify all original requirements have traceability links'))
