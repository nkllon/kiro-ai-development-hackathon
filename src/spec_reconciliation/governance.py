...
class GovernanceFramework(ReflectiveModule):
    ...
    def implement_training_programs(self) -> Dict[str, Any]:
        ...
        for program in self.training_programs:
            program_result = {'program_id': program.program_id, 'title': program.title, 'modules_created': len(program.modules), 'assessment_criteria': len(program.assessment_criteria), 'target_roles': [role.value if hasattr(role, 'value') else str(role) for role in program.target_roles]}
