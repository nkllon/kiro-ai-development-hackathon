...
def run_ghostbusters_critical_consultation():
    ...
    try:
        ...
        if (
            consultation_report["risk_assessment"]["level"] == "high"
            or consultation_report["risk_assessment"]["score"] > 0.8
        ):
...
def main():
    ...
    try:
        ...
        if consultation_report:
            ...
            print(f"Recommendation: {consultation_report['recommendation']}")
            print(
                f"Emergency Protocols: {'ACTIVATED' if consultation_report['risk_assessment']['level'] == 'high' else 'STANDBY'}"
            )
        else:
            print(f"\n❌ GHOSTBUSTERS CONSULTATION FAILED")
