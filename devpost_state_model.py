...
class DevPostStateModel:
    ...
    def identify_page_type(self, url: str, title: str, page_content: Dict[str, Any]) -> PageType:
        ...
        if "login" in url_lower or "sign" in url_lower:
        ...
        elif "manage-team" in url_lower or "team" in title_lower or 'team_management' in form_indicators:
        ...
        elif "project-details" in url_lower or "details" in title_lower or "project_details" in url_lower:
            return PageType.PROJECT_DETAILS
        elif "additional-info" in url_lower or "additional" in title_lower or 'additional_info' in form_indicators:
            return PageType.ADDITIONAL_INFO
        elif "review" in url_lower or "review" in title_lower or "finalization" in url_lower:
            ...
            if any('current_step' in indicator for indicator in step_indicators):
                current_step_text = next((indicator.split(': ')[1] for indicator in step_indicators if 'current_step' in indicator), '')
    ...
    def extract_navigation_elements(self, page_content: Dict[str, Any]) -> List[NavigationElement]:
        """Extract navigation elements from page content"""
        navigation_elements = []
        
        # Extract step navigation
        for nav in page_content.get('navigationElements', []):
            navigation_elements.append(NavigationElement(
                text=nav.get('text', ''),
                href=nav.get('href'),
                element_type='step',
                is_clickable=True,
        ...
        for interactive in page_content.get('interactiveElements', []):
            if interactive.get('tag') in ['button', 'a']:
                navigation_elements.append(NavigationElement(
                    ...
                    element_type='button' if interactive.get('tag') == 'button' else 'link',
                    is_clickable=True
                ))
    ...
    def update_page_state(self, url: str, title: str, page_content: Dict[str, Any]) -> PageState:
        ...
        page_state = PageState(
            ...
            other_navigation_options=[nav for nav in navigation_elements if nav.element_type != 'step'],
        ...
        self.session_state.total_steps_identified = len([nav for nav in navigation_elements if nav.element_type == 'step'])
        self.session_state.steps_completed = len([state for state in self.session_state.page_history if state.form_status == FormStatus.COMPLETED])
        self.session_state.forms_completed = len([state for state in self.session_state.page_history if state.form_status == FormStatus.COMPLETED])
    ...
    def get_navigation_recommendations(self) -> Dict[str, Any]:
        ...
        recommendations = {
            "current_step": current_page.current_step.text if current_page.current_step else "Unknown",
            "next_actions": [],
            "warnings": [],
            "progress": {
                "steps_completed": self.session_state.steps_completed,
                "total_steps": self.session_state.total_steps_identified,
                "completion_percentage": (self.session_state.steps_completed / max(self.session_state.total_steps_identified, 1)) * 100
            }
        }
    ...
    def save_state(self):
        ...
        state_data = {
            ...
            "session_preservation": {
                ...
                "last_url": self.session_state.current_page.url if self.session_state.current_page else None
            },
            "metadata": {
                "version": "1.0",
    ...
    def detect_current_condition(self, url: str, title: str, page_content: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if total_steps > 0:
            ...
            detection["progress"] = f"{completed_count}/{total_steps} ({progress_percentage:.1f}%)"
        ...
        if detection["confidence"] == "low":
            detection["warnings"].append("Unable to determine current state with high confidence")
    ...
    def check_session_resume_capability(self) -> Dict[str, Any]:
        ...
        if os.path.exists(browser_data_path):
            ...
            resume_info["resume_instructions"].append("💾 Browser session data preserved")
            ...
            try:
                with open(self.state_file, 'r') as f:
                    ...
                    if 'session_preservation' in state_data:
                        resume_info["last_url"] = state_data['session_preservation'].get('last_url')
                        resume_info["resume_instructions"].append("📄 Previous session state available")
                        resume_info["can_resume"] = True
            except:
                pass
