...
class BeastModeDebugSystem:
    ...
    def __init__(self):
        self.debug_session_id = (
            f"beast_mode_debug_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
        self.trace_data = {}
        self.call_stack = []
        self.debug_log = []
    ...
    def register_debug_hooks(self):
        ...
        def enhanced_excepthook(exc_type, exc_value, exc_traceback):
            ...
            self.capture_comprehensive_trace(
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                exit_trace = {
                    ...
                    "result_size": (
                        len(str(result)) if hasattr(result, "__len__") else "unknown"
                    ),
                }
    ...
    def capture_comprehensive_trace(
        self, trigger: str, exc_type=None, exc_value=None, exc_traceback=None
    ):
        ...
        try:
            ...
            self.logger.error(
                f"TRACE DATA: {json.dumps(trace_data, default=str)[:1000]}..."
            )
        ...
        try:
            ...
            return {
                ...
                "files_in_current_dir": [
                    f.name for f in current_dir.iterdir() if f.is_file()
                ],
                "directories_in_current_dir": [
...
def stop_and_dump_trace(reason: str = "stop_requested"):
    ...
    if debug_system:
        ...
        emergency_dump = (
            f"emergency_dump_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
        try:
