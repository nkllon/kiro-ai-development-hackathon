...
from dynamic_session_classifier import SessionVector, SessionClassifier, MultiDimensionalSessionAnalyzer
...
class AdjacencyClusterAnalyzer:
            ...
            for j, hash2 in enumerate(vector_hashes):
                if i != j:
                    ...
                    adjacency_result = AdjacencyResult(
                        ...
                        adjacency_type=self._classify_adjacency_type(similarity["overall_similarity"])
                    )
    ...
    def _calculate_vector_similarity(self, vector1: SessionVector, vector2: SessionVector) -> Dict[str, Any]:
        ...
        weighted_similarity = sum(
            similarities.get(dim, 0.0) * weights.get(dim, 1.0)
    ...
    def _find_clusters(self) -> Dict[str, ClusterAnalysis]:
        ...
        for vector_hash, vector in self.vectors.items():
            ...
            for other_hash, adjacency_result in self.adjacency_matrix[vector_hash].items():
                if (other_hash not in processed_vectors and 
                    adjacency_result.adjacency_type in ["exact_match", "high_similarity"]):
            ...
            if len(cluster_vectors) >= 2:
                ...
                print(f"✅ Found cluster: {cluster.cluster_name} ({len(cluster_vectors)} vectors)")
    ...
    def _create_cluster(self, vector_hashes: List[str]) -> ClusterAnalysis:
        ...
        for dimension in all_dimensions:
            ...
            variance[dimension] = sum((val - mean_val) ** 2 for val in values) / len(values)
        ...
        cohesion_score = sum(cohesion_scores) / len(cohesion_scores) if cohesion_scores else 0.0
            ...
            for vector_hash in vector_hashes:
                for other_hash in cluster.representative_vectors:
                    if other_hash in self.vectors:
                        similarity = self.adjacency_matrix[vector_hash][other_hash].similarity_score
                        distance = 1.0 - similarity
                        min_distance = min(min_distance, distance)
            
            separation_scores.append(min_distance)
        
        return sum(separation_scores) / len(separation_scores) if separation_scores else 1.0
    
    def _find_representative_vectors(self, vectors: List[SessionVector], centroid: Dict[str, float]) -> List[str]:
    ...
    def _find_cluster_outliers(self, vectors: List[SessionVector], centroid: Dict[str, float]) -> List[str]:
            ...
            for vector_hash in self.vectors.keys():
                ...
                for rep_hash in cluster.representative_vectors:
                    if rep_hash in self.vectors:
                        similarity = self.adjacency_matrix[vector_hash][rep_hash].similarity_score
    ...
    def _analyze_outlier(self, vector_hash: str, vector: SessionVector) -> OutlierAnalysis:
        ...
        if self.clusters:
            ...
            for other_hash, other_vector in self.vectors.items():
                if other_hash != vector_hash:
                    distance = 0.0
                    all_dimensions = set(vector.dimensions.keys()) | set(other_vector.dimensions.keys())
                    for dimension in all_dimensions:
    ...
    def _suggest_outlier_class_name(self, vector: SessionVector) -> str:
        ...
        for dimension, value in dimensions.items():
            if value > 0.8:
                extreme_dimensions.append(f"Extreme-{dimension.replace('_', ' ').title()}")
            elif value < 0.2:
                extreme_dimensions.append(f"Minimal-{dimension.replace('_', ' ').title()}")
    ...
    def _analyze_cluster_patterns(self) -> Dict[str, Any]:
        ...
        patterns = {
            "cluster_sizes": [cluster.vector_count for cluster in self.clusters.values()],
            "cohesion_scores": [cluster.cohesion_score for cluster in self.clusters.values()],
            "separation_scores": [cluster.separation_score for cluster in self.clusters.values()],
            "dominant_dimensions": {},
        ...
        for cluster1_id, cluster1 in self.clusters.items():
            ...
            for cluster2_id, cluster2 in self.clusters.items():
                if cluster1_id != cluster2_id:
                    ...
                    distance = 0.0
                    all_dimensions = set(cluster1.centroid.keys()) | set(cluster2.centroid.keys())
                    for dimension in all_dimensions:
                        val1 = cluster1.centroid.get(dimension, 0.0)
                        val2 = cluster2.centroid.get(dimension, 0.0)
                        distance += (val1 - val2) ** 2
                    
                    patterns["cluster_relationships"][cluster1_id][cluster2_id] = distance
        
        return patterns
    
    def _generate_insights(self, clusters: Dict[str, ClusterAnalysis], outliers: List[OutlierAnalysis]) -> List[str]:
        ...
        if clusters:
            ...
            insights.append(f"Largest cluster: {largest_cluster.cluster_name} ({largest_cluster.vector_count} vectors)")
            
            most_cohesive = max(clusters.values(), key=lambda c: c.cohesion_score)
            insights.append(f"Most cohesive cluster: {most_cohesive.cluster_name} (cohesion: {most_cohesive.cohesion_score:.2f})")
            
            most_separated = max(clusters.values(), key=lambda c: c.separation_score)
            insights.append(f"Most separated cluster: {most_separated.cluster_name} (separation: {most_separated.separation_score:.2f})")
        ...
        if outliers:
            ...
            if potential_new_classes:
                insights.append(f"{len(potential_new_classes)} outliers suggest new session classes:")
                for outlier in potential_new_classes:
                    insights.append(f"  - {outlier.suggested_class_name} (isolation: {outlier.isolation_level})")
            
            completely_isolated = [o for o in outliers if o.isolation_level == "completely_isolated"]
            if completely_isolated:
                insights.append(f"{len(completely_isolated)} vectors are completely isolated - strong candidates for new classes")
        
        # Pattern insights
        if len(clusters) > 1:
            insights.append("Multiple clusters suggest diverse session types in the dataset")
        
        if len(outliers) > len(clusters):
            insights.append("High number of outliers suggests the dataset may contain many unique session patterns")
