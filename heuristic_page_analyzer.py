...
def analyze_page_heuristics():
    ...
    try:
        ...
        def analyze_page_state(page, label=""):
            ...
            step_indicators = page.query_selector_all(
                "[class*='step'], [id*='step'], [data-step]"
            )
        ...
        for step in initial_state["step_analysis"]:
            ...
            if (
                current_found
                and "current" not in step["classes"]
                and step["visible"]
                and step["enabled"]
            ):
                next_step = step
        ...
        try:
            ...
            for i in range(10):
                ...
                if current_title != initial_state["title"]:
                    print(
                        f"✅ TITLE CHANGED! {initial_state['title']} -> {current_title}"
                    )
                    break

                # Check if step navigation changed
                current_steps = target_page.query_selector_all(
                    "#steps-navigation a.step"
                )
                if len(current_steps) != len(initial_state["step_analysis"]):
                    print(
                        f"✅ STEP COUNT CHANGED! {len(initial_state['step_analysis'])} -> {len(current_steps)}"
                    )
                    break
                ...
                for step in current_steps:
                    if "current" in step.get_attribute("class") or "":
                        ...
                        if current_step_text != next_step["text"]:
                            print(
                                f"✅ CURRENT STEP CHANGED! {next_step['text']} -> {current_step_text}"
                            )
                            break
            ...
            print(
                f"   Title changed: {initial_state['title']} -> {final_state['title']}"
            )
            print(
                f"   Step count: {len(initial_state['step_analysis'])} -> {len(final_state['step_analysis'])}"
            )

            # Check if we're on a different step
            if final_state["step_info"].get("current_step") != initial_state[
                "step_info"
            ].get("current_step"):
                print(f"✅ STEP NAVIGATION SUCCESSFUL!")
                print(
                    f"   From: {initial_state['step_info'].get('current_step', 'unknown')}"
                )
                print(
                    f"   To: {final_state['step_info'].get('current_step', 'unknown')}"
                )
            else:
