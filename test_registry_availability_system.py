...
def test_boot_time_registry_check():
    ...
    try:
        ...
        for registry_name, registry_info in results['critical_registries'].items():
            ...
            print(f"   {status_icon} {registry_name}: {registry_info['health_score']:.1%}")
...
'''
        ...
        if result.success:
            print("✅ Field modification completed successfully with registry availability check")
        else:
            print("❌ Field modification failed - registry availability check prevented unsafe operation")
...
def test_graceful_shutdown_scenario():
    ...
    try:
        ...
        print(f"   Graceful Shutdown Required: {health_report.graceful_shutdown_required}")
...
def main():
    ...
    try:
        ...
        print(f"   Field Modification with Registry Check: {'✅' if field_mod_safe else '❌'}")
        print(f"   Graceful Shutdown Required: {'🚨' if graceful_shutdown_required else '✅'}")
        
        overall_success = (
            git_available and 
            memory_available and 
            filesystem_available and 
            boot_time_safe and 
            pre_use_safe and 
            field_mod_safe and 
            not graceful_shutdown_required
        )
