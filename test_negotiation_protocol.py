...
from negotiation_protocol import negotiate_impasse_resolution
...
def test_technical_impasse():
    ...
    print(
        "   Chosen Option: {result.chosen_option.title if result.chosen_option else 'None'}"
    )
    print("   Session Preserved: {result.session_preserved}")
    print("   Breadcrumbs Left: {len(result.breadcrumbs_left)}")
...
def test_negotiation_options_generation():
    ...
    for impasse_type, description in test_cases:
        ...
        for i, option in enumerate(options[:3], 1):  # Show first 3 options
            ...
            print(
                "   {i}. {option.title} {risk_icon} (Success: {option.estimated_success_probability:.0%})"
            )
    return True
...
def test_breadcrumb_creation():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            protocol = NegotiationProtocol()
            protocol.initiate_negotiation(context)
            # Check if breadcrumb file was created
            breadcrumb_files = list(
                Path(temp_dir).glob("negotiation_breadcrumbs_*.json")
            )
            if breadcrumb_files:
                print("✅ Breadcrumb file created: {breadcrumb_files[0].name}")
                ...
                missing_fields = [
                    field for field in required_fields if field not in breadcrumb_data
                ]
                if not missing_fields:
                    print("✅ Breadcrumb content validation passed")
                    return True
                else:
                    print("❌ Missing breadcrumb fields: {missing_fields}")
                    return False
            else:
...
def test_session_preservation_priority():
    ...
    print("📊 SESSION PRESERVATION TEST:")
    print("   Session Preserved: {result.session_preserved}")
    print("   Breadcrumbs Left: {len(result.breadcrumbs_left)}")
    print(
        "   Chosen Option Impact: {result.chosen_option.session_impact if result.chosen_option else 'None'}"
    )
...
def main():
    ...
    print("Success Rate: {passed/total:.1%}")
    if passed == total:
