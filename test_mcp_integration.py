...
def test_enhanced_demo():
    ...
    try:
        ...
        result = subprocess.run(
            ["make", "enhanced-demo"], capture_output=True, text=True, timeout=30
        )
...
def test_interface_governance():
    ...
    try:
        result = subprocess.run(
            ["make", "validate-interfaces"], capture_output=True, text=True, timeout=15
        )
            ...
            if registry_file.exists():
                ...
                try:
                    with open(registry_file) as f:
                        ...
                        print(
                            f"✅ Registry contains {total_interfaces} interfaces, {duplicates} duplicates"
                        )
                except json.JSONDecodeError:
