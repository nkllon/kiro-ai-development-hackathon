...
try:
    ...
    print(
        "⚠️  Google Cloud SDK not available - install with: uv add --dev google-cloud-billing"
    )

from beast_mode.billing.interfaces import (
    BillingMetrics,
    BillingProviderType,
    HealthStatus,
    ReflectiveModule,
...
class LiveGCPBillingMonitor(ReflectiveModule):
    ...
    async def get_real_billing_data(self) -> BillingMetrics:
        ...
        try:
            ...
            projects_response = self.client.list_project_billing_info(
...
async def live_fire_test():
    ...
    try:
        ...
        print(
            f"   Account Status: {billing_metrics.usage_metrics.get('connection_status')}"
        )
