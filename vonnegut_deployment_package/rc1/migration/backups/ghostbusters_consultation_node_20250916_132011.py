...
class GhostbustersConsultation:
    ...
    def run_autonomous_investigation(self, state: DevPostState) -> Dict[str, Any]:
        ...
        investigation_results = self._run_comprehensive_investigation(
            current_page_data, state
        )
    ...
    def _run_comprehensive_investigation(
        self, page_data: Dict[str, Any], state: DevPostState
    ) -> Dict[str, Any]:
        ...
        investigation["similarity_type"] = self._determine_similarity_type(
            investigation, self.test_results
        )

        print(
            f"   Investigation complete: {investigation['similarity_type']} similarity detected"
        )
    ...
    def _identify_completion_indicators(self, buttons: List[Dict]) -> List[str]:
        ...
        for button in buttons:
            ...
            if any(
                word in text
                for word in ["submit", "save", "continue", "next", "finish"]
            ):
    ...
    def _identify_opportunities(self, page_data: Dict[str, Any]) -> List[str]:
        ...
        buttons = page_data.get("buttons", [])
        if any(
            btn.get("text", "").lower() in ["continue", "next", "submit"]
            for btn in buttons
        ):
    ...
    def _determine_similarity_type(
        self, investigation: Dict[str, Any], test_results: Dict[str, Any]
    ) -> str:
        """Determine the overall similarity type based on investigation"""

        # Use investigation results to determine similarity
        if (
            test_results.get("url_recognized")
            and investigation["page_analysis"]["url_pattern"]["pattern"] == "devpost"
        ):
    ...
    def _generate_recommendations(
        self, investigation: Dict[str, Any], confidence: float
    ) -> Dict[str, Any]:
