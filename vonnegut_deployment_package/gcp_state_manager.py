...
class GCPStateManager:
    ...
    def create_session(
        self, setup_type: SetupType, configuration: Dict[str, Any]
    ) -> str:
    ...
    def rollback_session(self) -> bool:
        ...
        with self.state_lock:
            ...
            completed_steps = [
                s for s in self.current_session.steps if s.state == SetupState.COMPLETED
            ]
    ...
    def is_session_complete(self) -> bool:
        ...
        return all(
            step.state == SetupState.COMPLETED for step in self.current_session.steps
        )
    ...
    def _get_steps_for_setup_type(self, setup_type: SetupType) -> List[SetupStep]:
        ...
        if setup_type == SetupType.PERSONAL:
            return [
                SetupStep(
                    "create_billing", "Create Billing Account", SetupState.NOT_STARTED
                ),
                SetupStep(
                    "create_project",
                    "Create Project",
                    SetupState.NOT_STARTED,
                    dependencies=["create_billing"],
                ),
                SetupStep(
                    "link_billing",
                ...
                SetupStep(
                    ...
                    "Enable APIs",
                    SetupState.NOT_STARTED,
                    dependencies=["link_billing"],
                ),
                SetupStep(
                    "create_gke",
                    "Create GKE Cluster",
                    SetupState.NOT_STARTED,
                    dependencies=["enable_apis"],
                ),
                SetupStep(
                    "setup_billing",
                    "Setup Billing Exports",
                    SetupState.NOT_STARTED,
                    dependencies=["create_gke"],
                ),
            ]
        elif setup_type == SetupType.ENTERPRISE:
            return [
                SetupStep(
                    "verify_billing", "Verify Billing Account", SetupState.NOT_STARTED
                ),
                SetupStep(
                    "create_project",
                    "Create Project",
                    SetupState.NOT_STARTED,
                    dependencies=["verify_billing"],
                ),
                SetupStep(
                    "link_billing",
                    "Link Project to Billing",
            ...
            return [
                SetupStep(
                    "request_auth", "Request Authorization", SetupState.NOT_STARTED
                ),
                SetupStep(
                    "admin_review",
                    "Admin Review",
                    SetupState.NOT_STARTED,
                    dependencies=["request_auth"],
                ),
                SetupStep(
                    "verify_auth",
                    "Verify Authorization",
                    SetupState.NOT_STARTED,
                    dependencies=["admin_review"],
                ),
                SetupStep(
                    "link_project",
                    "Link Project with Auth",
                    SetupState.NOT_STARTED,
                    dependencies=["verify_auth"],
                ),
                SetupStep(
                    "apply_iam",
                    "Apply IAM Permissions",
                    SetupState.NOT_STARTED,
                    dependencies=["link_project"],
                ),
            ]
        else:
            return []
...
def main():
    ...
    config = {"project_name": "test-project", "billing_account_id": "test-billing-123"}
