...
from src.competitive_launch.launch_execution import (
    LaunchExecutionSystem, LaunchStatus, PlatformStatus
)
...
class Phase5LaunchExecutionDemo:
    ...
    def _demo_production_deployment(self):
        ...
        try:
            ...
            if self.launch_system.launch_metrics:
                ...
                print(f"\n📈 Deployment Metrics:")
                print(f"   • Platforms Deployed: {metrics.platforms_deployed}/{metrics.total_platforms}")
                print(f"   • Launch Duration: {self.launch_system._calculate_launch_duration()}")
                print(f"   • Success Rate: {(metrics.platforms_deployed/metrics.total_platforms)*100:.1f}%")
            ...
            print(f"   • Overall System Status: {'✅ Healthy' if avg_health > 0.8 else '⚠️  Degraded' if avg_health > 0.5 else '❌ Unhealthy'}")
    ...
    def _demo_competitive_monitoring(self):
        ...
        try:
            ...
            intelligence = self.launch_system.intelligence_engine.calculate_competitive_advantage()
            print(f"   • Overall Advantage: {intelligence.get('overall_advantage', 0):.2f}")
            print(f"   • Systematic Superiority: {intelligence.get('systematic_superiority_score', 0):.2f}")
            print(f"   • Time to Market Advantage: {intelligence.get('time_to_market_advantage', 0):.2f}")
            print(f"   • Quality Advantage: {intelligence.get('quality_advantage', 0):.2f}")
    ...
    def _demo_success_metrics_tracking(self):
        ...
        try:
            ...
            print(f"   • Competitive Responses: {metrics.get('competitive_responses', 0)}")
            ...
            print(f"   • Average Improvement: {superiority.get('average_improvement_percentage', 0):.1f}%")
            print(f"   • High Confidence Metrics: {superiority.get('high_confidence_metrics', 0)}")
            print(f"   • Superiority Verified: {superiority.get('superiority_verified', False)}")
            print(f"   • Competitive Advantage Level: {superiority.get('competitive_advantage_level', 'Unknown')}")
            ...
            print(f"   • Platforms Active: {len([p for p in status.get('platforms', {}).values() if p.get('status') == 'active'])}")
    ...
    def _demo_integrated_launch_execution(self):
        ...
        try:
            ...
            if self.launch_system:
                status = self.launch_system.get_launch_status()
                print(f"   Overall success score: {status.get('success_score', 0):.1f}/100")
                print(f"   Launch status: {status.get('launch_status', 'Unknown')}")
                print(f"   Platforms active: {len([p for p in status.get('platforms', {}).values() if p.get('status') == 'active'])}")
                print(f"   Competitive responses: {status.get('competitive_responses', 0)}")
