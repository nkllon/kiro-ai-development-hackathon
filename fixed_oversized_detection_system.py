...
class FixedOversizedDetectionSystem:
    ...
    def _analyze_single_file(self, file_path: Path) -> Optional[ModuleAnalysis]:
        ...
        try:
            ...
            if line_count > 10000:
                logger.warning(
                    f"File {file_path} too large ({line_count} lines), skipping detailed analysis"
                )
            ...
            size_compliance = (
                line_count <= self.size_limits["max_lines"]
                and function_count <= self.size_limits["max_functions"]
                and class_count <= self.size_limits["max_classes"]
                and complexity_score <= self.size_limits["max_complexity"]
            )
    ...
    def _basic_file_analysis(
        ...
        return ModuleAnalysis(
            ...
            refactoring_priority=(
                999 if line_count > self.size_limits["max_lines"] else 5
            ),
    ...
    def _generate_suggested_actions(
        ...
        if function_count > self.size_limits["max_functions"]:
            actions.append(
                f"Split into multiple modules (currently {function_count} functions)"
            )

        if class_count > self.size_limits["max_classes"]:
            actions.append(
                f"Extract classes to separate modules (currently {class_count} classes)"
            )

        if complexity_score > self.size_limits["max_complexity"]:
            actions.append(
                f"Refactor complex logic (complexity: {complexity_score:.1f})"
            )
    ...
    def _calculate_refactoring_priority(
        self,
        line_count: int,
        function_count: int,
        class_count: int,
    ...
    def _identify_extraction_points(
    ...
    def _generate_compliance_report(self) -> Dict[str, Any]:
        ...
        compliant_modules = sum(
            1 for analysis in self.analysis_results.values() if analysis.size_compliance
        )
        ...
        most_problematic = sorted(
            oversized_modules, key=lambda x: x.refactoring_priority
        )[:5]
...
def main():
    ...
    if compliance["most_problematic_modules"]:
        ...
        for module in compliance["most_problematic_modules"][:3]:
            print(
                f"   {Path(module['file_path']).name}: {module['line_count']} lines (Priority: {module['refactoring_priority']})"
            )
    ...
    if analysis["refactoring_plans"]:
        ...
        for plan in analysis["refactoring_plans"][:3]:
            print(
                f"   {Path(plan['module_path']).name}: {plan['refactoring_strategy']} ({plan['estimated_effort']})"
            )
