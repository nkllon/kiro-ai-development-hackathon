...
class RefactoringValidator:
    ...
    def validate_module(self, module_path: str) -> ValidationResult:
        ...
        try:
            ...
            # 3. RM-DDD compliance
            result.rm_ddd_compliant = self._validate_rm_ddd_compliance(
                module_path, content, result
            )

            # 4. Functionality preservation
            result.functionality_preserved = self._validate_functionality(
                module_path, result
            )

            # 5. Performance impact
            result.performance_impact = self._assess_performance_impact(
                module_path, result
            )
    ...
    def _validate_rm_ddd_compliance(
        self, module_path: str, content: str, result: ValidationResult
    ) -> bool:
        ...
        try:
            ...
            if line_count > 300:
                result.warnings.append(
                    f"File size {line_count} lines exceeds 300-line limit"
                )
                return False
            ...
            if not has_reflective_module:
                result.warnings.append(
                    "Module does not implement ReflectiveModule interface"
                )
                return False
    ...
    def _validate_functionality(
        self, module_path: str, result: ValidationResult
    ) -> bool:
    ...
    def generate_validation_report(self, output_file: str = "validation_report.json"):
        ...
        report_data = {"summary": self._generate_summary(), "detailed_results": []}
    ...
    def _generate_summary(self) -> Dict[str, Any]:
        ...
        functionality_preserved = sum(
            1 for r in self.validation_results if r.functionality_preserved
        )
...
def main():
    ...
    parser.add_argument(
        "--output", default="validation_report.json", help="Output report file"
    )
