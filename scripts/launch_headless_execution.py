...
class HeadlessSpecExecutor:
    ...
    def __init__(self):
        ...
        if BEAST_MODE_AVAILABLE:
            try:
                ...
                self.logger.warning(
                    f"⚠️  Beast Mode component initialization failed: {e}"
                )
    ...
    def load_spec_dag(self) -> Dict[str, Any]:
        ...
        if not dag_file.exists():
            raise FileNotFoundError(
                "Comprehensive spec DAG not found. Run spec DAG analysis first."
            )
    ...
    def create_execution_plan(
        self, target_specs: Optional[List[str]] = None
    ) -> ExecutionPlan:
        ...
        estimated_duration = (
            total_hours / parallel_tracks if parallel_tracks > 0 else total_hours
        )
        ...
        self.logger.info(
            f"   ⏱️  Estimated duration: {plan.estimated_duration_hours:.1f} hours"
        )
    ...
    def _resolve_dependencies(
        self, spec_name: str, spec_dag: Dict[str, List[str]]
    ) -> List[str]:
    ...
    def get_execution_status(self) -> Dict[str, Any]:
        ...
        completed_tasks = len(
            [
                t
                for t in self.current_execution.tasks
                if t.status == TaskStatus.COMPLETED
            ]
        )
        failed_tasks = len(
            [t for t in self.current_execution.tasks if t.status == TaskStatus.FAILED]
        )
        running_tasks = len(
            [t for t in self.current_execution.tasks if t.status == TaskStatus.RUNNING]
        )
...
async def main():
    ...
    if success:
        ...
        print(
            f"   ✅ Completed: {status['completed_tasks']}/{status['total_tasks']} tasks"
        )
