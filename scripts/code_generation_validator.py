...
class CodeGenerationValidator:
    ...
    def validate_file(self, file_path: Path) -> ValidationResult:
        ...
        try:
            ...
            try:
                ...
                return ValidationResult(file_path=str(file_path), is_valid=False, 
    ...
    def _validate_no_module_level_self_functions(self, content: str, lines: List[str], tree: ast.AST) -> Dict[str, List[str]]:
            ...
            if isinstance(node, ast.FunctionDef):
                ...
                if not parent_is_class and node.args.args:
                    ...
                    if first_arg.arg == 'self':
                        errors.append(f"Module-level function '{node.name}' has 'self' parameter at line {node.lineno}")
                        suggestions.append(f"Wrap function '{node.name}' in a class or remove 'self' parameter")
        
        return {'errors': errors, 'warnings': warnings, 'suggestions': suggestions}
    
    def _validate_class_structure(self, content: str, lines: List[str], tree: ast.AST) -> Dict[str, List[str]]:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.ClassDef):
                ...
                if not node.body or not isinstance(node.body[0], ast.Expr) or not isinstance(node.body[0].value, ast.Constant):
                    warnings.append(f"Class '{node.name}' missing docstring at line {node.lineno}")
                    suggestions.append(f"Add docstring to class '{node.name}'")
                
                # Check for __init__ method
                has_init = any(isinstance(child, ast.FunctionDef) and child.name == '__init__' 
    ...
    def _validate_import_placement(self, content: str, lines: List[str], tree: ast.AST) -> Dict[str, List[str]]:
                ...
                if not non_import_content:
                    ...
                    for j in range(i, len(lines)):
                        if lines[j].strip().startswith(('import ', 'from ')):
                            warnings.append(f"Import statement at line {j + 1} should be at top of file")
                            suggestions.append("Move all imports to the top of the file")
                            break
        
        return {'errors': errors, 'warnings': warnings, 'suggestions': suggestions}
    
    def _validate_method_signatures(self, content: str, lines: List[str], tree: ast.AST) -> Dict[str, List[str]]:
                ...
                for child in node.body:
                    if isinstance(child, ast.FunctionDef):
                        ...
                        if not child.args.args or child.args.args[0].arg != 'self':
                            errors.append(f"Method '{child.name}' in class '{node.name}' missing 'self' parameter at line {child.lineno}")
                            suggestions.append(f"Add 'self' parameter to method '{child.name}'")
        
        return {'errors': errors, 'warnings': warnings, 'suggestions': suggestions}
    
    def _validate_indentation_consistency(self, content: str, lines: List[str], tree: ast.AST) -> Dict[str, List[str]]:
    ...
    def validate_directory(self, directory: Path, pattern: str = "*.py") -> CodeGenerationReport:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Validate generated code for proper structure")
