...
class DynamicSyntaxFixer:
    ...
    def _add_missing_imports(self, content: str) -> str:
        ...
        needs_reflective_module = 'ReflectiveModule' in content and 'from src.rm_ddd.core.base_reflective_module import ReflectiveModule' not in content
        ...
        if needs_reflective_module:
            imports_to_add.append('from src.rm_ddd.core.base_reflective_module import ReflectiveModule')
        if needs_enum:
        ...
        if imports_to_add:
            ...
            lines = ['#!/usr/bin/env python3', '"""Auto-generated module"""', ''] + imports_to_add + [''] + lines[import_end:]
    ...
    def _fix_indentation_errors(self, content: str) -> str:
        ...
        for line in lines:
            ...
            if stripped.startswith('return') or stripped.startswith('if ') or stripped.startswith('for ') or stripped.startswith('while '):
                if ':' in stripped:
                    fixed_lines.append('    ' * current_indent + stripped)
                    current_indent += 1
                else:
        ...
        for i, line in enumerate(lines):
            ...
            if stripped.startswith('def ') and ':' in stripped:
                ...
                if i + 1 < len(lines):
                    ...
                    if not next_line or next_line.startswith('def ') or next_line.startswith('class '):
    ...
    def fix_all_files_parallel(self, max_workers: int = 4) -> Dict[str, int]:
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in as_completed(future_to_file):
                ...
                if completed % 50 == 0:
                    print(f"📊 Progress: {completed}/{stats['total']} files processed...")
