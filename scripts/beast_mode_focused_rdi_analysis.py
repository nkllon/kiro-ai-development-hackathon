...
class BeastModeFocusedRDIAnalysis:
        ...
        for file_path in self.key_files:
            ...
            if full_path.exists():
                try:
                    ...
                    for node in ast.walk(tree):
                        if isinstance(node, ast.ClassDef):
                            ...
                            function_info = {
                                ...
                                "is_method": any(
                                    arg == "self"
                                    for arg in [arg.arg for arg in node.args.args]
                                ),
                            }
                    ...
                    print(
                        f"      ✅ Analyzed: {os.path.basename(file_path)} ({len(file_info['classes'])} classes, {len(file_info['functions'])} functions)"
                    )
    ...
    def assess_compliance(self, file_analysis, requirements):
        ...
        compliance["functional_score"] = (
            (functional_compliant / len(functional_reqs) * 100)
            if functional_reqs
            else 100
        )
        ...
        compliance["interface_score"] = (
            (interface_compliant / len(interface_reqs) * 100) if interface_reqs else 100
        )
        ...
        compliance["compliance_score"] = (
            (compliance_compliant / len(compliance_reqs) * 100)
            if compliance_reqs
            else 100
        )
        ...
        compliance["architecture_score"] = (
            (architecture_compliant / len(architecture_reqs) * 100)
            if architecture_reqs
            else 100
        )
        ...
        total_reqs = (
            ...
            + len(interface_reqs)
            + len(compliance_reqs)
            + len(architecture_reqs)
        )
        total_compliant = (
            functional_compliant
            + interface_compliant
            + compliance_compliant
            + architecture_compliant
        )
        compliance["overall_score"] = (
            (total_compliant / total_reqs * 100) if total_reqs > 0 else 100
        )
    ...
    def check_functional_compliance(self, file_analysis, requirement):
        ...
        if req_id == "IFR-001":  # Interface Registry should manage interface metadata
            for file_path, file_info in file_analysis.items():
                for class_info in file_info["classes"]:
                    if "InterfaceRegistry" in class_info["name"]:
                        ...
                        if any(
                            method in " ".join(class_info["methods"])
            ...
            req_id == "RFM-001"
        ):  # Reflective Module should register with interface registry
            for file_path, file_info in file_analysis.items():
                for class_info in file_info["classes"]:
            ...
            for file_path, file_info in file_analysis.items():
                for function_info in file_info["functions"]:
                    if function_info["name"].startswith("get_") or function_info[
                        "name"
                    ].startswith("retrieve_"):
                        return True
    ...
    def check_interface_compliance(self, file_analysis, requirement):
        ...
        if (
            req_id == "IFR-002"
        ):  # Interface Registry should provide registration methods
            for file_path, file_info in file_analysis.items():
                for class_info in file_info["classes"]:
                    if "InterfaceRegistry" in class_info["name"]:
                        # Check if class has registration methods
    ...
    def check_compliance_requirement(self, file_analysis, requirement):
        ...
        if (
            req_id == "COMP-001"
        ):  # Compliance system should validate interface standards
            for file_path, file_info in file_analysis.items():
                for class_info in file_info["classes"]:
    ...
    def generate_solution(self, gaps):
        ...
        for gap in gaps["critical_gaps"] + gaps["high_priority_gaps"]:
            solution["implementation_plan"].append(
                {
                    "phase": (
                        "immediate" if gap in gaps["critical_gaps"] else "priority"
                    ),
                    "area": gap["area"],
                    "actions": [
        ...
        print(
            f"         • Implementation Plan: {len(solution['implementation_plan'])} phases"
        )
        print(
            f"         • Compliance Improvements: {len(solution['compliance_improvements'])} items"
        )
        print(
            f"         • Architecture Enhancements: {len(solution['architecture_enhancements'])} items"
        )
    ...
    def generate_report(self, file_analysis, requirements, compliance, gaps, solution):
        ...
        print(f"   🎯 Critical Gaps: {len(gaps['critical_gaps'])}")
        print(
            f"   🔧 Implementation Plan Phases: {len(solution['implementation_plan'])}"
        )
