...
class RepositoryRefactoringOrchestrator:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if os.path.exists('repository_analysis_report.json'):
                    ...
                    logger.info(f"    📊 Compliance rate: {summary.get('compliance_rate', 0):.1f}%")
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                if os.path.exists('refactoring_execution_report.json'):
                    ...
                    successful = sum(1 for r in execution_data if r.get('success', False))
    ...
    def _run_validation_phase(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Run validation
            result = subprocess.run([
                'python', str(self.scripts_dir / 'refactoring_validator.py'),
            ...
            if result.returncode == 0:
                ...
                if os.path.exists('validation_report.json'):
                    ...
                    logger.info(f"    📊 Modules validated: {summary.get('total_modules', 0)}")
                    logger.info(f"    📊 Success rate: {summary.get('success_rate', 0):.1f}%")
                    logger.info(f"    📊 RM-DDD compliant: {summary.get('rm_ddd_compliant', 0)}")
                    ...
                    return {'success': False, 'error': 'Validation report not generated'}
            else:
    ...
    def _run_reporting_phase(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Save final report
            final_report_file = self.reports_dir / f"refactoring_session_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Orchestrate repository-wide refactoring")
    parser.add_argument("--dry-run", action="store_true", 
                       help="Perform dry run without making changes")
    parser.add_argument("--max-files", type=int, 
                       help="Maximum number of files to refactor")
    parser.add_argument("--src-dir", default="src", 
                       help="Source directory to refactor")
    ...
    phases = ['phase1_analysis', 'phase2_planning', 'phase3_execution', 'phase4_validation', 'phase5_reporting']
    ...
    for phase, name in zip(phases, phase_names):
        ...
        print(f"{status} {name}: {'Success' if result.get('success', False) else 'Failed'}")
