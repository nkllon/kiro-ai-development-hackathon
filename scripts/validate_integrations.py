...
class IntegrationValidator:
    ...
    def validate_github_mcp(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            if "login" in response:
                return (
                    True,
                    f"GitHub authentication successful for user: {response['login']}",
                )
            else:
    ...
    def validate_simone_mcp(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            result = subprocess.run(
                [
                    "node",
                    "/Users/lou/kiro-2/kiro-ai-development-hackathon/kiro_simone_adapter/mcp-server/dist/index.js",
                    "--help",
                ],
                capture_output=True,
                text=True,
    ...
    def validate_docker_github_mcp(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            if "result" in response and "serverInfo" in response["result"]:
                return (
                    True,
                    f"Docker GitHub MCP operational: {response['result']['serverInfo']['name']}",
                )
            else:
    ...
    def validate_interface_registry(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            result = subprocess.run(
                ["make", "check-registry"], capture_output=True, text=True, timeout=10
            )
        ...
        for name, message in self.failures:
            if "GitHub token authentication failed" in message:
                recommendations.append(
                    "Generate new GitHub Personal Access Token with required scopes"
                )
                ...
                recommendations.append(
                    "Rebuild Simone MCP server or check Node.js dependencies"
                )
            elif "Docker GitHub MCP" in message:
                recommendations.append(
                    "Pull latest Docker image or check Docker daemon status"
                )
            elif "Interface registry" in message:
                recommendations.append(
                    "Reinitialize interface registry or check file permissions"
                )
