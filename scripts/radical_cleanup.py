...
class RadicalCleanup:
    ...
    def should_keep_file(self, file_path: Path) -> bool:
        ...
        if (file_path.suffix == '.py' and 
            not any(pattern in file_path.name for pattern in ['_part_', '_class_', 'part_']) and
            not file_path.name.endswith('_part.py')):
            return True
        ...
        if file_path.name in ['__init__.py', 'conftest.py', 'pytest.ini', 'pyproject.toml', 'requirements.txt']:
            return True
