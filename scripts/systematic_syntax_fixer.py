...
class SystematicSyntaxFixer:
        ...
        try:
            ...
            if "expected an indented block" in error_msg:
                ...
                if error_line < len(lines):
                    ...
                    # Fix missing colons
                    if line_content.strip().endswith(')') and not line_content.strip().endswith(':'):
                        lines[error_line] = line_content.rstrip() + ':\n'
                        fixed = True
                    
                    # Fix missing parentheses in if statements
                    elif line_content.strip().startswith('if ') and ':' not in line_content:
                        lines[error_line] = line_content.rstrip() + ':\n'
                        fixed = True
            ...
            for error in errors:
                ...
                if os.path.exists(file_path):
                    if self.fix_common_syntax_errors(file_path, error):
                        ...
                        print(f"      ⚠️  Could not auto-fix: {os.path.basename(file_path)}")
    ...
    def validate_fixes(self):
        ...
        print(f"   📈 Syntax compliance: {((total_files - len(remaining_errors)) / total_files * 100):.1f}%")
    ...
    def run_systematic_fixes(self):
        ...
        print(f"Remaining Errors: {len(remaining_errors)}")
        print(f"Success Rate: {((len(syntax_errors) - len(remaining_errors)) / len(syntax_errors) * 100):.1f}%")
        ...
        return len(remaining_errors) < len(syntax_errors) / 2  # Success if we fixed at least 50%
