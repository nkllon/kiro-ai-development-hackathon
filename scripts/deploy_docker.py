    ...
    try:
        ...
        if validate and start_services:
            ...
            try:
                ...
                logger.info(f"Validation completed: {validation_report.passed_checks}/{validation_report.total_checks} checks passed")
        ...
        if start_services:
            ...
            logger.info(f"  View logs: docker-compose -f docker-compose-{deployment_id}.yml logs -f")
            logger.info(f"  Stop services: docker-compose -f docker-compose-{deployment_id}.yml down")
            logger.info(f"  Restart services: docker-compose -f docker-compose-{deployment_id}.yml restart")
...
def main():
    ...
    parser.add_argument("--agent-id", default="docker_agent",
