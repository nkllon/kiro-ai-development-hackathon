...
class BeastModeManualFixer:
        ...
        for pattern in problematic_patterns:
            for file_path in self.project_root.rglob(f"src/**/{pattern}"):
                try:
                    ...
                    base_name = file_name.replace('_core_core_core', '').replace('_services_services_services', '').replace('_handlers_handlers_handlers', '').replace('_utils_utils_utils', '')
    ...
    def run_beast_mode_fixes(self):
        ...
        if error_patterns['expected_indented_block']:
            fixes = self.aggressive_fix_indented_block_errors(error_patterns['expected_indented_block'])
            total_fixes += fixes
        
        if error_patterns['unindent_mismatch']:
            fixes = self.aggressive_fix_unindent_errors(error_patterns['unindent_mismatch'])
            total_fixes += fixes
