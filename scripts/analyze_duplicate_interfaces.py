...
def analyze_duplicate_patterns(registry_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for interface_name, interface_data in registry_data.get("interfaces", {}).items():
        ...
        interfaces_by_type[interface_data.get("interface_type", "unknown")].append(
            interface_data
        )
        interfaces_by_file[interface_data.get("file_path", "unknown")].append(
            interface_data
        )
    ...
    for interface_name, dup_list in duplicate_groups.items():
        if len(dup_list) > 1:
            ...
            backup_files = [
                f for f in files if "backup" in f.lower() or ".backup_" in f
            ]
            if backup_files:
            ...
            versioned_files = [
                f
                for f in files
                if any(v in f for v in ["_v2", "_v3", "_core", "_core_core"])
            ]
            if versioned_files:
    ...
    patterns["interface_types_with_duplicates"] = list(
        patterns["interface_types_with_duplicates"]
    )
...
def analyze_interface_quality(enhanced_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if quality_metrics["compliance_scores"]:
        quality_metrics["average_compliance"] = sum(
            quality_metrics["compliance_scores"]
        ) / len(quality_metrics["compliance_scores"])
...
def generate_consolidation_strategy(
    patterns: Dict[str, Any], quality_metrics: Dict[str, Any]
) -> Dict[str, Any]:
    ...
    backup_consolidations = [
        p for p in patterns["priority_consolidations"] if p["reason"] == "backup_files"
    ]
    ...
    versioned_consolidations = [
        p
        for p in patterns["priority_consolidations"]
        if p["reason"] == "versioned_files"
    ]
    ...
    same_dir_consolidations = [
        p
        for p in patterns["priority_consolidations"]
        if p["reason"] == "same_directory"
    ]
    ...
    low_compliance_interfaces = [
        i
        for i in quality_metrics["improvement_candidates"]
        if i["compliance_score"] < 50
    ]
    ...
    strategy["estimated_impact"] = {
        ...
        "estimated_reduction": sum(
            len(phase["actions"][0]["targets"])
            for phase in strategy["consolidation_phases"]
        ),
        "estimated_compliance_improvement": len(low_compliance_interfaces)
        * 20,  # Assume 20% improvement per interface
        "final_duplicate_count": max(
            0,
            total_duplicates
            - sum(
                len(phase["actions"][0]["targets"])
    ...
    strategy["risk_assessment"] = {
        "low_risk_actions": len(
            [
                p
                for p in strategy["consolidation_phases"]
                if any(a["risk"] == "low" for a in p["actions"])
            ]
        ),
        "medium_risk_actions": len(
            [
                p
                for p in strategy["consolidation_phases"]
                if any(a["risk"] == "medium" for a in p["actions"])
            ]
        ),
        "high_risk_actions": len(
            [
                p
                for p in strategy["consolidation_phases"]
                if any(a["risk"] == "high" for a in p["actions"])
            ]
        ),
        "overall_risk": "medium",
    }
...
def main():
    ...
    print(
        f"   Interface Types with Duplicates: {len(patterns['interface_types_with_duplicates'])}"
    )
    ...
    print(
        f"   Estimated Duplicate Reduction: {strategy['estimated_impact']['estimated_reduction']}"
    )
    print(
        f"   Target Compliance: {strategy['compliance_targets']['target_average']:.1f}%"
    )
    ...
    for i, phase in enumerate(strategy["consolidation_phases"], 1):
        ...
        for action in phase["actions"]:
            print(
                f"     - {action['action']}: {len(action['targets'])} targets (risk: {action['risk']})"
            )
