...
class DeepErrorAnalyzer:
    ...
    def perform_deep_analysis(self) -> List[DeepErrorAnalysis]:
        ...
        try:
            ...
            for i, line in enumerate(error_lines):
                ...
                elif current_error and ('ImportError' in line or 'ModuleNotFoundError' in line or 'NameError' in line):
                    error_analysis = self._parse_deep_import_error(line, current_test_file)
                    if error_analysis:
                        self.errors.append(error_analysis)
                
                elif current_error and ('SyntaxError' in line or 'IndentationError' in line):
                    error_analysis = self._parse_deep_syntax_error(line, current_test_file)
                    if error_analysis:
                        self.errors.append(error_analysis)
                
                elif current_error and ('AttributeError' in line):
                    error_analysis = self._parse_deep_attribute_error(line, current_test_file)
                    if error_analysis:
                        self.errors.append(error_analysis)
    ...
    def _parse_deep_import_error(self, error_line: str, test_file: str) -> DeepErrorAnalysis:
    ...
    def _parse_deep_syntax_error(self, error_line: str, test_file: str) -> DeepErrorAnalysis:
    ...
    def _parse_deep_attribute_error(self, error_line: str, test_file: str) -> DeepErrorAnalysis:
    ...
    def _fix_undefined_names_deep(self, errors: List[DeepErrorAnalysis]) -> int:
        ...
        for error in errors:
            if error.missing_class in common_undefined:
                ...
                fixed += self._add_import_to_files_deep(error.missing_class, common_undefined[error.missing_class])
...
'''
                    ...
                    if error.missing_class in content:
                        ...
                        for i, line in enumerate(lines):
                            if 'class ' in line and error.missing_class in line:
                                ...
                                fixed += 1
                                print(f"✅ Added {error.missing_class} attribute to {py_file}")
                                break
    ...
    def generate_deep_analysis_report(self, stats: Dict[str, int], validation_stats: Dict[str, int]) -> str:
