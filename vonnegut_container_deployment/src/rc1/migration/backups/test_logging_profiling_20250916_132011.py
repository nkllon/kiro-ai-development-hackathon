...
def test_logging_infrastructure():
    ...
    config = LoggingConfig(
        log_level="DEBUG", log_format="JSON", enable_console=True, enable_file=True
    )
...
def test_integration():
    ...
    with profiler.profile_operation("integrated_test", {"integration": "test"}):
        logging.log_event(
            LogLevel.INFO, "Starting integrated test", {"test": "integration"}
        )
        ...
        logging.log_event(
            LogLevel.INFO, "Integrated test completed", {"test": "integration"}
        )
