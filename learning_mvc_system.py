...
class TelemetryCollector:
    ...
    def log_event(self, event_type: str, page: Page, data: Dict[str, Any], success: bool = True, error: str = None):
...
class CuriousPageAnalyzer:
    ...
    def analyze_page(self) -> PageAnalysis:
        ...
        try:
            ...
            button_count = len(self.page.query_selector_all("button, input[type='button'], input[type='submit']"))
            ...
            # Log telemetry
            self.telemetry.log_event("page_analysis", self.page, {
    ...
    def _heuristically_detect_page_type(self, url: str, title: str, html_content: str) -> PageType:
        ...
        if "project" in title_lower and ("details" in title_lower or "photo" in title_lower):
    ...
    def _find_status_indicators(self) -> List[str]:
        ...
        status_selectors = [
            ".status", ".state", ".progress", ".loading", ".error", ".success",
            ".alert", ".notification", ".message", ".warning", ".info",
            "[role='status']", "[role='alert']", "[aria-live]",
            ".step", ".wizard-step", ".form-step", ".nav-step",
            ".completed", ".current", ".active", ".pending", ".disabled"
        ]
            ...
            try:
                ...
                for element in elements:
                    ...
                    if text and len(text) < 100:
                        indicators.append(f"{selector}: {text}")
            except:
                continue
    ...
    def _find_interactive_elements(self) -> List[Dict[str, Any]]:
        ...
        for selector in form_selectors:
            try:
                ...
                for i, element in enumerate(elements):
                    try:
                        element_type = element.get_attribute("type") or element.evaluate("el => el.tagName").lower()
    ...
    def _extract_meta_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if viewport:
                meta_info["viewport"] = f"{viewport['width']}x{viewport['height']}"
            
            # Get page load time
            load_time = self.page.evaluate("() => performance.timing.loadEventEnd - performance.timing.navigationStart")
            if load_time:
    ...
    def _learn_from_analysis(self, analysis: PageAnalysis):
        ...
        for nav_element in analysis.navigation_elements:
            if nav_element["text"]:
                ...
                self.learning_state.button_patterns[button_text].append(nav_element["classes"])
    ...
    def _display_curious_findings(self, analysis: PageAnalysis):
        ...
        print(f"\n🤔 Curious Findings:")
        print(f"   📄 Page Type: {analysis.page_type.value}")
        print(f"   📊 Elements: {analysis.form_count} forms, {analysis.button_count} buttons, {analysis.input_count} inputs")
        print(f"   🔗 Links: {analysis.link_count}, Images: {analysis.image_count}")
        ...
        if analysis.navigation_elements:
            ...
            for nav in analysis.navigation_elements[:5]:  # Show first 5
                status = "✅" if nav["is_visible"] and nav["is_enabled"] else "❌"
                print(f"      {status} {nav['text'][:50]}... (type: {nav['element_type']})")
        ...
        print(f"\n🧠 Learning Insights:")
        print(f"   📚 Total pages analyzed: {self.learning_state.total_pages_analyzed}")
        print(f"   🎯 Successful navigations: {self.learning_state.successful_navigations}")
        print(f"   ❌ Failed navigations: {self.learning_state.failed_navigations}")
        
        if self.learning_state.button_patterns:
            print(f"   🔘 Learned button patterns: {len(self.learning_state.button_patterns)}")
        if self.learning_state.form_patterns:
            print(f"   📝 Learned form patterns: {len(self.learning_state.form_patterns)}")
...
class LearningMVCSystem:
    ...
    def _on_console_message(self, msg):
        ...
        if msg.type in ["error", "warning"]:
            self.telemetry.log_event("console_message", self.page, {
                "type": msg.type,
                "text": msg.text
            })
    ...
    def _on_request(self, request):
        ...
        if "devpost.com" in request.url or "github.com" in request.url or "google.com" in request.url:
            self.telemetry.log_event("request", self.page, {
    ...
    def _on_response(self, response):
        """Handle responses."""
        if "devpost.com" in response.url or "github.com" in response.url or "google.com" in response.url:
            self.telemetry.log_event("response", self.page, {
    ...
    def _on_dialog(self, dialog):
        """Handle dialogs."""
        self.telemetry.log_event("dialog", self.page, {
            "type": dialog.type,
    ...
    def find_navigation_options(self) -> List[Dict[str, Any]]:
        ...
        for nav_element in self.current_analysis.navigation_elements:
            if nav_element["is_visible"] and nav_element["is_enabled"]:
                ...
                nav_keywords = ["next", "continue", "back", "previous", "submit", "save", "finish", "complete"]
    ...
    def navigate_to_element(self, element_info: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            self.telemetry.log_event("navigation", self.page, {
    ...
    def _take_curious_screenshot(self, prefix: str = "curious"):
        ...
        try:
            ...
            page_type = self.current_analysis.page_type.value if self.current_analysis else "unknown"
            
            filename = f"curious_{hackathon_id}_{submission_id}_{page_type}_{page_title}_{prefix}_{timestamp}.png"
            self.page.screenshot(path=filename)
            print(f"📸 Curious screenshot: {filename}")
            return filename
        except Exception as e:
        ...
        while True:
            try:
                ...
                if command == "quit":
                    ...
                    print(f"   Pages analyzed: {self.analyzer.learning_state.total_pages_analyzed}")
                    print(f"   Successful navigations: {self.analyzer.learning_state.successful_navigations}")
                    print(f"   Failed navigations: {self.analyzer.learning_state.failed_navigations}")
                    print(f"   Button patterns learned: {len(self.analyzer.learning_state.button_patterns)}")
                    print(f"   Form patterns learned: {len(self.analyzer.learning_state.form_patterns)}")
                else:
