...
def explore_all_steps():
    ...
    try:
        ...
        for step_info in available_steps:
            ...
            try:
                ...
                for form in form_analysis:
                    print(
                        f"   Form {form['index']}: {form['class']} ({len(form['inputs'])} inputs)"
                    )
                    for inp in form["inputs"][:5]:  # Show first 5 inputs
                        print(
                            f"      - {inp['type']} {inp['name']} {inp['placeholder']}"
                        )

                # Analyze step navigation
                current_step_links = target_page.query_selector_all(
            ...
            step_sequence.append(
                {
                    ...
                    "total_inputs": sum(
                        len(form["inputs"]) for form in step_data["forms"]
                    ),
                }
            )
        ...
        for i, step in enumerate(step_sequence, 1):
            print(
                f"   {i}. {step['name']} - {step['forms_count']} forms, {step['total_inputs']} inputs"
            )
        ...
        print(
            f"   Total forms: {sum(len(step['forms']) for step in devpost_model['steps'].values())}"
        )
        print(
            f"   Total inputs: {sum(len(form['inputs']) for step in devpost_model['steps'].values() for form in step['forms'])}"
        )
...
if __name__ == "__main__":
    ...
    if model:
        ...
        print(
            f"🔍 Found {len(model['heuristics']['step_detection']['step_names'])} step names"
        )
        print(
