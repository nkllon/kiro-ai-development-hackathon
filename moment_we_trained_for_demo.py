...
from dynamic_session_classifier import SessionClassifier, MultiDimensionalSessionAnalyzer
...
class MomentWeTrainedForDemo:
    ...
    def execute_the_moment(self) -> Dict[str, Any]:
        ...
        mission_report = {
            ...
            "systems_deployed": len([s for s in [
    ...
    def _initialize_all_systems(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.session_classifier = SessionClassifier(None)  # No planning graph for demo
        ...
        self.systems_ready = systems_initialized >= 3  # At least 3 systems must be ready
    ...
    def _demonstrate_full_capabilities(self) -> Dict[str, Any]:
        ...
        if self.enhanced_classifier:
            ...
            try:
                analysis = self.enhanced_classifier.analyze_page_comprehensive(test_page_data)
                ...
                print(f"   ✅ SPA Detection: {summary['application_type']} (confidence: {summary['spa_confidence']:.2f})")
                print(f"   ✅ Meta Integration: {summary['meta_integration']} (confidence: {summary['meta_confidence']:.2f})")
            except Exception as e:
                demonstration_results['enhanced_classification'] = {"error": str(e), "success": False}
        ...
        if self.fixed_detector:
            ...
            try:
                ...
                demonstration_results['oversized_detection'] = {"error": str(e), "success": False}
        ...
        if self.session_classifier:
            ...
            try:
                hypothesis, confidence, analysis = self.session_classifier.sniff_the_air(test_context)
                ...
                demonstration_results['session_classification'] = {"error": str(e), "success": False}
        ...
        if self.cluster_analyzer:
            ...
            try:
                ...
                demonstration_results['cluster_analysis'] = {"error": str(e), "success": False}
    ...
    def _execute_cross_domain_analysis(self) -> Dict[str, Any]:
        ...
        for test_case in cross_domain_test_cases:
            ...
            try:
                analysis = self.enhanced_classifier.analyze_page_comprehensive(test_case['page_data'])
                ...
                cross_domain_results[test_case['name']] = {
                    ...
                    "investigation_recommended": metaproperty_analysis['investigation_recommended'],
                    "success": True
                }
                
                print(f"      ✅ Cross-Domain Patterns: {len(cross_domain_patterns)}")
                print(f"      ✅ Metaproperty: {metaproperty_analysis['is_metaproperty']} (confidence: {metaproperty_analysis['metaproperty_confidence']:.2f})")
                print(f"      ✅ Investigation Recommended: {metaproperty_analysis['investigation_recommended']}")
                
            except Exception as e:
                cross_domain_results[test_case['name']] = {"error": str(e), "success": False}
    ...
    def _execute_metaproperty_detection(self) -> Dict[str, Any]:
        ...
        for test_case in metaproperty_test_cases:
            ...
            try:
                ...
                print(f"      {status} Metaproperty: {is_metaproperty} (confidence: {metaproperty_analysis['metaproperty_confidence']:.2f})")
                print(f"      Meta Integration: {meta_analysis['has_meta_integration']} (confidence: {meta_analysis['meta_confidence']:.2f})")
                
            except Exception as e:
                metaproperty_results[test_case['name']] = {"error": str(e), "success": False}
                print(f"      ❌ Analysis Failed: {e}")
        
        # Calculate accuracy
        successful_tests = [r for r in metaproperty_results.values() if r.get('success', False)]
        if successful_tests:
            correct_predictions = sum(1 for r in successful_tests if r.get('correct_prediction', False))
    ...
    def _execute_comprehensive_assessment(self) -> Dict[str, Any]:
        ...
        if self.enhanced_classifier:
            assessment_results['capabilities_assessed'].append("Enhanced Classification System")
            assessment_results['capabilities_assessed'].append("SPA Detection")
            assessment_results['capabilities_assessed'].append("Meta Ecosystem Detection")
            assessment_results['capabilities_assessed'].append("Cross-Domain Pattern Discovery")
            assessment_results['capabilities_assessed'].append("Metaproperty Detection")
            systems_assessed += 1
        
        if self.fixed_detector:
            assessment_results['capabilities_assessed'].append("Fixed Oversized Detection")
        ...
        if self.session_classifier:
            ...
            assessment_results['capabilities_assessed'].append("Multi-Dimensional Analysis")
            systems_assessed += 1
        ...
        if self.deployment_start_time:
            ...
            assessment_results['performance_metrics'] = {
                "deployment_time_seconds": deployment_time,
                "systems_per_second": systems_assessed / deployment_time if deployment_time > 0 else 0,
                "capabilities_per_system": len(assessment_results['capabilities_assessed']) / max(systems_assessed, 1)
            }
        ...
        if assessment_results['performance_metrics']:
            print(f"   📊 Deployment Time: {assessment_results['performance_metrics']['deployment_time_seconds']:.2f}s")
...
def main():
    ...
    print(f"   Systems Deployed: {mission_report['systems_deployed']}/5")
    print(f"   Deployment Time: {mission_report['deployment_time_seconds']:.2f} seconds")
