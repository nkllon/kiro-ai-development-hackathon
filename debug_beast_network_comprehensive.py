...
def check_redis_clients(r: redis.Redis) -> Dict[str, Any]:
    ...
    try:
        ...
        for i, client in enumerate(clients):
            ...
            print(f"  Client {i+1}: {info['name'] or 'unnamed'} | "
...
def send_network_ping(r: redis.Redis) -> bool:
    ...
    try:
        ...
        for msg in messages:
            try:
                decoded = json.loads(msg.decode('utf-8'))
                if decoded.get('type') == 'PONG' and 'NETWORK_DIAGNOSTICS' in decoded.get('message', ''):
                    pong_count += 1
...
def main():
    ...
    try:
        ...
        print(f"  Active Queues: {sum(1 for q, info in queue_status.items() if info.get('length', 0) > 0)}")
        print(f"  Agent Status Keys: {len(agent_status)}")
        print(f"  Pub/Sub Channels: {len(pubsub_info.get('channels', []))}")
        print(f"  Network Ping Response: {'✅ Success' if ping_success else '❌ No Response'}")
        ...
        if not ping_success:
            print(f"  • No ping responses - agents may not be listening to beast_mode_messages")
