...
track_event("Registry Integration", f"Registry ID: {module_info.get('registry_id', 'Not registered')}")
...
track_event("Health Check", f"Status: {health.status.value}, Score: {health.health_score}")

# Metrics collection
metrics = service_manager.get_metrics()
track_event("Metrics Collection", f"Registry Registered: {metrics.get('registry_registered', False)}")
...
track_event("Updated Metrics", f"Warning Count: {updated_metrics.get('warning_count', 0)}")
...
try:
    from beast_mode.interface_governance.interface_registry import BeastModeInterfaceRegistry
    registry = BeastModeInterfaceRegistry()
    status = registry.get_registry_status()
    track_event("Registry Status", f"Total Interfaces: {status.get('total_interfaces', 0)}")
    track_event("Registry Status", f"Active Interfaces: {status.get('active_interfaces', 0)}")
