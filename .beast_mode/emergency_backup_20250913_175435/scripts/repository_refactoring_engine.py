...
class RepositoryRefactoringEngine:
    ...
    def _classify_domain(
        self, file_path: str, classes: List[str], functions: List[str]
    ) -> str:
    ...
    def _generate_analysis_report(self) -> Dict[str, Any]:
        ...
        compliance_rate = (
            ((len(valid_files) - len(large_files)) / len(valid_files)) * 100
            if valid_files
            else 0
        )
        ...
        return {
            "summary": {
                ...
                "average_file_size": (
                    total_lines // len(valid_files) if valid_files else 0
                ),
            },
            "domain_breakdown": domain_stats,
            "largest_files": sorted(
                large_files, key=lambda x: x.line_count, reverse=True
            )[:20],
            "priority_files": sorted(
                [f for f in large_files if f.refactoring_priority >= 4],
    ...
    def _create_refactoring_plan(
        ...
        function_groups = self._group_functions_by_functionality(
    ...
    def _group_classes_by_functionality(
        self, classes: List[str]
    ) -> Dict[str, List[str]]:
        """Group classes by functionality patterns"""
        groups = {"models": [], "services": [], "handlers": [], "utils": [], "core": []}

        for class_name in classes:
            class_lower = class_name.lower()
            if any(
                pattern in class_lower
                for pattern in ["model", "data", "entity", "schema"]
            ):
                groups["models"].append(class_name)
            elif any(
                pattern in class_lower
                for pattern in ["service", "manager", "processor", "engine"]
            ):
                groups["services"].append(class_name)
            elif any(
                pattern in class_lower
                for pattern in ["handler", "controller", "adapter"]
            ):
    ...
    def _group_functions_by_functionality(
        self, functions: List[str]
    ) -> Dict[str, List[str]]:
        """Group functions by functionality patterns"""
        groups = {"validation": [], "processing": [], "utils": [], "core": []}
        ...
        for function_name in functions:
            ...
            if any(
                ...
                pattern in func_lower
                for pattern in ["process", "transform", "convert", "parse"]
            ):
                groups["processing"].append(function_name)
            elif any(
                pattern in func_lower
                for pattern in ["util", "helper", "tool", "format"]
            ):
    ...
    def export_analysis_report(
        self, output_file: str = "repository_analysis_report.json"
    ):
...
def main():
    ...
    for i, file in enumerate(priority_files[:5], 1):
        ...
        print(
            f"      Lines: {file.line_count}, Classes: {file.class_count}, Priority: {file.refactoring_priority}"
        )

    print(f"\n✅ Analysis complete! Generated {len(plans)} refactoring plans.")
    print(
        "📁 Check repository_analysis_report.json and refactoring_plans.json for details."
    )
