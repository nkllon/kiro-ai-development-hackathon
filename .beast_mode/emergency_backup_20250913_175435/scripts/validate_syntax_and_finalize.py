        ...
        if result['valid']:
            ...
            if fix_common_syntax_issues(file_path):
                ...
                if new_result['valid']:
                    ...
                    print(f"   ❌ Could not fix syntax in {file_path}: {new_result['error']}")
            else:
...
def generate_final_compliance_report() -> Dict[str, Any]:
    ...
    compliance_scores = [interface.get('compliance_score', 0) for interface in interfaces.values()]
    ...
    for interface_name, interface_data in interfaces.items():
        ...
        if interface_type not in type_stats:
            type_stats[interface_type] = {'count': 0, 'total_compliance': 0, 'interfaces': []}
    ...
    all_interfaces = [(name, data.get('compliance_score', 0)) for name, data in interfaces.items()]
...
def main():
    ...
    if 'error' not in final_report:
        ...
        for interface_type, stats in final_report['interface_types'].items():
            print(f"   {interface_type}: {stats['average_compliance']:.2f}% avg ({stats['count']} interfaces)")
