...
class GitHubPATFacilitator:
    ...
    def check_existing_token(self) -> Optional[str]:
        ...
        try:
            if self.cursor_config_path.exists():
                ...
                token = github_config.get("env", {}).get(
    ...
    def validate_token_authorizations(self, token: str) -> Dict[str, bool]:
                ...
                if result.success:
                    try:
                        ...
                        if (
                            ...
                            print(
                                f"❌ {scope}: {test_config['description']} - {response.get('message', 'Access denied')}"
                            )
                    except json.JSONDecodeError:
                        authorizations[scope] = False
                        print(
                            f"❌ {scope}: {test_config['description']} - Invalid response format"
                        )
                else:
                    authorizations[scope] = False
                    print(
                        f"❌ {scope}: {test_config['description']} - HTTP {result.return_code}"
                    )
        ...
        missing_scopes = [
            scope for scope in required_scopes if not authorizations.get(scope, False)
        ]
    ...
    def facilitate_token_retrieval(self) -> str:
        ...
        print("-" * 30)
        print(
            "To enable GitHub MCP integration, you need a GitHub Personal Access Token."
        )
        ...
        try:
            ...
            print(
                "   Please manually navigate to: https://github.com/settings/tokens/new"
            )
        ...
        if not self.validate_token_authorizations(token):
            print(
                "❌ Token authorization validation failed. Please fix token permissions and try again."
            )
            sys.exit(1)
        ...
        try:
            ...
            if docker_result.success:
                ...
                if "result" in response and "serverInfo" in response["result"]:
                    ...
                    print(
                        f"✅ GitHub MCP Server operational: {server_info['name']} v{server_info['version']}"
                    )
                    return True
