...
class PDCAConvergenceMonitor:
    ...
    def record_iteration(
        self, overall_compliance: float, size_compliance: float
    ) -> PDCAConvergenceMetrics:
        ...
        stopping_criteria_met = self._check_stopping_criteria(
            overall_compliance, improvement_rate, iteration
        )
    ...
    def _check_stopping_criteria(
        self, overall_compliance: float, improvement_rate: float, iteration: int
    ) -> bool:
        ...
        if (
            iteration >= 10 and improvement_rate < 0.01
        ):  # <1% improvement for 10+ iterations
            return True
    ...
    def get_convergence_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_improvement": sum(m.improvement_rate for m in self.history)
            / len(self.history),
        }
...
class RMComplianceAnalyzer:
    ...
    def analyze_module(self, file_path: Path) -> RMInterfaceGap:
        ...
        try:
            ...
            compliance_score = self._calculate_compliance_score(
                missing_inheritance, missing_methods
            )
    ...
    def _calculate_compliance_score(
        self, missing_inheritance: bool, missing_methods: List[str]
    ) -> float:
        """Calculate RM interface compliance score"""
        if missing_inheritance:
            return 0.0

        method_score = (len(self.REQUIRED_METHODS) - len(missing_methods)) / len(
...
class HealthMonitoringAnalyzer:
    ...
    def analyze_module(self, file_path: Path) -> HealthMonitoringGap:
        ...
        try:
            ...
            health_coverage_score = self._calculate_health_coverage_score(
                missing_health_indicators,
                missing_status_reporting,
    ...
    def _calculate_health_coverage_score(
        self, missing_indicators: bool, missing_status: bool, missing_degradation: bool
    ) -> float:
        """Calculate health monitoring coverage score"""
        total_checks = 3
        passed_checks = sum(
            [not missing_indicators, not missing_status, not missing_degradation]
        )
...
class RMComplianceAssessmentTool:
    ...
    def assess_all_modules(self) -> ComplianceReport:
        ...
        logger.info(
            f"Assessment complete. Overall compliance: {report.overall_compliance_score:.1f}%"
        )
        return report
    ...
    def _assess_single_module(self, file_path: Path) -> ModuleAssessment:
        ...
        refactoring_priority = (
            size_violation.refactoring_priority if size_violation else 999
        )
    ...
    def _generate_compliance_report(
        ...
        critical_violations = sum(
            ...
            if a.size_violation and a.size_violation.severity == "critical"
        )
        major_violations = sum(
            1
            for a in assessments
            if a.size_violation and a.size_violation.severity == "major"
        )
        minor_violations = sum(
            1
            for a in assessments
            if a.size_violation and a.size_violation.severity == "minor"
        )

        # Calculate overall compliance score
        overall_compliance = (
            sum(a.overall_compliance_score for a in assessments) / len(assessments)
            if assessments
            else 0.0
        )
    ...
    def _generate_recommendations(
        ...
        if oversized_modules:
            recommendations.append(
                f"Refactor {len(oversized_modules)} oversized modules to meet 200-line limit"
            )
            critical_modules = [
                a
                for a in oversized_modules
                if a.size_violation and a.size_violation.severity == "critical"
            ]
            if critical_modules:
                recommendations.append(
                    f"Priority: Refactor {len(critical_modules)} critical size violations first"
                )
        ...
        if non_rm_modules:
            recommendations.append(
                f"Implement ReflectiveModule interface for {len(non_rm_modules)} modules"
            )

        # Health monitoring recommendations
        non_health_modules = [
            a for a in assessments if not a.health_monitoring_compliant
        ]
        if non_health_modules:
            recommendations.append(
                f"Implement health monitoring for {len(non_health_modules)} modules"
            )
        ...
        if non_registry_modules:
            recommendations.append(
                f"Integrate {len(non_registry_modules)} modules with RM registry"
            )
    ...
    def save_report(
        ...
        output_file: str = "rm_ddd_compliance_report.json",
    ):
    ...
    def print_summary(self, report: ComplianceReport):
        ...
        print(
            f"  Size Compliant: {report.size_compliant_modules}/{report.total_modules} ({report.size_compliant_modules/report.total_modules*100:.1f}%)"
        )
        print(
            f"  RM Interface Compliant: {report.rm_interface_compliant_modules}/{report.total_modules} ({report.rm_interface_compliant_modules/report.total_modules*100:.1f}%)"
        )
        print(
            f"  Health Monitoring Compliant: {report.health_monitoring_compliant_modules}/{report.total_modules} ({report.health_monitoring_compliant_modules/report.total_modules*100:.1f}%)"
        )
        print(
            f"  Registry Integrated: {report.registry_integrated_modules}/{report.total_modules} ({report.registry_integrated_modules/report.total_modules*100:.1f}%)"
        )
        print()
        ...
        sorted_assessments = sorted(
            report.module_assessments, key=lambda x: x.refactoring_priority
        )
        for i, assessment in enumerate(sorted_assessments[:5], 1):
            if not assessment.size_compliant:
                print(
                    f"  {i}. {assessment.module_name} ({assessment.line_count} lines, {assessment.size_violation.severity})"
                )
        print()
