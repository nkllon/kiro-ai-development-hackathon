...
class SystemMonitor:
    ...
    async def _check_recovery_failure_alert(self, rule) -> Dict[str, Any]:
        ...
        recovery_summary = self.recovery_manager.get_recovery_summary()
        recent_attempts = self.recovery_manager.get_recovery_history(1)
        failed_attempts = [attempt for attempt in recent_attempts if attempt.result == RecoveryResult.FAILED]
