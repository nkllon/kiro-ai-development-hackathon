"""
Message Handlers Handlers Handlers Handlers
...
class PromptRequestHandler(BaseMessageHandler):
    ...
    async def handle_message(self, message: BeastModeMessage) -> Optional[BeastModeMessage]:
        ...
        response = BeastModeMessage(type=MessageType.PROMPT_RESPONSE, source=self.agent_id, target=message.source, payload={'response': response_text, 'original_prompt': prompt, 'processed_at': datetime.now().isoformat()}, correlation_id=message.id, priority=message.priority)
...
class AgentDiscoveryHandler(BaseMessageHandler):
    ...
    async def handle_message(self, message: BeastModeMessage) -> Optional[BeastModeMessage]:
        ...
        try:
            ...
            our_capabilities = AgentCapabilities(agent_id=self.agent_id, capabilities=self.capabilities, availability='ready_for_business')
            response = BeastModeMessage(type=MessageType.AGENT_RESPONSE, source=self.agent_id, target=message.source, payload={'agent_capabilities': our_capabilities.model_dump(), 'response_to': message.id}, correlation_id=message.id, priority=3)
...
class HelpWantedHandler(BaseMessageHandler):
    ...
    async def handle_message(self, message: BeastModeMessage) -> Optional[BeastModeMessage]:
        ...
        if can_help:
            ...
            response = BeastModeMessage(type=MessageType.HELP_RESPONSE, source=self.agent_id, target=message.source, payload={'request_id': message.payload.get('request_id', message.id), 'can_help': True, 'matching_capabilities': matching_caps, 'confidence_score': confidence, 'agent_capabilities': self.capabilities, 'response_message': f"I can help with: {', '.join(matching_caps)}"}, correlation_id=message.id, priority=message.priority)
...
class SporeDeliveryHandler(BaseMessageHandler):
    ...
    async def handle_message(self, message: BeastModeMessage) -> Optional[BeastModeMessage]:
        ...
        if self.spore_callback:
            try:
                self.spore_callback(message.source, spore_name, {'content': spore_content, 'metadata': metadata, 'delivered_at': datetime.now().isoformat()})
            except Exception as e:
    ...
    async def handle_message(self, message: BeastModeMessage) -> Optional[BeastModeMessage]:
        ...
        if self.spore_provider:
            try:
                ...
                if spore_data:
                    response = BeastModeMessage(type=MessageType.SPORE_DELIVERY, source=self.agent_id, target=message.source, payload={'spore_name': spore_name, 'spore_content': spore_data.get('content', ''), 'metadata': spore_data.get('metadata', {}), 'requested_by': message.source}, correlation_id=message.id, priority=message.priority)
