...
authoritative source for automated_quality_gates.
...
class AutomatedQualityGates(ReflectiveModule):
    ...
    def execute_quality_assessment(self, target_path: Optional[str]=None) -> QualityAssessment:
        ...
        try:
            ...
            assessment = QualityAssessment(overall_status=overall_status, overall_score=overall_score, gate_results=gate_results, total_execution_time=total_execution_time, timestamp=datetime.now(), recommendations=recommendations, compliance_status=compliance_status)
    ...
    def _execute_linting_gate(self, config: QualityGateConfig, target_path: Path, start_time: float) -> QualityGateResult:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                score = max(0.0, 1.0 - violations_per_file / 10)
    ...
    def _execute_coverage_gate(self, config: QualityGateConfig, target_path: Path, start_time: float) -> QualityGateResult:
        ...
        try:
            cmd = ['python3', '-m', 'pytest', '--cov=' + str(target_path), '--cov-report=json', '--cov-report=term-missing', str(self.tests_path) if self.tests_path.exists() else str(target_path)]
            result = subprocess.run(cmd, capture_output=True, text=True, timeout=config.timeout_seconds)
    ...
    def _generate_quality_recommendations(self, gate_results: List[QualityGateResult]) -> List[str]:
        ...
        recommendations.extend(['Ensure all modules inherit from ReflectiveModule for RM compliance', 'Implement comprehensive health monitoring in all components', 'Follow systematic approach principles (no ad-hoc implementations)', 'Maintain >90% test coverage as per DR8 requirements', 'Document all architectural decisions using ADR system'])
