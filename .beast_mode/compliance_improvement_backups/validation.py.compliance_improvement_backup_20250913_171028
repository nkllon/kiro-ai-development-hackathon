...
class ConsistencyValidator(ReflectiveModule):
    ...
    def generate_consistency_score(self, spec_set: List[str]) -> ConsistencyMetrics:
        ...
        try:
            ...
            overall_score = terminology_score * 0.3 + interface_score * 0.4 + pattern_score * 0.3
            consistency_level = self._determine_consistency_level(overall_score)
            improvement_priority = self._generate_improvement_priorities(terminology_score, interface_score, pattern_score)
            return ConsistencyMetrics(terminology_score=terminology_score, interface_score=interface_score, pattern_score=pattern_score, overall_score=overall_score, consistency_level=consistency_level, critical_issues=critical_issues, improvement_priority=improvement_priority)
        except Exception as e:
