...
authoritative source for production_readiness_assessor.
...
class ProductionReadinessAssessor(ReflectiveModule):
    ...
    def conduct_comprehensive_assessment(self) -> ProductionReadinessReport:
        ...
        overall_score = sum((ca.overall_score * self._get_category_weight(ca.category_name) for ca in category_assessments))
        ...
        production_ready = overall_score >= self.readiness_thresholds['production_threshold'] and len(production_blockers) == 0
        enterprise_ready = overall_score >= self.readiness_thresholds['enterprise_threshold'] and len(enterprise_gaps) == 0
        ...
        report = ProductionReadinessReport(assessment_timestamp=datetime.now(), overall_readiness_score=overall_score, overall_percentage=overall_percentage, readiness_level=readiness_level, category_assessments=category_assessments, production_blockers=production_blockers, enterprise_gaps=enterprise_gaps, security_concerns=security_concerns, performance_issues=performance_issues, production_ready=production_ready, enterprise_ready=enterprise_ready, deployment_approved=deployment_approved, immediate_actions=immediate_actions, short_term_improvements=short_term_improvements, long_term_enhancements=long_term_enhancements, compliance_frameworks=compliance_frameworks, audit_trail=audit_trail)
    ...
    def _assess_category(self, category_name: str) -> CategoryAssessment:
        ...
        total_weighted_score = sum((result.score * criterion.weight for result, criterion in zip(criteria_results, category_criteria)))
