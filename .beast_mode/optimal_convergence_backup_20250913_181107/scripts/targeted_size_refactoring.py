...
def refactor_methods_file(file_path: Path, lines: list, target_lines: int) -> bool:
    ...
    print(f"  📊 After refactoring: {new_line_count} lines (reduction: {current_lines - new_line_count})")
...
def refactor_main_file(file_path: Path, lines: list, target_lines: int) -> bool:
    ...
    methods_content = [
        ...
        "        super().__init__(module_id=\"{}\", version=\"1.0.0\")".format(class_name.lower()),
    ...
    print(f"  📊 After refactoring: {new_line_count} lines (reduction: {len(lines) - new_line_count})")
