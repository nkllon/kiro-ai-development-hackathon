...
def diagnose_makefile_issues(self) -> MakefileDiagnosisResult:
    ...
    try:
        ...
        if not makefile_path.exists():
            return MakefileDiagnosisResult(missing_files=['Makefile'], broken_targets=[], dependency_issues=[], root_cause='Main Makefile missing - complete system failure', systematic_fix_required = True, workaround_temptation='Create minimal Makefile with:
...
def fix_makefile_systematically(self, diagnosis: MakefileDiagnosisResult) -> MakefileRepairResult:
    ...
    try:
        ...
        if self.metrics_engine:
            self.metrics_engine.establish_baseline_measurement('tool_health_performance', 'systematic', 1.0 if:
