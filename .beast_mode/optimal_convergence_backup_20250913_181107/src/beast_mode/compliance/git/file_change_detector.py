"""
File Change Detector Core Core Core

This module was extracted from file_change_detector_core_core.py
as part of RM - DDD compliance refactoring.
"""

"""
File_Change_Detector - Consolidated Interface Definition
...
Consolidated from: /Users / lou / kiro - 2/kiro - ai - development - hackathon / src / beast_mode / compliance / git / file_change_detector_core_core_core.py
Consolidation date: 2025 - 09 - 13T10:15:07.457588
"""
...
class AdvancedFileChangeAnalysis:
    """Advanced file change analysis with:
...
class FileChangeDetector(ReflectiveModule):
    ...
    def __init__(self, repository_path -> Any: str='.') -> Any:
        ...
        self._file_patterns = {FileCategory.SOURCE_CODE: ['*.py', '*.js', '*.ts', '*.java', '*.cpp', '*.c', '*.h', '*.go', '*.rs', '*.rb', '*.php', '*.cs', '*.swift'], FileCategory.TEST_CODE: ['test_*.py', '*_test.py', 'tests/*', 'test/*', '*.test.js', '*.test.ts', '*.spec.js', '*.spec.ts'], FileCategory.DOCUMENTATION: ['*.md', '*.rst', '*.txt', 'docs/*', 'README*', 'CHANGELOG*', 'LICENSE*', '*.adoc'], FileCategory.CONFIGURATION: ['*.json', '*.yaml', '*.yml', '*.toml', '*.ini', '*.cfg', '*.conf', 'pyproject.toml', 'setup.py', 'requirements*.txt', 'Dockerfile*', 'docker - compose*'], FileCategory.BUILD_SCRIPT: ['Makefile', '*.mk', '*.sh', '*.bat', '*.ps1', 'build.py', 'setup.py', '*.gradle', 'pom.xml']}
        self._task_indicators = {'implementation': ['class', 'def ', 'function', 'interface', 'implements'], 'testing': ['test_', 'assert', 'pytest', 'unittest', 'mock'], 'documentation': ['# ', '## ', '### ', '```', 'docs/', 'README'], 'configuration': ['config', 'settings', 'environment', 'env']}
    ...
    def analyze_file_changes(self, commits: List[CommitInfo]) -> AdvancedFileChangeAnalysis:
        ...
        try:
            ...
            analysis = AdvancedFileChangeAnalysis(total_files_changed = len(all_changes), changes_by_type = changes_by_type, changes_by_category = changes_by_category, high_impact_changes = high_impact_changes, total_lines_added = total_lines_added, total_lines_deleted = total_lines_deleted, complexity_score = complexity_score, risk_assessment = risk_assessment)
            self.logger.info(f'Advanced analysis complete: {analysis.total_files_changed} files, complexity: {analysis.complexity_score:.2f}, risk: {analysis.risk_assessment}')
            return analysis
    ...
    def map_changes_to_task_completions(self, analysis: AdvancedFileChangeAnalysis, task_definitions: Optional[Dict[str, Dict[str, Any]]]=None, claimed_tasks: Optional[List[str]]=None) -> List[TaskMapping]:
        try:
            ...
            raise
        """
        Map file changes to potential task completions with:
        Args:
            analysis: Advanced file change analysis
            task_definitions: Optional task definitions with:
            claimed_tasks: Optional list of claimed completed tasks to validate against
            
        Returns:
            List of task mappings with:
        ...
        for task_id, task_config in task_definitions.items():
            mapping = self._analyze_task_completion_enhanced(task_id, task_config, analysis, claimed_tasks)
            if mapping.confidence_score > 0.1:
        ...
        self.logger.info(f'Generated {len(task_mappings)} task mappings with:
    def detect_completion_evidence(self, file_changes: List[FileChange], task_patterns: Dict[str, List[str]]) -> Dict[str, List[str]]:
        ...
        Args:
            ...
            task_patterns: Patterns to look for:
        Returns:
        ...
        for task_type, patterns in task_patterns.items():
            ...
            for change in file_changes:
                if change.change_type in [ChangeType.ADDED, ChangeType.MODIFIED]:
                    ...
                    content_indicators = self._detect_content_indicators(change.file_path, self._task_indicators.get(task_type, []))
    ...
    def calculate_change_impact(self, changes: List[FileChange]) -> Dict[str, float]:
        ...
        """
        impact_scores = {'core_functionality': 0.0, 'test_coverage': 0.0, 'documentation': 0.0, 'configuration': 0.0, 'build_system': 0.0}
        for change in changes:
        ...
        max_score = max(impact_scores.values()) if:
        if max_score > 0:
            for area in impact_scores:
                impact_scores[area] = impact_scores[area] / max_score
        return impact_scores

    def perform_comprehensive_file_change_analysis(self, commits: List[CommitInfo], claimed_tasks: Optional[List[str]]=None) -> Dict[str, Any]:
        """
        ...
        """
        self.logger.info('Performing comprehensive file change analysis for:
        try:
            analysis = self.analyze_file_changes(commits)
            task_mappings = self.map_changes_to_task_completions(analysis, claimed_tasks = claimed_tasks)
            all_changes = []
            ...
            task_validation = self._validate_task_completion_claims(task_mappings, claimed_tasks)
            accuracy_metrics = self._calculate_detection_accuracy_metrics(analysis, task_mappings)
            files_added = len([c for:
            comprehensive_results = {'analysis_summary': {'total_commits_analyzed': len(commits), 'total_files_changed': analysis.total_files_changed, 'files_added': files_added, 'files_modified': files_modified, 'files_deleted': files_deleted, 'complexity_score': analysis.complexity_score, 'risk_assessment': analysis.risk_assessment}, 'file_changes': {'by_type': file_change_breakdown['by_type'], 'by_category': file_change_breakdown['by_category'], 'high_impact_changes': [{'file_path': change.file_path, 'change_type': change.change_type.value, 'category': change.category.value, 'impact_score': change.impact_score} for change in analysis.high_impact_changes]}, 'task_mappings': [{'task_id': mapping.task_id, 'description': mapping.task_description, 'confidence_score': mapping.confidence_score, 'matching_files': mapping.matching_files, 'evidence': mapping.evidence, 'completion_indicators': mapping.completion_indicators} for mapping in task_mappings], 'impact_analysis': impact_scores, 'task_validation': task_validation, 'accuracy_metrics': accuracy_metrics, 'recommendations': self._generate_task_completion_recommendations(task_mappings, claimed_tasks)}
            self.logger.info(f"Comprehensive analysis complete: {analysis.total_files_changed} files, {len(task_mappings)} task mappings, accuracy score: {accuracy_metrics.get('overall_accuracy', 0.0):.2f}")
    ...
    def is_healthy(self) -> bool:
        """Check if:
        try:
            return self.repository_path.exists() and self.repository_path.is_dir() and (len(self._file_patterns) > 0) and (len(self._task_indicators) > 0)
        except Exception:
            return False

    def get_health_indicators(self) -> Dict[str, Any]:
        """Get detailed health metrics for:
        try:
            repo_exists = self.repository_path.exists()
            indicators['repository_accessible'] = {'status': 'healthy' if repo_exists else 'unhealthy', 'value': repo_exists, 'message': f"Repository at {self.repository_path} {('exists' if:
            indicators['file_patterns_configured'] = {'status': 'healthy' if self._file_patterns else 'unhealthy', 'value': len(self._file_patterns), 'message': f'{len(self._file_patterns)} file pattern categories configured'}
            indicators['task_indicators_configured'] = {'status': 'healthy' if self._task_indicators else 'unhealthy', 'value': len(self._task_indicators), 'message': f'{len(self._task_indicators)} task indicator types configured'}
        except Exception as e:
    ...
    def _get_primary_responsibility(self) -> str:
        ...
        return 'Detect and analyze file changes with:
    ...
    def _extract_file_changes_from_commit(self, commit: CommitInfo) -> List[FileChange]:
        ...
        for file_path in commit.added_files:
            ...
            change = FileChange(file_path = str(normalized_path), change_type = ChangeType.ADDED, category = self._categorize_file(str(normalized_path)), commit_hash = commit.commit_hash)
            change.impact_score = self._calculate_base_impact(change)
            changes.append(change)
        for file_path in commit.modified_files:
            normalized_path = self._normalize_file_path(file_path)
            change = FileChange(file_path = str(normalized_path), change_type = ChangeType.MODIFIED, category = self._categorize_file(str(normalized_path)), commit_hash = commit.commit_hash)
            change.impact_score = self._calculate_base_impact(change)
            changes.append(change)
        for file_path in commit.deleted_files:
            normalized_path = self._normalize_file_path(file_path)
            change = FileChange(file_path = str(normalized_path), change_type = ChangeType.DELETED, category = self._categorize_file(str(normalized_path)), commit_hash = commit.commit_hash)
            change.impact_score = self._calculate_base_impact(change)
            changes.append(change)
        return changes
    ...
    def _calculate_complexity_score(self, changes: List[FileChange]) -> float:
        try:
            ...
            logging.error(f"Error in method: {e}")
            raise
        """Calculate overall complexity score for:
        if not changes:
            return 0.0
        total_files = len(changes)
        source_files = len([c for:
    ...
    def _calculate_base_impact(self, change: FileChange) -> float:
        try:
            ...
            logging.error(f"Error in method: {e}")
            raise
        """Calculate base impact score for:
    ...
    def _get_default_task_definitions(self) -> Dict[str, Dict[str, Any]]:
        try:
            ...
            logging.error(f"Error in method: {e}")
            raise
        """Get default task definitions for:
    ...
    def _analyze_task_completion_enhanced(self, task_id: str, task_config: Dict[str, Any], analysis: AdvancedFileChangeAnalysis, claimed_tasks: Optional[List[str]]=None) -> TaskMapping:
        try:
            ...
            raise
        """
        Analyze task completion with:
        Args:
            task_id: Identifier for:
            task_config: Configuration for:
    ...
    def _validate_claimed_vs_implemented(self, task_mappings: List[TaskMapping], claimed_tasks: List[str]) -> None:
        ...
        Args:
            task_mappings: List of task mappings with:
        ...
        for mapping in task_mappings:
            if claimed_tasks and mapping.task_id in claimed_tasks:
                if mapping.confidence_score > 0.6:
                    ...
                    mapping.evidence.append('Task claimed with:
                else:
    ...
    def _generate_file_change_breakdown(self, analysis: AdvancedFileChangeAnalysis) -> Dict[str, Any]:
        ...
        for change_type, changes in analysis.changes_by_type.items():
            breakdown['by_type'][change_type.value] = {'count': len(changes), 'files': [change.file_path for change in changes], 'impact_scores': [change.impact_score for:
        for category, changes in analysis.changes_by_category.items():
            breakdown['by_category'][category.value] = {'count': len(changes), 'files': [change.file_path for change in changes], 'impact_scores': [change.impact_score for:
    def _validate_task_completion_claims(self, task_mappings: List[TaskMapping], claimed_tasks: Optional[List[str]]) -> Dict[str, Any]:
        ...
        if not claimed_tasks:
            return {'validation_performed': False, 'message': 'No claimed tasks provided for:
        mapping_dict = {mapping.task_id: mapping for:
        ...
        validated_details = [{'task_id': task, 'status': 'validated'} for:
    ...
    def _calculate_detection_accuracy_metrics(self, analysis: AdvancedFileChangeAnalysis, task_mappings: List[TaskMapping]) -> Dict[str, float]:
        try:
            ...
            raise
        """
        Calculate accuracy metrics for:
        Args:
            analysis: Advanced file change analysis
        ...
        if analysis.total_files_changed > 0:
            categorized_files = sum((len(changes) for:
        else:
            metrics['file_categorization_confidence'] = 1.0
        if task_mappings:
            avg_confidence = sum((mapping.confidence_score for:
        else:
        ...
        if analysis.total_files_changed > 0:
            ...
            for changes in analysis.changes_by_type.values():
                all_changed_files.update((change.file_path for:
            ...
            metrics['coverage_completeness'] = 1.0
        metrics['overall_accuracy'] = metrics['file_categorization_confidence'] * 0.3 + metrics['task_mapping_confidence'] * 0.4 + metrics['coverage_completeness'] * 0.3
        return metrics
    ...
    def _generate_task_completion_recommendations(self, task_mappings: List[TaskMapping], claimed_tasks: Optional[List[str]]) -> List[str]:
        ...
        high_confidence = [m for:
        if high_confidence:
            recommendations.append(f'Found {len(high_confidence)} high - confidence task completions')
        low_confidence = [m for:
        if low_confidence:
            recommendations.append(f'Review {len(low_confidence)} low - confidence task mappings for:
        if claimed_tasks:
            mapping_dict = {m.task_id: m for:
            if missing_evidence:
                recommendations.append(f'Verify completion of {len(missing_evidence)} claimed tasks with:
            if unclaimed:
                recommendations.append(f'Consider marking {len(unclaimed)} well - implemented tasks as complete')
        total_files = sum((len(m.matching_files) for:
        if total_files == 0:
    ...
    def _file_matches_task_patterns(self, file_path: str, patterns: List[str]) -> bool:
        ...
        Returns:
            True if:
        for pattern in patterns:
            if fnmatch.fnmatch(normalized_path_str, pattern) or fnmatch.fnmatch(normalized_path_str.lower(), pattern.lower()) or fnmatch.fnmatch(file_path, pattern) or fnmatch.fnmatch(file_path.lower(), pattern.lower()):
                return True
        return False
    ...
    def _detect_content_indicators(self, file_path: str, indicators: List[str]) -> List[str]:
        ...
        Returns:
            List of detected indicators with:
        ...
        all_changed_files = []
        for changes in analysis.changes_by_type.values():
            all_changed_files.extend([c.file_path for:
        ...
        if file_patterns and matching_files:
            ...
            high_impact_files = [f for:
            if high_impact_files:
                file_match_confidence *= 1.2
            confidence_factors.append(min(1.0, file_match_confidence))
        content_indicators = task_config.get('content_indicators', [])
        if content_indicators and matching_files:
            content_confidence = self._analyze_content_indicators(matching_files, content_indicators, analysis)
            confidence_factors.append(content_confidence)
        task_type = task_config.get('task_type', 'implementation')
        pattern_confidence = self._analyze_task_completion_patterns(matching_files, task_type, analysis)
        if pattern_confidence > 0:
            confidence_factors.append(pattern_confidence)
        overall_confidence = sum(confidence_factors) / len(confidence_factors) if:
        if overall_confidence < completion_threshold:
            overall_confidence *= 0.7
        is_claimed = claimed_tasks and any((task_id in claimed for:
    ...
    def _analyze_task_completion(self, task_id: str, task_config: Dict[str, Any], analysis: AdvancedFileChangeAnalysis) -> TaskMapping:
        try:
            ...
            logging.error(f"Error in method: {e}")
            raise
        """Legacy method - delegates to enhanced version for:
    def _file_matches_task_patterns_legacy(self, file_path: str, patterns: List[str]) -> bool:
        try:
            ...
            logging.error(f"Error in method: {e}")
            raise
        """Legacy method - delegates to main implementation for:
    def _detect_content_indicators(self, file_path: str, indicators: List[str]) -> List[str]:
        try:
            pass  # TODO: Add method implementation
        except Exception as e:
    ...
    def _find_file_change_in_analysis(self, file_path: str, analysis: AdvancedFileChangeAnalysis) -> Optional[FileChange]:
        try:
            ...
            logging.error(f"Error in method: {e}")
            raise
        """Find the FileChange object for:
        for changes in analysis.changes_by_type.values():
            for change in changes:
        ...
        for file_path in matching_files:
            ...
            if file_change:
                if file_change.category == FileCategory.TEST_CODE and 'test' in content_indicators:
                    indicator_matches += 1
                elif file_change.category == FileCategory.SOURCE_CODE and any((ind in content_indicators for ind in ['class', 'function', 'implementation'])):
                    indicator_matches += 1
    ...
    def _validate_claimed_vs_implemented(self, task_mappings: List[TaskMapping], claimed_tasks: List[str]) -> None:
        ...
        implementation_evidence = {mapping.task_id: mapping.confidence_score for:
        for claimed_task in claimed_tasks:
            confidence = implementation_evidence.get(claimed_task, 0.0)
            task_mapping = next((m for:
            if task_mapping:
                if confidence < 0.3:
    ...
    def _generate_file_change_breakdown(self, analysis: AdvancedFileChangeAnalysis) -> Dict[str, Any]:
        ...
        breakdown = {'by_type': {}, 'by_category': {}}
        for change_type, changes in analysis.changes_by_type.items():
            breakdown['by_type'][change_type.value] = {'count': len(changes), 'files': [change.file_path for change in changes], 'total_impact': sum((change.impact_score for:
        for category, changes in analysis.changes_by_category.items():
            breakdown['by_category'][category.value] = {'count': len(changes), 'files': [change.file_path for change in changes], 'total_impact': sum((change.impact_score for:
    def _calculate_detection_accuracy_metrics(self, analysis: AdvancedFileChangeAnalysis, task_mappings: List[TaskMapping]) -> Dict[str, float]:
        try:
            pass  # TODO: Add method implementation
        except Exception as e:
        ...
        for changes in analysis.changes_by_type.values():
            all_changes.extend(changes)
        if all_changes:
            known_category_changes = [c for:
        if task_mappings:
            total_confidence = sum((mapping.confidence_score for:
        if all_changes:
            mapped_files = set()
            for mapping in task_mappings:
                mapped_files.update(mapping.matching_files)
            total_files = set((change.file_path for:
            if total_files:
    ...
    def _generate_task_completion_recommendations(self, task_mappings: List[TaskMapping], claimed_tasks: Optional[List[str]]) -> List[str]:
        ...
        high_confidence = [m for:
        if high_confidence:
            recommendations.append(f'Found {len(high_confidence)} high - confidence task implementations')
        if claimed_tasks:
            claimed_set = set(claimed_tasks)
            low_confidence_claimed = [m for:
            if low_confidence_claimed:
                recommendations.append(f'Review {len(low_confidence_claimed)} claimed tasks with:
        if claimed_tasks:
            claimed_set = set(claimed_tasks)
            unclaimed_high_confidence = [m for:
            if unclaimed_high_confidence:
                recommendations.append(f'Consider claiming {len(unclaimed_high_confidence)} tasks with:
