"""
Installation Validator Core Core Core

This module was extracted from installation_validator_core_core.py
as part of RM - DDD compliance refactoring.
"""

"""
Installation_Validator - Consolidated Interface Definition
...
Consolidated from: /Users / lou / kiro - 2/kiro - ai - development - hackathon / src / hackathon_demo_framework / validation / installation_validator_core_core_core.py
Consolidation date: 2025 - 09 - 13T10:15:07.554719
"""
...
class InstallationSetupValidator:
    ...
    def __init__(self, project_path -> Any: Path) -> Any:
        ...
        self.config_files = ['requirements.txt', 'pyproject.toml', 'setup.py', 'Pipfile', 'environment.yml', 'poetry.lock']
        self.doc_files = ['README.md', 'README.rst', 'INSTALL.md', 'SETUP.md', 'docs / installation.md', 'docs / setup.md']
        self.required_sections = ['installation', 'setup', 'requirements', 'dependencies', 'getting started', 'quick start']
        self.logger.info(f'Installation validator initialized for:
    ...
    def generate_installation_guide(self) -> str:
        ...
        guide_sections.append('This guide provides step - by - step instructions for:
        ...
        config_files = ['.env.example', 'config.example.json', 'settings.example.py']
        found_config = False
        for config_file in config_files:
            if (self.project_path / config_file).exists():
                ...
                break
        if not found_config:
            guide_sections.append('No configuration files found. The project may work with:
    ...
    def _validate_requirements_txt(self, file_path: Path, content: str) -> List[InstallationIssue]:
        ...
        for i, line in enumerate(lines):
            ...
            if '==' not in line and '>=' not in line and ('~=' not in line):
                issues.append(InstallationIssue(issue_type = InstallationIssueType.DEPENDENCY_CONFLICT, severity='minor', message = f"Dependency '{line}' not version - pinned", file_path = str(file_path), suggestion='Consider pinning versions for:
    ...
    def _validate_pyproject_toml(self, file_path: Path, content: str) -> List[InstallationIssue]:
        ...
        try:
            ...
            if 'project' not in data and 'tool' not in data:
                issues.append(InstallationIssue(issue_type = InstallationIssueType.MISSING_REQUIREMENTS, severity='major', message='pyproject.toml missing project or tool sections', file_path = str(file_path), suggestion='Add [project] section with:
            if 'project' in data:
                project = data['project']
                if 'dependencies' not in project and 'requires' not in project:
                    issues.append(InstallationIssue(issue_type = InstallationIssueType.MISSING_REQUIREMENTS, severity='minor', message='No dependencies specified in pyproject.toml', file_path = str(file_path), suggestion='Add dependencies list if:
        except Exception as e:
    ...
    def _validate_setup_py(self, file_path: Path, content: str) -> List[InstallationIssue]:
        ...
        if 'install_requires' not in content:
            issues.append(InstallationIssue(issue_type = InstallationIssueType.MISSING_REQUIREMENTS, severity='minor', message='setup.py missing install_requires', file_path = str(file_path), suggestion='Add install_requires list if:
    ...
    def _validate_setup_instructions(self) -> Dict[str, Any]:
        ...
        if not doc_files_found:
            issues.append(InstallationIssue(issue_type = InstallationIssueType.DOCUMENTATION, severity='major', message='No installation documentation found', suggestion='Add README.md with:
        else:
            for doc_file in doc_files_found:
                try:
                    ...
                    if sections_found == 0:
                        ...
                        issues.append(InstallationIssue(issue_type = InstallationIssueType.SETUP_INSTRUCTIONS, severity='minor', message = f'Limited installation instructions in {doc_file.name}', file_path = str(doc_file), suggestion='Expand installation instructions with:
                    if '```' not in content and '`' not in content:
                        issues.append(InstallationIssue(issue_type = InstallationIssueType.SETUP_INSTRUCTIONS, severity='minor', message = f'No code examples in {doc_file.name}', file_path = str(doc_file), suggestion='Add code examples for:
                except Exception as e:
    ...
    def _validate_documentation(self) -> Dict[str, Any]:
        ...
        for readme_name in ['README.md', 'README.rst', 'README.txt']:
            if (self.project_path / readme_name).exists():
                ...
                break
        if not readme_path:
            issues.append(InstallationIssue(issue_type = InstallationIssueType.DOCUMENTATION, severity='critical', message='No README file found', suggestion='Add README.md with:
        else:
            try:
                with open(readme_path, 'r', encoding='utf - 8') as f:
                    content = f.read()
                if len(content.strip()) < 100:
                    issues.append(InstallationIssue(issue_type = InstallationIssueType.DOCUMENTATION, severity='major', message='README too short', file_path = str(readme_path), suggestion='Expand README with:
    ...
    def _generate_recommendations(self, issues: List[InstallationIssue], scores: Dict[str, float]) -> List[str]:
        ...
        if scores['setup'] < 70:
            recommendations.append('Add detailed setup instructions with:
    ...
    def _create_error_report(self, error_message: str) -> InstallationReport:
        ...
        return InstallationReport(overall_score = 0.0, requirements_score = 0.0, dependency_score = 0.0, setup_score = 0.0, documentation_score = 0.0, environment_score = 0.0, total_issues = 1, critical_issues = 1, major_issues = 0, minor_issues = 0, issues=[InstallationIssue(issue_type = InstallationIssueType.INSTALLATION_FAILURE, severity='critical', message = error_message, suggestion='Fix validation environment and try:
def __init__(self, project_path -> Any: Path) -> Any:
    """
        Initialize the installation validator.
