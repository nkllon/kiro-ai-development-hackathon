"""
Hackathon Demo View Core

This module was extracted from hackathon_demo_view.py
...
class HackathonDemoView:
    ...
    def render_complete_demo(self) -> Dict[str, Any]:
        ...
        total_duration = hook.duration_seconds + sum((demo.duration_seconds for demo in core_demos)) + deep_dive.duration_seconds + next_steps.duration_seconds
        demo_summary = {'demo_id': f"DEMO-{demo_start.strftime('%Y%m%d%H%M%S')}", 'total_duration_seconds': total_duration, 'target_duration_seconds': 180, 'phases': {'hook': {'title': hook.title, 'duration': hook.duration_seconds, 'success_metrics': hook.success_metrics}, 'core_demonstrations': [{'title': demo.title, 'duration': demo.duration_seconds, 'success_metrics': demo.success_metrics} for demo in core_demos], 'deep_dive': {'title': deep_dive.title, 'duration': deep_dive.duration_seconds, 'success_metrics': deep_dive.success_metrics}, 'next_steps': {'title': next_steps.title, 'duration': next_steps.duration_seconds, 'success_metrics': next_steps.success_metrics}}, 'interactive_elements': [element for demo in [hook] + core_demos + [deep_dive, next_steps] for element in demo.interactive_elements], 'overall_success_metrics': {'time_efficiency': '3_minutes_exact', 'judge_engagement': 'high', 'value_demonstration': 'comprehensive', 'actionability': 'immediate'}, 'created_at': demo_start.isoformat()}
        return demo_summary
