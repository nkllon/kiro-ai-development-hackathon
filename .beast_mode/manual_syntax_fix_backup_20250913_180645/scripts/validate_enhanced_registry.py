...
def validate_method_signatures(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if validation_results['total_methods'] > 0:
        validation_results['signature_completeness'] = round(
            validation_results['methods_with_signatures'] / validation_results['total_methods'] * 100, 2
        )
...
def validate_file_tracking(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for interface_id, interface_data in data.get('interfaces', {}).items():
        ...
        if file_path and os.path.exists(file_path):
            ...
            validation_results['location_validation_errors'].append(f"File not found: {file_path}")
    ...
    if validation_results['total_interfaces'] > 0:
        validation_results['location_precision_rate'] = round(
            validation_results['interfaces_with_precise_locations'] / validation_results['total_interfaces'] * 100, 2
        )
    
    return validation_results

def validate_domain_vocabulary(registry_data: Dict[str, Any], vocabulary_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if validation_results['total_domain_terms'] > 0:
        validation_results['vocabulary_coverage_rate'] = round(
            validation_results['terms_used_in_interfaces'] / validation_results['total_domain_terms'] * 100, 2
        )
    
    if validation_results['ubiquitous_language_terms'] > 0:
        validation_results['ubiquitous_coverage_rate'] = round(
            validation_results['ubiquitous_language_usage'] / validation_results['ubiquitous_language_terms'] * 100, 2
        )
...
def validate_registry_integrity(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    compliance_scores = []
    valid_interface_types = {'reflective_module', 'domain_service', 'api_interface', 'data_model', 'validation_rule', 'configuration'}
    
    for interface_id, interface_data in data.get('interfaces', {}).items():
        validation_results['total_interfaces'] += 1
        
        # Check required metadata
        required_fields = ['interface_name', 'interface_type', 'file_path', 'line_number']
        ...
        if interface_type in valid_interface_types:
            ...
            validation_results['integrity_errors'].append(f"Invalid interface type: {interface_type}")
    
    # Calculate average compliance
    if compliance_scores:
        validation_results['average_compliance'] = round(sum(compliance_scores) / len(compliance_scores), 2)
    
    # Calculate overall consistency score
    if validation_results['total_interfaces'] > 0:
        consistency_factors = [
            validation_results['interfaces_with_metadata'] / validation_results['total_interfaces'],
            validation_results['compliance_scores_valid'] / validation_results['total_interfaces'],
            validation_results['interface_types_valid'] / validation_results['total_interfaces']
        ]
        validation_results['registry_consistency_score'] = round(sum(consistency_factors) / len(consistency_factors) * 100, 2)
    
    return validation_results

def generate_validation_report(registry_data: Dict[str, Any], vocabulary_data: Dict[str, Any]) -> None:
    ...
    print(f"   Methods with type annotations: {method_validation['methods_with_type_annotations']}")
    print(f"   Signature completeness: {method_validation['signature_completeness']}%")
    
    print(f"\n   Most common method names:")
    for method_name, count in method_validation['common_method_patterns'].most_common(10):
    ...
    print(f"   Interfaces with precise locations: {location_validation['interfaces_with_precise_locations']}")
    print(f"   Location precision rate: {location_validation['location_precision_rate']}%")
    print(f"   File path accuracy: {location_validation['file_path_accuracy']}/{location_validation['total_interfaces']}")
    print(f"   Line number accuracy: {location_validation['line_number_accuracy']}/{location_validation['total_interfaces']}")
    print(f"   End line accuracy: {location_validation['end_line_accuracy']}/{location_validation['total_interfaces']}")
    
    if location_validation['location_validation_errors']:
        print(f"   Location errors: {len(location_validation['location_validation_errors'])}")
    ...
    print(f"   Terms used in interfaces: {vocabulary_validation['terms_used_in_interfaces']}")
    print(f"   Domain vocabulary coverage: {vocabulary_validation['vocabulary_coverage_rate']}%")
    print(f"   Ubiquitous language terms: {vocabulary_validation['ubiquitous_language_terms']}")
    print(f"   Ubiquitous language usage: {vocabulary_validation['ubiquitous_language_usage']}")
    print(f"   Ubiquitous language coverage: {vocabulary_validation['ubiquitous_coverage_rate']}%")
    print(f"   Semantic relationships: {vocabulary_validation['semantic_relationships']}")
    ...
    print(f"   Interfaces with metadata: {integrity_validation['interfaces_with_metadata']}")
    print(f"   Valid compliance scores: {integrity_validation['compliance_scores_valid']}")
    ...
    print(f"   Registry consistency score: {integrity_validation['registry_consistency_score']}%")
    ...
    print(f"   Method signature enhancement: {enhancement_scores['Method Signatures']}%")
