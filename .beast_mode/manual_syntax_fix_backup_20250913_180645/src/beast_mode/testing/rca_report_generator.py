"""
Rca Report Generator Core Core Core
...
"""
Rca_Report_Generator - Consolidated Interface Definition
...
authoritative source for rca_report_generator.

Consolidated from: /Users/lou/kiro-2/kiro-ai-development-hackathon/src/beast_mode/testing/rca_report_generator_core_core_core.py
Consolidation date: 2025-09-13T10:15:07.463151
"""
...
class RCAReportGenerator(ReflectiveModule):
    ...
    def __init__(self) -> Any:
        ...
        self.default_config = ReportConfiguration(format=ReportFormat.CONSOLE, include_sections=list(ReportSection), max_failures_displayed=10, max_recommendations=5, include_stack_traces=False, color_output=True, verbose_mode=False)
    ...
    def generate_report(self, rca_report: TestRCAReportData, config: Optional[ReportConfiguration]=None) -> FormattedReport:
        """
        ...
        try:
            ...
            for section_type in report_config.include_sections:
                try:
                    ...
                    error_section = FormattedReportSection(section_type=section_type, title=f'Error in {section_type.value}', content=f'Section generation failed: {e}', metadata={'error': str(e)})
                    sections.append(error_section)
            total_length = sum((len(section.content) for section in sections))
            formatted_report = FormattedReport(report_id=report_id, generation_timestamp=datetime.now(), format=report_config.format, sections=sections, total_length=total_length, metadata={'total_failures': rca_report.total_failures, 'failures_analyzed': rca_report.failures_analyzed, 'systematic_fixes': rca_report.summary.systematic_fixes_available, 'confidence_score': rca_report.summary.confidence_score, 'generation_config': asdict(report_config)})
            ...
            error_section = FormattedReportSection(section_type=ReportSection.HEADER, title='Report Generation Error', content=f'Failed to generate report: {e}', metadata={'error': str(e)})
            return FormattedReport(report_id=f'error_report_{int(time.time())}', generation_timestamp=datetime.now(), format=config.format if config else ReportFormat.CONSOLE, sections=[error_section], total_length=len(error_section.content), metadata={'generation_error': str(e)})
    ...
    def generate_json_report(self, rca_report: TestRCAReportData) -> Dict[str, Any]:
        ...
        try:
            ...
            formatted_report = self.generate_report(rca_report, config)
            json_data = {'report_metadata': {'report_id': formatted_report.report_id, 'generation_timestamp': formatted_report.generation_timestamp.isoformat(), 'format': formatted_report.format.value, 'total_length': formatted_report.total_length}, 'analysis_summary': {'total_failures': rca_report.total_failures, 'failures_analyzed': rca_report.failures_analyzed, 'analysis_timestamp': rca_report.analysis_timestamp.isoformat(), 'confidence_score': rca_report.summary.confidence_score, 'systematic_fixes_available': rca_report.summary.systematic_fixes_available, 'pattern_matches_found': rca_report.summary.pattern_matches_found, 'estimated_fix_time_minutes': rca_report.summary.estimated_fix_time_minutes}, 'root_causes': [{'type': cause_type.value, 'count': count} for cause_type, count in rca_report.summary.most_common_root_causes], 'critical_issues': rca_report.summary.critical_issues, 'recommendations': rca_report.recommendations, 'next_steps': rca_report.next_steps, 'prevention_patterns': [{'pattern_id': pattern.pattern_id, 'pattern_name': pattern.pattern_name, 'prevention_steps': pattern.prevention_steps} for pattern in rca_report.prevention_patterns], 'rca_results': [{'failure_id': result.failure.failure_id, 'component': result.failure.component, 'root_causes_count': len(result.root_causes), 'systematic_fixes_count': len(result.systematic_fixes), 'analysis_time_seconds': result.total_analysis_time_seconds, 'confidence_score': result.rca_confidence_score} for result in rca_report.rca_results]}
            return json_data
        except Exception as e:
            self.logger.error(f'JSON report generation failed: {e}')
            return {'error': f'JSON report generation failed: {e}', 'timestamp': datetime.now().isoformat()}
    ...
    def generate_markdown_report(self, rca_report: TestRCAReportData) -> str:
        ...
        """
        try:
            config = ReportConfiguration(format=ReportFormat.MARKDOWN, color_output=False, include_sections=list(ReportSection))
            formatted_report = self.generate_report(rca_report, config)
            markdown_content = []
            for section in formatted_report.sections:
    ...
    def _generate_header_section(self, rca_report: TestRCAReportData, config: ReportConfiguration) -> FormattedReportSection:
        ...
        if config.format == ReportFormat.CONSOLE:
            ...
            content = json.dumps({'section': 'header', 'title': 'Test RCA Analysis Report', 'timestamp': rca_report.analysis_timestamp.isoformat()}, indent=2)
    ...
    def _generate_summary_section(self, rca_report: TestRCAReportData, config: ReportConfiguration) -> FormattedReportSection:
        ...
        if config.format == ReportFormat.CONSOLE:
            ...
            content = json.dumps({'section': 'summary', 'metrics': {'total_failures': rca_report.total_failures, 'failures_analyzed': rca_report.failures_analyzed, 'systematic_fixes_available': summary.systematic_fixes_available, 'pattern_matches_found': summary.pattern_matches_found, 'confidence_score': summary.confidence_score, 'estimated_fix_time_minutes': summary.estimated_fix_time_minutes, 'critical_issues': summary.critical_issues}}, indent=2)
    ...
    def _generate_failures_section(self, rca_report: TestRCAReportData, config: ReportConfiguration) -> FormattedReportSection:
        ...
        if config.format == ReportFormat.CONSOLE:
            ...
            for group_name, failures in rca_report.grouped_failures.items():
                failure_data[group_name] = [{'test_name': f.test_name, 'test_file': f.test_file, 'failure_type': f.failure_type, 'error_message': f.error_message[:200] + '...' if len(f.error_message) > 200 else f.error_message} for f in failures[:config.max_failures_displayed]]
            content = json.dumps({'section': 'failures', 'grouped_failures': failure_data}, indent=2)
    ...
    def _generate_root_causes_section(self, rca_report: TestRCAReportData, config: ReportConfiguration) -> FormattedReportSection:
        ...
        if config.format == ReportFormat.CONSOLE:
            ...
            content = json.dumps({'section': 'root_causes', 'most_common_root_causes': [{'cause_type': cause_type.value, 'count': count, 'cause_name': cause_type.value.replace('_', ' ').title()} for cause_type, count in rca_report.summary.most_common_root_causes]}, indent=2)
    ...
    def _generate_fixes_section(self, rca_report: TestRCAReportData, config: ReportConfiguration) -> FormattedReportSection:
        ...
        if config.format == ReportFormat.CONSOLE:
            ...
            fixes_data = [{'fix_id': fix.fix_id, 'description': fix.fix_description, 'root_cause_type': fix.root_cause.cause_type.value, 'estimated_time_minutes': fix.estimated_time_minutes, 'implementation_steps': fix.implementation_steps, 'validation_criteria': fix.validation_criteria} for fix in all_fixes]
            content = json.dumps({'section': 'systematic_fixes', 'total_fixes': len(all_fixes), 'fixes': fixes_data}, indent=2)
    ...
    def _generate_recommendations_section(self, rca_report: TestRCAReportData, config: ReportConfiguration) -> FormattedReportSection:
        ...
        if config.format == ReportFormat.CONSOLE:
            ...
            content = json.dumps({'section': 'recommendations', 'recommendations': recommendations}, indent=2)
    ...
    def _generate_patterns_section(self, rca_report: TestRCAReportData, config: ReportConfiguration) -> FormattedReportSection:
        ...
        if config.format == ReportFormat.CONSOLE:
            ...
            patterns_data = [{'pattern_id': p.pattern_id, 'pattern_name': p.pattern_name, 'prevention_steps': p.prevention_steps, 'detection_criteria': p.detection_criteria} for p in patterns]
            content = json.dumps({'section': 'prevention_patterns', 'total_patterns': len(patterns), 'patterns': patterns_data}, indent=2)
    ...
    def _generate_next_steps_section(self, rca_report: TestRCAReportData, config: ReportConfiguration) -> FormattedReportSection:
        ...
        if config.format == ReportFormat.CONSOLE:
            ...
            content = json.dumps({'section': 'next_steps', 'next_steps': next_steps}, indent=2)
    ...
    def _generate_footer_section(self, rca_report: TestRCAReportData, config: ReportConfiguration) -> FormattedReportSection:
        ...
        if config.format == ReportFormat.CONSOLE:
            ...
            content = json.dumps({'section': 'footer', 'analysis_complete': True, 'overall_confidence': rca_report.summary.confidence_score, 'timestamp': datetime.now().isoformat()}, indent=2)
        return FormattedReportSection(section_type=ReportSection.FOOTER, title='Report Footer', content=content, metadata={'confidence': rca_report.summary.confidence_score})
