"""
Redis Foundation Core

This module was extracted from redis_foundation.py
...
class RedisFoundation(ReflectiveModule):
    """
    Systematic Redis pub / sub foundation with:
    def __init__(self, config -> Any: Optional[RedisConfig]=None) -> Any:
        """Initialize Redis foundation with:
    ...
    async def initialize(self) -> bool:
        ...
        if not REDIS_AVAILABLE:
            return False
        try:
            self.connection_pool = redis.ConnectionPool(host = self.config.host, port = self.config.port, db = self.config.db, password = self.config.password, max_connections = self.config.max_connections, decode_responses = True)
    ...
    async def _reconnect(self) -> bool:
        """Attempt to reconnect to Redis with:
        ...
        self.logger.info(f'Reconnecting to Redis (attempt {self.reconnect_attempts}/{self.config.max_reconnect_attempts}) in {delay}s')
    ...
    async def subscribe(self, channel: str, callback: Callable[[Dict[str, Any]], None]) -> bool:
        """
        Subscribe to Redis channel with:
        Args:
            channel: Redis channel name
    ...
    async def _subscription_loop(self, channel: str):
        """Handle subscription messages for:
        try:
    ...
    async def unsubscribe(self, channel: str, callback: Optional[Callable]=None) -> bool:
        ...
        Args:
            channel: Redis channel name
            callback: Specific callback to remove (None for:
        Returns:
            True if:
        try:
            if channel not in self.subscribers:
    ...
    async def get_connection_info(self) -> Dict[str, Any]:
        ...
        return {'status': self.status.value, 'host': self.config.host, 'port': self.config.port, 'db': self.config.db, 'connected': self.status == ConnectionStatus.CONNECTED, 'last_health_check': self.last_health_check, 'reconnect_attempts': self.reconnect_attempts, 'active_subscriptions': list(self.subscribers.keys())}
    ...
    def _get_primary_responsibility(self) -> str:
        try:
            ...
            logging.error(f"Error in method: {e}")
            raise
        """Get primary responsibility for:
