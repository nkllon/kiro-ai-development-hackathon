"""
Task Detector Core Core Core

This module was extracted from task_detector_core_core.py
as part of RM-DDD compliance refactoring.
"""

"""
Task_Detector - Consolidated Interface Definition
...
authoritative source for task_detector.

Consolidated from: /Users/lou/kiro-2/kiro-ai-development-hackathon/src/beast_mode/dag_orchestration/analysis/task_detector_core_core_core.py
Consolidation date: 2025-09-13T10:15:07.491393
"""
...
class TaskDetector:
    ...
    def detect_tasks_from_specs(self, parsed_specs: List[ParsedSpec]) -> TaskDetectionResult:
        ...
        circular_dependencies = self._detect_circular_dependencies(all_tasks, all_dependencies)
    ...
    def _extract_tasks_from_spec(self, spec: ParsedSpec) -> List[TaskNode]:
        ...
        for i, line in enumerate(lines):
            ...
            if task_info:
                task_counter += 1
                task_id, task_name, status, description, requirements, dependencies = task_info
                if not task_id:
                ...
                task = TaskNode(task_id=task_id, spec_name=spec.spec_name, task_name=task_name, description=description or task_name, estimated_effort=estimated_effort, completion_status=status, dependencies=dependencies, dependents=[], requirements_traced=requirements)
                tasks.append(task)
        return tasks
    ...
    def _extract_task_details(self, line_index: int, lines: List[str]) -> Tuple[str, List[str], List[str]]:
        ...
        for i in range(line_index + 1, min(line_index + 10, len(lines))):
            ...
            if line.startswith('- [') or line.startswith('#') or re.match('^\\d+\\.', line):
                break
            if lines[i].startswith('  ') or lines[i].startswith('\t') or line.startswith('-'):
                ...
                for keyword in self.dependency_keywords:
                    if keyword.lower() in line.lower():
                        dep_match = re.search(f'{keyword}\\s*:?\\s*([^,\\n]+)', line, re.IGNORECASE)
                        if dep_match:
                            dependencies.append(dep_match.group(1).strip())
                        break
                if not any((keyword in line.lower() for keyword in ['requirements', 'depends', 'requires'])):
    ...
    def _estimate_effort(self, task_name: str, description: str) -> int:
        ...
        effort_keywords = {'implement': 1.5, 'create': 1.3, 'build': 1.8, 'design': 1.2, 'integrate': 2.0, 'optimize': 1.7, 'test': 1.1, 'framework': 2.2, 'system': 1.8, 'engine': 2.5, 'comprehensive': 1.5, 'complete': 1.3, 'advanced': 1.8, 'complex': 2.0, 'setup': 0.8, 'configure': 0.9, 'update': 0.7, 'fix': 0.6}
        multiplier = 1.0
    ...
    def _resolve_task_dependencies(self, task: TaskNode, task_lookup: Dict[str, TaskNode], name_lookup: Dict[str, str]) -> List[DependencyEdge]:
        ...
        for dep_name in task.dependencies:
            ...
            if dep_name in task_lookup:
                dependencies.append(DependencyEdge(source_id=dep_name, target_id=task.task_id, dependency_type='requires'))
                continue
            if dep_name_lower in name_lookup:
                dep_task_id = name_lookup[dep_name_lower]
                dependencies.append(DependencyEdge(source_id=dep_task_id, target_id=task.task_id, dependency_type='requires'))
                continue
            for task_name, task_id in name_lookup.items():
                if dep_name_lower in task_name or task_name in dep_name_lower or self._similarity_score(dep_name_lower, task_name) > 0.8:
                    dependencies.append(DependencyEdge(source_id=task_id, target_id=task.task_id, dependency_type='requires'))
                    break
        return dependencies
