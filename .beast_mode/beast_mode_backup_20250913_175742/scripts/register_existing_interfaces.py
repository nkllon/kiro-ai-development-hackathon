...
class InterfaceScanner:
    ...
    def scan_codebase(self, root_path: str = "src") -> Dict[str, Any]:
        ...
        print(f"🔍 Scanning codebase at {root_path} for ReflectiveModule implementations...")
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.ClassDef):
                    ...
                    if self.is_reflective_module_class(node):
                        interface = self.extract_interface_metadata(node, file_path, content)
                        if interface:
    ...
    def extract_interface_metadata(self, class_node: ast.ClassDef, file_path: str, content: str) -> Optional[InterfaceMetadata]:
        ...
        try:
            ...
            print(f"⚠️ Error extracting metadata from {class_node.name} in {file_path}: {e}")
            return None
    ...
    def determine_interface_type(self, class_name: str, file_path: str) -> InterfaceType:
