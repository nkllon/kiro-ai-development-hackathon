    ...
    if entities_core_core_core_file.exists():
        with open(entities_core_core_core_file, 'r') as f:
            ...
            if 'class AggregateRoot(Entity[TAggregateId], ABC):' in content:
                print("  ❌ Found problematic class definition: AggregateRoot(Entity[TAggregateId])")
                print("  💡 Issue: Entity is not defined but AggregateRoot tries to inherit from it")
