...
def main():
    ...
    if files_without_docs > 10:
        print(
            f"❌ Documentation check failed: {files_without_docs} files missing documentation"
        )
        sys.exit(1)
    else:
        print(
            f"✅ Documentation check passed: {files_without_docs} files missing documentation"
        )
        sys.exit(0)
