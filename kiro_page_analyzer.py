...
def analyze_kiro_page():
    ...
    for keyword in edit_keywords:
        ...
        elements = soup.find_all(['button', 'a', 'input'], string=re.compile(keyword, re.I))
        edit_buttons.extend(elements)
        
        # Also look for elements with edit-related classes or IDs
        elements = soup.find_all(['button', 'a', 'input'], class_=re.compile(keyword, re.I))
        edit_buttons.extend(elements)
    ...
    if edit_buttons:
        ...
        for i, btn in enumerate(edit_buttons, 1):
            btn_text = btn.get_text(strip=True) or btn.get('value', '') or btn.get('title', '')
            btn_class = btn.get('class', [])
            btn_id = btn.get('id', '')
            btn_href = btn.get('href', '')
            print(f"   {i}. Text: '{btn_text}' | Class: {btn_class} | ID: {btn_id} | Href: {btn_href}")
    else:
    ...
    title_elements = soup.find_all(['h1', 'h2', 'h3'], string=re.compile('project', re.I))
    if title_elements:
