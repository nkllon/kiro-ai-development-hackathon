...
def main():
    ...
    try:
        ...
        for result in assessment.validation_results:
            ...
            print(f"   {status_icon} {result.component.value.title()}: {result.status} ({result.systematic_compliance_score:.2f})")
            
            if result.issues:
                print(f"      Issues: {len(result.issues)}")
                for issue in result.issues[:2]:  # Show top 2 issues
                    severity_icon = "🚨" if issue.severity.value == "critical" else "⚠️" if issue.severity.value == "high" else "ℹ️"
                    print(f"        {severity_icon} {issue.issue_type}: {issue.description}")
        ...
        if assessment.beast_mode_score >= 9.0:
            beast_assessment = "🏆 INFRASTRUCTURE EXCELLENCE: Systematic foundation achieved!"
        elif assessment.beast_mode_score >= 7.0:
            beast_assessment = "🥇 INFRASTRUCTURE PROFICIENCY: Strong systematic foundation!"
            ...
            beast_assessment = "🥉 INFRASTRUCTURE CRITICAL: Immediate systematic intervention required"
        ...
        logging_result = next((r for r in assessment.validation_results if r.component.value == "logging"), None)
        if logging_result:
            if logging_result.status == "PASS":
                print("   ✅ Logging infrastructure: SYSTEMATIC COMPLIANCE VERIFIED")
            else:
                print("   🚨 Logging infrastructure: SYSTEMATIC ISSUES DETECTED (Priority 1)")
        
        # Check if profiling was validated second
        profiling_result = next((r for r in assessment.validation_results if r.component.value == "profiling"), None)
        if profiling_result:
            if profiling_result.status == "PASS":
                print("   ✅ Profiling infrastructure: SYSTEMATIC COMPLIANCE VERIFIED")
            else:
                print("   🚨 Profiling infrastructure: SYSTEMATIC ISSUES DETECTED (Priority 2)")
        ...
        if assessment.critical_issues == 0 and assessment.high_priority_issues == 0:
            ...
            print("   ⚠️ Infrastructure requires systematic remediation before full operations")
        
        # Save assessment results
        results_file = Path("logs/infrastructure") / f"assessment_{assessment.assessment_id}.json"
        ...
        if assessment.critical_issues > 0:
            ...
            print("⚠️ HIGH PRIORITY ISSUES DETECTED - Systematic improvements recommended")
            sys.exit(0)
        else:
