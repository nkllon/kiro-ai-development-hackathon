...
def send_cdp_command(ws, method, params=None):
    ...
    command = {"id": int(time.time() * 1000), "method": method, "params": params or {}}
...
def main():
    ...
    try:
        ...
        title_result = send_cdp_command(
            ws, "Runtime.evaluate", {"expression": "document.title"}
        )
        url_result = send_cdp_command(
            ws, "Runtime.evaluate", {"expression": "window.location.href"}
        )
        ...
        # Count buttons
        buttons_result = send_cdp_command(
            ws,
            "Runtime.evaluate",
            {
        ...
        # Count inputs
        inputs_result = send_cdp_command(
            ws,
            "Runtime.evaluate",
            {
        ...
        # Count images
        images_result = send_cdp_command(
            ws,
            "Runtime.evaluate",
            ...
            for form in forms_data:
                ...
                if form["fieldCount"] > 0:
                    ...
                    for field in fields_data:
                        print(
                            f"      Field {field['index']}: {field['label']} ({field['type']})"
                        )
                        if field["name"]:
        ...
        nav_buttons_info = send_cdp_command(
            ws,
            "Runtime.evaluate",
            {
                "expression": """
        ...
        print(f"\n📸 Taking screenshot...")
        try:
            screenshot_result = send_cdp_command(
        ...
        while True:
            try:
                ...
                if command == "quit":
                    ...
                    button_count = buttons_result.get("result", {}).get("value", 0)
                    print(
                        f"📊 Current state: {form_count} forms, {button_count} buttons"
                    )
                    ...
                    try:
                        ...
                        if (
                            "result" in screenshot_result
                            and "data" in screenshot_result["result"]
                        ):
                            screenshot_data = screenshot_result["result"]["data"]
                            timestamp = int(time.time())
                    ...
                    if nav_buttons_data:
                        ...
                        try:
                            ...
                            if 0 <= choice < len(nav_buttons_data):
                                print(
                                    f"🔄 Clicking: {nav_buttons_data[choice]['text']}"
                                )

                                # Click the button
                                click_result = send_cdp_command(
                                    ws,
                                    "Runtime.evaluate",
                                    {
                                        "expression": f"""
                                ...
                                form_count = forms_result.get("result", {}).get(
                                    "value", 0
                                )
                                button_count = buttons_result.get("result", {}).get(
                                    "value", 0
                                )
                                ...
                                new_url = url_result.get("result", {}).get(
                                    "value", "Unknown"
                                )
                                new_title = title_result.get("result", {}).get(
                                    "value", "Unknown"
                                )

                                print(
                                    f"📊 New state: {form_count} forms, {button_count} buttons"
                                )
