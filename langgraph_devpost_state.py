...
def create_initial_state(
    ...
    return DevPostState(
        ...
        # Messages for LangGraph communication
        messages=[HumanMessage(content=f"Starting DevPost automation workflow {workflow_id}")],
...
def get_state_summary(state: DevPostState) -> Dict[str, Any]:
    ...
    return {
        ...
        "form_completion_status": {k: v.value for k, v in state["form_completion_status"].items()},
        "errors": state["errors"],
        ...
        ).total_seconds() if state["workflow_start_time"] else None,
    }
    ...
    try:
        ...
        for key, value in data.items():
            if key == "current_phase" and isinstance(value, str):
                ...
                state[key] = {
                    k: FormCompletionStatus(v) for k, v in value.items()
                }
...
def add_error(state: DevPostState, error: str) -> DevPostState:
    ...
    state["messages"].append(
        AIMessage(content=f"Error encountered: {error}")
    )
    return state
...
def update_performance_metrics(state: DevPostState, metrics: Dict[str, Any]) -> DevPostState:
