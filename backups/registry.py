"""
Global registry for RM-DDD component discovery and management.

This module provides a centralized registry for all RM components,
enabling systematic component discovery, health monitoring, and
dependency resolution across the entire framework.
"""
...
class GlobalRegistry:
    ...
    def __init__(self):
        ...
        self._capabilities: Dict[str, List[str]] = {}  # capability_name -> [module_ids]
        self._lock = Lock()
        self._health_check_task: Optional[asyncio.Task] = None
        self._health_check_interval = timedelta(seconds=60)  # Default 1 minute
    ...
    def register_module(self, module: "ReflectiveModuleBase", module_id: str):
        ...
        with self._lock:
            if module_id in self._modules:
                logger.warning(
                    f"Module {module_id} already registered, updating registration"
                )

            registered_module = RegisteredModule(
                module_id=module_id, module=module, registration_time=datetime.now()
            )

            self._modules[module_id] = registered_module

            # Start health monitoring if this is the first module
    ...
    def unregister_module(self, module_id: str):
        ...
        with self._lock:
            ...
            # Stop health monitoring if no modules remain
            if not self._modules and self._health_check_task:
                self._stop_health_monitoring()

    async def update_module_capabilities(
        self, module_id: str, capabilities: List[ModuleCapability]
    ):
        """
        ...
        with self._lock:
            if module_id not in self._modules:
                logger.warning(
                    f"Attempted to update capabilities for unknown module: {module_id}"
                )
                return
            ...
            logger.debug(
                f"Updated capabilities for module {module_id}: {[c.name for c in capabilities]}"
            )
    ...
    async def update_module_health(self, module_id: str, health_status: "ModuleHealth"):
        ...
        with self._lock:
            if module_id not in self._modules:
                logger.warning(
                    f"Attempted to update health for unknown module: {module_id}"
                )
                return
            ...
            if health_status.capabilities != registered_module.capabilities:
                await self.update_module_capabilities(
                    module_id, health_status.capabilities
                )
    ...
    async def discover_service(
        self, capability_name: str, prefer_healthy: bool = True
        ...
        # Return any available module if no healthy ones found
    ...
    def add_dependency(self, dependent_id: str, dependency_id: str):
        ...
        with self._lock:
            if dependent_id not in self._modules or dependency_id not in self._modules:
                logger.warning(
                    f"Cannot add dependency: one or both modules not registered"
                )
                return
    ...
    def get_dependency_graph(self) -> Dict[str, Dict[str, Any]]:
        ...
        with self._lock:
            ...
            for module_id, registered_module in self._modules.items():
                graph[module_id] = {
                    "dependencies": list(registered_module.dependencies),
                    "dependents": list(registered_module.dependents),
                    "is_healthy": registered_module.is_healthy,
                    "capabilities": [c.name for c in registered_module.capabilities],
                }
    ...
    async def _perform_health_checks(self):
        ...
        for registered_module in modules_to_check:
            try:
                ...
                await self.update_module_health(
                    registered_module.module_id, health_status
                )
            except Exception as e:
                logger.error(
                    f"Health check failed for module {registered_module.module_id}: {e}"
                )
...
def reset_global_registry():
    ...
    with _registry_lock:
        if _global_registry:
            # Note: In a real implementation, we'd want to properly shutdown
            # the existing registry, but for simplicity we'll just replace it
            pass
        _global_registry = GlobalRegistry()
