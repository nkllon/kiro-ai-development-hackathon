...
This module was extracted from controller_core.py
...
def _calculate_optimal_instances(
    ...
    optimal = min(
        max_parallel, self.config.instance_count, self.config.max_instances, len(tasks)
    )
...
def _create_distribution_plan(
    ...
    avg_task_duration = (
        sum((task.estimated_duration.total_seconds() for task in tasks)) / len(tasks)
        if tasks
        else 0
    )
...
def _update_swarm_metrics(self, swarm: SwarmState) -> None:
    ...
    failed_count = sum(
        (1 for status in swarm.execution_status.values() if status == TaskStatus.FAILED)
    )
