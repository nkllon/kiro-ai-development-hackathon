"""
Dependency Manager Services Core

This module was extracted from dependency_manager_services.py
...
def dfs_longest(
    ...
    for dependent in graph.get_dependents(node):
        if dependent in valid_nodes and dependent not in visited:
            dep_duration = self._estimate_dependency_duration(node, dependent)
            dfs_longest(
                dependent, current_path + [dependent], current_duration + dep_duration
            )
    visited.remove(node)
    return (path, duration)
