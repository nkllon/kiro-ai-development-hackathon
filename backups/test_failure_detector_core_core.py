"""
Test Failure Detector Core Core

This module was extracted from test_failure_detector_core.py
...
class TestFailureDetector(ReflectiveModule):
    ...
    def __init__(self, error_handler: Optional[RCAErrorHandler] = None):
        ...
        self.pytest_output_patterns = {
            "failure_header": "^FAILURES\\s*$",
            "test_failure_start": "^_{20,}\\s+(.+?)\\s+_{20,}$",
            "test_node_id": "^(.+?)::\\s*(.+?)(?:\\s+FAILED)?$",
            "error_line": "^E\\s+(.+)$",
            "traceback_line": "^>\\s+(.+)$",
            "assertion_error": "^>?\\s*assert\\s+(.+)$",
            "import_error": "ImportError:\\s*(.+)$",
            "file_not_found": "FileNotFoundError:\\s*(.+)$",
        }
    ...
    def monitor_test_execution(
        ...
        try:
            ...
            with self.error_handler.handle_rca_operation(
                "monitor_test_execution", "test_failure_detector"
            ):
                json_output_file = (
                    f"/tmp/pytest_output_{int(datetime.now().timestamp())}.json"
                )
            cmd_parts = test_command.split()
            if "--json-report" not in cmd_parts:
                cmd_parts.extend(
                    ["--json-report", f"--json-report-file={json_output_file}"]
                )
            result = subprocess.run(
                cmd_parts, cwd=working_dir, capture_output=True, text=True, timeout=300
            )
            failures = []
            if os.path.exists(json_output_file):
                try:
                    ...
                    self.logger.info(
                        f"Parsed {len(failures)} failures from JSON output"
                    )
                    self.error_handler.monitor_component_health(
                        "json_parser", True, 100.0
                    )
                except Exception as e:
                    self.logger.warning(
                        f"JSON parsing failed: {e}, falling back to text parsing"
                    )
                    self.error_handler.monitor_component_health(
                        "json_parser", False, 1000.0
                    )
                finally:
                    try:
            ...
            if not failures and result.returncode != 0:
                try:
                    ...
                    self.logger.info(
                        f"Parsed {len(failures)} failures from text output"
                    )
                    self.error_handler.monitor_component_health(
                        "text_parser", True, 200.0
                    )
                except Exception as e:
                    self.logger.error(f"Text parsing also failed: {e}")
                    self.error_handler.monitor_component_health(
                        "text_parser", False, 2000.0
                    )
        ...
        try:
            ...
            for i, line in enumerate(lines):
                ...
                failure_match = re.match(
                    self.pytest_output_patterns["test_failure_start"], line
                )
                if failure_match:
                    if current_failure:
    ...
    def create_failure_object(
        self, test_name: str, error_info: dict
    ) -> TestFailureData:
        """
    ...
    def _create_failure_data(
        self, test_name: str, traceback: List[str], error_lines: List[str]
    ) -> Optional[TestFailureData]:
    ...
    def _create_monitoring_failure(
        self, test_command: str, error: str
    ) -> TestFailureData:
