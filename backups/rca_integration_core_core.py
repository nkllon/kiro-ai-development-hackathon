...
This module was extracted from rca_integration_core.py
...
def get_module_status(self) -> Dict[str, Any]:
    ...
    return {
        ...
        "rca_engine_status": (
            self.rca_engine.get_module_status() if self.rca_engine else "unavailable"
        ),
...
def get_health_indicators(self) -> Dict[str, Any]:
    ...
    return {
        ...
        "performance": {
            "status": (
                "healthy"
                if self.total_analysis_time / max(1, self.successful_rca_analyses) < 30
                else "degraded"
            ),
            "average_analysis_time": self.total_analysis_time
            / max(1, self.successful_rca_analyses),
...
def prioritize_failures(self, failures: List[TestFailureData]) -> List[TestFailureData]:
    ...
    try:
        ...
        priority_info = [
            (f.test_name, self._get_failure_priority(f).value, scored_failures[i][1])
...
def detect_failure_correlations(
    ...
    try:
        ...
        correlations["dependency_correlations"] = self._analyze_dependency_correlations(
            failures
        )
...
def _apply_critical_priority_boosting(
    ...
    for failure in prioritized_failures:
        is_critical = any(
            (pattern in failure.error_message.lower() for pattern in critical_patterns)
        )
        if is_critical:
