"""
Parallel Optimizer Core Core

This module was extracted from parallel_optimizer_core.py
...
class ParallelOptimizer:
    """
    Optimizes DAG execution for maximum parallelism within constraints.

    Systematic approach to identifying and coordinating parallel execution
    ...
    def optimize_execution(
        ...
        Returns:
            OptimizedExecution with parallel groups and phases
        """
        opportunities = self._identify_parallel_opportunities(constraint_graph)
        parallel_groups = self._create_parallel_groups(opportunities)
        phases = self._generate_execution_phases(parallel_groups, constraint_graph)
        timeline = self._calculate_optimized_timeline(phases)
        max_parallelism = self._calculate_maximum_parallelism(parallel_groups)
    ...
    def _identify_parallel_opportunities(
        ...
        opportunities.extend(
            self._identify_resource_based_opportunities(constraint_graph)
        )
        opportunities.extend(self._identify_skill_based_opportunities(constraint_graph))
        return opportunities

    def _identify_layer_based_opportunities(
        self, constraint_graph: ConstraintGraph
    ) -> List[ParallelOpportunity]:
    ...
    def _identify_resource_based_opportunities(
        self, constraint_graph: ConstraintGraph
    ) -> List[ParallelOpportunity]:
        ...
        for resource_key, tasks in resource_groups.items():
            if len(tasks) >= 2:
                opportunity = self._create_parallel_opportunity(
                    f"resource_based_{resource_key}", tasks
                )
                opportunities.append(opportunity)
        return opportunities

    def _identify_skill_based_opportunities(
        self, constraint_graph: ConstraintGraph
    ) -> List[ParallelOpportunity]:
        ...
        for skill, tasks in skill_groups.items():
            if len(tasks) >= 2:
                opportunity = self._create_parallel_opportunity(
                    f"skill_based_{skill}", tasks
                )
                opportunities.append(opportunity)
        return opportunities

    def _create_parallel_opportunity(
        self, opportunity_id: str, tasks: List[TaskNode]
    ) -> ParallelOpportunity:
        ...
        estimated_savings = total_duration - max_duration
        resource_requirements = self._aggregate_resource_requirements(tasks)
    ...
    def _create_parallel_groups(
        self, opportunities: List[ParallelOpportunity]
    ) -> List[ParallelGroup]:
        ...
        for opportunity in opportunities:
            if self._is_viable_opportunity(opportunity):
                group = ParallelGroup(
                    ...
                    resource_allocation=self._allocate_resources(opportunity),
    ...
    def _generate_execution_phases(
        self, parallel_groups: List[ParallelGroup], constraint_graph: ConstraintGraph
    ) -> List[ExecutionPhase]:
        """Generate execution phases from parallel groups."""
        phases = []
        sorted_groups = self._sort_groups_by_dependencies(
            parallel_groups, constraint_graph
        )
        for i, group in enumerate(sorted_groups):
            phase = ExecutionPhase(
                ...
                resource_requirements=group.resource_allocation.total_requirements,
            ...
            skill_requirements=list(
                set(
                    (
                        skill
                        for task in tasks
                        for skill in getattr(task, "skill_requirements", [])
                    )
                )
            ),
        )
    ...
    def _is_viable_opportunity(self, opportunity: ParallelOpportunity) -> bool:
        ...
        return (
            opportunity.estimated_savings > opportunity.coordination_overhead
            and opportunity.risk_level != "high"
            and (len(opportunity.tasks) >= 2)
        )

    def _allocate_resources(
        self, opportunity: ParallelOpportunity
    ) -> ResourceAllocation:
        """Allocate resources for opportunity."""
        return ResourceAllocation(
            total_requirements=opportunity.resource_requirements,
            team_assignments=[
                TeamAssignment(
                    team_id=f"team_{i + 1}",
                    assigned_tasks=[task],
                ...
                for i, task in enumerate(opportunity.tasks)
            ],
        )

    def _sort_groups_by_dependencies(
        self, groups: List[ParallelGroup], constraint_graph: ConstraintGraph
    ) -> List[ParallelGroup]:
        ...
        return sorted(
            ...
            key=lambda g: min(
                (
                    constraint_graph.dependency_layers.get(task.task_id, 0)
                    for task in g.tasks
                )
            ),
        )
    ...
    def _define_success_criteria(self, group: ParallelGroup) -> List[str]:
        ...
        return [
            f"All {len(group.tasks)} tasks completed successfully",
            "No resource conflicts detected",
            "Coordination overhead within expected bounds",
        ]
    ...
    def _identify_bottlenecks(self, phases: List[ExecutionPhase]) -> List[str]:
        ...
        max_memory = max((phase.resource_requirements.memory_gb for phase in phases))
        if max_cpu > 16:
        ...
        high_demand_skills = [
            skill for skill, count in skill_counts.items() if count > len(phases) * 0.7
        ]
...
def __init__(
    self, optimization_strategy: OptimizationStrategy = OptimizationStrategy.BALANCED
):
    """Initialize optimizer with strategy."""
    self.optimization_strategy = optimization_strategy
    self.parallel_opportunities: List[ParallelOpportunity] = []
...
def _get_resource_key(self, resource_requirements: ResourceRequirements) -> str:
    """Generate a key for resource requirements."""
    return (
        f"cpu_{resource_requirements.cpu_cores}_mem_{resource_requirements.memory_gb}"
    )
...
def _allocate_resources(self, opportunity: ParallelOpportunity) -> ResourceAllocation:
    ...
    return ResourceAllocation(
        ...
        team_assignments=[
            TeamAssignment(
                team_id=f"team_{i + 1}", assigned_tasks=[task], skill_match_score=0.8
            )
            for i, task in enumerate(opportunity.tasks)
        ],
    )
