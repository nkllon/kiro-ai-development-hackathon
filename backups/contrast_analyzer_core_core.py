"""
Contrast Analyzer Core Core

This module was extracted from contrast_analyzer_core.py
...
class ContrastAnalyzer(BaseQualityAnalyzer):
    ...
    def _analyze_text_contrast(
        self, image: Image.Image, text_region: Dict[str, Any]
    ) -> None:
        """
        ...
        is_large_text = text_region["estimated_size"] >= self.large_text_size or (
            text_region["is_bold"]
            and text_region["estimated_size"] >= self.bold_large_text_size
        )
        required_ratio = (
            self.large_text_threshold if is_large_text else self.normal_text_threshold
        )
        if contrast_ratio < required_ratio:
            severity = (
                Severity.ERROR
                if contrast_ratio < required_ratio * 0.8
    ...
    def _analyze_graphical_contrast(
        self, image: Image.Image, element: Dict[str, Any]
    ) -> None:
        """
        ...
        if contrast_ratio < self.graphical_threshold:
            severity = (
                Severity.WARNING
                if contrast_ratio > self.graphical_threshold * 0.8
                else Severity.ERROR
            )
            self.add_violation(
    ...
    def _looks_like_graphical_element(self, region: np.ndarray) -> bool:
        ...
        gray_region = np.mean(region, axis=2) if len(region.shape) == 3 else region
        edges = (
            np.abs(np.diff(gray_region, axis=0)).sum()
            + np.abs(np.diff(gray_region, axis=1)).sum()
        )
        edge_density = edges / region.size
        return 5 < edge_density < 50

    def _extract_text_background_colors(
        self, image: Image.Image, bbox: BoundingBox
        ...
        if len(unique_colors) >= 2:
            ...
            text_color = (
                tuple(unique_colors[0]) if len(unique_colors) > 0 else (0, 0, 0)
            )
            bg_color = (255, 255, 255)
        return (text_color, bg_color)

    def _extract_element_background_colors(
        self, image: Image.Image, bbox: BoundingBox
    ) -> Tuple[Tuple[int, int, int], Tuple[int, int, int]]:
        """
...
def _analyze_text_contrast(
    ...
    if contrast_ratio < required_ratio:
        ...
        self.add_violation(
            rule_id=(
                "wcag_contrast_normal" if not is_large_text else "wcag_contrast_large"
            ),
            severity=severity,
            current_value=contrast_ratio,
            expected_value=required_ratio,
