...
class SystemMonitor:
    ...
    async def get_comprehensive_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "alerts": {
                ...
                "recent": [
                    alert.__dict__ for alert in self.alert_manager.get_alert_history(24)
                ],
            },
            "recovery": {
                ...
                "active": [
                    attempt.__dict__
                    for attempt in self.recovery_manager.get_active_recoveries()
                ],
                "recent": [
                    attempt.__dict__
                    for attempt in self.recovery_manager.get_recovery_history(24)
                ],
            },
    ...
    def record_message_received(
        ...
        if processing_time_ms is not None:
            self.metrics_collector.record_timer(
                "message_processing_time", processing_time_ms
            )
    ...
    def record_agent_connection(self, agent_id: str, connected: bool) -> None:
        ...
        current_connections = (
            self.metrics_collector.get_gauge_value("active_connections") or 0
        )
        if connected:
    ...
    async def _handle_alert(self, alert: Alert) -> None:
        ...
        if alert.severity in [AlertSeverity.CRITICAL, AlertSeverity.HIGH]:
            ...
            if recovery_action:
                self.logger.info(
                    f"Triggering recovery {recovery_action} for alert {alert.name}"
                )
    ...
    async def _handle_recovery_event(self, recovery_attempt) -> None:
        ...
        if recovery_attempt.result:
            self.metrics_collector.increment_counter(
                "recovery_attempts",
                labels={
                    "action": recovery_attempt.action_name,
                    "result": recovery_attempt.result.value,
                },
            )
    ...
    async def _check_component_health_alert(self, rule) -> Dict[str, Any]:
        ...
        return {
            ...
            "message": (
                f"{unhealthy_count} components are unhealthy"
    ...
    async def _check_recovery_failure_alert(self, rule) -> Dict[str, Any]:
        ...
        return {
            ...
            "message": (
                f"{len(failed_attempts)} recovery failures in the last hour"
                if should_alert
                else "Recovery operations normal"
            ),
