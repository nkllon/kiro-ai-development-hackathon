"""
Shared kernel management utilities for RM-DDD framework.

This module provides tools for managing shared kernel elements between
bounded contexts, including common domain elements, versioning, and
governance of shared concepts.
"""
...
class DefaultSharedKernelGovernance(SharedKernelGovernance):
    ...
    def can_modify_element(self, element: SharedElement, modifier_context: str) -> bool:
        ...
        # Owner context can always modify
        if element.owner_context == modifier_context:
            return True

        # Consumer contexts can only suggest changes, not modify directly
        return False
    ...
    def requires_approval(self, change: SharedKernelChange) -> bool:
        ...
        # Changes affecting multiple contexts require approval
    ...
    def get_required_approvers(self, change: SharedKernelChange) -> List[str]:
        ...
        # All affected contexts must approve breaking changes
        if change.impact_level == ChangeImpact.BREAKING:
            approvers.extend(change.affected_contexts)

        # Owner context must approve all changes
        # (This would need to be determined from the element)
...
class SharedKernel(DomainReflectiveModule):
    ...
    def __init__(
        self,
        kernel_name: str,
        participating_contexts: List[str],
        governance: Optional[SharedKernelGovernance] = None,
    ):
    ...
    def add_element(
        self,
        name: str,
        element_type: SharedElementType,
        owner_context: str,
        description: str = "",
        ...
        """
        # Validate owner context is participating
        if owner_context not in self.participating_contexts:
            raise DomainException(
                f"Owner context {owner_context} is not participating in shared kernel {self.kernel_name}",
                error_code="INVALID_OWNER_CONTEXT",
            )
        ...
        if not validation_result.is_valid:
            raise DomainException(
                f"Invalid shared element: {validation_result.errors}",
                error_code="INVALID_SHARED_ELEMENT",
            )
    ...
    def add_consumer(self, element_id: UUID, consumer_context: str):
        ...
        if element_id not in self._elements:
            raise DomainException(
                f"Shared element {element_id} not found", error_code="ELEMENT_NOT_FOUND"
            )

        if consumer_context not in self.participating_contexts:
            raise DomainException(
                f"Consumer context {consumer_context} is not participating in shared kernel",
                error_code="INVALID_CONSUMER_CONTEXT",
            )
        ...
        self._usage_metrics[consumer_context] = (
            self._usage_metrics.get(consumer_context, 0) + 1
        )
    ...
    def propose_change(
        ...
        change = SharedKernelChange(
            element_id=element_id,
            change_type=change_type,
            impact_level=impact_level,
            description=description,
            rationale=rationale,
            affected_contexts=element.consumer_contexts.copy(),
        ...
        if not validation_result.is_valid:
            raise DomainException(
                f"Invalid change proposal: {validation_result.errors}",
                error_code="INVALID_CHANGE_PROPOSAL",
            )

        # Check if proposer can modify
        if not self.governance.can_modify_element(element, proposed_by):
            # Non-owners can propose but not directly implement
            logger.info(
                f"Change proposed by non-owner {proposed_by} for element {element.name}"
            )
    ...
    def approve_change(self, change_id: UUID, approver: str):
        ...
        if change_id not in self._changes:
            raise DomainException(
                f"Change {change_id} not found", error_code="CHANGE_NOT_FOUND"
            )
        ...
        if approver not in required_approvers:
            raise DomainException(
                f"Context {approver} is not authorized to approve this change",
    ...
    def implement_change(self, change_id: UUID) -> bool:
        ...
        if self.governance.requires_approval(change) and not change.approved_by:
            raise DomainException(
                f"Change {change_id} requires approval before implementation",
                error_code="CHANGE_NOT_APPROVED",
            )

        # Implement the change (this would involve actual element modification)
    ...
    def get_element_usage(self, element_id: UUID) -> Dict[str, Any]:
        ...
        return {
            ...
            "owner_context": element.owner_context,
            "consumer_contexts": list(element.consumer_contexts),
            "consumer_count": len(element.consumer_contexts),
            "version": element.version,
            "version_history": self._version_history.get(element_id, []),
    ...
    def analyze_change_impact(self, change_id: UUID) -> Dict[str, Any]:
        ...
        return {
            ...
            "affected_contexts": list(change.affected_contexts),
            "requires_approval": self.governance.requires_approval(change),
            "required_approvers": self.governance.get_required_approvers(change),
            "migration_required": bool(change.migration_guide),
            "consumer_count": len(element.consumer_contexts),
            "version_impact": self._predict_version_impact(change.impact_level),
        }
    ...
    def get_kernel_health(self) -> Dict[str, Any]:
        ...
        pending_changes = len(
            [c for c in self._changes.values() if not c.implemented_at]
        )
        ...
        return {
            ...
            "participating_contexts": list(self.participating_contexts),
            "total_elements": total_elements,
            "total_changes": total_changes,
            "pending_changes": pending_changes,
            "context_usage": context_usage,
            "governance_type": self.governance.__class__.__name__,
        }
    ...
    async def get_module_status(self):
        ...
        return ModuleHealth(
            status=status,
            message=f"Shared kernel: {self.kernel_name}",
    ...
    async def get_module_capabilities(self):
        ...
        return [
            ModuleCapability(
                name=f"shared_kernel_{self.kernel_name}",
                description=f"Shared kernel management for {self.kernel_name}",
                available=True,
                version="1.0.0",
            )
        ]
    ...
    async def get_health_indicators(self):
        ...
        return {
            "kernel_health": self.get_kernel_health(),
    ...
    def get_domain_boundaries(self):
        ...
        return DomainBoundaries(
            ...
            invariants=[
                "Shared elements must have clear ownership",
    ...
    def validate_domain_invariants(self):
        ...
        for element in self._elements.values():
            ...
            if not element_result.is_valid:
                result.add_error(
                    f"Invalid element {element.name}: {element_result.errors}"
                )
        ...
        for change in self._changes.values():
            ...
            if not change_result.is_valid:
                result.add_error(
                    f"Invalid change {change.change_id}: {change_result.errors}"
                )
...
class SharedKernelRegistry(DomainReflectiveModule):
    ...
    def register_kernel(self, kernel: SharedKernel):
        ...
        # Update context participation tracking
    ...
    def detect_conflicts(self) -> List[Dict[str, Any]]:
        ...
        # Check for overlapping element names across kernels
        all_elements = {}
                    ...
                    conflicts.append(
                        {
                            ...
                            "kernels": [all_elements[element_key], kernel_name],
                            "severity": "high",
                        }
                    )
                else:
    ...
    def get_registry_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "kernel_names": list(self._kernels.keys()),
            "participating_contexts": list(self._context_participation.keys()),
            "context_participation": {
                context: list(kernels)
                for context, kernels in self._context_participation.items()
            },
    ...
    async def get_module_status(self):
        ...
        return ModuleHealth(
            status=status,
            message=f"Shared kernel registry with {len(self._kernels)} kernels",
            capabilities=await self.get_module_capabilities(),
            health_indicators={
                "kernels_count": len(self._kernels),
    ...
    async def get_module_capabilities(self):
        ...
        return [
            ModuleCapability(
                name="shared_kernel_registry",
                description="Manages shared kernel registration and discovery",
                available=True,
                version="1.0.0",
            )
        ]
    ...
    def get_domain_boundaries(self):
        ...
        return DomainBoundaries(
            context=self.domain_context,
            invariants=[
                "Shared kernel names must be unique",
                "Element names within type must be unique across kernels",
                "Context participation must be consistent",
            ],
        )
