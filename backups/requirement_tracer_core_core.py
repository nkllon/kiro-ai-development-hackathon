...
class RequirementTracer(ComplianceValidator):
    ...
    def __init__(self, repository_path: str):
        ...
        self.requirement_patterns = [
            "_Requirements?:\\s*([0-9]+(?:\\.[0-9]+)*(?:,\\s*[0-9]+(?:\\.[0-9]+)*)*)",
            "Requirements?:\\s*([0-9]+(?:\\.[0-9]+)*(?:,\\s*[0-9]+(?:\\.[0-9]+)*)*)",
            "Requirement\\s+([0-9]+(?:\\.[0-9]+)*)",
            "REQ-([0-9]+(?:\\.[0-9]+)*)",
            "#\\s*([0-9]+(?:\\.[0-9]+)*)",
        ]
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            if req_match:
                if current_requirement:
                    current_requirement.acceptance_criteria = (
                        current_acceptance_criteria.copy()
                    )
                    requirements[current_requirement.requirement_id] = (
                        current_requirement
                    )
                ...
                re.match("^\\d+\\.\\s+WHEN.*THEN.*SHALL", line) and current_requirement
            ):
    ...
    def _find_requirement_references(
        ...
        for pattern in search_patterns:
            if target_path.is_file():
                files_to_search = (
                    [target_path]
                    if target_path.match(pattern.replace("**/", ""))
                    else []
                )
            else:
    ...
    def _generate_traceability_issues(
        ...
        if orphaned_implementations:
            affected_files = list(
                set((ref.file_path for ref in orphaned_implementations))
            )
            orphaned_req_ids = list(
                set((ref.requirement_id for ref in orphaned_implementations))
            )
            issues.append(
                ComplianceIssue(
                    issue_type=ComplianceIssueType.REQUIREMENT_TRACEABILITY,
...
def _analyze_traceability(self, target_path: Path) -> TraceabilityResult:
    ...
    traced_requirements = len(referenced_requirements & all_requirements)
    traceability_score = (
        traced_requirements / total_requirements * 100 if total_requirements > 0 else 0
    )
    issues = self._generate_traceability_issues(
        untraced_requirements, orphaned_implementations, traceability_score
    )
...
def _find_requirement_references(self, target_path: Path) -> List[RequirementReference]:
    ...
    for pattern in search_patterns:
        if target_path.is_file():
            files_to_search = (
                [target_path] if target_path.match(pattern.replace("**/", "")) else []
            )
        else:
            files_to_search = list(target_path.glob(pattern))
        for file_path in files_to_search:
