"""
Bus Client Core Core

This module was extracted from bus_client_core.py
...
class BeastModeBusClient:
    ...
    async def _handle_agent_response(self, message: BeastModeMessage) -> None:
        ...
        if self.discovery_enabled:
            try:
                ...
                if updated_agent:
                    logger.debug(f'Updated agent {updated_agent.agent_id} from response')
                else:
                    logger.debug(f'Received response from unknown agent {message.source}, registering as discovery')
                    capabilities_data = message.payload.get('agent_capabilities', {})
                    if capabilities_data:
                        pseudo_discovery = BeastModeMessage(type=MessageType.AGENT_DISCOVERY, source=message.source, payload={'agent_capabilities': capabilities_data}, timestamp=message.timestamp)
                        discovered_agent = self.agent_registry.register_agent_discovery(pseudo_discovery)
                        logger.info(f'Registered responding agent {discovered_agent.agent_id} with capabilities: {discovered_agent.capabilities.capabilities}')
            except Exception as e:
    ...
    async def _handle_help_request(self, message: BeastModeMessage) -> None:
        ...
        if help_response:
            response_message = self.help_system.create_help_response_message(help_response)
            await self.send_message(response_message)
            logger.info(f'Offered help to {message.source} for request {help_response.request_id} (confidence: {help_response.confidence_score:.2f})')
    ...
    async def _handle_help_response(self, message: BeastModeMessage) -> None:
        ...
        if success:
            logger.info(f"Received help response from {message.source} for request {message.payload.get('request_id')}")
        else:
            logger.warning(f'Failed to process help response from {message.source}')

    async def _handle_office_hours_announcement(self, message: BeastModeMessage) -> None:
    ...
    async def _handle_collaboration_request(self, message: BeastModeMessage) -> None:
        ...
        try:
            ...
            is_available = self.collaboration_scheduler.is_agent_available(self.agent_id, requested_time)
            response_payload = {'request_id': payload.get('request_id', message.id), 'available': is_available, 'agent_capabilities': AgentCapabilities(agent_id=self.agent_id, capabilities=self.capabilities, availability='ready_for_business' if is_available else 'busy').model_dump()}
            if is_available and requested_time:
                next_slot = self.collaboration_scheduler.get_next_available_slot(self.agent_id, payload.get('duration_minutes', 30), requested_time)
                if next_slot:
                    response_payload['suggested_time'] = next_slot.isoformat()
            response = BeastModeMessage(type=MessageType.COLLABORATION_RESPONSE, source=self.agent_id, target=message.source, payload=response_payload, correlation_id=message.id, priority=3)
            await self.send_message(response)
    ...
    async def _handle_collaboration_response(self, message: BeastModeMessage) -> None:
        ...
        try:
            ...
            if payload.get('available', False):
                logger.info(f'Agent {message.source} is available for collaboration (request {request_id})')
                self.collaboration_scheduler.trigger_collaboration_callback('on_collaboration_response', message.source, payload)
            else:
                logger.info(f'Agent {message.source} is not available for collaboration (request {request_id})')
                if payload.get('suggested_time'):
                    logger.info(f"Agent {message.source} suggested alternative time: {payload['suggested_time']}")
        except Exception as e:
        ...
        try:
            ...
            if session_id:
                ...
                if success:
                    ...
                    self.collaboration_scheduler.trigger_collaboration_callback('on_collaboration_start', session_id, payload)
                else:
                    logger.warning(f'Failed to start collaboration session {session_id}')
        except Exception as e:
        ...
        try:
            ...
            if session_id:
                ...
                if ended:
                    ...
                    self.collaboration_scheduler.trigger_collaboration_callback('on_collaboration_end', session_id, success, success_metrics)
                else:
        ...
        try:
            ...
            if session_id:
                ...
                if session:
                    ...
                    self.collaboration_scheduler.trigger_collaboration_callback('on_collaboration_update', session_id, update_data)
                else:
                    logger.warning(f'Collaboration session {session_id} not found for update')
        except Exception as e:
