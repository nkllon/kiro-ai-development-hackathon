"""
Content Analyzer Core Core

This module was extracted from content_analyzer_core.py
...
class ContentAnalyzer:
        ...
        if GIT_AVAILABLE:
            try:
                self._git_repo = git.Repo(
                    self.project_path, search_parent_directories=True
                )
    ...
    def analyze_file_change(self, file_path: Path, change_type: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if change_type == "deleted":
                ...
                analysis["previous_content_hash"] = self._content_cache.get(
                    str(file_path)
                )
            else:
                ...
                if self._is_documentation_file(file_path):
                    analysis["is_significant_change"] = (
                        self._analyze_documentation_change(
    ...
    def get_project_changes_summary(self, since: datetime) -> Dict[str, Any]:
        ...
        try:
            commits = list(
                self._git_repo.iter_commits(since=since.strftime("%Y-%m-%d %H:%M:%S"))
            )
            ...
            if commits:
                ...
                for item in diff:
                    if item.a_blob and item.b_blob:
                        try:
                            summary["lines_added"] += len(
                                item.b_blob.data_stream.read()
                                .decode("utf-8", errors="ignore")
                                .splitlines()
                            )
                            summary["lines_removed"] += len(
                                item.a_blob.data_stream.read()
                                .decode("utf-8", errors="ignore")
                                .splitlines()
                            )
                        except:
    ...
    def _analyze_documentation_change(
        self, file_path: Path, previous_hash: Optional[str], current_hash: str
    ) -> bool:
        ...
        try:
            ...
            for line in content.splitlines():
                line = line.strip()
                if line and any(
                    (re.search(pattern, line) for pattern in significant_indicators)
                ):
                    significant_lines += 1
            if total_lines > 0:
    ...
    def _get_git_info(self, file_path: Path) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                commits = list(
                    self._git_repo.iter_commits(paths=str(relative_path), max_count=1)
                )
                if commits:
                ...
                pass
            try:
                git_info["is_tracked"] = str(relative_path) in [
                    item.a_path for item in self._git_repo.index.diff(None)
                ]
            except Exception:
                git_info["is_tracked"] = False
            try:
                git_info["is_staged"] = str(relative_path) in [
                    item.a_path for item in self._git_repo.index.diff("HEAD")
                ]
            except Exception:
    ...
    def _is_documentation_file(self, file_path: Path) -> bool:
        ...
        return (
            ...
            or file_path.suffix.lower() in {".md", ".txt", ".rst", ".adoc"}
    ...
    def _is_release_tag(self, tag_name: str) -> bool:
        ...
        return any(
            (re.match(pattern, tag_name.lower()) for pattern in release_patterns)
        )
