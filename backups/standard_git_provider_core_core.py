"""
Standard Git Provider Core Core

This module was extracted from standard_git_provider_core.py
...
class StandardGitProvider(GitProvider):
    ...
    def _run_git_command(
        self, args: List[str], input_data: str = None, timeout: int = 30
                ...
                data={
                    ...
                    "modified_files": len(
                        [
                            f
                            for f in files
                            if f.working_tree_status == "M"
                            or (f.status == "M" and (not f.staged))
                        ]
                    ),
        ...
        try:
            ...
            return self._create_result(
                ...
                data={
                    ...
                    "local_count": len(
                        [b for b in branches if not b.name.startswith("remotes/")]
                    ),
                    "remote_count": len(
                        [b for b in branches if b.name.startswith("remotes/")]
                    ),
                },
                execution_time_ms=execution_time,
            )
    ...
    def _parse_branch_output(self, output: str) -> List[BranchInfo]:
        ...
        for line in output.strip().split("\n"):
            ...
            ahead_behind = (
                self._get_ahead_behind_counts(branch_name)
                if is_current
    ...
    def create_branch(self, name: str, from_branch: str = "HEAD") -> GitOperationResult:
        ...
        try:
            ...
            if "already exists" in e.stderr:
                suggestions.append(
                    f"Use 'git checkout {name}' to switch to existing branch"
                )
            return self._create_result(
                success=False,
    ...
    def switch_branch(
        ...
        try:
            ...
            return self._create_result(
                ...
                data={"branch_name": name, "created": create_if_missing},
                execution_time_ms=execution_time,
            )
        except subprocess.CalledProcessError as e:
            execution_time = int((time.time() - start_time) * 1000)
    ...
    def delete_branch(self, name: str, force: bool = False) -> GitOperationResult:
        ...
        try:
            ...
            return self._create_result(
                ...
                data={"branch_name": name, "forced": force},
                execution_time_ms=execution_time,
            )
        except subprocess.CalledProcessError as e:
            execution_time = int((time.time() - start_time) * 1000)
    ...
    def rename_branch(self, old_name: str, new_name: str) -> GitOperationResult:
        ...
        try:
            try:
                self._run_git_command(
                    ["show-ref", "--verify", f"refs/heads/{old_name}"]
                )
            except subprocess.CalledProcessError:
                return self._create_result(
                    success=False,
    ...
    def set_upstream_branch(
        self, branch_name: str, upstream: str
    ) -> GitOperationResult:
        ...
        try:
            ...
            return self._create_result(
                ...
                data={"branch_name": branch_name, "upstream": upstream},
                execution_time_ms=execution_time,
            )
        except subprocess.CalledProcessError as e:
            execution_time = int((time.time() - start_time) * 1000)
    ...
    def get_commit_history(
        self, branch: str = None, limit: int = 50
    ) -> GitOperationResult:
    ...
    def push_changes(
        self, branch: str = None, remote: str = "origin"
    ) -> GitOperationResult:
...
def create_branch(self, name: str, from_branch: str = "HEAD") -> GitOperationResult:
    ...
    try:
        ...
        return self._create_result(
            success=True,
            message=f"Created and switched to branch '{name}' from '{from_branch}'",
            data={"branch_name": name, "from_branch": from_branch, "switched": True},
            execution_time_ms=execution_time,
        )
    except subprocess.CalledProcessError as e:
        execution_time = int((time.time() - start_time) * 1000)
...
def merge_branch(self, source: str, target: str = None) -> GitOperationResult:
    ...
    try:
        ...
        return self._create_result(
            success=True,
            message=f"Successfully merged '{source}' into current branch",
            data={"source_branch": source, "target_branch": target, "conflicts": False},
            execution_time_ms=execution_time,
        )
    except subprocess.CalledProcessError as e:
        execution_time = int((time.time() - start_time) * 1000)
