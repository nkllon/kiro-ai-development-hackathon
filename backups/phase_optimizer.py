...
class PhaseOptimizer:
    ...
    def _analyze_tasks_with_prejudice(self, tasks: List[TaskNode]) -> Dict[str, float]:
        ...
        for task in tasks:
            ...
            analysis[task.task_id] = {
                ...
                "combined": (value_score * priority_score)
    ...
    def _create_high_velocity_parallel_groups(
        ...
        while remaining_tasks:
            ...
            i = 0
            while (
                i < len(remaining_tasks) and len(current_group) < 4
            ):  # Max 4 tasks per group
    ...
    def _calculate_systematic_resource_requirements(
        ...
        developers_needed = min(
            max_concurrent_tasks, max(1, total_effort // 40)
        )  # At least 1 week per dev
    ...
    def _identify_required_tools(self, tasks: List[TaskNode]) -> List[str]:
        ...
        for task in tasks:
            ...
            if any(
                keyword in task_text for keyword in ["ci/cd", "pipeline", "deployment"]
            ):
    ...
    def _identify_critical_path_phases(self, phases: List[MVPPhase]) -> List[int]:
        ...
        for phase in phases:
            ...
            if (
                total_effort > 80 or total_dependencies > 10
            ):  # Thresholds for critical path
