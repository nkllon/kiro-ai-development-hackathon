"""
Complexity monitoring and management utilities for RM-DDD framework.

This module provides tools for measuring, monitoring, and managing
cognitive complexity in domain logic, helping maintain code quality
and comprehensibility as systems evolve.
"""
...
class ComplexityReport:
    ...
    def get_overall_score(self) -> float:
        ...
        weights = {
            ComplexityType.CYCLOMATIC: 0.3,
            ComplexityType.COGNITIVE: 0.4,
            ComplexityType.MAINTAINABILITY: 0.2,
            ComplexityType.LINES_OF_CODE: 0.1,
        }
        ...
        for metric_type, metric in self.metrics.items():
            ...
            # Normalize score based on threshold exceedance
    ...
    def get_critical_issues(self) -> List[ComplexityMetric]:
        ...
        return [
            ...
            if metric.severity_level == "critical"
        ]
        ...
        return {
            ...
            "metrics": {
                metric_type.value: {
                    ...
                    "exceeds_threshold": metric.exceeds_threshold,
...
class CyclomaticComplexityAnalyzer(ComplexityAnalyzer):
    ...
    def analyze(self, target: Any) -> ComplexityReport:
        ...
        try:
            ...
            metric = ComplexityMetric(
                metric_type=ComplexityType.CYCLOMATIC,
                value=complexity,
            ...
            # Add suggestions if complexity is high
    ...
    def _get_cyclomatic_suggestions(self, complexity: float) -> List[str]:
        ...
        if complexity > 20:
            suggestions.append(
                "This method is very complex - consider redesigning the algorithm"
            )
...
class CognitiveComplexityAnalyzer(ComplexityAnalyzer):
    ...
    def analyze(self, target: Any) -> ComplexityReport:
        ...
        try:
            ...
            metric = ComplexityMetric(
                metric_type=ComplexityType.COGNITIVE,
                value=complexity,
                threshold=self.threshold,
                description=f"Cognitive complexity: {complexity}",
            )
...
class ComplexityMonitor(DomainReflectiveModule):
    ...
    def _initialize_default_analyzers(self):
        ...
        self._thresholds = {
            ComplexityType.CYCLOMATIC: 10.0,
            ComplexityType.COGNITIVE: 15.0,
            ComplexityType.LINES_OF_CODE: 50.0,
            ComplexityType.METHOD_COUNT: 20.0,
            ComplexityType.PARAMETER_COUNT: 5.0,
        }

    def add_analyzer(
        self, complexity_type: ComplexityType, analyzer: ComplexityAnalyzer
    ):
    ...
    def analyze_element(
        self, target: Any, element_name: Optional[str] = None
    ) -> ComplexityReport:
        """
        ...
        combined_report = ComplexityReport(
            element_name=element_name, element_type=self._get_element_type(target)
        )
        ...
        for complexity_type, analyzer in self._analyzers.items():
            try:
                ...
                # Merge metrics from individual report
    ...
    def _add_basic_metrics(self, target: Any, report: ComplexityReport):
        ...
        try:
            ...
            if inspect.isclass(target) or inspect.isfunction(target):
                ...
                metric = ComplexityMetric(
                    metric_type=ComplexityType.LINES_OF_CODE,
                    value=float(loc),
                    threshold=self._thresholds.get(ComplexityType.LINES_OF_CODE, 50.0),
                    description=f"Lines of code: {loc}",
                )
                report.add_metric(metric)
            ...
            if inspect.isclass(target):
                ...
                metric = ComplexityMetric(
                    metric_type=ComplexityType.METHOD_COUNT,
                    value=float(method_count),
                    threshold=self._thresholds.get(ComplexityType.METHOD_COUNT, 20.0),
                    description=f"Method count: {method_count}",
                )
                report.add_metric(metric)
    ...
    def get_complexity_summary(self) -> Dict[str, Any]:
        ...
        for element_name, report in self._reports.items():
            ...
            if critical_issues:
                ...
                high_complexity_elements.append(
                    {"name": element_name, "score": overall_score}
                )

        return {
            "total_elements": len(self._reports),
            "critical_elements": critical_elements,
            "high_complexity_elements": high_complexity_elements,
            "average_score": sum(r.get_overall_score() for r in self._reports.values())
            / len(self._reports),
            "thresholds": {
                t.value: threshold for t, threshold in self._thresholds.items()
            },
        }
    ...
    async def get_module_status(self):
        ...
        status = (
            ModuleStatus.AVAILABLE if critical_count == 0 else ModuleStatus.DEGRADED
        )

        return ModuleHealth(
            status=status,
            message=f"Complexity monitor tracking {summary['total_elements']} elements",
    ...
    async def get_module_capabilities(self):
        ...
        return [
            ModuleCapability(
                name="complexity_monitoring",
                description="Monitors and analyzes code complexity",
                available=True,
                version="1.0.0",
            )
        ]
    ...
    def get_domain_boundaries(self):
        ...
        return DomainBoundaries(
            ...
            invariants=[
                "Complexity metrics must be within acceptable thresholds",
                "Critical complexity issues must be addressed",
                "Complexity trends should be monitored over time",
            ],
        )
    ...
    def validate_domain_invariants(self):
        ...
        if critical_count > 0:
            result.add_error(
                f"Found {critical_count} elements with critical complexity issues"
            )
...
# Convenience functions for complexity analysis
...
def check_complexity_thresholds(
    target: Any, cyclomatic_threshold: float = 10.0, cognitive_threshold: float = 15.0
) -> bool:
    """
