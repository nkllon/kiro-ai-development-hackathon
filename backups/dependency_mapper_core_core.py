"""
Dependency Mapper Core Core

This module was extracted from dependency_mapper_core.py
...
class DependencyMapper:
    ...
    def create_dependency_graph(
        ...
        specifications: List[SpecificationNode] = None,
    ) -> ConstraintGraph:
        """
    ...
    def resolve_dependency_conflicts(
        self, constraint_graph: ConstraintGraph, auto_resolve: bool = False
    ) -> ConstraintGraph:
        """
    ...
    def _resolve_spec_dependencies(
        self, specifications: List[SpecificationNode], tasks: List[TaskNode]
    ) -> List[DependencyEdge]:
        ...
        for spec in specifications:
            for dep_spec_name in spec.dependencies:
                dep_spec = next(
                    (s for s in specifications if s.spec_name == dep_spec_name), None
                )
                if not dep_spec:
                    continue
    ...
    def _calculate_topological_order(
        self, nodes: Dict[str, TaskNode], adjacency_list: Dict[str, List[str]]
    ) -> List[str]:
    ...
    def _calculate_dependency_layers(
        self, nodes: Dict[str, TaskNode], adjacency_list: Dict[str, List[str]]
    ) -> Dict[int, List[str]]:
        ...
        def calculate_layer(node_id: str, visited: Set[str]) -> int:
            ...
            if not dependencies:
                ...
                max_dep_layer = max(
                    (calculate_layer(dep, visited.copy()) for dep in dependencies)
                )
    ...
    def _detect_conflicting_statuses(
        self,
        nodes: Dict[str, TaskNode],
        dependencies: List[DependencyEdge],
        adjacency_list: Dict[str, List[str]],
        ...
        for node_id, task in nodes.items():
            ...
            if task.completion_status == TaskStatus.COMPLETED:
                for dep_id in dependencies_list:
                    ...
                    if dep_task and dep_task.completion_status not in [
        ...
        for layer, task_ids in layers.items():
            if len(task_ids) > 1:
                high_effort_tasks = [
                    ...
                    if nodes[task_id].estimated_effort > 20
                ]
    ...
    def _detect_timeline_conflicts(
        ...
        critical_path_lengths = {
            task_id: calculate_critical_path_length(task_id, memo) for task_id in nodes
        }
        max_length = max(critical_path_lengths.values()) if critical_path_lengths else 0
        long_path_threshold = max_length * 0.8
        long_path_tasks = [
            task_id
            for task_id, length in critical_path_lengths.items()
            if length > long_path_threshold and length > 100
        ]
        if long_path_tasks:
            conflicts.append(
                DependencyConflict(
    ...
    def _add_resolution_recommendations(
        self, constraint_graph: ConstraintGraph
    ) -> ConstraintGraph:
    ...
    def _auto_resolve_conflict(
        self, constraint_graph: ConstraintGraph, conflict: DependencyConflict
    ) -> ConstraintGraph:
        ...
        if conflict.conflict_type == "status_conflict" and conflict.severity == "low":
            for task_id in conflict.affected_tasks:
                ...
                if task and task.completion_status == TaskStatus.NOT_STARTED:
                    ...
                    all_completed = all(
                        (
                            constraint_graph.nodes[dep_id].completion_status
                            == TaskStatus.COMPLETED
                            for dep_id in deps
...
def _detect_resource_conflicts(
    ...
    for layer, task_ids in layers.items():
        if len(task_ids) > 1:
            high_effort_tasks = [
                task_id for task_id in task_ids if nodes[task_id].estimated_effort > 20
            ]
            if len(high_effort_tasks) > 3:
                conflicts.append(
                    DependencyConflict(
