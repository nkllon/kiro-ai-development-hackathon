"""
Beast Readiness Validator Core

This module was extracted from beast_readiness_validator.py
...
class BeastReadinessValidator(ReflectiveModule):
    ...
    def validate_beast_readiness(self, item: BacklogItem) -> ReadinessValidation:
        ...
        try:
            validation_id = (
                f"validation_{item.item_id}_{int(datetime.now().timestamp())}"
            )
            ...
            self.logger.error(
                f"Beast readiness validation failed for {item.item_id}: {str(e)}"
            )
            self._update_health_indicator(
                "validation_error",
    ...
    def verify_dependency_satisfaction(
    ...
    def _validate_requirements_completeness(
        ...
        if not requirements:
            ...
            for req in requirements:
                if not req.description or len(req.description.strip()) < 10:
                    missing.append(
                        f"Requirement {req.requirement_id} has insufficient description"
                    )
                    score -= 0.2
                if not req.acceptance_criteria:
                    missing.append(
                        f"Requirement {req.requirement_id} lacks acceptance criteria"
                    )
                    score -= 0.3
        score = max(0.0, score)
        ...
        if not criteria:
            ...
            for criterion in criteria:
                if not criterion.testable:
                    missing.append(
                        f"Criterion {criterion.criterion_id} is not testable"
                    )
                    score -= 0.3
                if not criterion.measurable:
                    missing.append(
                        f"Criterion {criterion.criterion_id} is not measurable"
                    )
                    score -= 0.3
                if not criterion.description or len(criterion.description.strip()) < 10:
                    missing.append(
                        f"Criterion {criterion.criterion_id} has insufficient description"
                    )
                    score -= 0.2
        score = max(0.0, score)
        passed = score >= 0.8 and (not missing)
    ...
    def _validate_ambiguity_absence(self, item: BacklogItem) -> ValidationCriterion:
        ...
        for req in item.requirements:
            ...
            for word in ambiguous_words:
                if word in desc_lower:
                    ambiguous.append(
                        f"Ambiguous word '{word}' in requirement {req.requirement_id}"
                    )
                    score -= 0.1
        for criterion in item.acceptance_criteria:
            ...
            for word in ambiguous_words:
                if word in desc_lower:
                    ambiguous.append(
                        f"Ambiguous word '{word}' in acceptance criterion {criterion.criterion_id}"
                    )
                    score -= 0.1
        score = max(0.0, score)
    ...
    def _check_dependency_satisfaction(
        self, dep_ref: DependencyReference, item: BacklogItem
    ) -> DependencyStatus:
        ...
        evidence = (
            dep_ref.description if satisfied else "No clear satisfaction criteria"
        )
    ...
    def _calculate_overall_readiness(
        self, completeness: CompletenessReport, dependencies: List[DependencyStatus]
    ) -> bool:
        ...
        if not overall_ready:
            ...
            for dep in dependencies:
                if not dep.satisfied:
                    actions.append(
                        f"Resolve dependency {dep.dependency_id}: {', '.join(dep.blocking_issues)}"
                    )
        if not actions:
