...
class ToolOrchestrator(ReflectiveModule):
    ...
    def get_module_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "systematic_compliance_rate": self.orchestration_metrics[
                "systematic_compliance_rate"
            ],
        }
    ...
    def is_healthy(self) -> bool:
        ...
        tools_healthy = all(
            status != ToolStatus.FAILED for status in self.tool_status.values()
        )
    ...
    def get_health_indicators(self) -> Dict[str, Any]:
        ...
        return {
            "tool_availability": {
                tool_id: status.value for tool_id, status in self.tool_status.items()
            },
            "performance_metrics": {
                "success_rate": self._calculate_success_rate(),
                "average_decision_time": self.orchestration_metrics[
                    "average_decision_time_ms"
                ],
                "systematic_compliance": self.orchestration_metrics[
                    "systematic_compliance_rate"
                ],
                "active_executions": len(self.active_executions),
            },
            "tool_health_summary": {
                ...
                "healthy_tools": len(
                    [s for s in self.tool_status.values() if s == ToolStatus.AVAILABLE]
                ),
                "failed_tools": len(
                    [s for s in self.tool_status.values() if s == ToolStatus.FAILED]
                ),
            },
        }
    ...
    def register_tool(self, tool_definition: ToolDefinition) -> Dict[str, Any]:
        ...
        if not self._validate_systematic_constraints(
            tool_definition.systematic_constraints
        ):
    ...
    def intelligent_tool_selection(
        ...
        execution_strategy: ExecutionStrategy = ExecutionStrategy.SYSTEMATIC_ONLY,
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "alternative_tools": [
                    ...
                    if t["tool_id"] != decision_result["selected_tool"]["tool_id"]
                ],
            ...
            return {
                "error": str(e),
                "fallback_recommendation": self._get_fallback_tool_recommendation(
                    task_context
                ),
            }

    def execute_tool_systematically(
        self, execution_request: ToolExecutionRequest
    ) -> ToolExecutionResult:
        ...
        try:
            ...
            constraint_validation = self._validate_execution_constraints(
                ...
                execution_request.execution_strategy,
            )

            if not constraint_validation["valid"]:
                raise ValueError(
                    f"Execution violates systematic constraints: {constraint_validation['violations']}"
                )
            ...
            self._update_orchestration_metrics(
                "success", execution_time, constraint_validation["valid"]
            )
    ...
    def optimize_tool_performance(
        ...
        try:
            ...
            return {
                ...
                "systematic_compliance_maintained": impact_analysis[
    ...
    def get_orchestration_analytics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "optimization_impact": {
                "performance_improvements": self.orchestration_metrics[
                    "tool_optimization_improvements"
                ],
    ...
    def _initialize_default_tools(self):
        ...
        for tool_def in default_tools:
            try:
                ...
                self.logger.warning(
                    f"Failed to register default tool {tool_def.tool_id}: {str(e)}"
                )
    ...
    def _apply_decision_framework(
        ...
        for tool in candidate_tools:
            score = self._calculate_tool_score(
                tool, task_requirements, execution_strategy
            )
    ...
    def _calculate_tool_score(
        ...
        systematic_score = (
            1.0
            if tool["systematic_constraints"].get("no_ad_hoc_commands", False)
            else 0.5
        )
        systematic_score *= health_metrics.systematic_compliance_rate

        # Performance score (30% weight)
        expected_time = tool["performance_profile"].get(
            "typical_execution_time_ms", 10000
        )
        ...
        reliability_score = health_metrics.success_rate * (
            health_metrics.availability_percentage / 100
        )
    ...
    def _get_fallback_tool_recommendation(
        self, task_context: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _validate_execution_constraints(
        ...
        if execution_strategy == ExecutionStrategy.SYSTEMATIC_ONLY:
            if not tool_def.systematic_constraints.get("no_ad_hoc_commands", False):
                violations.append(
                    "Tool allows ad-hoc commands but systematic-only execution requested"
                )
    ...
    def _update_tool_metrics(self, tool_id: str, execution_result: ToolExecutionResult):
        ...
        metrics.systematic_compliance_rate = (current_compliance * 0.9) + (
            new_compliance * 0.1
        )
    ...
    def _update_orchestration_metrics(
        self, status: str, execution_time_ms: int, systematic_compliance: bool
    ):
        ...
        self.orchestration_metrics["systematic_compliance_rate"] = (
            current_compliance * (total_executions - 1) + new_compliance
        ) / total_executions
    ...
    def _update_decision_metrics(self, decision_time_ms: int):
        ...
        if total_executions == 0:
            ...
            self.orchestration_metrics["average_decision_time_ms"] = (
                current_avg * total_executions + decision_time_ms
            ) / (total_executions + 1)
    ...
    def _calculate_overall_systematic_compliance(self) -> Dict[str, Any]:
        ...
        total_compliance = sum(
            metrics.systematic_compliance_rate for metrics in self.tool_metrics.values()
        )
    ...
    def _analyze_performance_patterns(self) -> Dict[str, Any]:
        ...
        return {
            "execution_times": {
                tool_id: metrics.average_execution_time_ms
                for tool_id, metrics in self.tool_metrics.items()
            },
            "success_rates": {
                tool_id: metrics.success_rate
                for tool_id, metrics in self.tool_metrics.items()
            },
        }

    def _identify_optimization_opportunities(
        self, performance_analysis: Dict[str, Any], optimization_context: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """Identify optimization opportunities"""
        opportunities = []
    ...
    def _apply_optimization(
        self, tool_id: str, optimization_type: str, parameters: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _validate_optimization_impact(
        ...
        total_performance_improvement = sum(
            r.get("improvement_percentage", 0) for r in successful_optimizations
        )
        ...
        return {
            ...
            "systematic_compliance": True,
            "improvements": {
                "performance": total_performance_improvement,
                "compliance": 0.0,
            },
        }
    ...
    def _calculate_average_execution_time(self) -> float:
        ...
        if not self.tool_metrics:
            return 0.0
        total_time = sum(
            metrics.average_execution_time_ms for metrics in self.tool_metrics.values()
        )
    ...
    def _analyze_criteria_effectiveness(self) -> Dict[str, Any]:
        ...
        return {
            "systematic_compliance_effectiveness": self.orchestration_metrics[
                "systematic_compliance_rate"
            ],
    ...
    def _rank_tools_by_performance(self) -> List[Dict[str, Any]]:
        ...
        for tool_id, metrics in self.tool_metrics.items():
            tool_def = self.registered_tools[tool_id]
            performance_score = metrics.success_rate * 0.7 + (
                metrics.systematic_compliance_rate * 0.3
            )
    ...
    def _analyze_reliability_trends(self) -> Dict[str, Any]:
        ...
        return {
            "overall_reliability_trend": "stable",
            "most_reliable_tools": list(self.tool_metrics.keys()),
        }
    ...
    def _improve_tool_compliance(
        self, tool_id: str = None, optimization_params: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "compliance_status": (
                "improving" if compliance_score > 0.7 else "needs_attention"
            ),
        }

    def _optimize_tool_performance(
        self, tool_id: str = None, optimization_params: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        ...
        if tool_id and optimization_params:
            ...
            if tool_id in self.tool_metrics:
                ...
                if hasattr(current_metrics, "average_execution_time_ms"):
                    ...
                    new_time = max(
                        current_time - target_reduction_ms, 100
                    )  # Minimum 100ms
                    improvement_ms = current_time - new_time
                else:
                    # Handle dict format
                    current_time = current_metrics.get(
        ...
        return {
            ...
            "average_execution_time": self.orchestration_metrics.get(
                "average_decision_time_ms", 0
            ),
            "tools_optimized": len(self.tool_metrics),
            "optimization_status": (
                "optimized" if performance_score > 0.8 else "optimization_needed"
            ),
        }
    ...
    def _calculate_compliance_score(self) -> float:
        ...
        for tool_id, tool_def in self.registered_tools.items():
            ...
            constraints_met = 0
            total_constraints = (
                len(tool_def.systematic_constraints)
            ...
            if hasattr(tool_def, "systematic_constraints"):
                ...
                constraints_met = (
                    1  # Default compliance for tools without explicit constraints
                )

            tool_compliance = (
                constraints_met / total_constraints if total_constraints > 0 else 1.0
            )
    ...
    def _generate_compliance_improvements(self) -> List[str]:
        ...
        for tool_id, tool_def in self.registered_tools.items():
            if hasattr(tool_def, "systematic_constraints"):
                for constraint, required in tool_def.systematic_constraints.items():
                    if required and constraint not in [
                        "no_ad_hoc_commands",
                        "systematic_error_handling",
                    ]:
                        suggestions.append(
                            f"Implement {constraint} for {tool_def.name}"
                        )
            else:
                suggestions.append(
                    f"Add systematic constraints definition for {tool_def.name}"
                )
    ...
    def _get_compliance_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "compliance_gaps": len(self.registered_tools)
            - sum(
                1
                for tool in self.registered_tools.values()
                if hasattr(tool, "systematic_constraints")
        ...
        for tool_id, metrics in self.tool_metrics.items():
            ...
            if success_rate < 0.9:
                optimizations.append(
                    f"Improve reliability for {tool_id} (current: {success_rate:.1%})"
                )

            if avg_time > 5000:  # More than 5 seconds
                optimizations.append(
                    f"Optimize execution time for {tool_id} (current: {avg_time}ms)"
                )
