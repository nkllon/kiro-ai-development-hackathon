"""
Infrastructure Integration Manager Services Services

This module was extracted from infrastructure_integration_manager_services.py
...
class InfrastructureIntegrationManager(ReflectiveModule):
    ...
    def __init__(self, project_root: str = "."):
        ...
        self.config = IntegrationConfig(
            ...
            beast_mode_config_path=self.project_root
            / ".kiro"
            / "specs"
    ...
    def get_module_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "integration_health_score": self.integration_metrics[
    ...
    def is_healthy(self) -> bool:
        ...
        return (
            ...
            and self.integration_metrics["integration_health_score"] >= 0.7
    ...
    def get_health_indicators(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "infrastructure_components": {
                ...
                "registry_status": self.integration_status.get(
                    "project_registry", "unknown"
                ),
                "cursor_rules_status": self.integration_status.get(
                    "cursor_rules", "unknown"
                ),
                "beast_mode_config_status": self.integration_status.get(
                    "beast_mode_config", "unknown"
                ),
            },
            "validation_metrics": {
                "total_validations": self.integration_metrics["total_validations"],
        ...
        self.validation_history.append(
            {
                ...
                "overall_status": (
                    "healthy" if integration_health >= 0.7 else "degraded"
                ),
            }
        )
        ...
        try:
            ...
            if "beast-mode.mk" not in makefile_content:
                ...
                recommendations.append(
                    "Add 'include makefiles/beast-mode.mk' to Makefile"
                )
            missing_targets = []
            ...
            if missing_targets:
                issues.append(
                    f"Missing Beast Mode targets: {', '.join(missing_targets)}"
                )
    ...
    def _validate_project_registry_integration(self) -> ValidationResult:
        ...
        if not self.config.project_registry_path.exists():
            return ValidationResult(
                ...
                recommendations=[
                    "Create project model registry with domain architecture"
                ],
            )
        try:
            ...
            if "domain_architecture" in registry_content:
                ...
                if total_domains < 50:
                    ...
                    recommendations.append(
                        "Expand domain architecture to meet requirements"
                    )
            compliance = (
                registry_content.get("domain_architecture", {})
                .get("overview", {})
            ...
            if compliance != "Reflective Module (RM)":
                ...
                recommendations.append(
                    "Set compliance_standard to 'Reflective Module (RM)'"
                )
            if not issues:
                status = IntegrationStatus.INTEGRATED
                details = (
                    "Project registry fully integrated with Beast Mode requirements"
                )
            elif len(issues) <= 3:
                status = IntegrationStatus.PARTIAL
                ...
                status = IntegrationStatus.FAILED
                details = (
                    "Project registry integration failed, multiple issues detected"
                )
    ...
    def _validate_cursor_rules_integration(self) -> ValidationResult:
        ...
        if not self.config.cursor_rules_path.exists():
            return ValidationResult(
                ...
                recommendations=[
                    "Create .cursor/rules directory with Beast Mode integration"
                ],
            )
        try:
            ...
            if missing_rules:
                ...
                recommendations.append(
                    "Create Beast Mode cursor rules for systematic development"
                )
    ...
    def _validate_beast_mode_configuration(self) -> ValidationResult:
        ...
        if not self.config.beast_mode_config_path.exists():
            return ValidationResult(
                ...
                recommendations=[
                    "Create Beast Mode configuration directory with specs"
                ],
            )
        try:
            ...
            if not beast_mode_src.exists():
                ...
                recommendations.append(
                    "Create src/beast_mode directory with framework code"
                )
            else:
                core_modules = [
            ...
            self.integration_status["beast_mode_config"] = (
    ...
    def _calculate_integration_health(
        ...
        total_score = sum(
            (status_scores[result.status] for result in validation_results)
        )
        return total_score / len(validation_results)

    def _update_integration_metrics(
        self, validation_results: List[ValidationResult], health_score: float
    ):
    ...
    def _generate_integration_recommendations(
        ...
        if failed_components:
            recommendations.append(
                f"Priority: Fix failed components: {', '.join(failed_components)}"
            )
        partial_components = [
            r.component
            for r in validation_results
            if r.status == IntegrationStatus.PARTIAL
        ]
        if partial_components:
            recommendations.append(
                f"Complete partial integrations: {', '.join(partial_components)}"
            )
        seen = set()
    ...
    def get_integration_status(self) -> Dict[str, Any]:
        ...
        return {
            "overall_health_score": self.integration_metrics[
                "integration_health_score"
            ],
    ...
    def get_integration_analytics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "recommendations": self._generate_integration_recommendations(
                [v["results"] for v in self.validation_history[-1:]][-1]
    ...
    def _analyze_health_trends(self) -> Dict[str, Any]:
        ...
        if len(self.validation_history) < 2:
            return {
                ...
                "message": "Need more validation history",
            }
        recent_scores = [v["health_score"] for v in self.validation_history[-5:]]
        older_scores = (
            [v["health_score"] for v in self.validation_history[-10:-5]]
