"""
Beast Mode Mailbox Logger

Persistent message logging system that runs continuously in the background
to capture all messages for later retrieval and analysis.
"""
...
class MailboxLogger:
    ...
    async def _log_message(self, raw_message: Dict[str, Any]) -> None:
        ...
        try:
            # Create log entry with full message preservation
            log_entry = {
    ...
    async def _write_log_entry(self, log_entry: Dict[str, Any]) -> None:
        ...
        try:
            ...
            # Write to file (using thread executor for file I/O)
    ...
    def save_full_content(self, message: BeastModeMessage) -> str:
        ...
        """
        try:
            # Create detailed content directory
                    ...
                    log_files.append(
                        {
                            ...
                            "is_current": file_path == self.current_log_file,
                        }
                    )
                        ...
                        for line in f:
                            ...
                            try:
                                ...
                                if log_entry.get("parsed_message"):
                                    ...
                                    if (
                                        source_agents
                                        and message_data.get("source")
                                        not in source_agents
                                    ):
                                        continue
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        return {
            "status": (
                "healthy" if (self.is_running and self.is_connected) else "unhealthy"
            ),
            "is_running": self.is_running,
            "is_connected": self.is_connected,
...
class MailboxLoggerManager:
        ...
        try:
            if self.event_loop and not self.event_loop.is_closed():
                ...
                future = asyncio.run_coroutine_threadsafe(
                    self.logger.stop_logging(), self.event_loop
                )
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "thread_alive": (
                self.background_thread.is_alive() if self.background_thread else False
            ),
            "logger_status": self.logger.get_health_status(),
        }
