...
class ConstraintComplianceValidator(ReflectiveModule):
    ...
    def validate_all_constraints(self) -> ComplianceReport:
        ...
        for constraint_id, constraint_info in self.constraints.items():
            validation_result = self._validate_constraint(
        ...
        compliant_count = sum(
            (
                ...
                if v.status == ConstraintStatus.COMPLIANT
            )
        )
        ...
        critical_violations = [
            ...
            for v in constraint_validations
            if v.status == ConstraintStatus.NON_COMPLIANT
            and self.constraints[v.constraint_id]["criticality"] == "CRITICAL"
        ]
        recommendations = self._generate_compliance_recommendations(
            constraint_validations, risk_mitigations
        )
    ...
    def _validate_constraint(
        self, constraint_id: str, constraint_info: Dict[str, Any]
    ...
    def _assess_risk_mitigation(
        self, risk_id: str, risk_info: Dict[str, Any]
        ...
        for validation in constraint_validations:
            if validation.status == ConstraintStatus.NON_COMPLIANT:
                recommendations.append(
                    f"CRITICAL: Address {validation.constraint_id} non-compliance immediately"
                )
            elif validation.status == ConstraintStatus.PARTIALLY_COMPLIANT:
                recommendations.append(
                    f"Improve {validation.constraint_id} compliance from {validation.compliance_percentage:.1f}%"
                )
        for mitigation in risk_mitigations:
            if mitigation.current_risk_level in ["HIGH", "CRITICAL"]:
                recommendations.append(
                    f"Address high residual risk: {mitigation.risk_id}"
                )
            elif mitigation.mitigation_effectiveness < 80.0:
                recommendations.append(
                    f"Improve mitigation effectiveness for {mitigation.risk_id}"
                )
