"""
Deployment Manager for Beast Mode Agent Collaboration Network

Handles deployment scripts for single-machine and distributed setups,
including service orchestration and process management.
"""
...
class DeploymentManager:
    ...
    def create_single_machine_deployment(self, environment: str) -> str:
        ...
        # Define services for single machine deployment
        services = {
            "redis": ServiceDefinition(
                name="redis",
                command=["redis-server", "--port", str(config.redis.port)],
                working_directory=".",
                environment={},
                health_check_url=f"redis://{config.redis.host}:{config.redis.port}"
            ),
            "mailbox_logger": ServiceDefinition(
                name="mailbox_logger",
                command=[
                    sys.executable, "-m", "beast_mode.messaging.mailbox_logger",
                    "--redis-url", f"redis://{config.redis.host}:{config.redis.port}",
                    "--log-file", config.agent.mailbox_log_file
                ],
                working_directory=".",
                environment=self.config_manager.get_environment_variables(environment),
                dependencies=["redis"]
            ),
            "agent": ServiceDefinition(
                name="agent",
                command=[
                    sys.executable, "-m", "beast_mode.messaging.bus_client",
                    "--agent-id", config.agent.agent_id,
                    "--capabilities", ",".join(config.agent.capabilities),
                    "--redis-url", f"redis://{config.redis.host}:{config.redis.port}"
                ],
                working_directory=".",
                environment=self.config_manager.get_environment_variables(environment),
                dependencies=["redis", "mailbox_logger"]
            )
        }
        ...
        deployment_status = DeploymentStatus(
            ...
            services={name: {"status": "pending", "pid": None} for name in services.keys()},
    ...
    def create_distributed_deployment(self, environment: str, nodes: List[str]) -> str:
        ...
        # For distributed deployment, we create deployment manifests
        # that can be executed on each node
        deployment_manifest = {
            "deployment_id": deployment_id,
            "environment": environment,
            "nodes": nodes,
        ...
        # Redis cluster setup (simplified - in production would use Redis Cluster)
        if len(nodes) > 0:
            redis_node = nodes[0]  # Primary Redis node
            deployment_manifest["services"]["redis"] = {
                "node": redis_node,
                "service": ServiceDefinition(
                    name="redis",
                    command=["redis-server", "--port", str(config.redis.port), "--bind", "0.0.0.0"],
                    working_directory=".",
                    environment={}
                ).__dict__
            }
        ...
        for i, node in enumerate(nodes):
            ...
            deployment_manifest["services"][f"agent_{i}"] = {
                "node": node,
                "service": ServiceDefinition(
                    name=f"agent_{i}",
                    command=[
            ...
            # Mailbox logger on each node
            deployment_manifest["services"][f"mailbox_logger_{i}"] = {
                "node": node,
                "service": ServiceDefinition(
                    name=f"mailbox_logger_{i}",
                    command=[
                        sys.executable, "-m", "beast_mode.messaging.mailbox_logger",
                        "--redis-url", f"redis://{config.redis.host}:{config.redis.port}",
                        "--log-file", f"mailbox_{node}.log"
                    ],
                    working_directory=".",
                    environment=self.config_manager.get_environment_variables(environment),
                    dependencies=["redis"]
        ...
        deployment_status = DeploymentStatus(
            deployment_id=deployment_id,
            type=DeploymentType.DISTRIBUTED,
            environment=environment,
            services={name: {"status": "pending", "node": service["node"]} 
                     for name, service in deployment_manifest["services"].items()},
            started_at=time.strftime("%Y-%m-%d %H:%M:%S"),
    ...
    def create_docker_deployment(self, environment: str) -> str:
        ...
        docker_compose = {
            ...
            "services": {
                ...
                "mailbox_logger": {
                    "build": ".",
                    "command": [
                        "python", "-m", "beast_mode.messaging.mailbox_logger",
                        "--redis-url", "redis://redis:6379",
                        "--log-file", "/app/logs/mailbox.log"
                    ],
                    "depends_on": ["redis"],
                ...
                "agent": {
                    ...
                    "depends_on": ["redis", "mailbox_logger"],
                    "volumes": ["./spores:/app/spores", "./logs:/app/logs"],
                    "environment": self.config_manager.get_environment_variables(environment),
                    "restart": "unless-stopped"
                }
            },
        ...
        deployment_status = DeploymentStatus(
            ...
            environment=environment,
            services={name: {"status": "pending"} for name in docker_compose["services"].keys()},
            started_at=time.strftime("%Y-%m-%d %H:%M:%S"),
            status="compose_created"
        )
    ...
    def _start_service(self, service: ServiceDefinition, deployment_id: str):
        ...
        try:
            ...
            if deployment_id in self.deployments:
                self.deployments[deployment_id].services[service.name] = {
                    "status": "running",
                    "pid": process.pid,
                    "started_at": time.strftime("%Y-%m-%d %H:%M:%S")
                }
    ...
    def stop_deployment(self, deployment_id: str):
        ...
        # Stop all processes for this deployment
