...
class DeploymentManager:
    ...
    def create_single_machine_deployment(self, environment: str) -> str:
        ...
        services = {
            ...
            "mailbox_logger": ServiceDefinition(
                ...
                command=[
                    sys.executable,
                    "-m",
                    "beast_mode.messaging.mailbox_logger",
                    "--redis-url",
                    f"redis://{config.redis.host}:{config.redis.port}",
                    "--log-file",
                    config.agent.mailbox_log_file,
                ],
                working_directory=".",
            ...
            "agent": ServiceDefinition(
                ...
                command=[
                    sys.executable,
                    "-m",
                    "beast_mode.messaging.bus_client",
                    "--agent-id",
                    config.agent.agent_id,
                    "--capabilities",
                    ",".join(config.agent.capabilities),
                    "--redis-url",
                    f"redis://{config.redis.host}:{config.redis.port}",
                ],
                working_directory=".",
        ...
        deployment_status = DeploymentStatus(
            ...
            services={
                name: {"status": "pending", "pid": None} for name in services.keys()
            },
    ...
    def create_distributed_deployment(self, environment: str, nodes: List[str]) -> str:
        ...
        for i, node in enumerate(nodes):
            ...
            deployment_manifest["services"][f"agent_{i}"] = {
                ...
                "service": ServiceDefinition(
                    ...
                    working_directory=".",
                    environment=self.config_manager.get_environment_variables(
                        environment
                    ),
                    dependencies=["redis"],
                ).__dict__,
        ...
        deployment_status = DeploymentStatus(
            ...
            environment=environment,
            services={
                name: {"status": "pending", "node": service["node"]}
    ...
    def create_docker_deployment(self, environment: str) -> str:
        ...
        deployment_status = DeploymentStatus(
            ...
            services={
                name: {"status": "pending"}
                for name in docker_compose["services"].keys()
            },
            started_at=time.strftime("%Y-%m-%d %H:%M:%S"),
        ...
        self.logger.info(
            f"Start with: docker-compose -f {compose_path} --env-file .env-{deployment_id} up -d"
        )

        return deployment_id

    def _start_services_in_order(
        self, services: Dict[str, ServiceDefinition], deployment_id: str
    ):
