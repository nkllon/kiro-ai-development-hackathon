...
class RCAReportGenerator(ReflectiveModule):
    ...
    def get_health_indicators(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "performance": {
                "status": (
                    "healthy"
                    if self.total_generation_time / max(1, self.reports_generated) < 5.0
                    else "degraded"
                ),
                "average_generation_time": self.total_generation_time
                / max(1, self.reports_generated),
    ...
    def generate_report(
        ...
        try:
            ...
            self.logger.info(
                f"Generating {report_config.format.value} report for {rca_report.total_failures} test failures"
            )
            ...
            for section_type in report_config.include_sections:
                try:
                    section = self._generate_section(
                        section_type, rca_report, report_config
                    )
                    if section:
            ...
            self.logger.info(
                f"Report generated: {len(sections)} sections, {total_length} chars in {generation_time:.3f}s"
            )
    ...
    def format_for_console(
        self, rca_report: TestRCAReportData, use_colors: bool = True
    ) -> str:
        """
    ...
    def generate_json_report(self, rca_report: TestRCAReportData) -> Dict[str, Any]:
        ...
        try:
            ...
            json_data = {
                ...
                "root_causes": [
                    {"type": cause_type.value, "count": count}
    ...
    def display_console_report(
        ...
        try:
            ...
            print(
                f"   • Systematic fixes: {rca_report.summary.systematic_fixes_available}"
            )
    ...
    def _generate_header_section(
        self, rca_report: TestRCAReportData, config: ReportConfiguration
    ) -> FormattedReportSection:
        """Generate report header section"""
        if config.format == ReportFormat.CONSOLE:
            title = ConsoleColors.colorize(
                "🔍 Test RCA Analysis Report",
                ConsoleColors.BOLD + ConsoleColors.CYAN,
                config.color_output,
            )
            separator = ConsoleColors.colorize(
                "=" * 60, ConsoleColors.CYAN, config.color_output
            )
    ...
    def _generate_summary_section(
        ...
        if config.format == ReportFormat.CONSOLE:
            ...
            total_failures = ConsoleColors.colorize(
                str(rca_report.total_failures), ConsoleColors.RED, config.color_output
            )
            analyzed = ConsoleColors.colorize(
                str(rca_report.failures_analyzed),
                ConsoleColors.GREEN,
                config.color_output,
            )
            fixes = ConsoleColors.colorize(
                str(summary.systematic_fixes_available),
                ConsoleColors.BLUE,
                config.color_output,
            )
            confidence = ConsoleColors.colorize(
                f"{summary.confidence_score:.1%}",
                ConsoleColors.MAGENTA,
                config.color_output,
            )
            time_est = ConsoleColors.colorize(
                f"{summary.estimated_fix_time_minutes} min",
                ConsoleColors.CYAN,
                config.color_output,
            )
            ...
            if summary.critical_issues:
                critical_title = ConsoleColors.colorize(
                    "⚠️  Critical Issues:",
                    ConsoleColors.BOLD + ConsoleColors.RED,
                    config.color_output,
                )
                critical_list = "\n".join(
                    [f"  • {issue}" for issue in summary.critical_issues[:3]]
                )
                content += f"\n\n{critical_title}\n{critical_list}"
        elif config.format == ReportFormat.MARKDOWN:
    ...
    def _generate_failures_section(
        ...
        if config.format == ReportFormat.CONSOLE:
            ...
            for group_name, failures in list(failure_groups.items())[
                : config.max_failures_displayed
            ]:
                group_title = ConsoleColors.colorize(
                ...
                for failure in failures[:3]:
                    test_name = ConsoleColors.colorize(
                        failure.test_name, ConsoleColors.WHITE, config.color_output
                    )
                    ...
                    if config.include_stack_traces and failure.stack_trace:
                        content_lines.append(
                            f"    Error: {failure.error_message[:100]}..."
                        )
                if len(failures) > 3:
            ...
            content = json.dumps(
                {"section": "failures", "grouped_failures": failure_data}, indent=2
            )
        return FormattedReportSection(
    ...
    def _generate_root_causes_section(
        self, rca_report: TestRCAReportData, config: ReportConfiguration
    ) -> FormattedReportSection:
        ...
        if config.format == ReportFormat.CONSOLE:
            ...
            if rca_report.summary.most_common_root_causes:
                ...
                for cause_type, count in rca_report.summary.most_common_root_causes[:5]:
                    cause_name = ConsoleColors.colorize(
                        cause_type.value.replace("_", " ").title(),
                        ConsoleColors.WHITE,
                        config.color_output,
                    )
                    count_str = ConsoleColors.colorize(
                        f"({count})", ConsoleColors.CYAN, config.color_output
                    )
        ...
        return FormattedReportSection(
            ...
            content=content,
            metadata={
                "root_causes_count": len(rca_report.summary.most_common_root_causes)
            },
        )

    def _generate_fixes_section(
        self, rca_report: TestRCAReportData, config: ReportConfiguration
    ) -> FormattedReportSection:
        ...
        if config.format == ReportFormat.CONSOLE:
            title = ConsoleColors.colorize(
                "🔧 Systematic Fixes",
                ConsoleColors.BOLD + ConsoleColors.GREEN,
                config.color_output,
            )
            content_lines = [title]
            if all_fixes:
                ...
                for i, fix in enumerate(all_fixes[:5], 1):
                    fix_title = ConsoleColors.colorize(
                        f"{i}. {fix.fix_description}",
                        ConsoleColors.WHITE,
                        config.color_output,
                    )
                    content_lines.append(f"\n{fix_title}")
                    content_lines.append(
                        f"   Root Cause: {fix.root_cause.cause_type.value}"
                    )
                    content_lines.append(
                        f"   Estimated Time: {fix.estimated_time_minutes} minutes"
                    )
        ...
        if config.format == ReportFormat.CONSOLE:
            ...
            if recommendations:
                for i, rec in enumerate(recommendations, 1):
                    rec_text = ConsoleColors.colorize(
                        f"{i}. {rec}", ConsoleColors.WHITE, config.color_output
                    )
                    content_lines.append(f"\n{rec_text}")
            else:
        ...
        if config.format == ReportFormat.CONSOLE:
            ...
            if patterns:
                ...
                for pattern in patterns[:3]:
                    pattern_name = ConsoleColors.colorize(
                        pattern.pattern_name, ConsoleColors.WHITE, config.color_output
                    )
                    content_lines.append(f"\n• {pattern_name}")
        ...
        if config.format == ReportFormat.CONSOLE:
            ...
            if next_steps:
                for i, step in enumerate(next_steps, 1):
                    step_text = ConsoleColors.colorize(
                        f"{i}. {step}", ConsoleColors.WHITE, config.color_output
                    )
                    content_lines.append(f"\n{step_text}")
            else:
            ...
            content = json.dumps(
                {"section": "next_steps", "next_steps": next_steps}, indent=2
            )
        return FormattedReportSection(
    ...
    def _generate_footer_section(
        ...
        if config.format == ReportFormat.CONSOLE:
            ...
            footer_text = ConsoleColors.colorize(
                "Beast Mode RCA Analysis Complete",
                ConsoleColors.BOLD,
                config.color_output,
            )
            confidence = (
                f"Overall Confidence: {rca_report.summary.confidence_score:.1%}"
            )
            content = f"{separator}\n{footer_text}\n{confidence}\n{separator}"
        elif config.format == ReportFormat.MARKDOWN:
...
def generate_report(
    self, rca_report: TestRCAReportData, config: Optional[ReportConfiguration] = None
) -> FormattedReport:
    """
    Generate formatted RCA report from analysis results
    Requirements: 2.2, 2.3, 2.4 - Comprehensive reporting with actionable recommendations
...
def _generate_header_section(
    ...
    if config.format == ReportFormat.CONSOLE:
        ...
        timestamp = (
            f"Generated: {rca_report.analysis_timestamp.strftime('%Y-%m-%d %H:%M:%S')}"
        )
        content = f"{separator}\n{title}\n{separator}\n{timestamp}\n"
    elif config.format == ReportFormat.MARKDOWN:
        content = f"# 🔍 Test RCA Analysis Report\n\n**Generated:** {rca_report.analysis_timestamp.strftime('%Y-%m-%d %H:%M:%S')}\n"
...
def _generate_summary_section(
    ...
    if config.format == ReportFormat.CONSOLE:
        ...
        analyzed = ConsoleColors.colorize(
            str(rca_report.failures_analyzed), ConsoleColors.GREEN, config.color_output
        )
        fixes = ConsoleColors.colorize(
            str(summary.systematic_fixes_available),
            ConsoleColors.BLUE,
...
def _generate_recommendations_section(
    ...
    if config.format == ReportFormat.CONSOLE:
        ...
        content = json.dumps(
            {"section": "recommendations", "recommendations": recommendations}, indent=2
        )
    return FormattedReportSection(
        section_type=ReportSection.RECOMMENDATIONS,
        title="Actionable Recommendations",
...
def _generate_footer_section(
    ...
    if config.format == ReportFormat.CONSOLE:
        ...
        footer_text = ConsoleColors.colorize(
            "Beast Mode RCA Analysis Complete", ConsoleColors.BOLD, config.color_output
        )
