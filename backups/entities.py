"""
Entity and Aggregate Root base classes for DDD patterns.

This module provides systematic implementations of DDD entities and aggregate roots
with built-in RM compliance, domain validation, and event handling capabilities.
"""
...
# Type variables for generic entity and aggregate implementations
TEntityId = TypeVar("TEntityId", bound=Union[str, int, UUID])
TAggregateId = TypeVar("TAggregateId", bound=Union[str, int, UUID])
...
class Entity(DomainReflectiveModule, Generic[TEntityId], ABC):
    ...
    def __init__(
        self, entity_id: TEntityId, domain_context: str, module_id: Optional[str] = None
    ):
        """
        ...
        # Initialize RM base with domain context
        super().__init__(domain_context, module_id)

        logger.debug(
            f"Entity created: {self.__class__.__name__}({entity_id}) in context: {domain_context}"
        )
    ...
    def add_domain_event(self, event: "DomainEvent"):
        ...
        logger.debug(
            f"Domain event added to {self.__class__.__name__}({self.id}): {event.__class__.__name__}"
        )
    ...
    def clear_domain_events(self):
        ...
        if event_count > 0:
            logger.debug(
                f"Cleared {event_count} domain events from {self.__class__.__name__}({self.id})"
            )
    ...
    def update_version(self):
        ...
        self._updated_at = datetime.now()
        logger.debug(
            f"Entity version updated: {self.__class__.__name__}({self.id}) -> v{self._version}"
        )
    ...
    def get_entity_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "pending_events": len(self._domain_events),
    ...
    async def get_module_status(self) -> "ModuleHealth":
        ...
        # Validate domain invariants as part of health check
    ...
    async def get_module_capabilities(self) -> List[ModuleCapability]:
        ...
        return [
            ModuleCapability(
                name=f"entity_{self.__class__.__name__.lower()}",
                description=f"Domain entity: {self.__class__.__name__}",
    ...
    async def is_healthy(self) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Health check failed for entity {self.__class__.__name__}({self.id}): {e}"
            )
            return False
    ...
    async def get_health_indicators(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "entity_type": self.__class__.__name__,
            "version": self._version,
            "domain_valid": validation_result.is_valid,
...
class AggregateRoot(Entity[TAggregateId], ABC):
    ...
    def __init__(
        ...
        self._max_size = max_size or 100  # Default max size
        self._child_entities: Dict[str, List[Entity]] = {}
        self._aggregate_version = 1

        logger.debug(
            f"AggregateRoot created: {self.__class__.__name__}({aggregate_id}) with max_size: {self._max_size}"
        )
    ...
    def add_child_entity(self, entity_type: str, entity: Entity):
        ...
        # Check aggregate size constraints
        current_size = self.get_aggregate_size()
        if current_size >= self._max_size:
            raise DomainException(
                f"Aggregate size limit exceeded: {current_size} >= {self._max_size}",
                error_code="AGGREGATE_SIZE_LIMIT",
                context={
        ...
        logger.debug(
            f"Child entity added to aggregate {self.__class__.__name__}({self.id}): {entity_type}"
        )
    ...
    def remove_child_entity(self, entity_type: str, entity: Entity):
        ...
        if entity_type in self._child_entities:
            try:
                ...
                self.update_version()
                logger.debug(
                    f"Child entity removed from aggregate {self.__class__.__name__}({self.id}): {entity_type}"
                )
            except ValueError:
                logger.warning(
                    f"Attempted to remove non-existent child entity from aggregate {self.id}"
                )

    def get_child_entities(
        self, entity_type: Optional[str] = None
    ) -> Union[List[Entity], Dict[str, List[Entity]]]:
        """
    ...
    def get_aggregate_size(self) -> int:
        ...
        total_size = 1  # Count the aggregate root itself
    ...
    def validate_aggregate_constraints(self) -> ValidationResult:
        ...
        if current_size > self._max_size:
            result.add_error(
                f"Aggregate size {current_size} exceeds maximum {self._max_size}",
                code="AGG_001",
                component=self.__class__.__name__,
                context={"current_size": current_size, "max_size": self._max_size},
            )
        ...
        try:
            ...
            if not boundaries.aggregate_type:
                result.add_error(
                    "Aggregate boundaries must specify aggregate_type",
                    code="AGG_002",
                    component=self.__class__.__name__,
                )
        except Exception as e:
            result.add_error(
                f"Failed to get aggregate boundaries: {str(e)}",
                code="AGG_002",
                component=self.__class__.__name__,
            )
            ...
            for i, entity in enumerate(entities):
                try:
                    ...
                    if not child_validation.is_valid:
                        result.add_error(
                            f"Child entity {entity_type}[{i}] validation failed",
                            code="AGG_003",
                            component=self.__class__.__name__,
                            context={"child_errors": child_validation.errors},
                        )
                except Exception as e:
                    result.add_error(
                        f"Child entity {entity_type}[{i}] validation error: {str(e)}",
                        code="AGG_003",
                        component=self.__class__.__name__,
                    )
    ...
    def clear_all_domain_events(self):
        ...
        # Clear events from child entities
        for entities in self._child_entities.values():
            for entity in entities:
    ...
    def get_aggregate_info(self) -> Dict[str, Any]:
        ...
        base_info.update(
            {
                "aggregate_type": self.__class__.__name__,
                "aggregate_size": self.get_aggregate_size(),
                "max_size": self._max_size,
                "child_entity_types": list(self._child_entities.keys()),
                "child_entity_counts": {
    ...
    # Override entity methods to include aggregate-specific validation
    ...
    def validate_domain_invariants(self) -> ValidationResult:
        ...
        """
        # Get base entity validation
        entity_result = super().validate_domain_invariants()

        # Add aggregate-specific validation
    ...
    async def get_module_capabilities(self) -> List[ModuleCapability]:
        ...
        # Add aggregate-specific capability
        base_capabilities.append(
            ModuleCapability(
                name=f"aggregate_{self.__class__.__name__.lower()}",
                description=f"Domain aggregate: {self.__class__.__name__}",
                available=await self.is_healthy(),
                version=str(self._aggregate_version),
                metadata={
                    "aggregate_size": self.get_aggregate_size(),
                    "max_size": self._max_size,
                    "child_types": list(self._child_entities.keys()),
                },
            )
        )
    ...
    async def get_health_indicators(self) -> Dict[str, Any]:
        ...
        # Add aggregate-specific indicators
        base_indicators.update(
            {
                "aggregate_size": self.get_aggregate_size(),
                "max_size": self._max_size,
                "size_utilization": (self.get_aggregate_size() / self._max_size) * 100,
                "child_entity_types": len(self._child_entities),
                "total_child_entities": sum(
                    len(entities) for entities in self._child_entities.values()
                ),
                "aggregate_version": self._aggregate_version,
                "total_pending_events": len(self.collect_all_domain_events()),
            }
