...
class TestCoverageValidator(ComplianceValidator):
    ...
    def _run_coverage_analysis(self, target_path: Path) -> Dict[str, Any]:
        ...
        try:
            result = subprocess.run(['python', '-m', 'pytest', '--cov=src', '--cov-report=json', '--cov-report=term-missing', str(target_path / 'tests') if (target_path / 'tests').exists() else 'tests'], capture_output=True, text=True, cwd=self.repository_path, timeout=300)
    ...
    def _manual_coverage_analysis(self, target_path: Path) -> Dict[str, Any]:
        ...
        for src_file in src_files:
            try:
                ...
                file_coverage[str(relative_path)] = {'total_lines': file_total, 'covered_lines': file_covered, 'coverage_percent': file_covered / file_total * 100 if file_total > 0 else 0}
