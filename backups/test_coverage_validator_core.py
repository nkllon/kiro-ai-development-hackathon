"""
Test Coverage Validator Core

This module was extracted from test_coverage_validator.py
...
class TestCoverageValidator(ComplianceValidator):
    ...
    def _run_coverage_analysis(self, target_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            if coverage_json_path.exists():
                ...
                coverage_data["overall_coverage"] = coverage_json.get("totals", {}).get(
                    "percent_covered", 0.0
                )
                coverage_data["total_lines"] = coverage_json.get("totals", {}).get(
                    "num_statements", 0
                )
                coverage_data["covered_lines"] = coverage_json.get("totals", {}).get(
                    "covered_lines", 0
                )
        ...
        for src_file in src_files:
            try:
                ...
                if relative_path is None:
                    continue
                test_file_exists = self._has_corresponding_test_file(
                    src_file, test_files
                )
                if test_file_exists:
    ...
    def _has_corresponding_test_file(
        self, src_file: Path, test_files: List[Path]
    ) -> bool:
        """
        ...
        for test_file in test_files:
            ...
            if (
                src_name in test_name
                or test_name.replace("test_", "") == src_name
                or test_name == f"test_{src_name}"
            ):
                return True
        return False
    ...
    def _find_uncovered_modules(
        self, target_path: Path, coverage_data: Dict[str, Any]
    ) -> List[str]:
        """
    ...
    def _generate_coverage_issues(
        self, report: CoverageReport, meets_baseline: bool, coverage_gap: float
        ...
        if report.failing_tests:
            ...
            issues.append(
                ComplianceIssue(
                    ...
                    affected_files=list(
                        set((test.test_file for test in report.failing_tests))
                    ),
                    remediation_steps=[
    ...
    if not meets_baseline:
        issues.append(
            ComplianceIssue(
                issue_type=ComplianceIssueType.TEST_FAILURE,
                severity=(
                    IssueSeverity.HIGH if coverage_gap > 10 else IssueSeverity.MEDIUM
                ),
                description=f"Test coverage {report.overall_coverage:.1f}% below baseline {self.baseline_coverage}%",
                affected_files=[],
                remediation_steps=[
