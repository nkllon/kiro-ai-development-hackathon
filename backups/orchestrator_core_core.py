...
This module was extracted from orchestrator_core.py
...
class ComplianceOrchestrator(ReflectiveModule):
    ...
    def __init__(self, repository_path: str='.'):
        ...
        self._config = {'target_branch': 'main', 'base_branch': 'origin/master', 'test_coverage_baseline': 96.7, 'max_module_size': 200, 'phase2_expected_failing_tests': 7}
    ...
    def analyze_commits_ahead_of_main(self) -> ComplianceAnalysisResult:
        ...
        try:
            context = ValidationContext(repository_path=str(self.repository_path), target_branch=self._config['target_branch'], base_branch=self._config['base_branch'])
            ...
            result = ComplianceAnalysisResult()
            result.critical_issues.append(ComplianceIssue(issue_type=ComplianceIssueType.ARCHITECTURAL_VIOLATION, severity=IssueSeverity.CRITICAL, description=f'Compliance analysis failed: {str(e)}', blocking_merge=True))
            return result
    ...
    def generate_compliance_report(self) -> str:
        ...
        report_lines = ['# Beast Mode Framework Compliance Report', f'Generated: {analysis_result.analysis_timestamp}', '', f'## Overall Compliance Score: {analysis_result.overall_compliance_score:.2f}/100', '', f'## Analysis Summary', f'- Commits analyzed: {len(analysis_result.commits_analyzed)}', f'- RDI compliance score: {analysis_result.rdi_compliance.compliance_score:.2f}', f'- RM compliance score: {analysis_result.rm_compliance.compliance_score:.2f}', f'- Test coverage: {analysis_result.test_coverage_status.current_coverage:.1f}%', f"- Phase 3 ready: {('Yes' if analysis_result.phase3_ready else 'No')}", '']
        if analysis_result.critical_issues:
            report_lines.extend(['## Critical Issues', ''])
            for issue in analysis_result.critical_issues:
                report_lines.extend([f"### {issue.issue_type.value.replace('_', ' ').title()}", f'**Severity:** {issue.severity.value.upper()}', f'**Description:** {issue.description}', f"**Blocking merge:** {('Yes' if issue.blocking_merge else 'No')}", ''])
    ...
    def get_module_status(self) -> Dict[str, Any]:
        ...
        return {'module_name': 'ComplianceOrchestrator', 'repository_path': str(self.repository_path), 'registered_validators': len(self._validators), 'registered_analyzers': len(self._analyzers), 'configuration': self._config, 'is_healthy': self.is_healthy()}

    def is_healthy(self) -> bool:
        """Check if the compliance orchestrator is healthy."""
        try:
            return self.repository_path.exists() and self.repository_path.is_dir() and (self.repository_path / '.git').exists()
        except Exception:
            return False
    ...
    def get_health_indicators(self) -> Dict[str, Any]:
        ...
        try:
            ...
            indicators['validators_registered'] = {'status': 'healthy' if self._validators else 'degraded', 'value': len(self._validators), 'message': f'{len(self._validators)} validators registered'}
            indicators['analyzers_registered'] = {'status': 'healthy' if self._analyzers else 'degraded', 'value': len(self._analyzers), 'message': f'{len(self._analyzers)} analyzers registered'}
        except Exception as e:
    ...
    def _calculate_overall_compliance_score(self, result: ComplianceAnalysisResult) -> float:
        ...
        scores = [result.rdi_compliance.compliance_score, result.rm_compliance.compliance_score, result.test_coverage_status.current_coverage, result.task_completion_reconciliation.reconciliation_score]
    ...
    def _identify_critical_issues(self, result: ComplianceAnalysisResult) -> List[ComplianceIssue]:
        ...
        critical_issues = []
        for component_status in [result.rdi_compliance, result.rm_compliance, result.test_coverage_status, result.task_completion_reconciliation]:
            if hasattr(component_status, 'issues'):
                critical_issues.extend([issue for issue in component_status.issues if issue.severity == IssueSeverity.CRITICAL])
    ...
    def _assess_phase3_readiness(self, result: ComplianceAnalysisResult) -> bool:
        ...
        return result.overall_compliance_score >= 80 and len(result.critical_issues) == 0 and (result.test_coverage_status.current_coverage >= result.test_coverage_status.baseline_coverage * 0.95)
