...
class GenerationSpec:
    ...
    def validate_spec(self) -> ValidationResult:
        ...
        for attr in self.attributes:
            ...
            if "type" not in attr:
                result.add_error(
                    f"Attribute {attr.get('name', 'unknown')} must have a type"
                )
...
'''
    ...
    def _prepare_context(self, spec: GenerationSpec) -> Dict[str, Any]:
        ...
        id_attr = next(
            (attr for attr in spec.attributes if attr.get("is_id", False)), None
        )
        if id_attr:
        ...
        return {
            ...
            "description": spec.metadata.get(
                "description", f"{spec.name} domain entity"
            ),
            "id_type": id_type,
            "id_param": id_param,
    ...
    def _merge_template_content(self, parent_content: str, child_content: str) -> str:
        ...
        for block_name, block_content in merged_blocks.items():
            ...
            result = result.replace(
                f"{{% block {block_name} %}}{parent_blocks.get(block_name, '')}{{% endblock %}}",
                replacement,
            )
...
class CustomizableCodeTemplate(CodeTemplate):
    ...
    def apply_extensions(
        self, context: Dict[str, Any], spec: GenerationSpec
    ) -> Dict[str, Any]:
...
class EnhancedEntityTemplate(CustomizableCodeTemplate):
    ...
    def _generate_with_custom_template(
        self, template: Template, spec: GenerationSpec
    ) -> GeneratedCode:
...
class RMDDDCodeGenerator(DomainReflectiveModule):
    ...
    def _initialize_default_templates(self):
        ...
        self._templates[GenerationTarget.ENTITY] = EnhancedEntityTemplate(
...
'''
        ...
        self._template_registry.register_template(
            "entity_simple", simple_entity_template, GenerationTarget.ENTITY
        )
...
'''
        ...
        self._template_registry.register_template(
            "entity_rich", rich_entity_template, GenerationTarget.ENTITY
        )
    ...
    def generate_code(self, spec: GenerationSpec) -> GeneratedCode:
        ...
        try:
            ...
            raise DomainException(
                f"Code generation failed: {str(e)}", error_code="CODE_GENERATION_FAILED"
            )

    def generate_entity(
        self, name: str, domain_context: str, attributes: List[Dict[str, Any]], **kwargs
    ) -> GeneratedCode:
        """
    ...
    def validate_domain_invariants(self):
        ...
        if not JINJA2_AVAILABLE:
            result.add_warning(
                "Jinja2 not available - code generation capabilities limited"
            )
...
def generate_aggregate_from_dict(aggregate_def: Dict[str, Any]) -> GeneratedCode:
    ...
    def generate_with_template(
        self, spec: GenerationSpec, template_name: Optional[str] = None
    ) -> GeneratedCode:
        """
        ...
        if template_name:
            ...
            if custom_template and spec.target_type in self._templates:
                template = self._templates[spec.target_type]
                if isinstance(template, CustomizableCodeTemplate):
                    return template._generate_with_custom_template(
                        custom_template, spec
                    )
    ...
    def create_template_composition(
        ...
        self._template_registry.register_template(
            composition_name, composed_content, target_type
        )
...
def create_custom_entity_template(
    name: str,
    ...
    generator.register_custom_template(
        name, template_content, GenerationTarget.ENTITY, custom_filters=custom_filters
    )
    return name


def generate_with_custom_template(
    spec_dict: Dict[str, Any], template_name: str
) -> GeneratedCode:
    """
    Generate code using a custom template.
