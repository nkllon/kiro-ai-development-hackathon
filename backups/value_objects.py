"""
Value Object base classes for DDD patterns.

This module provides systematic implementations of DDD value objects with
built-in validation, immutability enforcement, and equality semantics.
"""
...
class ValueObject(ABC):
    ...
    def __hash__(self) -> int:
        ...
        """
        # Create a tuple of all attribute values for hashing
        values = []
        ...
        if not validation_result.is_valid:
            raise ValidationException(
                ...
                context={
                    ...
                    "value_object_data": self.to_dict(),
                },
            )
...
class Money(ImmutableValueObject):
    ...
    def __truediv__(self, divisor: Union[int, float, Decimal]) -> "Money":
        ...
        if divisor == 0:
            raise DomainException(
                "Cannot divide by zero", error_code="DIVISION_BY_ZERO"
            )
...
class EmailAddress(ImmutableValueObject):
    ...
    def validate(self) -> ValidationResult:
        ...
        if len(self.address) > 254:  # RFC 5321 limit
...
class PhoneNumber(ImmutableValueObject):
    ...
    def validate(self) -> ValidationResult:
        ...
        if not re.match(r"^\+?[\d]{7,15}$", self.number):
...
class Percentage(ImmutableValueObject):
    ...
    def as_fraction(self) -> Tuple[int, int]:
        ...
        if decimal_value == 0:
            ...
            # For simplicity, return as percentage over 100
...
# Utility functions for value object creation


def create_money(
    amount: Union[int, float, str, Decimal], currency: str = "USD"
) -> Money:
    """
