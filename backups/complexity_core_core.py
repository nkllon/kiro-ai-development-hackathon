"""
Complexity Core Core

This module was extracted from complexity_core.py
...
class ComplexityMonitor(DomainReflectiveModule):
    ...
    def get_complexity_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_score": sum(
                (r.get_overall_score() for r in self._reports.values())
            )
            / len(self._reports),
            "thresholds": {
    ...
    try:
        ...
        if inspect.isclass(target):
            method_count = len(
                [
                    m
                    for m in inspect.getmembers(target, predicate=inspect.isfunction)
                    if not m[0].startswith("_")
                ]
            )
            metric = ComplexityMetric(
...
def get_complexity_summary(self) -> Dict[str, Any]:
    ...
    return {
        ...
        / len(self._reports),
        "thresholds": {t.value: threshold for t, threshold in self._thresholds.items()},
    }
