"""
Layer processor for DAG orchestration system.

Categorizes specifications by dependency layers and provides
systematic analysis for parallel execution optimization.
"""
...
class LayerProcessingResult:
    ...
    task_layers: Dict[int, List[str]]  # layer -> task_ids
    parallel_opportunities: List[Tuple[int, List[str]]]  # (layer, parallel_task_ids)
...
class LayerProcessor:
    ...
    def __init__(self):
        self.parallel_threshold = 2  # Minimum tasks for parallel consideration
        self.bottleneck_threshold = 0.3  # 30% of total effort in one layer

    def process_layers(
        self, specifications: List[SpecificationNode], constraint_graph: ConstraintGraph
        ...
        """
        # Process specification layers
        spec_layers = self._process_specification_layers(
            specifications, constraint_graph
        )

        # Get task layers from constraint graph
        task_layers = constraint_graph.dependency_layers

        # Identify parallel opportunities
        parallel_opportunities = self._identify_parallel_opportunities(
            task_layers, constraint_graph
        )

        # Identify bottleneck layers
        bottleneck_layers = self._identify_bottleneck_layers(
            task_layers, constraint_graph
        )

        # Identify critical path layers
        critical_path_layers = self._identify_critical_path_layers(
            task_layers, constraint_graph
        )

        return LayerProcessingResult(
            specification_layers=spec_layers,
    ...
    def categorize_specifications_by_layer(
        self, specifications: List[SpecificationNode]
    ) -> Dict[int, List[SpecificationNode]]:
        """
        ...
        """
        # Build specification dependency graph
        spec_graph = {}
        ...
        def calculate_spec_layer(spec_name: str, visited: Set[str]) -> int:
            ...
            if not dependencies:
                ...
                max_dep_layer = max(
                    calculate_spec_layer(dep, visited.copy())
                    for dep in dependencies
                    if dep in spec_graph
                )
        ...
        # Calculate layers for all specifications
        for spec in specifications:
    ...
    def analyze_layer_dependencies(
        self,
        layer_number: int,
        specifications: List[SpecificationNode],
        all_specifications: List[SpecificationNode],
    ) -> List[str]:
        """
        ...
        # Build spec name to spec mapping
        ...
        return list(set(blocking_dependencies))  # Remove duplicates

    def identify_parallel_execution_opportunities(
        self, task_layers: Dict[int, List[str]], constraint_graph: ConstraintGraph
    ...
    def _process_specification_layers(
        self, specifications: List[SpecificationNode], constraint_graph: ConstraintGraph
    ) -> List[SpecificationLayer]:
        ...
        for layer_num in sorted(spec_layers_dict.keys()):
            ...
            # Estimate effort for this layer
            estimated_effort = self._estimate_layer_effort(specs, constraint_graph)

            # Check if layer can start in parallel
            can_start_parallel = self._can_layer_start_parallel(specs, specifications)

            # Identify blocking dependencies
            blocking_dependencies = self.analyze_layer_dependencies(
                layer_num, specs, specifications
            )
            ...
            spec_layers.append(
                SpecificationLayer(
                    ...
                    can_start_parallel=can_start_parallel,
    ...
    def _estimate_layer_effort(
        ...
        for spec in specifications:
            # Find tasks belonging to this specification
            spec_tasks = [
                task
                for task in constraint_graph.nodes.values()
                if task.spec_name == spec.spec_name
            ]

            # Sum effort for incomplete tasks
    ...
    def _identify_parallel_opportunities(
        ...
        return self.identify_parallel_execution_opportunities(
            task_layers, constraint_graph
        )

    def _identify_bottleneck_layers(
        self, task_layers: Dict[int, List[str]], constraint_graph: ConstraintGraph
    ) -> List[int]:
        ...
        for layer, task_ids in task_layers.items():
            # Calculate effort for this layer
            layer_effort = sum(
                constraint_graph.nodes[task_id].estimated_effort
                for task_id in task_ids
                if task_id in constraint_graph.nodes
            )

            # Check if this layer has disproportionate effort
            if (
                total_effort > 0
                and layer_effort / total_effort > self.bottleneck_threshold
            ):
                bottleneck_layers.append(layer)

        return bottleneck_layers

    def _identify_critical_path_layers(
        self, task_layers: Dict[int, List[str]], constraint_graph: ConstraintGraph
    ) -> List[int]:
        ...
        for layer, task_ids in task_layers.items():
            # Check if any task in this layer has high effort and many dependents
            ...
            for task_id in task_ids:
                ...
                # High effort task with many dependents is likely on critical path
    ...
    def _group_parallel_tasks(
        self, task_ids: List[str], constraint_graph: ConstraintGraph
    ) -> List[List[str]]:
        """Group tasks that can run in parallel within a layer."""
        # For now, assume all tasks in the same layer can run in parallel
        # In practice, would need more sophisticated analysis of resource conflicts
