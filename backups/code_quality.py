...
Provides systematic analysis of code quality including syntax,
style, maintainability, and best practices.
"""
...
class CodeQualityExpert(GhostbustersExpertAgent):
    ...
    def __init__(self, name: str = "CodeQualityExpert", version: str = "1.0.0"):
        ...
        # Quality patterns and thresholds
        ...
        try:
            ...
            if file_extension == ".py":
                ...
                recommendations.extend(
                    await self._generate_python_recommendations(findings)
                )
            elif file_extension in [".js", ".ts"]:
                findings.extend(
                    await self._analyze_javascript_code(content, target_path)
                )
                recommendations.extend(
                    await self._generate_javascript_recommendations(findings)
                )
            elif file_extension in [".java"]:
                findings.extend(await self._analyze_java_code(content, target_path))
                recommendations.extend(
                    await self._generate_java_recommendations(findings)
                )
            else:
                # Generic text-based analysis
                findings.extend(await self._analyze_generic_code(content, target_path))
                recommendations.extend(
                    await self._generate_generic_recommendations(findings)
                )
            ...
            result = AnalysisResult(
                ...
                metadata={
                    ...
                    "analysis_types": self._get_analysis_types_performed(
                        file_extension
                    ),
                },
            )
            ...
            logger.info(
                f"Code quality analysis completed for {target_path} with {len(findings)} findings"
            )
            return result

        except Exception as e:
            logger.error(
                f"Code quality analysis failed for {context.target_path}: {str(e)}"
            )
            ...
            return AnalysisResult(
                ...
                recommendations=[
                    Recommendation(
                        ...
                        description=f"Resolve the issue preventing analysis: {str(e)}",
    ...
    def validate_confidence(self, result: AnalysisResult) -> bool:
        ...
        if result.confidence > 0.8:
            return (
                len(result.findings) > 0 or result.metadata.get("lines_analyzed", 0) > 0
            )

        # Low confidence acceptable for failed analysis
        ...
        try:
            ...
            findings.append(
                Finding(
                    type=FindingType.SYNTAX_ERROR,
                    severity=Severity.CRITICAL,
                    location=CodeLocation(str(file_path), e.lineno or 1, e.offset),
                ...
                if complexity > 10:
                    self.findings.append(
                        Finding(
                            ...
                            confidence=0.7,
                            evidence={"complexity": complexity, "threshold": 10},
                        )
                    )
                ...
                if not ast.get_docstring(node):
                    self.findings.append(
                        Finding(
                            ...
                            confidence=0.9,
                            evidence={"missing_docstring": True},
                        )
                    )
                ...
                findings.append(
                    Finding(
                        ...
                        evidence={
                            ...
                            "threshold": self._line_length_threshold,
                        },
                    )
                )
            ...
            if line.endswith(" ") or line.endswith("\t"):
                findings.append(
                    Finding(
                        ...
                        confidence=0.95,
                        evidence={"trailing_whitespace": True},
                    )
                )
            ...
            if re.search(r"\b(TODO|FIXME|HACK|XXX)\b", line, re.IGNORECASE):
                findings.append(
                    Finding(
                        ...
                        confidence=0.8,
                        evidence={"comment_type": "todo_fixme"},
                    )
                )
    ...
    async def _analyze_javascript_code(
        ...
        # Basic JavaScript analysis (would use proper parser in production)
        lines = content.splitlines()
    ...
    async def _analyze_generic_code(
        ...
        for line_num, line in enumerate(lines, 1):
            # Check line length
            if (
                len(line) > self._line_length_threshold * 1.5
            ):  # More lenient for generic files
                findings.append(
                    Finding(
                        type=FindingType.QUALITY_ISSUE,
                        severity=Severity.INFO,
                        location=CodeLocation(str(file_path), line_num),
                        description=f"Very long line ({len(line)} characters)",
                        confidence=0.6,
                        evidence={"line_length": len(line)},
                    )
                )
    ...
    async def _generate_python_recommendations(
        self, findings: List[Finding]
    ) -> List[Recommendation]:
        """Generate Python-specific recommendations"""
        recommendations = []

        # Group findings by type for better recommendations
        ...
        if syntax_errors:
            recommendations.append(
                Recommendation(
                    ...
                    description=f"Fix {len(syntax_errors)} syntax error(s) to enable proper analysis",
                    priority=Severity.CRITICAL,
        ...
        # Recommendations for common quality issues
        long_functions = [
            f
            for f in quality_issues
            if "too long" in f.description and "Function" in f.description
        ]
        if long_functions:
            recommendations.append(
                Recommendation(
                    title="Refactor Long Functions",
                    description=f"Break down {len(long_functions)} long function(s) into smaller, more manageable pieces",
                    priority=Severity.MEDIUM,
                    effort_estimate="30-60 minutes per function",
                    automated_fix_available=False,
                )
            )

        missing_docstrings = [
            f for f in quality_issues if "missing docstring" in f.description
        ]
        if missing_docstrings:
            recommendations.append(
                Recommendation(
                    title="Add Documentation",
                    description=f"Add docstrings to {len(missing_docstrings)} function(s)/class(es)",
                    priority=Severity.LOW,
                    effort_estimate="5-10 minutes per item",
                    automated_fix_available=True,
                    fix_command="add_docstrings",
                )
            )

        return recommendations

    async def _generate_javascript_recommendations(
        self, findings: List[Finding]
    ) -> List[Recommendation]:
        ...
        if console_logs:
            recommendations.append(
                Recommendation(
                    ...
                    description=f"Remove {len(console_logs)} console.log statement(s) before production",
                    priority=Severity.LOW,
                    effort_estimate="2-5 minutes",
                    automated_fix_available=True,
                    fix_command="remove_console_logs",
                )
            )
        ...
        if var_usage:
            recommendations.append(
                Recommendation(
                    ...
                    description=f"Replace {len(var_usage)} 'var' declaration(s) with 'let' or 'const'",
                    priority=Severity.MEDIUM,
                    effort_estimate="1-2 minutes per declaration",
                    automated_fix_available=True,
                    fix_command="replace_var_with_let_const",
                )
            )

        return recommendations

    async def _generate_java_recommendations(
        self, findings: List[Finding]
    ) -> List[Recommendation]:
        ...
        debug_statements = [
            f for f in findings if "System.out.println" in f.description
        ]
        if debug_statements:
            recommendations.append(
                Recommendation(
                    title="Replace Debug Statements with Logging",
                    description=f"Replace {len(debug_statements)} System.out.println statement(s) with proper logging",
                    priority=Severity.LOW,
                    effort_estimate="5-10 minutes",
                    automated_fix_available=True,
                    fix_command="replace_sysout_with_logging",
                )
            )

        return recommendations

    async def _generate_generic_recommendations(
        self, findings: List[Finding]
    ) -> List[Recommendation]:
        ...
        if whitespace_issues:
            recommendations.append(
                Recommendation(
                    ...
                    description=f"Remove trailing whitespace from {len(whitespace_issues)} line(s)",
                    priority=Severity.LOW,
                    effort_estimate="1-2 minutes",
                    automated_fix_available=True,
                    fix_command="trim_whitespace",
                )
            )
    ...
    def _calculate_confidence(
        self, findings: List[Finding], content: str, file_extension: str
    ) -> float:
        """Calculate confidence score for analysis"""
        base_confidence = 0.7

        # Adjust based on file type support
        if file_extension == ".py":
            base_confidence = 0.9  # Strong Python support
        elif file_extension in [".js", ".ts"]:
            base_confidence = 0.7  # Basic JavaScript support
        elif file_extension == ".java":
            base_confidence = 0.6  # Basic Java support
        else:
            base_confidence = 0.4  # Generic analysis only
        ...
        # Higher confidence for files with findings (shows analysis worked)
        if findings:
            finding_confidence = sum(f.confidence for f in findings) / len(findings)
            base_confidence = (base_confidence + finding_confidence) / 2

        # Adjust for file size (very large files might have incomplete analysis)
        if lines_count > 1000:
            base_confidence *= 0.9
