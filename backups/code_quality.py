...
class CodeQualityExpert(GhostbustersExpertAgent):
        ...
        try:
            ...
            result = AnalysisResult(
                ...
                metadata={
                    ...
                    "analysis_types": self._get_analysis_types_performed(file_extension)
                }
            )
                ...
                if hasattr(node, 'end_lineno') and node.end_lineno:
                    ...
                    if func_length > 50:  # threshold
                        self.findings.append(Finding(
                            ...
                            confidence=0.8,
                            evidence={"function_length": func_length, "threshold": 50}
                        ))
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            if len(line) > self._line_length_threshold:
                findings.append(Finding(
                    ...
                    confidence=0.9,
                    evidence={"line_length": len(line), "threshold": self._line_length_threshold}
                ))
    ...
    async def _analyze_generic_code(self, content: str, file_path: Path) -> List[Finding]:
        ...
        for line_num, line in enumerate(lines, 1):
            # Check line length
            if len(line) > self._line_length_threshold * 1.5:  # More lenient for generic files
                findings.append(Finding(
                    type=FindingType.QUALITY_ISSUE,
                    severity=Severity.INFO,
                    location=CodeLocation(str(file_path), line_num),
