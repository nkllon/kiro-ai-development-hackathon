...
class ContextMapping:
    ...
    data_flow_direction: str = (
        "bidirectional"  # bidirectional, source_to_target, target_to_source
    )
...
class DomainAdapter(DomainReflectiveModule, Generic[ExternalType, DomainType]):
    ...
    async def adapt_from_external(self, external_data: ExternalType) -> DomainType:
        ...
        try:
            ...
            validation_result = self.translator.validate_translation(
                external_data, domain_model
            )
    ...
    async def get_module_status(self):
        ...
        total_adaptations = (
            self._adaptation_metrics["successful_adaptations"]
            + self._adaptation_metrics["failed_adaptations"]
        )
        success_rate = 0.0
        if total_adaptations > 0:
            success_rate = (
                self._adaptation_metrics["successful_adaptations"] / total_adaptations
            )
