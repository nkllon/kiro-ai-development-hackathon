    ...
    async def _generate_performance_recommendations(self, findings: List[Finding]) -> List[Recommendation]:
        ...
        for issue_type, issue_findings in issue_types.items():
            if issue_type == 'nested_loops':
                ...
                recommendations.append(Recommendation(title='Optimize String Operations', description=f'Replace {len(issue_findings)} string concatenation(s) with join() or f-strings', priority=Severity.MEDIUM, effort_estimate='15-30 minutes', automated_fix_available=True, fix_command='optimize_string_concat'))
