...
This module was extracted from performance_core.py
...
class PerformanceExpert(GhostbustersExpertAgent):
    ...
    def _init_performance_patterns(self):
        ...
        self.db_patterns = {
            ...
            "missing_indexes": ["\\.filter\\s*\\(\\s*\\w+\\s*=", "WHERE\\s+\\w+\\s*="],
            "select_star": ["SELECT\\s+\\*\\s+FROM", "\\.all\\s*\\(\\s*\\)"],
        }
        self.memory_patterns = {
            ...
            "large_objects": [
                "range\\s*\\(\\s*\\d{6,}",
                "\\[\\s*.*\\s*for\\s+.*\\s+in\\s+range\\s*\\(\\s*\\d{4,}",
            ],
        }
        self.io_patterns = {
            "synchronous_io": [
        ...
        self.concurrency_patterns = {
            ...
            "race_conditions": ["global\\s+\\w+", "threading\\.Thread.*target"],
        }
    ...
    async def _analyze_javascript_performance(
        ...
        js_patterns = {
            "dom_queries_in_loop": [
                "for\\s*\\(.*\\)\\s*\\{[^}]*document\\.querySelector"
            ],
            "inefficient_array_methods": ["\\.forEach\\s*\\([^)]*\\.push"],
            "memory_leaks": ["setInterval\\s*\\(", "addEventListener\\s*\\("],
            "blocking_operations": ["alert\\s*\\(", "confirm\\s*\\(", "prompt\\s*\\("],
        }
    ...
    async def _analyze_sql_performance(
        ...
        sql_patterns = {
            ...
            "cartesian_join": ["FROM\\s+\\w+\\s*,\\s*\\w+", "Potential cartesian join"],
            "function_in_where": [
                "WHERE\\s+\\w+\\s*\\(\\s*\\w+\\s*\\)",
