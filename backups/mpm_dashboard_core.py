...
This module was extracted from mpm_dashboard.py
...
class MPMDashboard:
    ...
    def get_portfolio_status(self) -> PortfolioStatus:
        ...
        try:
            ...
            for track in StrategicTrack:
                track_items = [
                    item for item in self._backlog_items.values() if item.track == track
                ]
                track_breakdown[track] = {
                    ...
                    "beast_ready": sum(
                        (
                            ...
                            if item.beast_readiness_status
                            == BeastReadinessStatus.BEAST_READY
                        )
                    ),
                    "in_progress": sum(
                        (
                            ...
                            for item in track_items
                            if item.beast_readiness_status
                            == BeastReadinessStatus.IN_EXECUTION
                        )
                    ),
                    "completed": sum(
                        (
                            ...
                            for item in track_items
                            if item.beast_readiness_status
                            == BeastReadinessStatus.COMPLETED
                        )
                    ),
                    "blocked": sum(
                        (
        ...
        try:
            ...
            if audience == StakeholderType.MPM:
                ...
                report = self._generate_generic_report(
                    report_id, portfolio_status, audience
                )
            if custom_sections:
    ...
    def perform_strategic_reprioritization(
        self, changes: List[PriorityChange]
    ) -> ImpactAnalysis:
        """
        ...
        try:
            ...
            for change in changes:
                ...
                resource_impact[change.item_id] = self._calculate_resource_impact(
                    change
                )
                ...
                item_recommendations = self._generate_mitigation_recommendations(
                    change, dependency_impact
                )
                mitigation_recommendations.extend(item_recommendations)
            confidence_score = self._calculate_reprioritization_confidence(
                changes, affected_items
            )
    ...
    def run_scenario_planning(
        ...
        start_time = time.time()
        try:
            scenario_params = self._generate_scenario_parameters(
                scenario_type, constraints
            )
            estimated_completion = self._calculate_scenario_timeline(
                scenario_params, constraints
            )
            resource_utilization = self._calculate_resource_utilization(
                scenario_params, constraints
            )
            deliverable_timeline = self._calculate_track_timelines(
                scenario_params, constraints
            )
            risk_profile = self._assess_scenario_risks(scenario_params, constraints)
            success_probability = self._calculate_success_probability(
                scenario_params, risk_profile
            )
            key_assumptions = self._generate_scenario_assumptions(
                scenario_type, constraints
            )
    ...
    def _get_critical_path_items(self) -> List[str]:
        ...
        try:
            ...
            for item_id, item in self._backlog_items.items():
                if item.beast_readiness_status in [
                    BeastReadinessStatus.BLOCKED,
                    BeastReadinessStatus.DRAFT,
                ]:
                    has_dependents = any(
                        (
    ...
    def _calculate_delivery_confidence(self) -> float:
        ...
        confidence = max(
            0.0, min(1.0, readiness_score - blocked_penalty + validation_bonus)
        )
        return confidence
    ...
    def _update_performance_metrics(self, operation: str, start_time: float) -> None:
        ...
        total_ops = sum(
            (
                ...
                for key in ["portfolio_queries", "impact_analyses", "scenario_plans"]
    ...
    def _generate_mpm_report(
        self, report_id: str, portfolio_status: PortfolioStatus
    ) -> StakeholderReport:
        ...
        return StakeholderReport(
            ...
            executive_summary=f"Portfolio contains {portfolio_status.total_items} items across 4 strategic tracks. {portfolio_status.beast_ready_items} items are beast-ready ({portfolio_status.beast_ready_items / portfolio_status.total_items * 100:.1f}%). Delivery confidence: {portfolio_status.delivery_confidence:.1%}.",
            key_metrics={
                "beast_readiness_rate": (
                    portfolio_status.beast_ready_items / portfolio_status.total_items
                    if portfolio_status.total_items > 0
                    else 0
                ),
    ...
    def _generate_executive_report(
        ...
        return StakeholderReport(
            ...
            executive_summary=f"Strategic development is {portfolio_status.completion_percentage:.1f}% complete. Delivery confidence is {portfolio_status.delivery_confidence:.1%} with {portfolio_status.blocked_items} items requiring attention.",
            key_metrics={
                "overall_progress": portfolio_status.completion_percentage,
                "delivery_confidence": portfolio_status.delivery_confidence,
                "items_at_risk": portfolio_status.blocked_items,
                "tracks_on_schedule": sum(
                    (
                        1
                        for track_data in portfolio_status.track_breakdown.values()
            ...
            detailed_sections={
                "strategic_tracks": {
                    track.value: {
                        ...
                        "status": (
                            "on_track" if track_data["blocked"] == 0 else "at_risk"
                        ),
                    }
    ...
    def _generate_developer_report(
        ...
        return StakeholderReport(
            ...
            executive_summary=f"{portfolio_status.beast_ready_items} items are ready for pickup. {portfolio_status.in_progress_items} items currently in development. Focus on critical path items to unblock dependencies.",
            key_metrics={
    ...
    def _generate_operations_report(
        self, report_id: str, portfolio_status: PortfolioStatus
    ) -> StakeholderReport:
        ...
        return StakeholderReport(
            ...
            executive_summary=f"System processing {portfolio_status.total_items} items with {self._performance_metrics['avg_response_time_ms']:.1f}ms avg response time. Cache hit rate: {self._performance_metrics['cache_hit_rate']:.1%}.",
            key_metrics={
    ...
    def _generate_generic_report(
        ...
        return StakeholderReport(
            ...
            executive_summary=f"Portfolio status: {portfolio_status.completion_percentage:.1f}% complete, {portfolio_status.beast_ready_items} items ready for execution.",
            key_metrics={
            ...
            recommendations=[
                "Review portfolio status regularly",
                "Focus on unblocking critical items",
            ],
            next_steps=[
                "Contact MPM for detailed planning",
                "Review specific track progress",
            ],
            generated_at=datetime.now(),
            valid_until=datetime.now() + timedelta(days=1),
        )

    def _generate_mpm_recommendations(
        ...
        if portfolio_status.blocked_items > 0:
            recommendations.append(
                f"Address {portfolio_status.blocked_items} blocked items to improve delivery confidence"
            )
        if portfolio_status.delivery_confidence < 0.7:
            recommendations.append(
                "Improve beast-readiness validation to increase delivery confidence"
            )
        if len(portfolio_status.critical_path_items) > 5:
            recommendations.append(
                "Focus on critical path items to reduce delivery risk"
            )
        for track, track_data in portfolio_status.track_breakdown.items():
            if track_data["blocked"] > track_data["total"] * 0.2:
                recommendations.append(
                    f"Prioritize unblocking {track.value} track items"
                )
    ...
    def _generate_mpm_next_steps(self, portfolio_status: PortfolioStatus) -> List[str]:
        ...
        if portfolio_status.critical_path_items:
            next_steps.append(
                "Review critical path items and resolve blocking dependencies"
            )
        next_steps.append(
            "Conduct Ghostbusters validation for items in MPM_REVIEW status"
        )
        next_steps.append(
            "Update stakeholder reports and communicate delivery timeline changes"
        )
        if portfolio_status.delivery_confidence < 0.8:
            next_steps.append(
                "Run scenario planning to identify resource allocation improvements"
            )
        return next_steps
    ...
    def _generate_executive_recommendations(
        ...
        if portfolio_status.delivery_confidence < 0.8:
            recommendations.append(
                "Consider additional resources to improve delivery confidence"
            )
        if portfolio_status.blocked_items > portfolio_status.total_items * 0.1:
            recommendations.append(
                "Escalate dependency resolution to unblock strategic progress"
            )
        return recommendations
    ...
    def _generate_developer_recommendations(
        ...
        if portfolio_status.beast_ready_items > 0:
            recommendations.append(
                f"Pick up any of {portfolio_status.beast_ready_items} beast-ready items"
            )
        if portfolio_status.critical_path_items:
            recommendations.append(
                "Prioritize critical path items to unblock dependent work"
            )
        return recommendations

    def _generate_developer_next_steps(
        self, portfolio_status: PortfolioStatus
    ) -> List[str]:
    ...
    def _analyze_dependency_impact(self, change: PriorityChange) -> Dict[str, Any]:
        ...
        return {
            "timeline_days": abs(change.new_priority - change.old_priority) * 2,
    ...
    def _assess_priority_risk_change(
        self, change: PriorityChange
    ) -> Dict[RiskLevel, int]:
    ...
    def _generate_mitigation_recommendations(
        self, change: PriorityChange, dependency_impact: Dict[str, Any]
    ) -> List[str]:
        """Generate mitigation recommendations for priority change"""
        recommendations = []
        if dependency_impact["timeline_days"] > 7:
            recommendations.append(
                f"Consider parallel work streams to mitigate {dependency_impact['timeline_days']} day delay"
            )
        if change.new_priority < change.old_priority:
            recommendations.append(
                "Communicate priority reduction to affected stakeholders"
            )
        return recommendations

    def _calculate_reprioritization_confidence(
        self, changes: List[PriorityChange], affected_items: List[str]
    ) -> float:
        """Calculate confidence score for reprioritization"""
        total_priority_delta = sum(
            (abs(change.new_priority - change.old_priority) for change in changes)
        )
    ...
    def _generate_scenario_parameters(
        self, scenario_type: ScenarioType, constraints: ResourceConstraints
    ) -> Dict[str, Any]:
        """Generate parameters for scenario planning"""
        base_velocity = (
            constraints.available_developers * constraints.available_hours_per_week / 40
        )
    ...
    def _calculate_scenario_timeline(
        self, scenario_params: Dict[str, Any], constraints: ResourceConstraints
    ) -> datetime:
        ...
        weeks_needed *= 1 + scenario_params["risk_factor"]
    ...
    def _calculate_track_timelines(
        self, scenario_params: Dict[str, Any], constraints: ResourceConstraints
    ) -> Dict[StrategicTrack, datetime]:
        ...
        for track in StrategicTrack:
            ...
            remaining_items = sum(
                (
                    ...
                    for item in track_items
                    if item.beast_readiness_status
                    not in [BeastReadinessStatus.COMPLETED]
                )
            )
            weeks_needed = remaining_items / (scenario_params["velocity"] / 4)
    ...
    def _calculate_success_probability(
        self, scenario_params: Dict[str, Any], risk_profile: Dict[RiskLevel, float]
    ) -> float:
    ...
    def _identify_critical_dependencies(
        ...
        return [
            item_id
            for item_id in self._get_critical_path_items()
            if self._backlog_items[item_id].beast_readiness_status
            != BeastReadinessStatus.BEAST_READY
        ]
...
def generate_stakeholder_report(
    self, audience: StakeholderType, custom_sections: Optional[Dict[str, Any]] = None
) -> StakeholderReport:
    """
    Generate tailored report for specific stakeholder audience
...
def _calculate_validation_quality_bonus(self) -> float:
    """Calculate bonus confidence from MPM validation quality"""
    validated_items = [
        item for item in self._backlog_items.values() if item.mpm_validation is not None
    ]
    if not validated_items:
        return 0.0
    avg_completeness = sum(
...
def _generate_generic_report(
    self, report_id: str, portfolio_status: PortfolioStatus, audience: StakeholderType
) -> StakeholderReport:
    """Generate generic report for other audiences"""
    return StakeholderReport(
        report_id=report_id,
