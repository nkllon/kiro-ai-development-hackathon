"""
Rca Engine Services Services

This module was extracted from rca_engine_services.py
...
class RCAEngine(ReflectiveModule):
    ...
    def get_health_indicators(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "performance": {
                "status": (
                    "healthy"
                    if self.total_analysis_time / max(1, self.rca_count) < 30
                    else "degraded"
                ),
                "average_analysis_time": self.total_analysis_time
                / max(1, self.rca_count),
                "pattern_matching_performance": (
                    "sub_second" if len(self.pattern_library) < 10000 else "optimized"
                ),
            },
        }
    ...
    def perform_systematic_rca(self, failure: Failure) -> RCAResult:
        ...
        try:
            self.logger.info(
                f"Starting systematic RCA for failure: {failure.failure_id}"
            )
            ...
            prevention_patterns = self.document_prevention_patterns(
                failure, root_causes, systematic_fixes
            )
            analysis_time = time.time() - start_time
            self.total_analysis_time += analysis_time
            rca_confidence = self._calculate_rca_confidence(
                analysis_result, root_causes, validation_results
            )
            ...
            self.logger.info(
                f"RCA complete: {len(root_causes)} root causes, {len(systematic_fixes)} fixes, confidence: {rca_confidence:.2f}"
            )
            return rca_result
    ...
    def analyze_comprehensive_factors(
        ...
        try:
            self.logger.info(
                f"Analyzing comprehensive factors for failure: {failure.failure_id}"
            )
            analysis_results = {}
            for component_name, analyzer in self.analysis_components.items():
                try:
                    ...
                    self.logger.warning(
                        f"Analysis component {component_name} failed: {e}"
                    )
                    analysis_results[component_name] = {
                        "error": str(e),
                        "status": "failed",
                    }
            symptoms = analysis_results.get("symptoms", {}).get(
                "identified_symptoms", []
            )
    ...
    def implement_systematic_fixes(
        ...
        for root_cause in root_causes:
            try:
                ...
                self.logger.info(
                    f"Generated systematic fix for {root_cause.cause_type}: {fix.fix_description}"
                )
            except Exception as e:
                self.logger.error(
                    f"Failed to generate fix for {root_cause.cause_type}: {e}"
                )
        return systematic_fixes

    def validate_root_cause_addressed(
        self, fix: SystematicFix, original_failure: Failure
    ) -> ValidationResult:
        """
        ...
        try:
            ...
            for criteria in fix.validation_criteria:
                try:
                    validation_result = self._execute_validation_criteria(
                        criteria, original_failure
                    )
                    validation_evidence.append(
                        f"Criteria '{criteria}': {validation_result['status']}"
                    )
                    if validation_result["status"] == "passed":
                        symptoms_resolved.extend(
                            validation_result.get("resolved_symptoms", [])
                        )
                    else:
                        remaining_issues.extend(
                            validation_result.get("remaining_issues", [])
                        )
                except Exception as e:
            ...
            confidence_score = len(symptoms_resolved) / max(
                1, len(symptoms_resolved) + len(remaining_issues)
            )
    ...
    def document_prevention_patterns(
        self, failure: Failure, root_causes: List[RootCause], fixes: List[SystematicFix]
        ...
        for root_cause, fix in zip(root_causes, fixes):
            try:
                ...
                self.logger.info(
                    f"Documented prevention pattern: {pattern.pattern_name}"
                )
            except Exception as e:
    ...
    def match_existing_patterns(self, failure: Failure) -> List[PreventionPattern]:
        ...
        try:
            ...
            self.logger.info(
                f"Pattern matching completed in {match_time:.3f}s, found {len(matching_patterns)} matches"
            )
            if match_time > 1.0:
                self.logger.warning(
                    f"Pattern matching exceeded 1 second: {match_time:.3f}s"
                )
                ...
                categorization.update(
                    {
                        ...
                        "confidence": 0.7,
                        "analysis_details": self._analyze_infrastructure_details(
                            failure
                        ),
                    }
                )
            elif (
                failure.component.startswith("test:")
                or "test" in failure.component.lower()
                or (failure.context and "test_file" in failure.context)
            ):
                categorization.update(
                    {
                        ...
                        "confidence": 0.5,
                        "analysis_details": {
                            "error": "Could not categorize test failure specifically"
                        },
                    }
                )
            self.logger.info(
                f"Test failure categorized as: {categorization['primary_category']}/{categorization['subcategory']}"
            )
    ...
    def generate_test_specific_systematic_fixes(
        self, root_causes: List[RootCause]
    ) -> List[SystematicFix]:
        """
        ...
        for root_cause in root_causes:
            try:
                if root_cause.cause_type in [
                ...
                elif root_cause.cause_type in [
                    RootCauseType.MAKEFILE_ERROR,
                    RootCauseType.BUILD_DEPENDENCY_ERROR,
                ]:
                    ...
                    test_specific_fixes.append(fix)
                self.logger.info(
                    f"Generated test-specific fix for {root_cause.cause_type}"
                )
            except Exception as e:
                self.logger.error(
                    f"Failed to generate test-specific fix for {root_cause.cause_type}: {e}"
                )
        return test_specific_fixes

    def add_test_specific_patterns_to_library(
        self, failure: Failure, root_causes: List[RootCause], fixes: List[SystematicFix]
    ) -> List[PreventionPattern]:
        """
    ...
    def _analyze_dependencies(self, failure: Failure) -> Dict[str, Any]:
        ...
        if "python" in failure.component.lower():
            try:
                ...
                dependency_analysis["pip_package_count"] = (
    ...
    def _analyze_installation_integrity(self, failure: Failure) -> Dict[str, Any]:
        ...
        try:
            ...
            installation_analysis["python_version"] = subprocess.run(
        ...
        try:
            is_test_failure = (
                ...
                in [
                    ...
                    FailureCategory.MAKE_TARGET_FAILURE,
                    FailureCategory.INFRASTRUCTURE_FAILURE,
                    FailureCategory.TEST_ENVIRONMENT_FAILURE,
                ]
                or self._is_pytest_failure(failure)
                or self._is_make_failure(failure)
                or self._is_infrastructure_failure(failure)
            )
            if is_test_failure:
                test_analysis["is_test_failure"] = True
                test_analysis["test_categorization"] = (
                    self.analyze_test_failure_categorization(failure)
                )
                if failure.context and "test_file" in failure.context:
                    test_analysis["test_file"] = failure.context["test_file"]
                    test_analysis["test_function"] = failure.context.get(
                        "test_function", "unknown"
                    )
                    test_analysis["pytest_node_id"] = failure.context.get(
                        "pytest_node_id", "unknown"
                    )
                test_analysis["test_environment"] = self._analyze_test_environment(
                    failure
                )
            else:
    ...
    def _analyze_pytest_failures(self, failure: Failure) -> Dict[str, Any]:
        ...
        try:
            if self._is_pytest_failure(failure):
                ...
                pytest_analysis["import_analysis"] = self._analyze_import_issues(
                    failure
                )
                pytest_analysis["dependency_analysis"] = (
                    self._analyze_test_dependencies(failure)
                )
                pytest_analysis["syntax_analysis"] = self._analyze_syntax_issues(
                    failure
                )
                pytest_analysis["test_structure"] = self._analyze_test_structure(
                    failure
                )
    ...
    def _analyze_makefile_failures(self, failure: Failure) -> Dict[str, Any]:
        ...
        try:
            if self._is_make_failure(failure):
                makefile_analysis["makefile_issues"] = self._analyze_makefile_issues(
                    failure
                )
                makefile_analysis["missing_files"] = self._analyze_missing_files(
                    failure
                )
                makefile_analysis["build_dependencies"] = (
                    self._analyze_build_dependencies(failure)
                )
                makefile_analysis["target_analysis"] = self._analyze_make_targets(
                    failure
                )
    ...
    def _analyze_infrastructure_failures(self, failure: Failure) -> Dict[str, Any]:
        ...
        try:
            if self._is_infrastructure_failure(failure):
                infrastructure_analysis["system_config"] = (
                    self._analyze_system_configuration(failure)
                )
                infrastructure_analysis["permissions"] = self._analyze_permissions(
                    failure
                )
                infrastructure_analysis["environment"] = (
                    self._analyze_infrastructure_environment(failure)
                )
                infrastructure_analysis["resources"] = (
                    self._analyze_resource_availability(failure)
                )
    ...
    def _identify_root_causes(
        self, failure: Failure, analysis: ComprehensiveAnalysisResult
    ) -> List[RootCause]:
        """Identify root causes from comprehensive analysis including test-specific causes"""
        root_causes = []
        test_analysis = {}
        if (
            hasattr(analysis, "environmental_factors")
            and analysis.environmental_factors
        ):
            test_analysis = analysis.environmental_factors.get("test_specific", {})
        is_test_failure = (
            test_analysis.get("is_test_failure", False)
            or failure.component.startswith("test:")
            or "test" in failure.component.lower()
            or (
                failure.category
                in [
        ...
        if is_test_failure:
            test_root_causes = self._identify_test_specific_root_causes(
                failure, analysis
            )
            ...
            if not analysis.tool_health_status.get("makefiles_dir_exists", True):
                root_causes.append(
                    RootCause(
                        ...
                        evidence=[
                            "makefiles/ directory does not exist",
                            "make help fails with include errors",
                        ],
    ...
    def _execute_validation_criteria(
        self, criteria: str, original_failure: Failure
    ) -> Dict[str, Any]:
        ...
        if "makefiles/ directory exists" in criteria:
            ...
            return {
                ...
                "remaining_issues": (
                    [] if exists else ["makefiles/ directory still missing"]
                ),
            }
            ...
            try:
                ...
                return {
                    ...
                    "remaining_issues": (
                        [] if success else [f"make help failed: {result.stderr}"]
                    ),
                }
            except Exception as e:
                return {
    ...
    def _create_prevention_pattern(
        self, failure: Failure, root_cause: RootCause, fix: SystematicFix
    ) -> PreventionPattern:
    ...
    def _verify_pattern_match(
        self, failure: Failure, pattern: PreventionPattern
    ) -> bool:
        """Verify if failure matches existing pattern"""
        failure_signature = self._generate_failure_signature(failure)
        return (
            failure.component in pattern.failure_signature
            and failure.category.value in pattern.failure_signature
        )
    ...
    def _calculate_analysis_confidence(self, analysis_results: Dict[str, Any]) -> float:
        ...
        successful_analyses = sum(
            (1 for result in analysis_results.values() if "error" not in result)
        )
    ...
    def _calculate_rca_confidence(
        ...
        root_cause_confidence = sum((rc.confidence_score for rc in root_causes)) / max(
            1, len(root_causes)
        )
        validation_confidence = sum((vr.confidence_score for vr in validations)) / max(
            1, len(validations)
        )
        ...
        try:
            if Path(self.pattern_library_path).exists():
                ...
                for pattern_data in data.get("patterns", []):
                    ...
                    self.pattern_index[pattern.pattern_hash].append(pattern.pattern_id)
                self.logger.info(
                    f"Loaded {len(self.pattern_library)} patterns from library"
                )
        except Exception as e:
    ...
    def _is_pytest_failure(self, failure: Failure) -> bool:
        ...
        return (
            "pytest" in failure.error_message.lower()
            or "test_" in failure.component
            or failure.context.get("pytest_node_id") is not None
            or ("ImportError" in failure.error_message)
            or ("AssertionError" in failure.error_message)
        )

    def _is_make_failure(self, failure: Failure) -> bool:
        """Check if failure is make-related"""
        return (
            "make" in failure.component.lower()
            or "Makefile" in failure.error_message
            or "No rule to make target" in failure.error_message
            or ("missing separator" in failure.error_message)
        )

    def _is_infrastructure_failure(self, failure: Failure) -> bool:
        """Check if failure is infrastructure-related"""
        return (
            "PermissionError" in failure.error_message
            or "ConnectionError" in failure.error_message
            or "system" in failure.component.lower()
            or ("environment" in failure.error_message.lower())
        )
    ...
    def _analyze_test_environment(self, failure: Failure) -> Dict[str, Any]:
        ...
        env_analysis = {}
        try:
            env_analysis["python_available"] = (
                subprocess.run(["python3", "--version"], capture_output=True).returncode
                == 0
            )
            env_analysis["pytest_available"] = (
                subprocess.run(
                    ["python3", "-m", "pytest", "--version"], capture_output=True
                ).returncode
                == 0
            )
    ...
    def _analyze_python_issues(self, failure: Failure) -> Dict[str, Any]:
        ...
        if failure.stack_trace:
            if "SyntaxError" in failure.stack_trace:
                python_issues["syntax_errors"].append(
                    "SyntaxError detected in stack trace"
                )
            if "ImportError" in failure.stack_trace:
                python_issues["import_errors"].append(
                    "ImportError detected in stack trace"
                )
            ...
            if "RuntimeError" in failure.stack_trace:
                python_issues["runtime_errors"].append(
                    "RuntimeError detected in stack trace"
                )
    ...
    def _analyze_import_issues(self, failure: Failure) -> Dict[str, Any]:
        ...
        if (
            ...
            if "No module named" in failure.error_message:
                import_analysis["missing_module"] = (
                    failure.error_message.split("No module named")[1]
                    .strip()
                    .strip("'\"")
                )
            import_analysis["relative_import_issue"] = (
                "relative import" in failure.error_message.lower()
            )
        else:
    ...
    def _analyze_syntax_issues(self, failure: Failure) -> Dict[str, Any]:
        ...
        syntax_analysis = {}
        if "SyntaxError" in failure.error_message or (
            failure.stack_trace and "SyntaxError" in failure.stack_trace
        ):
    ...
    def _analyze_test_structure(self, failure: Failure) -> Dict[str, Any]:
        ...
        if failure.context and "test_file" in failure.context:
            ...
            structure_analysis["follows_naming_convention"] = test_file.startswith(
                "test_"
            ) or test_file.endswith("_test.py")
    ...
    def _analyze_missing_files(self, failure: Failure) -> Dict[str, Any]:
        ...
        if "No such file" in failure.error_message:
            ...
            if "No such file or directory:" in failure.error_message:
                missing_files["missing_file"] = failure.error_message.split(
    ...
    def _analyze_build_dependencies(self, failure: Failure) -> Dict[str, Any]:
        ...
        build_deps = {}
        try:
            build_deps["make_available"] = (
                subprocess.run(["which", "make"], capture_output=True).returncode == 0
            )
            build_deps["gcc_available"] = (
                subprocess.run(["which", "gcc"], capture_output=True).returncode == 0
            )
            build_deps["python_available"] = (
                subprocess.run(["which", "python3"], capture_output=True).returncode
                == 0
            )
        except Exception as e:
    ...
    def _analyze_permissions(self, failure: Failure) -> Dict[str, Any]:
        ...
        if (
            "PermissionError" in failure.error_message
            or "Permission denied" in failure.error_message
        ):
    ...
    def _analyze_resource_availability(self, failure: Failure) -> Dict[str, Any]:
        ...
        resource_analysis = {}
        if (
            "MemoryError" in failure.error_message
            or "resource" in failure.error_message.lower()
        ):
    ...
    def _identify_test_specific_root_causes(
        self, failure: Failure, analysis: ComprehensiveAnalysisResult
    ) -> List[RootCause]:
            ...
            if "AssertionError" in failure.error_message:
                test_root_causes.append(
                    RootCause(
                        ...
                        evidence=[
                            "AssertionError in test execution",
                            failure.error_message,
                        ],
                        confidence_score=0.8,
                        impact_severity="medium",
            ...
            if "fixture" in failure.error_message.lower():
                test_root_causes.append(
                    RootCause(
                        ...
                        evidence=[
                            "Fixture error in test execution",
                            failure.error_message,
                        ],
                        confidence_score=0.8,
                        impact_severity="medium",
            ...
            if "PermissionError" in failure.error_message:
                test_root_causes.append(
                    RootCause(
                        ...
                        evidence=[
                            "Permission error in system operation",
                            failure.error_message,
                        ],
                        confidence_score=0.8,
                        impact_severity="high",
    ...
    def _generate_infrastructure_specific_fix(
    ...
    def _create_test_specific_pattern(
        self, failure: Failure, root_cause: RootCause, fix: SystematicFix
    ) -> PreventionPattern:
    ...
    def _add_test_pattern_to_library(self, pattern: PreventionPattern):
        ...
        self._add_pattern_to_library(pattern)
        self.logger.info(
            f"Added test-specific pattern to library: {pattern.pattern_id}"
        )

    def analyze_systematic_failure(
        self, failure_context: Dict[str, Any], systematic_constraints: bool = True
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "recommendations": [
                fix.fix_description for fix in rca_result.systematic_fixes
            ],
