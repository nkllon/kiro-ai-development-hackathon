"""
Mvp Calculator Core Core

This module was extracted from mvp_calculator_core.py
...
class MVPRouteCalculator:
    ...
    async def calculate_mvp_route(
        self, ecosystem_dag: EcosystemDAG, mvp_criteria: MVPCriteria
    ) -> MVPRoute:
        """
        ...
        mvp_route = await self._create_detailed_mvp_route(
            best_route, ecosystem_dag, mvp_criteria
        )
        return mvp_route
    ...
    def _generate_route_options(
        ...
        if min_viable_tasks:
            route_options.append(
                self._create_route_option(
                    "minimum_viable", min_viable_tasks, ecosystem_dag
                )
            )
        value_optimized_tasks = self._find_value_optimized_tasks(
        ...
        risk_minimized_tasks = self._find_risk_minimized_tasks(
            ecosystem_dag, mvp_criteria
        )
        ...
        if balanced_tasks:
            route_options.append(
                self._create_route_option("balanced", balanced_tasks, ecosystem_dag)
            )
        return route_options

    def _find_minimum_viable_tasks(
        self, ecosystem_dag: EcosystemDAG, mvp_criteria: MVPCriteria
    ) -> List[TaskNode]:
        ...
        all_tasks = self._add_task_dependencies(
            list(required_tasks), ecosystem_dag.tasks
        )
        all_tasks.sort(key=lambda t: t.estimated_effort)
        return all_tasks

    def _find_value_optimized_tasks(
        self, ecosystem_dag: EcosystemDAG, mvp_criteria: MVPCriteria
    ) -> List[TaskNode]:
    ...
    def _find_risk_minimized_tasks(
        self, ecosystem_dag: EcosystemDAG, mvp_criteria: MVPCriteria
    ) -> List[TaskNode]:
    ...
    def _find_balanced_tasks(
        ...
        for task in ecosystem_dag.tasks:
            ...
            combined_score = (
                0.4 * value_score + 0.3 * (1.0 - risk_score) + 0.3 * effort_score
            )
    ...
    def _create_route_option(
        self, route_id: str, tasks: List[TaskNode], ecosystem_dag: EcosystemDAG
    ) -> RouteOption:
        ...
        dependencies_satisfied = self._check_dependencies_satisfied(
            tasks, ecosystem_dag.tasks
        )
        return RouteOption(
            route_id=route_id,
            tasks=tasks,
    ...
    def _select_best_route(
        self, route_options: List[RouteOption], mvp_criteria: MVPCriteria
    ) -> Optional[RouteOption]:
        """Select the best route option based on criteria."""
        viable_routes = []
        for route in route_options:
            if (
                route.estimated_timeline <= mvp_criteria.maximum_timeline
                and route.estimated_effort <= mvp_criteria.maximum_effort
                and route.dependencies_satisfied
                and self._meets_minimum_deliverables(route, mvp_criteria)
            ):
        ...
        for route in viable_routes:
            score = (
                ...
                + 0.3 * (1.0 - route.risk_score)
                + 0.2 * (1.0 - route.estimated_timeline / mvp_criteria.maximum_timeline)
                + 0.1 * (1.0 - route.estimated_effort / mvp_criteria.maximum_effort)
            )
            if score > best_score:
                best_score = score
                best_route = route
    ...
    async def _create_detailed_mvp_route(
        self,
        ...
        phases = await self.optimize_mvp_phases(
            MVPRoute(
        ...
        mvp_route.success_probability = await self.calculate_success_probability(
            mvp_route
        )
        return mvp_route

    def _find_tasks_for_deliverable(
        self, tasks: List[TaskNode], deliverable: str
    ) -> List[TaskNode]:
        ...
        for task in tasks:
            ...
            if (
                deliverable_lower in task_desc_lower
                or deliverable_lower in task_name_lower
                or any(
                    (
                        keyword in task_desc_lower
                        for keyword in deliverable_lower.split()
                    )
                )
            ):
    ...
    def _calculate_task_value_score(
        self, task: TaskNode, mvp_criteria: MVPCriteria
    ) -> float:
    ...
    def _add_task_dependencies(
        self, selected_tasks: List[TaskNode], all_tasks: List[TaskNode]
    ) -> List[TaskNode]:
        ...
        return [
            task_lookup[task_id] for task_id in result_tasks if task_id in task_lookup
        ]
    ...
    def _group_tasks_into_phases(
        ...
        foundation_tasks = [
            ...
            if any(
                (
                    keyword in task.task_name.lower()
                    for keyword in [
                        "setup",
                        "infrastructure",
                        "foundation",
        ...
        remaining_tasks = [
            ...
            if task not in foundation_tasks
            and task not in core_tasks
            and (task not in integration_tasks)
        ]
        if remaining_tasks:
    ...
    def _define_phase_objectives(
        ...
        if phase_number == 1:
            objectives.append(
                "Establish foundational infrastructure and core interfaces"
            )
        elif phase_number == 2:
    ...
    def _add_minimal_dependencies(
        self, tasks: List[TaskNode], all_tasks: List[TaskNode]
    ) -> List[TaskNode]:
    ...
    def _calculate_route_value_score(self, tasks: List[TaskNode]) -> float:
        ...
        total_score = sum(
            (
                self._calculate_task_value_score(
                    task,
                    MVPCriteria(
    ...
    def _meets_minimum_deliverables(
        self, route: RouteOption, mvp_criteria: MVPCriteria
    ) -> bool:
        """Check if route meets minimum deliverable requirements."""
        route_deliverables = set((d.lower() for d in route.deliverables))
        required_deliverables = set(
            (d.lower() for d in mvp_criteria.required_deliverables)
        )
    ...
    def _identify_risk_factors(
        self, tasks: List[TaskNode], mvp_criteria: MVPCriteria
    ) -> List[RiskFactor]:
...
def _generate_route_options(
    ...
    if min_viable_tasks:
        route_options.append(
            self._create_route_option("minimum_viable", min_viable_tasks, ecosystem_dag)
        )
    value_optimized_tasks = self._find_value_optimized_tasks(
        ecosystem_dag, mvp_criteria
    )
...
def _find_value_optimized_tasks(
    ...
    total_effort = 0
    sorted_tasks = sorted(
        ecosystem_dag.tasks, key=lambda t: task_scores.get(t.task_id, 0), reverse=True
    )
    for task in sorted_tasks:
        if total_effort + task.estimated_effort <= mvp_criteria.maximum_effort:
            selected_tasks.append(task)
...
def _find_tasks_for_deliverable(
    ...
    for task in tasks:
        ...
        if (
            ...
            or deliverable_lower in task_name_lower
            or any(
                (keyword in task_desc_lower for keyword in deliverable_lower.split())
            )
        ):
            matching_tasks.append(task)
    return matching_tasks
