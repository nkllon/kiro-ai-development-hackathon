...
class PerformanceExpert(GhostbustersExpertAgent):
        ...
        try:
            ...
            result = AnalysisResult(
                ...
                metadata={
                    ...
                    "optimization_opportunities": self._get_optimization_opportunities(findings)
                }
            )
            
            logger.info(f"Performance analysis completed for {target_path} with {len(findings)} findings")
            return result
            ...
            def visit_For(self, node):
                ...
                if self.loop_depth > 1:
                    self.findings.append(Finding(
                        ...
                        confidence=0.8,
                        evidence={"issue": "nested_loops", "depth": self.loop_depth}
                    ))
                        ...
                        if isinstance(child.func, ast.Attribute):
                            if child.func.attr in ['append', 'extend'] and isinstance(child.func.value, ast.Name):
                                ...
                                self.findings.append(Finding(
                                    type=FindingType.PERFORMANCE_ISSUE,
                                    severity=Severity.LOW,
                                    location=CodeLocation(str(self.file_path), getattr(child, 'lineno', node.lineno)),
                ...
                if isinstance(node.func, ast.Name):
                    ...
                    if node.func.attr in ['join'] and self.in_loop:
                        ...
                        self.findings.append(Finding(
                            ...
                            confidence=0.6,
                            evidence={"issue": "repeated_string_operation", "operation": node.func.attr}
                        ))
    ...
    async def _analyze_sql_performance(self, content: str, file_path: Path) -> List[Finding]:
        ...
        sql_patterns = {
            'select_star': [r'SELECT\s+\*\s+FROM', 'Avoid SELECT * - specify columns explicitly'],
            'missing_where': [r'DELETE\s+FROM\s+\w+\s*;', 'DELETE without WHERE clause'],
            'cartesian_join': [r'FROM\s+\w+\s*,\s*\w+', 'Potential cartesian join'],
            'function_in_where': [r'WHERE\s+\w+\s*\(\s*\w+\s*\)', 'Function in WHERE clause prevents index usage']
        }
    ...
    def _calculate_performance_confidence(self, findings: List[Finding], target_path: Path) -> float:
        ...
        if target_path.is_dir():
            code_files = [f for f in target_path.rglob("*") if f.suffix in {'.py', '.js', '.ts', '.sql'}]
            if code_files:
                base_confidence = 0.8
    ...
    def _get_optimization_opportunities(self, findings: List[Finding]) -> List[str]:
        ...
        for finding in findings:
            ...
            if issue_type == "string_concatenation":
                ...
                opportunities.append("Implement query optimization with joins or prefetch")
