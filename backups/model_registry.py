...
class ModelRegistry(ReflectiveModule):
    ...
    def _load_registry(self) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"Loaded registry with {len(self.registry_data)} top-level keys"
            )
    ...
    def update_learning(self, pattern: Pattern) -> bool:
        ...
        try:
            ...
            if existing_pattern is not None:
                ...
                intelligence.patterns[existing_pattern] = self._merge_patterns(
                    old_pattern, pattern
                )
                self.logger.info(
                    f"Updated existing pattern {pattern.pattern_id} for domain {domain}"
                )
            else:
                intelligence.patterns.append(pattern)
                self.logger.info(
                    f"Added new pattern {pattern.pattern_id} for domain {domain}"
                )
            ...
            avg_success_rate = sum(intelligence.success_metrics.values()) / max(
                len(intelligence.success_metrics), 1
            )
            metrics_boost = avg_success_rate * metrics_confidence_weight

            intelligence.confidence_score = min(
                1.0,
                (
                    intelligence.confidence_score
                    * (1 - pattern_confidence_weight - metrics_confidence_weight)
            ...
            self.last_updated = datetime.now()
            self.logger.info(
                f"Updated learning for domain {domain}: {pattern.name} (confidence: {intelligence.confidence_score:.3f})"
            )
    ...
    def _merge_patterns(self, old_pattern: Pattern, new_pattern: Pattern) -> Pattern:
        ...
        return Pattern(
            ...
            confidence_score=max(
                old_pattern.confidence_score, new_pattern.confidence_score
            ),
        )
    ...
    def _persist_learning_update(self, domain: str, pattern: Pattern):
        ...
        try:
            ...
            self.logger.info(
                f"Persisted learning pattern {pattern.pattern_id} to {pattern_file}"
            )
    ...
    def load_persisted_learning(self, domain: str) -> List[Pattern]:
        ...
        try:
            ...
            with open(pattern_file, "r") as f:
                ...
                self.logger.info(
                    f"Loaded {len(patterns)} persisted patterns for domain {domain}"
                )
                return patterns

        except Exception as e:
            self.logger.warning(
                f"Failed to load persisted learning for domain {domain}: {e}"
            )
            return []
        ...
        try:
            ...
            for d in domains_to_analyze:
                if d in self.intelligence_cache:
                    ...
                    insights["domain_insights"][d] = {
                        ...
                        "avg_confidence": sum(
                            p.confidence_score for p in domain_patterns
                        )
                        / max(len(domain_patterns), 1),
            ...
            if all_patterns:
                ...
                insights["avg_confidence"] = sum(
                    p.confidence_score for p in all_patterns
                ) / len(all_patterns)
