...
class SystematicSuperiorityModel(ReflectiveModule):
    ...
    def compare_approaches(
        self, systematic: Approach, adhoc: Approach
    ) -> ComparisonResult:
        ...
        for metric in ComparisonMetric:
            if adhoc.metrics[metric] > 0:
                improvement_factors[metric.value] = (
                    systematic.metrics[metric] / adhoc.metrics[metric]
                )
            else:
                improvement_factors[metric.value] = 1.0

        # Calculate overall improvement factor (Beast Mode: 20.4% improvement)
        overall_improvement = sum(improvement_factors.values()) / len(
            improvement_factors
        )
    ...
    def create_evidence_package(self) -> EvidencePackage:
        ...
        evidence_package = EvidencePackage(
            ...
            statistical_validation=comparison.evidence_package[
                "statistical_validation"
            ],
    ...
    def check_health(self) -> ModuleHealth:
        ...
        try:
            ...
            health_score = (
                ...
                + (1.0 if evidence_available else 0.0)
            ) / 3
