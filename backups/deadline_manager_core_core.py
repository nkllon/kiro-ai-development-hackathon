"""
Deadline Manager Core Core

This module was extracted from deadline_manager_core.py
...
class DeadlineManagementSystem:
    ...
    def _optimize_scope_emergency(self, delay_risk: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "competitive_impact_preserved": 0.85,
            "time_saved_days": 3,
    ...
    def _calculate_scope_impact(self, plan: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "risk_reduction": min(1.0, plan["total_time_saved"] / 5),
...
def _calculate_deadline_risk(
    ...
    total_critical_duration = sum((task["duration_days"] for task in critical_path))
    risk_ratio = (
        total_critical_duration / days_remaining if days_remaining > 0 else float("inf")
    )
    if risk_ratio > 1.2:
        risk_level = "critical"
        acceleration_required = True
...
def _reallocate_resources_emergency(self, delay_risk: Dict[str, Any]) -> Dict[str, Any]:
    """Reallocate resources for emergency acceleration."""
    return {
        "additional_resources": ["emergency_team_members", "priority_platform_access"],
        "resource_prioritization": "critical_path_only",
        "cost_impact": "high",
        "duration": "until_deadline",
    }
...
def _calculate_scope_impact(self, plan: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "competitive_impact_preserved": plan["competitive_impact_preserved"],
        "risk_reduction": min(1.0, plan["total_time_saved"] / 5),
        "implementation_effort": "low" if len(plan["reductions"]) <= 2 else "medium",
    }
