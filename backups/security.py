...
Provides systematic security analysis including vulnerability detection,
security pattern validation, and security best practices enforcement.
"""
...
class SecurityExpert(GhostbustersExpertAgent):
    ...
    def _init_security_patterns(self):
        ...
        self.sql_injection_patterns = [
            ...
            r'SELECT\s+.*\+.*FROM',              # Direct SQL concatenation
            r'INSERT\s+.*\+.*VALUES',            # Direct SQL concatenation
            r'UPDATE\s+.*SET.*\+',               # Direct SQL concatenation
            r'DELETE\s+.*WHERE.*\+',             # Direct SQL concatenation
        ]
        ...
        self.xss_patterns = [
            ...
            r'eval\s*\(',                       # eval usage
            r'setTimeout\s*\(\s*["\'].*\+',      # setTimeout with string concat
            r'setInterval\s*\(\s*["\'].*\+',     # setInterval with string concat
        ]
        ...
        self.secret_patterns = [
            ...
            (r'private_key\s*=\s*["\']-----BEGIN', 'hardcoded_private_key'),
        ]
        ...
        self.crypto_patterns = [
            ...
            (r'DES\s*\(', 'weak_cipher_des'),
            (r'RC4\s*\(', 'weak_cipher_rc4'),
        ...
        try:
            ...
            result = AnalysisResult(
                ...
                metadata={
                    ...
                    "vulnerability_categories": self._get_vulnerability_categories(findings),
    ...
    def validate_confidence(self, result: AnalysisResult) -> bool:
        ...
        # High confidence should correlate with thorough analysis
    ...
    async def _analyze_directory(self, directory: Path) -> List[Finding]:
        ...
        # Analyze all relevant files in directory
    ...
    def _check_python_security(self, content: str, file_path: Path) -> List[Finding]:
        ...
        python_patterns = [
            (r'pickle\.loads?\s*\(', 'unsafe_deserialization', Severity.HIGH),
            (r'yaml\.load\s*\(', 'unsafe_yaml_load', Severity.HIGH),
            (r'input\s*\(\s*\)', 'unsafe_input', Severity.MEDIUM),
            (r'__import__\s*\(', 'dynamic_import', Severity.MEDIUM),
        ]
    ...
    def _check_javascript_security(self, content: str, file_path: Path) -> List[Finding]:
        ...
        js_patterns = [
            (r'localStorage\.setItem\s*\(\s*["\'][^"\']*password', 'password_in_localstorage', Severity.HIGH),
            (r'sessionStorage\.setItem\s*\(\s*["\'][^"\']*password', 'password_in_sessionstorage', Severity.HIGH),
            (r'window\.location\.href\s*=.*\+', 'open_redirect', Severity.MEDIUM),
            (r'postMessage\s*\(\s*.*\*', 'unsafe_postmessage', Severity.MEDIUM),
        ]
    ...
    def _check_java_security(self, content: str, file_path: Path) -> List[Finding]:
        ...
        java_patterns = [
            (r'Runtime\.getRuntime\(\)\.exec', 'runtime_exec', Severity.HIGH),
            (r'ObjectInputStream\s*\(', 'unsafe_deserialization', Severity.HIGH),
            (r'Random\s*\(\s*\)', 'weak_random', Severity.MEDIUM),
            (r'MessageDigest\.getInstance\s*\(\s*["\']MD5', 'weak_hash', Severity.MEDIUM),
        ]
    ...
    def _calculate_security_confidence(self, findings: List[Finding], target_path: Path) -> float:
        ...
        if target_path.is_dir():
            # Directory analysis is less comprehensive
            base_confidence = 0.7
