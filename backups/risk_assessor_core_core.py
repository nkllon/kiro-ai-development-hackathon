...
class RiskAssessor:
    ...
    def assess_systematic_risk_with_prejudice(
        self,
        mvp_route: MVPRoute,
        success_factors: Optional[SuccessProbabilityFactors] = None,
        ...
        overall_risk_score = self._calculate_systematic_risk_score(
            risk_factors, mvp_route
        )
    ...
    def run_monte_carlo_timeline_simulation(
        self, mvp_route: MVPRoute, confidence_level: float = 0.95
    ) -> MonteCarloResult:
        """
        ...
        for _ in range(self.simulation_runs):
            ...
            simulation_results.append(
                {"duration": total_duration, "success": phase_success}
            )
    ...
    def calculate_success_probability_with_factors(
        self, mvp_route: MVPRoute, success_factors: SuccessProbabilityFactors
    ) -> float:
        """
    ...
    def _identify_timeline_risks(self, mvp_route: MVPRoute) -> List[RiskFactor]:
        ...
        if mvp_route.estimated_timeline < 4:
            risks.append(
                RiskFactor(
                    ...
                    affected_tasks=[
                        task.task_id for task in mvp_route.critical_tasks[:3]
                    ],
        ...
        if len(mvp_route.phases) > 0:
            phase_efforts = [
                sum((task.estimated_effort for task in phase.tasks))
                for phase in mvp_route.phases
            ]
    ...
    def _identify_technical_risks(self, mvp_route: MVPRoute) -> List[RiskFactor]:
        ...
        risks = []
        complex_tasks = [
            task for task in mvp_route.critical_tasks if task.estimated_effort > 40
        ]
        if complex_tasks:
            risks.append(
                RiskFactor(
        ...
        unstarted_critical = [
            task
            for task in mvp_route.critical_tasks
    ...
    def _identify_dependency_risks(self, mvp_route: MVPRoute) -> List[RiskFactor]:
        ...
        complex_dep_tasks = [
            task for task in mvp_route.critical_tasks if len(task.dependencies) > 3
        ]
    ...
    def _identify_quality_risks(self, mvp_route: MVPRoute) -> List[RiskFactor]:
        ...
        testing_tasks = [
            task
            for task in mvp_route.critical_tasks
            if "test" in task.task_name.lower()
            or "validation" in task.task_name.lower()
        ]
    ...
    def _identify_integration_risks(self, mvp_route: MVPRoute) -> List[RiskFactor]:
        ...
        integration_tasks = [
            task
            for task in mvp_route.critical_tasks
            if "integration" in task.task_name.lower()
            or "integrate" in task.task_name.lower()
        ]
    ...
    def _calculate_systematic_risk_score(
        self, risk_factors: List[RiskFactor], mvp_route: MVPRoute
    ) -> float:
    ...
    def _calculate_systematic_success_probability(
        self,
        mvp_route: MVPRoute,
        risk_factors: List[RiskFactor],
        ...
        if success_factors:
            factor_adjustment = self._calculate_success_factor_adjustments(
                success_factors
            )
        else:
    ...
    def _calculate_route_specific_adjustments(self, mvp_route: MVPRoute) -> float:
        ...
        if len(mvp_route.phases) > 0:
            ...
            effort_std = math.sqrt(
                sum(
                    (
                        (e - sum(phase_efforts) / len(phase_efforts)) ** 2
                        for e in phase_efforts
                    )
                )
                / len(phase_efforts)
            )
        ...
        completion_ratio = (
            len(completed_tasks) / len(mvp_route.critical_tasks)
            if mvp_route.critical_tasks
            else 0
        )
    ...
    def _calculate_success_factor_adjustments(
        self, success_factors: SuccessProbabilityFactors
    ) -> float:
    ...
    def _calculate_complexity_adjustments(self, mvp_route: MVPRoute) -> float:
        ...
        adjustment = 0.0
        complex_tasks = [
            task for task in mvp_route.critical_tasks if task.estimated_effort > 30
        ]
        complexity_ratio = (
            len(complex_tasks) / len(mvp_route.critical_tasks)
            if mvp_route.critical_tasks
            else 0
        )
    ...
    def _generate_mitigation_strategies(
        ...
        if RiskType.TIMELINE_RISK in risk_types:
            strategies.add(
                "Implement systematic timeline monitoring with early warning alerts"
            )
        if RiskType.TECHNICAL_RISK in risk_types:
            strategies.add(
                "Conduct systematic technical risk assessment and proof-of-concept validation"
            )
        if RiskType.RESOURCE_RISK in risk_types:
            strategies.add(
                "Establish systematic resource allocation with backup team members"
            )
        if RiskType.DEPENDENCY_RISK in risk_types:
            strategies.add(
                "Create systematic dependency tracking with alternative path planning"
            )
        if RiskType.QUALITY_RISK in risk_types:
            strategies.add(
                "Implement systematic quality gates with continuous validation"
            )
    ...
    def _generate_contingency_plans(
        ...
        high_impact_risks = [
            risk
            for risk in risk_factors
            if risk.impact in [RiskImpact.HIGH, RiskImpact.CRITICAL]
        ]
        if high_impact_risks:
            plans.append(
                "🚨 Emergency scope reduction plan: Remove non-critical features to meet timeline"
            )
            plans.append(
                "⚡ Resource escalation plan: Add additional team members or external expertise"
            )
            plans.append(
                "🔄 Alternative implementation plan: Simplified approach for critical components"
            )
        timeline_risks = [
            risk for risk in risk_factors if risk.risk_type == RiskType.TIMELINE_RISK
        ]
        if timeline_risks:
            plans.append(
                "📅 Timeline extension plan: Negotiate additional time with stakeholders"
            )
            plans.append(
                "🎯 MVP scope adjustment: Focus on core value demonstration only"
            )
        technical_risks = [
            risk for risk in risk_factors if risk.risk_type == RiskType.TECHNICAL_RISK
        ]
        if technical_risks:
            plans.append(
                "🛠️ Technical pivot plan: Alternative technology stack or approach"
            )
            plans.append(
                "🔬 Prototype validation plan: Build minimal proof-of-concept first"
            )
        resource_risks = [
            risk for risk in risk_factors if risk.risk_type == RiskType.RESOURCE_RISK
        ]
        if resource_risks:
            plans.append(
                "👥 Team augmentation plan: Bring in specialists or consultants"
            )
            plans.append(
                "🔄 Task redistribution plan: Rebalance workload across team members"
            )
        return (
            plans
            if plans
            else ["✅ No specific contingency plans required - risk levels acceptable"]
        )
...
def _calculate_systematic_success_probability(
    ...
    complexity_adjustment = self._calculate_complexity_adjustments(mvp_route)
    final_probability = (
        base_probability + risk_adjustment + factor_adjustment + complexity_adjustment
    )
    return max(0.1, min(0.95, final_probability))
...
def _simulate_phase_execution(self, phase: MVPPhase) -> Tuple[float, bool]:
    ...
    if len(phase.parallel_groups) > 3:
        success_probability -= 0.05
    incomplete_tasks = [
        task for task in phase.tasks if task.completion_status != TaskStatus.COMPLETED
    ]
    if len(incomplete_tasks) > len(phase.tasks) * 0.8:
        success_probability -= 0.1
    phase_succeeded = random.random() < success_probability
