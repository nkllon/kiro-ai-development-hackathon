...
class DependencyAnalyzer:
    ...
    async def analyze_complete_ecosystem(self, spec_directory: str) -> EcosystemAnalysisResult:
        ...
        constraint_graph = self.dependency_mapper.create_dependency_graph(
            task_detection.tasks,
            task_detection.dependencies,
            specification_nodes
        )
    ...
    def _create_specification_nodes(self, 
                                  parsed_specs: List[ParsedSpec],
                                  task_detection: TaskDetectionResult) -> List[SpecificationNode]:
        ...
        for parsed_spec in parsed_specs:
            ...
            specification_nodes.append(SpecificationNode(
                spec_name=spec_name,
    ...
    def _create_ecosystem_dag(self, 
                            specification_nodes: List[SpecificationNode],
                            task_detection: TaskDetectionResult,
                            constraint_graph: ConstraintGraph,
                            critical_path_analysis: CriticalPathAnalysis) -> EcosystemDAG:
        ...
        completed_effort = sum(
            task.estimated_effort for task in task_detection.tasks
            if task.completion_status.value == 'completed'
        )
        ...
        for layer, task_ids in constraint_graph.dependency_layers.items():
            if len(task_ids) > 1:
                ...
                if parallel_tasks:
                    parallel_opportunities.append(ParallelGroup(
                        group_id=f"layer_{layer}",
                        tasks=parallel_tasks,
                        estimated_duration=max(task.estimated_effort for task in parallel_tasks) // 8,  # Convert to days
