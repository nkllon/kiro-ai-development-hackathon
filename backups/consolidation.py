...
class SpecConsolidator(ReflectiveModule):
    ...
    def _calculate_complexity_score(
        ...
        complexity_factors = {
            "requirements_count": len(
                re.findall(r"### Requirement \d+", requirements_content)
            )
            * 0.1,
            "acceptance_criteria_count": len(
                re.findall(r"\d+\.\s+WHEN", requirements_content)
            )
            * 0.05,
    ...
    def _calculate_requirement_complexity(self, req_text: str) -> float:
        ...
        complexity_indicators = {
            ...
            "conditional_logic": len(
                re.findall(r"\bIF\b|\bWHEN\b|\bAND\b", req_text, re.IGNORECASE)
            )
            * 0.05,
    ...
    def _calculate_requirement_quality(self, criteria_lines: List[str]) -> float:
        ...
        quality_indicators = {
            ...
            "criteria_completeness": min(len(criteria_lines) / 5, 1.0)
            * 0.3,  # Up to 5 criteria
            "uses_ears_format": (
                0.3
                if any(
    ...
    def create_consolidation_plan(
        ...
        try:
            ...
            primary_opportunity = overlap_analysis.consolidation_opportunities[
                0
            ]  # Highest priority
    ...
    def _are_contradictory_criteria(self, criterion1: str, criterion2: str) -> bool:
        ...
        contradictory_patterns = [
            ...
            (
                r"WHEN\s+([^T]+)\s+THEN\s+([^.]+)",
                r"WHEN\s+\1\s+THEN\s+(?!.*\2)",
            ),  # Different outcomes for same condition
        ]
    ...
    def _create_merged_content(
        ...
        user_story_match = re.search(
            r"\*\*User Story:\*\*([^#]+)", base_requirement.content
        )
...
"""
    ...
    def _validate_merged_requirements(
        ...
        if (
            merged_avg_quality < original_avg_quality * 0.9
        ):  # Allow 10% quality reduction
            validation_result["issues"].append(
    ...
    def _create_unified_requirement(
        ...
        unique_criteria = list(
            dict.fromkeys(all_criteria)
