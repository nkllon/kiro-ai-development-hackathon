...
This module was extracted from performance_monitor.py
...
class RCAPerformanceMonitor(ReflectiveModule):
    ...
    def is_healthy(self) -> bool:
        ...
        return (
            ...
            and timeout_rate < 0.1
            and (degradation_rate < 0.2)
        )
    ...
    def get_health_indicators(self) -> Dict[str, Any]:
        ...
        return {
            "performance_monitoring": {
                ...
                "active_operations": len(self.active_operations),
                "monitoring_thread_alive": self.monitor_thread
                and self.monitor_thread.is_alive(),
            },
            "timeout_compliance": {
                "status": (
                    "healthy"
                    if self.timeout_count / max(1, self.total_operations) < 0.1
                    else "degraded"
                ),
                "timeout_rate": self.timeout_count / max(1, self.total_operations),
                "average_operation_time": self._calculate_average_operation_time(),
            ...
            "resource_management": {
                "status": (
                    "healthy" if self.resource_limit_violations < 5 else "degraded"
                ),
    ...
    def start_monitoring(self) -> None:
        ...
        try:
            ...
            self.monitor_thread = threading.Thread(
                target=self._monitoring_loop, name="RCAPerformanceMonitor", daemon=True
            )
    ...
    def monitor_operation(
        self, operation_id: str, timeout_seconds: Optional[int] = None
    ):
        """
        ...
        metrics = PerformanceMetrics(
            operation_id=operation_id, start_time=datetime.now()
        )
        ...
        if timeout > 0:
            timeout_handler = threading.Timer(
                timeout, self._handle_timeout, args=[operation_id]
            )
            timeout_handler.start()
        try:
            self.logger.info(
                f"Starting monitored operation: {operation_id} (timeout: {timeout}s)"
            )
            ...
            if (
                metrics.duration_seconds
                > self.resource_limits.warning_threshold_seconds
            ):
                metrics.operation_status = PerformanceStatus.TIMEOUT_RISK
                self.logger.warning(
                    f"Operation {operation_id} exceeded warning threshold: {metrics.duration_seconds:.2f}s"
                )
            self.logger.info(
                f"Operation {operation_id} completed in {metrics.duration_seconds:.2f}s"
            )
            ...
            if operation_id in self.active_operations:
                ...
                if not completed_metrics.end_time:
                    ...
                    completed_metrics.duration_seconds = (
                        completed_metrics.end_time - completed_metrics.start_time
                    ).total_seconds()
    ...
    def implement_graceful_degradation(
        self, operation_id: str, degradation_strategy: str = "simplified_analysis"
    ) -> Dict[str, Any]:
        """
        ...
        try:
            self.logger.warning(
                f"Implementing graceful degradation for operation {operation_id}: {degradation_strategy}"
            )
            if operation_id in self.active_operations:
    ...
    def get_performance_report(self) -> PerformanceReport:
        ...
        try:
            ...
            successful_ops = [
                ...
                if not op.timeout_occurred
                and op.operation_status != PerformanceStatus.CRITICAL
            ]
            timeout_ops = [
                op for op in self.completed_operations if op.timeout_occurred
            ]
            degraded_ops = [
                op for op in self.completed_operations if op.graceful_degradation
            ]
            avg_duration = sum(
                (op.duration_seconds for op in self.completed_operations)
            ) / len(self.completed_operations)
            avg_memory = sum(
                (op.memory_usage_mb for op in self.completed_operations)
            ) / len(self.completed_operations)
            peak_memory = (
                max((op.peak_memory_mb for op in self.completed_operations))
    ...
    def optimize_resource_usage(self, operation_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if current_cpu > self.resource_limits.max_cpu_percent * 0.8:
                self._optimize_cpu_usage()
                optimization_result["actions_taken"].append("cpu_optimization")
                optimization_result["optimization_applied"] = True
            if (
                current_memory > self.resource_limits.max_memory_mb * 0.9
                or current_cpu > self.resource_limits.max_cpu_percent * 0.9
            ):
                degradation_result = self.implement_graceful_degradation(
                    operation_id, "resource_conservation"
                )
    ...
    def _update_operation_metrics(
        self, operation_id: str, metrics: PerformanceMetrics
    ) -> None:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to update metrics for operation {operation_id}: {e}"
            )
    ...
    def _check_resource_limits(self) -> None:
        ...
        try:
            ...
            if current_memory > self.resource_limits.max_memory_mb:
                self.logger.warning(
                    f"Memory limit exceeded: {current_memory}MB > {self.resource_limits.max_memory_mb}MB"
                )
                self.resource_limit_violations += 1
            ...
            if current_cpu > self.resource_limits.max_cpu_percent:
                self.logger.warning(
                    f"CPU limit exceeded: {current_cpu}% > {self.resource_limits.max_cpu_percent}%"
                )
                self.resource_limit_violations += 1
        except Exception as e:
    ...
    def _handle_timeout(self, operation_id: str) -> None:
        ...
        try:
            ...
            self.logger.error(
                f"Timeout handling failed for operation {operation_id}: {e}"
            )
    ...
    def _calculate_average_operation_time(self) -> float:
        ...
        if not self.completed_operations:
            return 0.0
        return sum((op.duration_seconds for op in self.completed_operations)) / len(
    ...
    def _get_current_performance_status(self) -> PerformanceStatus:
        ...
        timeout_rate = sum((1 for op in recent_ops if op.timeout_occurred)) / len(
            recent_ops
        )
        degradation_rate = sum(
            (1 for op in recent_ops if op.graceful_degradation)
        ) / len(recent_ops)
    ...
    def _calculate_performance_trend(self) -> str:
        ...
        recent_ops = self.completed_operations[-10:]
        earlier_ops = (
            self.completed_operations[-20:-10]
            if len(self.completed_operations) >= 20
            else []
        )
        if not earlier_ops:
            return "stable"
        recent_avg = sum((op.duration_seconds for op in recent_ops)) / len(recent_ops)
        earlier_avg = sum((op.duration_seconds for op in earlier_ops)) / len(
            earlier_ops
        )
...
def is_healthy(self) -> bool:
    ...
    degradation_rate = self.degradation_count / max(1, self.total_operations)
    return (
        not self._degradation_active and timeout_rate < 0.1 and (degradation_rate < 0.2)
    )
...
def monitor_operation(self, operation_id: str, timeout_seconds: Optional[int] = None):
    ...
    """
    timeout = timeout_seconds or self.resource_limits.timeout_seconds
    metrics = PerformanceMetrics(operation_id=operation_id, start_time=datetime.now())
    self.active_operations[operation_id] = metrics
    self.total_operations += 1
    timeout_handler = None
    if timeout > 0:
...
def _get_current_performance_status(self) -> PerformanceStatus:
    ...
    degradation_rate = sum((1 for op in recent_ops if op.graceful_degradation)) / len(
        recent_ops
    )
    avg_time = sum((op.duration_seconds for op in recent_ops)) / len(recent_ops)
    if timeout_rate > 0.2:
