"""
Unified Beast Mode Client

Provides a single client interface that works with any transport implementation.
Integrates transport abstraction with shared state management.
"""
...
class BeastModeClient:
    ...
    def __init__(
        ...
        shared_state_config: Optional[SharedStateConfig] = None,
        ...
        self.transport = TransportFactory.create_transport(
            transport_type, agent_id=agent_id, **(transport_config or {})
        )
    ...
    async def start(self) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"Beast Mode client started: {self.agent_id} using {self.transport_type}"
            )
            return True
    ...
    async def send_message(self, message: BeastModeMessage) -> bool:
        ...
        try:
            ...
            if success:
                ...
                await self.shared_state.increment_counter(
                    "messages_sent", self.agent_id
                )

                # Update agent state with activity
                await self.shared_state.update_agent_state(
                    self.agent_id,
                    {"last_activity": datetime.now().isoformat(), "status": "active"},
                )
    ...
    async def _handle_message(self, message: BeastModeMessage):
        ...
        try:
            ...
            # Update shared state metrics
            await self.shared_state.increment_counter(
                "messages_received", self.agent_id
            )
    ...
    def get_capabilities(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "client_features": [
                ...
                "automatic_presence_management",
                "built_in_discovery_response",
            ],
        }
    ...
    def send_spore(self, spore_data: Dict[str, Any]):
        ...
        # Send asynchronously (fire and forget for compatibility)
