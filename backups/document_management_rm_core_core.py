...
class DocumentManagementRM(ReflectiveModule):
    ...
    def is_healthy(self) -> bool:
        ...
        return (
            self.docs_root.exists()
            and self.doc_metrics["rm_compliance_rate"] >= 0.8
            and (self.doc_metrics["cross_reference_violations"] == 0)
    ...
    def get_health_indicators(self) -> Dict[str, Any]:
        ...
        return {
            "documentation_health": {
                "total_documents": self.doc_metrics["total_documents"],
                "rdi_compliance_rate": self.doc_metrics["rdi_compliant_documents"]
                / max(1, self.doc_metrics["total_documents"]),
                "rm_compliance_rate": self.doc_metrics["rm_compliance_rate"],
                "cross_reference_integrity": self.doc_metrics[
    ...
    def register_rm_documentation(
        self, rm_name: str, documents: List[RDIDocument]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for doc in documents:
                ...
                if compliance["compliant"]:
                    ...
                    self.logger.info(
                        f"Registered RDI document: {doc.title} for RM: {rm_name}"
                    )
                else:
                    self.logger.warning(
                        f"RDI compliance failed for document: {doc.title}"
                    )
            self._update_documentation_metrics()
            return {
                "success": True,
                "rm_name": rm_name,
                "documents_registered": len(
                    [r for r in compliance_results if r["compliant"]]
                ),
    ...
    def enforce_rm_documentation_constraint(self, rm_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            for doc in rm_documents:
                ...
                if not doc_compliance["compliant"]:
                    compliance_issues.append(
                        {"document": doc.title, "issues": doc_compliance["issues"]}
                    )
            ...
            self.logger.error(
                f"RM documentation constraint enforcement failed: {str(e)}"
            )
            return {"error": f"Constraint enforcement failed: {str(e)}"}

    def create_rdi_document_template(
        self, rm_name: str, document_type: DocumentType, title: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
            doc_id = (
                f"{rm_name}_{document_type.value}_{int(datetime.now().timestamp())}"
            )
            file_path = self.docs_root / "rms" / rm_name / f"{document_type.value}.md"
            file_path.parent.mkdir(parents=True, exist_ok=True)
            template_content = self._generate_document_template(
                rm_name, document_type, title
            )
            ...
            self.logger.info(
                f"Created RDI document template: {title} for RM: {rm_name}"
            )
            return {
                "success": True,
                "document_id": doc_id,
    ...
    def validate_cross_references(self, document_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            validation_results["all_references_valid"] = (
                len(validation_results["invalid_references"]) == 0
                and len(validation_results["circular_references"]) == 0
            )
    ...
    def generate_rm_documentation_report(
        self, rm_name: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for rm in rms_to_check:
                ...
                if rm in self.rm_document_mapping:
                    report["summary"]["total_documents"] += len(
                        self.rm_document_mapping[rm]
                    )
            report["overall_compliance"]["compliant_rms"] = compliant_count
            report["overall_compliance"]["non_compliant_rms"] = (
                len(rms_to_check) - compliant_count
            )
            report["overall_compliance"]["compliance_rate"] = compliant_count / max(
                1, len(rms_to_check)
            )
            return report
        except Exception as e:
    ...
    def _register_existing_document(self, file_path: Path):
        ...
        try:
            ...
            self.logger.warning(
                f"Failed to register existing document {file_path}: {str(e)}"
            )
        ...
        if self.rm_document_mapping:
            ...
            for rm_name in self.rm_document_mapping:
                ...
                if compliance.get("compliant", False):
                    compliant_rms += 1
            self.doc_metrics["rm_compliance_rate"] = compliant_rms / len(
                self.rm_document_mapping
            )
        else:
    ...
    def _generate_document_template(
        self, rm_name: str, doc_type: DocumentType, title: str
    ) -> str:
...
def get_health_indicators(self) -> Dict[str, Any]:
    ...
    return {
        "documentation_health": {
            ...
            / max(1, self.doc_metrics["total_documents"]),
            "rm_compliance_rate": self.doc_metrics["rm_compliance_rate"],
            "cross_reference_integrity": self.doc_metrics["cross_reference_violations"]
            == 0,
        },
        "structure_compliance": {
            "rdi_structure_exists": self._check_rdi_structure_exists(),
