...
class DomainHealthMonitor(DomainSystemComponent, HealthMonitorInterface):
    ...
    def __init__(self, registry_manager=None, config: Optional[Dict[str, Any]] = None):
        ...
        self.max_issues_per_domain = self.config_obj.get(
    ...
    def set_registry_manager(self, registry_manager):
        ...
        if registry_manager:
            self.health_reporter = HealthReportGenerator(
                health_monitor=self, config=self.config
            )
            ...
            try:
                ...
                if self.parallel_checks and len(all_domains) > 1:
                    ...
                    for domain_name, domain in all_domains.items():
                        try:
                            health_statuses[domain_name] = self._perform_health_check(
                                domain
                            )
                        except Exception as e:
                            self.failed_checks += 1
                            self.logger.error(
                                f"Health check failed for {domain_name}: {e}"
                            )
                            health_statuses[domain_name] = (
                                self._create_failed_health_status(str(e))
                            )
                self._health_cache.update(health_statuses)
                self._last_full_check = datetime.now()
                self.logger.info(
                    f"Completed health checks for {len(health_statuses)} domains"
                )
    ...
    def _parallel_health_checks(
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            for future in as_completed(
                future_to_domain, timeout=self.check_timeout * len(domains)
            ):
                ...
                try:
                    ...
                    self.failed_checks += 1
                    self.logger.error(
                        f"Parallel health check failed for {domain_name}: {e}"
                    )
                    health_statuses[domain_name] = self._create_failed_health_status(
                        str(e)
                    )
        return health_statuses
        ...
        try:
            ...
            for dependency in domain.dependencies:
                if dependency not in all_domains:
                    ...
                    if (
                        hasattr(dep_domain, "health_status")
                        and dep_domain.health_status
                    ):
    ...
    def _calculate_health_metrics(
        self, domain: Domain, issues: List[HealthIssue]
    ) -> HealthMetrics:
        """Calculate health metrics based on issues found"""
        critical_count = sum(
            (1 for issue in issues if issue.severity == IssueSeverity.CRITICAL)
        )
        warning_count = sum(
            (1 for issue in issues if issue.severity == IssueSeverity.WARNING)
        )
        info_count = sum(
            (1 for issue in issues if issue.severity == IssueSeverity.INFO)
        )
        ...
        makefile_score = 0.8
        overall_score = (
            dependency_score + pattern_score + file_score + makefile_score
        ) / 4.0
    ...
    def _determine_health_status_type(
        self, issues: List[HealthIssue], metrics: HealthMetrics
    ) -> HealthStatusType:
            ...
            try:
                ...
                if domain_name:
                    ...
                    for domain in all_domains.values():
                        ...
                        issues.extend(
                            [
                                f"{domain.name}: {issue.description}"
    ...
    def _file_matches_pattern(self, file_path: str, pattern: str) -> bool:
        ...
        if "**" in pattern:
            ...
            if len(pattern_parts) == 2:
                ...
                return file_path.startswith(prefix.rstrip("/")) and file_path.endswith(
                    suffix.lstrip("/")
                )
    ...
    def get_health_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            total_issues = sum(
                (len(status.issues) for status in self._health_cache.values())
            )
            critical_issues = sum(
                (
                    sum(
            ...
            return {
                ...
                "overall_health_percentage": healthy_count
                / max(total_domains, 1)
                * 100,
    ...
    def schedule_health_check(self, domain_name: str, interval_minutes: int) -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"Scheduled health check for {domain_name} every {interval_minutes} minutes"
            )
            return True
        except Exception as e:
    ...
    def get_monitor_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "failed_checks": self.failed_checks,
            "success_rate": (self.total_checks - self.failed_checks)
    ...
    def generate_health_report(
        self, report_type: str = "full", domain_name: str = None
    ) -> Dict[str, Any]:
