...
from .message_handlers import (
    ...
    HelpWantedHandler,
    HelpResponseHandler,
    SporeDeliveryHandler,
    SporeRequestHandler,
    SporeSpawnHandler,
    TechnicalExchangeHandler,
    SystemHealthHandler,
...
class StandardMessageRouter(MessageRouter):
    ...
    def _setup_standard_handlers(self) -> None:
        ...
        simple_handler = SimpleMessageHandler(
            self.agent_id, message_callback=self.callbacks.get("on_simple_message")
        )
        self.register_handler(simple_handler)

        # Prompt request handler
        prompt_handler = PromptRequestHandler(
            self.agent_id, prompt_processor=self.callbacks.get("on_prompt_request")
        )
        self.register_handler(prompt_handler)

        # Prompt response handler
        response_handler = PromptResponseHandler(
            self.agent_id, response_callback=self.callbacks.get("on_prompt_response")
        )
        ...
        agent_response_handler = AgentResponseHandler(
            self.agent_id, response_callback=self.callbacks.get("on_agent_response")
        )
    ...
    def track_sent_message(self, message: BeastModeMessage) -> None:
        ...
        for handlers in self.handlers.values():
            for handler in handlers:
                if (
                    hasattr(handler, "track_request")
                    and message.type == MessageType.PROMPT_REQUEST
                ):
                    handler.track_request(message)
                elif (
                    hasattr(handler, "track_help_request")
                    and message.type == MessageType.HELP_WANTED
                ):
    ...
    def _trim_history(self) -> None:
        ...
        if len(self.sent_messages) > self.max_history:
            ...
            sorted_messages = sorted(
                self.sent_messages.items(), key=lambda x: x[1].timestamp
            )
            ...
            if self.auto_convert_legacy:
                try:
                    ...
                    result["warnings"].append(
                        "Message was converted from legacy format"
                    )
    ...
    def create_test_message(self, msg_type: MessageType, **kwargs) -> BeastModeMessage:
        ...
        if msg_type == MessageType.PROMPT_REQUEST:
            ...
            base_payload["required_capabilities"] = kwargs.get(
            ...
            base_payload["spore_content"] = kwargs.get(
                "spore_content", "Test spore content"
            )
