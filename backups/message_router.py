"""
Beast Mode Message Router

Provides message routing and handler management for the Beast Mode messaging system.
Integrates with BeastModeBusClient to provide standardized message type handling.
"""
...
from .message_handlers import (
    MessageRouter, BaseMessageHandler, SimpleMessageHandler, PromptRequestHandler,
    PromptResponseHandler, AgentDiscoveryHandler, AgentResponseHandler,
    HelpWantedHandler, HelpResponseHandler, SporeDeliveryHandler,
    SporeRequestHandler, SporeSpawnHandler, TechnicalExchangeHandler, SystemHealthHandler,
...
class StandardMessageRouter(MessageRouter):
    ...
    def __init__(
        ...
        # Disable auto legacy conversion by default for strict validation
    ...
    def _setup_standard_handlers(self) -> None:
        ...
        simple_handler = SimpleMessageHandler(
            self.agent_id,
            message_callback=self.callbacks.get('on_simple_message')
        )
        ...
        prompt_handler = PromptRequestHandler(
            self.agent_id,
            prompt_processor=self.callbacks.get('on_prompt_request')
        )
        ...
        response_handler = PromptResponseHandler(
            self.agent_id,
            response_callback=self.callbacks.get('on_prompt_response')
        )
        ...
        discovery_handler = AgentDiscoveryHandler(
            self.agent_id,
            self.capabilities,
            discovery_callback=self.callbacks.get('on_agent_discovery')
        )
        ...
        agent_response_handler = AgentResponseHandler(
            self.agent_id,
            response_callback=self.callbacks.get('on_agent_response')
        )
        self.register_handler(agent_response_handler)
        
        # Help wanted handler
        help_handler = HelpWantedHandler(
            self.agent_id,
            self.capabilities,
            help_callback=self.callbacks.get('on_help_wanted')
        )
        self.register_handler(help_handler)
        
        # Help response handler
        help_response_handler = HelpResponseHandler(
            self.agent_id,
            response_callback=self.callbacks.get('on_help_response')
        )
        self.register_handler(help_response_handler)
        
        # Spore delivery handler
        spore_handler = SporeDeliveryHandler(
            self.agent_id,
            spore_callback=self.callbacks.get('on_spore_delivery')
        )
        self.register_handler(spore_handler)
        
        # Spore request handler
        spore_request_handler = SporeRequestHandler(
            self.agent_id,
            spore_provider=self.callbacks.get('on_spore_request')
        )
        self.register_handler(spore_request_handler)
        
        # Spore spawn handler
        spore_spawn_handler = SporeSpawnHandler(
            self.agent_id,
            spawn_callback=self.callbacks.get('on_spore_spawn')
        )
        self.register_handler(spore_spawn_handler)
        
        # Technical exchange handler
        tech_handler = TechnicalExchangeHandler(
            self.agent_id,
            tech_callback=self.callbacks.get('on_technical_exchange')
        )
        self.register_handler(tech_handler)
        
        # System health handler
        health_handler = SystemHealthHandler(
            self.agent_id,
            health_callback=self.callbacks.get('on_system_health')
        )
    ...
    def track_sent_message(self, message: BeastModeMessage) -> None:
        ...
        # Update specific handlers that need correlation tracking
    ...
    def update_capabilities(self, capabilities: List[str]) -> None:
        ...
        # Update handlers that use capabilities
        for handlers in self.handlers.values():
            for handler in handlers:
    ...
    def get_correlation_info(self, message_id: str) -> Optional[Dict[str, Any]]:
        ...
        for msg in self.received_messages:
            if msg.id == message_id:
                # Find original if this is a response
                original = None
    ...
    def get_handler_info(self) -> Dict[str, Any]:
        ...
        for msg_type, handlers in self.handlers.items():
            info['handlers_by_type'][msg_type.value] = [
                h.get_stats() for h in handlers
            ]
            info['total_handlers'] += len(handlers)
        
        # Callback status
        expected_callbacks = [
            'on_simple_message', 'on_prompt_request', 'on_prompt_response',
            'on_agent_discovery', 'on_agent_response', 'on_help_wanted',
            'on_help_response', 'on_spore_delivery', 'on_spore_request',
            'on_spore_spawn', 'on_technical_exchange', 'on_system_health'
        ]
    ...
    def create_test_message(self, msg_type: MessageType, **kwargs) -> BeastModeMessage:
        ...
        if msg_type == MessageType.PROMPT_REQUEST:
            ...
            base_payload['agent_capabilities'] = AgentCapabilities(
                agent_id=kwargs.get('test_agent_id', 'test_agent'),
                capabilities=kwargs.get('test_capabilities', ['testing'])
            ).model_dump()
...
class MessageTypeRegistry:
    ...
    def _build_type_info(self) -> Dict[MessageType, Dict[str, Any]]:
        ...
        return {
            MessageType.SIMPLE_MESSAGE: {
                'description': 'Basic text communication between agents',
                'required_fields': ['content'],
                'optional_fields': ['context', 'metadata'],
                'response_type': None,
                'handler_class': 'SimpleMessageHandler'
            },
            
            MessageType.PROMPT_REQUEST: {
                'description': 'Request for processing a prompt',
                'required_fields': ['prompt'],
                'optional_fields': ['context', 'parameters', 'timeout'],
                'response_type': MessageType.PROMPT_RESPONSE,
                'handler_class': 'PromptRequestHandler'
            },
            
            MessageType.PROMPT_RESPONSE: {
                'description': 'Response to a prompt request',
                'required_fields': ['response'],
                'optional_fields': ['original_prompt', 'processed_at', 'metadata'],
                'response_type': None,
                'handler_class': 'PromptResponseHandler'
            },
            
            MessageType.AGENT_DISCOVERY: {
                'description': 'Agent presence announcement',
                'required_fields': ['agent_capabilities'],
                'optional_fields': ['announcement', 'metadata'],
                'response_type': MessageType.AGENT_RESPONSE,
                'handler_class': 'AgentDiscoveryHandler'
            },
            
            MessageType.AGENT_RESPONSE: {
                'description': 'Response to agent discovery',
                'required_fields': ['agent_capabilities'],
                'optional_fields': ['response_to', 'metadata'],
                'response_type': None,
                'handler_class': 'AgentResponseHandler'
            },
            
            MessageType.HELP_WANTED: {
                'description': 'Request for assistance with specific capabilities',
                'required_fields': ['required_capabilities', 'description'],
                'optional_fields': ['urgency', 'max_helpers', 'timeout_hours', 'context'],
                'response_type': MessageType.HELP_RESPONSE,
                'handler_class': 'HelpWantedHandler'
            },
            
            MessageType.HELP_RESPONSE: {
                'description': 'Offer to help with a request',
                'required_fields': ['request_id', 'can_help'],
                'optional_fields': ['matching_capabilities', 'confidence_score', 'response_message'],
                'response_type': None,
                'handler_class': 'HelpResponseHandler'
            },
            
            MessageType.SPORE_DELIVERY: {
                'description': 'Delivery of a spore with methodology',
                'required_fields': ['spore_name', 'spore_content'],
                'optional_fields': ['metadata', 'version', 'dependencies'],
                'response_type': None,
                'handler_class': 'SporeDeliveryHandler'
            },
            
            MessageType.SPORE_REQUEST: {
                'description': 'Request for a specific spore',
                'required_fields': ['spore_name'],
                'optional_fields': ['version', 'metadata'],
                'response_type': MessageType.SPORE_DELIVERY,
                'handler_class': 'SporeRequestHandler'
            },
            
            MessageType.TECHNICAL_EXCHANGE: {
                'description': 'Technical information exchange',
                'required_fields': [],
                'optional_fields': ['topic', 'data', 'metadata'],
                'response_type': None,
                'handler_class': 'TechnicalExchangeHandler'
            },
            
            MessageType.SPORE_SPAWN: {
                'description': 'Spore spawn request for creating new spores',
                'required_fields': ['spore_type'],
                'optional_fields': ['metadata', 'parameters'],
                'response_type': None,
                'handler_class': 'SporeSpawnHandler'
            },
            
            MessageType.SYSTEM_HEALTH: {
                'description': 'System health and monitoring information',
                'required_fields': [],
                'optional_fields': ['status', 'metrics', 'alerts'],
                'response_type': None,
                'handler_class': 'SystemHealthHandler'
            }
        }
    ...
    def get_types_with_responses(self) -> Dict[MessageType, MessageType]:
        ...
        return {
            msg_type: info['response_type']
            for msg_type, info in self.type_info.items()
