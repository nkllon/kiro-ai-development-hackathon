"""
Adr System Core Core

This module was extracted from adr_system_core.py
...
class ADRSystem(ReflectiveModule):
        ...
        return {
            ...
            "accepted_decisions": len(
                [
                    adr
                    for adr in self.adrs.values()
                    if adr.status == DecisionStatus.ACCEPTED
                ]
            ),
    ...
    def create_adr(
        ...
        if template_id and template_id in self.adr_templates:
            ...
            adr = self._create_from_template(
                adr_id, title, category, context, decision_makers, template
            )
        else:
    ...
    def add_decision_option(
        self, adr_id: str, option: DecisionOption
    ) -> Dict[str, Any]:
        """
    ...
    def make_decision(
        ...
        if not any(
            (opt.option_id == chosen_option_id for opt in adr.options_considered)
        ):
        ...
        self._update_adr_metrics()
        self.logger.info(
            f"Decision made for {adr_id}: Option {chosen_option_id} chosen"
        )
        return {
            "success": True,
            "adr_id": adr_id,
    ...
    def supersede_adr(
        self, old_adr_id: str, new_adr_id: str, reason: str
    ) -> Dict[str, Any]:
        """
    ...
    def search_adrs(
        ...
        for adr in self.adrs.values():
            ...
            if "category" in filters and adr.category.value not in filters["category"]:
                continue
            if "decision_maker" in filters and (
                not any((dm in adr.decision_makers for dm in filters["decision_maker"]))
            ):
                continue
            if "tag" in filters and (
                not any((tag in adr.tags for tag in filters["tag"]))
            ):
                continue
            results.append(
                {
                    ...
                    "summary": (
                        adr.context.problem_statement[:200] + "..."
                        if len(adr.context.problem_statement) > 200
                        else adr.context.problem_statement
                    ),
                }
            )
    ...
    def get_adr_details(self, adr_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "superseding_adr": (
                ...
                if adr.superseded_by and adr.superseded_by in self.adrs
                else None
            ),
        }
    ...
    def generate_adr_report(self, filters: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        return {
            ...
            "stakeholder_involvement": self._generate_stakeholder_analysis(
                filtered_adrs
            ),
    ...
    def export_adrs(
        self, format: str = "json", filters: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        """
...
def get_module_status(self) -> Dict[str, Any]:
    ...
    return {
        ...
        "total_adrs": len(self.adrs),
        "active_decisions": len(
            [adr for adr in self.adrs.values() if adr.status == DecisionStatus.PROPOSED]
        ),
        "accepted_decisions": len(
            [adr for adr in self.adrs.values() if adr.status == DecisionStatus.ACCEPTED]
        ),
        "categories_covered": len(self.category_index),
        "storage_path": str(self.storage_path),
    }
...
def search_adrs(
    ...
    for adr in self.adrs.values():
        if query:
            searchable_text = (
                f"{adr.title} {adr.context.problem_statement} {adr.rationale}".lower()
            )
            if query.lower() not in searchable_text:
                continue
        if "status" in filters and adr.status.value not in filters["status"]:
...
def get_adr_details(self, adr_id: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "superseding_adr": (
            {"adr_id": adr.superseded_by, "title": self.adrs[adr.superseded_by].title}
            if adr.superseded_by and adr.superseded_by in self.adrs
            else None
        ),
    }
