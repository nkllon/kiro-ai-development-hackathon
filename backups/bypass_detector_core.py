...
class GovernanceBypassDetector:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.escalation_timeout = timedelta(
            hours=self.config.get("escalation_hours", 48)
        )
    ...
    def detect_bypass_attempts(
        ...
        circumvention_pattern = self._detect_process_circumvention(
            actor_id, governance_events
        )
        if circumvention_pattern:
            patterns.append(circumvention_pattern)
        escalation_pattern = self._detect_authority_escalation(
            actor_id, recent_decisions
        )
    ...
    def check_escalation_needed(
        self, actor_id: str, pattern_duration: timedelta
    ...
    def create_governance_intervention(
        self, actor_id: str, bypass_alert: BypassAlert
    ) -> GovernanceIntervention:
        """
    ...
    def _detect_emergency_abuse(
        self, actor_id: str, decisions: List[Decision]
    ) -> Optional[BypassPattern]:
        ...
        if len(emergency_decisions) > len(decisions) * 0.3:
            return BypassPattern(
                ...
                severity=(
                    "high"
                    if len(emergency_decisions) > len(decisions) * 0.5
                    else "medium"
                ),
                evidence=[
    ...
    def _detect_approval_skipping(
        ...
        high_impact_decisions = [
            d
            for d in decisions
            if d.impact_level in ["high", "critical"]
            and (not d.accountability_verified)
        ]
    ...
    def _detect_process_circumvention(
        self, actor_id: str, governance_events: List[Dict]
    ) -> Optional[BypassPattern]:
        ...
        circumvention_events = [
            ...
            in ["process_skip", "alternative_path", "unauthorized_access"]
        ]
        if len(circumvention_events) >= 3:
            return BypassPattern(
                ...
                confidence=0.8,
                first_detected=datetime.now() - timedelta(days=1),
    ...
    def _detect_authority_escalation(
        ...
        escalation_decisions = [
            d
            for d in decisions
            if d.metadata.get("authority_escalation", False)
            or d.metadata.get("self_authorized", False)
        ]
    ...
    def _calculate_bypass_severity(self, patterns: List[BypassPattern]) -> float:
        ...
        for pattern in patterns:
            pattern_weight = self.bypass_indicators.get(pattern.pattern_type, {}).get(
                "weight", 0.5
            )
            severity_score = {
                "low": 0.3,
    ...
    def _create_bypass_alert(
        self, actor_id: str, patterns: List[BypassPattern], severity: float
    ) -> BypassAlert:
    ...
    def _store_patterns(self, actor_id: str, patterns: List[BypassPattern]):
        ...
        self.detected_patterns[actor_id] = [
            p
            for p in self.detected_patterns[actor_id]
    ...
    def _create_escalation_path(
        self, actor_id: str, intervention_type: InterventionType
    ) -> List[Dict]:
        ...
        if intervention_type in [
            ...
            InterventionType.EMERGENCY_GOVERNANCE,
        ]:
            base_path.append(
                EscalationStep(
                    step_order=2,
    ...
    def _create_rollback_plan(
        ...
        if intervention_type == InterventionType.EMERGENCY_GOVERNANCE:
            timeline = timedelta(hours=2)
            trigger_conditions = [
                "Intervention proves ineffective",
                "Actor compliance restored",
            ]
        else:
            timeline = timedelta(hours=8)
            trigger_conditions = [
                "Voluntary compliance restored",
...
def detect_bypass_attempts(
    self, actor_id: str, recent_decisions: List[Decision], governance_events: List[Dict]
) -> Optional[BypassAlert]:
    """
    Detect governance bypass attempts for an actor.
...
def _detect_emergency_abuse(
    ...
    if len(emergency_decisions) > len(decisions) * 0.3:
        return BypassPattern(
            pattern_type="emergency_abuse",
            severity=(
                "high" if len(emergency_decisions) > len(decisions) * 0.5 else "medium"
            ),
            evidence=[
                f"{len(emergency_decisions)} emergency claims out of {len(decisions)} decisions",
                f"Emergency rate: {len(emergency_decisions) / len(decisions):.1%}",
...
def _calculate_bypass_severity(self, patterns: List[BypassPattern]) -> float:
    ...
    for pattern in patterns:
        ...
        severity_score = {"low": 0.3, "medium": 0.6, "high": 0.8, "critical": 1.0}.get(
...
def _store_patterns(self, actor_id: str, patterns: List[BypassPattern]):
    ...
    cutoff_time = datetime.now() - self.pattern_memory
    self.detected_patterns[actor_id] = [
        p for p in self.detected_patterns[actor_id] if p.last_detected >= cutoff_time
    ]
