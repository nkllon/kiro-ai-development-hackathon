...
class ArchitectureExpert(GhostbustersExpertAgent):
    ...
    def _init_architecture_patterns(self):
        ...
        self.solid_violations = {
            ...
            "dip": [
                r"import\s+\w+\.\w+\.\w+"
            ],  # Dependency Inversion (concrete imports)
        }
    ...
    async def analyze(self, context: AnalysisContext) -> AnalysisResult:
        ...
        try:
            ...
            result = AnalysisResult(
                ...
                context=context,
                metadata={
                    "architecture_patterns_detected": self._get_detected_patterns(
                        findings
                    ),
                    "anti_patterns_detected": self._get_detected_anti_patterns(
                        findings
                    ),
            ...
            logger.info(
                f"Architecture analysis completed for {target_path} with {len(findings)} findings"
            )
            return result

        except Exception as e:
            logger.error(
                f"Architecture analysis failed for {context.target_path}: {str(e)}"
            )
        ...
        try:
            ...
            if file_path.suffix == ".py":
                try:
                    ...
                    findings.extend(
                        self._analyze_python_architecture(tree, content, file_path)
                    )
                except SyntaxError:
    ...
    def _analyze_python_architecture(
        ...
        class ArchitectureVisitor(ast.NodeVisitor):
            ...
            def visit_ClassDef(self, node):
                ...
                if any(
                    ...
                    for pattern in ["manager", "handler", "controller", "util"]
                ):
                    self.findings.append(
                        Finding(
                            type=FindingType.ARCHITECTURE_VIOLATION,
                ...
                if param_count > 5:
                    self.findings.append(
                        Finding(
                            ...
                            confidence=0.8,
                            evidence={
                                "anti_pattern": "long_parameter_list",
    ...
    def _analyze_generic_architecture(
    ...
    def _check_solid_violations(self, content: str, file_path: Path) -> List[Finding]:
        ...
        if re.search(r"if\s+isinstance\s*\(.*,\s*\w+\)", content) or re.search(
            r"type\s*\(.*\)\s*==", content
        ):
            findings.append(
                Finding(
    ...
    async def _generate_architecture_recommendations(
        ...
        arch_violations = [
            f for f in findings if f.type == FindingType.ARCHITECTURE_VIOLATION
        ]
        god_classes = [
            f for f in arch_violations if f.evidence.get("anti_pattern") == "god_class"
        ]
        solid_violations = [
            f for f in arch_violations if "solid_violation" in f.evidence
        ]
        ...
        circular_deps = [
            f
            for f in arch_violations
            if f.evidence.get("anti_pattern") == "circular_dependency"
        ]
        if circular_deps:
            recommendations.append(
                Recommendation(
