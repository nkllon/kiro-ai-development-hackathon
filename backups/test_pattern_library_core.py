...
class TestPatternLibrary(ReflectiveModule):
    ...
    def get_module_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "performance_target_met": avg_match_time < 1000,
        ...
        return {
            ...
            "learning_health": {
                ...
                "patterns_learned": len(
                    [r for r in self.learning_data if r.successful_fix_applied]
                ),
            },
        }
    ...
    def match_test_patterns(self, failure: Failure) -> List[PreventionPattern]:
        ...
        try:
            ...
            self.logger.info(
                f"Test pattern matching completed in {match_time_ms:.2f}ms, found {len(matching_patterns)} matches"
            )
            if match_time_ms > 1000:
                self.logger.warning(
                    f"Test pattern matching exceeded 1 second: {match_time_ms:.2f}ms"
                )
    ...
    def learn_from_successful_rca(
        ...
        try:
            if validation_score < self.learning_threshold:
                self.logger.debug(
                    f"Skipping pattern learning - validation score {validation_score} below threshold {self.learning_threshold}"
                )
                return False
            new_pattern = self._generate_pattern_from_rca(
                failure, root_causes, systematic_fixes
            )
            if new_pattern:
                existing_pattern = self._find_similar_pattern(new_pattern)
                if existing_pattern:
                    self._enhance_existing_pattern(
                        existing_pattern, new_pattern, validation_score
                    )
                else:
                ...
                learning_record = TestPatternLearning(
                    ...
                    generalization_potential=self._calculate_generalization_potential(
                        failure, root_causes
                    ),
                )
                ...
                self.logger.info(
                    f"Learned new test pattern: {new_pattern.pattern_name}"
                )
                return True
        except Exception as e:
    ...
    def optimize_pattern_performance(self) -> Dict[str, Any]:
        ...
        try:
            avg_time_before = self.total_match_time_ms / max(
                1, self.total_matches_performed
            )
            patterns_to_remove = []
            for pattern_id, metrics in self.pattern_metrics.items():
                if (
                    metrics.match_count > 10
                    and metrics.effectiveness_score < 0.3
                    and (metrics.false_positive_count > metrics.successful_applications)
                ):
            ...
            avg_time_after = self.total_match_time_ms / max(
                1, self.total_matches_performed
            )
            ...
            self.logger.info(
                f"Pattern optimization complete: removed {optimization_results['patterns_removed']} patterns, performance improvement: {optimization_results['performance_improvement']:.2f}ms"
            )
        except Exception as e:
        ...
        try:
            ...
            for pattern_id, metrics in self.pattern_metrics.items():
                if (
                    ...
                    and metrics.last_matched < cutoff_date
                    and (metrics.match_count < 5)
                ):
            ...
            if len(self.learning_data) > 1000:
                self.learning_data.sort(
                    key=lambda x: x.learning_timestamp, reverse=True
                )
            ...
            self.last_cleanup = datetime.now()
            self.logger.info(
                f"Pattern library cleanup complete: removed {cleanup_results['duplicate_patterns_removed']} duplicates, {cleanup_results['stale_patterns_removed']} stale patterns"
            )
        except Exception as e:
    ...
    def get_pattern_effectiveness_report(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            "average_match_time_ms": self.total_match_time_ms
            / max(1, self.total_matches_performed),
                ...
                report["top_performing_patterns"].append(
                    {
                        ...
                        "success_rate": metrics.successful_applications
                        / max(1, metrics.match_count),
                    }
                )
        avg_match_time = report["average_match_time_ms"]
        if avg_match_time > 1000:
            report["performance_issues"].append(
                f"Average match time {avg_match_time:.2f}ms exceeds 1 second target"
            )
        if report["cache_hit_rate"] < 0.7:
            report["performance_issues"].append(
                f"Cache hit rate {report['cache_hit_rate']:.2f} below optimal threshold"
            )
        return report
    ...
    def _load_test_patterns(self):
        ...
        try:
            if Path(self.test_patterns_path).exists():
                ...
                self.logger.info(
                    f"Loaded {len(self.test_patterns)} test-specific patterns"
                )
        except Exception as e:
            ...
            if Path(self.pattern_metrics_path).exists():
                ...
                for metrics_data in data.get("metrics", []):
                    metrics = TestPatternMetrics(
                        ...
                        successful_applications=metrics_data.get(
                            "successful_applications", 0
                        ),
                        average_match_time_ms=metrics_data.get(
                            "average_match_time_ms", 0.0
                        ),
                        last_matched=(
                            datetime.fromisoformat(metrics_data["last_matched"])
                            if metrics_data.get("last_matched")
                            else None
                        ),
                        effectiveness_score=metrics_data.get(
                            "effectiveness_score", 0.0
                        ),
                        false_positive_count=metrics_data.get(
                ...
                metrics_data.append(
                    {
                        ...
                        "last_matched": (
                            metrics.last_matched.isoformat()
                            if metrics.last_matched
                            else None
                        ),
                        "effectiveness_score": metrics.effectiveness_score,
            ...
            if Path(self.learning_data_path).exists():
                ...
                for learning_data in data.get("learning_records", []):
                    record = TestPatternLearning(
                        ...
                        source_failure_signature=learning_data[
                        ...
                        learning_timestamp=datetime.fromisoformat(
                            learning_data["learning_timestamp"]
                        ),
                        context_factors=learning_data.get("context_factors", {}),
                        generalization_potential=learning_data.get(
                            "generalization_potential", 0.0
                        ),
                    )
    ...
    def _build_performance_indexes(self):
        ...
        for pattern_id, pattern in self.test_patterns.items():
            ...
            component = self._extract_component_from_signature(
    ...
    def _verify_test_pattern_match(
        ...
        if len(signature_parts) >= 3 and len(failure_parts) >= 3:
            component_match = (
                signature_parts[0] in failure_parts[0]
                or failure_parts[0] in signature_parts[0]
            )
            error_type_match = signature_parts[1] == failure_parts[1]
            error_msg_similarity = self._calculate_message_similarity(
                signature_parts[2], failure_parts[2]
            )
    ...
    def _update_pattern_metrics(self, pattern_id: str, match_successful: bool):
        ...
        metrics.effectiveness_score = metrics.successful_applications / max(
            1, metrics.match_count
        )
        if metrics.match_count % 10 == 0:
            self._save_pattern_metrics()

    def _generate_pattern_from_rca(
        self,
        failure: Failure,
        root_causes: List[RootCause],
        systematic_fixes: List[SystematicFix],
    ) -> Optional[PreventionPattern]:
        ...
        pattern_id = (
            f"test_pattern_{primary_root_cause.cause_type.value}_{int(time.time())}"
        )
        failure_signature = self._generate_test_failure_signature(failure)
    ...
    def _calculate_pattern_similarity(
        self, pattern1: PreventionPattern, pattern2: PreventionPattern
    ) -> float:
        """Calculate similarity between two patterns"""
        sig_similarity = self._calculate_message_similarity(
            pattern1.failure_signature, pattern2.failure_signature
        )
        cause_similarity = self._calculate_message_similarity(
            pattern1.root_cause_pattern, pattern2.root_cause_pattern
        )
        return (sig_similarity + cause_similarity) / 2

    def _enhance_existing_pattern(
        self,
        existing_pattern_id: str,
        new_pattern: PreventionPattern,
        validation_score: float,
    ):
        ...
        if existing_pattern_id in self.pattern_metrics:
            ...
            metrics.effectiveness_score = (
                metrics.effectiveness_score + validation_score
            ) / 2
    ...
    def _add_new_test_pattern(self, pattern: PreventionPattern):
        ...
        self.test_patterns[pattern.pattern_id] = pattern
        self.pattern_metrics[pattern.pattern_id] = TestPatternMetrics(
            pattern_id=pattern.pattern_id
        )
        self._build_performance_indexes()
        self._save_test_patterns()
    ...
    def _find_duplicate_patterns(self) -> List[str]:
        ...
        for pattern_id, pattern in self.test_patterns.items():
            ...
            if pattern_hash in seen_hashes:
                ...
                existing_metrics = self.pattern_metrics.get(
                    existing_id, TestPatternMetrics(pattern_id=existing_id)
                )
                current_metrics = self.pattern_metrics.get(
                    pattern_id, TestPatternMetrics(pattern_id=pattern_id)
                )
                if (
                    current_metrics.effectiveness_score
                    < existing_metrics.effectiveness_score
                ):
    ...
    def _calculate_generalization_potential(
        self, failure: Failure, root_causes: List[RootCause]
    ) -> float:
    ...
    def _trigger_performance_optimization(self):
        ...
        self.logger.info(
            "Triggering performance optimization due to slow pattern matching"
        )
