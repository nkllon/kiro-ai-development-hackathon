...
This module was extracted from analyzer_core.py
...
class GitAnalyzer(ReflectiveModule, ComplianceAnalyzer):
    ...
    def __init__(self, repository_path: str = "."):
        ...
        self.logger.info(
            f"GitAnalyzer initialized for repository: {self.repository_path}"
        )

    def get_commits_ahead_of_main(
        self, target_branch: str = "HEAD", base_branch: str = "origin/master"
    ) -> List[CommitInfo]:
        """
        ...
        try:
            ...
            self.logger.info(
                f"Successfully analyzed {len(commits)} commits ahead of {base_branch}"
            )
            return commits
    ...
    def analyze_file_changes(
        ...
        try:
            ...
            for commit in commits:
                ...
                lines_added, lines_deleted = self._get_commit_line_changes(
                    commit.commit_hash
                )
            ...
            analysis = FileChangeAnalysis(
                total_files_changed=len(net_added)
                + len(net_modified)
                + len(net_deleted),
            ...
            self.logger.info(
                f"File change analysis complete: {analysis.total_files_changed} files changed ({len(analysis.files_added)} added, {len(analysis.files_modified)} modified, {len(analysis.files_deleted)} deleted)"
            )
            return analysis
        except Exception as e:
    ...
    def map_changes_to_tasks(
        self,
        file_changes: FileChangeAnalysis,
        task_patterns: Optional[Dict[str, List[str]]] = None,
    ) -> Dict[str, List[str]]:
        """
    ...
    def analyze(self, context: Dict[str, Any]) -> ComplianceAnalysisResult:
        ...
        try:
            ...
            result.commits_analyzed = self.get_commits_ahead_of_main(
                target_branch, base_branch
            )
            file_changes = self.analyze_file_changes(result.commits_analyzed)
            result.recommendations.append(
                f"Analyzed {len(result.commits_analyzed)} commits with {file_changes.total_files_changed} file changes"
            )
    ...
    def is_healthy(self) -> bool:
        ...
        try:
            return (
                ...
                and (self.repository_path / ".git").exists()
                and self._can_execute_git_commands()
            )
        except Exception:
            return False
    ...
    def get_health_indicators(self) -> Dict[str, Any]:
        ...
        try:
            ...
            git_exists = (
                (self.repository_path / ".git").exists() if repo_exists else False
            )
            ...
            indicators["git_repository"] = {
                ...
                "value": git_exists,
                "message": (
                    "Git repository detected" if git_exists else "Not a git repository"
                ),
            }
            indicators["git_executable"] = {
                ...
                "message": (
                    ...
                    if git_executable
                    else "Cannot execute git commands"
                ),
            }
        except Exception as e:
            indicators["error"] = {
    ...
    def _get_commit_hashes_ahead(
        self, target_branch: str, base_branch: str
    ) -> List[str]:
    ...
    def _get_commit_info(self, commit_hash: str) -> Optional[CommitInfo]:
        ...
        try:
            ...
            if result.returncode != 0:
                self.logger.error(
                    f"Failed to get commit info for {commit_hash}: {result.stderr}"
                )
                return None
    ...
    def _get_default_task_patterns(self) -> Dict[str, List[str]]:
        ...
        return {
            ...
            "git_analysis": ["src/beast_mode/compliance/git/*", "tests/*git*"],
            "rdi_validation": ["src/beast_mode/compliance/rdi/*", "tests/*rdi*"],
            "rm_validation": ["src/beast_mode/compliance/rm/*", "tests/*rm*"],
            "reporting": ["src/beast_mode/compliance/reporting/*", "tests/*report*"],
