...
This module was extracted from tool_orchestration_engine.py
...
from ..ghostbusters.multi_perspective_validator import (
    MultiPerspectiveValidator as MultiStakeholderPerspectiveEngine,
)
...
class ToolOrchestrationEngine(ReflectiveModule):
    ...
    def __init__(self, project_root: str = "."):
        ...
        self.orchestration_metrics = {
            ...
            "decision_confidence_distribution": {"high": 0, "medium": 0, "low": 0},
        }
    ...
    def get_module_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "healthy_tools": len(
                [t for t in self.tool_health_cache.values() if t == ToolStatus.HEALTHY]
            ),
            "total_orchestrations": self.orchestration_metrics["total_orchestrations"],
            "success_rate": self._calculate_success_rate(),
            "average_execution_time_ms": self.orchestration_metrics[
    ...
    def is_healthy(self) -> bool:
        ...
        return (
            ...
            and len(self.tools_registry) > 0
            and self.intelligence_engine.is_healthy()
        ...
        return {
            "orchestration_status": {
                ...
                "healthy_tools": len(
                    [
                        t
                        for t in self.tool_health_cache.values()
                        if t == ToolStatus.HEALTHY
                    ]
                ),
                "failed_tools": len(
                    [
                        t
                        for t in self.tool_health_cache.values()
                        if t == ToolStatus.FAILED
                    ]
                ),
                "success_rate": self._calculate_success_rate(),
            },
            "decision_framework": {
                "confidence_distribution": self.orchestration_metrics[
            ...
            "performance_metrics": {
                "total_orchestrations": self.orchestration_metrics[
                    "total_orchestrations"
                ],
                "average_execution_time": self.orchestration_metrics[
                    "average_execution_time_ms"
                ],
    ...
    def register_tool(self, tool_definition: ToolDefinition) -> Dict[str, Any]:
        ...
        self.logger.info(
            f"Tool registered: {tool_definition.name} ({tool_definition.tool_id})"
        )
        return {
            "success": True,
    ...
    def orchestrate_tool_execution(
        self,
        ...
        self.logger.info(
            f"Decision confidence: {confidence_level.value} ({confidence_score:.2f})"
        )
        decision_result = self._route_decision_by_confidence(
            decision_context, confidence_level, preferred_tools
        )
        execution_result = self._execute_tools_systematically(
            decision_result["selected_tools"], decision_context, operation_id
        )
        if not execution_result["success"]:
            repair_result = self._handle_tool_failures_systematically(
                execution_result["failed_tools"], decision_context, operation_id
            )
        ...
        self.logger.info(
            f"Tool orchestration completed: {operation_id} (Success: {result.success})"
        )
        return result
    ...
    def _assess_decision_confidence(self, context: DecisionContext) -> Dict[str, Any]:
        ...
        if context.confidence_score > 0.0:
            ...
            if context.risk_tolerance == "low":
                ...
                confidence_factors.append(
                    "Low risk tolerance requires higher confidence"
                )
        ...
        return {
            ...
            "registry_result": (
                registry_result if context.confidence_score == 0.0 else None
            ),
        }

    def _route_decision_by_confidence(
        self,
    ...
    def _make_high_confidence_decision(
        self, context: DecisionContext, preferred_tools: Optional[List[str]] = None
    ) -> Dict[str, Any]:
        """
        ...
        self.logger.info(
            f"Making high confidence decision (80%+): {context.confidence_score:.1%}"
        )
        domain_tools = self.intelligence_engine.get_domain_tools(
            context.domain or "general"
        )
        available_tools = [
            tool_id for tool_id in domain_tools if tool_id in self.tools_registry
        ]
        if preferred_tools:
            prioritized_tools = [
                tool for tool in preferred_tools if tool in available_tools
            ] + [
                tool for tool in available_tools if tool not in (preferred_tools or [])
            ]
    ...
    def _make_medium_confidence_decision(
        ...
        self.logger.info(
            f"Making medium confidence decision (50-80%): {context.confidence_score:.1%}"
        )
        model_recommendation = self._make_high_confidence_decision(
    ...
    def _make_low_confidence_decision(
        ...
        self.logger.info(
            f"Making low confidence decision (<50%): {context.confidence_score:.1%}"
        )
        rca_analysis = None
        if hasattr(context, "failure_context") and context.failure_context:
            try:
                ...
                failure = Failure(
                    ...
                    error_message=context.failure_context.get(
                ...
                self.logger.info(
                    "Comprehensive RCA completed for low confidence decision"
                )
            except Exception as e:
                self.logger.warning(f"RCA analysis failed: {e}")
        stakeholder_analysis = (
            self.multi_perspective_engine.analyze_low_percentage_decision(context)
        )
        all_recommendations = []
            ...
            for fix in rca_analysis.systematic_fixes:
                if "tools" in fix.fix_description.lower():
                    all_recommendations.extend(
                        ["systematic_repair_tools", "rca_recommended_tools"]
                    )
        ...
        consensus_tools = [
            tool for tool, votes in tool_votes.most_common() if votes >= 2
        ]
        if not consensus_tools:
            consensus_tools = stakeholder_analysis.get(
                "synthesized_recommendation", {}
            ).get("tools", [])
        available_consensus = [
            tool for tool in consensus_tools if tool in self.tools_registry
        ]
        if not available_consensus and preferred_tools:
            available_consensus = [
                tool for tool in preferred_tools if tool in self.tools_registry
            ]
        return {
    ...
    def _execute_tools_systematically(
        self, selected_tools: List[str], context: DecisionContext, operation_id: str
    ) -> Dict[str, Any]:
        """
    ...
    def _execute_single_tool(
        self, tool_id: str, context: DecisionContext, operation_id: str
    ) -> ToolExecutionResult:
        """
        ...
        try:
            ...
            self.tool_health_cache[tool_id] = (
                ToolStatus.HEALTHY if success else ToolStatus.DEGRADED
            )
    ...
    def _handle_tool_failures_systematically(
        self, failed_tools: List[str], context: DecisionContext, operation_id: str
    ) -> Dict[str, Any]:
        """
    ...
    def _perform_tool_rca(
        self, tool_id: str, context: DecisionContext
    ) -> Dict[str, Any]:
        """
    ...
    def _attempt_systematic_repair(
        self, tool_id: str, rca_result: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
            ...
            try:
                ...
                if repair_result["success"]:
                    ...
                    if health_check["status"] == ToolStatus.HEALTHY:
                        self.logger.info(
                            f"Tool {tool_id} successfully repaired using procedure: {procedure}"
                        )
                        return {
                            "success": True,
                ...
                self.logger.warning(
                    f"Repair procedure failed for {tool_id}: {procedure} - {str(e)}"
                )
                continue
        return {
            "success": False,
    ...
    def _check_tool_health(self, tool_id: str) -> Dict[str, Any]:
        ...
        if tool_def.health_check_command:
            ...
            try:
                ...
                status = (
                    ToolStatus.HEALTHY if result.returncode == 0 else ToolStatus.FAILED
                )
            except Exception:
                status = ToolStatus.FAILED
    ...
    def _select_tools_by_health_and_priority(
        ...
        if execution_result["success"]:
            ...
            if confidence_result["confidence_level"] == DecisionConfidenceLevel.LOW:
                recommendations.append(
                    "Consider improving domain intelligence to increase decision confidence"
                )
        else:
            recommendations.append(
                "Tool orchestration failed - systematic repair attempted"
            )
            failed_tools = execution_result.get("failed_tools", [])
            if failed_tools:
                recommendations.append(
                    f"Consider alternative tools for: {', '.join(failed_tools)}"
                )
            if confidence_result["confidence_level"] == DecisionConfidenceLevel.HIGH:
                recommendations.append(
                    "High confidence decision failed - review domain intelligence accuracy"
                )
        total_time = execution_result.get("total_execution_time_ms", 0)
        if total_time > 5000:
            recommendations.append(
                "Execution time exceeded 5 seconds - consider tool optimization"
            )
        decision_method = decision_result.get("decision_method", "")
        if decision_method == "full_multi_stakeholder_analysis":
            recommendations.append(
                "Low confidence required full analysis - consider expanding domain intelligence"
            )
    ...
    def _update_orchestration_metrics(self, result: OrchestrationResult):
        ...
        new_avg = (
            current_avg * (total_ops - 1) + result.total_execution_time_ms
        ) / total_ops
    ...
    def get_decision_analytics(self) -> Dict[str, Any]:
        ...
        for level in ["high", "medium", "low"]:
            if confidence_counts[level] > 0:
                success_rates[level] = (
                    success_by_confidence[level] / confidence_counts[level]
                )
            else:
    ...
    def integrate_with_rca_engine(
        self, rca_engine_instance: Optional[Any] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            return {
                ...
                "rca_engine_healthy": self.rca_engine.is_healthy(),
                "rca_pattern_library_size": len(
                    getattr(self.rca_engine, "pattern_library", {})
                ),
    ...
    def add_adaptive_patterns_for_unknown_failures(
        self, unknown_failure_types: List[str]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            self.logger.info(
                f"Adding adaptive patterns for {len(unknown_failure_types)} unknown failure types"
            )
            adaptive_patterns = {}
    ...
    def handle_unknown_tool_failure(
        ...
        try:
            ...
            if matching_pattern:
                self.logger.info(
                    f"Applying adaptive pattern for unknown failure: {matching_pattern['failure_type']}"
                )
                response = self._apply_adaptive_response(
                    failure_context, matching_pattern
                )
    ...
    def _matches_failure_pattern(
        self, failure_signature: str, pattern: Dict[str, Any]
    ) -> bool:
    ...
    def _apply_adaptive_response(
        self, failure_context: Dict[str, Any], pattern: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _create_adaptive_pattern_for_unknown(
        self, failure_context: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _update_adaptive_pattern_learning(
        self, pattern: Dict[str, Any], response_outcome: Dict[str, Any]
    ):
        ...
        success_rate = sum(
            (1 for entry in pattern["learning_history"] if entry["success"])
        ) / len(pattern["learning_history"])
        if success_rate < 0.5:
            pattern["response_strategy"] = "escalate_to_rca"
            pattern["pattern_evolution"] = "evolved_due_to_low_success_rate"
        self.logger.info(
            f"Updated adaptive pattern learning: {pattern['failure_type']} (success rate: {success_rate:.2f})"
        )
    ...
    def get_decision_framework_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "rca_integration": {
                "integrated": hasattr(self, "rca_engine")
                and self.rca_engine is not None,
                "rca_engine_healthy": (
                    self.rca_engine.is_healthy()
                    if hasattr(self, "rca_engine")
                    else False
                ),
            },
            "adaptive_patterns": {
            ...
            "decision_metrics": self.orchestration_metrics[
                "decision_confidence_distribution"
            ],
