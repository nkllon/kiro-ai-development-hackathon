"""
Installation Validator Core Core

This module was extracted from installation_validator_core.py
...
class InstallationSetupValidator:
    ...
    def validate_installation_setup(self) -> InstallationReport:
        ...
        try:
            ...
            self.logger.info(
                f"Installation validation complete. Overall score: {overall_score:.1f}"
            )
            return report
    ...
    def validate_installation_reliability(self, num_tests: int = 3) -> ValidationResult:
        ...
        if success_rate < 50:
            recommendations.append(
                "Critical: Fix installation process - more than half of attempts fail"
            )
    ...
    def generate_installation_guide(self) -> str:
        ...
        guide_sections.append(
            "This guide provides step-by-step instructions for installing and setting up the project.\n"
        )
        ...
        for config_file in config_files:
            if (self.project_path / config_file).exists():
                guide_sections.append(
                    f"Copy and configure the example configuration file:"
                )
                guide_sections.append("```bash")
                guide_sections.append(
                    f"cp {config_file} {config_file.replace('.example', '')}"
                )
                guide_sections.append("```")
        ...
        if not found_config:
            guide_sections.append(
                "No configuration files found. The project may work with default settings.\n"
            )
        ...
        if (self.project_path / "main.py").exists():
            ...
            guide_sections.append(
                "python -c \"import <main_module>; print('Installation successful!')\""
            )
        guide_sections.append("```\n")
        ...
        guide_sections.append(
            "3. **Virtual environment**: Make sure the virtual environment is activated"
        )
        guide_sections.append(
            "4. **Dependencies**: Try upgrading pip: `pip install --upgrade pip`\n"
        )
    ...
    def _validate_requirements_txt(
        ...
        for i, line in enumerate(lines):
            ...
            if not re.match(
                "^[a-zA-Z0-9_-]+", line.split("=")[0].split(">")[0].split("<")[0]
            ):
                issues.append(
                    InstallationIssue(
                        issue_type=InstallationIssueType.DEPENDENCY_CONFLICT,
    ...
    def _generate_recommendations(
        self, issues: List[InstallationIssue], scores: Dict[str, float]
    ) -> List[str]:
        ...
        if critical_issues:
            recommendations.append(
                f"Fix {len(critical_issues)} critical installation issues immediately"
            )
