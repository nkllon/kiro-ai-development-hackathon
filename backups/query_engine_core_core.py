"""
Query Engine Core Core

This module was extracted from query_engine_core.py
...
def natural_language_query(self, query: str) -> QueryResult:
    ...
    with self._time_operation("natural_language_query"):
        ...
        try:
            ...
            domains.sort(
                key=lambda d: (relevance_scores.get(d.name, 0.0), d.name), reverse=True
            )
            ...
            if self.registry_manager:
                ...
                for domain_name in matching_domains:
                    if domain_name in all_domains:
                        domains.append(all_domains[domain_name])
            domains.sort(
                key=lambda d: self._calculate_capability_relevance(d, capability_lower),
                reverse=True,
            )
            return domains
        except Exception as e:
...
def _find_similar_domains(
    ...
    similar_domains.sort(
        key=lambda d: self._calculate_domain_similarity(target_domain, d), reverse=True
    )
...
def _calculate_domain_similarity(self, domain1: Domain, domain2: Domain) -> float:
    ...
    indicator_jaccard = (
        len(indicators1.intersection(indicators2)) / len(indicators1.union(indicators2))
    ...
    with self._time_operation("advanced_relationship_analysis"):
        try:
            ...
            analysis = {
                "domain": domain_name,
                "direct_dependencies": [
                    d.name for d in self.relationship_query(domain_name, "dependencies")
                ],
                "direct_dependents": [
                    d.name for d in self.relationship_query(domain_name, "dependents")
                ],
                ...
                "extraction_impact": [
                    d.name
                    for d in self.relationship_query(domain_name, "extraction_related")
                ],
            }
            analysis["metrics"] = {
