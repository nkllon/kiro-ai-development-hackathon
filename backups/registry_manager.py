...
class DomainRegistryManager(CachedComponent, DomainRegistryInterface):
    ...
    def __init__(
        ...
        self.logger.info(
            f"Initialized DomainRegistryManager with registry: {self.registry_path}"
        )
    ...
    def load_registry(self) -> bool:
        ...
        with self._time_operation("load_registry"):
            try:
                ...
                self.logger.info(
                    f"Successfully loaded {len(self._domains)} domains from registry"
                )
                return True
        ...
        for category_name, category_data in domain_arch.items():
            ...
            if isinstance(category_data, dict) and "domains" in category_data:
                for domain_name in category_data["domains"]:
                    domain = self._create_domain_from_registry(
                        domain_name, category_name, category_data
                    )
                    if domain:
                        self._domains[domain_name] = domain

    def _create_domain_from_registry(
        self, domain_name: str, category: str, category_data: Dict
    ) -> Optional[Domain]:
        ...
        try:
            ...
            domain = Domain(
                ...
                description=category_data.get(
                    "description", f"Domain in {category} category"
                ),
    ...
    def _warm_cache(self) -> None:
        ...
        try:
            ...
            warmed_count = self._domain_cache.warm_domains(
                domain_loader, priority_domains
            )
