"""
Path Normalizer Core Core

This module was extracted from path_normalizer_core.py
...
class PathNormalizer:
    ...
    def ensure_relative_to(path: Union[str, Path], base: Union[str, Path]) -> Path:
        ...
        try:
            ...
            if not path_obj.is_absolute():
                ...
                raise ValueError(
                    f"Path '{path}' cannot be made relative to '{base}'. Normalized path '{normalized_path}' is not under normalized base '{normalized_base}'"
                ) from e
    ...
    def validate_path_consistency(
        paths: List[Union[str, Path]], base: Optional[Union[str, Path]] = None
    ) -> bool:
        """
    ...
    def get_common_base(paths: List[Union[str, Path]]) -> Optional[Path]:
        ...
        try:
            ...
            for path in normalized_paths[1:]:
                ...
                new_common = []
                for i, (common_part, path_part) in enumerate(
                    zip(common_parts, path_parts)
                ):
