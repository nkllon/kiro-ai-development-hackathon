...
class TestSafetyConfiguration:
    ...
    def __init__(self, test_mode: bool = None):
        ...
        self.logger.info(
            f"Test safety configuration initialized - test_mode: {self.test_mode}"
        )
    ...
    def is_operation_allowed(
        ...
        if operation in self.restricted_operations:
            self.logger.warning(
                f"Operation {operation} is restricted even in test mode"
            )
            return False
        ...
        if self.test_mode:
            ...
            if any(
                pattern in workflow_id for pattern in self.allowed_workflow_patterns
            ):
                self.logger.debug(
                    f"Workflow {workflow_id} matches allowed test pattern"
                )
                return True
        ...
        self.logger.warning(
            f"Operation {operation} not allowed - test_mode: {self.test_mode}"
        )
        return False
    ...
    def _is_test_context(self, context: Dict[str, Any]) -> bool:
        ...
        for key, value in context.items():
            ...
            if any(
                indicator in key_str or indicator in value_str
    ...
    def validate_workflow_safety(
        ...
        self.logger.debug(
            f"Validating workflow safety: {workflow_id} (test_mode: {self.test_mode})"
        )
        ...
        if self.test_mode:
            ...
            if any(
                ...
                self.logger.debug(
                    f"Workflow {workflow_id} allowed - matches test pattern"
                )
                return True
            ...
            if workflow_config.get("test_mode", False):
                self.logger.debug(
                    f"Workflow {workflow_id} allowed - test_mode flag set"
                )
                return True
            ...
            if workflow_config.get("read_only", True):
                self.logger.debug(
                    f"Workflow {workflow_id} allowed - read-only workflow"
                )
                return True
    ...
    def _validate_production_workflow(
        ...
        if max_memory > limits.max_memory_mb:
            self.logger.warning(
                f"Workflow {workflow_id} rejected - memory requirement too high"
            )
            return False

        if max_cpu > limits.max_cpu_percent:
            self.logger.warning(
                f"Workflow {workflow_id} rejected - CPU requirement too high"
            )
            return False
    ...
    def get_configuration_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "safety_limits": {
                "max_cpu_percent": (
                    self.test_limits.max_cpu_percent if self.test_mode else 25.0
                ),
                "max_memory_mb": (
                    self.test_limits.max_memory_mb if self.test_mode else 512.0
                ),
                "max_analysis_time_seconds": (
                    self.test_limits.max_analysis_time_seconds
                    if self.test_mode
                    else 300
                ),
            },
