"""
Beast Mode Message History and Retrieval System

Provides comprehensive message history scanning, retrieval, filtering,
and status tracking capabilities for the Beast Mode agent network.
"""
...
class SortOrder(str, Enum):
    """Sort order for message retrieval"""

    NEWEST_FIRST = "newest_first"
    OLDEST_FIRST = "oldest_first"
    PRIORITY_HIGH_FIRST = "priority_high_first"
    PRIORITY_LOW_FIRST = "priority_low_first"
...
@dataclass
class MessageEntry:
    """Enhanced message entry with metadata"""
...
class MessageHistoryManager:
    ...
    def __init__(
        self,
        log_directory: str = "beast_mode_mailbox",
        status_file: str = "message_status.json",
        cache_size: int = 1000,
    ...
    async def scan_messages(
        self,
        filter_criteria: Optional[MessageFilter] = None,
    ...
    def _file_in_time_range(
        self, log_file_info: Dict[str, Any], filter_criteria: MessageFilter
    ) -> bool:
    ...
    async def _scan_log_file(
        self, file_path: Path, filter_criteria: MessageFilter
    ) -> List[MessageEntry]:
        ...
        try:
            ...
            for line in content.split("\n"):
                ...
                try:
                    ...
                    message_entry = self._create_message_entry(
                        log_entry, str(file_path)
                    )
    ...
    def _create_message_entry(
        self, log_entry: Dict[str, Any], log_file: str
    ) -> MessageEntry:
    ...
    def _message_matches_filter(
        self, message_entry: MessageEntry, filter_criteria: MessageFilter
    ) -> bool:
        ...
        # Time range filters
        if (
            filter_criteria.since
            and message_entry.log_timestamp < filter_criteria.since
        ):
            return False

        if (
            filter_criteria.until
            and message_entry.log_timestamp > filter_criteria.until
        ):
            return False

        # Message type filter
        if (
            filter_criteria.message_types
            and message.type not in filter_criteria.message_types
        ):
            return False

        # Source agent filter
        if (
            filter_criteria.source_agents
            and message.source not in filter_criteria.source_agents
        ):
            return False
        ...
        if filter_criteria.target_agents:
            if (
                not message.target
                or message.target not in filter_criteria.target_agents
            ):
                return False
        ...
        if (
            ...
            and message_entry.status not in filter_criteria.status
        ):
            return False

        # Priority filters
        if (
            filter_criteria.priority_min
            and message.priority < filter_criteria.priority_min
        ):
            return False

        if (
            filter_criteria.priority_max
            and message.priority > filter_criteria.priority_max
        ):
            return False
        ...
        if filter_criteria.correlation_ids:
            if (
                not message.correlation_id
                or message.correlation_id not in filter_criteria.correlation_ids
            ):
                return False
    ...
    def _message_contains_text(
        self, message: BeastModeMessage, search_text: str
    ) -> bool:
    ...
    def _apply_final_filters(
        self, messages: List[MessageEntry], filter_criteria: MessageFilter
    ) -> List[MessageEntry]:
    ...
    async def check_mail(
        ...
        filter_criteria = MessageFilter(
            ...
            status=[MessageStatus.UNREAD] if mark_as_read else None,
            limit=limit,
        )
    ...
    async def search_messages(
        ...
        filter_criteria = MessageFilter(
            ...
            target_agents=[agent_id] if agent_id else None,
            message_types=message_types,
            since=since,
            limit=limit,
        )
    ...
    async def mark_message_read(self, message_id: str) -> None:
        ...
        await self._update_message_status(
            message_id, MessageStatus.READ, read_timestamp=datetime.now()
        )
    ...
    async def _update_message_status(
        ...
        if read_timestamp:
            self.message_status[message_id][
                "read_timestamp"
            ] = read_timestamp.isoformat()
        elif status == MessageStatus.UNREAD:
            # Remove read timestamp when marking as unread
    ...
    async def get_message_counts(
        ...
        """
        filter_criteria = MessageFilter(
            target_agents=[agent_id] if agent_id else None, since=since
        )
    ...
    async def cleanup_old_status(self, days_old: int = 30) -> int:
        ...
        # Get all message IDs that still exist in logs
        ...
        # Remove status for messages that no longer exist
        to_remove = []
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            **self.stats,
            ...
            "message_status_count": len(self.message_status),
