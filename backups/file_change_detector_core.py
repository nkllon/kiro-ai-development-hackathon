...
class AdvancedFileChangeAnalysis:
    ...
    changes_by_category: Dict[FileCategory, List[FileChange]] = field(
...
class FileChangeDetector(ReflectiveModule):
    ...
    def __init__(self, repository_path: str = "."):
        ...
        self._task_indicators = {
            "implementation": ["class", "def ", "function", "interface", "implements"],
            "testing": ["test_", "assert", "pytest", "unittest", "mock"],
            "documentation": ["# ", "## ", "### ", "```", "docs/", "README"],
            "configuration": ["config", "settings", "environment", "env"],
        }
        self.logger.info(
            f"FileChangeDetector initialized for repository: {self.repository_path}"
        )
    ...
    def analyze_file_changes(
        ...
        try:
            ...
            self.logger.info(
                f"Advanced analysis complete: {analysis.total_files_changed} files, complexity: {analysis.complexity_score:.2f}, risk: {analysis.risk_assessment}"
            )
            return analysis
    ...
    def map_changes_to_task_completions(
        ...
        """
        self.logger.info(
            "Mapping file changes to task completions with enhanced validation"
        )
        ...
        for task_id, task_config in task_definitions.items():
            mapping = self._analyze_task_completion_enhanced(
                task_id, task_config, analysis, claimed_tasks
            )
        ...
        self.logger.info(
            f"Generated {len(task_mappings)} task mappings with enhanced validation"
        )
        return task_mappings

    def detect_completion_evidence(
        self, file_changes: List[FileChange], task_patterns: Dict[str, List[str]]
    ) -> Dict[str, List[str]]:
        """
            ...
            for change in file_changes:
                if change.change_type in [ChangeType.ADDED, ChangeType.MODIFIED]:
                    ...
                    content_indicators = self._detect_content_indicators(
                        change.file_path, self._task_indicators.get(task_type, [])
                    )
    ...
    def perform_comprehensive_file_change_analysis(
        self, commits: List[CommitInfo], claimed_tasks: Optional[List[str]] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            files_added = len(
                [c for c in all_changes if c.change_type == ChangeType.ADDED]
            )
            files_modified = len(
            ...
            self.logger.info(
                f"Comprehensive analysis complete: {analysis.total_files_changed} files, {len(task_mappings)} task mappings, accuracy score: {accuracy_metrics.get('overall_accuracy', 0.0):.2f}"
            )
    ...
    def is_healthy(self) -> bool:
        ...
        try:
            return (
                ...
                and (len(self._file_patterns) > 0)
                and (len(self._task_indicators) > 0)
            )
        except Exception:
            return False
    ...
    ) -> Dict[ChangeType, List[FileChange]]:
    ...
    def _categorize_by_file_type(
        self, changes: List[FileChange]
    ) -> Dict[FileCategory, List[FileChange]]:
    ...
    def _identify_high_impact_changes(
        self, changes: List[FileChange]
    ) -> List[FileChange]:
        ...
        if impact_scores:
            threshold = max(
                0.7, sorted(impact_scores, reverse=True)[int(len(impact_scores) * 0.2)]
            )
            high_impact = [
                change for change in changes if change.impact_score >= threshold
            ]
        return high_impact
    ...
    def _calculate_complexity_score(self, changes: List[FileChange]) -> float:
        ...
        source_files = len(
            [c for c in changes if c.category == FileCategory.SOURCE_CODE]
        )
        test_files = len([c for c in changes if c.category == FileCategory.TEST_CODE])
        config_files = len(
            [c for c in changes if c.category == FileCategory.CONFIGURATION]
        )
        complexity = (
            source_files * 1.0
            + test_files * 0.5
    ...
    def _assess_risk_level(
        self, changes: List[FileChange], complexity_score: float
    ) -> str:
    ...
    def _get_default_task_definitions(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            ...
            "rdi_validation_implementation": {
                ...
                "task_type": "implementation",
                "file_patterns": ["src/beast_mode/compliance/rdi/*", "tests/*rdi*"],
                "content_indicators": [
                    "RDI",
                    "requirement",
                    "design",
            ...
            "rm_validation_implementation": {
                ...
                "task_type": "implementation",
                "file_patterns": ["src/beast_mode/compliance/rm/*", "tests/*rm*"],
                "content_indicators": [
                    "ReflectiveModule",
                    "RM",
                    "health",
            ...
            "documentation_updates": {
                ...
                "file_patterns": ["docs/*", "*.md", "README*", ".kiro/specs/*"],
    ...
    def _analyze_task_completion_enhanced(
        ...
        for change in all_changes:
            ...
            content_evidence = self._detect_content_indicators(
                change.file_path, content_indicators
            )
            if content_evidence:
    ...
    def _validate_claimed_vs_implemented(
        self, task_mappings: List[TaskMapping], claimed_tasks: List[str]
    ) -> None:
        """
            ...
            if claimed_tasks and mapping.task_id in claimed_tasks:
                if mapping.confidence_score > 0.6:
                    mapping.evidence.append(
                        "Task claimed and well-supported by evidence"
                    )
                ...
                mapping.evidence.append(
                    "Strong implementation evidence but task not claimed"
                )

    def _generate_file_change_breakdown(
        self, analysis: AdvancedFileChangeAnalysis
    ) -> Dict[str, Any]:
        """
    ...
    def _validate_task_completion_claims(
        self, task_mappings: List[TaskMapping], claimed_tasks: Optional[List[str]]
    ) -> Dict[str, Any]:
        """
        ...
        validated_details = [
            {"task_id": task, "status": "validated"} for task in validated_tasks
        ]
        ...
        for task in missing_evidence_tasks:
            if task in mapping_dict:
                ...
                missing_evidence_details.append(
                    {"task_id": task, "confidence": 0.0, "status": "no_evidence"}
                )
        return {
    ...
    def _calculate_detection_accuracy_metrics(
        self, analysis: AdvancedFileChangeAnalysis, task_mappings: List[TaskMapping]
    ) -> Dict[str, float]:
        """
        ...
        if analysis.total_files_changed > 0:
            categorized_files = sum(
                (len(changes) for changes in analysis.changes_by_category.values())
            )
            metrics["file_categorization_confidence"] = (
                categorized_files / analysis.total_files_changed
            )
        else:
            metrics["file_categorization_confidence"] = 1.0
        if task_mappings:
            avg_confidence = sum(
                (mapping.confidence_score for mapping in task_mappings)
            ) / len(task_mappings)
            metrics["task_mapping_confidence"] = avg_confidence
            high_confidence_count = sum(
                (1 for mapping in task_mappings if mapping.confidence_score > 0.6)
            )
            metrics["high_confidence_mappings_ratio"] = high_confidence_count / len(
                task_mappings
            )
        else:
        ...
        if analysis.total_files_changed > 0:
            ...
            if all_changed_files:
                metrics["coverage_completeness"] = len(mapped_files) / len(
                    all_changed_files
                )
            else:
    ...
    def _generate_task_completion_recommendations(
        ...
        if not task_mappings:
            recommendations.append(
                "No task mappings found - consider reviewing file change patterns"
            )
        ...
        if total_files == 0:
            recommendations.append(
                "No files mapped to tasks - review task patterns and file changes"
            )
        return recommendations
    ...
    def _file_matches_task_patterns(self, file_path: str, patterns: List[str]) -> bool:
        ...
        for pattern in patterns:
            if (
                fnmatch.fnmatch(normalized_path_str, pattern)
                or fnmatch.fnmatch(normalized_path_str.lower(), pattern.lower())
    ...
    def _detect_content_indicators(
        self, file_path: str, indicators: List[str]
    ) -> List[str]:
        """
        ...
        if file_path.endswith(".py") and any(
            (ind in indicators for ind in ["class", "def", "function"])
        ):
        ...
        is_claimed = claimed_tasks and any(
            (task_id in claimed for claimed in claimed_tasks)
        )
        if is_claimed and overall_confidence < 0.3:
            evidence.append(
                f"WARNING: Task claimed as complete but low implementation evidence"
            )
        elif not is_claimed and overall_confidence > 0.8:
            evidence.append(
                f"INFO: High implementation evidence but task not claimed as complete"
            )
        return TaskMapping(
            task_id=task_id,
    ...
    def _file_matches_task_patterns_legacy(
    ...
    def _find_file_change_in_analysis(
        self, file_path: str, analysis: AdvancedFileChangeAnalysis
    ) -> Optional[FileChange]:
    ...
    def _analyze_content_indicators(
        ...
        for file_path in matching_files:
            ...
            if file_change:
                if (
                    file_change.category == FileCategory.TEST_CODE
                    and "test" in content_indicators
                ):
                    indicator_matches += 1
                elif file_change.category == FileCategory.SOURCE_CODE and any(
                    (
                        ind in content_indicators
                        for ind in ["class", "function", "implementation"]
                    )
                ):
                    indicator_matches += 1
    ...
    def _validate_claimed_vs_implemented(
        ...
        implementation_evidence = {
            mapping.task_id: mapping.confidence_score for mapping in task_mappings
        }
        for claimed_task in claimed_tasks:
            confidence = implementation_evidence.get(claimed_task, 0.0)
            task_mapping = next(
                (m for m in task_mappings if m.task_id == claimed_task), None
            )
            if task_mapping:
                if confidence < 0.3:
    ...
    def _calculate_detection_accuracy_metrics(
        ...
        for changes in analysis.changes_by_type.values():
            all_changes.extend(changes)
        if all_changes:
            known_category_changes = [
                c for c in all_changes if c.category != FileCategory.UNKNOWN
            ]
            metrics["file_categorization_confidence"] = len(
                known_category_changes
            ) / len(all_changes)
        if task_mappings:
            ...
            high_confidence_mappings = [
                m for m in task_mappings if m.confidence_score >= 0.7
            ]
            metrics["high_confidence_mappings_ratio"] = len(
                high_confidence_mappings
            ) / len(task_mappings)
        if all_changes:
            mapped_files = set()
    ...
    def _generate_task_completion_recommendations(
        ...
        if total_files_mapped == 0:
            ...
            recommendations.append(
                "Low file coverage in task mappings - consider expanding task patterns"
            )
        return recommendations
...
def _get_primary_responsibility(self) -> str:
    """Define the single primary responsibility of this module."""
    return (
        "Detect and analyze file changes with advanced categorization and task mapping"
    )
...
def _get_default_task_definitions(self) -> Dict[str, Dict[str, Any]]:
    ...
    return {
        ...
        "configuration_updates": {
            "description": "Update configuration files",
            "task_type": "configuration",
            "file_patterns": ["*.json", "*.yaml", "*.yml", "*.toml", "pyproject.toml"],
            "content_indicators": ["config", "settings", "dependencies"],
            "completion_threshold": 0.5,
        },
    }
...
def _calculate_detection_accuracy_metrics(
    ...
    if all_changes:
        ...
        metrics["file_categorization_confidence"] = len(known_category_changes) / len(
            all_changes
        )
    if task_mappings:
        ...
        metrics["high_confidence_mappings_ratio"] = len(high_confidence_mappings) / len(
            task_mappings
        )
    if all_changes:
        mapped_files = set()
