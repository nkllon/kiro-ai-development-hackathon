"""
Code Quality Validator Core Core

This module was extracted from code_quality_validator_core.py
...
def generate_quality_improvement_plan(self, report: CodeQualityReport) -> List[str]:
    ...
    if report.critical_issues > 0:
        ...
        critical_files = set(
            (issue.file_path for issue in report.issues if issue.severity == "critical")
        )
...
def _analyze_complexity(self, tree: ast.AST, file_path: Path) -> Dict[str, Any]:
    ...
    avg_score = (
        sum(complexity_scores) / len(complexity_scores) if complexity_scores else 100
    )
