...
class HealthTrendAnalyzer:
    ...
    def record_health_metrics(self, domain_name: str, metrics: HealthMetrics):
        ...
        for metric_name, value in metric_values.items():
            ...
            self.historical_data[domain_name][metric_name] = [
                (ts, val)
                for ts, val in self.historical_data[domain_name][metric_name]
                if ts >= cutoff_time
            ]
    ...
    def get_trending_domains(
        ...
        for domain_name in self.historical_data:
            ...
            if relevant_trends:
                avg_strength = sum((t.trend_strength for t in relevant_trends)) / len(
                    relevant_trends
                )
...
class HealthReportGenerator(DomainSystemComponent):
        ...
        with self._time_operation("generate_full_report"):
            ...
            try:
                ...
                if self.dependency_analyzer:
                    dependency_analysis = (
                        self.dependency_analyzer.perform_comprehensive_analysis()
                    )
                domain_trends = {}
                for domain_name in health_statuses.keys():
                    ...
                    domain_trends[domain_name] = (
                        self.trend_analyzer.analyze_domain_trends(domain_name)
                    )
                critical_issues, warning_issues, info_issues = (
                    self._categorize_all_issues(health_statuses)
                )
                ...
                report = HealthReport(
                    ...
                    data_freshness={
                        ...
                        "dependency_data": (
                            datetime.now() if dependency_analysis else None
                        ),
                    },
    ...
    def generate_domain_report(self, domain_name: str) -> HealthReport:
        ...
        with self._time_operation("generate_domain_report"):
            ...
            try:
                ...
                if self.dependency_analyzer:
                    dependency_analysis = (
                        self.dependency_analyzer.analyze_domain_impact(domain_name)
                    )
                domain_trends = {
                    domain_name: self.trend_analyzer.analyze_domain_trends(domain_name)
                }
                critical_issues, warning_issues, info_issues = (
                    self._categorize_all_issues(health_statuses)
                )
                ...
                report = HealthReport(
                    ...
                    total_domains=1,
                    healthy_domains=(
                        1 if domain_health.status == HealthStatusType.HEALTHY else 0
                    ),
                    degraded_domains=(
                        1 if domain_health.status == HealthStatusType.DEGRADED else 0
                    ),
                    failed_domains=(
                        1 if domain_health.status == HealthStatusType.FAILED else 0
                    ),
    ...
    def generate_trend_report(self, days: int = 7) -> HealthReport:
        ...
        with self._time_operation("generate_trend_report"):
            start_time = time.time()
            try:
                degrading_domains = self.trend_analyzer.get_trending_domains(
                    "degrading"
                )
                improving_domains = self.trend_analyzer.get_trending_domains(
                    "improving"
                )
                all_trends = {}
                for domain_name in self.trend_analyzer.historical_data.keys():
                    all_trends[domain_name] = self.trend_analyzer.analyze_domain_trends(
                        domain_name
                    )
                recommendations = self._generate_trend_recommendations(
                    degrading_domains, improving_domains
                )
                report = HealthReport(
    ...
    def _categorize_all_issues(
        self, health_statuses: HealthStatusCollection
    ) -> Tuple[List[HealthIssue], List[HealthIssue], List[HealthIssue]]:
    ...
    def _calculate_overall_health_score(
        ...
        if not health_statuses:
            return 0.0
        total_score = sum(
            (status.metrics.overall_health_score for status in health_statuses.values())
        )
        return total_score / len(health_statuses)

    def _generate_comprehensive_recommendations(
        self,
        health_statuses: HealthStatusCollection,
        ...
        if dependency_analysis.get("circular_dependencies", {}).get(
        ...
        for domain_name, trends in domain_trends.items():
            if any(
                (
                    t.trend_direction == "degrading" and t.trend_strength > 0.2
                    for t in trends
                )
            ):
            ...
            recommendations.append(
                {
                    ...
                    "actions": [issue.suggested_fix for issue in critical_issues[:3]],
                }
            )
    ...
    def _generate_trend_recommendations(
        self,
        degrading_domains: List[Tuple[str, float]],
        improving_domains: List[Tuple[str, float]],
    ) -> List[Dict[str, Any]]:
    ...
    def _store_report(self, report: HealthReport):
        ...
        self.report_history = [
            r for r in self.report_history if r.generated_at >= cutoff_date
        ]
    ...
    def _process_alerts(self, alerts: List[Alert]):
        ...
        for alert in alerts:
            rule = next(
                (
                    r
                    for r in self.alert_manager.alert_rules
    ...
    def get_report_history(
        self, report_type: Optional[str] = None, days: int = 7
    ) -> List[HealthReport]:
    ...
    def export_report(self, report: HealthReport, format: str = "json") -> str:
        ...
        if format == "json":
            ...
            return SerializationHandler.safe_serialize(
                report_dict, indent=2, default=combined_handler
            )
        else:
...
def evaluate_alerts(
    self, health_statuses: HealthStatusCollection, trends: Dict[str, List[HealthTrend]]
) -> List[Alert]:
...
def _evaluate_threshold_rule(
    self, rule: AlertRule, health_statuses: HealthStatusCollection
) -> List[Alert]:
...
def _evaluate_trend_rule(
    self, rule: AlertRule, trends: Dict[str, List[HealthTrend]]
) -> List[Alert]:
    ...
    for domain_name, domain_trends in trends.items():
        ...
        if degrading_trends:
            ...
            avg_trend_strength = sum(
                (t.trend_strength for t in degrading_trends)
            ) / len(degrading_trends)
            alert = Alert(
                id=f"{rule.name}_{domain_name}_{int(time.time())}",
                rule_name=rule.name,
                severity=rule.severity,
...
def _get_metric_value(
    self, health_status: HealthStatus, metric_name: str
) -> Optional[float]:
    ...
    with self._time_operation("generate_full_report"):
        ...
        try:
            ...
            for domain_name in health_statuses.keys():
                ...
                domain_trends[domain_name] = self.trend_analyzer.analyze_domain_trends(
                    domain_name
                )
            critical_issues, warning_issues, info_issues = self._categorize_all_issues(
                health_statuses
            )
            total_domains = len(health_statuses)
            healthy_count = sum(
...
def generate_domain_report(self, domain_name: str) -> HealthReport:
    ...
    with self._time_operation("generate_domain_report"):
        ...
        try:
            ...
            dependency_analysis = {}
            if self.dependency_analyzer:
                dependency_analysis = self.dependency_analyzer.analyze_domain_impact(
                    domain_name
                )
            domain_trends = {
                domain_name: self.trend_analyzer.analyze_domain_trends(domain_name)
