...
Collects and aggregates performance metrics for Beast Mode components
including message throughput, latency, error rates, and system performance.
"""
...
class MetricsCollector:
    ...
    def get_all_metrics_summary(self) -> Dict[str, Any]:
        ...
        for metric_key, values in self.histograms.items():
            if values:
                name, labels = self._parse_metric_key(metric_key)
                summary_stats = self._calculate_summary(
                    name, MetricType.HISTOGRAM, values, labels
                )
        ...
        for metric_key, values in self.timers.items():
            if values:
                name, labels = self._parse_metric_key(metric_key)
                summary_stats = self._calculate_summary(
                    name, MetricType.TIMER, values, labels
                )
    ...
    def get_performance_report(self) -> Dict[str, Any]:
        ...
        if messages_sent > 0 or messages_received > 0:
            report["kpis"]["message_throughput"] = {
        ...
        if latency_summary:
            report["kpis"]["message_latency"] = {
                "avg_ms": latency_summary.avg_value,
                "p95_ms": latency_summary.percentiles.get("p95", 0),
                "p99_ms": latency_summary.percentiles.get("p99", 0),
            }
