"""
Rate Limiter for Ghostbusters API

Implements rate limiting to prevent API abuse and ensure
...
class RateLimiter:
    ...
    def __init__(
        ...
        # Token buckets per client per operation
    ...
    def set_operation_limit(
        self, operation: str, requests_per_minute: int, burst_size: Optional[int] = None
    ) -> None:
        """
        ...
        self._operation_limits[operation] = {
            "requests_per_minute": requests_per_minute,
            "burst_size": burst_size,
        }

        logger.info(
            f"Set rate limit for {operation}: {requests_per_minute}/min, burst {burst_size}"
        )
    ...
    async def check_limit(self, operation: str, client_id: str = "default") -> bool:
        ...
        # Get or create bucket for client/operation
    ...
    async def get_reset_time(
        ...
        # Calculate time for bucket to refill completely
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "operation_limits": dict(self._operation_limits),
        }
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        # Count allowed vs rejected requests in last hour
        ...
        return {
            ...
            "recent_allowed_requests": recent_allowed,
            "recent_rejected_requests": recent_rejected,
            "rejection_rate": recent_rejected
            / max(1, recent_allowed + recent_rejected),
            "active_operations": len(self._request_history),
    ...
    def get_client_stats(self, client_id: str) -> Dict[str, Any]:
        ...
        stats = {
            "client_id": client_id,
            "operations": {},
            "total_operations": len(client_buckets),
        }
    ...
    def _get_bucket(self, client_id: str, operation: str) -> Dict:
        ...
        if operation not in self._client_buckets[client_id]:
            limits = self._get_operation_limits(operation)
            self._client_buckets[client_id][operation] = {
    ...
    def _get_operation_limits(self, operation: str) -> Dict[str, int]:
        ...
        return self._operation_limits.get(
            operation,
            {
    ...
    def _refill_bucket(self, bucket: Dict) -> None:
        ...
        if time_elapsed > 0:
            ...
            operation_limits = self._get_operation_limits(
                "default"
            )  # This should be passed in
            refill_rate = (
                operation_limits["requests_per_minute"] / 60.0
            )  # tokens per second
            tokens_to_add = time_elapsed * refill_rate

            # Add tokens but don't exceed burst size
            bucket["tokens"] = min(
                operation_limits["burst_size"], bucket["tokens"] + tokens_to_add
            )
            bucket["last_refill"] = now

    def _record_request(self, operation: str, client_id: str, allowed: bool) -> None:
        """Record request for statistics"""
        self._request_history[operation].append(
            {"timestamp": datetime.utcnow(), "client_id": client_id, "allowed": allowed}
        )
