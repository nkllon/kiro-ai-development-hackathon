...
class DomainQueryEngine(CachedComponent, QueryEngineInterface):
        ...
        with self._time_operation("build_indexes"):
            try:
                ...
                for domain_name, domain in domains.items():
                    ...
                    capabilities = domain.requirements + [
                        domain.tools.linter,
        ...
        with self._time_operation("natural_language_query"):
            ...
            try:
                ...
                if parsed_query.get("filters"):
                    domains = self._apply_parsed_filters(
                        domains, parsed_query["filters"]
                    )
                suggestions = self._generate_intelligent_suggestions(
                    query, parsed_query, domains
                )
                relevance_scores = self._calculate_enhanced_relevance_scores(
                    domains, parsed_query
                )
                domains.sort(
                    key=lambda d: (relevance_scores.get(d.name, 0.0), d.name),
                    reverse=True,
                )
    ...
    def _determine_intent(self, query: str) -> str:
        ...
        if any((word in query for word in ["pattern", "file", "path", "*.py", "src/"])):
            ...
            (word in query for word in ["contains", "content", "indicator", "includes"])
        ):
            return "content_search"
        elif any(
            (
                word in query
                for word in ["tool", "capability", "can", "does", "supports", "run"]
            )
        ):
        ...
        with self._time_operation("content_search"):
            ...
            try:
                ...
                for indexed_indicator, domain_names in self._content_index.items():
                    if (
                        indicator_lower in indexed_indicator
                        or indexed_indicator in indicator_lower
                    ):
                        matching_domains.update(domain_names)
                domains = []
                if self.registry_manager:
                    ...
                    for domain_name, domain in all_domains.items():
                        for requirement in domain.requirements:
                            if self._capability_matches(
                                capability_lower, requirement.lower()
                            ):
                        ...
                        for tool in tool_capabilities:
                            if tool and self._capability_matches(
                                capability_lower, tool.lower()
                            ):
                                matching_domains.add(domain_name)
                        for indicator in domain.content_indicators:
                            if self._capability_matches(
                                capability_lower, indicator.lower()
                            ):
                                matching_domains.add(domain_name)
                domains = []
                if self.registry_manager:
                    ...
                    for domain_name in matching_domains:
                        if domain_name in all_domains:
                            domains.append(all_domains[domain_name])
                domains.sort(
                    key=lambda d: self._calculate_capability_relevance(
                        d, capability_lower
                    ),
                    reverse=True,
                )
    ...
    def _capability_matches(
        self, search_capability: str, target_capability: str
    ) -> bool:
        """Check if a search capability matches a target capability"""
        if search_capability == target_capability:
            return True
        if (
            search_capability in target_capability
            or target_capability in search_capability
        ):
            return True
        capability_synonyms = {
        ...
        for base_capability, synonyms in capability_synonyms.items():
            if search_capability == base_capability or search_capability in synonyms:
                if target_capability == base_capability or any(
                    (syn in target_capability for syn in synonyms)
                ):
                    return True
            if target_capability == base_capability or target_capability in synonyms:
                if search_capability == base_capability or any(
                    (syn in search_capability for syn in synonyms)
                ):
                    return True
        return False
    ...
    def _pattern_suggests_capability(self, pattern: str, capability: str) -> bool:
        ...
        if capability in capability_patterns:
            return any(
                (
                    cap_pattern in pattern_lower
                    for cap_pattern in capability_patterns[capability]
                )
            )
        return False
        ...
        with self._time_operation("relationship_query"):
            try:
                ...
                if relationship_type == "dependencies":
                    ...
                    related_domains = self._get_transitive_dependencies(
                        domain, all_domains
                    )
                elif relationship_type == "transitive_dependents":
                    related_domains = self._get_transitive_dependents(
                        domain, all_domains
                    )
                elif relationship_type == "similar":
                    related_domains = self._find_similar_domains(
                        target_domain, all_domains
                    )
                elif relationship_type == "circular":
                    circular_chains = self._detect_circular_dependencies(
                        domain, all_domains
                    )
                    ...
                    related_domains = self._find_high_coupling_domains(
                        target_domain, all_domains
                    )
                elif relationship_type == "extraction_related":
                    related_domains = self._find_extraction_related_domains(
                        target_domain, all_domains
                    )
    ...
    def suggest_queries(self, partial_query: str) -> List[str]:
        ...
        with self._time_operation("suggest_queries"):
            try:
                ...
                suggestions.extend(
                    self._generate_contextual_suggestions(partial_lower, partial_parsed)
                )
                suggestions.extend(self._generate_completion_suggestions(partial_lower))
                suggestions.extend(
                    self._generate_template_suggestions(partial_lower, partial_parsed)
                )
                seen = set()
                unique_suggestions = []
    ...
    def _generate_contextual_suggestions(
        self, partial_query: str, partial_info: Dict[str, Any]
    ) -> List[str]:
        ...
        if "domain_name" in partial_info["entity_hints"]:
            if self.registry_manager:
                ...
                for domain_name in list(all_domains.keys())[:5]:
                    if domain_name.lower().startswith(
                        partial_info["last_token"].lower()
                    ):
                        suggestions.append(
                            partial_query.rsplit(" ", 1)[0] + f" {domain_name}"
                        )
        return suggestions
    ...
    def _generate_completion_suggestions(self, partial_query: str) -> List[str]:
        ...
        self._ensure_indexes_built()
        last_word = (
            partial_query.split()[-1] if partial_query.split() else partial_query
        )
        for pattern in self._pattern_index.keys():
            if pattern.startswith(last_word.lower()):
                completed_query = (
                    partial_query.rsplit(" ", 1)[0] + f" {pattern}"
                    if " " in partial_query
                    else pattern
                )
        ...
        for indicator in self._content_index.keys():
            if indicator.startswith(last_word.lower()):
                completed_query = (
                    partial_query.rsplit(" ", 1)[0] + f" {indicator}"
                    if " " in partial_query
                    else indicator
                )
                suggestions.append(completed_query)
        for capability in self._capability_index.keys():
            if capability.startswith(last_word.lower()):
                completed_query = (
                    partial_query.rsplit(" ", 1)[0] + f" {capability}"
                    if " " in partial_query
                    else capability
                )
                suggestions.append(completed_query)
        ...
        with self._time_operation("explain_query_results"):
            try:
                ...
                if intent == "pattern_search":
                    explanation["matching_strategy"] = (
                        "Searched for domains with matching file patterns"
                    )
                elif intent == "capability_search":
                    explanation["matching_strategy"] = (
                        "Searched for domains with matching capabilities or tools"
                    )
                elif intent == "content_search":
                    explanation["matching_strategy"] = (
                        "Searched for domains with matching content indicators"
                    )
                else:
                    explanation["matching_strategy"] = (
                        "Performed combined search across patterns, content, and capabilities"
                    )
                if results.domains:
                    ...
                    explanation["result_ranking"] = {
                        ...
                        "ranking_factors": self._explain_relevance_factors(
                            top_domain, parsed_query
                        ),
                    }
    ...
    def _explain_relevance_factors(
        self, domain: Domain, parsed_query: Dict[str, Any]
    ) -> List[str]:
        ...
        for keyword in keywords:
            matching_patterns = [
                p for p in domain.patterns if keyword.lower() in p.lower()
            ]
            if matching_patterns:
                factors.append(
                    f"File patterns match '{keyword}': {matching_patterns[0]}"
                )
        for capability in entities.get("capabilities", []):
            if capability in domain.requirements or capability in [
                domain.tools.linter,
                domain.tools.formatter,
            ]:
    ...
    def _calculate_relevance_scores(
        self, domains: List[Domain], keywords: List[str]
    ) -> Dict[str, float]:
    ...
    def _determine_enhanced_intent(
        ...
        pattern_count = sum(
            (1 for indicator in pattern_indicators if indicator in query)
        )
        content_count = sum(
            (1 for indicator in content_indicators if indicator in query)
        )
        capability_count = sum(
            (1 for indicator in capability_indicators if indicator in query)
        )
        relationship_count = sum(
            (1 for indicator in relationship_indicators if indicator in query)
        )
        analysis_count = sum(
            (1 for indicator in analysis_indicators if indicator in query)
        )
    ...
    def _determine_query_type(self, query: str) -> str:
        ...
        relationship_patterns = [
            "\\b(depend\\w*\\s+on|depends?\\s+on)\\b",
            "\\b(similar\\s+to|like)\\b",
            "\\b(related\\s+to|connected\\s+to)\\b",
            "\\b(circular\\s+depend|cycle)\\b",
            "\\b(coupling|coupled)\\b",
            "\\b(extract\\w*\\s+impact|extraction)\\b",
        ]
        for pattern in relationship_patterns:
            if re.search(pattern, query):
                return "relationship"
        analysis_patterns = [
            "\\b(analy[sz]e|analysis)\\b",
            "\\b(metrics?|statistics?|stats)\\b",
            "\\b(health|status|report)\\b",
            "\\b(complexity|coupling|quality)\\b",
        ]
        ...
        comparison_patterns = [
            ...
            "\\b(difference|different|differ)\\b",
            "\\b(better|worse|best|worst)\\b",
        ]
    ...
    def _extract_query_filters(self, query: str) -> Dict[str, Any]:
        ...
        status_patterns = [
            "\\b(healthy|degraded|failed)\\s+domains?\\b",
            "\\bdomains?\\s+that\\s+are\\s+(healthy|degraded|failed)\\b",
        ]
        ...
        complexity_patterns = [
            "\\b(simple|complex|high|low)\\s+complexity\\b",
            "\\bcomplexity\\s+(above|below|over|under)\\s+(\\d+(?:\\.\\d+)?)\\b",
        ]
    ...
    def _extract_relationship_info(
        ...
        if entities["domain_names"]:
            ...
            for match in domain_matches:
                if match.endswith("_domain") or match in [
                    "core",
                    "test",
                    "api",
                    "data",
    ...
    def _apply_parsed_filters(
        self, domains: List[Domain], filters: Dict[str, Any]
    ) -> List[Domain]:
        ...
        for domain in domains:
            ...
            if "status" in filters:
                if (
                    not domain.health_status
                    or domain.health_status.status.value != filters["status"]
                ):
                    continue
    ...
    def _generate_intelligent_suggestions(
        self, original_query: str, parsed_query: Dict[str, Any], results: List[Domain]
    ) -> List[str]:
        ...
        if len(results) == 0:
            ...
            if entities.get("domain_names"):
                suggestions.append(
                    f"Check if domain '{entities['domain_names'][0]}' exists"
                )
        elif len(results) > 20:
    ...
    def _calculate_enhanced_relevance_scores(
        self, domains: List[Domain], parsed_query: Dict[str, Any]
    ) -> Dict[str, float]:
        ...
        for domain in domains:
            ...
            if intent == "capability_search":
                score += self._calculate_capability_relevance(
    ...
    def _rank_and_filter_results(
        ...
        if sort_field:
            if sort_field == "name":
                ...
                domains.sort(
                    key=lambda d: len(d.dependencies), reverse=sort_order == "desc"
                )
        for modifier in modifiers:
    ...
    def _generate_query_suggestions(self, query: str, keywords: List[str]) -> List[str]:
        ...
        suggestions.append(
            f"domains similar to {(keywords[0] if keywords else 'current')}"
        )
        suggestions.append(
            f"dependencies of {(keywords[0] if keywords else 'domains')}"
        )
        return suggestions[: self.suggestion_limit]
        ...
        for dep_name in domain.dependencies:
            if dep_name in all_domains:
                ...
                transitive_deps.extend(
                    self._get_transitive_dependencies(dep_name, all_domains, visited)
                )
        seen = set()
        unique_deps = []
    ...
    def _get_transitive_dependents(
        self, domain_name: str, all_domains: Dict[str, Domain]
    ) -> List[Domain]:
    ...
    def _find_similar_domains(
        self, target_domain: Domain, all_domains: Dict[str, Domain]
    ) -> List[Domain]:
        ...
        for domain_name, domain_obj in all_domains.items():
            if domain_name == target_domain.name:
                continue
            pattern_overlap = len(
                target_patterns.intersection(set(domain_obj.patterns))
            )
            indicator_overlap = len(
                target_indicators.intersection(set(domain_obj.content_indicators))
            )
            requirement_overlap = len(
                target_requirements.intersection(set(domain_obj.requirements))
            )
            total_patterns = len(target_patterns.union(set(domain_obj.patterns)))
            total_indicators = len(
                target_indicators.union(set(domain_obj.content_indicators))
            )
            total_requirements = len(
                target_requirements.union(set(domain_obj.requirements))
            )
        ...
        similar_domains.sort(
            key=lambda d: self._calculate_domain_similarity(target_domain, d),
            reverse=True,
        )
    ...
    def _calculate_domain_similarity(self, domain1: Domain, domain2: Domain) -> float:
        ...
        indicator_jaccard = (
            len(indicators1.intersection(indicators2))
            / len(indicators1.union(indicators2))
            if indicators1.union(indicators2)
            else 0
        )
        requirement_jaccard = (
            len(requirements1.intersection(requirements2))
            / len(requirements1.union(requirements2))
            if requirements1.union(requirements2)
            else 0
        )
        return (
            pattern_jaccard * 0.4 + indicator_jaccard * 0.3 + requirement_jaccard * 0.3
        )
    ...
    def _find_high_coupling_domains(
        self, target_domain: Domain, all_domains: Dict[str, Domain]
    ) -> List[Domain]:
        ...
        high_coupling_domains.sort(
            key=lambda d: self._calculate_coupling_score(target_domain, d), reverse=True
        )
    ...
    def _find_extraction_related_domains(
        self, target_domain: Domain, all_domains: Dict[str, Domain]
    ) -> List[Domain]:
        ...
        with self._time_operation("advanced_relationship_analysis"):
            try:
                ...
                analysis = {
                    ...
                    "direct_dependencies": [
                        d.name
                        for d in self.relationship_query(domain_name, "dependencies")
                    ],
                    "direct_dependents": [
                        d.name
                        for d in self.relationship_query(domain_name, "dependents")
                    ],
                    "transitive_dependencies": [
                        d.name
                        for d in self.relationship_query(
                            domain_name, "transitive_dependencies"
                        )
                    ],
                    "transitive_dependents": [
                        d.name
                        for d in self.relationship_query(
                            domain_name, "transitive_dependents"
                        )
                    ],
                    "similar_domains": [
                        d.name for d in self.relationship_query(domain_name, "similar")
                    ],
                    ...
                    "high_coupling_domains": [
                        d.name
                        for d in self.relationship_query(domain_name, "coupling_high")
                    ],
                    "extraction_impact": [
                        d.name
                        for d in self.relationship_query(
                            domain_name, "extraction_related"
                        )
                    ],
                }
                analysis["metrics"] = {
