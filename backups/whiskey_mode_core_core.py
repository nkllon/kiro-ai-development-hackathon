...
This module was extracted from whiskey_mode_core.py
...
class WhiskeyModeDisplay(ReflectiveModule):
    ...
    def _setup_layout(self) -> None:
        ...
        self.layout["left"].split_column(
            Layout(name="test_matrix", ratio=3), Layout(name="system_health", ratio=1)
        )
        self.layout["right"].split_column(
            Layout(name="hubris_panel", ratio=1), Layout(name="mama_panel", ratio=1)
        )
    ...
    def _create_header(self) -> Panel:
        ...
        status_text = Text(
            f"● {self.status.value.upper()}", style=f"bold {status_color}"
        )
        header_content = Columns(
            [Align.left(title), Align.center(subtitle), Align.right(status_text)]
        )
    ...
    def _create_test_matrix(self) -> Panel:
        ...
        if self.test_metrics.total_tests > 0:
            pass_rate = (
                self.test_metrics.passed_tests / self.test_metrics.total_tests * 100
            )
            if pass_rate >= 95:
            ...
            table.add_row(
                "No tests run yet", Text("⏳ WAITING", style="dim"), "-", "-", "-"
            )
    ...
    def _create_system_health(self) -> Panel:
        ...
        cpu_color = (
            "green"
            if self.system_health.cpu_percent < 70
            else "yellow" if self.system_health.cpu_percent < 90 else "red"
        )
        cpu_sparkline = self._create_sparkline(
            [45, 52, 48, 55, self.system_health.cpu_percent], cpu_color
        )
        health_table.add_row(
            "CPU", f"{self.system_health.cpu_percent:.1f}%", cpu_sparkline
        )
        mem_color = (
            "green"
            if self.system_health.memory_percent < 80
            else "yellow" if self.system_health.memory_percent < 95 else "red"
        )
        mem_sparkline = self._create_sparkline(
            [60, 65, 62, 68, self.system_health.memory_percent], mem_color
        )
        health_table.add_row(
            "Memory", f"{self.system_health.memory_percent:.1f}%", mem_sparkline
        )
        velocity_sparkline = self._create_sparkline(
            [12, 15, 18, 16, self.system_health.test_velocity], "cyan"
        )
        health_table.add_row(
            "Test Velocity",
    ...
    def _create_hubris_panel(self) -> Panel:
        ...
        if not self.hubris_alerts:
            ...
            content = Text(
                "\n".join(
                    ["⚠️  Hubris Prevention Active", "", *self.hubris_alerts[-3:]]
                ),
                style="yellow",
            )
    ...
    def _create_mama_panel(self) -> Panel:
        ...
        if not self.mama_discoveries:
            ...
            content = Text(
                "\n".join(
                    ["👸 Mama Discovery Active", "", *self.mama_discoveries[-2:]]
                ),
                style="magenta",
            )
    ...
    def _create_footer(self) -> Panel:
        ...
        controls = Text(
            "Press 'q' to quit • 'p' to pause • 'r' to refresh", style="dim"
        )
        time_text = Text(f"Last update: {timestamp}", style="dim")
        footer_content = Columns([Align.left(controls), Align.right(time_text)])
...
def _create_system_health(self) -> Panel:
    ...
    health_table.add_row("CPU", f"{self.system_health.cpu_percent:.1f}%", cpu_sparkline)
    mem_color = (
        "green"
        if self.system_health.memory_percent < 80
        else "yellow" if self.system_health.memory_percent < 95 else "red"
...
def _create_hubris_panel(self) -> Panel:
    ...
    if not self.hubris_alerts:
        ...
        content = Text(
            "\n".join(["⚠️  Hubris Prevention Active", "", *self.hubris_alerts[-3:]]),
            style="yellow",
        )
    return Panel(content, title="🛡️  Hubris Prevention", border_style="yellow")
...
def _create_mama_panel(self) -> Panel:
    ...
    if not self.mama_discoveries:
        ...
        content = Text(
            "\n".join(["👸 Mama Discovery Active", "", *self.mama_discoveries[-2:]]),
            style="magenta",
        )
    return Panel(content, title="👸 Mama Discovery", border_style="magenta")
