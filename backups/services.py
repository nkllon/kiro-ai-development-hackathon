...
class DomainService(DomainReflectiveModule, ABC):
    ...
    def __init__(
        self, domain_context: str, service_name: str, module_id: Optional[str] = None
    ):
        """
        ...
        logger.info(
            f"DomainService initialized: {service_name} in context: {domain_context}"
        )
    ...
    def _validate_statelessness(self) -> ValidationResult:
        ...
        problematic_variables = [
            var for var in new_variables if not var.startswith("_")
        ]
    ...
    async def get_health_indicators(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "validation_errors": len(service_validation.errors)
            + len(domain_validation.errors),
            "validation_warnings": len(service_validation.warnings)
        ...
        base_indicators.update(
            {
                ...
                "transaction_success_rate": (
                    (
                        (self._transaction_count - self._failed_transaction_count)
        ...
        base_indicators.update(
            {
                ...
                "external_success_rate": (
                    (
                        (self._external_call_count - self._external_failure_count)
                        / max(self._external_call_count, 1)
                    )
                    * 100
                    if self._external_call_count > 0
                    else 100.0
                ),
            }
        )
...
def create_domain_service(
    service_class: type, domain_context: str, service_name: str, **kwargs
) -> DomainService:
    """
