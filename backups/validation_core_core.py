...
This module was extracted from validation_core.py
...
class ConsistencyValidator(ReflectiveModule):
    ...
    def validate_terminology(self, spec_content: str) -> TerminologyReport:
        ...
        try:
            ...
            consistency_score = (
                consistent_count / total_terms if total_terms > 0 else 1.0
            )
            ...
            for interface_name, methods in interfaces.items():
                ...
                if compliance_result["compliant"]:
                    ...
                    if compliance_result["missing_methods"]:
                        missing_methods[interface_name] = compliance_result[
                            "missing_methods"
                        ]
            total_interfaces = len(interfaces)
            compliant_count = len(compliant_interfaces)
            compliance_score = (
                compliant_count / total_interfaces if total_interfaces > 0 else 1.0
            )
        ...
        try:
            ...
            for pattern in design_patterns:
                ...
                if consistency_check["consistent"]:
                    ...
                    pattern_violations[pattern] = consistency_check[
                        "violation_description"
                    ]
            total_patterns = len(design_patterns)
            consistent_count = len(consistent_patterns)
            pattern_score = (
                consistent_count / total_patterns if total_patterns > 0 else 1.0
            )
    ...
    def generate_consistency_score(self, spec_set: List[str]) -> ConsistencyMetrics:
        ...
        try:
            ...
            for spec_path in spec_set:
                ...
                if term_report.consistency_score < 0.5:
                    critical_issues.append(
                        f"Critical terminology issues in {spec_path}"
                    )
            ...
            terminology_score = (
                sum(terminology_scores) / len(terminology_scores)
            ...
            pattern_score = (
                sum(pattern_scores) / len(pattern_scores) if pattern_scores else 0.0
            )
            overall_score = (
                terminology_score * 0.3 + interface_score * 0.4 + pattern_score * 0.3
            )
    ...
    def _load_interface_patterns(self) -> Dict[str, Dict]:
        ...
        return {
            ...
            "ServiceInterface": {
                ...
                "optional_methods": ["authenticate_request", "log_service_call"],
            },
        }
        ...
        for other_term in all_terms:
            if other_term != term:
                similarity = SequenceMatcher(
                    None, term_lower, other_term.lower()
                ).ratio()
                if similarity > 0.8:
    ...
    def _find_canonical_term(self, term: str) -> Optional[str]:
        ...
        for canonical_term in self.terminology_registry:
            similarity = SequenceMatcher(
                None, term_lower, canonical_term.lower()
            ).ratio()
            if similarity > 0.8:
                return canonical_term
        return None

    def _generate_terminology_recommendations(
        self, inconsistent_terms: Dict, new_terms: Set[str]
    ) -> List[str]:
    ...
    def _extract_interfaces_from_definition(
        self, interface_def: str
    ) -> Dict[str, List[str]]:
    ...
    def _check_single_interface_compliance(
        self, interface_name: str, methods: List[str]
    ) -> Dict:
        """Check compliance of a single interface"""
        if "ReflectiveModule" in interface_name or any(
            ("reflective" in m.lower() for m in methods)
        ):
            required_methods = self.interface_patterns["ReflectiveModule"][
                "required_methods"
            ]
    ...
    def _generate_interface_remediation_steps(
        self, non_compliant: List[str], missing_methods: Dict
    ) -> List[str]:
        ...
        for interface in non_compliant:
            if interface in missing_methods:
                ...
                steps.append(
                    f"Add missing methods to {interface}: {', '.join(methods)}"
                )
        return steps
    ...
    def _check_pattern_consistency(self, pattern: str) -> Dict:
        ...
        if "pdca" in pattern_lower:
            ...
            has_all_phases = all(
                (phase.lower() in pattern_lower for phase in required_phases)
            )
            return {
                "consistent": has_all_phases,
                "violation_description": (
                    "Missing PDCA phases" if not has_all_phases else ""
                ),
            }
        return {"consistent": True, "violation_description": ""}

    def _generate_pattern_improvement_suggestions(
        self, inconsistent_patterns: List[str], violations: Dict
    ) -> List[str]:
    ...
    def _generate_improvement_priorities(
        self, terminology_score: float, interface_score: float, pattern_score: float
    ) -> List[str]:
    ...
    def is_healthy(self) -> bool:
        ...
        try:
            return (
                ...
                and len(self.interface_patterns) > 0
                and (len(self.design_patterns) > 0)
            )
        except Exception:
            return False
...
def generate_consistency_score(self, spec_set: List[str]) -> ConsistencyMetrics:
    ...
    try:
        ...
        interface_score = (
            sum(interface_scores) / len(interface_scores) if interface_scores else 0.0
        )
        pattern_score = (
            sum(pattern_scores) / len(pattern_scores) if pattern_scores else 0.0
        )
...
def _load_interface_patterns(self) -> Dict[str, Dict]:
    ...
    return {
        ...
        "ServiceInterface": {
            "required_methods": ["provide_service", "validate_request", "handle_error"],
            "optional_methods": ["authenticate_request", "log_service_call"],
        },
    }
