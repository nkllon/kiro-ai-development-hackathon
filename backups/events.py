...
class DomainEvent(ABC):
    ...
    def __str__(self) -> str:
        ...
        return (
            f"{self.event_type}(id={self.event_id}, aggregate_id={self.aggregate_id})"
        )
...
class DomainEventHandler(ABC):
    ...
    async def handle_with_metrics(self, event: DomainEvent) -> None:
        ...
        try:
            ...
            logger.debug(
                f"Handler {self.handler_name} successfully handled {event.event_type}"
            )
        except Exception as e:
            self._failed_events += 1
            logger.error(
                f"Handler {self.handler_name} failed to handle {event.event_type}: {e}"
            )
            raise
    ...
    def get_handler_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_handled": (
                self._last_handled.isoformat() if self._last_handled else None
            ),
        }
...
class DomainEventPublisher(DomainReflectiveModule):
    ...
    def subscribe(self, event_type: str, handler: DomainEventHandler):
        ...
        logger.info(
            f"Subscribed handler {handler.handler_name} to event type {event_type}"
        )
    ...
    def unsubscribe(self, event_type: str, handler: DomainEventHandler):
        ...
        if event_type in self._handlers:
            ...
            logger.info(
                f"Unsubscribed handler {handler.handler_name} from event type {event_type}"
            )
    ...
    async def publish(self, event: DomainEvent):
        ...
        try:
            ...
            logger.info(
                f"Published event {event.event_type} for aggregate {event.aggregate_id}"
            )
    ...
    async def get_module_status(self):
        ...
        total_publications = self._published_events + self._failed_publications
        success_rate = (
            self._published_events / max(total_publications, 1)
    ...
    def validate_domain_invariants(self):
        ...
        if self._processing_queue.qsize() > 100:
            result.add_warning(
                f"Large event queue size: {self._processing_queue.qsize()}"
            )
