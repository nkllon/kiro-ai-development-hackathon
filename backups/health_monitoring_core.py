...
This module was extracted from health_monitoring.py
...
class HealthAlert:
    ...
    def __post_init__(self):
        ...
        if not isinstance(self.metric_value, (int, float)):
            raise ValueError(
                f"metric_value must be a number, got {type(self.metric_value)}"
            )
        if not isinstance(self.threshold_value, (int, float)):
            raise ValueError(
                f"threshold_value must be a number, got {type(self.threshold_value)}"
            )
        if self.metric_value < 0:
            raise ValueError(
                f"metric_value must be non-negative, got {self.metric_value}"
            )
        if self.threshold_value < 0:
            raise ValueError(
                f"threshold_value must be non-negative, got {self.threshold_value}"
            )
...
class HealthMonitoringSystem(ReflectiveModule):
    ...
    def get_health_indicators(self) -> Dict[str, Any]:
        ...
        return {
            "uptime_compliance": {
                "status": (
                    "healthy"
                    if uptime_metrics.availability_percentage >= 99.9
                    else "degraded"
                ),
            ...
            "component_health": {
                "status": (
                    "healthy" if len(self.degraded_components) == 0 else "degraded"
                ),
                "total_components": len(self.registered_components),
                "healthy_components": len(self.registered_components)
                - len(self.degraded_components),
                "degraded_components": len(self.degraded_components),
            },
            "alert_system": {
    ...
    def register_component(self, component: ReflectiveModule) -> bool:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to register component {component.module_name}: {e}"
            )
            return False
    ...
    def _check_component_health(self, component: ReflectiveModule):
        ...
        try:
            ...
            if len(self.component_health_history[component_name]) > 100:
                self.component_health_history[component_name] = (
                    self.component_health_history[component_name][-100:]
                )
    ...
    def _handle_component_degradation(
        self, component_name: str, component: ReflectiveModule, error: str = None
    ):
        ...
        try:
            ...
            self.logger.warning(
                f"Component {component_name} degraded. Graceful degradation: {degradation_result.degradation_applied}"
            )
        except Exception as e:
            self.logger.error(f"Failed to handle degradation for {component_name}: {e}")

    def _handle_component_recovery(
        self, component_name: str, component: ReflectiveModule
    ):
    ...
    def _update_uptime_tracking(self):
        ...
        critical_degraded = any(
            (comp in self.degraded_components for comp in self.critical_components)
        )
        if critical_degraded and self.current_downtime_start is None:
            ...
            downtime_duration = (
                datetime.now() - self.current_downtime_start
            ).total_seconds()
            downtime_event = {
    ...
    def calculate_uptime_metrics(self) -> UptimeMetrics:
        ...
        if total_runtime > 0:
            availability_percentage = (
                (total_runtime - total_downtime) / total_runtime * 100
            )
        else:
    ...
    def shutdown(self):
        ...
        try:
            with open(report_file, "w") as f:
                serialized_report = SerializationHandler.safe_serialize(
                    final_report, indent=2
                )
...
def __init__(self):
    ...
    self.monitoring_thread.start()
    self._update_health_indicator(
        "uptime_monitoring", HealthStatus.HEALTHY, "99.9%", "Uptime monitoring active"
    )
...
def calculate_uptime_metrics(self) -> UptimeMetrics:
    ...
    mtbf = 0.0
    if self.downtime_events:
        mttr = sum((event["duration_seconds"] for event in self.downtime_events)) / len(
            self.downtime_events
        )
        if len(self.downtime_events) > 1:
            time_between_failures = []
