"""
Bounded context and strategic design utilities for RM-DDD framework.

This module provides tools for defining bounded contexts, managing context
boundaries, implementing context mapping patterns, and supporting strategic
design decisions in domain-driven design.
"""
...
class ContextIntegration:
    ...
    data_flow: str = (
        "bidirectional"  # bidirectional, upstream_to_downstream, downstream_to_upstream
    )
    ...
    def validate_integration(self) -> ValidationResult:
        ...
        # Validate relationship and pattern compatibility
        if (
            self.relationship_type == ContextRelationshipType.SHARED_KERNEL
...
class BoundedContext(DomainReflectiveModule):
    ...
    def __init__(
        self, context_name: str, description: str, owner_team: Optional[str] = None
    ):
    ...
    def define_boundary(self, boundary: ContextBoundary):
        ...
        if not validation_result.is_valid:
            raise DomainException(
                f"Invalid context boundary: {validation_result.errors}",
                error_code="INVALID_CONTEXT_BOUNDARY",
            )
    ...
    def add_integration(self, integration: ContextIntegration):
        ...
        if not validation_result.is_valid:
            raise DomainException(
                f"Invalid context integration: {validation_result.errors}",
                error_code="INVALID_CONTEXT_INTEGRATION",
            )

        # Determine the other context name
        other_context = (
            integration.downstream_context
            if integration.upstream_context == self.context_name
            else integration.upstream_context
        )

        self._integrations[other_context] = integration
        logger.info(
            f"Added integration between {self.context_name} and {other_context}"
        )
    ...
    def register_service(self, service_name: str):
        ...
        logger.debug(
            f"Registered service {service_name} in context {self.context_name}"
        )
    ...
    def register_value_object(self, value_object_name: str):
        ...
        logger.debug(
            f"Registered value object {value_object_name} in context {self.context_name}"
        )
    ...
    def validate_boundary_integrity(self) -> ValidationResult:
        ...
        for other_context, integration in self._integrations.items():
            ...
            if not integration_result.is_valid:
                result.add_error(
                    f"Invalid integration with {other_context}: {integration_result.errors}"
                )

        # Check for boundary violations
        if self._boundary_violations:
            result.add_error(
                f"Boundary violations detected: {self._boundary_violations}"
            )

        return result

    def detect_boundary_violation(
        self, component_name: str, external_dependency: str
    ) -> bool:
        """
        ...
        """
        # Check if external dependency is from another context
        # without proper integration
        for integration in self._integrations.values():
            if external_dependency.startswith(
                integration.upstream_context
            ) or external_dependency.startswith(integration.downstream_context):
                # Dependency is through a defined integration
                return False

        # Check if dependency is on infrastructure (allowed)
        if any(
            infra_pattern in external_dependency.lower()
            for infra_pattern in ["infrastructure", "repository", "adapter", "client"]
        ):
            return False

        # Check if dependency is on shared kernel
        shared_kernel_contexts = [
            integration.upstream_context
            for integration in self._integrations.values()
            if integration.relationship_type == ContextRelationshipType.SHARED_KERNEL
        ]

        if any(
            external_dependency.startswith(context)
            for context in shared_kernel_contexts
        ):
            return False

        # If we get here, it might be a boundary violation
        ...
        logger.warning(
            f"Boundary violation detected in {self.context_name}: {violation_msg}"
        )
    ...
    def get_context_map(self) -> Dict[str, ContextMap]:
        ...
        for other_context, integration in self._integrations.items():
            context_map = ContextMap(
                upstream_context=integration.upstream_context,
                downstream_context=integration.downstream_context,
                relationship_type=integration.relationship_type.value,
                integration_pattern=integration.integration_pattern.value,
    ...
    def get_boundary_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "core_concepts": self._boundary.core_concepts,
            "business_capabilities": self._boundary.business_capabilities,
            "registered_entities": list(self._registered_entities),
            "registered_services": list(self._registered_services),
            "registered_value_objects": list(self._registered_value_objects),
            "integrations": list(self._integrations.keys()),
            "boundary_violations": len(self._boundary_violations),
        }
    ...
    async def get_module_status(self):
        ...
        return ModuleHealth(
            status=status,
            message=f"Bounded context: {self.context_name}",
            ...
            health_indicators={
                ...
                "registered_components": (
                    ...
                    + len(self._registered_value_objects)
                ),
            },
        )
    ...
    async def get_module_capabilities(self):
        ...
        return [
            ModuleCapability(
                name=f"bounded_context_{self.context_name}",
                description=f"Bounded context management for {self.context_name}",
                available=True,
                version="1.0.0",
            )
        ]
    ...
    async def get_health_indicators(self):
        ...
        return {
            ...
            "context_maps": self.get_context_map(),
    ...
    def get_domain_boundaries(self):
        ...
        return DomainBoundaries(
            ...
            invariants=[
                ...
                "Cross-context dependencies must go through defined integrations",
                "Ubiquitous language must be consistent within context",
            ],
            ubiquitous_language=self._boundary.ubiquitous_language,
            external_dependencies=self._boundary.external_dependencies,
        )
...
class ContextMapper(DomainReflectiveModule):
    ...
    def create_integration(
        ...
        **kwargs,
        ...
        if upstream_context not in self._contexts:
            raise DomainException(
                f"Upstream context {upstream_context} not registered",
                error_code="CONTEXT_NOT_FOUND",
            )

        if downstream_context not in self._contexts:
            raise DomainException(
                f"Downstream context {downstream_context} not registered",
                error_code="CONTEXT_NOT_FOUND",
            )
        ...
        if not validation_result.is_valid:
            raise DomainException(
                f"Invalid integration: {validation_result.errors}",
                error_code="INVALID_INTEGRATION",
            )

        # Add integration to both contexts
        ...
        logger.info(
            f"Created integration: {upstream_context} -> {downstream_context} "
            f"({relationship_type.value}, {integration_pattern.value})"
        )
    ...
    def validate_context_map(self) -> ValidationResult:
        ...
        for context_name, context in self._contexts.items():
            ...
            if not context_result.is_valid:
                result.add_error(
                    f"Context {context_name} validation failed: {context_result.errors}"
                )
        ...
        for integration in self._global_integrations:
            ...
            if not integration_result.is_valid:
                result.add_error(
                    f"Integration validation failed: {integration_result.errors}"
                )
    ...
    def _detect_circular_dependencies(self) -> List[str]:
        ...
        # In practice, you'd implement a proper graph cycle detection algorithm
        circular_deps = []

        for integration in self._global_integrations:
            # Check if there's a reverse integration
            reverse_integration = next(
                (
                    i
                    for i in self._global_integrations
                    if (
                        i.upstream_context == integration.downstream_context
                        and i.downstream_context == integration.upstream_context
                    )
                ),
                None,
            )
    ...
    def get_context_relationships(self, context_name: str) -> List[ContextIntegration]:
        ...
        return [
            ...
            for integration in self._global_integrations
            if (
                integration.upstream_context == context_name
                or integration.downstream_context == context_name
            )
        ]
    ...
    def generate_context_map_report(self) -> Dict[str, Any]:
        ...
        return {
            "contexts": {
                name: context.get_boundary_info()
                for name, context in self._contexts.items()
            },
            "integrations": [
                {
                    "upstream": integration.upstream_context,
                    "downstream": integration.downstream_context,
                    "relationship": integration.relationship_type.value,
            ...
            "validation_summary": (
                self.validate_context_map().to_dict()
                if hasattr(ValidationResult, "to_dict")
                else {}
            ),
    ...
    def suggest_integration_patterns(
        self, upstream_context: str, downstream_context: str
    ) -> List[Dict[str, Any]]:
        """
        ...
        # Suggest based on context characteristics
        # This is a simplified heuristic - in practice, you'd have more sophisticated logic

        # If contexts have shared concepts, suggest shared kernel
        ...
        if shared_concepts:
            suggestions.append(
                {
                    "relationship": ContextRelationshipType.SHARED_KERNEL,
                    "pattern": IntegrationPattern.SYNCHRONOUS_API,
                    "rationale": f"Shared concepts detected: {list(shared_concepts)}",
                    "confidence": 0.8,
                }
            )

        # Default suggestion: Anti-corruption layer with async messaging
        suggestions.append(
            {
                "relationship": ContextRelationshipType.ANTICORRUPTION_LAYER,
                "pattern": IntegrationPattern.ASYNCHRONOUS_MESSAGING,
                "rationale": "Safe default for maintaining context independence",
                "confidence": 0.6,
            }
        )

        # If one context is much larger, suggest customer-supplier
        upstream_size = len(upstream._registered_entities) + len(
            upstream._registered_services
        )
        downstream_size = len(downstream._registered_entities) + len(
            downstream._registered_services
        )

        if upstream_size > downstream_size * 2:
            suggestions.append(
                {
                    "relationship": ContextRelationshipType.CUSTOMER_SUPPLIER,
                    "pattern": IntegrationPattern.SYNCHRONOUS_API,
                    "rationale": f"Upstream context is significantly larger ({upstream_size} vs {downstream_size} components)",
                    "confidence": 0.7,
                }
            )
    ...
    async def get_module_status(self):
        ...
        return ModuleHealth(
            status=status,
            message=f"Context mapper managing {len(self._contexts)} contexts",
            capabilities=await self.get_module_capabilities(),
            health_indicators={
                "contexts_count": len(self._contexts),
                "integrations_count": len(self._global_integrations),
                "mapping_violations": len(self._mapping_violations),
            },
        )
    ...
    async def get_module_capabilities(self):
        ...
        return [
            ModuleCapability(
                ...
                description="Manages bounded context relationships and mappings",
                available=True,
                version="1.0.0",
            )
        ]
    ...
    async def get_health_indicators(self):
        """Get health indicators."""
        return {
            "context_map_report": self.generate_context_map_report(),
            "domain_context": self.domain_context,
        }
    ...
    def get_domain_boundaries(self):
        ...
        return DomainBoundaries(
            ...
            invariants=[
                "All contexts must be properly registered",
                "Integrations must be bidirectionally consistent",
