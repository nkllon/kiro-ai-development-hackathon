...
This module was extracted from manager_services.py
...
class DevpostProjectManager(ProjectManagerInterface):
    ...
    def get_project_metadata(self) -> ProjectMetadata:
        ...
        if readme_data:
            ...
            if not metadata.description:
                metadata.description = readme_data.get(
        ...
        if pyproject_data:
            ...
            metadata.repository_url = (
                metadata.repository_url or self._extract_repository_url(pyproject_data)
            )
            metadata.package_info.update(pyproject_data)
        git_data = self._extract_git_metadata()
        if git_data:
            metadata.repository_url = metadata.repository_url or git_data.get(
                "repository_url"
            )
            metadata.team_members.extend(git_data.get("contributors", []))
        metadata.title = (
            metadata.title.strip() if metadata.title != "Untitled Project" else ""
        )
        ...
        if not metadata.title or metadata.title == "Untitled Project":
            metadata.title = (
                self.project_root.name.replace("-", " ").replace("_", " ").title()
            )
        return metadata
    ...
    def _load_current_connection(self) -> None:
        ...
        try:
            self._current_connection = self.config_manager.load_connection(
                self.project_root
            )
        except Exception:
    ...
    def _parse_readme_content(self, content: str, path: Path) -> Dict[str, Any]:
        ...
        for i, line in enumerate(lines):
            ...
            if line.startswith("##") and (
                not line.lower().startswith("## description")
            ):
    ...
    def _validate_readme(self) -> List[str]:
        ...
        try:
            ...
            if len(content) < 200:
                issues.append(
                    "README is too short (minimum 200 characters recommended)"
                )
            ...
            if missing_sections:
                issues.append(
                    f"README missing recommended sections: {', '.join(missing_sections)}"
                )
        except Exception:
    ...
    def switch_project(self, project_id: str) -> bool:
        ...
        if not target_connection.local_path.exists():
            raise ConfigurationError(
                f"Project path {target_connection.local_path} no longer exists"
            )
    ...
    def get_project_status(self, project_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            status = {
                ...
                "last_sync": (
                    target_connection.last_sync.isoformat()
                    if target_connection.last_sync
                    else None
                ),
                "created_at": target_connection.created_at.isoformat(),
                "is_active": target_connection.devpost_project_id
                == original_project_id,
                ...
                conflicts.append(
                    {
                        ...
                        "project_id": project_id,
                        "paths": [
                            str(project_ids[project_id]),
                            str(connection.local_path),
                        ],
        ...
        for connection in connections:
            ...
            if hackathon_id in hackathon_paths:
                if path_str in hackathon_paths[hackathon_id]:
                    ...
                    hackathon_paths[hackathon_id][path_str] = [
                        connection.devpost_project_id
                    ]
            else:
                hackathon_paths[hackathon_id] = {
                    path_str: [connection.devpost_project_id]
                }
        for connection in connections:
    ...
    def _resolve_duplicate_project_id(self, resolution: str, **kwargs) -> bool:
        ...
        if resolution == "keep_one":
            if not project_id or not keep_path:
                raise ConfigurationError(
                    "project_id and keep_path required for keep_one resolution"
                )
            connections = self.config_manager.list_connections()
            for connection in connections:
                if (
                    connection.devpost_project_id == project_id
                    and str(connection.local_path) != keep_path
                ):
    ...
    def _resolve_duplicate_hackathon_path(self, resolution: str, **kwargs) -> bool:
        ...
        if resolution == "keep_one":
            if not all([hackathon_id, path, keep_project_id]):
                raise ConfigurationError(
                    "hackathon_id, path, and keep_project_id required"
                )
            connections = self.config_manager.list_connections()
            for connection in connections:
                if (
                    connection.hackathon_id == hackathon_id
                    and str(connection.local_path) == path
                    and (connection.devpost_project_id != keep_project_id)
                ):
                    self.config_manager.remove_connection(connection.local_path)
            return True
        return False
    ...
    def _resolve_missing_path(self, resolution: str, **kwargs) -> bool:
        ...
        if resolution == "remove":
            ...
            if not project_id or not new_path:
                raise ConfigurationError(
                    "project_id and new_path required for update_path resolution"
                )
            connections = self.config_manager.list_connections()
            for connection in connections:
                if connection.devpost_project_id == project_id:
