...
class ComparativeAnalysisEngine(ReflectiveModule):
    ...
    def compare_approaches(
        ...
        try:
            ...
            if (
                len(adhoc_values) < self.superiority_thresholds["minimum_sample_size"]
                or len(systematic_values)
                < self.superiority_thresholds["minimum_sample_size"]
            ):
                self.logger.warning(
                    f"Insufficient samples for {category}: adhoc={len(adhoc_values)}, systematic={len(systematic_values)}"
                )
                return None
            ...
            if higher_is_better:
                improvement_ratio = (
                    systematic_mean / adhoc_mean if adhoc_mean > 0 else float("inf")
                )
            else:
                improvement_ratio = (
                    adhoc_mean / systematic_mean
                    if systematic_mean > 0
                    else float("inf")
                )
            ...
            pooled_variance = (
                ((n1 - 1) * adhoc_std**2 + (n2 - 1) * systematic_std**2) / (n1 + n2 - 2)
                if n1 + n2 > 2
                else 1
            )
            standard_error = (
                math.sqrt(pooled_variance * (1 / n1 + 1 / n2))
            ...
            superiority_proven = (
                improvement_ratio
                >= self.superiority_thresholds["minimum_improvement_ratio"]
                and t_statistic
                >= self.superiority_thresholds["minimum_statistical_significance"]
                and ci_lower > 0  # Confidence interval doesn't include zero
            )
    ...
    def generate_superiority_report(
        ...
        superiority_scores = [
            min(2.0, r.improvement_ratio) for r in comparison_results.values()
        ]  # Cap at 2.0
        overall_superiority_score = statistics.mean(superiority_scores)

        # Calculate evidence quality score
        significance_scores = [
            min(1.0, r.statistical_significance / 3.0)
            for r in comparison_results.values()
        ]  # Normalize to 0-1
        sample_quality_scores = [
            min(1.0, min(r.sample_sizes) / 20) for r in comparison_results.values()
        ]  # 20 samples = perfect
        proven_scores = [
            1.0 if r.superiority_proven else 0.0 for r in comparison_results.values()
        ]
        ...
        statistical_summary = {
            ...
            "categories_with_proven_superiority": sum(
                1 for r in comparison_results.values() if r.superiority_proven
            ),
            "average_improvement_ratio": statistics.mean(improvement_ratios),
            "average_statistical_significance": statistics.mean(
                [r.statistical_significance for r in comparison_results.values()]
            ),
            "total_samples": sum(
                sum(r.sample_sizes) for r in comparison_results.values()
            ),
            "confidence_level": self.superiority_thresholds["confidence_level"],
        }
        ...
        if overall_superiority_score >= 1.5:
            recommendations.append(
                "Strong evidence of systematic approach superiority - ready for production deployment"
            )
        elif overall_superiority_score >= 1.2:
            recommendations.append(
                "Moderate evidence of systematic approach superiority - consider additional validation"
            )
        else:
            recommendations.append(
                "Insufficient evidence of systematic approach superiority - investigate methodology"
            )

        if evidence_quality_score >= 0.8:
            recommendations.append(
                "High-quality evidence suitable for stakeholder presentation"
            )
        elif evidence_quality_score >= 0.6:
            recommendations.append(
                "Moderate-quality evidence - consider collecting more samples"
            )
        else:
            recommendations.append(
                "Low-quality evidence - increase sample sizes and improve statistical rigor"
            )
        ...
        for category, result in comparison_results.items():
            if not result.superiority_proven:
                recommendations.append(
                    f"Investigate {category} - superiority not statistically proven"
                )
            if min(result.sample_sizes) < 10:
                recommendations.append(
                    f"Collect more {category} samples for better statistical power"
                )
    ...
    def validate_superiority_claims(self, report: SuperiorityReport) -> Dict[str, Any]:
        ...
        if (
            report.overall_superiority_score
            < self.superiority_thresholds["minimum_improvement_ratio"]
        ):
        ...
        for category, result in report.comparison_results.items():
            category_validation = {
                ...
                "improvement_ratio_ok": result.improvement_ratio
                >= self.superiority_thresholds["minimum_improvement_ratio"],
                "statistical_significance_ok": result.statistical_significance
                >= self.superiority_thresholds["minimum_statistical_significance"],
                "sample_size_ok": min(result.sample_sizes)
                >= self.superiority_thresholds["minimum_sample_size"],
            }
            ...
            if not all(category_validation.values()):
                validation_results["warnings"].append(
                    f"Category {category} has validation issues"
                )
