...
This module was extracted from architecture.py
...
class ArchitectureExpert(GhostbustersExpertAgent):
    ...
    def _init_architecture_patterns(self):
        ...
        self.design_patterns = {
            ...
            "factory": ["class\\s+\\w*Factory", "def\\s+create_\\w+"],
            "observer": ["class\\s+\\w*Observer", "def\\s+notify", "def\\s+update"],
            "strategy": ["class\\s+\\w*Strategy", "def\\s+execute"],
            "decorator": ["@\\w+", "def\\s+wrapper"],
            "adapter": ["class\\s+\\w*Adapter", "def\\s+adapt"],
        }
        self.anti_patterns = {
        ...
        self.solid_violations = {
            "srp": ["class\\s+\\w*Manager\\w*", "class\\s+\\w*Handler\\w*"],
            "ocp": ["if\\s+isinstance.*:", "type\\(.*\\)\\s*=="],
            "lsp": ["raise\\s+NotImplementedError"],
            "isp": [],
            "dip": ["import\\s+\\w+\\.\\w+\\.\\w+"],
        }
    ...
    def _check_solid_violations(self, content: str, file_path: Path) -> List[Finding]:
        ...
        findings = []
        if re.search("if\\s+isinstance\\s*\\(.*,\\s*\\w+\\)", content) or re.search(
            "type\\s*\\(.*\\)\\s*==", content
        ):
            findings.append(
                Finding(
