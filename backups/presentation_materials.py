"""
Presentation Materials Creator for Hackathon Demo Framework.

Creates professional presentation materials including slides, visual assets,
and supporting documentation optimized for hackathon judging.
"""
...
class PresentationMaterialsCreator:
    ...
    def __init__(self, project_path: Path):
        ...
        # Slide templates for different hackathon types
        ...
        self.logger.info(
            f"Presentation materials creator initialized for {self.project_path}"
        )
    ...
    def create_presentation_package(
        ...
        """
        self.logger.info(
            f"Creating presentation package using template: {template_name}"
        )
        ...
        visual_assets = self._generate_visual_assets(
            slides, systematic_evidence, technical_assessment
        )
        ...
        # Generate backup materials
        backup_materials = self._generate_backup_materials(
            demo_script, systematic_evidence
        )

        # Create judge handout
        judge_handout = self._create_judge_handout(
            hackathon_config, systematic_evidence, technical_assessment
        )
        ...
        package = PresentationPackage(
            ...
            judge_handout=judge_handout,
        )

        self.logger.info(f"Presentation package created with {len(slides)} slides")
        return package

    def generate_slide_deck_markdown(
        self, presentation_package: PresentationPackage
    ) -> str:
        """
        ...
        markdown_content.append(
            f"**Total Presentation Time:** {total_time} seconds ({total_time/60:.1f} minutes)\n"
        )
    ...
    def create_visual_asset_specifications(
        self, visual_assets: List[VisualAsset]
    ) -> str:
        """
        ...
        for i, asset in enumerate(visual_assets, 1):
            ...
            specifications.append(
                "- Text: Large enough to read from presentation distance"
            )
            specifications.append("")
    ...
    def optimize_for_time_constraints(
        self, presentation_package: PresentationPackage, max_duration_seconds: int
    ) -> PresentationPackage:
        """
        ...
        current_duration = sum(
            slide.timing_seconds for slide in presentation_package.slides
        )

        if current_duration <= max_duration_seconds:
            return presentation_package  # Already within limits

        self.logger.info(
            f"Optimizing presentation: {current_duration}s -> {max_duration_seconds}s"
        )
        ...
        for slide in presentation_package.slides:
            optimized_slide = SlideContent(
                ...
                title=slide.title,
                content=self._condense_content(slide.content, reduction_factor),
                visual_elements=slide.visual_elements,
                speaker_notes=self._condense_content(
                    slide.speaker_notes, reduction_factor
                ),
                timing_seconds=int(slide.timing_seconds * reduction_factor),
            )
            optimized_slides.append(optimized_slide)

        # Update timing guide
        optimized_timing_guide = {
            section: int(time * reduction_factor)
            for section, time in presentation_package.timing_guide.items()
        }

        return PresentationPackage(
            slides=optimized_slides,
            visual_assets=presentation_package.visual_assets,
            speaker_notes=presentation_package.speaker_notes,
            timing_guide=optimized_timing_guide,
            backup_materials=presentation_package.backup_materials,
            judge_handout=presentation_package.judge_handout,
        )
...
""",
            ...
            speaker_notes="Welcome judges and introduce the project with confidence. Emphasize systematic approach from the start.",
            timing_seconds=30,
        )
...
""",
            visual_elements=[
                ...
                "Before/current state diagram",
                "User pain point icons",
            ],
            speaker_notes="Establish clear problem context. Use specific examples and quantified impacts to make it relatable.",
            timing_seconds=60,
        )
...
""",
            ...
            speaker_notes="Present solution clearly with emphasis on systematic approach and key differentiators.",
            timing_seconds=90,
        )
...
""",
            ...
            speaker_notes="Transition to live demo. Have backup screenshots ready. Narrate clearly and highlight systematic elements.",
            timing_seconds=180,
        )

    def _create_technical_slide(
        self, technical_assessment: TechnicalAssessment
    ) -> SlideContent:
        """Create technical excellence slide."""
        return SlideContent(
            slide_type=SlideType.TECHNICAL,
...
""",
            ...
            speaker_notes="Highlight technical excellence and systematic development practices. Show concrete metrics.",
            timing_seconds=60,
        )

    def _create_systematic_slide(
        self, systematic_evidence: SystematicEvidence
    ) -> SlideContent:
        """Create systematic excellence slide."""
        return SlideContent(
            slide_type=SlideType.SYSTEMATIC,
...
""",
            visual_elements=[
                ...
                "Beast Mode framework illustration",
            ],
            speaker_notes="Emphasize systematic development advantages. Show how this differentiates from typical hackathon projects.",
            timing_seconds=60,
        )
...
""",
            ...
            speaker_notes="Present clear business value with quantified benefits. Show real-world applicability.",
            timing_seconds=60,
        )
...
""",
            visual_elements=[
                ...
                "Contact information",
                "QR code for project access",
                "Thank you message",
            ],
            speaker_notes="Strong closing with clear call-to-action. Invite questions and engagement.",
            timing_seconds=30,
        )
    ...
    def _generate_visual_assets(
        ...
        assets.append(
            VisualAsset(
                asset_type=VisualAssetType.DIAGRAM,
                name="System Architecture",
                description="High-level system architecture showing key components and data flow",
                generation_instructions="Create clean, professional diagram showing system components, data flow, and key integrations. Use consistent colors and clear labels.",
            )
        )
        ...
        assets.append(
            VisualAsset(
                asset_type=VisualAssetType.CHART,
                name="Quality Metrics Dashboard",
                description=f"Visual representation of technical quality metrics",
                generation_instructions=f"Create dashboard showing: Code Quality ({technical_assessment.code_quality_score:.1f}/100), Test Coverage ({technical_assessment.test_coverage_percentage:.1f}%), Documentation ({technical_assessment.documentation_score:.1f}/100). Use green/yellow/red color coding.",
            )
        )
        ...
        assets.append(
            VisualAsset(
                asset_type=VisualAssetType.DIAGRAM,
                name="Systematic Development Process",
                description="Flowchart showing systematic development methodology",
                generation_instructions="Create flowchart showing: Requirements → Design → Implementation → Testing → Validation. Include Beast Mode principles and quality gates.",
            )
        )
        ...
        assets.append(
            VisualAsset(
                asset_type=VisualAssetType.SCREENSHOT,
                name="Demo Screenshots",
                description="Key screenshots of the application in action",
                generation_instructions="Capture clean, high-resolution screenshots showing: 1) Starting state, 2) Key functionality, 3) Results/output. Ensure UI is clean and professional.",
            )
        )

        return assets

    def _create_speaker_notes(
        self, slides: List[SlideContent], demo_script: DemoScript
    ) -> str:
        ...
        for i, slide in enumerate(slides, 1):
            ...
            notes.append(
                f"**Cumulative Time:** {total_time}s ({total_time/60:.1f} minutes)"
            )
    ...
    def _create_timing_guide(
        self, slides: List[SlideContent], demo_script: DemoScript
    ) -> Dict[str, int]:
        """Create timing guide for presentation."""
        timing_guide = {}

        # Map slides to demo script sections
        section_mapping = {
            SlideType.TITLE: "opening_hook",
            SlideType.PROBLEM: "problem_statement",
            SlideType.SOLUTION: "solution_overview",
            SlideType.DEMO: "technical_demonstration",
            SlideType.TECHNICAL: "technical_demonstration",
            SlideType.SYSTEMATIC: "systematic_excellence",
            SlideType.IMPACT: "business_impact",
            SlideType.CLOSING: "closing_call_to_action",
        }
    ...
    def _generate_backup_materials(
        self, demo_script: DemoScript, systematic_evidence: SystematicEvidence
    ) -> List[str]:
        """Generate backup materials for presentation."""
        return [
            "Pre-recorded demo video (full functionality)",
            "Screenshot sequence with narration script",
            "Architecture diagram walkthrough",
            "Code quality metrics presentation",
            "Systematic development process showcase",
            "Static slides with all key points",
            "Judge handout with project summary",
        ]

    def _create_judge_handout(
        self,
        hackathon_config: HackathonConfig,
        systematic_evidence: SystematicEvidence,
        technical_assessment: TechnicalAssessment,
        ...
        handout.append(
            f"- **Code Quality:** {technical_assessment.code_quality_score:.1f}/100"
        )
        handout.append(
            f"- **Test Coverage:** {technical_assessment.test_coverage_percentage:.1f}%"
        )
        handout.append(
            f"- **Documentation:** {technical_assessment.documentation_score:.1f}/100"
        )
        handout.append(
            f"- **Overall Technical Score:** {technical_assessment.overall_technical_score:.1f}/100\n"
        )
        ...
        for criterion in hackathon_config.judging_criteria:
            handout.append(
                f"- **{criterion.criterion_name}** ({criterion.weight_percentage}%): [How project addresses this]"
            )
        handout.append("")
    ...
    def _condense_content(self, content: str, reduction_factor: float) -> str:
        ...
        # Simple content reduction - in practice, this would be more sophisticated
        lines = content.split("\n")
        target_lines = max(1, int(len(lines) * reduction_factor))

        # Keep most important lines (first and last, plus key bullet points)
        if len(lines) <= target_lines:
            return content

        important_lines = []
        important_lines.extend(lines[:2])  # Keep first 2 lines

        # Keep bullet points and key content
        for line in lines[2:-1]:
            if len(important_lines) >= target_lines - 1:
                break
            if (
                line.strip().startswith(("•", "-", "*", "#"))
                or "systematic" in line.lower()
            ):
                important_lines.append(line)

        important_lines.append(lines[-1])  # Keep last line
