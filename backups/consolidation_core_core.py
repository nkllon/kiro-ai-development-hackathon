"""
Consolidation Core Core

This module was extracted from consolidation_core.py
...
class SpecConsolidator(ReflectiveModule):
    ...
    def analyze_overlap(self, spec_set: List[str]) -> OverlapAnalysis:
        ...
        try:
            ...
            dependency_relationships = self._analyze_dependency_relationships(
                parsed_specs
            )
            ...
            risk_assessment = self._assess_consolidation_risks(
                consolidation_opportunities
            )
            effort_estimates = self._estimate_consolidation_effort(
                consolidation_opportunities
            )
    ...
    def _parse_spec_comprehensively(self, spec_name: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            requirements_content = (
                requirements_file.read_text() if requirements_file.exists() else ""
            )
            ...
            terminology = self._extract_terminology_detailed(
            ...
            interfaces.append(
                {
                    ...
                    "type": "class",
                    "methods": [
                        {"name": method, "return_type": return_type.strip()}
    ...
    def _calculate_complexity_score(
        ...
        complexity_factors = {
            "requirements_count": len(
                re.findall("### Requirement \\d+", requirements_content)
            )
            * 0.1,
            "acceptance_criteria_count": len(
                re.findall("\\d+\\.\\s+WHEN", requirements_content)
            )
            * 0.05,
            "interface_count": len(re.findall("class\\s+\\w+", design_content)) * 0.15,
            "method_count": len(re.findall("def\\s+\\w+", design_content)) * 0.02,
            "content_length": (len(requirements_content) + len(design_content))
            / 10000
            * 0.1,
        }
    ...
    def _calculate_quality_score(
        ...
        quality_factors = {
            ...
            "has_acceptance_criteria": (
                0.3 if "Acceptance Criteria" in requirements_content else 0
            ),
    ...
    def _calculate_requirement_complexity(self, req_text: str) -> float:
        ...
        complexity_indicators = {
            "criteria_count": len(re.findall("\\d+\\.\\s+", req_text)) * 0.1,
            "conditional_logic": len(
                re.findall("\\bIF\\b|\\bWHEN\\b|\\bAND\\b", req_text, re.IGNORECASE)
            )
            * 0.05,
    ...
    def _calculate_requirement_quality(self, criteria_lines: List[str]) -> float:
        ...
        quality_indicators = {
            ...
            "criteria_completeness": min(len(criteria_lines) / 5, 1.0) * 0.3,
            "uses_ears_format": (
                0.3
                if any(
                    (
                        "WHEN" in line and "THEN" in line and ("SHALL" in line)
    ...
    def _analyze_functional_overlaps(
        ...
        for i, spec1 in enumerate(spec_names):
            for spec2 in spec_names[i + 1 :]:
                ...
                overlap_percentage = (
                    len(overlap_keywords) / len(spec1_keywords.union(spec2_keywords))
                    if spec1_keywords.union(spec2_keywords)
                    else 0
                )
    ...
    def _identify_interface_conflicts(
        self, parsed_specs: Dict[str, Dict[str, Any]]
    ) -> Dict[str, List[str]]:
                ...
                interface_usage[interface_name].append(
                    {
                        ...
                        "follows_reflective_module": interface[
                            "follows_reflective_module"
                        ],
                    }
                )
        for interface_name, usages in interface_usage.items():
            if len(usages) > 1:
                ...
                if len(set((frozenset(sig) for sig in method_signatures))) > 1:
                    interface_conflicts[interface_name] = [
                        usage["spec"] for usage in usages
                    ]
        return interface_conflicts

    def _analyze_dependency_relationships(
        self, parsed_specs: Dict[str, Dict[str, Any]]
    ) -> Dict[str, List[str]]:
        ...
        for spec_name, spec_data in parsed_specs.items():
            ...
            for dependency in spec_data["dependencies"]:
                for other_spec in parsed_specs.keys():
                    if (
                        other_spec != spec_name
                        and other_spec.lower() in dependency.lower()
                    ):
            ...
            for other_spec, other_data in parsed_specs.items():
                if other_spec != spec_name:
                    ...
                    shared_interfaces = set(
                        (interface["name"] for interface in spec_data["interfaces"])
                    ).intersection(
                        set(
                            (
                                interface["name"]
                                for interface in other_data["interfaces"]
                            )
                        )
                    )
    ...
    def _generate_consolidation_opportunities(
        self,
        ...
        for pair_key, overlapping_functions in functional_overlaps.items():
            ...
            all_functions = spec1_data["functionality_keywords"].union(
                spec2_data["functionality_keywords"]
            )
            overlap_percentage = (
                len(overlapping_functions) / len(all_functions) if all_functions else 0
            )
            ...
            effort_estimate = self._estimate_consolidation_effort_for_pair(
                spec1_data, spec2_data, overlap_percentage
            )
            risk_level = self._assess_consolidation_risk_for_pair(
                spec1_data, spec2_data, dependency_relationships
            )
            benefits = self._identify_consolidation_benefits(
                spec1_data, spec2_data, overlap_percentage
            )
            challenges = self._identify_consolidation_challenges(
                spec1_data, spec2_data, dependency_relationships
            )
        ...
        opportunities.sort(
            ...
            * (
                1.0
                if x.risk_level == "low"
                else 0.5 if x.risk_level == "medium" else 0.2
            ),
            reverse=True,
        )
        return opportunities

    def _estimate_consolidation_effort_for_pair(
        self, spec1_data: Dict, spec2_data: Dict, overlap_percentage: float
    ) -> int:
        """Estimate effort in hours for consolidating a pair of specs"""
        base_effort = 8
        complexity_effort = (
            spec1_data["complexity_score"] + spec2_data["complexity_score"]
        ) * 20
        requirements_effort = (
            len(spec1_data["requirements"]) + len(spec2_data["requirements"])
        ) * 2
        interface_effort = (
            len(spec1_data["interfaces"]) + len(spec2_data["interfaces"])
        ) * 4
        overlap_reduction = overlap_percentage * 0.3
        total_effort = (
            base_effort + complexity_effort + requirements_effort + interface_effort
        )
        total_effort *= 1 - overlap_reduction
        return int(total_effort)

    def _assess_consolidation_risk_for_pair(
        self,
        spec1_data: Dict,
        spec2_data: Dict,
        dependency_relationships: Dict[str, List[str]],
    ...
    def _identify_consolidation_benefits(
        self, spec1_data: Dict, spec2_data: Dict, overlap_percentage: float
    ) -> List[str]:
    ...
    def _identify_consolidation_challenges(
        self,
        spec1_data: Dict,
        spec2_data: Dict,
        dependency_relationships: Dict[str, List[str]],
    ...
    def _recommend_consolidation_strategy(
        self,
        overlap_percentage: float,
        risk_level: str,
        spec1_data: Dict,
    ...
    def _identify_overlapping_spec_pairs(
        self, functional_overlaps: Dict[str, List[str]]
    ) -> List[Tuple[str, str]]:
    ...
    def is_healthy(self) -> bool:
        ...
        try:
            return (
                ...
                and self.governance_controller.is_healthy()
            )
        except Exception:
            return False
    ...
    def _get_primary_responsibility(self) -> str:
        ...
        return (
            "Systematically merge overlapping specs while preserving all functionality"
        )

    def create_consolidation_plan(
        self, overlap_analysis: OverlapAnalysis
    ) -> ConsolidationPlan:
        """
        ...
        try:
            ...
            requirement_mapping = self._create_requirement_mapping(
                target_specs, overlap_analysis
            )
            interface_standardization = self._plan_interface_standardization(
                target_specs, overlap_analysis
            )
            terminology_unification = self._plan_terminology_unification(
                target_specs, overlap_analysis
            )
            migration_steps = self._create_migration_steps(
                target_specs, primary_opportunity
            )
    ...
    def merge_requirements(
        self, overlapping_requirements: List[RequirementAnalysis]
    ) -> "UnifiedRequirement":
        """
        ...
        try:
            self.logger.info(
                f"Merging {len(overlapping_requirements)} overlapping requirements"
            )
            if not overlapping_requirements:
                raise ValueError("No requirements provided for merging")
            equivalent_groups = self._group_functionally_equivalent_requirements(
            ...
            validation_result = self._validate_merged_requirements(
                overlapping_requirements, merged_requirements
            )
            if not validation_result["is_valid"]:
                self.logger.warning(
                    f"Validation issues found: {validation_result['issues']}"
                )
            unified_requirement = self._create_unified_requirement(
                merged_requirements, validation_result
            )
    ...
    def _group_functionally_equivalent_requirements(
        self, requirements: List[RequirementAnalysis]
    ) -> List[List[RequirementAnalysis]]:
    ...
    def _are_functionally_equivalent(
        self, req1: RequirementAnalysis, req2: RequirementAnalysis
    ) -> bool:
        """Determine if two requirements are functionally equivalent using semantic analysis"""
        keyword_overlap = len(
            req1.functionality_keywords.intersection(req2.functionality_keywords)
        )
        keyword_union = len(
            req1.functionality_keywords.union(req2.functionality_keywords)
        )
        keyword_similarity = keyword_overlap / keyword_union if keyword_union > 0 else 0
        stakeholder_overlap = len(
            set(req1.stakeholder_personas).intersection(set(req2.stakeholder_personas))
        )
        stakeholder_similarity = stakeholder_overlap > 0
        criteria_similarity = self._calculate_criteria_similarity(
            req1.acceptance_criteria, req2.acceptance_criteria
        )
        return (
            keyword_similarity > 0.6
            and stakeholder_similarity
            and (criteria_similarity > 0.5)
        )

    def _calculate_criteria_similarity(
        self, criteria1: List[str], criteria2: List[str]
    ) -> float:
    ...
    def _merge_requirement_group(
        self, requirement_group: List[RequirementAnalysis]
    ) -> "UnifiedRequirement":
        ...
        for req in requirement_group:
            for criterion in req.acceptance_criteria:
                criterion_keywords = frozenset(
                    self._extract_meaningful_words(criterion)
                )
                if criterion_keywords not in seen_criteria:
                    merged_criteria.append(criterion)
                    seen_criteria.add(criterion_keywords)
        merged_personas = list(
            set(
                (
                    persona
                    for req in requirement_group
                    for persona in req.stakeholder_personas
                )
            )
        )
        ...
        merged_complexity_score = sum(
            (req.complexity_score for req in requirement_group)
        ) / len(requirement_group)
        return UnifiedRequirement(
            ...
            merged_content=self._create_merged_content(
                base_requirement, merged_criteria, merged_personas
            ),
    ...
    def _convert_to_unified_requirement(
        self, requirement: RequirementAnalysis
    ) -> "UnifiedRequirement":
    ...
    def _identify_and_resolve_conflicts(
        self, requirement_group: List[RequirementAnalysis]
    ) -> List[Dict[str, Any]]:
        ...
        for i, (req_id1, criterion1) in enumerate(all_criteria):
            for req_id2, criterion2 in all_criteria[i + 1 :]:
                if self._are_contradictory_criteria(criterion1, criterion2):
                    resolution = self._resolve_contradictory_criteria(
                        criterion1, criterion2, req_id1, req_id2
                    )
    ...
    def _are_contradictory_criteria(self, criterion1: str, criterion2: str) -> bool:
        ...
        contradictory_patterns = [
            ("SHALL\\s+([^.]+)", "SHALL\\s+NOT\\s+\\1"),
            ("MUST\\s+([^.]+)", "MUST\\s+NOT\\s+\\1"),
            ("WHEN\\s+([^T]+)\\s+THEN\\s+([^.]+)", "WHEN\\s+\\1\\s+THEN\\s+(?!.*\\2)"),
        ]
    ...
    def _resolve_contradictory_criteria(
        self, criterion1: str, criterion2: str, req_id1: str, req_id2: str
    ) -> Dict[str, Any]:
    ...
    def _validate_merged_requirements(
        self,
        original_requirements: List[RequirementAnalysis],
        merged_requirements: List["UnifiedRequirement"],
    ) -> Dict[str, Any]:
        ...
        original_avg_quality = sum(
            (req.quality_score for req in original_requirements)
        ) / len(original_requirements)
        merged_avg_quality = sum(
            (req.quality_score for req in merged_requirements)
        ) / len(merged_requirements)
        if merged_avg_quality < original_avg_quality * 0.9:
            validation_result["issues"].append(
                "Significant quality reduction in merged requirements"
            )
    ...
    def _create_unified_requirement(
        self,
        merged_requirements: List["UnifiedRequirement"],
        ...
        return UnifiedRequirement(
            ...
            merged_content=self._create_composite_content(
                unique_criteria, unique_personas
            ),
            functionality_keywords=all_keywords,
            acceptance_criteria=unique_criteria,
            stakeholder_personas=unique_personas,
            quality_score=validation_result["quality_assessment"][
                "merged_average_quality"
            ],
            complexity_score=sum((req.complexity_score for req in merged_requirements))
            / len(merged_requirements),
            conflicts_resolved=[
                conflict
                for req in merged_requirements
                for conflict in req.conflicts_resolved
            ],
    ...
    def _create_composite_content(
        self, criteria: List[str], personas: List[str]
    ) -> str:
        """Create content for composite unified requirement"""
        persona_text = (
            ", ".join(personas)
            if len(personas) > 1
            else personas[0] if personas else "system user"
        )
...
class UnifiedRequirement:
    ...
    def preserve_traceability(
        self, original_specs: List[str], unified_spec: str
    ) -> TraceabilityMap:
        """
        ...
        try:
            self.logger.info(
                f"Creating traceability map for consolidation: {original_specs} -> {unified_spec}"
            )
            consolidation_id = f"CONSOLIDATION_{self._generate_requirement_id()}"
            traceability_links = self._create_traceability_links(
                original_specs, unified_spec, consolidation_id
            )
            impact_analysis = self._perform_impact_analysis(
                original_specs, unified_spec, traceability_links
            )
            change_log = self._create_change_tracking_log(
                original_specs, unified_spec, traceability_links
            )
            validation_status = self._validate_traceability_completeness(
                original_specs, traceability_links
            )
    ...
    def _create_traceability_links(
        self, original_specs: List[str], unified_spec: str, consolidation_id: str
        ...
        if consolidation_plan and consolidation_plan.requirement_mapping:
            for (
                original_req_id,
                unified_req_id,
            ) in consolidation_plan.requirement_mapping.items():
                original_spec = self._find_spec_containing_requirement(
                    original_req_id, original_requirements
                )
                if original_spec:
    ...
    def _create_semantic_traceability_links(
        self,
        original_requirements: Dict[str, List[RequirementAnalysis]],
        ...
        for spec_name, requirements in original_requirements.items():
            for original_req in requirements:
                ...
                for unified_req in unified_requirements:
                    similarity = self._calculate_requirement_similarity(
                        original_req, unified_req
                    )
                    if similarity > best_similarity and similarity > 0.3:
                        best_similarity = similarity
                        best_match = unified_req
                if best_match:
                    transformation_type = self._infer_transformation_type(
                        original_req, best_match, best_similarity
                    )
    ...
    def _calculate_requirement_similarity(
        self, req1: RequirementAnalysis, req2: RequirementAnalysis
    ) -> float:
        """Calculate similarity between two requirements"""
        keyword_intersection = req1.functionality_keywords.intersection(
            req2.functionality_keywords
        )
        keyword_union = req1.functionality_keywords.union(req2.functionality_keywords)
        keyword_similarity = (
            len(keyword_intersection) / len(keyword_union) if keyword_union else 0
        )
        stakeholder_intersection = set(req1.stakeholder_personas).intersection(
            set(req2.stakeholder_personas)
        )
        stakeholder_union = set(req1.stakeholder_personas).union(
            set(req2.stakeholder_personas)
        )
        stakeholder_similarity = (
            len(stakeholder_intersection) / len(stakeholder_union)
            if stakeholder_union
            else 0
        )
        ...
        content_similarity = (
            len(content_intersection) / len(content_union) if content_union else 0
        )
        return (
            keyword_similarity * 0.5
            + stakeholder_similarity * 0.2
            + content_similarity * 0.3
        )

    def _infer_transformation_type(
        self,
        original_req: RequirementAnalysis,
        unified_req: RequirementAnalysis,
        similarity: float,
    ) -> str:
    ...
    def _generate_similarity_rationale(
        self, original_req: RequirementAnalysis, unified_req: RequirementAnalysis
    ) -> str:
        """Generate rationale for similarity-based traceability"""
        shared_keywords = original_req.functionality_keywords.intersection(
            unified_req.functionality_keywords
        )
        shared_personas = set(original_req.stakeholder_personas).intersection(
            set(unified_req.stakeholder_personas)
        )
        rationale_parts = []
        if shared_keywords:
            rationale_parts.append(
                f"Shared functionality: {', '.join(list(shared_keywords)[:3])}"
            )
        if shared_personas:
            rationale_parts.append(
                f"Common stakeholders: {', '.join(list(shared_personas))}"
            )
        return (
            ". ".join(rationale_parts)
            if rationale_parts
            else "General content similarity"
        )
    ...
    def _perform_impact_analysis(
        ...
        deprecated_links = [
            ...
            if link.transformation_type == "deprecated"
        ]
        ...
        unified_req_ids = set(
            (link.consolidated_requirement_id for link in traceability_links)
        )
        unified_data = self._parse_spec_comprehensively(unified_spec)
        if unified_data:
            all_unified_reqs = set(
                (req.requirement_id for req in unified_data["requirements"])
            )
        ...
        interface_changes = self._analyze_interface_changes(
            original_specs, unified_spec
        )
        impact_analysis["interface_changes"] = interface_changes
        migration_reqs = self._determine_migration_requirements(
            traceability_links, interface_changes
        )
    ...
    def _analyze_interface_changes(
        self, original_specs: List[str], unified_spec: str
    ) -> List[str]:
    ...
    def _determine_migration_requirements(
        self, traceability_links: List[TraceabilityLink], interface_changes: List[str]
    ) -> List[str]:
        """Determine migration requirements based on traceability and interface changes"""
        migration_requirements = []
        transformation_counts = {}
        for link in traceability_links:
            transformation_counts[link.transformation_type] = (
                transformation_counts.get(link.transformation_type, 0) + 1
            )
        if transformation_counts.get("deprecated", 0) > 0:
            migration_requirements.append(
                f"Remove {transformation_counts['deprecated']} deprecated requirements from implementations"
            )
        if transformation_counts.get("merged", 0) > 0:
            migration_requirements.append(
                f"Update {transformation_counts['merged']} merged requirements in implementations"
            )
        if transformation_counts.get("split", 0) > 0:
            migration_requirements.append(
                f"Refactor {transformation_counts['split']} split requirements in implementations"
            )
        if interface_changes:
            migration_requirements.append(
                f"Update implementations for {len(interface_changes)} interface changes"
            )
        return migration_requirements

    def _create_change_tracking_log(
        self,
        original_specs: List[str],
        unified_spec: str,
        traceability_links: List[TraceabilityLink],
                ...
                "metrics": {
                    ...
                    "transformation_types": {
                        t_type: len(
                            [
                                l
                                for l in traceability_links
                                if l.transformation_type == t_type
                            ]
                        )
                        for t_type in set(
                            (l.transformation_type for l in traceability_links)
                        )
                    },
                },
            }
    ...
    def _validate_traceability_completeness(
        self, original_specs: List[str], traceability_links: List[TraceabilityLink]
    ) -> Dict[str, bool]:
        ...
        for spec_name in original_specs:
            ...
            if spec_data:
                original_req_ids = set(
                    (req.requirement_id for req in spec_data["requirements"])
                )
                linked_req_ids = set(
                    (
                        link.original_requirement_id
                        for link in traceability_links
                        if link.original_spec == spec_name
                    )
                )
                missing_links = original_req_ids - linked_req_ids
                validation_status[f"{spec_name}_complete_traceability"] = (
                    len(missing_links) == 0
                )
                if missing_links:
                    self.logger.warning(
                        f"Missing traceability links for requirements in {spec_name}: {missing_links}"
                    )
        validation_status["bidirectional_consistency"] = (
            self._validate_bidirectional_consistency(traceability_links)
        )
        link_signatures = [
            (link.original_spec, link.original_requirement_id)
            for link in traceability_links
        ]
        validation_status["no_duplicate_links"] = len(link_signatures) == len(
            set(link_signatures)
        )
        validation_status["overall_valid"] = all(validation_status.values())
        return validation_status

    def _validate_bidirectional_consistency(
        self, traceability_links: List[TraceabilityLink]
    ) -> bool:
        """Validate that traceability links are bidirectionally consistent"""
        return True

    def _find_consolidation_plan_for_specs(
    ...
    def _find_spec_containing_requirement(
        self,
        requirement_id: str,
        original_requirements: Dict[str, List[RequirementAnalysis]],
    ) -> Optional[str]:
    ...
    def _generate_transformation_rationale(
        self,
        original_req_id: str,
        unified_req_id: str,
        transformation_type: str,
        ...
        return rationale_map.get(
            transformation_type,
            f"Requirement transformed using {consolidation_plan.consolidation_strategy.value} strategy",
        )
    ...
    def _create_requirement_mapping(
        self, target_specs: List[str], overlap_analysis: OverlapAnalysis
    ) -> Dict[str, str]:
    ...
    def _plan_interface_standardization(
        self, target_specs: List[str], overlap_analysis: OverlapAnalysis
        ...
        for (
            interface_name,
            conflicting_specs,
        ) in overlap_analysis.interface_conflicts.items():
            if len(conflicting_specs) > 1:
                best_interface = self._select_best_interface(
                    interface_name, conflicting_specs
                )
    ...
    def _select_best_interface(
        self, interface_name: str, conflicting_specs: List[str]
    ) -> Dict[str, Any]:
    ...
    def _plan_terminology_unification(
        self, target_specs: List[str], overlap_analysis: OverlapAnalysis
    ) -> List[TerminologyChange]:
    ...
    def _select_unified_terminology(
        self, term: str, conflicting_specs: List[str]
    ) -> Dict[str, str]:
    ...
    def _create_migration_steps(
        self, target_specs: List[str], opportunity: ConsolidationOpportunity
    ) -> List[MigrationStep]:
        ...
        for challenge in opportunity.challenges:
            if "complexity" in challenge.lower():
                risk_mitigation.append(
                    "Break down complex consolidation into smaller steps"
                )
...
def _generate_consolidation_opportunities(
    ...
    opportunities.sort(
        key=lambda x: x.overlap_percentage
        * (1.0 if x.risk_level == "low" else 0.5 if x.risk_level == "medium" else 0.2),
        reverse=True,
    )
    return opportunities
...
def _recommend_consolidation_strategy(
    self, overlap_percentage: float, risk_level: str, spec1_data: Dict, spec2_data: Dict
) -> ConflictResolutionStrategy:
    """Recommend consolidation strategy based on analysis"""
    if risk_level == "high":
        return ConflictResolutionStrategy.MANUAL_REVIEW
...
def _generate_similarity_rationale(
    ...
    return (
        ". ".join(rationale_parts) if rationale_parts else "General content similarity"
    )
...
def _perform_impact_analysis(
    ...
    for spec_name in original_specs:
        ...
        impact_analysis["affected_implementations"].extend(implementations)
    deprecated_links = [
        link for link in traceability_links if link.transformation_type == "deprecated"
    ]
    for link in deprecated_links:
        impact_analysis["deprecated_functionality"].append(
            f"{link.original_spec}:{link.original_requirement_id} - {link.rationale}"
