...
class AlertManager:
    ...
    async def resolve_alert(
        self, alert_id: str, resolution_message: str = "Manually resolved"
    ) -> bool:
        ...
        while self.alerting_active:
            try:
                ...
                for rule_name, rule in self.alert_rules.items():
                    ...
                    if (
                        not last_eval
                        or (datetime.now() - last_eval).total_seconds()
    ...
    async def _evaluate_rule(self, rule_name: str, rule: AlertRule) -> None:
        ...
        try:
            ...
            if (
                last_alert
                and (datetime.now() - last_alert).total_seconds()
    ...
    async def _check_auto_resolve(
        self, rule_name: str, rule: AlertRule, result: Dict[str, Any]
    ) -> None:
        """Check if any active alerts for this rule should be auto-resolved."""
        # Find active alerts for this rule
        rule_alerts = [
            alert for alert in self.active_alerts.values() if alert.name == rule_name
        ]
