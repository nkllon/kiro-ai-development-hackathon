    ...
    if "title" in metadata:
        ...
        if not isinstance(title, str) or len(title.strip()) == 0:
            ...
            validation_result["warnings"].append(
                "Title is very long (over 200 characters)"
            )
            ...
            for i, member in enumerate(team_members):
                if not isinstance(member, dict):
                    ...
                    validation_result["errors"].append(
                        f"Team member {i + 1} must be a dictionary"
                    )
                elif "name" not in member or "email" not in member:
                    validation_result["warnings"].append(
                        f"Team member {i + 1} missing name or email"
                    )
...
def validate_team_member_data(member_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "name" in member_data:
        ...
        if not isinstance(name, str) or len(name.strip()) < 2:
            ...
            validation_result["warnings"].append(
                "Name is very long (over 100 characters)"
            )
    if "email" in member_data:
        ...
        if not isinstance(email, str):
            ...
            validation_result["warnings"].append(
                "Email is very long (over 254 characters)"
            )
