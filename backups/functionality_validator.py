"""
Core Functionality Validator for Hackathon Demo Framework.

Provides systematic validation of core feature implementation and functionality
coverage to ensure hackathon submissions demonstrate working capabilities.
"""
...
class CoreFunctionalityValidator:
    ...
    def validate_core_functionality(self) -> ValidationResult:
        ...
        try:
            ...
            if coverage_score < 70:
                ...
                recommendations.append(
                    "Implement missing core features or add feature tests"
                )

            if integration_score < 75:
                issues.append(f"Integration validation failed: {integration_score:.1f}")
                recommendations.append(
                    "Fix integration issues and end-to-end workflows"
                )
        ...
        return ValidationResult(
            is_valid=score >= 80.0 and len(issues) == 0,
            score=score,
            issues=issues,
        ...
        try:
            ...
            if len(test_files) == 0:
                ...
                gaps["missing_tests"].append(
                    "Insufficient test coverage - less than 50% of source files have tests"
                )
            ...
            # Check for broken imports/integrations
    ...
    def generate_remediation_plan(self, gaps: Dict[str, Any]) -> List[str]:
        ...
        # High priority: Broken integrations
        if gaps.get("broken_integrations"):
            remediation_steps.append(
                "CRITICAL: Fix broken imports and integration issues"
            )
    ...
    def _analyze_feature_coverage(self) -> Dict[str, Any]:
        ...
        try:
            # Analyze source files for feature implementation
            source_files = list(self.project_path.rglob("src/**/*.py"))
                ...
                try:
                    ...
                    for node in ast.walk(tree):
                        if isinstance(node, ast.FunctionDef):
                            ...
                            # Check if function has implementation (not just pass/raise NotImplementedError)
                            ...
                            if has_implementation:
                                ...
                                coverage["implemented_features"].append(
            ...
            if total_functions > 0:
                coverage["coverage_percentage"] = (
                    implemented_functions / total_functions
                ) * 100
    ...
    def _validate_integrations(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if not import_issues:
                integration_results["working_integrations"].append(
                    "All imports working"
                )
                integration_results["integration_score"] = 100.0
            else:
                integration_results["broken_integrations"] = import_issues
                integration_results["integration_score"] = max(
                    0, 100 - len(import_issues) * 20
                )
                    ...
                    try:
                        if config_file.name == "requirements.txt":
                            with open(config_file, "r") as f:
                                lines = f.readlines()
                                if lines:  # Has content
                                    valid_configs += 1
                        elif config_file.name in ["pyproject.toml", "setup.py"]:
                            valid_configs += 1
                    except Exception as e:
                        integration_results["errors"].append(
                            f"Config file error {config_file}: {e}"
                        )

            if valid_configs > 0:
                integration_results["working_integrations"].append(
                    f"Valid configuration files: {valid_configs}"
                )
            else:
                integration_results["broken_integrations"].append(
    ...
    def _validate_interfaces(self) -> Dict[str, Any]:
        ...
        try:
            # Look for interface definitions (classes, functions with docstrings)
            source_files = list(self.project_path.rglob("src/**/*.py"))
            source_files.extend(
                [
                    f
                    ...
                    for node in ast.walk(tree):
                        if isinstance(node, (ast.ClassDef, ast.FunctionDef)):
                            ...
                            if (
                                ...
                                interface_results["defined_interfaces"].append(
                                    f"{source_file.name}::{node.name}"
                                )
                            else:
                                interface_results["missing_interfaces"].append(
                                    f"{source_file.name}::{node.name} (missing docstring)"
                                )

                except Exception as e:
                    interface_results["errors"].append(
                        f"Interface analysis error {source_file}: {e}"
                    )

            if total_interfaces > 0:
                interface_results["interface_score"] = (
                    documented_interfaces / total_interfaces
                ) * 100
            else:
                        ...
                        if isinstance(node, ast.Import):
                            for alias in node.names:
                                try:
                                    ...
                                    import_issues.append(
                                        f"Cannot import {alias.name} in {source_file.name}"
                                    )
                            ...
                            if node.module:
                                try:
                                    ...
                                    import_issues.append(
                                        f"Cannot import from {node.module} in {source_file.name}"
                                    )
    ...
    def _analyze_feature_implementation(self) -> Dict[str, List[str]]:
        ...
        # this would be more sophisticated based on project requirements
        try:
            source_files = list(self.project_path.rglob("src/**/*.py"))
            ...
            for source_file in source_files:
                try:
                    ...
                    if "TODO" in content or "FIXME" in content:
                        features["incomplete"].append(
                            f"{source_file.name} has TODO/FIXME items"
                        )

                    if "NotImplementedError" in content:
                        features["incomplete"].append(
                            f"{source_file.name} has NotImplementedError"
                        )

                    # If file has substantial content, consider it complete
                    if len(content.strip()) > 100:  # Arbitrary threshold
                        features["complete"].append(source_file.name)

                except Exception as e:
                    features["incomplete"].append(
                        f"Could not analyze {source_file}: {e}"
                    )
    ...
    def _check_documentation_completeness(self) -> List[str]:
        ...
        if undocumented_files:
            doc_issues.append(
                f"Files missing docstrings: {', '.join(undocumented_files[:5])}"
            )
