...
class BeastModeCLI(ReflectiveModule):
    ...
    def is_healthy(self) -> bool:
        ...
        return (
            ...
            and self.integration_manager.is_healthy()
    ...
    def _execute_health_command(self, args: List[str]) -> CLIResult:
        ...
        try:
            ...
            return CLIResult(
                command="health", success=False, output=f"Health check failed: {str(e)}"
            )
    ...
    def _execute_validate_command(self, args: List[str]) -> CLIResult:
        ...
        try:
            ...
            if validation_type == "infrastructure":
                ...
                consistency_result = (
                    self.consistency_validator.validate_complete_self_consistency()
                )
                output_lines = [
            ...
            return CLIResult(
                command="validate", success=True, output=output, data=result
            )
        except Exception as e:
            return CLIResult(
                command="validate", success=False, output=f"Validation failed: {str(e)}"
            )
    ...
    def _execute_pdca_command(self, args: List[str]) -> CLIResult:
        ...
        try:
            ...
            return CLIResult(
                command="pdca", success=False, output=f"PDCA command failed: {str(e)}"
            )
    ...
    def _execute_debug_command(self, args: List[str]) -> CLIResult:
        ...
        try:
            ...
            return CLIResult(
                command="debug", success=False, output=f"Debug command failed: {str(e)}"
            )
        ...
        try:
            ...
            if action == "list":
                ...
                for risk_id, risk_info in unknown_risks.items():
                    status_icon = (
                        "✅"
                        if risk_info["status"] == "mitigated"
                        else "🔄" if risk_info["status"] == "adaptive" else "⚠️"
                    )
                    output_lines.extend(
                        [
                ...
                if risk_id in unknown_risks:
                    ...
                    output_lines = [
                        f"Unknown risk ID: {risk_id}. Use 'list' to see all risks."
                    ]
            return CLIResult(
    ...
    def _calculate_success_rate(self) -> float:
        ...
        if self.cli_metrics["total_commands"] == 0:
            return 1.0
        return (
            self.cli_metrics["successful_commands"] / self.cli_metrics["total_commands"]
        )

    def _get_session_duration_minutes(self) -> float:
        """Get session duration in minutes"""
        return (
            datetime.now() - self.cli_metrics["session_start_time"]
        ).total_seconds() / 60
    ...
    def _update_average_execution_time(self, execution_time_ms: int):
        ...
        self.cli_metrics["average_execution_time_ms"] = (
            current_avg * (total_commands - 1) + execution_time_ms
        ) / total_commands
    ...
    def _update_most_used_command(self, command: str):
        ...
        if command_counts:
            self.cli_metrics["most_used_command"] = max(
                command_counts, key=command_counts.get
            )
...
def _execute_status_command(self, args: List[str]) -> CLIResult:
    ...
    try:
        ...
        return CLIResult(
            command="status", success=False, output=f"Status command failed: {str(e)}"
        )
...
def _execute_metrics_command(self, args: List[str]) -> CLIResult:
    ...
    try:
        ...
        return CLIResult(
            command="metrics", success=False, output=f"Metrics command failed: {str(e)}"
        )
