...
class EmergencyClaimValidator:
    ...
    def validate_emergency_claim(self, claim: EmergencyClaim) -> EmergencyValidation:
        ...
        """
        self.logger.info(
            f"Validating emergency claim {claim.claim_id} of type {claim.claim_type}"
        )
        ...
        if not emergency_config:
            return self._create_invalid_validation(
                claim.claim_id, f"Unknown emergency type: {claim.claim_type}"
            )
    ...
    def validate_emergency_evidence(
        self, evidence_text: str, evidence_type: str
    ) -> EmergencyEvidence:
        """
        ...
        if any(
            ...
            for keyword in ["confirmed", "verified", "validated"]
        ):
            confidence += 0.1
        ...
        description = (
            f"Evidence pattern match: {bool(match)}, confidence: {confidence:.2f}"
        )
    ...
    def _validate_evidence(
        self, claim: EmergencyClaim, config: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _validate_objective_criteria(
        self, claim: EmergencyClaim, config: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Validate against objective criteria."""
        ...
        for criterion, expected_value in criteria.items():
            ...
            if isinstance(expected_value, str):
                ...
                criterion_met = (
                    expected_value.lower() in claim.justification.lower()
                    or any(
                        keyword in claim.justification.lower()
                        for keyword in [
                            "critical",
                            "severe",
                            "high",
                ...
                criterion_met = any(
                    int(num) >= expected_value for num in numbers
                ) or any(
                    keyword in claim.justification.lower()
                    for keyword in [
                        "many",
    ...
    def _determine_approved_bypasses(
        self,
    ...
    def _generate_conditions(
        ...
        criteria_validation: Dict[str, Any],
        auto_approved: bool,
    ) -> List[str]:
    ...
    def _calculate_expiry(
        self, claim: EmergencyClaim, config: Dict[str, Any], is_valid: bool
    ) -> Optional[datetime]:
