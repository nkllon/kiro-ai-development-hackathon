"""
Beast Mode Help Wanted System

Implements help request broadcasting, capability matching, and collaboration tracking.
"""
...
class CapabilityMatcher:
    ...
    def calculate_match_score(
        ...
        # Base match score - use the actual weighted score, not normalized
        if total_weight > 0:
            # Normalize by the average weight to maintain relative differences
        ...
        # Collaboration history bonus (up to 0.3)
        collaboration_bonus = min(0.3, agent_collaboration_score * 0.1)
        
        # Final score (capped at 1.0, but allow bonuses to show effect)
    ...
    def find_best_matches(
        ...
        for agent in available_agents:
            ...
            # Include if score meets minimum threshold
            if score >= min_score:
                matches.append((agent, score))
        
        # Sort by score descending, then by collaboration score, then by recency
        matches.sort(
...
class HelpWantedSystem:
    ...
    def __init__(self, agent_registry: AgentRegistry):
        ...
        self.auto_accept_threshold = 1.5  # Disabled by default (score > 1.0 impossible)
    ...
    def process_help_request(self, message: BeastModeMessage, responder_id: str) -> Optional[HelpResponse]:
        ...
        # Only respond if we have a reasonable match
        if match_score < 0.3:
    ...
    def process_help_response(self, message: BeastModeMessage) -> bool:
        ...
        # Auto-accept high-confidence responses if enabled
        if (help_response.confidence_score >= self.auto_accept_threshold and 
            len(help_request.accepted_helpers) < help_request.max_helpers):
    ...
    def accept_help_response(self, request_id: str, response_id: str) -> Optional[CollaborationSession]:
        ...
        # Check if we can accept more helpers
        ...
        # Update request status if we have enough helpers
        if len(help_request.accepted_helpers) >= help_request.max_helpers:
    ...
    def complete_collaboration(self, session_id: str, success: bool, metrics: Optional[Dict[str, Any]] = None) -> bool:
        ...
        if total_collaborations > 0:
            self.stats['average_collaboration_duration'] = (
                (current_avg * (total_collaborations - 1) + duration) / total_collaborations
            )
    ...
    def find_matching_agents(self, required_capabilities: List[str]) -> List[Tuple[DiscoveredAgent, float]]:
        ...
        # Create a temporary help request for matching
