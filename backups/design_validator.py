...
class DesignValidator(ComplianceValidator):
    ...
    def _match_component_header(self, line: str) -> Optional[Tuple[str, ComponentType]]:
        ...
        class_match = re.search(
            r"###?\s+\d*\.?\s*(\w+)(?:\s+class|\s+Class)", line, re.IGNORECASE
        )
        if class_match:
        ...
        interface_match = re.search(
            r"###?\s+\d*\.?\s*(\w+)(?:\s+interface|\s+Interface)", line, re.IGNORECASE
        )
        if interface_match:
    ...
    def _parse_code_block(
        ...
        class_matches = re.finditer(
            r"class\s+(\w+)(?:\([^)]*\))?:", code_content, re.MULTILINE
        )
    ...
    def _is_method_in_class(self, content: str, function_start: int) -> bool:
        ...
        function_line = content[
            function_line_start : function_start + 20
