...
def analyze(spec_directory: str, output: str, save: Optional[str], verbose: bool):
    ...
    try:
        ...
        ecosystem_dag = loop.run_until_complete(
            orchestration_engine.dependency_analyzer.analyze_ecosystem_dependencies(spec_directory)
        )
...
@click.option('--strategy', type=click.Choice(['minimum', 'value', 'risk', 'balanced']), default='balanced',
...
def mvp_route(spec_directory: str, timeline: int, effort: int, strategy: str, output: str, save: Optional[str]):
    ...
    try:
        ...
        mvp_data = {
            ...
            "parallel_groups": len(orchestration_result.optimized_execution.parallel_groups),
...
@beast_dag.command()
@click.argument('spec_directory', type=click.Path(exists=True, file_okay=False, dir_okay=True))
@click.option('--complexity', type=click.Choice(['broken', 'chaotic', 'impossible']), default='chaotic',
