...
This module was extracted from auth_service_services.py
...
class DevPostAuthService:
    ...
    def _save_credentials(self) -> None:
        ...
        data = {
            ...
            "expires_at": (
                self.credentials.expires_at.isoformat()
    ...
    def refresh_access_token(self) -> bool:
        ...
        try:
            ...
            token_response = response.json()
            self.credentials.access_token = token_response["access_token"]
            self.credentials.refresh_token = token_response.get(
                "refresh_token", self.credentials.refresh_token
            )
            expires_in = token_response.get("expires_in", 3600)
            self.credentials.expires_at = datetime.now() + timedelta(seconds=expires_in)
    ...
    def is_authenticated(self) -> bool:
        ...
        if (
            ...
            and datetime.now() >= self.credentials.expires_at
        ):
