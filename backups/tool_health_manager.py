...
class ToolHealthManager(ReflectiveModule):
    ...
    def __init__(self):
        ...
        self.logger.info(
            "🔧 Tool Health Manager initialized - ready to fix tools first!"
        )
    ...
    def diagnose_tool_systematically(self, tool_name: str) -> ToolDiagnosis:
        ...
        self.logger.info(
            f"🔍 Diagnosis complete: {tool_name} {'healthy' if is_healthy else 'needs repair'}"
        )
        return diagnosis

    def repair_tool_systematically(
        self, tool_name: str, diagnosis: ToolDiagnosis
    ) -> ToolRepairResult:
        ...
        try:
            ...
            prevention_pattern = self._document_prevention_pattern(
                tool_name, diagnosis, repairs_applied
            )
            ...
            self.logger.info(
                f"🔧 Repair {status}: {tool_name} in {repair_duration.total_seconds():.1f}s"
            )
    ...
    def monitor_tool_health_continuously(self) -> Dict[str, Any]:
        ...
        for tool_name in self.monitored_tools:
            ...
            if tool_health["status"] in ["degraded", "failed"]:
                self.logger.warning(
                    f"⚠️ Tool {tool_name} needs attention: {tool_health['status']}"
                )
                # Could trigger automatic repair here

        self.logger.info(
            f"👀 Health monitoring complete: {health_report['healthy_tools']}/{health_report['tools_monitored']} tools healthy"
        )
    ...
    def fix_makefile_health_systematically(self) -> Dict[str, Any]:
        ...
        performance_comparison = self._measure_systematic_vs_adhoc_performance(
            "makefile", repair_result
        )
        ...
        if repair_result.repair_successful:
            self.logger.info(
                "🏆 SELF-APPLICATION SUCCESS! Beast Mode fixed its own Makefile systematically!"
            )
        else:
            self.logger.warning(
                "⚠️ Makefile repair needs additional work - but systematic approach captured learning!"
            )
    ...
    def _generate_repair_recommendations(
        self, tool_name: str, root_causes: List[str]
    ) -> List[str]:
        ...
        for cause in root_causes:
            if cause == "modular_makefile_structure_not_created":
                recommendations.append(
                    "Create makefiles/ directory with modular structure"
                )
            else:
    ...
    def _calculate_diagnosis_confidence(
        self, issues: List[str], root_causes: List[str]
    ) -> float:
    ...
    def _apply_systematic_repair(
        self, tool_name: str, root_cause: str
    ) -> Dict[str, Any]:
    ...
    def _document_prevention_pattern(
        self, tool_name: str, diagnosis: ToolDiagnosis, repairs: List[str]
    ) -> str:
        """Document pattern to prevent similar failures"""
        pattern = (
            f"Tool: {tool_name}, Issues: {diagnosis.issues_found}, Repairs: {repairs}"
        )
        return pattern
    ...
    def _measure_systematic_vs_adhoc_performance(
        self, tool_name: str, repair_result: ToolRepairResult
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "adhoc_estimated_time": repair_result.time_to_repair.total_seconds()
            * 3,  # Estimate ad-hoc takes 3x longer
    ...
    def get_module_status(self) -> Dict[str, Any]:
        ...
        successful_repairs = len(
            [r for r in self.repair_history if r.repair_successful]
        )
        repair_success_rate = (
            (successful_repairs / len(self.repair_history))
            ...
            indicators.append(
                {
                    ...
                    "status": (
                        "healthy"
                        if success_rate >= 0.8
                        else "degraded" if success_rate >= 0.6 else "unhealthy"
                    ),
