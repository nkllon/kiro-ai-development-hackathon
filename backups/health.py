...
RM components, including module health tracking, domain health assessment,
and systematic health indicator collection.
"""
...
class ModuleHealth:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "domain_health": (
                self.domain_health.to_dict() if self.domain_health else None
            ),
            "health_indicators": self.health_indicators,
            "performance_metrics": (
                {
                    "response_time_ms": self.performance_metrics.response_time_ms,
...
class DomainHealth:
    ...
    language_consistency: float  # 0.0 to 1.0
    ...
    def is_healthy(self) -> bool:
        ...
        return (
            ...
            and self.invariant_compliance
            and self.language_consistency > 0.8
            and self.complexity_score < 0.8
        )
...
class HealthMonitor:
    ...
    def __init__(self, module: "ReflectiveModuleBase"):
        ...
        self._check_interval = timedelta(seconds=30)  # Default 30 second checks
    ...
    async def start_monitoring(self, check_interval: Optional[timedelta] = None):
        ...
        logger.info(
            f"Health monitoring started for {self.module_id} with {self._check_interval} interval"
        )
        ...
        try:
            while self._monitoring_active:
                try:
                    ...
                    logger.error(
                        f"Error in health monitoring loop for {self.module_id}: {e}"
                    )
    ...
    async def update_health_status(self, health_status: ModuleHealth):
        ...
        # Keep only last 100 health checks to prevent memory growth
        ...
        # Check for health changes and log
        if len(self._health_history) > 1:
            previous_status = self._health_history[-2]
            if previous_status.status != health_status.status:
                logger.info(
                    f"Health status changed for {self.module_id}: {previous_status.status.value} -> {health_status.status.value}"
                )
    ...
    async def _update_health_indicators(self, health_status: ModuleHealth):
        ...
        # Status indicator
        self._health_indicators["status"] = HealthIndicator(
            name="status",
            status=health_status.status.value,
            value=health_status.status.value,
        ...
        if health_status.performance_metrics:
            ...
            self._health_indicators["response_time"] = HealthIndicator(
                name="response_time",
                status="healthy" if metrics.response_time_ms < 100 else "degraded",
                value=metrics.response_time_ms,
                threshold=100.0,
                message=f"Response time: {metrics.response_time_ms:.2f}ms",
                timestamp=timestamp,
            )

            self._health_indicators["error_rate"] = HealthIndicator(
                name="error_rate",
            ...
            self._health_indicators["cpu_usage"] = HealthIndicator(
                name="cpu_usage",
                status="healthy" if metrics.cpu_usage_percent < 80 else "degraded",
            ...
            self._health_indicators["memory_usage"] = HealthIndicator(
                name="memory_usage",
                status="healthy" if metrics.memory_usage_mb < 1000 else "degraded",
        ...
        if health_status.domain_health:
            ...
            self._health_indicators["domain_boundary_integrity"] = HealthIndicator(
                name="domain_boundary_integrity",
                status="healthy" if domain_health.boundary_integrity else "unhealthy",
                value=domain_health.boundary_integrity,
                message=f"Domain boundary integrity: {'OK' if domain_health.boundary_integrity else 'VIOLATED'}",
                timestamp=timestamp,
            )

            self._health_indicators["domain_invariant_compliance"] = HealthIndicator(
                name="domain_invariant_compliance",
                status="healthy" if domain_health.invariant_compliance else "unhealthy",
                value=domain_health.invariant_compliance,
                message=f"Domain invariant compliance: {'OK' if domain_health.invariant_compliance else 'VIOLATED'}",
                timestamp=timestamp,
            )

            self._health_indicators["domain_complexity"] = HealthIndicator(
                name="domain_complexity",
                status="healthy" if domain_health.complexity_score < 0.8 else "warning",
                value=domain_health.complexity_score,
                threshold=0.8,
                message=f"Domain complexity score: {domain_health.complexity_score:.2f}",
                timestamp=timestamp,
            )
    ...
    async def get_health_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "domain_health": (
                current_health.domain_health.to_dict()
                if current_health.domain_health
                else None
            ),
    ...
    def _calculate_health_trend(self) -> str:
        ...
        # Count healthy vs unhealthy states
        healthy_count = sum(
            1 for status in recent_statuses if status == ModuleStatus.AVAILABLE
        )
        degraded_count = sum(
            1 for status in recent_statuses if status == ModuleStatus.DEGRADED
        )
    ...
    def _calculate_uptime(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "degraded_checks": total_checks - healthy_checks,
        }
