...
class DesignValidator(ComplianceValidator):
    ...
    def validate(self, target: str) -> List[ComplianceIssue]:
        ...
        if self.implementation_cache is None:
            self.implementation_cache = self._load_implementation_components(
                target_path
            )
    ...
    def _parse_design_file(self, file_path: Path) -> Dict[str, DesignComponent]:
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            if line_stripped.startswith("```"):
                if in_code_block:
                    if code_block_content:
                        parsed_components = self._parse_code_block(
                            code_block_content, file_path, line_num
                        )
                        if (
                            current_component
                            and current_component.name in parsed_components
                        ):
            ...
            if component_match:
                ...
                if line_stripped.startswith("**Purpose**:") or line_stripped.startswith(
                    "Purpose:"
                ):
    ...
    def _match_component_header(self, line: str) -> Optional[Tuple[str, ComponentType]]:
        ...
        class_match = re.search(
            "###?\\s+\\d*\\.?\\s*(\\w+)(?:\\s+class|\\s+Class)", line, re.IGNORECASE
        )
        if class_match:
            return (class_match.group(1), ComponentType.CLASS)
        interface_match = re.search(
            "###?\\s+\\d*\\.?\\s*(\\w+)(?:\\s+interface|\\s+Interface)",
            line,
            re.IGNORECASE,
        )
        if interface_match:
        ...
        if component_match:
            ...
            if (
                component_name.endswith("Validator")
                or component_name.endswith("Analyzer")
                or component_name.endswith("Orchestrator")
                or component_name.endswith("Manager")
                or component_name.endswith("Handler")
    ...
    def _parse_code_block(
        self, code_lines: List[str], file_path: Path, line_num: int
    ) -> Dict[str, DesignComponent]:
        """
        ...
        class_matches = re.finditer(
            "class\\s+(\\w+)(?:\\([^)]*\\))?:", code_content, re.MULTILINE
        )
        ...
        for i, line in enumerate(lines):
            ...
            if stripped_line.startswith("def ") and (not in_class):
                ...
                if func_match:
                    ...
                    if not any(
                        (function_name in comp.methods for comp in components.values())
                    ):
    ...
    def _load_implementation_components(
        self, target_path: Path
    ) -> Dict[str, ImplementationComponent]:
        """
    ...
    def _parse_implementation_file(
        self, file_path: Path
    ) -> Dict[str, ImplementationComponent]:
        """
    ...
    def _extract_class_members(
        ...
        attr_matches = re.finditer(
            "^\\s+self\\.(\\w+)\\s*=", class_content, re.MULTILINE
        )
    ...
    def _analyze_alignment(self) -> AlignmentResult:
        ...
        alignment_score = (
            implemented_components / total_design_components * 100
            if total_design_components > 0
            else 100
        )
        if misaligned_components:
            alignment_penalty = (
                len(misaligned_components) / total_design_components * 20
            )
    ...
    def _components_aligned(
        self, design_comp: DesignComponent, impl_comp: ImplementationComponent
    ) -> bool:
        """
    ...
    def _generate_alignment_issues(
        ...
        if missing_implementations:
            affected_files = list(
                set((comp.file_path for comp in missing_implementations))
            )
        ...
        if extra_implementations:
            affected_files = list(
                set((comp.file_path for comp in extra_implementations))
            )
        ...
        if misaligned_components:
            affected_files = list(
                set((impl.file_path for _, impl in misaligned_components))
            )
        ...
        if alignment_score < 80.0:
            issues.append(
                ComplianceIssue(
                    ...
                    metadata={"alignment_score": alignment_score, "target_score": 80.0},
                )
            )
        return issues
...
def _match_component_header(self, line: str) -> Optional[Tuple[str, ComponentType]]:
    ...
    if class_match:
        return (class_match.group(1), ComponentType.CLASS)
    interface_match = re.search(
        "###?\\s+\\d*\\.?\\s*(\\w+)(?:\\s+interface|\\s+Interface)", line, re.IGNORECASE
    )
    if interface_match:
        return (interface_match.group(1), ComponentType.INTERFACE)
    component_match = re.search("###?\\s+\\d*\\.?\\s*(\\w+)", line)
