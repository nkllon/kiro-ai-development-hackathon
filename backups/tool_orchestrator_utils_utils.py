"""
Tool Orchestrator Utils Utils

This module was extracted from tool_orchestrator_utils.py
...
class ToolOrchestrator(ReflectiveModule):
    ...
    def register_tool(self, tool_definition: ToolDefinition) -> Dict[str, Any]:
        ...
        self.tool_metrics[tool_id] = ToolHealthMetrics(tool_id=tool_id, availability_percentage=100.0, average_execution_time_ms=0.0, success_rate=1.0, error_patterns=[], performance_trend='stable', last_health_check=datetime.now(), systematic_compliance_rate=1.0)
        self.logger.info(f'Tool registered: {tool_definition.name} ({tool_id})')
        return {'success': True, 'tool_id': tool_id, 'registration_time': datetime.now().isoformat(), 'systematic_constraints_validated': True, 'initial_status': ToolStatus.AVAILABLE.value}

    def intelligent_tool_selection(self, task_context: Dict[str, Any], execution_strategy: ExecutionStrategy=ExecutionStrategy.SYSTEMATIC_ONLY) -> Dict[str, Any]:
        ...
        try:
            ...
            decision_result = self._apply_decision_framework(candidate_tools, task_requirements, execution_strategy)
            decision_time = int((time.time() - start_time) * 1000)
            self._update_decision_metrics(decision_time)
            return {'selected_tool_id': decision_result['selected_tool']['tool_id'], 'tool_name': decision_result['selected_tool']['name'], 'decision_confidence': decision_result['confidence'], 'decision_rationale': decision_result['rationale'], 'alternative_tools': [t['tool_id'] for t in candidate_tools if t['tool_id'] != decision_result['selected_tool']['tool_id']], 'systematic_compliance': decision_result['systematic_compliance'], 'decision_time_ms': decision_time, 'execution_strategy': execution_strategy.value}
        except Exception as e:
            self.logger.error(f'Tool selection failed: {str(e)}')
            return {'error': str(e), 'fallback_recommendation': self._get_fallback_tool_recommendation(task_context)}
    ...
    def execute_tool_systematically(self, execution_request: ToolExecutionRequest) -> ToolExecutionResult:
        ...
        try:
            ...
            constraint_validation = self._validate_execution_constraints(tool_def, execution_request.parameters, execution_request.execution_strategy)
            ...
            execution_output = {'stdout': f'Simulated execution of {tool_id} with parameters {execution_request.parameters}', 'stderr': '', 'exit_code': 0}
            execution_time = int((time.time() - start_time) * 1000)
            result = ToolExecutionResult(request_id=request_id, tool_id=tool_id, status='success' if execution_output['exit_code'] == 0 else 'error', output=execution_output['stdout'], error_output=execution_output['stderr'], execution_time_ms=execution_time, exit_code=execution_output['exit_code'], systematic_compliance=constraint_validation['valid'], performance_metrics={'execution_successful': True}, timestamp=datetime.now(), recommendations=[])
    ...
    def monitor_tool_health(self, tool_id: Optional[str]=None) -> Dict[str, Any]:
        ...
        if tool_id:
            ...
            return {'tool_id': tool_id, 'health_status': health_result, 'health_metrics': self.tool_metrics[tool_id].__dict__, 'systematic_compliance': health_result['systematic_compliance'], 'recommendations': health_result['recommendations']}
        else:
            overall_health = {}
            for tid in self.registered_tools.keys():
                health_result = self._perform_tool_health_check(tid)
                overall_health[tid] = {'status': health_result['status'], 'availability': self.tool_metrics[tid].availability_percentage, 'success_rate': self.tool_metrics[tid].success_rate, 'performance_trend': self.tool_metrics[tid].performance_trend}
            return {'overall_health': overall_health, 'health_summary': self._generate_health_summary(), 'systematic_compliance_overview': self._calculate_overall_systematic_compliance(), 'monitoring_recommendations': self._generate_monitoring_recommendations()}
    ...
    def optimize_tool_performance(self, optimization_context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            optimization_opportunities = self._identify_optimization_opportunities(performance_analysis, optimization_context)
            optimization_results = []
            for opportunity in optimization_opportunities:
                if opportunity['systematic_safe']:
                    result = self._apply_optimization(opportunity['tool_id'], opportunity['optimization_type'], opportunity['parameters'])
                    optimization_results.append(result)
            impact_analysis = self._validate_optimization_impact(optimization_results)
            return {'optimization_applied': len(optimization_results), 'performance_improvements': impact_analysis['improvements'], 'systematic_compliance_maintained': impact_analysis['systematic_compliance'], 'bottleneck_analysis': {'bottlenecks': [], 'systematic_issues': 0}, 'optimization_recommendations': self._generate_optimization_recommendations(impact_analysis), 'next_optimization_cycle': self._schedule_next_optimization()}
        except Exception as e:
    ...
    def _get_candidate_tools(self, task_requirements: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for tool_id, tool_def in self.registered_tools.items():
            ...
            if self.tool_status[tool_id] == ToolStatus.FAILED:
                continue
            candidates.append({'tool_id': tool_id, 'name': tool_def.name, 'tool_type': tool_def.tool_type.value, 'performance_profile': tool_def.performance_profile, 'health_metrics': self.tool_metrics[tool_id], 'systematic_constraints': tool_def.systematic_constraints})
        return candidates
    ...
    def _calculate_tool_score(self, tool: Dict[str, Any], task_requirements: Dict[str, Any], execution_strategy: ExecutionStrategy) -> Dict[str, Any]:
        ...
        total_score = systematic_score * self.decision_criteria['systematic_compliance'] + performance_score * self.decision_criteria['performance'] + reliability_score * self.decision_criteria['reliability'] + availability_score * self.decision_criteria['availability']
    ...
    def _get_most_used_tools(self) -> List[Dict[str, Any]]:
        ...
        for tool_id, metrics in self.tool_metrics.items():
            tool_def = self.registered_tools[tool_id]
            usage_data.append({'tool_id': tool_id, 'tool_name': tool_def.name, 'usage_count': int(metrics.success_rate * 100), 'success_rate': metrics.success_rate})
        return usage_data[:5]
    ...
    def _rank_tools_by_performance(self) -> List[Dict[str, Any]]:
        ...
        for tool_id, metrics in self.tool_metrics.items():
            ...
            performance_ranking.append({'tool_id': tool_id, 'tool_name': tool_def.name, 'performance_score': performance_score})
    ...
    def _improve_tool_compliance(self, tool_id: str=None, optimization_params: Dict[str, Any]=None) -> Dict[str, Any]:
        ...
        if tool_id and optimization_params:
            ...
            if tool_id in self.registered_tools:
                ...
                return {'success': True, 'optimization_type': 'compliance_improvement', 'tool_id': tool_id, 'new_compliance_rate': new_compliance_rate, 'target_compliance': target_compliance, 'improvement_applied': True}
            else:
                return {'success': False, 'error': f'Tool {tool_id} not found', 'optimization_type': 'compliance_improvement'}
        ...
        if tool_id and optimization_params:
            ...
            if tool_id in self.tool_metrics:
                ...
                if hasattr(current_metrics, 'average_execution_time_ms'):
                    ...
                    new_time = max(current_time - target_reduction_ms, 100)
                    improvement_ms = current_time - new_time
                return {'success': True, 'optimization_type': 'performance_tuning', 'tool_id': tool_id, 'improvement_ms': improvement_ms, 'new_average_time_ms': new_time, 'target_reduction_ms': target_reduction_ms, 'optimization_applied': True}
            else:
                return {'success': False, 'error': f'Tool metrics for {tool_id} not found', 'optimization_type': 'performance_tuning'}
