...
class HubrisDetectorImpl(HubrisDetector):
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.accountability_gap_threshold = self.config.get(
        ...
        self.escalation_timeout = timedelta(
            hours=self.config.get("escalation_timeout_hours", 48)
        )
    ...
    def detect_patterns(
        self, actor_id: str, decision_history: List[Decision]
    ) -> HubrisScore:
        """
    ...
    def analyze_velocity(
        self, decisions: List[Decision], timeframe: timedelta
    ) -> VelocityAlert:
        """
        ...
        threshold_exceeded = (
            decisions_per_hour > self.velocity_threshold
            and verification_rate < 1 - self.accountability_gap_threshold
        )
    ...
    def check_bypass_attempts(
        self, actor_id: str, governance_events: List
    ) -> BypassAlert:
        """
    ...
    def escalate_persistent_patterns(
    ...
    def _detect_velocity_pattern(self, decisions: List[Decision]) -> HubrisPattern:
        ...
        if len(decisions) < 5:
            return None
        recent_decisions = [
            d for d in decisions if d.timestamp >= datetime.now() - timedelta(hours=24)
        ]
        if not recent_decisions:
            return None
        verification_rate = sum(
            (1 for d in recent_decisions if d.accountability_verified)
    ...
    def _detect_emergency_abuse_pattern(
    ...
    def _detect_self_authorization_pattern(
        self, decisions: List[Decision]
    ) -> HubrisPattern:
    ...
    def _calculate_hubris_score(self, patterns: List[HubrisPattern]) -> float:
        ...
        if not patterns:
            return 0.0
        weighted_score = sum(
            (pattern.confidence * pattern.weight for pattern in patterns)
        )
    ...
    def _analyze_trend(
        ...
        recent_verification_rate = sum(
            (1 for d in recent_decisions if d.accountability_verified)
        ) / len(recent_decisions)
        older_verification_rate = sum(
            (1 for d in older_decisions if d.accountability_verified)
        ) / len(older_decisions)
        ...
        if difference < -0.2:
            ...
            return (
                ...
                if recent_verification_rate < 0.3
    ...
    def _assess_risk_level(
        self, hubris_score: float, patterns: List[HubrisPattern]
    ) -> RiskLevel:
    ...
    def _generate_recommendations(
        self, patterns: List[HubrisPattern], risk_level: RiskLevel
        ...
        if risk_level in [RiskLevel.HIGH, RiskLevel.CRITICAL]:
            recommendations.append(
                RecommendedAction(
                    ...
                    success_criteria=[
                        "100% accountability verification",
        ...
        if any((p.pattern_type == "velocity_without_accountability" for p in patterns)):
            recommendations.append(
                RecommendedAction(
                    ...
                    success_criteria=[
                        "Decision rate < 5 per hour",
                        "Accountability verification > 95%",
                    ],
                )
            )
            ...
            recommendations.append(
                RecommendedAction(
                    ...
                    success_criteria=[
                        ...
                        "All attempts logged and escalated",
                    ],
                )
            )
...
def _detect_velocity_pattern(self, decisions: List[Decision]) -> HubrisPattern:
    ...
    if len(recent_decisions) >= self.velocity_threshold and verification_rate < 0.5:
        confidence = min(
            1.0,
            len(recent_decisions) / self.velocity_threshold * (1 - verification_rate),
        )
        return HubrisPattern(
            pattern_type="velocity_without_accountability",
            confidence=confidence,
...
def _detect_bypass_pattern(self, decisions: List[Decision]) -> HubrisPattern:
    """Detect governance bypass patterns."""
    bypass_indicators = [
        d for d in decisions if d.emergency_claimed and (not d.accountability_verified)
    ]
    if len(bypass_indicators) >= 3:
        confidence = min(1.0, len(bypass_indicators) / 5)
        return HubrisPattern(
