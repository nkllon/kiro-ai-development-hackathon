...
This module was extracted from contexts.py
...
class ContextMapper(DomainReflectiveModule):
    ...
    def create_integration(
        ...
        **kwargs,
        ...
        logger.info(
            f"Created integration: {upstream_context} -> {downstream_context} ({relationship_type.value}, {integration_pattern.value})"
        )
        return integration
        ...
        for integration in self._global_integrations:
            reverse_integration = next(
                (
                    ...
                    if i.upstream_context == integration.downstream_context
    ...
    def get_context_relationships(self, context_name: str) -> List[ContextIntegration]:
        ...
        return [
            integration
            for integration in self._global_integrations
            if integration.upstream_context == context_name
...
def detect_boundary_violation(
    ...
    if any(
        (external_dependency.startswith(context) for context in shared_kernel_contexts)
    ):
        return False
    violation_msg = f"Component {component_name} depends on {external_dependency} without defined integration"
    self._boundary_violations.append(violation_msg)
...
def _detect_circular_dependencies(self) -> List[str]:
    ...
    for integration in self._global_integrations:
        ...
        if reverse_integration:
            cycle = (
                f"{integration.upstream_context} <-> {integration.downstream_context}"
            )
            if cycle not in circular_deps:
                circular_deps.append(cycle)
    return circular_deps
