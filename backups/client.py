...
class DevpostAPIClient(DevpostAPIClientInterface):
    ...
    async def authenticate(self, credentials: Dict[str, Any]) -> AuthResult:
        ...
        try:
            ...
            if result.success:
                ...
                logger.error(
                    f"API client authentication failed: {result.error_message}"
                )
            ...
            return AuthResult(
                success=False, error_message=f"Authentication failed: {str(e)}"
            )
            ...
            for project_data in project_list:
                try:
                    ...
                    if not all(field in project_data for field in ["id", "title"]):
                        logger.warning(
                            f"Project data missing required fields: {project_data}"
                        )
                        continue
    ...
    async def update_project(
        self, project_id: str, updates: Dict[str, Any], partial_update: bool = True
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            response_data = await self._make_request(
                method, endpoint, json_data=update_payload
            )
            ...
            if success:
                logger.info(
                    f"Successfully updated project {project_id} fields: {list(updates.keys())}"
                )
            else:
                logger.warning(
                    f"Project update returned success=False for {project_id}"
                )
    ...
    async def upload_media(
        ...
        if file_size > self.MAX_REQUEST_SIZE:
            raise ValidationError(
                f"File too large: {file_size} bytes (max: {self.MAX_REQUEST_SIZE})"
            )
        ...
        try:
            ...
            logger.info(
                f"Successfully uploaded media {media_path.name} to project {project_id}"
            )
            return result

        except Exception as e:
            logger.error(
                f"Failed to upload media {media_path} to project {project_id}: {e}"
            )
            raise

    async def create_project(
        self, hackathon_id: str, project_data: Dict[str, Any], auto_submit: bool = False
    ) -> DevpostProject:
        """
        ...
        try:
            ...
            response_data = await self._make_request(
                "POST", endpoint, json_data=creation_payload
            )

            # Handle different response formats
            project_data_response = response_data.get(
                "project", response_data.get("data", response_data)
            )
            project = DevpostProject.from_dict(project_data_response)

            logger.info(
                f"Successfully created project {project.id} for hackathon {hackathon_id}"
            )
    ...
    async def delete_project(self, project_id: str) -> bool:
        ...
        try:
            ...
            if success:
                ...
                logger.warning(
                    f"Project deletion returned success=False for {project_id}"
                )
    ...
    async def add_team_member(
        self, project_id: str, username: str, role: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if result["success"]:
                logger.info(
                    f"Successfully added team member {username} to project {project_id}"
                )
            else:
                logger.warning(
                    f"Failed to add team member {username} to project {project_id}"
                )
            ...
            logger.error(
                f"Failed to add team member {username} to project {project_id}: {e}"
            )
            raise
    ...
    async def remove_team_member(
        ...
        try:
            ...
            if result["success"]:
                logger.info(
                    f"Successfully removed team member {username} from project {project_id}"
                )
            else:
                logger.warning(
                    f"Failed to remove team member {username} from project {project_id}"
                )
                result["error"] = response_data.get(
            ...
            logger.error(
                f"Failed to remove team member {username} from project {project_id}: {e}"
            )
            raise

    async def get_hackathon_deadlines(
        self, hackathon_id: str, include_past: bool = False
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            logger.info(
                f"Retrieved {len(deadlines)} deadlines for hackathon {hackathon_id}"
            )
            return deadlines
    ...
    async def get_submission_requirements(
        self, hackathon_id: str, project_id: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            logger.info(
                f"Retrieved {len(valid_requirements)} requirements for hackathon {hackathon_id}"
            )
            return valid_requirements

        except Exception as e:
            logger.error(
                f"Failed to get submission requirements for {hackathon_id}: {e}"
            )
            raise

    async def update_submission_status(
        self, project_id: str, status: str, completion_notes: Optional[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if result["success"]:
                logger.info(
                    f"Successfully updated status for project {project_id} to {status}"
                )
            else:
                logger.warning(
                    f"Status update returned success=False for project {project_id}"
                )
            ...
            logger.error(
                f"Failed to update submission status for project {project_id}: {e}"
            )
            raise

    async def validate_project_requirements(
        self, project_id: str, hackathon_id: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for req in missing_requirements:
                if isinstance(req, str):
                    ...
                    processed_req = {
                        "requirement_id": req.get(
                            "id", req.get("requirement_id", "unknown")
                        ),
                        "title": req.get("title", "Unknown Requirement"),
                        "description": req.get("description", ""),
                        "required": req.get("required", True),
                        "suggested_action": req.get(
                            "suggested_action", "Please complete this requirement"
                        ),
                    }
            ...
            logger.info(
                f"Validated project {project_id} - {result['completion_percentage']:.1f}% complete"
            )
            return result

        except Exception as e:
            logger.error(
                f"Failed to validate project requirements for {project_id}: {e}"
            )
            raise
    ...
    async def get_project_submission_history(
        ...
        try:
            ...
            logger.info(
                f"Retrieved {len(history)} history entries for project {project_id}"
            )
            return history

        except Exception as e:
            logger.error(
                f"Failed to get submission history for project {project_id}: {e}"
            )
            raise
    ...
    async def schedule_deadline_notification(
        ...
        try:
            ...
            if result["success"]:
                logger.info(
                    f"Scheduled {deadline_type} notification for project {project_id}"
                )
            else:
                logger.warning(
                    f"Failed to schedule notification for project {project_id}"
                )
            ...
            logger.error(
                f"Failed to schedule deadline notification for project {project_id}: {e}"
            )
            raise
    ...
    async def get_deadline_notifications(
        ...
        try:
            ...
            logger.info(
                f"Retrieved {len(processed_notifications)} notifications for project {project_id}"
            )
            return processed_notifications

        except Exception as e:
            logger.error(
                f"Failed to get deadline notifications for project {project_id}: {e}"
            )
            raise

    async def cancel_deadline_notification(
        self, project_id: str, notification_id: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if result["success"]:
                logger.info(
                    f"Cancelled notification {notification_id} for project {project_id}"
                )
            else:
            ...
            logger.error(
                f"Failed to cancel notification {notification_id} for project {project_id}: {e}"
            )
            raise
    ...
    async def _upload_large_media(
        ...
        init_response = await self._make_request(
            "POST", init_endpoint, json_data=metadata
        )
        ...
        try:
            ...
            # Complete upload
            complete_endpoint = (
                f"/projects/{project_id}/media/upload/{upload_id}/complete"
            )
            complete_payload = {"chunks": uploaded_chunks, "total_size": file_size}

            complete_response = await self._make_request(
                "POST", complete_endpoint, json_data=complete_payload
            )
            ...
            try:
                abort_endpoint = (
                    f"/projects/{project_id}/media/upload/{upload_id}/abort"
                )
    ...
    async def delete_media(self, project_id: str, media_id: str) -> bool:
        ...
        try:
            ...
            if success:
                logger.info(
                    f"Successfully deleted media {media_id} from project {project_id}"
                )
            else:
            ...
            logger.error(
                f"Failed to delete media {media_id} from project {project_id}: {e}"
            )
            raise
    ...
    async def get_project_media(self, project_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.info(
                f"Retrieved {len(media_files)} media files for project {project_id}"
            )
            return media_files
    ...
    async def update_media_metadata(
        self, project_id: str, media_id: str, updates: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            response_data = await self._make_request(
                "PATCH", endpoint, json_data=updates
            )
            ...
            for i, media_path in enumerate(media_files):
                try:
                    ...
                    upload_result = await self.upload_media(
                        project_id, media_path, progress_callback=file_progress
                    )

                    results["successful_uploads"].append(
                        {"file": str(media_path), "result": upload_result}
                    )
                    ...
                    results["failed_uploads"].append(
                        {"file": str(media_path), "error": str(e)}
                    )

            results["success_rate"] = len(results["successful_uploads"]) / len(
                media_files
            )
            ...
            try:
                ...
                if e.status in self.AUTH_ERROR_STATUS_CODES:
                    ...
                    if attempt < self.max_retry_attempts - 1:
                        ...
                        logger.warning(
                            f"Request failed with status {e.status}, retrying in {delay:.2f}s"
                        )
                        ...
                        logger.error(
                            f"Request failed after {self.max_retry_attempts} attempts: {e}"
                        )
                    ...
                    # Non-retryable error
                    logger.error(
                        f"Request failed with non-retryable status {e.status}: {e}"
                    )
                    raise NetworkError(f"Request failed: {str(e)}")
                ...
                if attempt < self.max_retry_attempts - 1:
                    ...
                    logger.error(
                        f"Network error after {self.max_retry_attempts} attempts: {e}"
                    )
    ...
    async def _handle_response(
        ...
        if response.status >= 400:
            ...
            logger.error(
                f"{method} {url} failed with status {response.status}: {error_text}"
            )
    ...
    def _check_rate_limit(self) -> bool:
        ...
        self._burst_timestamps = [
            ts for ts in self._burst_timestamps if ts > now - 10
    ...
    def get_client_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "rate_limit_remaining": max(
                0, self.MAX_REQUESTS_PER_WINDOW - len(self._request_timestamps)
            ),
        }
    ...
    # Deadline and Submission Requirement API Methods (Task 4.4)
    ...
    async def get_hackathon_deadlines(
        ...
        try:
            ...
            # Ensure each deadline has required fields
            processed_deadlines = []
            for deadline_data in deadlines:
                if not all(
                    field in deadline_data for field in ["type", "deadline_time"]
                ):
                    logger.warning(
                        f"Deadline data missing required fields: {deadline_data}"
                    )
                    continue

                # Standardize deadline format
                processed_deadline = {
                    "type": deadline_data.get("type", "submission"),
                    "deadline_time": deadline_data.get("deadline_time"),
                    "description": deadline_data.get("description", ""),
                    "is_hard_deadline": deadline_data.get("is_hard_deadline", True),
                    "requirements": deadline_data.get("requirements", []),
                    "notification_settings": deadline_data.get(
            ...
            logger.info(
                f"Retrieved {len(processed_deadlines)} deadlines for hackathon {hackathon_id}"
            )
    ...
    async def get_submission_requirements(
        ...
        try:
            ...
            requirements = response_data.get(
                "requirements", response_data.get("data", [])
            )
            ...
            for req_data in requirements:
                if not all(field in req_data for field in ["id", "title"]):
                    logger.warning(
                        f"Requirement data missing required fields: {req_data}"
                    )
                    continue
                ...
                processed_requirement = {
                    ...
                    "allowed_values": req_data.get("allowed_values", []),
                    "file_types": req_data.get("file_types", []),
                    "category": req_data.get("category", "general"),
                    "order": req_data.get("order", 0),
                }

                # Add project-specific completion status if available
                if project_id:
                    processed_requirement["completed"] = req_data.get(
                        "completed", False
                    )
                    processed_requirement["completion_notes"] = req_data.get(
                        "completion_notes", ""
                    )
            ...
            logger.info(
                f"Retrieved {len(processed_requirements)} requirements for hackathon {hackathon_id}"
            )
    ...
    async def update_submission_status(
        ...
        try:
            ...
            result = {
                "success": response_data.get("success", True),
                "previous_status": response_data.get("previous_status"),
                "new_status": response_data.get("status", status.lower()),
                "updated_at": response_data.get("updated_at"),
                "completion_notes": completion_notes,
            }
            ...
            if result["success"]:
                logger.info(
                    f"Successfully updated project {project_id} status to {status}"
                )
            else:
                logger.warning(
                    f"Status update returned success=False for project {project_id}"
    ...
    async def validate_project_requirements(
        ...
        try:
            ...
            result = {
                ...
                "validated_at": response_data.get(
                    "validated_at", datetime.now().isoformat()
                ),
            }
            ...
            for missing_req in result["missing_requirements"]:
                if isinstance(missing_req, dict):
                    processed_missing.append(
                        {
                            "requirement_id": missing_req.get("id"),
                            "title": missing_req.get("title"),
                            "description": missing_req.get("description", ""),
                            "required": missing_req.get("required", True),
                            "suggested_action": missing_req.get("suggested_action", ""),
                        }
                    )
                else:
                    # Handle simple string requirements
                    processed_missing.append(
                        {
                            "requirement_id": missing_req,
                            "title": missing_req,
                            "description": "",
                            "required": True,
                            "suggested_action": f"Please provide {missing_req}",
                        }
                    )
    ...
    async def get_project_submission_history(
        ...
        try:
            ...
            logger.info(
                f"Retrieved {len(processed_history)} submission history entries for project {project_id}"
            )
    ...
    async def schedule_deadline_notification(
        ...
        try:
            ...
            if result["success"]:
                logger.info(
                    f"Successfully scheduled {deadline_type} notification for project {project_id}"
                )
            else:
                logger.warning(
                    f"Failed to schedule notification for project {project_id}"
    ...
    async def get_deadline_notifications(
        ...
        try:
            ...
            notifications = response_data.get(
            ...
            for notification in notifications:
                processed_notification = {
                    ...
                    "deadline_type": notification.get("deadline_type"),
                    "scheduled_time": notification.get("scheduled_time"),
                    "notification_type": notification.get("notification_type"),
                    "status": notification.get("status", "scheduled"),
                    "sent_at": notification.get("sent_at"),
                    "custom_message": notification.get("custom_message"),
    ...
    async def cancel_deadline_notification(
        ...
        try:
            ...
            result = {
                "success": response_data.get("success", True),
                "notification_id": notification_id,
                "cancelled_at": response_data.get(
            ...
            if result["success"]:
                logger.info(
                    f"Successfully cancelled notification {notification_id} for project {project_id}"
                )
            else:
                logger.warning(
                    f"Failed to cancel notification {notification_id} for project {project_id}"
                )
                result["error"] = response_data.get(
                    "error", "Notification cancellation failed"
                )
