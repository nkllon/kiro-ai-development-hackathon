...
class ConsistencyCheck:
    ...
    def check(
        self, domains: DomainCollection, context: Dict[str, Any]
    ) -> List[HealthIssue]:
...
class DomainValidator(DomainSystemComponent):
    ...
    def validate_domain(
        ...
        with self._time_operation("validate_domain"):
            ...
            warnings = [
                issue for issue in all_issues if issue.severity == IssueSeverity.WARNING
            ]
            suggestions = [
                issue.suggested_fix
                for issue in all_issues
    ...
    def validate_domain_collection(
        self, domains: DomainCollection
    ...
    def _initialize_builtin_rules(self) -> None:
        ...
        def validate_required_fields(
            domain: Domain, context: Dict[str, Any]
        ) -> List[HealthIssue]:
            issues = []
    ...
    def _initialize_builtin_consistency_checks(self) -> None:
        ...
        def check_dependency_consistency(
            domains: DomainCollection, context: Dict[str, Any]
        ) -> List[HealthIssue]:
...
def __init__(self):
    self.domain_schema = {
        ...
        "properties": {
            "name": {"type": "string", "pattern": "^[a-z][a-z0-9_]*$", "minLength": 1},
