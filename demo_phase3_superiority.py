...
from src.competitive_launch.superiority_engine import (
    SystematicSuperiorityEngine, SuperiorityMetric, ROICalculation, EvidencePackage
)
...
class Phase3SuperiorityDemo:
    ...
    def _demo_quantitative_metrics(self):
        ...
        try:
            ...
            print(f"  Superiority Verified: {avg_improvement > 20 and high_confidence > len(metrics)//2}")
    ...
    def _demo_roi_analysis(self):
        ...
        try:
            ...
            print(f"\n💼 Business Value Analysis:")
            print(f"  Break-even point: {roi_12_months.payback_period_months:.1f} months")
            print(f"  Annual savings: ${roi_12_months.net_benefit:,.0f}")
            print(f"  3-year value: ${roi_24_months.net_benefit:,.0f}")
            print(f"  Risk level: {'Low' if roi_12_months.risk_adjusted_roi > 100 else 'Medium' if roi_12_months.risk_adjusted_roi > 50 else 'High'}")
    ...
    def _demo_evidence_packages(self):
        ...
        try:
            ...
            print(f"  ROI Analysis: {evidence_package.roi_calculation.roi_percentage:.1f}% ROI")
            print(f"  Competitive Advantages: {len(evidence_package.competitive_advantages)}")
            print(f"  Customer Testimonials: {len(evidence_package.customer_testimonials)}")
            print(f"  Case Studies: {len(evidence_package.case_studies)}")
            print(f"  Generated: {evidence_package.generated_at.strftime('%Y-%m-%d %H:%M:%S')}")
            
            # Show competitive advantages
            print(f"\n🏆 Top Competitive Advantages:")
            for i, advantage in enumerate(evidence_package.competitive_advantages[:3], 1):
                print(f"  {i}. {advantage}")
            if len(evidence_package.competitive_advantages) > 3:
                print(f"  ... and {len(evidence_package.competitive_advantages) - 3} more")
            
            # Show customer testimonials
            print(f"\n💬 Customer Testimonials:")
            for i, testimonial in enumerate(evidence_package.customer_testimonials[:2], 1):
                print(f"  {i}. \"{testimonial}\"")
            if len(evidence_package.customer_testimonials) > 2:
                print(f"  ... and {len(evidence_package.customer_testimonials) - 2} more")
    ...
    def _demo_competitive_positioning(self):
        ...
        try:
            ...
            print(f"  Systematic Superiority Score: {advantage['systematic_superiority_score']:.2f}")
            print(f"  Time to Market Advantage: {advantage['time_to_market_advantage']:.2f}")
    ...
    def _demo_integrated_superiority(self):
        ...
        try:
            ...
            if self.superiority_engine:
                ...
                print(f"     - Competitive advantage level: {summary['competitive_advantage_level']}")
            ...
            if self.intelligence_engine:
                advantage = self.intelligence_engine.calculate_competitive_advantage()
                print(f"     - Overall advantage: {advantage['overall_advantage']:.2f}")
                print(f"     - Differentiation factors: {len(advantage['differentiation_factors'])}")
