...
class GhostbustersConsultationRefactored:
    ...
    def run_autonomous_investigation(self, state: DevPostState) -> Dict[str, Any]:
        ...
        investigation_results = self.investigation_orchestrator.run_investigation(current_page_data)
    ...
    def _generate_recommendations(self, investigation_results: Dict[str, Any], confidence: float) -> Dict[str, Any]:
        ...
        structure_analysis = investigation_results["results"].get("PageStructureAnalyzer", {}).data
        navigation_analysis = investigation_results["results"].get("NavigationAnalyzer", {}).data
        content_analysis = investigation_results["results"].get("ContentAnalyzer", {}).data
        ...
        if structure_analysis.get("url_pattern", {}).get("pattern") == "devpost":
            ...
            recommendations["summary"] = "Project content page - focus on content analysis"
            recommendations["detailed"] = "This appears to be a project information page. Focus on content analysis and navigation."
            recommendations["next_steps"] = ["Analyze content", "Identify key information", "Navigate based on content"]
    ...
    def _assess_risk(self, investigation_results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        diagnostic_tests = investigation_results["results"].get("DiagnosticTester", {}).data.get("tests", {})
        overall_confidence = investigation_results["overall_confidence"]
        
        # Calculate risk factors
        failed_tests = sum(1 for test_result in diagnostic_tests.values() if not test_result)
        total_tests = len(diagnostic_tests)
        
        risk_score = (failed_tests / total_tests if total_tests > 0 else 0.5) + (1.0 - overall_confidence) * 0.5
...
def ghostbusters_consultation_refactored_node(state: DevPostState) -> DevPostState:
    ...
    try:
        ...
        if "ghostbusters_consultation_refactored" not in state:
            state["ghostbusters_consultation_refactored"] = GhostbustersConsultationRefactored()
        ...
        update_performance_metrics(state, {
            ...
            "modules_used": consultation_report['investigation_results']['successful_modules'],
            "total_modules": consultation_report['investigation_results']['total_modules']
        })
        ...
        print(f"   Modules used: {consultation_report['investigation_results']['successful_modules']}/{consultation_report['investigation_results']['total_modules']}")
