...
def investigate_redis_thoroughly():
    ...
    for db_num in range(16):
        try:
            db_redis = redis.Redis(host='localhost', port=6379, db=db_num, decode_responses=False)
            db_redis.ping()
        ...
        for pattern in patterns_to_check:
            try:
                ...
                for channel in test_channels:
                    ...
                    if result > 0:
                        print(f"  📡 Found subscribers on: {channel} ({result} subscribers)")
                        ...
                        if key_type == 'string':
                            ...
                            if value:
                                try:
                                    ...
                                    print(f"    └─ {key_name}: {value.decode('utf-8')[:100]}...")
                        else:
                            print(f"    └─ {key_name} ({key_type})")
    ...
    try:
        ...
        while time.time() - start_time < 5:
            try:
                ...
                if any(keyword in cmd_str.lower() for keyword in ['publish', 'subscribe', 'lpush', 'rpop']):
