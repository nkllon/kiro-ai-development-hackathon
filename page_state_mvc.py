    ...
    def get_page_info(self) -> Dict[str, Any]:
        ...
        try:
            return {
                ...
                "is_loading": self.page.evaluate("() => document.readyState")
                != "complete",
            }
        ...
        try:
            ...
            for i, form in enumerate(forms):
                form_info = {
                    ...
                    "field_count": len(
                        form.query_selector_all("input, textarea, select")
                    ),
                }
...
class FormController:
        ...
        try:
            ...
            for i, field in enumerate(fields):
                try:
                    ...
                    if field_id:
                        label_elem = self.view.page.query_selector(
            ...
            form_data = FormData(
                ...
                method=method,
                step=(
                    self.model.current_state.step
                    if self.model.current_state
                    else PageStep.UNKNOWN
                ),
    ...
    def _fill_field(self, field: FormField, project_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            print(
                f"   ✅ {field.label}: {str(value)[:50]}{'...' if len(str(value)) > 50 else ''}"
            )
            return True
    ...
    def _get_field_value(
        self, field: FormField, project_data: Dict[str, Any]
    ) -> Optional[str]:
...
class NavigationController:
    ...
    def _log_state_change(self, state: PageState):
        """Log state changes."""
        print(
            f"🔄 State: {state.step.value} | Auth: {state.is_authenticated} | Forms: {state.form_count}"
        )
...
class PageStateMVC:
    ...
    def process_current_page(self):
        ...
        print(
            f"Step: {self.model.current_state.step.value if self.model.current_state else 'Unknown'}"
        )
        print(
            f"URL: {self.model.current_state.url if self.model.current_state else 'Unknown'}"
        )
