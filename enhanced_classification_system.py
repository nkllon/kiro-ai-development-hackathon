...
class EnhancedClassificationSystem:
    ...
    def analyze_page_comprehensive(self, page_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        metaproperty_analysis = self.detect_metaproperties(
            page_data, cross_domain_patterns
        )
    ...
    def detect_spa_characteristics(self, page_data: Dict[str, Any]) -> SPAAnalysis:
        ...
        if (
            "history.pushState" in html_content
            or "history.replaceState" in html_content
        ):
    ...
    def detect_cross_domain_patterns(
        ...
        for pattern_type, indicators in self.cross_domain_indicators.items():
            pattern_matches = sum(
                1 for indicator in indicators if indicator in html_content
            )
                ...
                for similar_pattern in similar_patterns:
                    if similar_pattern["domain"] != domain:
                        cross_domain_pattern = CrossDomainPattern(
                            ...
                            investigation_priority=self._determine_priority(
                                similarity_score
                            ),
    ...
    def detect_metaproperties(
        self, page_data: Dict[str, Any], cross_domain_patterns: List[CrossDomainPattern]
    ) -> MetapropertyDetection:
        ...
        is_metaproperty = (
            len(metaproperty_indicators) > 2
            and novelty_score > 0.3
            and len(similar_patterns_found) > 0
        )
        ...
        return MetapropertyDetection(
            ...
            source_pattern=(
                cross_domain_patterns[0].source_domain
    ...
    def _find_similar_patterns(
        self, pattern_type: str, current_domain: str
    ) -> List[Dict[str, Any]]:
    ...
    def _extract_metaproperty_indicators(
        self, pattern_type: str, similar_pattern: Dict[str, Any]
    ) -> List[str]:
    ...
    def _generate_classification_summary(
        self,
        spa_analysis: SPAAnalysis,
        meta_analysis: MetaEcosystemAnalysis,
        metaproperty_analysis: MetapropertyDetection,
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "investigation_priority": (
                "high" if metaproperty_analysis.investigation_recommended else "low"
            ),
        }

    def _generate_investigation_priorities(
        self,
        cross_domain_patterns: List[CrossDomainPattern],
        metaproperty_analysis: MetapropertyDetection,
    ) -> List[Dict[str, Any]]:
