    ...
    def load_session_memory(self, filepath: str):
        ...
        if os.path.exists(filepath):
            with open(filepath, "r") as f:
                ...
                self.memory_qualification_queue = session_data.get(
                    "qualification_queue", []
                )
    ...
    try:
        ...
        memory_manager.add_short_term_memory(
            "ghostbusters_state",
            {
                ...
                "similarity_type": state.get("session_recovery", {}).get(
        ...
        recovery_message += (
            "I'm completely confused and need your help to continue.\n\n"
        )
        recovery_message += "📊 Current Situation:\n"
        recovery_message += f"   • URL: {current_page_data.get('url', 'Unknown')}\n"
        recovery_message += (
            f"   • Page Title: {current_page_data.get('title', 'Unknown')}\n"
        )
        ...
        recovery_message += (
            "   1. 📍 Tell me where we are (provide context about current page)\n"
        )
        recovery_message += (
            "   2. 🧭 Guide me step by step (provide specific navigation directions)\n"
        )
        recovery_message += (
        ...
        recovery_message += (
            "   • You can decide what data to persist for future sessions\n"
        )
...
def handle_recovery_choice(
    state: DevPostState, user_choice: str, user_input: str = ""
) -> DevPostState:
    """
    ...
    try:
        ...
        memory_manager.add_short_term_memory(
            "recovery_choice",
            {"choice": user_choice, "user_input": user_input, "timestamp": time.time()},
            importance="high",
        )
        ...
        if user_choice in ["1", "tell me where we are", "context"]:
            ...
            clarification_message = (
                f"🤔 I didn't understand your choice: '{user_choice}'\n\n"
            )
...
def handle_context_guidance(
    state: DevPostState, user_input: str, memory_manager: TieredMemoryManager
):
    ...
    memory_manager.add_short_term_memory(
        "user_context",
        {"context": user_input, "timestamp": time.time(), "source": "user_guidance"},
        importance="critical",
    )
    ...
    context_message += (
        "🔄 Attempting to adapt navigation model based on your context...\n\n"
    )
    ...
    if "devpost" in user_input.lower():
        context_message += (
            "🎯 Detected DevPost context - adapting for DevPost navigation\n"
        )
        ...
        context_message += (
            "👥 Detected team management context - adapting for team page navigation\n"
        )
...
def handle_step_by_step_guidance(
    state: DevPostState, user_input: str, memory_manager: TieredMemoryManager
):
    ...
    memory_manager.add_short_term_memory(
        "user_guidance",
        {"guidance": user_input, "timestamp": time.time(), "source": "step_by_step"},
        importance="critical",
    )
...
def handle_collaborative_analysis(
    state: DevPostState, user_input: str, memory_manager: TieredMemoryManager
):
    ...
    memory_manager.add_short_term_memory(
        ...
        {"user_input": user_input, "timestamp": time.time(), "mode": "collaborative"},
        importance="high",
    )
        ...
        if needs_user_input:
            ...
            for i, item in enumerate(needs_user_input[:3], 1):  # Show first 3
                ...
                qualification_message += (
                    f"   Data preview: {str(item['data'])[:100]}...\n\n"
                )
