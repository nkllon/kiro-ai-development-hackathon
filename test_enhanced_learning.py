...
def test_enhanced_learning():
    ...
    for i, pattern in enumerate(learning_patterns, 1):
        ...
        if result:
            ...
            print(
                f"     Updated Confidence: {updated_confidence:.3f} ({updated_confidence - initial_confidence:+.3f})"
            )
            print(
                f"     Updated Patterns: {updated_patterns} ({updated_patterns - initial_patterns:+d})"
            )
    ...
    if merge_result:
        ...
        print(
            f"   Post-merge Confidence: {post_merge_confidence:.3f} ({post_merge_confidence - pre_merge_confidence:+.3f})"
        )
        ...
        merged_pattern = next(
            (
                p
                for p in post_merge_intelligence.patterns
                if p.pattern_id == "enhanced-ghostbusters-001"
            ),
            None,
        )
        if merged_pattern:
            print(
                f"   Merged Pattern Steps: {len(merged_pattern.implementation_steps)}"
            )
            print(f"   Merged Pattern Metrics: {len(merged_pattern.success_metrics)}")
            print(
                f"   New Metrics: {[k for k in merged_pattern.success_metrics.keys() if k not in ['agent_coordination', 'systematic_compliance', 'failure_recovery', 'performance_optimization']]}"
            )
    ...
    if overall_insights["top_success_metrics"]:
        ...
        for metric, stats in list(overall_insights["top_success_metrics"].items())[:3]:
            print(
                f"       • {metric}: avg={stats['avg']:.3f}, max={stats['max']:.3f}, count={stats['count']}"
            )
