    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            for i, field in enumerate(fields, 1):
                ...
                print(f"      ID: {field_id}")
                print(f"      Value: {field_value[:50]}{'...' if len(field_value) > 50 else ''}")
                print(f"      Required: {field_required}")
                if field_placeholder:
                    print(f"      Placeholder: {field_placeholder}")
                if help_text:
                    print(f"      Help: {help_text[:50]}{'...' if len(help_text) > 50 else ''}")
                print()
            ...
            print(f"📊 Extracted {len(form_data['fields'])} fields from the DevPost submission form")
