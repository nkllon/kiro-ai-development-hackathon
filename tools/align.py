...
def calculate_with_audit(outcome, reqs, bounds=None, user_context=None, trace_id=None):
    ...
    try:
        ...
        if not is_feasible:
            emit_event("FeasibilityViolated", {
                "calculation_id": calculation_id,
                "outcome": outcome,
                "bounds": bounds,
                "timestamp": datetime.now(timezone.utc).isoformat()
...
def main():
    parser = argparse.ArgumentParser(description="Compute alignment metrics with audit trail.")
    parser.add_argument("--outcome", required=True, help='JSON list, e.g., "[0.8,0.6,-0.2]"')
    parser.add_argument("--reqs", required=True, help='JSON list of {"vector":[...],"weight":w}')
    ...
    try:
        ...
        if args.audit:
            result = calculate_with_audit(outcome, reqs, bounds, user_context, args.trace_id)
        else:
            ...
            for i in range(len(reqs)):
                for j in range(i+1, len(reqs)):
                    conf.append({"i": i, "j": j, "conflict": conflict(reqs[i]["vector"], reqs[j]["vector"])})
