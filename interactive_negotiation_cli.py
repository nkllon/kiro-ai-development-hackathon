...
class InteractiveNegotiationCLI:
    ...
    def _conduct_interactive_negotiation(
        ...
        print(
            "🤖 AI: I will stay in this negotiation until you provide a clear direction."
        )
        print("=" * 60)
        ...
        while negotiation_rounds < max_rounds:
            ...
            try:
                ...
                choice = (
                    input(f"\n🤖 AI: Your choice (round {negotiation_rounds}): ")
                    .strip()
                    .lower()
                )
                ...
                if choice in [str(i) for i in range(1, len(options) + 1)]:
                    ...
                    if chosen_option.risk_level in ["high", "experimental"]:
                        confirm = (
                            input(
                                f"🤖 AI: This is a {chosen_option.risk_level} risk option. Are you sure? (yes/no): "
                            )
                            .strip()
                            .lower()
                        )
                        if confirm not in ["yes", "y"]:
                            print(
                                f"🤖 AI: Understood. Let's continue negotiating other options."
                            )
                            continue
                    ...
                    print(
                        f"🤖 AI: Session state displayed. What would you like to do next?"
                    )
                ...
                print(
                    f"🤖 AI: Let's try a different approach. I'm still waiting for your direction."
                )

        # If we've exhausted negotiation rounds without clear direction
        print(f"\n🤖 AI: Maximum negotiation rounds reached ({max_rounds}).")
        print(
            f"🤖 AI: This is a terminal failure mode - I cannot proceed without clear direction."
        )
        print(f"🤖 AI: Preserving session with breadcrumbs.")
        return self._create_abandonment_result(context)
    ...
    def _handle_information_request(self, options: List[NegotiationOption]):
        ...
        try:
            ...
            if 0 <= option_index < len(options):
                ...
                print(f"   Session Impact: {option.session_impact}")
                print(
                    f"   Success Probability: {option.estimated_success_probability:.1%}"
                )
                print(
                    f"   Requires Human Approval: {'Yes' if option.requires_human_approval else 'No'}"
                )
                print(
                    f"   Fallback Available: {'Yes' if option.fallback_available else 'No'}"
                )
                ...
                if option.session_impact == "significant":
                    ...
                    print(
                        f"   Consider alternatives if session preservation is critical."
                    )
            else:
    ...
    def _handle_alternative_request(
        self, context: ImpasseContext, current_options: List[NegotiationOption]
    ):
        ...
        if alternatives:
            ...
            for i, option in enumerate(alternatives, 1):
                ...
                print(f"      Risk Level: {option.risk_level}")
                print(
                    f"      Success Probability: {option.estimated_success_probability:.0f}%"
                )
            ...
            print(
                f"🤖 AI: The current options represent the best approaches I can think of."
            )
    ...
    def _handle_custom_solution_request(self, context: ImpasseContext):
        ...
        if custom_solution:
            ...
            if analysis["feasible"]:
                ...
                proceed = (
                    input(
                        f"🤖 AI: Should we proceed with your custom solution? (yes/no): "
                    )
                    .strip()
                    .lower()
                )
                ...
                if proceed in ["yes", "y"]:
                    ...
                    print(
                        f"🤖 AI: Clear custom direction received! Executing your solution..."
                    )
    ...
    def _handle_diagnostics_request(self, context: ImpasseContext):
        ...
        diagnostic_file = (
            f"negotiation_diagnostics_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
        try:
    ...
    def _show_session_state(self, context: ImpasseContext):
        ...
        print(
            f"   Session Preservation Priority: {context.session_preservation_priority}"
        )
    ...
    def _analyze_custom_solution(self, solution: str, context: ImpasseContext) -> dict:
        ...
        risk_score = sum(
            1 for indicator in risk_indicators if indicator in solution_lower
        )
        safety_score = sum(
            1 for indicator in safe_indicators if indicator in solution_lower
        )
