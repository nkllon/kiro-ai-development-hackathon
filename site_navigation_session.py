...
class DevPostNavigationSession(SiteNavigationSession):
    ...
    def start_form_session(
        self, form_id: str, form_name: str, page_url: str, fields: List[FormField]
    ) -> FormSession:
    ...
    def get_session_analytics(self) -> Dict[str, Any]:
        ...
        successful_actions = sum(
            1 for action in self.navigation_actions if action.success
        )
        ...
        form_analytics = {
            ...
            "total_auto_fills": sum(
                form.auto_fill_attempts
                for form in self.active_forms + self.completed_forms
            ),
            "total_manual_edits": sum(
                form.manual_edits for form in self.active_forms + self.completed_forms
            ),
        }
...
if __name__ == "__main__":
    ...
    session.update_form_field(
        "project-overview-form",
        "description",
        "An amazing project that does amazing things",
        "1password",
    )
