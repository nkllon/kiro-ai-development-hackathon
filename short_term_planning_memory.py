...
class PlanningMemoryManager:
    ...
    def update_planning_depth(self, depth: int):
        ...
        if self.current_context:
            self.current_context.planning_depth = max(self.current_context.planning_depth, depth)
            self.current_context.last_updated = datetime.now()
    ...
    def create_first_contact_scenario(self, scenario_name: str) -> FirstContactScenario:
        ...
        # Get key insights
        key_insights = [insight for insight in self.insights if insight.importance in ['critical', 'high']]
    ...
    def load_planning_memory(self, filepath: str):
        ...
        self.current_context = PlanningContext(
            ...
            planning_dimensions=[PlanningDimension(**dim) for dim in context_data["planning_dimensions"]],
        ...
        # Restore insights
        self.insights = [PlanningInsight(**insight) for insight in memory_data["insights"]]
        
        # Restore scenarios
        self.scenarios = [FirstContactScenario(**scenario) for scenario in memory_data["scenarios"]]
    ...
    def _update_context_hash(self):
        ...
        if self.current_context:
            context_str = json.dumps(asdict(self.current_context), sort_keys=True, default=str)
            self.current_context.context_hash = hashlib.md5(context_str.encode()).hexdigest()
    
    def _create_decision_framework(self) -> Dict[str, Any]:
        """Create decision framework based on planning analysis"""
        return {
            "risk_assessment": {
                "critical_risks": [dim.name for dim in self.current_context.planning_dimensions if dim.risk_level == "critical"],
                "high_risks": [dim.name for dim in self.current_context.planning_dimensions if dim.risk_level == "high"],
                "mitigation_status": "ongoing"
            },
            "constraint_analysis": {
                "critical_constraints": [dim.name for dim in self.current_context.planning_dimensions if "critical" in dim.constraints],
                "constraint_count": self.current_context.total_constraints
            },
            "unknown_factors": {
                "critical_unknowns": [dim.name for dim in self.current_context.planning_dimensions if "critical" in dim.unknown_factors],
...
def create_planning_dimension(name: str, risk_level: str, unknown_factors: List[str], 
                            constraints: List[str], mitigation_strategies: List[str]) -> PlanningDimension:
...
def create_planning_insight(insight_type: str, title: str, description: str, 
                          importance: str, related_dimensions: List[str]) -> PlanningInsight:
