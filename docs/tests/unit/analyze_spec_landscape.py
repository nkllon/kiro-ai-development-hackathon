...
def generate_conflict_report(analysis) -> Dict[str, Any]:
    ...
    for term, affected_specs in analysis.terminology_conflicts.items():
        report["terminology_conflicts"][term] = {
            ...
            "severity": (
                "high"
                if len(affected_specs) > 3
                else "medium" if len(affected_specs) > 2 else "low"
            ),
            "resolution_priority": len(affected_specs)
    ...
    for interface, affected_specs in analysis.interface_conflicts.items():
        report["interface_conflicts"][interface] = {
            ...
            "resolution_priority": len(affected_specs)
            * 20,  # Higher priority than terminology
        }

    # Analyze dependency conflicts (circular dependencies, etc.)
    dependency_conflicts = analyze_dependency_conflicts(
    ...
    report["conflict_summary"] = {
        ...
        "high_priority_conflicts": len(
            [
                c
                for c in report["terminology_conflicts"].values()
                if c["severity"] == "high"
            ]
        ),
        "resolution_recommendations": generate_conflict_resolution_recommendations(
            report
        ),
    }
...
def analyze_dependency_conflicts(
    ...
    for spec, deps in dependency_relationships.items():
        for dep in deps:
            if (
                dep in dependency_relationships
                and spec in dependency_relationships[dep]
            ):
...
def generate_conflict_resolution_recommendations(report: Dict[str, Any]) -> List[str]:
    ...
    if report["interface_conflicts"]:
        recommendations.append(
            "CRITICAL: Resolve interface conflicts immediately - these prevent successful integration"
        )
        recommendations.append(
            "Create unified interface definitions in a shared module"
        )
        recommendations.append(
            "Implement interface versioning strategy for backward compatibility"
        )

    # Terminology conflict recommendations
    if report["terminology_conflicts"]:
        recommendations.append(
            "Establish unified terminology registry with canonical definitions"
        )
        recommendations.append(
            "Implement automated terminology validation in CI/CD pipeline"
        )
        recommendations.append(
            "Create glossary document with approved terms and definitions"
        )

    # Dependency conflict recommendations
    if report["dependency_conflicts"]:
        recommendations.append(
            "Resolve circular dependencies through architectural refactoring"
        )
        recommendations.append("Implement dependency injection to reduce coupling")
        recommendations.append(
            "Consider breaking large specs into smaller, focused components"
        )
...
def generate_consolidation_roadmap(ranking: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for consolidation in ranking["high_priority_consolidations"]:
        if (
            consolidation["risk_level"] == "low"
            and consolidation["effort_estimate_hours"] < 40
        ):
    ...
    roadmap["phase_3_strategic"]["consolidations"].extend(
        ranking["low_priority_consolidations"]
    )
...
def generate_comprehensive_summary(analysis, all_specs: List[str]) -> Dict[str, Any]:
    ...
    total_conflicts = len(analysis.terminology_conflicts) + len(
        analysis.interface_conflicts
    )
    ...
    summary["executive_summary"] = {
        ...
        "fragmentation_level": (
            "high" if total_overlaps > 10 else "medium" if total_overlaps > 5 else "low"
        ),
    ...
    summary["key_findings"] = {
        ...
        "dependency_complexity": {
            "total_dependencies": sum(
                len(deps) for deps in analysis.dependency_relationships.values()
            ),
            "average_per_spec": sum(
                len(deps) for deps in analysis.dependency_relationships.values()
            )
            / len(all_specs),
            "highly_coupled_specs": [
                spec
                for spec, deps in analysis.dependency_relationships.items()
                if len(deps) > 5
            ],
        },
    }
