    ...
    try:
        ...
        if success:
            ...
            if "Error: google: could not find default credentials" in stderr:
                print(
                    "✅ Terraform plan syntax valid (GCP auth expected to fail in test)"
                )
            else:
...
def show_deployment_instructions():
    ...
    print(
        '   terraform plan -var="project_id=$PROJECT_ID" -var-file="environments/dev.tfvars"'
    )
    print(
        '   terraform apply -var="project_id=$PROJECT_ID" -var-file="environments/dev.tfvars"'
    )
    print("")
    print("4. Configure kubectl:")
    print(
        "   gcloud container clusters get-credentials beast-mode-dev --region us-central1"
    )
    print("")
