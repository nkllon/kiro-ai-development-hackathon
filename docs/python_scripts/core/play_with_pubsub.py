...
from beast_mode.messaging.pubsub import (
    PubSubManager,
    BeastModeMessage,
    MessageType,
    MessageHandler,
    ...
    try:
        ...
        while True:
            ...
            if command == "q":
                ...
                message_id = await pubsub.send_prompt_request(
                    prompt, "playground", priority=8
                )
                print(f"✅ Sent prompt request: {message_id}")

            elif command == "2":
                spore_type = input("Enter spore type (default: tidb): ") or "tidb"
                message_id = await pubsub.send_spore_spawn_request(
                    spore_type, {"test": True}
                )
                ...
                print(
                    f"   Messages processed: {health['metrics']['messages_processed']}"
                )
