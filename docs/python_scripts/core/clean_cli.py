...
def execute_tasks(args):
    ...
    engine = ExecutionEngine(
        auto_merge=not args.no_merge, auto_revert_on_failure=not args.no_revert
    )
...
def main():
    ...
    execute_parser.add_argument(
        "--no-merge", action="store_true", help="Don't auto-merge successful execution"
    )
    execute_parser.add_argument(
        "--no-revert", action="store_true", help="Don't auto-revert failed execution"
    )
