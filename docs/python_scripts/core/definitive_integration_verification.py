    ...
    try:
        ...
        mock_state["session_save_data"] = {
            "current_page_data": {
                ...
                "navigation": [{"text": "Submit", "type": "submit", "href": None}],
        ...
        def capture_execution(state):
            ...
            execution_details["messages_added"] = len(result.get("messages", [])) - len(
                state.get("messages", [])
            )
...
def analyze_node_execution(
    result_state: Dict[str, Any], execution_details: Dict[str, Any]
) -> Dict[str, Any]:
    ...
    if "ghostbusters_report" in result_state:
        ...
        if "consultation_id" in report and "gb_consult_" in str(
        ...
        if (
            "investigation_results" in report
            and "overall_confidence" in report["investigation_results"]
        ):
    ...
    if (
        execution_details["execution_time"] < 0.1
    ):  # Very fast execution suggests modular approach
