...
class RefactoredVerificationSystem:
    ...
    def _combine_scenario_results(
        ...
        total_confidence = sum(
            result["confidence"] for result in all_results if result["success"]
        )
        successful_scenarios = sum(1 for result in all_results if result["success"])
        avg_confidence = (
            total_confidence / successful_scenarios if successful_scenarios > 0 else 0.0
        )

        # Determine consensus node type
        node_types = [
            result["node_type"] for result in all_results if result["success"]
        ]
        if node_types:
            # Count occurrences
            type_counts = {}
...
def main():
    ...
    try:
        ...
        if (
            ...
            and result["node_type"] == "refactored_modular"
        ):
            print(
