...
def demo_systematic_vs_adhoc_comparison():
    ...
    print_result(
        "Improvement Factor", f"{systematic_result['improvement_factor']:.1f}x"
    )
    ...
    systematic_metrics.append(
        metrics_engine.collect_metric(
            ...
            "score",
            {"approach": "systematic", "task": "authentication"},
            "systematic",
        )
    )
    systematic_metrics.append(
        metrics_engine.collect_metric(
            "success_rate",
            systematic_result["success_rate"],
            "rate",
            {"approach": "systematic", "task": "authentication"},
            "systematic",
        )
    ...
    print_result(
        "IMPROVEMENT OVER AD-HOC", f"{comparison['improvement_percentage']:.1f}%"
    )
...
def demo_domain_intelligence():
    ...
    for term in search_terms:
        ...
        print_result(
            f"'{term}' domains found",
            f"{search_result.total_count} (in {search_result.query_time_ms:.2f}ms)",
        )
        ...
        if search_result.domains:
            ...
            print(
                f"      🎯 Best match: {best_match.name} (relevance: {relevance:.3f})"
            )
...
def demo_hackathon_preparation():
    ...
    print_result(
        "Differentiation", f"{engagement_analysis['differentiation_highlight']:.3f}"
    )
...
def demo_complete_workflow():
    ...
    systematic_metrics.append(
        metrics_engine.collect_metric(
            "microservice_quality",
            pdca_result["systematic_score"],
            "score",
            {"task": "microservice", "approach": "systematic"},
            "systematic",
        )
    )
    ...
    comparison = metrics_engine.compare_approaches(systematic_metrics, adhoc_metrics)
    print(
        f"      ✅ Systematic vs Ad-hoc: {comparison['improvement_percentage']:.1f}% improvement"
    )
    ...
    print(f"\n   5️⃣ FINAL RESULTS:")
    print_result(
        "Systematic Development Score", f"{pdca_result['systematic_score']:.3f}"
    )
    print_result(
        "Improvement Over Ad-hoc", f"{comparison['improvement_percentage']:.1f}%"
    )
