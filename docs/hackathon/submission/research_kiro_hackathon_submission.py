...
class KiroHackathonResearcher:
    ...
    def print_research_summary(self):
        ...
        # Kiro endpoints
        kiro_count = len(
            [e for e in self.findings.get("kiro_endpoints", []) if e["status"] == 200]
        )
        print(f"\n🎯 Kiro Endpoints:")
        print(
            f"  • Checked {len(self.findings.get('kiro_endpoints', []))} potential endpoints"
        )
        print(f"  • Found {kiro_count} active endpoints")

        # Requirements
        known_reqs = len(
            self.findings.get("requirements", {}).get("known_requirements", {})
        )
        unknown_reqs = len(
            self.findings.get("requirements", {}).get("unknown_requirements", [])
        )
        print(f"\n📋 Requirements:")
