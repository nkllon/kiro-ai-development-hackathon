...
'''
    ...
    def repair_all_tool_health_modules(self) -> Dict[str, Any]:
        ...
        tool_health_files = list(
            Path("src/beast_mode/tool_health").glob(
                "makefile_health_manager_services_part_*.py"
            )
        )
        ...
        return {
            ...
            "success_rate": (
                len(self.repaired_files) / len(tool_health_files)
                if tool_health_files
                else 0
            ),
        }
