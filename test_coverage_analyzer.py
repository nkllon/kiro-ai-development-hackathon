...
class TestCoverageAnalyzer:
    ...
    def analyze_test_structure(self):
        ...
        self.coverage_data["test_structure"] = {
            ...
            "test_file_patterns": {
                ...
                "other": len([f for f in test_files if not (f.endswith("/test_*.py") or f.endswith("_test.py"))])
            }
        }
        
        print(f"📊 Found {len(test_files)} test files across {len(test_directories)} test directories")
    ...
    def analyze_source_structure(self):
        ...
        for src_dir in self.src_dir.rglob("*"):
            if src_dir.is_dir() and not src_dir.name.startswith("__pycache__"):
                source_directories.append(str(src_dir.relative_to(self.repository_root)))
        ...
        print(f"📊 Found {len(source_files)} source files across {len(source_directories)} directories")
    ...
    def analyze_coverage_gaps(self):
        ...
        for source_file in source_files:
            ...
            for test_file in test_files:
                if module_name.split(".")[-1] in test_file or any(part in test_file for part in module_name.split(".")[-2:]):
            ...
            coverage_mapping[source_file] = {
                ...
                "test_coverage_type": self._determine_coverage_type(potential_test_files)
            }
        ...
        domain_coverage = {}
        for domain, files in self.coverage_data["source_structure"]["domain_categories"].items():
            ...
            domain_coverage[domain] = {
                ...
                "uncovered_files": [f for f in files if not coverage_mapping[f]["has_direct_test"]]
            }
        ...
        self.coverage_data["gaps_analysis"] = {
            ...
            "overall_coverage_percentage": len([f for f in source_files if coverage_mapping[f]["has_direct_test"]]) / len(source_files) * 100,
            "critical_gaps": self._identify_critical_gaps(uncovered_files)
        }
        
        print(f"📊 Overall coverage: {self.coverage_data['gaps_analysis']['overall_coverage_percentage']:.1f}%")
    ...
    def _identify_critical_gaps(self, uncovered_files: List[str]) -> List[str]:
        ...
        critical_patterns = [
            "core", "main", "cli", "api", "service", "manager", "controller",
            "engine", "handler", "processor", "validator", "monitor"
        ]
    ...
    def generate_recommendations(self):
        ...
        overall_coverage = self.coverage_data["gaps_analysis"]["overall_coverage_percentage"]
        ...
        if overall_coverage < 50:
            recommendations.append({
                ...
                "description": f"Current coverage is {overall_coverage:.1f}%. Aim for at least 70% coverage.",
            ...
            recommendations.append({
                ...
                "description": f"Current coverage is {overall_coverage:.1f}%. Target 80%+ coverage.",
        ...
        for domain, coverage_data in self.coverage_data["gaps_analysis"]["domain_coverage"].items():
            if coverage_data["coverage_percentage"] < 60:
                recommendations.append({
                    "priority": "HIGH",
                    "category": f"Domain Coverage - {domain}",
                    "title": f"Improve {domain} Test Coverage",
                    "description": f"Only {coverage_data['coverage_percentage']:.1f}% of {domain} files have tests.",
                    "action": f"Create test files for {len(coverage_data['uncovered_files'])} uncovered files",
                    "impact": f"Critical for {domain} module reliability"
                })
        ...
        if critical_gaps:
            recommendations.append({
                ...
                "description": f"Found {len(critical_gaps)} critical files without tests.",
                "action": f"Implement tests for: {', '.join(critical_gaps[:5])}{'...' if len(critical_gaps) > 5 else ''}",
        ...
        if len(test_categories["integration"]) < len(test_categories["unit"]) * 0.3:
            recommendations.append({
                ...
                "description": f"Only {len(test_categories['integration'])} integration tests vs {len(test_categories['unit'])} unit tests.",
        ...
        large_files = size_dist["large (500-1000 lines)"] + size_dist["very_large (>1000 lines)"]
        if large_files > 0:
            recommendations.append({
                "priority": "MEDIUM",
                "category": "Code Quality",
                "title": "Test Large Files",
                "description": f"Found {large_files} large files that may need comprehensive testing.",
    ...
    def generate_action_plan(self):
        ...
        phase1 = [rec for rec in sorted_recommendations if rec["priority"] in ["CRITICAL", "HIGH"]]
        if phase1:
            action_plan.append({
                "phase": 1,
        ...
        print(f"📋 Generated {len(action_plan)}-phase action plan")
    ...
    def generate_summary(self):
        ...
        self.coverage_data["analysis_summary"] = {
            "total_source_files": self.coverage_data["source_structure"]["total_source_files"],
            "total_test_files": self.coverage_data["test_structure"]["total_test_files"],
            "overall_coverage_percentage": self.coverage_data["gaps_analysis"]["overall_coverage_percentage"],
            "critical_gaps_count": len(self.coverage_data["gaps_analysis"]["critical_gaps"]),
    ...
    def run_analysis(self):
        ...
        print("=" * 60)
        print(f"Analysis started at: {self.analysis_start.strftime('%Y-%m-%d %H:%M:%S')}")
        print()
    ...
    def print_summary(self):
        ...
        print(f"⏱️ Analysis Duration: {summary['analysis_duration']:.1f} seconds")
        
        print(f"\n📈 TOP DOMAINS BY FILE COUNT:")
        for domain, data in summary['top_domains']:
            print(f"   {domain}: {data['total_files']} files ({data['coverage_percentage']:.1f}% covered)")
