...
def test_complete_system_integration():
    ...
    print(
        f"      ✅ Found {domain_search.total_count} relevant domains in {domain_search.query_time_ms:.2f}ms"
    )

    if domain_search.domains:
        best_domain = domain_search.domains[0]
        print(
            f"      ✅ Best match: {best_domain.name} (relevance: {domain_search.relevance_scores.get(best_domain.name, 0):.3f})"
        )
    ...
    validation_result = demo_framework.validate_technical_completeness(
    ...
    engagement_analysis = demo_framework.optimize_judge_engagement(
    ...
    print(
        f"         - Technical Balance: {engagement_analysis['technical_balance']:.3f}"
    )

    # Final readiness assessment
    readiness_level = demo_framework.assess_demo_readiness(
...
def test_performance_benchmarks():
    ...
    print(
        f"   ✅ 10 domain queries: {domain_query_time:.2f}ms (avg: {domain_query_time/10:.2f}ms per query)"
    )
    ...
    print(
        f"   ✅ 5 PDCA cycles: {pdca_cycle_time:.2f}ms (avg: {pdca_cycle_time/5:.2f}ms per cycle)"
    )
    ...
    for i in range(20):
        metrics_engine.collect_metric(
            f"benchmark_metric_{i}", i * 0.1, "unit", {}, "systematic"
        )

    metrics_time = (datetime.now() - start_time).total_seconds() * 1000
    print(
        f"   ✅ 20 metrics collected: {metrics_time:.2f}ms (avg: {metrics_time/20:.2f}ms per metric)"
    )
    ...
    print(
        f"   ✅ 3 demo preparations: {demo_time:.2f}ms (avg: {demo_time/3:.2f}ms per demo)"
    )
