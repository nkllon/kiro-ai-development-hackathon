...
Enhanced: 2025-09-14T06:30:15.600069
"""
...
class TestPerformanceBenchmarks:
    ...
    @pytest.mark.asyncio
    async def test_client_startup_performance(
        self, mock_fast_transport, mock_fast_shared_state
    ):
        ...
        with patch(
            ...
            with patch(
                ...
                assert (
                    avg_startup < 0.1
                ), f"Average startup too slow: {avg_startup:.3f}s"
                assert (
                    max_startup < 0.2
                ), f"Maximum startup too slow: {max_startup:.3f}s"
            ...
            with patch(
                ...
                try:
                    ...
                    for count in message_counts:
                        ...
                        messages_per_second = (
                            count / total_time if total_time > 0 else float("inf")
                        )
                        ...
                        ), f"Message sending too slow: {messages_per_second:.1f} msg/s"
        ...
        with patch(
            ...
            with patch(
                ...
                try:
                    ...
                    ), f"Message handling too slow: {messages_per_second:.1f} msg/s"
        ...
        with patch(
            ...
            with patch(
                ...
                try:
                    ...
                    messages_per_second = (
                        total_messages / total_time if total_time > 0 else float("inf")
                    )
                    ...
                    ), f"Concurrent operations too slow: {messages_per_second:.1f} msg/s"
        ...
        with patch(
            ...
            with patch(
                ...
                try:
                    ...
                    print(
                        f"   Growth: {object_growth} objects ({growth_percentage:.1f}%)"
                    )

                    # Memory assertions (should not grow excessively)
                    assert (
                        growth_percentage < 50
                    ), f"Memory growth too high: {growth_percentage:.1f}%"
    ...
    def test_daemon_client_performance_baseline(self):
        ...
        with patch(
            "src.beast_mode.messaging.daemon_client.BeastModeDaemon"
            ...
            return {"startup_time": startup_time, "send_rate": messages_per_second}
    ...
    async def test_performance_comparison(
        ...
        with patch(
            ...
            with patch(
                ...
                print(
                    f"   Daemon Client - Startup: {baseline['startup_time']:.3f}s, Send: {baseline['send_rate']:.1f} msg/s"
                )
                print(
                    f"   Unified Client - Startup: {startup_time:.3f}s, Send: {messages_per_second:.1f} msg/s"
                )
                ...
                send_ratio = (
                    baseline["send_rate"] / messages_per_second
                ...
                assert (
                    startup_ratio < 3.0
                ), f"Startup too slow compared to baseline: {startup_ratio:.2f}x"
                assert (
                    send_ratio < 2.0
                ), f"Sending too slow compared to baseline: {send_ratio:.2f}x"
