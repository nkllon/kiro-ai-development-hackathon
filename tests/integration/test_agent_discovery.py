...
Enhanced: 2025-09-14T06:30:15.545602
"""
...
async def agent_network():
    ...
    def test_rdi_chain_validation(self):
        ...
        rdi_validation = {
            "module": "/Users/lou/kiro-2/kiro-ai-development-hackathon/tests/integration/test_agent_discovery.py",
            "requirements": ["R1"],
            "validation_timestamp": "2025-09-14T06:30:15.545686",
            "chain_integrity": True,
            "traceability_complete": True,
            "test_classes": 2,
            "test_methods": 6,
...
class TestAgentDiscoveryProtocol(ReflectiveModule):
    ...
    async def test_single_agent_discovery(self, agent_network):
        ...
        listen_task = asyncio.create_task(agent2.listen_for_messages(message_callback))
        ...
        discovery_messages = [
            m for m in received_messages if m.type == MessageType.AGENT_DISCOVERY
        ]
        ...
        agent1_discovered = next(
            (a for a in discovered_agents if a.agent_id == agent1.agent_id), None
        )
        assert agent1_discovered is not None
        assert set(agent1.capabilities).issubset(
            set(agent1_discovered.capabilities.capabilities)
        )
    ...
    async def test_multi_agent_discovery(self, agent_network):
        ...
        for agent in other_agents:
            ...
            discoverer_found = any(
                a.agent_id == discoverer.agent_id for a in agent_discovered_agents
            )
    ...
    async def test_capability_matching(self, agent_network):
        ...
        expected_python_agents = [
            agent for agent in other_agents if "python" in agent.capabilities
        ]
        ...
        python_testing_agents = searcher.find_agents_with_all_capabilities(
            ["python", "testing"]
        )
    ...
    async def test_help_request_with_discovery(self, agent_network):
        ...
        if devops_agent:
            ...
            assert (
                devops_agent.stats["messages_sent"] >= 2
            )  # discovery response + help response
    ...
    def test_rdi_chain_validation(self):
        ...
        rdi_validation = {
            "module": "/Users/lou/kiro-2/kiro-ai-development-hackathon/tests/integration/test_agent_discovery.py",
            "requirements": ["R1"],
            "validation_timestamp": "2025-09-14T06:30:15.545757",
            "chain_integrity": True,
            "traceability_complete": True,
            "test_classes": 2,
            "test_methods": 6,
...
class TestAgentDiscoveryEdgeCases(ReflectiveModule):
    ...
    async def test_discovery_with_disabled_registry(self):
        ...
        agent = BeastModeBusClient(agent_id="test_agent", capabilities=["testing"])
    ...
    def get_health_status(self):
        ...
        assert (
            discovered_agent.discovery_count >= 10
        )  # Should have counted all discoveries
