...
Enhanced: 2025-09-14T06:30:15.491200
"""
...
async def bus_client():
    ...
    client = BeastModeBusClient(
        agent_id="test_agent", capabilities=["testing", "python_coding"]
    )
...
async def two_bus_clients():
    ...
    client1 = BeastModeBusClient(
        agent_id="test_agent_1", capabilities=["testing", "python_coding"]
    )

    client2 = BeastModeBusClient(
        agent_id="test_agent_2", capabilities=["testing", "data_analysis"]
    )
...
class TestBusClientConnection:
    ...
    async def test_connection_failure_handling(self):
        ...
        client = BeastModeBusClient(
            redis_url="redis://nonexistent:6379", agent_id="test_failure"
        )
...
class TestMessageSending:
    ...
    async def test_send_without_connection(self):
        ...
        message = BeastModeMessage(
            type=MessageType.SIMPLE_MESSAGE, source="test", payload={"content": "test"}
        )
...
class TestMessageReceiving:
    ...
    async def test_basic_message_receiving(self, two_bus_clients):
        ...
        listen_task = asyncio.create_task(client2.listen_for_messages(message_callback))
    ...
    async def test_agent_discovery_response(self, two_bus_clients):
        ...
        listen_task = asyncio.create_task(client1.listen_for_messages(message_callback))
    ...
    async def test_help_request_response(self, two_bus_clients):
        ...
        await client2.send_help_request(
            required_capabilities=["testing"], description="Need help with tests"
        )
        ...
        help_requests = [
            m for m in received_messages if m.type == MessageType.HELP_WANTED
        ]
