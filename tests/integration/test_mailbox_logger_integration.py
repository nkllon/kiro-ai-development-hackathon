...
Enhanced: 2025-09-14T06:30:15.603428
"""
...
class TestMailboxLoggerIntegration:
    ...
    @pytest.mark.asyncio
    async def test_logger_with_bus_client_simulation(
        self, temp_log_dir, mock_redis_client
    ):
        ...
        async def mock_listen():
            ...
            while logger.is_running and message_count < 50:
                message = BeastModeMessage(
                    ...
                    payload={
                        "content": f"Message {message_count} with some content to make it larger"
                    },
                )
    ...
    @pytest.mark.asyncio
    async def test_error_recovery_during_operation(
        self, temp_log_dir, mock_redis_client
    ):
        ...
        async def mock_listen():
            for data in test_data:
                yield {"type": "message", "channel": "test_channel", "data": data}
                await asyncio.sleep(0.01)
    ...
    async def test_mail_checking_with_large_history(self, temp_log_dir):
        ...
        for file_idx in range(3):
            ...
            for msg_idx in range(100):
                ...
                log_entry = {
                    "timestamp": (
                        datetime.now() - timedelta(hours=file_idx, minutes=msg_idx)
                    ).isoformat(),
                    "channel": "test_channel",
        ...
        assert all(
            msg["message"]["source"] in ["agent_0", "agent_1"] for msg in agent_messages
        )

        # Check with message type filter
        simple_messages = await logger.check_mail(
    ...
    def test_manager_lifecycle_integration(self, temp_log_dir):
        ...
        manager = MailboxLoggerManager(
            log_directory=temp_log_dir, channel="test_channel"
        )
    ...
    @pytest.mark.asyncio
    async def test_concurrent_logging_and_reading(
        self, temp_log_dir, mock_redis_client
    ):
