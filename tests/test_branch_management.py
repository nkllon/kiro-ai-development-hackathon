...
Enhanced: 2025-09-14T06:30:15.614466
"""
...
from src.gitkraken_integration.providers.standard_git_provider import (
    StandardGitProvider,
)
...
class TestBranchManagement:
    ...
    def test_get_branch_details_success(self, mock_exists, mock_run):
        ...
        mock_run.side_effect = [
            ...
            Mock(
                stdout="abc123|abc123d|Add new feature|John Doe|john@example.com|2024-01-15 10:30:00 +0000\n",
                returncode=0,
            ),  # show
    ...
    def test_get_branch_details_not_found(self, mock_exists, mock_run):
        ...
        mock_run.side_effect = [
            ...
            subprocess.CalledProcessError(
                1, "git show-ref", stderr="fatal: branch 'nonexistent' not found"
            ),
        ]
    ...
    def test_rename_branch_success(self, mock_exists, mock_run):
        ...
        mock_run.side_effect = [
            ...
            # rename_branch mocks
            Mock(
                stdout="ref: refs/heads/old-branch\n", returncode=0
    ...
    def test_set_upstream_branch_upstream_not_found(self, mock_exists, mock_run):
        ...
        mock_run.side_effect = [
            ...
            subprocess.CalledProcessError(
                1,
                "git branch",
                stderr="error: the requested upstream branch 'origin/nonexistent' does not exist",
            ),
        ]
    ...
    def test_unset_upstream_branch_no_upstream(self, mock_exists, mock_run):
        ...
        mock_run.side_effect = [
            ...
            subprocess.CalledProcessError(
                1,
                "git branch",
                stderr="error: No upstream branch found for 'feature-branch'",
            ),
        ]
        ...
        assert any(
            "no upstream" in suggestion.lower() for suggestion in result.suggestions
        )
    ...
    def test_compare_branches_ahead(self, mock_exists, mock_run):
        ...
        mock_run.side_effect = [
            ...
            Mock(
                stdout="abc123|Add feature A|John Doe|2024-01-15 10:30:00\ndef456|Add feature B|Jane Doe|2024-01-14 15:20:00\n",
                returncode=0,
            ),  # commits ahead
        ]
    ...
    def test_compare_branches_behind(self, mock_exists, mock_run):
        ...
        mock_run.side_effect = [
            ...
            Mock(stdout="3\n", returncode=0),  # behind count
            Mock(
                stdout="xyz789|Fix bug C|Alice Smith|2024-01-16 09:15:00\n",
                returncode=0,
            ),  # commits behind
        ]
    ...
    def test_compare_branches_diverged(self, mock_exists, mock_run):
        ...
        mock_run.side_effect = [
            ...
            Mock(stdout="1\n", returncode=0),  # behind count
            Mock(
                stdout="abc123|Feature commit|John Doe|2024-01-15 10:30:00\n",
                returncode=0,
            ),  # commits ahead
            Mock(
                stdout="def456|Main commit|Jane Doe|2024-01-14 15:20:00\n", returncode=0
            ),  # commits behind
        ]
    ...
    def test_compare_branches_invalid_branch(self, mock_exists, mock_run):
        ...
        mock_run.side_effect = [
            ...
            subprocess.CalledProcessError(
                128, "git rev-list", stderr="fatal: bad revision 'nonexistent'"
            ),
        ]
