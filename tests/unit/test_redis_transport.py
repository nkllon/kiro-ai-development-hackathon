...
Enhanced: 2025-09-14T06:30:15.593464
"""
...
@pytest.mark.asyncio
async def test_redis_transport_initialization(redis_transport):
    """Test Redis transport initialization"""
    result = await redis_transport.initialize({'redis_url': 'redis://test:6379'})
    assert result is True
    assert 'redis_url' in redis_transport.config


@pytest.mark.asyncio
async def test_send_message(redis_transport):
    """Test sending messages through Redis transport"""
...
@pytest.mark.asyncio
async def test_subscribe_handler(redis_transport):
...
@pytest.mark.asyncio
async def test_start_daemon(redis_transport):
...
def test_get_status(redis_transport):
    ...
    assert 'daemon_connected' in status
...
def test_get_capabilities(redis_transport):
    ...
    assert capabilities['scalability'] == 'moderate'
    assert capabilities['background_processing'] is True


@pytest.mark.asyncio
async def test_message_processing_loop(redis_transport):
...
def test_backward_compatibility_methods(redis_transport):
...
def test_transport_factory_registration():
    ...
    assert transport.agent_id == 'factory_test'


@pytest.mark.asyncio
async def test_error_handling(redis_transport):
