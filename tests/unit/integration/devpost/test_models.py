...
Enhanced: 2025-09-14T06:30:15.449941
"""
...
class TestDevpostProject:
    ...
    def test_tagline_validation(self):
        ...
        with pytest.raises(
            ValidationError, match="Tagline must be 120 characters or less"
        ):
            DevpostProject(
                id="test-123",
                title="Test Project",
    ...
    def test_description_validation(self):
        ...
        with pytest.raises(
            ValidationError, match="Description must be 5000 characters or less"
        ):
            DevpostProject(
                id="test-123",
                title="Test Project",
...
class TestProjectMetadata:
    ...
    def test_url_validation(self):
        ...
        with pytest.raises(ValidationError, match="URLs must start with http"):
            ProjectMetadata(title="Test", repository_url="invalid-url")
    ...
    def test_team_members_validation(self):
        ...
        with pytest.raises(ValidationError, match="Maximum 20 team members allowed"):
            ProjectMetadata(title="Test", team_members=many_members)

        # Too long member name
        long_name = "x" * 101
        with pytest.raises(
            ValidationError, match="Team member name must be 100 characters or less"
        ):
            ProjectMetadata(title="Test", team_members=[long_name])
    ...
    def test_serialization(self):
        ...
        metadata = ProjectMetadata(
            title="Test Project", description="Test description", tags=["python", "ai"]
        )
...
class TestSyncOperation:
    ...
    def test_retry_validation(self):
        ...
        with pytest.raises(
            ValidationError, match="Retry count cannot exceed max retries"
        ):
            SyncOperation(
                operation_type=SyncOperationType.METADATA_UPDATE,
                target_field="title",
...
class TestFileChangeEvent:
    ...
    def test_valid_file_change_event(self):
        ...
        event = FileChangeEvent(
            file_path=Path("README.md"), change_type=ChangeType.MODIFIED, file_size=1024
        )
    ...
    def test_file_size_validation(self):
        ...
        with pytest.raises(ValidationError, match="File size cannot be negative"):
            FileChangeEvent(
                file_path=Path("test.txt"), change_type=ChangeType.CREATED, file_size=-1
            )
    ...
    def test_deleted_file_validation(self):
        ...
        with pytest.raises(
            ValidationError, match="Deleted files should not have file size"
        ):
            FileChangeEvent(
    ...
    def test_file_type_detection(self):
        ...
        media_event = FileChangeEvent(
            file_path=Path("screenshot.png"), change_type=ChangeType.CREATED
        )
        ...
        doc_event = FileChangeEvent(
            file_path=Path("README.md"), change_type=ChangeType.MODIFIED
        )
        ...
        regular_event = FileChangeEvent(
            file_path=Path("main.py"), change_type=ChangeType.CREATED
        )
    ...
    def test_sync_trigger_detection(self):
        ...
        code_event = FileChangeEvent(
            file_path=Path("main.py"), change_type=ChangeType.MODIFIED
        )
...
class TestDevpostConfig:
    ...
    def test_valid_config_creation(self):
        ...
        config = DevpostConfig(project_id="proj-123", hackathon_id="hack-456")
    ...
    def test_sync_interval_validation(self):
        ...
        # Too small interval
        with pytest.raises(
            ValidationError, match="Sync interval must be at least 60 seconds"
        ):
            DevpostConfig(
                project_id="proj-123", hackathon_id="hack-456", sync_interval=30
            )

        # Negative interval
        with pytest.raises(ValidationError, match="Sync interval must be positive"):
            DevpostConfig(
                project_id="proj-123", hackathon_id="hack-456", sync_interval=-1
            )
    ...
    def test_watch_patterns_validation(self):
        ...
        # Empty patterns list
        with pytest.raises(
            ValidationError, match="At least one watch pattern is required"
        ):
            DevpostConfig(
                project_id="proj-123", hackathon_id="hack-456", watch_patterns=[]
            )
    ...
    def test_configuration_validation(self):
        ...
        # Missing project_id
        config = DevpostConfig(project_id="", hackathon_id="hack-456")
        issues = config.validate_configuration()
        assert "project_id is required" in issues

        # Missing hackathon_id
        config = DevpostConfig(project_id="proj-123", hackathon_id="")
        issues = config.validate_configuration()
...
class TestModelIntegration:
    ...
    def test_project_metadata_to_devpost_project(self):
        ...
        team_members = [
            TeamMember(username=name, display_name=name)
            for name in metadata.team_members
        ]
