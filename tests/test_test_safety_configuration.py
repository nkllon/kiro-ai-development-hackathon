...
Enhanced: 2025-09-14T06:30:15.547457
"""
...
class TestTestSafetyConfiguration:
    ...
    def test_workflow_safety_validation_production_mode(self):
        ...
        assert not config.validate_workflow_safety(
            "write_workflow", {"read_only": False}
        )

        # Workflows exceeding resource limits should be blocked
        assert not config.validate_workflow_safety(
            "heavy_workflow",
            {"read_only": True, "max_memory_mb": 2000},  # Exceeds production limit
        )
