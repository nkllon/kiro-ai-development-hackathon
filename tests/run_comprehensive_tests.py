...
class TestRunner(ReflectiveModule):
    ...
    def run_linting(self) -> Dict[str, Any]:
        ...
        linting_results = {
            ...
            "success": all(
                r.returncode == 0 for r in [flake8_result, mypy_result, black_result]
            ),
        }
    ...
    def print_summary(self):
        ...
        if self.coverage_data:
            ...
            if total_coverage < self.config.coverage_threshold:
                print(
                    f"⚠️  Coverage below threshold: {total_coverage:.1f}% < {self.config.coverage_threshold}%"
                )
...
def main():
    ...
    parser.add_argument(
        "--coverage-threshold", type=float, default=90.0, help="Coverage threshold"
    )
    parser.add_argument(
        "--parallel", action="store_true", help="Enable parallel execution"
    )
    parser.add_argument(
        "--no-cleanup", action="store_true", help="Don't cleanup temp files"
    )
