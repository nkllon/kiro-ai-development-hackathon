...
def test_metrics_engine():
    ...
    systematic_metrics.append(metrics_engine.collect_metric(
        "development_speed", 8.5, "tasks/hour", 
        {"domain": "authentication", "complexity": "medium"}, "systematic"
    ))
    systematic_metrics.append(metrics_engine.collect_metric(
        "quality_score", 0.92, "score", 
        {"domain": "database", "validation": "passed"}, "systematic"
    ))
    systematic_metrics.append(metrics_engine.collect_metric(
        "bug_rate", 0.05, "bugs/task", 
        {"domain": "api", "testing": "comprehensive"}, "systematic"
    ))
    ...
    adhoc_metrics = []
    adhoc_metrics.append(MetricsData(
...
def test_pdca_metrics_integration():
    ...
    print(f"   Systematic vs Ad-hoc Improvement: {comparison['improvement_percentage']:.1f}%")
...
def test_multiple_cycle_metrics():
    ...
    test_tasks = [
        PDCATask(task_id="multi_001", name="Task 1", description="First task", domain="domain1"),
        PDCATask(task_id="multi_002", name="Task 2", description="Second task", domain="domain2"),
        PDCATask(task_id="multi_003", name="Task 3", description="Third task", domain="domain3")
    ]
    ...
    for i, task in enumerate(test_tasks, 1):
        ...
        # Collect metrics
        systematic_metrics.append(metrics_engine.collect_metric(
            "cycle_systematic_score", result['systematic_score'], "score",
            {"cycle": i, "task_id": task.task_id, "domain": task.domain}, "systematic"
        ))
        
        print(f"   ✅ Systematic Score: {result['systematic_score']:.3f}")
    
    # Calculate average systematic performance
    avg_systematic_score = sum(m.value for m in systematic_metrics) / len(systematic_metrics)
